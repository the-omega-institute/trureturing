...
**反例与开放违例**:`Evidence/D5/values.json` 的「producer 读单个 toml」不能推出「输入不分区」。当前样本含 14 条记录；分别仅修改 `D5/Bh`、`D5/T1` 的非空 `open_reason`,保持其它记录及 Lean 绑定字段不变,均通过实际 loader 与当前 Lean 报告的绑定校验,各自只改变对应投影记录,但 `ValuesEmitter` 均写入同一 tracked 路径且全目录输入摘要随之改变。这给出两个独立合法变更共用写入点的实际反例,不只依赖记录数,也不声称已遍历全部合法变化。**开放违例(D5-T0031),非合规**；spec A2 的 values 单文件规范地址仍在,解决分区违例须同时迁移该地址契约、producer、FILEMAP 和消费者,只拆输出或复制全局摘要不能结案。
