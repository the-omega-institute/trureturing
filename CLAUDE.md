...
**数学坐标的条件**:选定等价关系 `~` 后,`x~y ⟹ f(x)=f(y)` 只保证 f 在等价类上有定义;要由 f 区分不同等价类,还须反向蕴含。`quotient_invariant_coordinate_injective` 在两向条件下给出商集上的单射坐标;常值函数也是不变量,却不区分非平凡的等价类。此为坐标比喻的数学核心,具体路径与符号身份仍按上述现役规则核验。
...
- **节点/边**:命题或工件(closed/open/tail/semantic 四态账目)/证明依赖;每个有限快照的依赖图无环,故良基、可归纳分层。无限图须另证良基性,无环本身不够。
...
- **两个偏序禁混用**:有限无环快照的图深度 `depth(v)` 是沿原依赖边到达该节点的最长路径长度(无前置=0,否则 `1+max depth(前置)`),非任意拓扑序号或入度;无限良基图仍可有无限高度,不能无条件套用自然数深度和有限最大值。信任地层 τ 是 harness 保护面的变更治理分层(信任根 τ=0,内容 τ_max,第 7.12 条);它与图深度分别定义,上述有限图深度可计算不等于 τ 机制已经落地。
- **成本**:在依赖边固定、保留节点必须对前置依赖闭合的撤销模型中,最小撤销集合是含源节点的后继闭包(`consequenceClosure_least`);共同后代按集合并集去重。对有限受影响集合,在固定逐节点成本的可加模型中,`Cost ∝ |受影响子图|` 是每节点等成本的特例;一般按各节点的资源向量求和,逐坐标权重非负时,受影响集合扩大保证各坐标成本不减。后代集合按依赖关系求取,真实运行成本仍须测量;第 7.12 条的 τ 计价另有模型与前提。
...
>
> **我们是在寻找因陀罗网的珍珠**:证明节点以全部前置哈希内容寻址,绑定其可达前置。在同一依赖图中,若各前置摘要完整进入节点载荷、比较的载荷对无哈希碰撞,一节点的摘要变化才沿依赖路径传至后继。有限摘要空间不能在无限对象域上全局单射;该传播条件不由“内容寻址”之名自动成立。“一即一切”对应冻结/保守扩展的可判结构,C0 机器为 inactive;内化塔层层判前层,顶上 Gödel 标 open。
...
**对错,机器判;真 ⊋ 证,诚实标 open。** 正确性由形式验证(`lake build`、Lean kernel 无 sorry 无私 axiom)与多模型对抗(sshx 三席+异模型)检验忠实性、空洞性、矛盾;自信也须交独立机器,不得自证。给定形式系统内确实不可证亦不可反证的命题,使用同一公理与推理规则的人和机器均不能在该系统内判定它,应标 `open`,不是等人裁决。人类门只授权不可逆动作、选择价值/方向,不替机器判对错(第 5.9 条)。
**Gödel 的明确条件**:Foundation 的[第二不完备定理](https://github.com/FormalizedFormalLogic/Foundation/blob/30a16ffa93d79d73ab4d02427fa00f50e039bf29/Foundation/FirstOrder/Incompleteness/Second.lean)中,`consistent_unprovable` 针对算术理论 T,要求公理集具有该库规定的 Δ₁ 表示、T 扩展 IΣ₁ 且 T 一致,结论是 T 不能证明其标准可证性编码下的一致性句。`inconsistent_independent` 对该句的双向独立性另要求 Σ₁ 健全性;一致性句不可证与该句及其否定均不可证不能混称。本文的 Gödel 边界均相对于指定理论和编码;改变公理会改变可证性问题,不能外推为所有更强理论或任意判官均不能判。该外部结果未作为本仓依赖或移植准入,实际 harness 到上述理论、表示与前提的对应仍未验证。
**“真 ⊋ 证”的集合条件**:固定陈述域、语义真集与可证集后,严格包含恰要求“每个可证命题为真”及“存在真而不可证的命题”两项。前项是健全性,后项提供严格性见证;单独的不可证性结果不能替代健全性或确定语义真值。
**可计算性边界**:在全部部分递归程序代码上,若性质只依赖所计算的部分函数且同时有满足和不满足它的代码,`closure_reading_unreachable` 由 Rice 定理排除总可计算的精确判定程序。这是全体代码上的统一判定限制,不把某次未找到证明判为不可证,也不替代 Gödel 定理的算术前提。现役门在声明的输入域与判据内作可计算检查,其结果的语义保证仍以已验证的范围为限。
...
- **查表复制器**(`lookup_copy_zero_loss_and_nonanticipating_failure`):有限样本上,自比较损失为零且承诺依赖全部复制记录时,查表复制器回顾损失为零、每条复制记录均不满足这里定义的“已在承诺形成前冻结且不属于承诺证据依赖闭包”的非预见条件,即使该记录此前已冻结也一样;回顾全对不能保证任意给定的未来收益函数都为正,也不推出实际未来收益为零。只复述已见数据、能解释一切的解释按零信息处理是外推纪律,不是未来收益的测量结论。
- **盲核/电荷分解**(`blind_residual_charge_decomposition`):在可数词汇表、正候选成本、非负预算、正基线电荷及定理要求的集合环归属和非负有限可加电荷条件下,任意有限选择后的残差仍包含共同盲核,其电荷等于盲核电荷加剩余可消去部分的电荷。因此仅用该词汇表内更多有限选择不能切开盲核;换语言/框架须另证新观察确能区分目标,并非换名即可。
- **预算包络**(`budget_envelope_infimum_and_limit`):在非负预算、非负且单调的质量及正基线质量这些定理条件下,预算包络非增并趋于所有有限选择残差质量的下确界。若残差质量分解为盲核质量加非负可消去质量,该下确界等于盲核质量当且仅当有限选择能使可消去质量任意小;有限可加性本身不保证这一点。`submodular_capture` 在有限可加质量等前提下另证同一新增定义相对嵌套有限选择的边际捕获递减,不由预算包络非增推出。连续投入无边际改进即换 Γ(表述/工具/层级),不继续 grind,同于盲修两轮重估 worth;这是外推纪律,不是上述极限定理的结论。
- **追加目标不改旧结算**(`append_only_old_settlement_unchanged`):固定纯求值器时,向记录表尾部追加记录不改变原表有效索引处的结算;改变求值器不在该保持结论内。修假设/目标/规则须明列旧结论哪些仍立、哪些作废,禁静默重算历史。
- **Pareto 弱支配与增益 cocycle**(`pareto_weak_reflexive_transitive`/`gain_difference_self_zero_and_cocycle`):方案逐坐标弱支配比较,不拍单标量;各坐标为预序时,弱支配自反且传递,但不保证任意两个方案可比。对同一个固定的五维绝对值映射,各坐标为加法群时,逐坐标差分满足自差为零及三点 cocycle 恒等式。叙事之间对不上账时先核对所用映射、坐标与端点;该恒等式不将任意转移收益或实际路径成本自动变为端点差分。
- **准入反单调**(`dependency_closure_admission_antitone`):固定准入上下文时,依赖闭包按集合包含扩大,可准入裁决者集合只会缩小或不变;不保证严格缩小,改变上下文也不在该定理内。把陈旧读数、利益相关来源、单一先验视为污染并降低自信、引独立席或标 `ASSUMED-UNVERIFIED` 是外推纪律,不得因急放宽。
- **局部结算**(`spectrum_commitment_local_settlement`):该定理针对五项记录,将 `.open` 端化为 `.invalid`,并以 proved/refuted 项数至少三项为成功、少于三项为失败;它验证纯结算函数,不验证时钟、证据真实性或任意任务的终止。开工即定截止与判据、到期结算是执行义务,open 不得以“进行中”拖延;卡死单按第 5.10 条重开。
...
**可逆与有效实现的形式边界**:令 H(c) 表示程序代码 c 在固定输入 0 上停机,定义 F(c,b)=(c,若 H(c) 则翻转布尔位 b,否则保留 b)。F 的两次复合是恒等映射,但不存在实现它的统一全可计算算法,其完整输入输出关系也不可半判定；否则分别可判定停机性,或通过 F(c,false)=(c,false) 半判定不停机性。这只排除上述统一算法与关系半判定,不否定个别实例的证明、受限域算法或额外 oracle。此处可计算性取 Mathlib 的递归函数语义；仓内 `IdentifiabilityEstimabilityComputation.Computable` 则是指定算法正确且不超给定成本预算的谓词,两者不能混用。
**成本迁移的条件**:`transformation_description_complexity_le` 的描述成本上界依赖给定的应用编译器、实现关系保持及固定开销界,并计入变换本身的描述成本。在 `BinaryDescriptionMachine(c)` 的全部接口条件下,若 `2 * Nat.log 2 (l+1) + c < l`,则 `xor_transformation_description_tight` 给出一个 XOR 对合,把长度 l 的零串送到最短二进制描述长度严格更大的串。这里比较的是该描述机中的最短代码长度,不等于执行时间；并未由此构造实际工程的描述机或证明其运行成本。迁移所需成本界仍须按实际表示、算法和资源模型核对。
...
**局部零点的现有结果**:上述缺口读数只指 2026-09-05 窗口。现有 `GermZeroCertificate` 的 `g60_center_norm_lt` 与 `g60_center_deriv_re_gt` 已分别证明 `‖g 60 c‖ < 4/10^10` 和 `187/100 < Re((g 60)′(c))`;结合现有曲率界,`germLocalFactor_two_has_zero_near_candidate` 给出 `p=2` 黄金局部因子在该模块族所定义中心 c 的 `10⁻⁸` 开球内有零点。该结论的两条中心界由证书履行,不再是待给定假设;结论不涉及 RH。
**仍有用的区别**:`GapEndpoints` 的历史注释报告两枚举器在周期九相差 3 词,并将其对应到 gap 边界的三点环。现有 `gap_endpoints_form_a_three_cycle` 证明三条端点转移及三个坐标等于各自 gap 长度;它不陈述两枚举输出的差集等式。历史实数过滤器的实现、端点接纳规则和差异方向尚未绑定到形式陈述,三词差集对应仍未验证。保留已证端点结果及有用探针,无需据此保留整塔。既有 `clause_depth_sixty_claim_is_false` 是一般定理在 60 的实例化,仍判 bind-only;新同形 atom 走消化,反驳用途不豁免包装禁令。
...
**地址由算法算出,不开会。** GID 是规范地址(F 层为字面路径),地层由 import 偏序算;桶满只裂不迁,历史只追加。不同规范化身份的并行贡献不撞址,要求定址函数在该合法身份域上单射;解码后能还原身份是充分条件,确定性本身不保证单射。同一身份仍落同一地址,对该地址写入不同内容不会因算法定址自动相容;在直接覆盖的状态模型中,同址两次赋值可交换当且仅当值相等。不同地址的赋值可交换也不保证合并后的全局约束,并集与增量保持条件另见第 7.7、9.7 条。
...
**投影按真源变更分区分片(merge unit ≤ mutation unit)**:聚合独立写集合会制造共同写入点;tracked 投影路径分区不得粗于输入变更分区。局部性要求 `改一个输入单元 ⟹ 其它键对应的投影不变`;本条还要求非平凡合法单键变更确实改变该键对应的一个投影文件,两者合起来才是 `恰好改一个对应文件`。
**分片要求**:①**禁止提交跨分区聚合物**——全局计数、跨分区 join、manifest、index、目录 digest 会把多个分区耦合到共同路径;它们并非每次输入变化都变,例如只改已有记录的值不改变记录总数,该事实不豁免跨分区聚合物;②这类聚合一律**按需现算到 stdout,不落盘**(它们是查询,不是工件);③**每片只依赖它自己那个分区的输入**,片内不得写入依赖其他分区的字段,否则耦合只是从路径退回内容;④分片的**内容地址必须绑定其分区键**,使一片的字节不能被重放到另一片路径下;此保证须由实际校验的键绑定承担,仅把键拼入一个未证无碰撞的摘要函数不构成数学单射证明;⑤**分片键由 producer 从输入键派生,不得手写清单**——否则新增一个分区就要改 harness(第 4.9 条商余结构之病)。
...
**只有**当键提取结果恒为一项,或存在**机器校验的跨键原子约束**使任何合法变更都必须整体更新时,单文件投影才合规。**机器判据**:producer 须暴露确定性的 mutation-key 提取;对每个能独立通过权威校验的**非平凡单键变更夹具**,重放 producer 后 **changed tracked projection paths 必须恰为该键对应的单文件集合**。只查基数为 1 不证明键对齐;有限夹具通过也不证明覆盖全部合法变更。
**从单步到全局的条件**:须先证明每片输出对全部其它键的合法单键变化不变,且同一保留键取值的任意两个合法状态可由不改变该键的合法单键步骤连接,才能推出该片只依赖本键。无约束的有限键乘积满足此连通性;无限键乘积只在两状态的差异键集有限时保证有限步可达,有跨键约束的合法状态集合须另证。即使局部性成立,要求非平凡合法本键变化必改输出,仍须区分每一对可由合法单键步骤互换的本键取值;在全部合法本键取值上单射是充分条件,但在有跨键约束时不能作为一般必要条件。
**还须查第二层**:producer 的 attestation/输入哈希若覆盖**整个**输入闭包且写入每片,则该全局摘要一旦改变就会重写全部输出——多个独立分区因此共同变化是**现状违律的证据**,也意味着分片时必须把 attestation 改为每片只绑该键的最小依赖闭包,否则**名拆实聚**。
**不得**把具体仓库路径写进本条当作永久豁免:成员数会随数据增长,「恒为一项」不能由某次样本永久担保——**举例只用于说明判据,不构成对某文件的赦免**。
**values 分区实测(D5-T0031)**:当前 `ValuesEmitter` 从 `ValueDefinition.Id` 派生 `Evidence/D5/values/<encoded-id>.value.json`，`encoded-id` 为严格 UTF-8 字节的小写十六进制加 `k`，键限 120 UTF-8 字节；不正规化 Unicode，解码往返及自产路径/payload/GID 绑定均实际校验，不以摘要充当单射证明。每片包含该行完整规范化输入（含计算未使用的已接受参数），不带整个 catalog 或 Lean 模块摘要、全局 provenance。spec A2/A8、route、FILEMAP 单 glob、inventory、snapshot digest 和当前消化引用已迁移，聚合地址不保留。在复制的当前 14 行输入中，分别修改 `D5/Bh`、`D5/T1` 的非空 `open_reason` 或 `D5/Ah` 的 `error`，均经实际 loader 和当前 Values 报告绑定校验接受，重发射后完整字节各恰改变对应一片；其中 `D5/Ah` 的求值结果不变，输入变化仍保留在本片。合成夹具另查键增删、未使用参数、Unicode/斜杠/点区分与错误路径拒绝。该读数验证这些夹具，不声称遍历全部合法变化；数值仍非 Lean kernel 求值，`--check` 只判 producer 确定性，不判 tracked 快照新鲜度。
...
**商集判据**:给定等价关系 `~`,原判词能下降到 `X/~` 当且仅当 `x~y` 总蕴含两者判词相同。这个数学条件刻画语义不变性;它不证明实现没有手写实例表、等价关系选得正确或成员规则已接线,这些仍须按本条的实现纪律核验。
...
**模型多样性 > 单模型自证。** 为减少共享盲区,能引入不同模型族的独立视角就引入;仅单模型则如实声明,不冒充多样性共识。
**形式模型与正确性边界**:异模型优先是评审纪律,模型族标签不同不自动证明统计独立或准确率提高。`IndependentConvergenceDiscernment` 中的 `Independent` 只要求可见输入集不交且模型族不同;这些条件容许读出完全相同、在同一输入上共同答错,也容许共同随机来源造成联合律不等于边缘律乘积。该模型的 `same_family_same_input_discernment_zero` 只结算其定义的计分分支,不表示同族输出对任意目标均无新增信息。
**确定性恢复的明确前提**:`deterministic_recovery_exact_threshold` 在 n 个布尔报告针对同一个真值、实际错误数至多 f 的模型中,证明存在对所有允许报告均正确的确定性恢复规则当且仅当 n>2f。模型族、席位数或一致投票本身不提供该错误上界;实际评审的误差与联合律仍须另验,不能从流程独立推出。
...
**收缩的数学范围**:在固定的债务身份与判据下,集合包含排除新增债务,而计数不增允许旧债换新债。沿债务非空时能继续完成修复的迁移序列,若初始债务集有限、每步不新增且非空时严格删减,`finite_defect_repairs_terminate` 给出至多初始债务数的清零步数界;可将任意身份域限制到有限初始债务集后应用。这里的“步”是满足上述条件的迁移事件,不提供停滞期间的日历时间界;无限初始集合也可能永远严格收缩而不为空。`D_base=∅` 时,同一包含条件已等价于候选没有违例;这项逻辑事实不证明门已覆盖全部输入或删除门必被阻断,五项机器义务仍须分别核验。
...
**分区可行性边界**:给定本批工件的依赖图与固定判词,仅就“每批同判”与“依赖连通分量不跨 PR”两项而言,可同时满足当且仅当判词在每个连通分量内恒定,此时按连通分量分区即足够。这里的连通分量取依赖图忽略方向后的弱连通分量;一条单向依赖也不能跨批,只看强连通分量不够。若同一分量含红绿两种判词,重新分组不能同时满足两项,不能把切断依赖后的同判分组称为已验证。该判据不证明实际依赖图完整、判词读数有效、重分区后的判词保持或各批满足其余准入条件。
...
**系统在动,先立门后补账**:建立“全体 X 满足 P”时,先约束增量影响面(新增、改动及依赖受影响的对象)满足 P,再补其余存量,否则两步间会漏新违例、永追移动靶。门先作用增量影响面,对未受影响存量盲,避免立门全树红反逼先补。
**不变量的数学条件**:固定对象身份与性质 P,若本次新增或改动的对象满足 P,且其余保留对象的判词不变,则候选违例集包含于旧违例集。允许修改旧对象时,只检查新身份不能保证该结论。性质依赖其它对象时,可依第 9.7 条的两状态局部性,重查新增、改动及依赖受影响的对象,以覆盖新旧实际读取的依赖集合证明其余判词保持;规则自身改变也不能仅凭对象字节未变沿用旧判词。这些条件解释新增违例如何被阻止,不要求扫描不受影响的存量。
〔守护:**软+硬投影**·新增与改动域可由 changed-path 交集判;依赖影响域须有第 9.7 条的覆盖依据,先后顺序靠评审〕
...
**保守性的模型桥梁**:在语义模型层,`old_language_validity_conservative` 要求扩展模型的投影保持可容许性,并且每个可容许旧模型都有可容许扩展原像;在这两项条件下,任意旧谓词与其拉回在各自模型域上的有效性等价。只保持投影的可容许性不足以反映旧有效性。实际 harness 到这些模型、投影与覆盖条件的对应仍未验证;语义有效性也不能在缺少对应健全性与完备性依据时直接换成形式可证性。
...
**替代检测/缓解**:①准入容量只判本次碰过目录,准入与全仓容忍阈值以 `RepositoryRules` 的 `DirectoryFileLimit` / `DirectoryToleranceLimit` 为准,当前为 96/192。抽象并集模型中,两个候选的计数成员集各不超过 L 时,其并集不超过 2L;这不涵盖已超准入阈值的存量例外、额外冲突解法或任意多个并发候选。一般地,在保留共同基线成员的追加模型中,一笔候选尚有 r 格余量且另一笔相对共同基线至多新增 r 个成员,才有相应并集界;仅留 1 格不能保证任意两个批量新增合并后不超限。②全仓不变量留检测层,架构测试超限即红并裂桶(第 4.8 条)。③共享聚合物按第 4.5 条分片。④PR merge-tree CI 判 check-time union 错误,M1→M2 残余由 dev push CI 检测并快速勘正(第 7.8 条)。内容寻址的独立定理/前置、逐定理叙事及路径 GID 不因粗路径分类成为真交互;全仓目录计数/共享汇总才有聚合交互。
...
- **`C(τ)=C_leaf·α^(τ_max−τ)` 的有限层模型**:取实数 C_leaf>0、α>1 及自然数地层 0≤τ≤τ_max,则内容层成本为 C_leaf,每向核心一层×α,信任根成本为 C_leaf·α^τ_max,并在该范围内严格向核心递增。若令 C_leaf=0,全部层成本均为零,不能保留严格梯度;“内容≈免费”须理解为较小的正叶层成本。该曲线是设定的治理成本模型,实际耗时与资源读数须另测。三种机器可算货币为验证宽度 W(独立席/异模型/人审数)、证明深度 D(lint→类型→Lean→保守扩展证明)、保守扩展义务 E。
- **承重保守扩展律**:`∀a, τ(a)≥τ: H(a)=admit ⟹ H′(a)=admit`,新 harness 不推翻该范围内的旧 admit。新坏输入的拒绝与既有检测不得降级,仍按第 7.8 条承担独立义务。范围越核心越大,核心改动须证不破坏整塔。
- **蕴含方向的边界**:该式只保持旧 admit,不推出旧 reject 不变或新 admit 都正确;这两项不能从一个方向的集合包含冒领。若要求旧域的二值准入判定完全不变,须在同一范围上同时证明反向蕴含;若以旧语言的可证性定义逻辑保守扩展,也须明确语言嵌入并核验双向可证性。旧 admit 保持可以逐次复合,新接纳项的正确性仍须另证,不由升级链长度产生。
- **递归**:改 τ 的保守性由更核心 τ−1 harness 判,至 τ=0 人类可信 bootstrap 注入。Gödel 第二不完备的自洽性限制针对满足其算术表达、有效可公理化及一致性等前提的形式理论和相应可证性谓词;有限无环判断图有根不证明这些前提,实际 harness 到该理论的对应仍未验证。塔顶成本不可判/∞ 是本款待实现的治理模型,不由无环性或该不完备定理直接推出。塔无封顶;dev-baseline 判 candidate 只是 inactive 旧例,现役不执行 base 代码。
...
**自锁与真红**:门对 no-op/revert 也 fail,只证明这些候选被拒,不单独证明全部修复均被阻断、门没有区分能力或越过它不降级。门可以误拒这两类合法候选,同时放行另一合法修复并拦住坏候选;直接绕过仍会丢失后者的检测。`deadlocked_iff_empty_joint_allowance` 将修复自锁刻画为指定修复类与各门共同允许集合的交集为空,实际修复域和准入关系须另行核验。误拒、真红及绕过后新增放行项的正确性分别核对,不得以局部误拒读数代替第 7.8 条的检测保持义务。旧“仅 base-owned no-op 自锁证明后才允许候选参与+base 回归背书的 admin”条件逃生舱亦 inactive;现役撤因例外仅依第 7.14 条。
...
**逆运算的数学边界**:设原变更为 U、后续操作为 G、撤回为 R,仅有 `R(U(s))=s` 不推出 `R(G(U(s)))=G(s)`;后式可由左逆与 R、G 的交换律共同保证。即使后式成立,恢复所需性质 P 仍须在 `G(s)` 上成立。以上限定“时光倒流”的数学解释,不是给现役四条件增加第五道门;实际因果归因与恢复读数仍按本款核验。
...
**hang guard/TRX**:`TestProcessRunner`/watchdog 行为测试把已路由子进程 guard 钉为 SkipException;`make -C tools test` 与 candidate engineering executor 经 `TestResultEvidence.Load` 读 TRX,reason 以 `infrastructure-hang-guard expired` 开头即独立 INFRASTRUCTURE_UNRESOLVED、非零退。同轮业务失败与 guard 可同时出现,也可分布于不同 TRX;读取器在成功性校验前收集 guard,拒绝时同时保留业务失败和校验诊断。在登记目标解析成功时,测试脚本即使原命令非零也继续读取 TRX;原测试与证据验证均已返回时,脚本取原测试的非零码,否则取证据验证码。该合成值为零当且仅当两者均为零,不由此推出测试覆盖完备或报告真实;登记解析等提前失败仍按其自身错误非零退出。base engineering execution floor/独立 TRX 读者已由 #5319 退役。
...
**检测放本次变更的依赖闭包,不全仓重算;漏项就补闭包(含反向依赖),禁全量兜底。** 归纳保持须有明确前提:初态一致;被保持的性质对工件本身及实际读取依赖具有两状态局部性;规划闭包覆盖变更前、后两套读取依赖;所有本身或依赖受影响的工件重验通过。`two_state_locality_yields_incremental_preservation` 在这些条件下支持未受影响工件的性质保持,再对有限步骤归纳。闭包遗漏新依赖或性质含闭包外读取时不能套用此结论;实际 planner 满足这些前提仍须核验,不由“增量”之名保证。
**前后依赖图的受影响判定**:固定变更节点集 C,若两个有向依赖图在每个 C 外节点的所有出边上相同,则“存在到 C 中某节点的有限路径”的起点集合在两图中相同,零步路径计入。路径首次进入 C 之前只用未变节点的出边,进入后无须继续；因此仅为此判定可用任一图,不表示两套完整依赖闭包相等。实际实现仍须证明变更标记完整、未标记节点出边稳定、图包含全部实际读取；漏标允许这些出边变化时,只读当前图可漏掉旧图中的受影响起点。这不豁免前述两状态局部性及实际读取覆盖义务。
...
**三类已实测的病因与处方(2026-09-10,`CrimGrundyRefutation`,同一模块同一台机)**:①**一次性 `decide +kernel` 判一个大集合** —— 若不共享的中间项同时驻留,模型峰值为公共基线加各项驻留量之和;若逐项执行、项间完全释放且无额外峰值,则为基线加最大项。该模型不自动证明内核满足这些条件;本样本先 `reduce`、`simp only` 拆合取、`repeat' apply And.intro` 分解,再 `all_goals decide +kernel`,实测峰值变化见下表。②**证书用线性表** —— 从表头查找 n 个位置各一次、每次比较单位成本时,总比较次数为 `n(n+1)/2`(样本 n=377),为二次增长。换成平衡树后,若每次查找只访问一条根到叶路径、路径节点数为 `O(log(n+1))`,且每节点操作成本由与 n 及查询无关的同一常数界定,单次查找为 `O(log(n+1))`,n 次总量为 `O(n log(n+1))`;这不是全部查找总量降为对数级,也不直接给出内核墙钟成本。③**伞形 `import Mathlib.Tactic`** —— 载入整个 tactic 库的环境;处方是精确导入,并把无限制的 `simp` 换成 `simp only [具名引理]` 配 `omega`。配置比较:
...
- **cache key 恰好覆盖能改变被复用值的输入,不多不少;目标不是完全命中,是不假失效**——承上条:「一事不再理」要求按内容地址记忆化,而**地址取错范围,两个方向都出病**。key **少了**能改变值的输入 ⟹ **假命中**(复用了不该复用的东西;#1201 判据管这一侧:「缓存 key 必须覆盖所有能改变被复用值或其接受语义的输入,或复用后存在独立等价重证」);key **多了**不改变值的输入 ⟹ **假失效**(命中率被一个零信息的字节归零),**本条管这一侧,二者是同一条判据的两端**。
  **判据(逐条可查)**:①删除 key 中某项输入,须证明在**全部合法上下文与替换值**下,保留字段相同就有被复用值及其接受语义相同;一次改变而产物不变不够,例如 `a AND b` 在 `b=false` 时不依赖 a,在 `b=true` 时却依赖 a。精确复用要求 `key(x)=key(y) ⟹ result(x)=result(y)` 对全部合法输入成立;若还要求无假失效,才另需反向蕴含。②**不追求完全命中**——下游若本身是增量构建系统(lake / Bazel / dotnet),cache 可提供近似起点,前提是权威增量机制在使用前重新核验全部影响正确性的输入并补编差量;把整个配置文件哈希进 key 会让无关字段变化也触发失效,不能据此省略真正相关的输入核验。③**扩大可复用起点范围 + 差量补编**的正确性依赖上述权威核验,效率收益须实测,不由 key 宽窄单独推出。④凡 key 的某项输入是**一个文件的全文**,核对哪些字段在全部合法上下文中影响产物,只删已证无关的部分。**cache 不是真源(第 4.4 条)**;同 key 下存在不同结果且没有独立重验会造成假命中,同结果被不同 key 分开会造成假失效,两者分别核验。
...
**信任的对象与条件**:这里直接采用已核验记录是系统的信任约定;对象库的定位与完整性不推出记录所述外部事件真实,也不推出任意校验器语义健全。`ledger_integrity_does_not_imply_input_veracity` 给出无损、单射记录仍可忠实保存错误报告的反例。有限摘要的全域唯一性边界见第 1.4 条;冻结真值的依据仍是第 1.2 条所定的证明及其公理闭包。
...
**零信息的数学范围**:对完整观察 q 与固定纯函数 f,增加 `f(q(x))` 后的联合观察 `(q(x),f(q(x)))` 与 q 有相同的不可区分关系。把重算与已存结果的比较归入这个结论,须已知存值就是同一 f 在同一完整输入上的结果;比对的形式本身不证明输入闭包完整、存值正确或计算环境相同。重放与必要查询仍依本款及第 10.5 条区分。
...
**账本冻真值、许可边界不退化;保证在门,不在存档。** 公理依赖取给定 toolchain/pin 下的闭包。写入时的公理许可分门通过 ⟺ 当下已验证该节点公理闭包落在固定许可集内;这只刻画该分门,其余冻结准入条件仍须履行。升级门须在**新环境**全量重算,全部冻结节点仍在同一许可集才放行,任一越界红。旧新闭包分别包含于许可集,不推出两者相等或新闭包包含于旧闭包;这里保持的是许可边界,不是公理依赖集合不变。
