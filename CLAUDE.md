...
**严格禁止新增或交付 bind-only 数学声明;全部为 bind-only 的 atom 必须直接走消化结算,以已处理终态结束,零新增 Lean。** 本禁令逐声明适用,公开定理、私有包装、别名、伴随定理与独立模块均无例外;同模块另有 content、真实消费者、API 便利、覆盖需求、任务终点或反驳用途均不能豁免。现有定理的直接应用是 content 证明内部的正常步骤,不得将这一步另立为新的绑定声明或形式化成果。
...
**唯一准入依据**:`admission_basis: escape-witness` 要求模块存在真实逃逸内容,且每条新增定理逐项通过上述判形与活路径审查;任一新增定理为 bind-only 即拒绝该交付,不得以同模块其他 content 掩护。全部为 bind-only 的候选须报 `admission_basis: none`,不得新增 Lean 声明、首次冻结、deposit,也不得另派实施席重做。探针只用于核实判形,一经确认 bind-only 即转消化结算。
...
**终态必须由现役 writer 产生**:bind-only 仍是命题,不得伪装成 `nonpropositional-inapplicable`,不得用 quarantine、失败的 cover disposition、手改目录或手填状态冒领已处理。当前 `cover` 只接受既有冻结项目声明 GID;仅有 Mathlib 前置而无可用项目覆盖 GID 等无法由现役 writer 表达的结算,属于具名工具缺口 `open(BIND-ONLY-PROPOSITIONAL-SETTLEMENT)`。其后续义务是补齐结算工具并完成 atom 终结,不得回派数学实施或新增绑定包装补 GID;工具真正写出并验证终态前,如实保留当前机器状态,不宣称 atom 已处理完成。新增终态或收据须按 spec 与 harness 的正门实现,本条不冒领该机器已经存在。
**先库后证与停手**:上游已有证明必须直接复用,禁止重证或新增包装。连续两次候选靶均为 bind-only,在完成可达的消化结算后按第 7.11 条换 Γ,转向真正缺失的数学内容或结算工具缺口;席位空闲不构成派题理由。
**逃逸内容须写在探针之前,不得事后追认**:预登记义务须点名拟议的非绑定事实。观测见证与拟议不同须重新预登记,不得事后改标;报告逐声明列 `proof_shape`、直接冻结依赖(GID + `statement_id`)、`escape_witness` 与 `admission_basis`。评审对任一新增 bind-only 声明、无逃逸依据的首次冻结或虚报 atom 终态均 reject;判形真实性仍属第 3.5 条所列语义评审边界,不冒充现役机器分类。

### 3.3 计算性内容的用途准入

**用途准入**是与判形、准入依据正交的第三个合取:新内容不保证有用途;不改 `proof_shape`、两种见证、第 3.2 条唯一 `escape-witness` 依据或既有冻结。用途规则的作用域为受保护 immutable baseline 未冻结的新增或字节变化 D5 Lean 模块,以及本次首次取得 state pin 的模块之并集;不随 judge-only 改动扫未变历史。分类与用途须逐声明评审,不得以同模块的无关反例掩护普通实例。
...
**硬规则**:①先答文献有无该陈述;已有只 `make cover`,或在第一/二档靶的真实消费者内用 `FromLiterature` 前置,不绕第 3.2 条独立首次冻结禁令、不单派探针/实施席。②搜题 brief/预登记须写档位及核对结果(查 X 无证明/计算未见于 Y),缺则评审打回。③按解决何已发表问题/推进何已知范围报进展,不用模块/席位/行数;有限证书只有排除此前未排除情形才可称部分进展。④同 lane 第三次因非数学缺陷重做即停(第 7.11 条);卷内事实错误单独勘注,不为证书做第四次 deposit。
...
- **消化态(账目侧,四值)**:`residual-open`(尚无 GID 覆盖)/ `partial-closed`(子项部分覆盖)/ `absorbed-closed`(覆盖 GID 与 coverage 数据齐备)/ `nonpropositional-inapplicable`(**非命题、无需覆盖**;#5770 方向③)。第四态**不是**由 atom 字节或 kind 派生的,而是由 `receipts.nonpropositional`(`justification` + 连读的 `previous_atom_id`/`next_atom_id`,边界为 `null`)经 `DigestionStatusEvaluator` 机器派生:收据在而目录不对、目录在而收据缺、或与 coverage / quarantine / cover_disposition 共存,皆 SL-016 红。唯一写者是 `settle-atom`(`make settle` / `make settle-clear`),写前必须以 `make atom-context ATOM_ID=x` 连读前后 atom,writer 复算邻接不等即拒;`--clear` 原路退回 `residual-open`。它对链闭合视同已闭(`chain_atoms` 子项为 Absorbed 或 Nonpropositional 即闭),对 Lean 真值零主张、不计入可形式化分母、`Deletable` 不因它为真。「不新增持久化状态」(#5533 L3)的例外仅限此类;`section/*` 等仍走查询时 `not-formalizable(kind)` 投影。
- **bind-only atom 的结算**:全 bind-only 的命题 atom 按第 3.2 条直接走既有覆盖与子句链闭合,到达 `absorbed-closed` 即处理完成;不产生新的 Lean 声明或冻结事件。现役 writer 无法表达的结算须补工具,不得套用非命题终态或手写已完成。
- **已发表、仓内无冻结 GID 覆盖的命题 atom 当前无终态(`open`)**:留 `residual-open`;无 GID 不满足 `absorbed-closed`,命题不得冒用 `nonpropositional-inapplicable`。`receipts.quarantine` 仅有封闭 `blocker_class={already-covered, missing-prerequisite, multi-clause-guard}` 与 `reentry_condition`,可用 `make quarantine-clear` 撤;`receipts.cover_disposition` 是 cover 未闭合时 writer 的失败回执。两者均非终态,不得挪用,文献判词因此仍有重复选题/核对的浮账缺口。所缺是带外部出处与回来条件的终态,属元层改动由 τ=0 owner 裁决;补齐前只在 PR/issue 正文保留必要判词,不冒报已处理。
...
- **合并门(纯机器)**:PR → dev 须过 **三 required check**:① engineering(build --warnaserror + `make -C tools test` 全量 + selftest 字节比对 + 能力链编译证明);② lean-inspect(post-merge Lean 报告内容寻址生产);③ admission(**候选自带判官**跑 `check --protected-base <dev-parent>`——base 只以受审 merge 对象第一父提交的 SHA 参与 git 对象级 diff,**不 checkout、不编译 base**;`pull_request_target` 的入口 workflow 文本来自 base 仓库的默认分支,不由 PR 目标分支或该 protected-base SHA 选取;版本核对见第 8.12 条)。冻结面(Hearts / `Golden/Frozen/state/` 状态片与当前 report 的 pin 一致 / 冻结模块不可变)由 SL-008 机器判;check-time union 级错误由 PR merge-ref CI 检测,M1→M2 残余由 dev push CI 检测 + 清扫(第 7.8 条形态)。绿且显式选择 auto-merge=自动合(缺省不 arm);红=修根因不绕。`enforce_admins` 实测 false(见第 7.1 条无外部特权的保证边界)。
...
**一致性义务**是被验树即落地树。三处 CI checkout 取 `refs/pull/N/merge`(push 回退 `github.sha`),验 `merge(dev_tip,head)` 并集;base 从 merge 第一父 `HEAD^1` 取,delta=`HEAD^1..HEAD`(契约见 `ci.yml`、`GitRepositoryGateway.cs`)。这只把 head/merge 缺口缩为 check-time M1/land-time M2:检查后 dev 前进可能落 M2,不保证严格同树,也不许可 strict。
**替代检测/缓解**:①准入容量只判本次碰过目录,阈值比全仓不变量留 1 格,使两个并发新增并集仍在界内。②全仓不变量留检测层,架构测试超限即红并裂桶(第 4.8 条)。③共享聚合物按第 4.5 条分片。④merge-ref CI 判 check-time union 错误,M1→M2 残余由 dev push CI 检测并快速勘正(第 7.8 条)。内容寻址的独立定理/前置、逐定理叙事及路径 GID 不因粗路径分类成为真交互;全仓目录计数/共享汇总才有聚合交互。
...
**开工先利器**:新 worktree 经 `make worktree` 钉版校验,创建时不物化 Lean 缓存。构建走 `make lean` / `make lean-report`,显式依赖物化走 `make lean-cache-ensure`;禁冷裸 `lake build` / `lake env lean`(#2762)。普通入口只读官方 Lake 共享工件,恢复私有 `.lake/build`,缺项由 Lake 本地编译;Mathlib 下载用私有 `.lake/mathlib-cache`。每次 ensure 均让原生 Lake 校验/物化当前私有依赖源码,pin stamp 匹配不证明源码仍在;保留无关私有构建输出。可选 Mathlib 下载失败/超时为 degraded,真实物化/构建失败仍须传递。
共享工件仅由物理主检出 clean dev 的 `make warm-donor` 集中发布,按钉版工具链/平台分区,不按 commit/整份 manifest 字节分区。共享读隔离仅支持 macOS sandbox-exec;无已发布共享库的平台保留私有缓存路径。
Git 引导发现先清继承的 trace 文件目的地,显式置 `GIT_TRACE2` / `GIT_TRACE2_EVENT` / `GIT_TRACE2_PERF` 为 `0`,覆盖 system/global Trace2 配置(仓库配置与 `-c` 对 Trace2 无效),保留普通配置及传输/凭据环境。report 指纹、发布地址、serial-lean 外层 Git 经 `lean-cache-run.sh --git` 用同一 C# 策略。引导仅以不刷新 index 的固定 `rev-parse --git-common-dir` 发现位置,保留 `--path` / Git `-C` 等实际仓库选择;随后请求的 Git 命令及配置回调、后代进程均先准入并受该仓库共享根写隔离,无需 Lake 工具链、依赖物化或 writer 锁。所有 Lake 子命令(含版本查询)同受隔离。
整个共享根不得含 symlink 或多链接常规文件:一次原生元数据遍历同查两者,不哈希内容。私有旧硬链接使共享 inode 的 nlink>1 时 reader 必须拒绝,不得 chmod、unlink 或自动拆链接修复共享元数据。
迁移/恢复由缓存属主在相关 reader/warmer 全停后执行:拆旧别名、恢复独立私有输出;旧别名若已可能写坏工件,须从可信输出重建受影响共享缓存,不能仅拆链接便信任旧字节。主检出普通构建也是 reader,不反向收集 worktree 输出;`.lake` 不得 symlink。reader 不承担维护。
POSIX 缓存命令经系统 `/usr/bin/perl` 建独立 session,启动期间继承 flock 原子交接。锁记录 session 数字、内核 boot UUID、发起进程 PID/出生时间及进程内命令序号;既有调用链后代继承实例标识,由内核进程环境/session 成员关系核对,非超时租约。wrapper/leader 死后,同 session 的 subgroup 写者仍使 guard 报 busy;全部写者退出、旧 session 数字被无关进程复用或重启后可恢复。仅支持保留 session/继承环境的现有调用链,不承诺主动脱离 session/清空环境的命令。正常或仍有写者的锁不得删除;旧无实例身份记录明确拒绝,仅在相关缓存用户停止后由属主移除该过期记录。〔守护:**软+硬投影**·make 与 C# process policy 执法构建入口;裸命令纪律仍软守〕
...
仅本会话一次性探针/搬运胶水例外,判据为第二次是否还用;被 CLAUDE.md/记忆/skill 引用即跨会话,必须入仓,仓外引用按第 9.2 条悬空。胶水入仓不因此欠单测债(第 9.1 条)。*成熟锚*:monorepo tooling、vendoring、IaC、可移植性。〔守护:**软+硬投影**·归属靠评审;CLAUDE.md/skill 工具路径须仓内 tracked,指向 ~/或 scratchpad 即红〕
...
**改 CI 三类验证,本条唯一主责**:管 `.github/workflows/**`、其脚本、权限/job 拓扑、required check 名/判据。①**最小 hotfix**仅恢复既有行为,普通 PR 直达 dev,无 integration 或稳定条数前置,三门/相关回归/真实触发仍须验。②**feature**全部功能覆盖且连续稳定合入 **≥4** 集成 PR。③**重构**全部受影响行为覆盖且 **≥8**。后两类无论单/多层,先在 `integration-ci-<主题>-tests` 大量集成,覆盖/性能/条数齐备才可将 dev PR 转 Ready 或合 dev,可先建下述 Draft;禁改名 hotfix 规避。
...
**候选来源与两类 Draft**:原始 lane 承载全部层及修复/优化,保留 lane→dev 正式交付 Draft,不另抽第三交付分支。integration 从不可变 dev SHA 建,各层以实际测试分支为 base 的独立安装 PR 三门绿后正常合入,记录源 lane/提交→安装 PR;候选修复回原 lane,再独立安装复验,不只留测试分支,原 lane 不合镜像历史。实际测试分支→dev 的集成跟踪 Draft 只作证据入口,始终 Draft/auto-merge off,验后关闭不合;交付 Draft 达标前同样保持 Draft/off,两者互引。无可审 diff 待真实 diff 再建;integration 先有首个安装 PR 三门正常合入,禁 dummy commit/绕检查。
...
**逐原 PR 同步**:围绕当前固定集成基线与覆盖需求,按依赖顺序镜像已合 dev 的原 PR;基线已包含的历史 PR 不再镜像,一个原 PR 对应一个真实独立 integration PR,只带该 PR 净变更,实际测试分支为 base,在候选 CI 下三门验绿正常合入;收集合入后 push 结果、分析及必要复验后才开下一项。禁批量 merge dev、打包多原 PR、直推同步。integration 只经 lane 层 PR、单原 PR 镜像、下款清理 PR 进改动,push CI 照触发。候选判官按 integration protected-base FILEMAP 与实际 delta 判单面;混面 SL-029 `ADMISSION-PLANE-MIXED`,dev 旧绿/镜像名均不豁免。既不能断言单镜像必单面,也不能断言整批必混面。搬运不替代判官;必要结果映射为原 PR/源 merge SHA/链接→集成 PR→被测 head/base、workflow 版本、run/判词。最后镜像 dev SHA 只标覆盖范围,其余增量由交付 merge-ref 验,汇总绿不替代逐项稳定证据。
...
**刷新后的测试边界**:新基线重新完成受影响行为、真实事件、缓存与性能验证,新 integration 的稳定计数从零开始;旧 runs 只证明原候选/环境,不继承条数。新基线前已经包含的 dev 历史无需逐 PR 补跑;此后继续按上款逐个引入独立 PR,每次分析 PR 与首个 push 后再推进。不得用重建回避具名失败、跳过回归或降低保护。分支年龄或 dev tip 前移本身不触发无条件重建,决定须附冲突路径、提交范围或实际兼容/同步成本读数;无固定等待期,也无机械追平要求。
**逐项分析/优化后再推进**:每项正常合入后收集 PR+push CI,关联上述身份,分析各阶段成功/失败/跳过原因、总/关键路径/分阶段耗时、缓存命中/失效/恢复/写入、Lean 重编/报告重检范围、重复工作/回归/异常。按实测修瓶颈与可执行优化并以可比条件复验效果;无需改动也须给证据,不扩无关改进。CI 行为变更/修复从该版本真实绿起重新计满本节 **4/8** 定义;正常输入 PR 在 CI 未变且成功时按序计入,head/base 前移本身不重置,不要求整树不变。
...
**稳定计数**:单位为最终候选 CI 版本在实际测试分支经真实事件三门绿、正常合入且首个 push 必需检查成功的集成 PR,每条一次,从该版本首次真实绿起连续 feature 4/重构 8。版本指实际 workflow/程序/判据/执行配置,非整树 SHA;正常内容镜像不重置,改变 CI 行为则重置。可计逐原 PR 镜像及候选实现/安装/修复/优化 PR;空分支、等待、旧 workflow 绿、重跑、dummy、验后关闭的探针 PR 不计,同原 PR 不拆包重复计。非预期失败先诊断处置、复验影响面,已计清零,从处置版本真绿起重计;重跑成功不抹失败。跑前预登记且命中具名预期判词的负向测试既不算不稳定也不计条数;漏检/非预期判词仍须修。
...
`pull_request_target` 入口来自 **base 仓库默认分支(dev),不是 PR 目标分支**;[GitHub 官方文档](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#pull_request_target)的 GITHUB_SHA/GITHUB_REF 是默认分支末次提交/默认分支,不同于 checkout M 和数据 B。branches 只过滤目标,安装 integration 后再开 PR 也不选择其 workflow。workflow/脚本接口同 PR 原子更新并核实际调用者/被调用者版本配对;仅同 PR 安装不保证默认旧调用者匹配新接口,禁兼容垫层。
...
**清理与最终对应**:专用探针载荷放触发 PR,验后关闭不合;合法镜像改动/历史留 integration,不为整理交付 diff 撤镜像。清理只删已有实验载荷/临时 wrapper,独立 PR 三门正常合入并分析 push,保留新 CI;改变 CI 则复验重计。交付前钉原 lane head、已验 integration head/各自基线,列全层候选/修复/优化/安装映射,逐文件核路径、增删、mode、内容,区分候选与已在 dev 的镜像增量。不能仅靠标题/数量/patch-id,不要求整树同一,不重放已验历史替代对应核对。基线差异/冲突适配须说明,遗漏或未经验证差异先回流修复复验,CI 改变重计。
**交付收尾**:候选对应与最终覆盖/性能/条数均达标后,原 lane Draft 转 Ready、普通 PR 合 dev;只带本次候选及必要修复,SL-029 照判,不并入 integration/镜像历史、不把跟踪 Draft 当交付。记录最后镜像 dev SHA,无追平/固定等待要求;交付 merge-ref 独立验当时并集并给 M/B,integration 绿不替代。dev 前移不自动重计或逼追平;实际冲突/失败须诊断,影响候选则重核复验,CI 改变重计。M1→M2 残余依第 7.7 条检测恢复,不冒领同树、不启 strict。
CI/权限/门控改动的独立 PR 开前评审归位;交付 Draft Ready 前完成最终评审,显式 AUTO_MERGE=1 前亦归位,缺省不 arm。交付 PR MERGED 且首个 dev push/适用新 PR 入口核验成功后,关闭跟踪 Draft、保留必要验证证据链接,回收测试分支/worktree;跟踪 CLOSED 只表验证结束,不等于交付完成。**政策文档可独立普通 PR 到 dev**,不声称已部署重构或验稳定。
...
**判官逻辑先 integration 触发、逐原 PR 复测,达标后 dev Ready/merge**:管 `tools/StrataLint.Engine/**` 规则/策略、门判据、新测试程序集、拓扑/IO/容量/棘轮等准入改动。CI 分类/覆盖/计数/两 Draft/同步/同保护/版本取源/无时长门/交付条件均以第 8.12 条为唯一主责,不另加追平或等待。单次 merge-ref 不消除 M1→M2,判官红会堵全仓。
**多层三段**(最小单层 hotfix 依第 5.5 条直 dev):①原 lane 保存各层,独立安装 PR 三门绿后合实际 `integration-<主题>`,维护两 Draft。②逐原 PR 镜像、分析/必要复验;候选修复回原 lane再安装,实测须重建时按第 8.12 条新分支/Draft/重计。③必要清理、候选净改动对应、全部适用验证后由原 lane 交付,merge-ref 验剩余 dev 增量,合后真触发再关跟踪 Draft清理。逐层 PR 在 integration 验,交付其已验各层,SL-029 照常;不携镜像历史、不另抽交付支。
...
改投不绕门:integration 三门照跑,只改候选/受保护基线,不改变 pull_request_target 取默认分支 workflow;逐 run 版本/范围依第 8.12 条,回 dev 再验。改投不替代第 7.14 条恢复 dev。重复红在无关 harness 时应换 base,不陪跑。**归因边界**:测试计划读取 BaseSnapshot、candidate 程序集及发现输入,并非只由 base 决定;一个实证 dev 红不推出全部 open PR 同红,未逐一核实者 `ASSUMED-UNVERIFIED`。
...
**越界反例**:读取 origin/dev 会使同一候选红绿随别人合并或本机 fetch 改变,可能合前绿、合后全仓自锁;merge-ref/strict 都补不上读第三棵树的错误。见此先查输入角色。
...
**剥夺名字不等于远端不可达**:默认分支的 pull_request_target workflow 每个 checkout 后仍移除 remote/refs/remotes、断言空并烟测 HEAD/HEAD^1;但紧跟步骤及其 12 条契约的 workflow 测试族 inactive,改坏不红,仅真跑/评审守。已测按名 origin/dev/refs/remotes 解析 exit=128;未消除①fetch-depth:0 下缓存全部分支对象,记下原始 OID 仍可 rev-parse/cat-file;②本地 donor 下显式 URL ls-remote 或 remote add+fetch exit=0。②在 CI 私有 HTTPS+persist-credentials:false 未测,本地不可外推。未收窄 fetch-depth:会影响 diff/changed-path/ingest 历史需求,且该 OID 逃逸案底为 0(第 7.10 条)。
...
**坐标一:跑得久先查缓存**。第 8.3 条裸 lake 禁令是软守,无机器拦截,不冒领硬门。`make lean` / `make lean-report` / `lake build` 跑数分钟未完时,不要继续等或以耗时推题难,须读每次 ensure 的 `LEAN_CACHE` 及本次 Lake 输出。现役字段为 `status`(`ready` 可继续 / `degraded` 私有 Mathlib 下载失败后可继续本地构建 / `failed` 准入或运行失败)、`worktree`、`mode`(`shared-reader` / `private`)、`cache`、`reason`、`archive_status`、`archive_reason`;失败收据仅含 `status`、`worktree`、`reason`。**ready 只表示依赖物化/缓存选择完成,不保证编译产物齐全或命中**。私有 `.lake/build` 缺 `.olean` 仍可从共享 native artifacts 恢复;以 `make lean LEAN_TARGETS="-v"` 的逐模块 `Reused` / `Built` 区分命中/实际重编,不得据本地缺文件推断全量重编。**历史参照(2026-08-23,同日同机,旧缓存机制,非当前字段或性能保证)**:热 donor 新 lane 13s(内容1430/依赖8550,双 warm),release 归档 6.5min(取5m08s+补1m18s,重编19/1513);本地超过约10min先查缓存。没有可信冷编时长参照;旧17m51s不承重,树/枚举子图不同且日志 Built=0。引用冷编代价必须给树及是否含昂贵枚举子图。〔守护:**全软**·称数学导致慢须引本次 LEAN_CACHE,缺即归因无效;无 lint 不豁免〕
**坐标二:缓存正常仍付不起时量证明项,不抬预算**。症状为已核对 LEAN_CACHE 缓存选择及本次 Lake 逐模块复用/重编读数、本地 make lean 成功且远低于10min而 CI `Build candidate Lean project` 失败/被杀,或同类40–44s而该模块数分钟;不能只沿冷缓存诊断。本地绿只报成败,不报 kernel checked 时间/峰值内存,也不证明 runner 能承受。
先经 `tools/scripts/worktree/lean-cache-run.sh /usr/bin/time -l lake env lean -Dprofiler=true -Dtrace.profiler=true -Dtrace.profiler.threshold=1000 <模块>.lean` 量 checked 秒数与 maximum resident set size,按声明找主成本,每改一处重量,PR 给前后读数(第 2.8 条)。
