...
**开工先利器**:新 worktree 经 `make worktree` 钉版校验,创建时不物化 Lean 缓存。构建走 `make lean` / `make lean-report`,显式依赖物化走 `make lean-cache-ensure`;禁冷裸 `lake build` / `lake env lean`(#2762)。canonical runner 为每个 worktree 设置 Lean 4.33.0 官方可写 artifact cache 环境;各 worktree 都可复用并发布工件,缺项由 Lake 编译。Mathlib 下载用私有 `.lake/mathlib-cache`,其显式 `restoreAllArtifacts := true` 仍由 Lake 执行。每次 ensure 均让原生 Lake 校验/物化当前依赖源码;pin stamp 匹配不证明源码仍在;保留无关私有构建输出。真实物化/构建失败仍须传递。
官方 cache 的路径、哈希、映射和发布由 Lake 维护;本地与 CI 统一设置 `LAKE_ARTIFACT_CACHE=true` 和 `LAKE_RESTORE_ARTIFACTS=true`,恢复模块路径,供现有报告消费者与 build/dependency archives 使用;包的显式设置仍按官方优先级执行。Lake 对缺失工件先尝试 hardlink,跨文件系统或链接失败时复制,并非 reflink 实现。各 worktree 的 `.lake/build`、trace、setup、sources 和 reports 独立;hardlink 只共享工件 inode,新工件与新 cache generation 仍增加存储。本机小夹具的 inode/字节/可用空间读数不外推为全仓性能或 APFS 物理节省;无零增长、事务或自动容量上限保证。显式 archive publish 在同一 runner reservation 内先 build 再 pack,fetch 经 runner 直接调用所选 Lake unpack;不新增 YAML cache 服务。
Git 引导发现先清继承的 trace 文件目的地,显式置 `GIT_TRACE2` / `GIT_TRACE2_EVENT` / `GIT_TRACE2_PERF` 为 `0`,覆盖 system/global Trace2 配置(仓库配置与 `-c` 对 Trace2 无效),保留普通配置及传输/凭据环境。report 指纹、发布地址、serial-lean 外层 Git 经 `lean-cache-run.sh --git` 用同一 C# 策略。引导仅以不刷新 index 的固定 `rev-parse --git-common-dir` 发现位置,保留 `--path` / Git `-C` 等实际仓库选择。`LeanProcessPolicy.Create` 先在空写者集合中执行只读 `lake --version` 校验;随后 ensure 的依赖物化和请求的 Lake 命令才在 canonical runner 的 worktree writer reservation 内运行。独立的 `cache-git` 使用清理后的 Git 环境执行 bootstrap 与请求命令,不取得 writer reservation,也不宣称配置回调或后代进程隔离。
旧 Git-common `stratalint-lake` store、主检出 `.lake` 和已发布 archives 保留,不由新入口自动迁移、清理或反向收集。`.lake`、`build`、`packages` 仍为真实目录;显式 archive fetch 只有在清单、摘要和资产验证通过后才经 canonical runner 解包。失败发布不得破坏上一代 cache,并发 worktree 通过各自 reservation 保护可变状态。
...
**坐标一:跑得久先查缓存**。第 8.3 条裸 lake 禁令是软守,无机器拦截,不冒领硬门。`make lean` / `make lean-report` / `lake build` 跑数分钟未完时,不要继续等或以耗时推题难,须读每次 ensure 的 `LEAN_CACHE` 及本次 Lake 输出。现役字段为 `status`(`ready` 可继续 / `failed` 准入或运行失败)、`worktree`、`mode`(`official-writable`)、`cache`(Lake 解析出的 artifact 路径)、`reason`;失败收据仅含 `status`、`worktree`、`reason`。**ready 只表示依赖物化/缓存选择完成,不保证编译产物齐全或命中**。本地与 CI 的 canonical 构建均启用官方完整恢复,命中工件恢复至各树 `.lake/build` 的模块路径,供报告和 archive 使用。以 `make lean LEAN_TARGETS="-v"` 的逐模块 `Reused` / `Built` 区分命中/实际重编;文件存在或目录大小不证明命中率、全仓性能或物理占用。**历史参照(2026-08-23,同日同机,旧缓存机制,非当前字段或性能保证)**:热 donor 新 lane 13s(内容1430/依赖8550,双 warm),release 归档 6.5min(取5m08s+补1m18s,重编19/1513);本地超过约10min先查缓存。没有可信冷编时长参照;旧17m51s不承重,树/枚举子图不同且日志 Built=0。引用冷编代价必须给树及是否含昂贵枚举子图。〔守护:**全软**·称数学导致慢须引本次 LEAN_CACHE,缺即归因无效;无 lint 不豁免〕
