...
**终态必须由现役 writer 产生**:bind-only 仍是命题,不得伪装成 `nonpropositional-inapplicable`,不得用 quarantine、失败的 cover disposition、手改目录或手填状态冒领已处理。`cover` 只接受既有冻结项目声明 GID;全部子句仅由钉版上游(`lake-manifest.json` 所钉 Mathlib 及其闭包)既有声明闭合、仓内无可用项目覆盖 GID 的命题 atom,走第五终态 `upstream-closed`(第 4.7 条;唯一 writer `settle-upstream`,探针源只 import 上游、禁 import `D5.`;案 #8049,原具名缺口 `open(BIND-ONLY-PROPOSITIONAL-SETTLEMENT)` 由它关闭),不得回派数学实施或新增绑定包装补 GID。两条路径不混用:需要任一项目声明者走 cover,纯上游者走 upstream。writer 未写出并验证终态前,如实保留当前机器状态,不宣称 atom 已处理完成;判形真实性(全部子句确为 bind-only 于所列上游声明、探针陈述忠实于 atom 文本)仍是本条软评审边界,机器只判形状、编译、公理与 import 面。
...
docs/develop/theory/**                       参考输入·当前正确,历史归 git,程序对其零知识
  ...
  └─► Meta/Digestion/backfill/<source_id>/<态>/<atom_id>.yaml   消化账目·五态见下
...
- **消化态(账目侧,五值)**:`residual-open`(尚无 GID 覆盖)/ `partial-closed`(子项部分覆盖)/ `absorbed-closed`(覆盖 GID 与 coverage 数据齐备)/ `nonpropositional-inapplicable`(**非命题、无需覆盖**;#5770 方向③)/ `upstream-closed`(**纯上游命题、无项目 GID**;#8049,见下款)。第四态**不是**由 atom 字节或 kind 派生的,而是由 `receipts.nonpropositional`(`justification` + 连读的 `previous_atom_id`/`next_atom_id`,边界为 `null`)经 `DigestionStatusEvaluator` 机器派生:收据在而目录不对、目录在而收据缺、或与 coverage / quarantine / cover_disposition 共存,皆 SL-016 红。唯一写者是 `settle-atom`(`make settle` / `make settle-clear`),写前必须以 `make atom-context ATOM_ID=x` 连读前后 atom,writer 复算邻接不等即拒;`--clear` 原路退回 `residual-open`。它对链闭合视同已闭(`chain_atoms` 子项为 Absorbed 或 Nonpropositional 即闭),对 Lean 真值零主张、不计入可形式化分母、`Deletable` 不因它为真。「不新增持久化状态」(#5533 L3)的例外仅限此类;`section/*` 等仍走查询时 `not-formalizable(kind)` 投影。
- **bind-only atom 的结算**:全 bind-only 的命题 atom 按第 3.2 条直接走既有覆盖与子句链闭合,到达 `absorbed-closed` 即处理完成;不产生新的 Lean 声明或冻结事件。仅由钉版上游闭合而无项目 GID 者走下款 `upstream-closed`;两者之外现役 writer 无法表达的结算须补工具,不得套用非命题终态或手写已完成。
- **上游命题终态(第五态,#8049;spec §11.21「上游命题终态」)**:全部子句仅由钉版上游既有声明经实例化、投影与规范化推出、且仓内无可用冻结 GID 的命题 atom,由唯一 writer `settle-upstream`(`make settle-upstream REQUEST=<strict TOML> BASE=<rev>` / `make settle-upstream-clear ATOM_ID=x BASE=<rev>`)写入 `receipts.upstream`(键集恰为 `{justification, declarations, mathlib_rev, probe_sha256, probe_axioms, previous_atom_id, next_atom_id}`)并由 `residual-open` 迁入 `upstream-closed`;探针源与账目同一原子写入 `Meta/Digestion/upstream/<atom_id>.lean`(只 import 上游包、禁 `D5.`,以具名 `theorem` 逐子句陈述并 `#print axioms`,公理闭包 ⊆ `{propext, Classical.choice, Quot.sound}`),writer 在同一进程内以 `lake env lean` 编译探针并逐个解析 `declarations`。检查次序与 `settle-atom` 同为「可写性先于邻居」:定位(`ATOM_ABSENT`/`ATOM_AMBIGUOUS`)→ `CHAIN_PARENT` → 动词专属可写性(`NOT_RESIDUAL_OPEN`、`COVERAGE_PRESENT`、`QUARANTINE_PRESENT`、`DISPOSITION_PRESENT`、`NONPROPOSITIONAL_PRESENT`、`UNRESOLVED_SUBITEMS_PRESENT`)→ 邻居上下文复算(`CONTEXT_MISMATCH`)→ 探针面(`PROBE_PATH_INVALID`、`PROBE_IMPORT_SYNTAX`、`PROBE_IMPORTS_PROJECT`、`PROBE_DECLARATION_UNSUPPORTED`、`PROBE_FAILED`、`PROBE_AXIOMS`、`DECLARATION_UNRESOLVED`);同值重放为零字节变化,内容不同的重设以 `UPSTREAM_CONFLICT` 拒,`--clear` 原路退回 `residual-open` 并删探针(无收据即 `UPSTREAM_ABSENT`)。该终态对链闭合视同已闭(与 `absorbed`、`nonpropositional` 并列),对项目 Lean 真值零主张,不计入项目已证与可形式化分母,不进入 `formalization_frontier`;SL-016 对收据形状、目录↔派生态、探针文件存在与 `probe_sha256` 相符逐项 fail-closed(delta-only,对存量盲)。它是命题的终态,与 `nonpropositional-inapplicable` 互斥不混用;判形真实性与探针忠实性仍是第 3.2 条软评审边界。
- **文献已发表、既无冻结 GID 覆盖亦不能由钉版上游探针闭合的命题 atom 当前无终态(`open`)**:留 `residual-open`;无 GID 不满足 `absorbed-closed`,不能由上游探针闭合者不满足 `upstream-closed`,命题不得冒用 `nonpropositional-inapplicable`。`receipts.quarantine` 仅有封闭 `blocker_class={already-covered, missing-prerequisite, multi-clause-guard}` 与 `reentry_condition`,可用 `make quarantine-clear` 撤;`receipts.cover_disposition` 是 cover 未闭合时 writer 的失败回执。两者均非终态,不得挪用,文献判词因此仍有重复选题/核对的浮账缺口。所缺是带外部出处与回来条件的终态,属元层改动由 τ=0 owner 裁决;补齐前只在 PR/issue 正文保留必要判词,不冒报已处理。
