...
claim: In particular, there is no lattice tiling of Z cubed by the radius-two Lee ball; equivalently, no additive subgroup of index 25 has a quotient map injective on that ball.
strata_touched:
...
The nine FromLiterature references in
`D5/S3/Arith/Coding/LeeBallTwoLatticeObstruction` share this result-level
attestation. The ball definition, its membership characterization and
cardinality, the second and fourth moments, the two finite-group readout
obstructions, and the order-25 classification are the ingredients of this
module's independent kernel-checked proof. This note does not claim that the
paper states each helper in the module's exact algebraic form or uses the
same proof. In particular, the identities with coefficients 18, 30, and 12
are verified in Lean, and the group classification uses pinned Mathlib.
...
- The prior probe's Crossref and arXiv receipts independently record the
  Leung-Zhou all-dimensions lattice statement. Exact helper numbering in
  the 1998 article is not claimed or verified here.
