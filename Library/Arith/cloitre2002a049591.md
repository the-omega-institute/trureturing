...
- URL: https://oeis.org/A049591
- NAME (verbatim): Odd primes p such that p+2 is composite.
- COMMENT characterization (verbatim): Sequence appears also to give all n > 1 such that there is no prime p satisfying the inequality n < p < n+tau(n)^2 where tau(n)=A000005(n).

Only Cloitre's gap characterization is refuted: 529 satisfies the stated
interval condition but is not a sequence term; the NAME definition and all
