...
第五条直接复用公开第四、八条尾项结论，由同源低阶系数补齐 P=I4(F)=X+4X² 与 I8(F)=X，其中 F=A mod8。令 J=I5(F)，将 P 代入真实源式 F=X+PJ 得 J=P+XF。消元并在特征8展开得到 (J-X)(1-X⁴)=5X²+X³+X⁴+5X⁵。分母1-X⁴常数项为1，是单位；其几何逆元只在4的倍数指标有系数1，因此全部 n>1 的第五次迭代系数给出上述周期。自然数卷积保留移位不超过指标的边界，未把截断减法的0误当有效项。第四、第五、第八条均为全指标符号结论，不依赖有限前缀、完整有理式候选或第2条的模八强式，未另造源定义。
