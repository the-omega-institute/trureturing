...
同一定理另给 BH 与经验 Bayes $`\ell`$-value 程序的适应性结论；该部分额外要求
第 11 页 (12) 的多项式稀疏性 $`s_N\lesssim N^\eta`$（某个 $`\eta\lt1`$），
以及名义水平 $`\alpha_N=o(1)`$、
$`-\log\alpha_N=o(\sqrt{\log N})`$。
这些程序条件不应混入前一条基本极小极大风险陈述，也不能省略后再声称已引用其适应性结论。
