...
同一版本第 11 页定理 2 给出独立高斯模型中不使用稀疏度与边界偏移的
经验 Bayes 和 BH 规则；定理 7 及 §S-6.3 将有关结论用于归一化 Hamming 损失。
因此，利用整份数据决定阈值并达到一阶最优风险，是明确的既有方法。
第 11 页注记 2 另指出：在多项式稀疏性下，BH 取
$`\alpha_N=1/\sqrt{\log N}`$ 时，组合风险
$`R=\mathrm{FDR}+\mathrm{FNR}`$ 的误差可达
$`(\log\log N)/\sqrt{\log N}+e^{-c_0s_N}`$ 的阶数；
该注记引用定理 5 及其后的讨论。此处的 $`R`$ 不是原始 Hamming 损失，
不能不核对分母就把这个速率直接当作所有程序的 Hamming 下界。

[支持恢复卷第 24 章](../../docs/develop/theory/PARITY_HIDDEN_ARROW_RECOVERY.md#24-同批观测的参数修正与自适应二阶精度)
在补偿路径和相邻对的实际律下，用初选集合内的汇总计数估计振幅，再按估计分数筛选。
它不把该比值称为选择后条件无偏估计，也不把估计后的分数当作仍有效的 e-value。
证明直接以所有真信号的绝对偏差控制任意初选子集，再用同一计数实现上的
得分与导数联合交叉界控制重筛选。得分的期望导数在真参数处为零，
并不意味着得分与导数不相关，也不能消除同批数据带来的依赖。

上述一般选择偏差与似然原理不取得新理论的名义。
本仓推导的范围是这个实际依赖模型、固定参数与有限内在偏移下，
数据规则的 Hamming 损失除以真基数具有 $`O(\lambda^{-1/2})`$ 余项。
这不直接改进另一统计实验、另一损失或全部先验类中的文献结论。
