...
## Related derivation boundary, 2026-09-16

The proposed section 15A of `docs/develop/theory/CONTEXTUAL_SPACETIME_ARITHMETIC_ML.md` studies a different question: recovering the external Gram state of a two-layer real linear learner from one passive trajectory on the fixed data `X=I, Y=0`. The derivation reuses the existing exact Gram recurrences. It classifies the remaining state directions after two steps by a graph in the initial singular-vector coordinates, and gives an observable third-step criterion that removes the last direction in the connected case. It also accounts for errors in the propagation matrices reconstructed from noisy output data.

These passive-identification statements are not attributed to Holzmüller and Steinwart. They are supplied as an ordinary-proof, insertion-only [theory patch](../../docs/reports/state-physics/passive-gram-calculus-0916.patch) against the existing volume, blob `fb5b3308f198ffd9c7511edefd732cdc69342e99` at commit `e67699dc905b37ef05859ed0075831fd01b1e1ce`. The patch contains the full proposed section, including the degeneracy and solver boundaries. Its presence is not a claim that the volume was updated or formally ingested. The previous #8029 is closed and unmerged; its uncompiled Lean candidate is not treated as a verified dependency or reintroduced here.
...
Read scope this round: this existing Library note, the current repository's exact Gram and tangent definitions, the cited arXiv records and abstracts, and the Lindsey–Menon HTML overview. The Holzmüller–Steinwart appendix locator above is retained from its earlier verification, not represented as freshly reread full text. There is no exhaustive priority audit, new kernel proof, or external named conjecture-resolution claim.
