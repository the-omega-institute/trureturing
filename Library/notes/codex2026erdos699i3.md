...
全程共有 402255 个界内奇 `u`，其中 284528 个满足互素与三进赋值条件；检查 3162855 个 `R(Mu-1)` 素数幂分配后得到 590 个第一低块重构值，其中 528 个满足 `tu>=4`。程序对这 528 对全部完成了 `C(n,3)` 分解及 Kummer/Legendre 交叉核对；没有一个重构值同时满足第二低块，故本范围内 (56) 解数和忠实反例数均为 0。实际检查到的最大 `n` 为 1924862608146432；这是约化参数中的最大值，不是声称连续检查了此前到该数之间的每个 `n`。
...
此结果只是有限自审，没有 Lean 证明或独立评审；`a>=33` 未搜索。第一个仍未证明的陈述是：无界 `a` 下不存在满足完整 `i=3` 条件的合法 `(M,u,t)`；更弱的“(58) 在全部合法参数上无解”也仍未证明。没有发现忠实反例，Erdős 699 状态不变。
