...
/- TASK D5-T0008
    ...
    retired: the signed-power classification, PID, UFD, and CarrierFoundations deliveries are active Frozen nodes:
...
/- TASK D5-T0050
   DECT role-admission direction design remains open under issue #3218. The source
   leaves three questions unresolved for the independent-carrier formalization:
   (1) `AdjudicationSnapshot.dependencyClosure : Set Artifact` and
   `EvidenceFiltration.seen : EventId -> Set Evidence` have no source-defined
   Artifact-to-Evidence bridge with which to state `Dep*(K_n) ⊆ F_(t_n)`;
   (2) the sketch constrains `seen` only by monotonicity, while section 48.1 prose
   says it is generated by accessed objects and their reflexive-transitive
   dependencies, without specifying the production relation or carrier bridge;
   (3) no source-owned method enumerates and checks every direction choice, so a
   future implementation must supply a complete direction inventory whose omission
   is fail-closed before any direction-sensitive witness is admitted.
   Separately, section 48.2 says replication requires a new observation event and
   an explicit protocol-independence relation but defines neither object. Therefore
   no `ReplicateIndependent` predicate is introduced here. Close only after the
   source defines these carriers and relations, or explicitly retracts the claims;
   do not close by adding a free bridge or a caller-supplied independence Prop. -/
def dectRoleAdmissionDirectionGapTicket : Unit := ()
