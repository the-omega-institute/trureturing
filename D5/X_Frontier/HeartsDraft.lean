...
1. Faithfulness view. PZG 26.4 asks for even smooth compact tests, the
...
`Meta/BACKFILL.yaml` already lists `D5/X_Frontier/HeartsDraft` under
`pzg-26.4` and `pzg-o6`. Both entries truthfully remain
...
1. Verify the anchored source, extract the patch below, and run
   `git apply --check --unidiff-zero -`; then repeat the pipeline with
   `git apply --unidiff-zero -`. The same patch adds
   `D5/X_Frontier/Hearts` to the `pzg-26.4` and `pzg-o6` coverage lists while
...
test "$(git hash-object D5/X_Frontier/Hearts.lean)" = \
  d9b272d7eff465351f4871cda8ea93ec50a6765f
...
/-- The exact, source-anchored Hearts patch; data only, never applied by this draft. -/
...
-be added.
+O-6 is the classical Weil positivity heart from PZG 26.4. D5-T0018 binds its
