...
/-- Galois conjugation exchanges the two complex embedding branches. -/
def branchSwap : Matrix (Fin 2) (Fin 2) ℂ :=
  !![0, 1; 1, 0]
...
theorem golden_local_branch_classification (p : Nat) (hp : p.Prime) :
    Matrix.det (goldenLocalBranchOperator p) = (legendreSym 5 p : ℂ) ∧
      (legendreSym 5 p = 1 →
        Matrix.det (goldenLocalBranchOperator p) = 1 ∧ goldenLocalBranchOperator p = 1) ∧
      (legendreSym 5 p = -1 →
        Matrix.det (goldenLocalBranchOperator p) = -1 ∧
          goldenLocalBranchOperator p = branchSwap) ∧
      (legendreSym 5 p = 0 →
      ...
      (¬ IsUnit (goldenLocalBranchOperator p) ↔ p = 5) ∧
  ...
  have splitAction :
    ...
    · ext i j
      fin_cases i <;> fin_cases j <;>
        simp [goldenLocalBranchOperator, evenBranchProjection, oddBranchProjection,
          branchSwap, hsplit] <;> norm_num
  have inertAction :
      legendreSym 5 p = -1 →
        Matrix.det (goldenLocalBranchOperator p) = -1 ∧
          goldenLocalBranchOperator p = branchSwap := by
    intro hinert
    constructor
  ...
  have ramifiedAction :
    ...
    · rw [hop]
      ext i
      fin_cases i <;>
        norm_num [evenBranchProjection, branchSwap, Matrix.mulVec, dotProduct,
          Fin.sum_univ_two]
    · rw [hop]
      ext i
      fin_cases i <;>
        norm_num [evenBranchProjection, branchSwap, Matrix.mulVec, dotProduct,
          Fin.sum_univ_two]
  have zeroCharacterIff : legendreSym 5 p = 0 ↔ p = 5 := by
    ...
    · intro h
      subst p
      norm_num
  have castZeroCharacterIff : (legendreSym 5 p : ℂ) = 0 ↔ p = 5 := by
    constructor
    · intro hzero
      apply zeroCharacterIff.mp
      exact_mod_cast hzero
    · intro hfive
      exact_mod_cast zeroCharacterIff.mpr hfive
  have uniqueNoninvertible : ¬ IsUnit (goldenLocalBranchOperator p) ↔ p = 5 := by
    rw [Matrix.isUnit_iff_isUnit_det, determinantIdentity, isUnit_iff_ne_zero]
    simp only [not_not]
    exact castZeroCharacterIff
  have ramifiedSquare : (5 : GoldenInt) = (-1 + 2 * phi) ^ 2 := by
    rw [D5.S3.PrimeForms.GoldenPrimeClassification.golden_five_eq_ramified_square]
    congr 1
  exact ⟨determinantIdentity, splitAction, inertAction, ramifiedAction,
