...
/- Library-search audit trail (2026-09-05):
   ...
   * Pinned Mathlib has no Erdos-Moser theorem. `FiniteField.sum_pow_units` supplies only the
     finite-field power-sum dichotomy; the block decomposition and standalone transport
     statements below are new. The units embedding, `univ.map = univ \ {0}`, and zero-term
     removal proof pattern already appears in `FiniteField.sum_pow_lt_card_sub_one`.
