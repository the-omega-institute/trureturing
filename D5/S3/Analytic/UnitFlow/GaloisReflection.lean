...
/-- The source domain `Re(s) > 1` for the Golden principal zeta. -/
abbrev PrincipalHalfPlane := {s : ℂ // 1 < s.re}
...
theorem galois_reflection
    (s : PrincipalHalfPlane)
    (hregulator : Function.Periodic (principalZeta (s : ℂ)) regulatorPeriod) :
    (∀ eta : ℝ,
        principalZeta (s : ℂ) eta = principalZeta (s : ℂ) (-eta)) ∧
      Function.Injective (unitFlowAction regulatorPeriod) ∧
      ∀ (g : DihedralGroup 0) (eta : ℝ),
        principalZeta (s : ℂ) (unitFlowAction regulatorPeriod g eta) =
          principalZeta (s : ℂ) eta := by
  have hreflection : ∀ eta : ℝ,
      principalZeta (s : ℂ) eta = principalZeta (s : ℂ) (-eta) := by
    intro eta
    unfold principalZeta principalZetaTerm
    calc
      ∑' α : {α : NumberField.RingOfIntegers GoldenField // α ≠ 0},
          ((anisotropicForm (K := GoldenField)
            goldenEmbeddingPlus goldenEmbeddingMinus eta α : ℝ) : ℂ) ^ (-(s : ℂ)) =
          ∑' α : {α : NumberField.RingOfIntegers GoldenField // α ≠ 0},
            ((anisotropicForm (K := GoldenField) goldenEmbeddingPlus goldenEmbeddingMinus eta
              (nonzeroIntegerConjugation goldenConjugation α) : ℝ) : ℂ) ^ (-(s : ℂ)) :=
        ((nonzeroIntegerConjugation goldenConjugation).tsum_eq _).symm
      _ = ∑' α : {α : NumberField.RingOfIntegers GoldenField // α ≠ 0},
          ((anisotropicForm (K := GoldenField)
            goldenEmbeddingPlus goldenEmbeddingMinus (-eta) α : ℝ) : ℂ) ^
              (-(s : ℂ)) := by
        apply tsum_congr
        intro α
...
example
    (s : PrincipalHalfPlane)
    (hregulator : Function.Periodic (principalZeta (s : ℂ)) regulatorPeriod) :
    principalZeta (s : ℂ) 1 = principalZeta (s : ℂ) (-1) ∧
      principalZeta (s : ℂ) (unitFlowAction regulatorPeriod (.r 1) 1) =
        principalZeta (s : ℂ) 1 ∧
      principalZeta (s : ℂ) (unitFlowAction regulatorPeriod (.sr 0) 1) =
        principalZeta (s : ℂ) 1 := by
  rcases galois_reflection s hregulator with
