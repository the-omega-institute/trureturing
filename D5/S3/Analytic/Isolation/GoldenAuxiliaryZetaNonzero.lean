/- GID: D5/S3/Analytic/Isolation/GoldenAuxiliaryZetaNonzero
   generality: I
   mirror-B: D5/B/S3/Analytic/Isolation/GoldenAuxiliaryZetaNonzero
   mirror-E: none(waiver:evidence-not-specified-by-formal-manifest)
   anchors: []
   digest: Riemann zeta is nonzero at the golden auxiliary point. -/

/- Library-search audit trail (2026-09-03):
   * Pinned Mathlib has no direct theorem that Riemann zeta is negative or
     nonzero on the real interval `(0, 1)`.
   * The proof uses the absolutely convergent adjacent-pair form of the
     Dirichlet eta series on `0 < re s`, the identity theorem on the upper
     half-plane, and positivity of every real pair.
   * `o5_beta_power_law` supplies the frozen golden-coordinate input; the
     Mathlib inputs include `hasSum_expZeta_of_one_lt_re`, the locally uniform
     tsum criterion, and the analytic identity principle.

   STOPPING JUSTIFICATION: this theorem isolates only the nonvanishing of
   zeta at the single auxiliary point `1 / phi`. It neither asserts a
   zero-free region nor proves any O-5 estimate or RH implication. -/
...
namespace D5.S3.Analytic.Isolation.GoldenAuxiliaryZetaNonzero
...
private lemma pairedEta_eq_zetaEtaFactor_of_real {x : ℝ} (hx : 0 < x) (hx1 : x ≠ 1) :
    pairedEta (x : ℂ) = zetaEtaFactor (x : ℂ) := by
  let seq : ℕ → ℂ := fun n => (x : ℂ) + I * (1 / ((n + 1 : ℕ) : ℂ))
  have hone_div : Tendsto (fun n : ℕ => 1 / ((n + 1 : ℕ) : ℂ)) atTop (𝓝 0) :=
    by
...
private lemma riemannZeta_ne_zero_of_real_pos {x : ℝ} (hx : 0 < x) (hx1 : x ≠ 1) :
  ...
  intro hzeta
  apply heta_ne
  rw [pairedEta_eq_zetaEtaFactor_of_real hx hx1, zetaEtaFactor, hzeta, mul_zero]

private lemma golden_auxiliary_numeric_bracket :
    (1 : ℝ) / 2 < 1 / Real.goldenRatio ∧ 1 / Real.goldenRatio < 1 := by
  constructor
  · exact one_div_lt_one_div_of_lt Real.goldenRatio_pos Real.goldenRatio_lt_two
  · simpa only [one_div] using
      (inv_lt_one_of_one_lt₀ Real.one_lt_goldenRatio)

/-- Riemann zeta does not vanish at the golden auxiliary point `1 / phi`. -/
theorem riemannZeta_golden_auxiliary_ne_zero :
    riemannZeta (((1 / Real.goldenRatio : ℝ) : ℂ)) ≠ 0 := by
  have hbeta :
      D5.S3.Analytic.GoldenEulerBeta.o5Beta 1 = Real.goldenRatio ^ 2 :=
    D5.S3.Analytic.GoldenEulerBeta.o5_beta_power_law.1
  have hcoordinate :
      1 / Real.goldenRatio =
        Real.goldenRatio / D5.S3.Analytic.GoldenEulerBeta.o5Beta 1 := by
    rw [hbeta]
    field_simp [Real.goldenRatio_ne_zero]
  have haux_pos : (0 : ℝ) < 1 / Real.goldenRatio := by
    rw [hcoordinate, hbeta]
    positivity
  have haux_ne_one : (1 / Real.goldenRatio : ℝ) ≠ 1 :=
    ne_of_lt golden_auxiliary_numeric_bracket.2
  exact riemannZeta_ne_zero_of_real_pos haux_pos haux_ne_one

#print axioms riemannZeta_golden_auxiliary_ne_zero

end

end D5.S3.Analytic.Isolation.GoldenAuxiliaryZetaNonzero
