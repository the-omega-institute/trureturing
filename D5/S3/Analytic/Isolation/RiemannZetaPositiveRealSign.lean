/- GID: D5/S3/Analytic/Isolation/RiemannZetaPositiveRealSign
   generality: G
   mirror-B: D5/B/S3/Analytic/Isolation/RiemannZetaPositiveRealSign
   mirror-E: none(waiver:evidence-not-specified-by-formal-manifest)
   anchors: []
   digest: Riemann zeta is real and signed on the positive real axis away from one. -/
...
/- Library-search audit trail (2026-09-03):
   * Repository searches found no public theorem with this general statement.
     The frozen instance modules `GoldenAuxiliaryZetaNonzero` and
     `GoldenGermSecondOrderRealAxisSign` contain the required eta architecture,
     but both are `generality: I` and expose the needed local facts only as
     private declarations. To preserve SL-010, this `generality: G` module
     imports neither and rebuilds the argument from the same definitions.
   * Pinned Mathlib supplies
     `Complex.differentiableOn_tsum_of_summable_norm`, the analytic identity
     principle, `Real.strictAntiOn_rpow_Ioi_of_exponent_neg`,
     `Complex.ofReal_tsum`, `zeta_eq_tsum_one_div_nat_cpow`, and the two
     positive-real zeta lemmas above one. Their names and signatures were
     checked by a successful repository `make lean` run before this proof.

   STOPPING JUSTIFICATION: the result determines only realness and sign on the
   positive real axis away from the pole at one. It asserts no zero-free
   complex region, no O-5 estimate, and no Riemann-hypothesis implication. -/

open Complex Filter Set Topology

namespace D5.S3.Analytic.Isolation.RiemannZetaPositiveRealSign
  ...
  have hbound (x : ℝ) (hx : x ∈ Set.Icc a b) :
    ...
    have hbase : x ^ (-s.re - 1) ≤ a ^ (-s.re - 1) :=
      Real.rpow_le_rpow_of_nonpos
        (lt_of_lt_of_le zero_lt_one ha) hx.1 hexp_nonpos
...
private lemma pairedEta_eq_zeta_factor_of_one_lt_re {s : ℂ} (hs : 1 < s.re) :
  ...
  have hs0 : -s ≠ 0 :=
    neg_ne_zero.mpr (ne_zero_of_re_pos (zero_lt_one.trans hs))
  ...
  have heven :
      ∑' n : ℕ, f (2 * n) = (2 : ℂ) ^ (-s) * riemannZeta s := by
    calc
      ∑' n : ℕ, f (2 * n) =
          ∑' n : ℕ, (2 : ℂ) ^ (-s) * f n := by
        apply tsum_congr
...
private lemma pairedEta_eq_zetaEtaFactor_upper_half_plane :
  ...
  have hfactor_diff : DifferentiableOn ℂ zetaEtaFactor U := by
    ...
    have hfactor :
        DifferentiableAt ℂ (fun w : ℂ => 1 - 2 * (2 : ℂ) ^ (-w)) s :=
...
private lemma pairedEta_eq_zetaEtaFactor_of_real {x : ℝ}
  ...
  have hone_div :
      Tendsto (fun n : ℕ => 1 / ((n + 1 : ℕ) : ℂ)) atTop (𝓝 0) := by
  ...
  have hfactor_cont : ContinuousAt zetaEtaFactor (x : ℂ) := by
    ...
    have hpow :
        DifferentiableAt ℂ (fun w : ℂ => (2 : ℂ) ^ (-w)) (x : ℂ) :=
      ((hasDerivAt_neg' (x : ℂ)).const_cpow
        (Or.inl (by norm_num))).differentiableAt
    have hfactor :
        DifferentiableAt ℂ (fun w : ℂ => 1 - 2 * (2 : ℂ) ^ (-w)) (x : ℂ) :=
      (differentiableAt_const (c := (1 : ℂ))).sub
        ((differentiableAt_const (c := (2 : ℂ))).mul hpow)
  ...
  have hleft' :
      Tendsto (zetaEtaFactor ∘ seq) atTop (𝓝 (pairedEta (x : ℂ))) := by
    apply hleft.congr'
...
private theorem riemannZeta_ofReal_negative_of_lt_one {x : ℝ}
    (hx : 0 < x) (hx1 : x < 1) :
  ...
  have hetaPos' : 0 < ∑' n : ℕ, realEtaPairTerm n x := by
    ...
    simpa using hetaPos
  exact ⟨him, by nlinarith [hetaPos']⟩

private theorem positive_real_sign_numeric_check :
    (0 : ℝ) < 1 / 2 ∧ (1 / 2 : ℝ) < 1 ∧ (1 : ℝ) < 2 := by
  norm_num

/-- Riemann zeta is real at every positive real argument other than its pole. -/
theorem riemannZeta_ofReal_im_eq_zero {x : ℝ}
    (hx : 0 < x) (hx1 : x ≠ 1) :
    (riemannZeta (x : ℂ)).im = 0 := by
  rcases lt_or_gt_of_ne hx1 with hxlt | hxgt
  · exact (riemannZeta_ofReal_negative_of_lt_one hx hxlt).1
  · exact riemannZeta_im_eq_zero_of_one_lt hxgt

/-- On the positive real axis, Riemann zeta is negative below one and
positive above one. -/
theorem riemannZeta_ofReal_sign {x : ℝ}
    (hx : 0 < x) (hx1 : x ≠ 1) :
    (riemannZeta (x : ℂ)).im = 0 ∧
      ((x < 1 ∧ (riemannZeta (x : ℂ)).re < 0) ∨
        (1 < x ∧ 0 < (riemannZeta (x : ℂ)).re)) := by
  fail_if_success rfl
  fail_if_success (solve | simp)
  fail_if_success (solve | trivial)
  refine ⟨riemannZeta_ofReal_im_eq_zero hx hx1, ?_⟩
  rcases lt_or_gt_of_ne hx1 with hxlt | hxgt
  · exact Or.inl ⟨hxlt, (riemannZeta_ofReal_negative_of_lt_one hx hxlt).2⟩
  · exact Or.inr ⟨hxgt, riemannZeta_re_pos_of_one_lt hxgt⟩

#print axioms riemannZeta_ofReal_sign

end

end D5.S3.Analytic.Isolation.RiemannZetaPositiveRealSign
