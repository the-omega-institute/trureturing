...
private lemma Q_four_nonneg {u : ℝ} (hu : 0 ≤ u) : 0 ≤ Q 4 u := by
  unfold Q
  norm_num [s]
...
private lemma D_four_reflection (x : ℝ) : D 4 (4 - x) = -D 4 x := by
  have hD_nonpos {x : ℝ} (hx : x ≤ 0) : D 4 x = 0 := by
    norm_num [D, T, Finset.sum_range_succ, Nat.choose]
    repeat' rw [max_eq_right (by linarith)]
    norm_num
...
private lemma C_succ_neg_left_core (m : ℕ) (hm : 4 ≤ m) (h : SplineInvariant m)
    {x : ℝ} (hx : x ∈ Icc (s (m + 1)) (((m + 1 : ℕ) : ℝ) / 2)) :
    C (m + 1) x < 0 := by
  let t := x - s m
  ...
  have hs : s (m + 1) = s m + 1 / 2 := by
    ...
    push_cast
    ring
  have hxform : x = s m + t := by dsimp [t]; ring
  have ht0 : 1 / 2 ≤ t := by
    rw [hs] at hx
    dsimp [t]
    linarith [hx.1]
...
private lemma C_succ_neg_core (m : ℕ) (hm : 4 ≤ m) (h : SplineInvariant m)
  ...
  · exact C_succ_neg_left_core m hm h ⟨hx.1, hmid⟩
  · let y := ((m + 1 : ℕ) : ℝ) - x
    ...
    have hreflect (z : ℝ) : C (m + 1) (((m + 1 : ℕ) : ℝ) - z) = C (m + 1) z := by
      ...
      ring
    rw [← hreflect x]
    exact C_succ_neg_left_core m hm h hy

private lemma invariant_of_four_le (m : ℕ) (hm : 4 ≤ m) : SplineInvariant m := by
  induction m, hm using Nat.le_induction with
  | base =>
      refine {
        support := ?_, reflection := D_four_reflection,
        q_nonneg := fun _ hu => Q_four_nonneg hu, q_half_pos := ?_, strict_core := D_four_strictAntiOn_core }
      · intro x hx
        rcases hx with hx | hx
        · norm_num [D, T, Finset.sum_range_succ, Nat.choose]
          repeat' rw [max_eq_right (by linarith)]
        ...
        · norm_num [D, T, Finset.sum_range_succ, Nat.choose]
          ...
          rw [max_eq_left hx0, max_eq_left (by nlinarith [hx]),
            max_eq_left (by nlinarith [hx]), max_eq_left (by nlinarith [hx]),
            max_eq_left (by nlinarith [hx])]
          ring
