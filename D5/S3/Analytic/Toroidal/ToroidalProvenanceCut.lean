...
import D5.S3.Analytic.Adelic.ToroidalJetDepth
import D5.S3.Analytic.Adelic.ToroidalObserverSetCover
import D5.S3.Analytic.Adelic.ToroidalTemperednessCriterion

/- Library-search audit trail (2026-09-02):
   * Exact searches for `ToroidalVanishingProfile`,
     `toroidalVanishingProfile`, `toroidal_provenance_cut`, `periodZero`, and
     `twistZero` missed in both D5 and pinned Mathlib.
   ...
   * `finite_toroidal_spectral_tomography` has a proof-local `periodZero`, and
     `rh_iff_all_toroidal_eisenstein_tempered` uses `mul_eq_zero` internally;
     neither proof-local fact is an addressable per-index characterization.
