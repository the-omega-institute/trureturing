...
private theorem o5_beta_zero_closed_form : o5Beta 0 = 0 := by
  rw [o5_beta_closed_form]
  simp only [Nat.cast_zero, mul_zero, zero_add, Nat.cast_one, one_mul]
...
private theorem natCast_le_o5Beta (v : Nat) :
    (v : Real) <= o5Beta v := by
  cases v with
  | zero => simp [o5_beta_zero_closed_form]
  | succ v =>
