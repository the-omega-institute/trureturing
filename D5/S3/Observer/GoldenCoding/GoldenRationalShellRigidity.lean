...
injectivity theorem. A positive power of the orientation-preserving golden unit
`φ²` is irrational, so a nonzero rational scale cannot be translated by a
...
near-collisions at finite precision, and it does not identify equality in the
additive-circle quotient with an integer shell displacement.
-/
