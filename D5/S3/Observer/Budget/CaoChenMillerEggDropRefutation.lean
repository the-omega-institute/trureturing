...
/-- Critical points and drop locations, with `Fin (N i)` representing the
paper's integer coordinates `1, ..., N_i`. -/
abbrev Point {d : Nat} (N : Fin d -> Nat) := forall i, Fin (N i)

/-- A drop is intact exactly when every queried coordinate is strictly below
the corresponding hidden coordinate. `1` means intact and `0` means broken. -/
def dropOutcome {d : Nat} {N : Fin d -> Nat} (query hidden : Point N) : Fin 2 :=
...
drops. A terminal leaf may occur under unused drop budget. On a broken outcome
one egg is lost; on an intact outcome the egg count is preserved. -/
inductive EggStrategy {d : Nat} (N : Fin d -> Nat) : Nat -> Nat -> Type
  | stop {eggs drops : Nat} (guess : Point N) : EggStrategy N eggs drops
  | drop {eggs drops : Nat} (query : Point N)
