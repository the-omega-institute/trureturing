...
private theorem card_compositionAsSetEquiv {total : ℕ} (htotal : 0 < total)
...
private theorem card_fixedLengthComposition_of_pos (total length : ℕ)
...
private theorem card_fixedLengthComposition (total length : ℕ) :
    Fintype.card {c : Composition total // c.length = length} =
...
private theorem card_indexedComposition (total length : ℕ) :
...
private theorem parity_succ_mod {n : ℕ} (_hn : 2 ≤ n) (a : Fin (2 * n)) :
    ((a.val + 1) % (2 * n)) % 2 = (a.val + 1) % 2 := by
  rw [Nat.mod_mod_of_dvd]
  exact dvd_mul_right 2 n

private theorem pred_mod_succ_mod {N a b : ℕ} (hN : 0 < N) (ha : a < N) (hb : b < N)
...
private theorem succ_mod_of_pred_mod {N a b : ℕ} (hN : 0 < N) (ha : a < N) (hb : b < N)
...
/-- For `n ≥ 2`, an edge between two actual crown vertices in the real CCP graph is exactly a
cycle edge whose endpoints belong to the same partition block.  This identifies the source's
cyclic cut graph inside `crownPartitionGraph`; the exceptional `n = 1` two-vertex graph is excluded. -/
theorem crownPartitionGraph_vertex_adj_iff_cycleGraph_adj {n : ℕ} (hn : 2 ≤ n)
...
private theorem crownPartition_originalBlock_cycleCutInterval {n : ℕ} (hn : 2 ≤ n)
  ...
  have hi_block : i ∈ block := by
    simp [block]
  have hranks : ranks.Nonempty := by
    exact ⟨cycleCutRank c i, Finset.mem_image.mpr ⟨i, hi_block, rfl⟩⟩
  let a := ranks.min' hranks
  let b := ranks.max' hranks
  have ha_mem : a ∈ ranks := by
    exact Finset.min'_mem ranks hranks
  have hb_mem : b ∈ ranks := by
    exact Finset.max'_mem ranks hranks
  ...
  have hamin_ne : amin ≠ c := by
    intro h
    exact hc (h ▸ hamin)
  have hbmax_ne : bmax ≠ c := by
    intro h
    exact hc (h ▸ hbmax)
  ...
  · rintro ⟨hav, hvb⟩
    have hv_rank : cycleCutRank c v ∈ ranks := by
      ...
      · have hav_strict : a < cycleCutRank c v := lt_of_le_of_ne hav (Ne.symm hva)
        ...
        have hdfst_ne : d.fst.1 ≠ c := by
          intro h
          exact hc (h ▸ d.fst.2)
        have hdsnd_ne : d.snd.1 ≠ c := by
          intro h
          exact hc (h ▸ d.snd.2)
        have hdadj : (SimpleGraph.cycleGraph (2 * n)).Adj d.fst.1 d.snd.1 := d.adj
        have hstep := cycleGraph_adj_cycleCutRank (N := 2 * n) (by omega)
          c d.fst.1 d.snd.1 hdfst_ne hdsnd_ne hdadj
        have hfst : cycleCutRank c d.fst.1 < cycleCutRank c v := hd_lower
        have hsnd : cycleCutRank c v ≤ cycleCutRank c d.snd.1 := by
          exact Nat.le_of_not_gt hd_upper
        ...
        have hdsnd_block : d.snd.1 ∈ block := by
          change d.snd.1 ∈ Finset.univ.filter
            (fun j => P.toSetoid.r (.vertex i) (.vertex j))
