/- GID: D5/S3/Combinatorics/Parking/CircularTwoChoiceParkingBijection
   generality: I
   mirror-B: D5/B/S3/Combinatorics/Parking/CircularTwoChoiceParkingBijection
   mirror-E: none(waiver:evidence-not-specified-by-formal-manifest)
   anchors: []
   utility: none
   digest: Canonical two-choice circular parking bijections. -/

import D5.S3.Combinatorics.Parking.FixedIncrementFiberTransport
import D5.S3.Combinatorics.Parking.CutRunCorrespondence
...
section BoundaryConsumers

example {n : Nat} (occupied : List (Spot n)) (q : ActualChoice n)
    (hanchor : q.anchor ∉ occupied) :
    actualStep n occupied q = q.anchor := by
  simp [actualStep, hanchor]

example {n : Nat} (occupied : List (Spot n)) (q : ActualChoice n)
    (hanchor : q.anchor ∈ occupied) (hsecond : q.second ∉ occupied) :
    actualStep n occupied q = q.second := by
  simp [actualStep, hanchor, firstFree_of_not_mem, hsecond]

example :
    actualStep 1 []
      ({ anchor := 0, second := 1, distinct := by decide } : ActualChoice 1) = 0 := by
  rfl

example :
    actualStep 1 [0]
      ({ anchor := 0, second := 1, distinct := by decide } : ActualChoice 1) = 1 := by
  change firstFree 1 [0] 1 = 1
  exact firstFree_of_not_mem [0] 1 (by decide)

end BoundaryConsumers
