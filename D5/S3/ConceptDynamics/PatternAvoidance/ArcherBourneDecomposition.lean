...
noncomputable def pattern312 : Equiv.Perm (Fin 3) :=
  Equiv.ofBijective (fun i : Fin 3 => ![2, 0, 1] i) (by decide)

/-- The permutation pattern `321`, in zero-based value notation `[2, 1, 0]`. -/
noncomputable def pattern321 : Equiv.Perm (Fin 3) :=
  Equiv.ofBijective (fun i : Fin 3 => ![2, 1, 0] i) (by decide)
...
private abbrev Before {d : List Nat} : Position d → Position d → Prop :=
  Sigma.Lex (fun i j : Fin d.length => i < j) (fun _ x y => x < y)
...
private lemma before_block_le {d : List Nat} {p q : Position d} (h : Before p q) :
    p.1.val ≤ q.1.val := by
  rcases h with ⟨_, _, hij⟩ | ⟨_, _, _⟩
  · exact Nat.le_of_lt hij
  · exact Nat.le_refl _

private lemma before_same_block {d : List Nat} {i : Fin d.length}
    {x y : Fin (d.get i)} (h : Before (⟨i, x⟩ : Position d) ⟨i, y⟩) : x < y := by
  rcases h with ⟨_, _, hii⟩ | ⟨_, _, hxy⟩
  · exact (lt_irrefl _ hii).elim
  · exact hxy
...
private lemma before_iff_fin_lt {d : List Nat} (p q : Position d) :
  ...
  rcases p with ⟨i, x⟩
  rcases q with ⟨j, y⟩
  change Sigma.Lex (fun i j : Fin d.length => i < j)
      (fun i (x y : Fin (d.get i)) => x < y)
      ⟨i, x⟩ ⟨j, y⟩ ↔ _
  change _ ↔
    ((@finSigmaFinEquiv d.length (fun i => d.get i)) ⟨i, x⟩ : Nat) <
      ((@finSigmaFinEquiv d.length (fun i => d.get i)) ⟨j, y⟩ : Nat)
  rw [finSigmaFinEquiv_val, finSigmaFinEquiv_val]
  simp only [Fin.lt_def]
  constructor
  · intro h
    rcases h with ⟨x, y, hij⟩ | ⟨x, y, hxy⟩
    · have hbound := blockOffset_add_block_le hij
      have hxLt : x.val < d.get _ := x.isLt
      omega
    · omega
  · intro hlt
    by_cases hij : i < j
    ...
    · have hidx : i = j := Fin.ext (by omega)
      subst j
      exact Sigma.Lex.right _ _ (by omega)
...
private lemma tagged_no_312 {d : List Nat} (hpos : ∀ i : Fin d.length, 0 < d.get i) :
  ...
  rintro ⟨a, b, c, hab, hbc, hbcv, hcav⟩
  have hac : Before a c := _root_.trans hab hbc
  ...
  subst j
  subst k
  have hxy : x.val < y.val := by simpa only [Fin.lt_def] using before_same_block hab
  have hyz : y.val < z.val := by simpa only [Fin.lt_def] using before_same_block hbc
  have hyzv : (y.val + 1) % d.get i < (z.val + 1) % d.get i := by
    simpa only [Fin.lt_def] using before_same_block hbcv
  have hxzv : (z.val + 1) % d.get i < (x.val + 1) % d.get i := by
    simpa only [Fin.lt_def] using before_same_block hcav
...
private lemma tagged_no_321 {d : List Nat} (hpos : ∀ i : Fin d.length, 0 < d.get i) :
  ...
  have hyxv : (y.val + 1) % d.get i < (x.val + 1) % d.get i := by
    simpa only [Fin.lt_def] using before_same_block hbav
  have hzyv : (z.val + 1) % d.get i < (y.val + 1) % d.get i := by
    simpa only [Fin.lt_def] using before_same_block hcbv
...
private lemma finRotate_le_iff_last {m : Nat} (j : Fin m) :
  ...
  obtain ⟨m, rfl⟩ :=
    Nat.exists_eq_succ_of_ne_zero (Nat.ne_of_gt (Nat.zero_lt_of_lt j.isLt))
  have hrotate := lt_finRotate_iff_ne_last j
  constructor
  · intro hle
    have hnot : ¬j < finRotate (m + 1) j := not_lt.mpr hle
    have hlast : j = Fin.last m :=
      not_ne_iff.mp ((not_congr hrotate).mp hnot)
  ...
  · intro hval
    have hlast : j = Fin.last m := by
      apply Fin.ext
      simpa using hval
    have hnot : ¬j < finRotate (m + 1) j :=
      (not_congr hrotate).mpr (not_ne_iff.mpr hlast)
...
private theorem rotationSumComposition_cube_avoids_2143_iff {n : Nat} (c : Composition n) :
    (¬Contains2143 (⇑(rotationSumComposition c ^ 3))) ↔
