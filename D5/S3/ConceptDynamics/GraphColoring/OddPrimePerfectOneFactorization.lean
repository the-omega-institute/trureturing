...
/-- Every odd prime gives a complete graph admitting a perfect one-factorization. -/
theorem exists_odd_prime_perfect_one_factorization (hp2 : p ≠ 2) :
    ∃ factors : ZMod p → SimpleGraph (Vertex p),
      (∀ a : ZMod p, (⊤ : (factors a).Subgraph).IsPerfectMatching) ∧
      (∀ u v : Vertex p, u ≠ v → ∃! a : ZMod p, (factors a).Adj u v) ∧
      (∀ a b : ZMod p, a ≠ b → (factors a ⊔ factors b).IsHamiltonian) := by
  exact ⟨factor, odd_prime_perfect_one_factorization hp2⟩
...
#print axioms exists_odd_prime_perfect_one_factorization
