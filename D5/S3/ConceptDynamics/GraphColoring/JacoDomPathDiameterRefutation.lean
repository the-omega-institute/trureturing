/- GID: D5/S3/ConceptDynamics/GraphColoring/JacoDomPathDiameterRefutation
   ...
   anchors: [mathlib/module/Mathlib.Combinatorics.SimpleGraph.Hasse, mathlib/module/Mathlib.Combinatorics.SimpleGraph.Diam]
...
private def within {V : Type*} [Fintype V] [DecidableEq V]
    (G : SimpleGraph V) [DecidableRel G.Adj] : ℕ → V → Finset V
  | 0, u => {u}
  | k + 1, u => Finset.univ.filter fun v =>
      ∃ w ∈ within G k u, v = w ∨ G.Adj w v

private theorem exists_walk_of_mem_within {V : Type*} [Fintype V] [DecidableEq V]
    (G : SimpleGraph V) [DecidableRel G.Adj] {k : ℕ} {u v : V}
    (hv : v ∈ within G k u) :
    ∃ p : G.Walk u v, p.length ≤ k := by
  induction k generalizing v with
  | zero =>
      simp only [within, Finset.mem_singleton] at hv
      subst v
      exact ⟨.nil, by simp⟩
  | succ k ih =>
      simp only [within, Finset.mem_filter, Finset.mem_univ, true_and] at hv
      rcases hv with ⟨w, hw, rfl | hadj⟩
      · obtain ⟨p, hp⟩ := ih hw
        exact ⟨p, hp.trans (by omega)⟩
      · obtain ⟨p, hp⟩ := ih hw
        exact ⟨p.concat hadj, by simp only [SimpleGraph.Walk.length_concat]; omega⟩
...
theorem result : ¬ claim := by
  ...
  have jaco33_diam_le_seven : (jaco 33).diam ≤ 7 := by
    have jaco33_ediam_le_seven : (jaco 33).ediam ≤ (7 : ℕ∞) := by
      have hfull : ∀ u : V33, within (jaco 33) 7 u = Finset.univ := by
        decide +kernel
      exact SimpleGraph.ediam_le_of_edist_le fun u v => by
        have hv : v ∈ within (jaco 33) 7 u := by
          rw [hfull u]
          simp
        obtain ⟨p, hp⟩ := exists_walk_of_mem_within (jaco 33) hv
        calc
          (jaco 33).edist u v ≤ p.length := p.edist_le
          _ ≤ (7 : ℕ∞) := by exact_mod_cast hp
    rw [SimpleGraph.diam]
