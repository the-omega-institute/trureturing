/- GID: D5/S3/ConceptDynamics/ExperimentDesign/FinitePrefixInfiniteCompletionSeparation
   generality: I
   mirror-B: D5/B/S3/ConceptDynamics/ExperimentDesign/FinitePrefixInfiniteCompletionSeparation
...
/- Library-search audit trail (2026-08-26):
   * The exact current-tree search found the canonical Bernoulli observation system
     `InfiniteIdentificationFiniteInexactness`, including its `marginal`, `productLaw`,
     `stateLaw`, `finiteTranscript`, and `distinguishingEvent` objects.
   * Exact pinned-Mathlib hits `Measure.map_infinitePi_infinitePi_of_inj`,
     `Measure.infinitePi_eq_pi`, and `Measure.pi_singleton` identify every finite-prefix
     law with a full-support finite product. `Measure.MutuallySingular` states the
     completed-law clause directly.
   * Body-shape searches found no family definition of the mapped finite-prefix law,
     so the public theorem exposes the canonical `Measure.map (finiteTranscript m)`
     expression directly and introduces no sibling law definition.
-/

set_option autoImplicit false
set_option relaxedAutoImplicit false

namespace D5.S3.ConceptDynamics.ExperimentDesign.FinitePrefixInfiniteCompletionSeparation
...
/-- The canonical two-state Bernoulli observation system has mutually absolutely
...
end D5.S3.ConceptDynamics.ExperimentDesign.FinitePrefixInfiniteCompletionSeparation
