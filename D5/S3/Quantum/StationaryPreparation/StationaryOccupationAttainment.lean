...
theorem stationary_attainment_full [Nonempty σ] (a : Multiset σ) (head : σ)
    ...
    ∃ (e : PaddingTransition.K a head ≃ Fin (PaddingTransition.N a))
      ...
      V = (PaddingResidualAction.tensorCoordinates (σ := σ) (PaddingTransition.N a)).toLinearIsometry.comp
        (PaddingResidualAction.emissionCoordinates a head e) ∧
  ...
  classical
  have phi_gram (a : Multiset σ) (head : σ) :
      Matrix.gram ℂ (fun r : PaddingTransition.Box a => PaddingResidualGram.phi a head (PaddingResidualGram.occ a r)) = PaddingResidualGram.G a head := by
    ...
    have same_tail_count {σ : Type u} [Fintype σ] [DecidableEq σ] (head : σ) (r s : Multiset σ)
        (h : PaddingResidualGram.tailOcc head r = PaddingResidualGram.tailOcc head s) (i : σ) (hi : i ≠ head) :
        r.count i = s.count i := by
      simpa [PaddingResidualGram.tailOcc, Multiset.count_filter, hi] using congrArg (Multiset.count i) h
    have same_tail_le {σ : Type u} [Fintype σ] [DecidableEq σ] (head : σ) (r s : Multiset σ)
        (h : PaddingResidualGram.tailOcc head r = PaddingResidualGram.tailOcc head s) : r ≤ s ↔ r.count head ≤ s.count head := by
      constructor
      · intro hrs
    ...
    have tail_sub_zero_iff {σ : Type u} [Fintype σ] [DecidableEq σ] (head : σ) (r s : Multiset σ) (h : s ≤ r) :
        PaddingResidualGram.tailOcc head (r - s) = 0 ↔ PaddingResidualGram.tailOcc head r = PaddingResidualGram.tailOcc head s := by
      have hle : PaddingResidualGram.tailOcc head s ≤ PaddingResidualGram.tailOcc head r := Multiset.filter_le_filter _ h
      rw [PaddingResidualGram.tailOcc, Multiset.filter_sub, tsub_eq_zero_iff_le]
      exact ⟨fun h' => le_antisymm h' hle, fun h' => h'.le⟩
    have head_only_M {σ : Type u} [Fintype σ] [DecidableEq σ] (head : σ) (r : Multiset σ) (h : PaddingResidualGram.tailOcc head r = 0) :
        PaddingResidualGram.M r = 1 := by
    ...
    have scale_pos {σ : Type u} [Fintype σ] [DecidableEq σ] (r : Multiset σ) : 0 < PaddingResidualGram.residualScale r := by
      exact Real.sqrt_pos.mpr (by exact_mod_cast multiplicity_pos r rfl)
    have scale_sq {σ : Type u} [Fintype σ] [DecidableEq σ] (r : Multiset σ) : PaddingResidualGram.residualScale r ^ 2 = (multiplicity r.card r : ℝ) := by
      exact Real.sq_sqrt (Nat.cast_nonneg _)
    have normalized_scalar {σ : Type u} [Fintype σ] [DecidableEq σ] (r s : Multiset σ) :
        (PaddingResidualGram.residualScale r : ℂ)⁻¹ * ((PaddingResidualGram.residualScale s : ℂ)⁻¹ * (PaddingResidualGram.M s : ℂ)) =
          (Real.sqrt ((PaddingResidualGram.M s : ℝ) / (PaddingResidualGram.M r : ℝ)) : ℂ) := by
      have h : (PaddingResidualGram.residualScale r)⁻¹ * ((PaddingResidualGram.residualScale s)⁻¹ * (PaddingResidualGram.M s : ℝ)) =
          Real.sqrt ((PaddingResidualGram.M s : ℝ) / (PaddingResidualGram.M r : ℝ)) := by
        rw [Real.sqrt_div (Nat.cast_nonneg _)]
        change (PaddingResidualGram.residualScale r)⁻¹ * ((PaddingResidualGram.residualScale s)⁻¹ * (PaddingResidualGram.M s : ℝ)) =
          PaddingResidualGram.residualScale s / PaddingResidualGram.residualScale r
        field_simp [(scale_pos r).ne', (scale_pos s).ne']
        simpa only [pow_two, PaddingResidualGram.M] using (scale_sq s).symm
      exact_mod_cast h
    have phi_inner_of_le {σ : Type u} [Fintype σ] [DecidableEq σ] (a : Multiset σ) (head : σ) (r s : Multiset σ)
        (hr : r ≤ a) (hs : s ≤ a) (hle : s ≤ r) :
        inner ℂ (PaddingResidualGram.phi a head r) (PaddingResidualGram.phi a head s) =
          (Real.sqrt (((PaddingResidualGram.M s : ℝ) * (PaddingResidualGram.M (r - s) : ℝ)) / (PaddingResidualGram.M r : ℝ)) : ℂ) * PaddingResidualGram.z head (r - s) := by
      simp only [PaddingResidualGram.phi, inner_smul_left, inner_smul_right, map_inv₀,
        Complex.conj_ofReal]
      rw [PaddingResidualGram.padding_inner a head r s hr hs]
      change (PaddingResidualGram.residualScale s : ℂ)⁻¹ * ((PaddingResidualGram.residualScale r : ℂ)⁻¹ *
    ...
    ext r s
    change inner ℂ (PaddingResidualGram.phi a head (PaddingResidualGram.occ a r)) (PaddingResidualGram.phi a head (PaddingResidualGram.occ a s)) = PaddingResidualGram.G a head r s
    rw [PaddingResidualGram.G]
    ...
    · rw [if_neg hsr]
      ...
      · rw [if_neg hrs]
        ...
        dsimp only [PaddingResidualGram.tailOcc] at ht
        simp only [PaddingResidualGram.phi, inner_smul_left, inner_smul_right,
          PaddingResidualGram.padding_inner a head _ _ (box_occupation_le a r) (box_occupation_le a s),
          if_neg ht, mul_zero]

  have phi_norm (a : Multiset σ) (head : σ) (r : Multiset σ) (hr : r ≤ a) :
      ‖PaddingResidualGram.phi a head r‖ = 1 := by
    ...
    have hp : 0 < PaddingResidualGram.residualScale r :=
    ...
    have hn : ‖PaddingResidualGram.padding a head r‖ = PaddingResidualGram.residualScale r := by
      have hh : ‖PaddingResidualGram.padding a head r‖ ^ 2 = (multiplicity r.card r : ℝ) := by
        ...
        rfl
      have hsq : PaddingResidualGram.residualScale r ^ 2 = (multiplicity r.card r : ℝ) :=
        Real.sq_sqrt (Nat.cast_nonneg _)
      nlinarith [norm_nonneg (PaddingResidualGram.padding a head r)]
    change ‖(PaddingResidualGram.residualScale r : ℂ)⁻¹ • PaddingResidualGram.padding a head r‖ = 1
    rw [norm_smul, norm_inv, Complex.norm_real, Real.norm_eq_abs,
      abs_of_pos hp, hn, inv_mul_cancel₀ hp.ne']

  have image_coordinates (a : Multiset σ) (head : σ)
      (r : Multiset σ) (hr : r ≤ a) :
      ((EuclideanSpace.basisFun σ ℂ).tensorProduct
        (EuclideanSpace.basisFun (PaddingTransition.K a head) ℂ)).repr (PaddingResidualAction.image a head r) =
          PaddingResidualAction.emitLinear a head (PaddingResidualGram.phi a head r) := by
    have emit_linear_basis  (s : PaddingTransition.K a head)
        (i : σ) (k : PaddingTransition.K a head) : PaddingResidualAction.emitLinear a head (basis s) (i, k) = PaddingTransition.W a head (i, k) s := by
  ...
  have prescribed_image_gram (a : Multiset σ) (head : σ)
      (r s : Multiset σ)
      (hr : r ≤ a) (hs : s ≤ a) :
      inner ℂ (PaddingResidualAction.image a head r) (PaddingResidualAction.image a head s) = inner ℂ (PaddingResidualGram.phi a head r) (PaddingResidualGram.phi a head s) := by
  ...
  have padding_gram_data (a : Multiset σ) (head : σ) :
      (∀ b : PaddingTransition.Tail a head,
        PaddingResidualGram.block a head b = PaddingResidualGram.L (a.count head) *
          Matrix.diagonal (fun j : Fin (a.count head + 1) => (PaddingResidualGram.delta a head b j.val : ℂ)) *
            (PaddingResidualGram.L (a.count head)).transpose ∧
    ...
    have tail_occ_card (r : Multiset σ) :
      ...
      have ht : PaddingResidualGram.tailCount head (PaddingResidualGram.tailOcc head r) =
          PaddingResidualGram.tailCount head r := by
        apply Finset.sum_congr rfl
        intro i _
    ...
    have gram_congruence : PaddingResidualGram.D a * PaddingResidualGram.G a head * PaddingResidualGram.D a = PaddingResidualGram.B a head := by
      rw [← phi_gram a head, ← padding_gram_eq_diagonal, padding_gram_eq_B]
  ...
  have prescribed_dependencies
    ...
    have hi (j : J) : E (PaddingResidualAction.image a head (r j)) =
      ...
      image_coordinates a head (r j) (hr j)
    apply E.injective
  ...
  have emit_linear_sink :
    ...
    have h := congrArg (fun x : Space (σ × PaddingTransition.K a head) => x (i, k))
      (image_coordinates a head 0 (Multiset.zero_le _))
