...
escape_witness: form (2) — the three-term recurrence of X ↦ {S^+, {S^-, X}} on the count
  patterns Z_r (the local `step_z` inside `result`) on the live path of the span argument
...
Direct frozen dependencies: D5/S3/Quantum/Dynamics/PronkoFredkinNonCyclicAnnihilation
  (sigmaPlus, sigmaMinus, tensor, site, sigmaPow, Sigma)
-/
...
theorem result : claim := by
  have cnt_nonneg {N : ℕ} (p q : Fin 2) (y x : Fin N → Fin 2) : 0 ≤ cnt p q y x :=
    Finset.sum_nonneg fun i _ => by split_ifs <;> norm_num
  have cnt_partition {N : ℕ} (y x : Fin N → Fin 2) :
    ...
    simp [h]
  have cnt_update_left {N : ℕ} (p q : Fin 2) (y x : Fin N → Fin 2) (j : Fin N)
      (v : Fin 2) :
    ...
    have h : cnt p q (Function.update y j v) x - cnt p q y x =
      ...
      · simp
    linarith
  have cnt_update_right {N : ℕ} (p q : Fin 2) (y x : Fin N → Fin 2) (j : Fin N)
      (v : Fin 2) :
    ...
    have h : cnt p q y (Function.update x j v) - cnt p q y x =
      ...
      · simp
    linarith
  have sum_indicator {N : ℕ} (p q : Fin 2) (y x : Fin N → Fin 2) (K : ℂ) :
    ...
    split_ifs <;> simp
  have site_apply {N : ℕ} (j : Fin N) (A : Matrix (Fin 2) (Fin 2) ℂ)
      (y z : Fin N → Fin 2) :
    ...
    · rw [if_neg hz]
      ...
      simp only [hij, if_false, Matrix.one_apply, hyz]
  have site_mul_apply {N : ℕ} (j : Fin N) (A : Matrix (Fin 2) (Fin 2) ℂ)
    ...
    simp
  have mul_site_apply {N : ℕ} (j : Fin N) (A : Matrix (Fin 2) (Fin 2) ℂ)
      (M : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ) (y x : Fin N → Fin 2) :
    ...
    rw [Finset.sum_ite_eq']
    simp
  have minus_mul_apply {N : ℕ} (M : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ)
      (y x : Fin N → Fin 2) :
      (totalMinus N * M) y x =
  ...
  have mul_minus_apply {N : ℕ} (M : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ)
      (y x : Fin N → Fin 2) :
      (M * totalMinus N) y x =
    ...
    generalize x j = u
    fin_cases u <;> simp [sigmaMinus]
  have plus_mul_apply {N : ℕ} (M : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ)
      (y x : Fin N → Fin 2) :
      (totalPlus N * M) y x =
  ...
  have mul_plus_apply {N : ℕ} (M : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ)
      (y x : Fin N → Fin 2) :
      (M * totalPlus N) y x =
    ...
    generalize x j = u
    fin_cases u <;> simp [sigmaPlus]
  have partition_cast {N : ℕ} (y x : Fin N → Fin 2) :
  ...
  have antiAd_minus_patt {N : ℕ} (F : ℤ → ℤ → ℂ) :
  ...
  have antiAd_plus_patt {N : ℕ} (F : ℤ → ℤ → ℂ) :
  ...
  have antiAd_plus_e {N : ℕ} (s : ℤ) :
    ...
    · by_cases h2 : b = s + 1 ∧ c = s
      ...
      · rw [if_neg h1, if_neg h2]
        ring
  have antiAd_smul_add {N : ℕ} (a X Y : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ)
  ...
  have step_z {N : ℕ} (r : ℤ) :
    ...
    push_cast
    ring_nf
  have krylov_mono {N n m : ℕ} (h : n ≤ m) : krylov N n ≤ krylov N m := by
    apply Submodule.span_mono
    rintro _ ⟨i, rfl⟩
    exact ⟨Fin.castLE h i, rfl⟩
  have krylov_step {N n : ℕ} {X : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ}
  ...
  have cnt_eq_zero_iff {N : ℕ} (p q : Fin 2) (y x : Fin N → Fin 2) :
    ...
    · intro h i _
      rw [if_neg (h i)]
  have ePatt_zero (N : ℕ) : ePatt N 0 = 1 := by
    ext y x
    ...
    · rw [if_neg hyx, if_neg]
      ...
      revert a b
      fin_cases u <;> fin_cases w <;> decide
  have zPatt_neg_one (N : ℕ) : zPatt N (-1) = 0 := by
    ext y x
    ...
    omega
  have totalPlus_eq_z (N : ℕ) : totalPlus N = zPatt N 0 := by
  ...
  have zPatt_mem (N : ℕ) : ∀ r : ℕ, zPatt N r ∈ krylov N (r + 1) := by
    intro r
    ...
    | _ r ih =>
      ...
      · have hr : zPatt N r ∈ krylov N (r + 2) :=
        ...
        push_cast
        exact this
  have sigma_apply {N : ℕ} (ε : ℤ) (y x : Fin N → Fin 2) :
      Sigma N ε y x = if cnt 0 1 y x - cnt 1 0 y x = ε then 1 else 0 := by
    classical
    -- every Kronecker factor of (3.1) is `0` or `1`
    have h01 : ∀ (q : Fin 3) (a b : Fin 2), sigmaPow q a b = 0 ∨ sigmaPow q a b = 1 := by
      intro q a b
      fin_cases q <;> fin_cases a <;> fin_cases b <;>
        simp [sigmaPow, sigmaPlus, sigmaMinus, Matrix.one_apply]
    -- a factor equal to `1` fixes its exponent by the site pair
    have hexp : ∀ (q : Fin 3) (a b : Fin 2), sigmaPow q a b = 1 →
        ((q : ℕ) : ℤ) - 1 =
          (if a = 0 ∧ b = 1 then 1 else 0) - (if a = 1 ∧ b = 0 then 1 else 0) := by
      intro q a b
      fin_cases q <;> fin_cases a <;> fin_cases b <;>
        simp [sigmaPow, sigmaPlus, sigmaMinus, Matrix.one_apply]
    -- at each site pair exactly one exponent gives the factor `1`
    have hone : ∀ a b : Fin 2,
        (Finset.univ.filter fun q : Fin 3 => sigmaPow q a b = 1).card = 1 := by
      intro a b
      rw [Finset.card_eq_one]
      fin_cases a <;> fin_cases b
      · exact ⟨1, by
          ext q; rw [Finset.mem_filter, Finset.mem_singleton]
          fin_cases q <;> simp [sigmaPow, sigmaPlus, sigmaMinus, Matrix.one_apply]⟩
      · exact ⟨2, by
          ext q; rw [Finset.mem_filter, Finset.mem_singleton]
          fin_cases q <;> simp [sigmaPow, sigmaPlus, sigmaMinus, Matrix.one_apply]⟩
      · exact ⟨0, by
      ...
      · exact ⟨1, by
          ext q; rw [Finset.mem_filter, Finset.mem_singleton]
          fin_cases q <;> simp [sigmaPow, sigmaPlus, sigmaMinus, Matrix.one_apply]⟩
    have hprod : ∀ r : Fin N → Fin 3, tensor N (fun i => sigmaPow (r i)) y x =
        if ∀ i, sigmaPow (r i) (y i) (x i) = 1 then 1 else 0 := by
      intro r
      rw [tensor]
      by_cases h : ∀ i, sigmaPow (r i) (y i) (x i) = 1
      · rw [if_pos h]
        exact Finset.prod_eq_one fun i _ => h i
      · rw [if_neg h]
        push Not at h
        obtain ⟨i, hi⟩ := h
        exact Finset.prod_eq_zero (Finset.mem_univ i) ((h01 _ _ _).resolve_right hi)
    have hsum : ∀ r : Fin N → Fin 3, (∀ i, sigmaPow (r i) (y i) (x i) = 1) →
        (∑ i, (((r i : ℕ) : ℤ) - 1)) = cnt 0 1 y x - cnt 1 0 y x := by
      intro r hr
      unfold cnt
      rw [← Finset.sum_sub_distrib]
      exact Finset.sum_congr rfl fun i _ => hexp _ _ _ (hr i)
    set T := Fintype.piFinset fun i => Finset.univ.filter fun q : Fin 3 =>
      sigmaPow q (y i) (x i) = 1 with hTdef
    have hT : T.card = 1 := by
      rw [hTdef, Fintype.card_piFinset]
      exact Finset.prod_eq_one fun i _ => hone (y i) (x i)
    have hmemT : ∀ r, r ∈ T ↔ ∀ i, sigmaPow (r i) (y i) (x i) = 1 := by
      intro r
      simp [hTdef, Fintype.mem_piFinset]
    rw [PronkoFredkinNonCyclicAnnihilation.Sigma, Matrix.sum_apply]
    simp only [hprod]
    rw [Finset.sum_boole]
    have hfilter : ((Finset.univ : Finset (Fin N → Fin 3)).filter
        (fun r => (∑ i, (((r i : ℕ) : ℤ) - 1)) = ε)).filter
        (fun r => ∀ i, sigmaPow (r i) (y i) (x i) = 1) =
        if cnt 0 1 y x - cnt 1 0 y x = ε then T else ∅ := by
      ext r
      ...
      · rw [hmemT]
        ...
        · rintro ⟨_, hr⟩
          exact hr
        · intro hr
          exact ⟨(hsum r hr).trans h, hr⟩
      · simp only [Finset.notMem_empty, iff_false, not_and]
        intro hp hr
        exact h ((hsum r hr).symm.trans hp)
    rw [hfilter]
    split_ifs <;> simp [hT]
  have sigma_plus_eq_sum (N : ℕ) :
    ...
    · rw [if_neg h]
      ...
      apply h
      omega
  have cnt_swap {N : ℕ} (p q : Fin 2) (y x : Fin N → Fin 2) :
      cnt p q x y = cnt q p y x := by
    unfold cnt
    refine Finset.sum_congr rfl fun i _ => ?_
    simp only [and_comm]
  have sigma_minus_eq_transpose (N : ℕ) : Sigma N (-1) = (Sigma N 1)ᵀ := by
    ext y x
    rw [Matrix.transpose_apply, sigma_apply, sigma_apply, cnt_swap 0 1 y x, cnt_swap 1 0 y x]
    congr 1
    apply propext
    omega
  have site_transpose {N : ℕ} (j : Fin N) (A : Matrix (Fin 2) (Fin 2) ℂ) :
    ...
    · simp only [hij, if_false, Matrix.one_apply]
      simp [eq_comm]
  have totalPlus_transpose (N : ℕ) : (totalPlus N)ᵀ = totalMinus N := by
    have h : sigmaPlusᵀ = sigmaMinus := by
      ext a b
      fin_cases a <;> fin_cases b <;> rfl
    simp only [totalPlus, totalMinus, Matrix.transpose_sum, site_transpose, h]
  have totalMinus_transpose (N : ℕ) : (totalMinus N)ᵀ = totalPlus N := by
    rw [← totalPlus_transpose, Matrix.transpose_transpose]
  have antiAd_transpose {N : ℕ} (a b : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ) :
      (antiAd a b)ᵀ = antiAd aᵀ bᵀ := by
    simp only [antiAd, Matrix.transpose_add, Matrix.transpose_mul]
    abel
  have iterate_transpose (N k : ℕ) (X : Matrix (Fin N → Fin 2) (Fin N → Fin 2) ℂ) :
