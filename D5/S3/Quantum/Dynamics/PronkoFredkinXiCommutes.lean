...
escape_witness: move_up / move_down (the depth parity of a balanced word is invariant under every periodic Fredkin exchange)
admission_basis: escape-witness; open-problem-resolution requested (issue #9996)
Direct frozen dependencies: D5/S3/Quantum/Dynamics/PronkoFredkinNonCyclicAnnihilation (nUp, nDown, tensor, site, P, Pi, F, H)
-/
...
private lemma pathHeight_eq {N : ℕ} (ℓ : Fin N → Fin 2) (i : ℕ) :
    pathHeight ℓ i = ∑ t ∈ Finset.univ.filter (fun t : Fin N => t.val < i), stepVal (ℓ t) :=
  rfl

private lemma pathHeight_zero {N : ℕ} (ℓ : Fin N → Fin 2) : pathHeight ℓ 0 = 0 := by
  simp [pathHeight]

private lemma pathHeight_succ {N : ℕ} (ℓ : Fin N → Fin 2) (i : ℕ) (hi : i < N) :
...
private lemma stepVal_bound (l : Fin 2) : stepVal l = 1 ∨ stepVal l = -1 := by
  unfold stepVal
  split_ifs <;> simp
...
private lemma neg_height_le_depth {N : ℕ} (ℓ : Fin N → Fin 2) {i : ℕ} (hi : i ≤ N) :
    -pathHeight ℓ i ≤ depth ℓ :=
  Finset.le_sup' (fun i => -pathHeight ℓ i) (Finset.mem_range.2 (by omega))

private lemma depth_le_iff {N : ℕ} (ℓ : Fin N → Fin 2) (c : ℤ) :
...
private lemma depth_attained {N : ℕ} (ℓ : Fin N → Fin 2) :
...
private lemma depth_nonneg {N : ℕ} (ℓ : Fin N → Fin 2) : 0 ≤ depth ℓ := by
  have := neg_height_le_depth ℓ (Nat.zero_le N)
  rwa [pathHeight_zero, neg_zero] at this

private lemma inClass_diag_iff {N : ℕ} (a : ℕ) (ℓ : Fin N → Fin 2) :
...
/-- Heights move by at most one per step. -/
private lemma height_step_bound {N : ℕ} (ℓ : Fin N → Fin 2) {i j : ℕ} (hij : i ≤ j)
...
private lemma two_depth_le {N : ℕ} (ℓ : Fin N → Fin 2) (h0 : pathHeight ℓ N = 0) :
    2 * depth ℓ ≤ N := by
...
/-- If two words agree on the letters with index in `[i, j)`, the height difference is
the same at `i` and at `j`. -/
private lemma height_diff_const {N : ℕ} (x y : Fin N → Fin 2) {i j : ℕ} (hij : i ≤ j)
...
/-- Changing heights only at one index `p`, where the smaller value is attained
elsewhere in both words, does not change the depth. -/
private lemma depth_eq_of_local {N : ℕ} (x y : Fin N → Fin 2) (p q : ℕ) (hq : q ≤ N)
...
/-- Shifting all interior heights by `δ`, with an interior witness at height `≤ 0` in both
words, shifts the depth by `-δ`. -/
private lemma depth_eq_of_shift {N : ℕ} (x y : Fin N → Fin 2) (δ : ℤ) (q : ℕ)
...
/-- Exchanging the adjacent letters `p, p + 1` changes the height only at `p + 1`. -/
private lemma height_swap_adjacent {N : ℕ} (x : Fin N → Fin 2) (p : ℕ) (hp : p + 1 < N) :
    ∀ i ≤ N, i ≠ p + 1 →
...
/-- Exchanging the first and the last letter shifts every interior height by the same
amount and keeps the final height. -/
private lemma height_swap_ends {N : ℕ} (x : Fin N → Fin 2) (hN : 2 ≤ N) :
    (∀ i, 0 < i → i < N →
...
private lemma neg_one_pow_depth_eq {x y : Fin N → Fin 2} (m : ℤ)
...
private lemma xiVal_eq (N : ℕ) (w : Fin N → Fin 2) :
...
/-- Words related by an exchange with even depth change have the same `Ξ`-value. -/
private lemma xiVal_congr {N : ℕ} (x y : Fin N → Fin 2)
...
private lemma stepVal_ge (l : Fin 2) : -1 ≤ stepVal l := by
  unfold stepVal
  split_ifs <;> norm_num

private lemma stepVal_le (l : Fin 2) : stepVal l ≤ 1 := by
  unfold stepVal
  split_ifs <;> norm_num

private lemma stepVal_zero : stepVal 0 = 1 := by simp [stepVal]

private lemma stepVal_one : stepVal 1 = -1 := by simp [stepVal]

private lemma fin_ne {N a b : ℕ} (ha : a < N) (hb : b < N) (h : a ≠ b) :
    (⟨a, ha⟩ : Fin N) ≠ ⟨b, hb⟩ :=
  fun e => h (congrArg Fin.val e)

private lemma height_add_two {N : ℕ} (w : Fin N → Fin 2) (i : ℕ) (hi : i + 1 < N) :
    pathHeight w (i + 1 + 1) =
      pathHeight w i + stepVal (w ⟨i, by omega⟩) + stepVal (w ⟨i + 1, hi⟩) := by
  rw [pathHeight_succ w (i + 1) hi, pathHeight_succ w i (by omega)]

private lemma height_last {N : ℕ} (w : Fin N → Fin 2) (hN : 1 ≤ N) :
...
private lemma swapped_eq {N : ℕ} (a b : Fin N) (w : Fin N → Fin 2) :
    ((Equiv.swap a b).arrowCongr (Equiv.refl (Fin 2))) w = fun t => w (Equiv.swap a b t) := by
  funext t
  simp [Equiv.arrowCongr_apply, Equiv.symm_swap]

/-- Exchange of type `n↑_j Π_{j+1,j+2}`: with `ℓ_j = ↑`, exchanging the letters at `j + 1`
and `j + 2` (mod `N`) keeps the `Ξ`-value. -/
private lemma move_up {N : ℕ} (hN : 3 ≤ N) (j : Fin N) (w : Fin N → Fin 2) (hw : w j = 0) :
...
/-- Exchange of type `Π_{j,j+1} n↓_{j+2}`: with `ℓ_{j+2} = ↓`, exchanging the letters at `j`
and `j + 1` (mod `N`) keeps the `Ξ`-value. -/
private lemma move_down {N : ℕ} (hN : 3 ≤ N) (j : Fin N) (w : Fin N → Fin 2)
...
private lemma tensor_diagonal {N : ℕ} (d : Fin N → Fin 2 → ℂ) :
...
private lemma nUp_diag : nUp = Matrix.diagonal (fun v : Fin 2 => if v = 0 then 1 else 0) := by
  ext a b
  fin_cases a <;> fin_cases b <;> norm_num [nUp, qubitZ, Matrix.one_apply, Matrix.diagonal_apply]

private lemma nDown_diag : nDown = Matrix.diagonal (fun v : Fin 2 => if v = 0 then 0 else 1) := by
  ext a b
  fin_cases a <;> fin_cases b <;> norm_num [nDown, qubitZ, Matrix.one_apply, Matrix.diagonal_apply]

private lemma site_diagonal {N : ℕ} (j : Fin N) (d : Fin 2 → ℂ) :
...
private lemma Xi_eq (N : ℕ) : Xi N = Matrix.diagonal (xiVal N) := by
  classical
...
private lemma P_apply {N : ℕ} (j : Fin N) (y x : Fin N → Fin 2) :
...
private lemma swap_swap_word {N : ℕ} (a b : Fin N) (y : Fin N → Fin 2) :
    (fun t => (fun s => y (Equiv.swap a b s)) (Equiv.swap a b t)) = y := by
  funext t
  simp

/-- `Ξ` commutes with each Fredkin density `F_{j,j+1,j+2}`. -/
private lemma Xi_comm_F {N : ℕ} (hN : 3 ≤ N) (j : Fin N) : Xi N * F N j = F N j * Xi N := by
  rw [Xi_eq]
  ext y x
  ...
  · rw [if_neg hyx]
    have e1 : (if y j = 0 then (1 : ℂ) else 0) *
        (if (fun t => y (Equiv.swap k k2 t)) = x then 1 else 0) * (xiVal N y - xiVal N x) = 0 := by
