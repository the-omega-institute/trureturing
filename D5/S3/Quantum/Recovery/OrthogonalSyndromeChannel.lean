...
private theorem encoding_kraus_column_gram (S : s → Matrix n d ℂ)
  ...
  calc
    _ = ∑ i, ∑ k, (star (B i j) * B k j) • ((S i)ᴴ * S k) := by
      ...
      apply Finset.sum_congr rfl
      intro k hk
      rw [smul_smul, mul_comm (B k j) (star (B i j))]
    _ = _ := by
      calc
        _ = ∑ i, ∑ k, (star (B i j) * B k j) •
            (if i = k then (1 : Matrix d d ℂ) else 0) := by
          apply Finset.sum_congr rfl
          intro i hi
          apply Finset.sum_congr rfl
          intro k hk
          rw [hS i k]
        _ = _ := by simp [Finset.sum_smul]
...
theorem encoding_kraus_action (S : s → Matrix n d ℂ) (B : Matrix s s ℂ)
  ...
  calc
    _ = ∑ j, ∑ i, ∑ k,
        (B i j * star (B k j)) • (S i * rho * (S k)ᴴ) := by
      apply Finset.sum_congr rfl
      intro j hj
      simp only [encodingKraus, Matrix.conjTranspose_sum, Matrix.conjTranspose_smul,
        Matrix.sum_mul, Matrix.mul_sum, Matrix.smul_mul, Matrix.mul_smul,
    ...
    _ = _ := by
      simp only [syndromeEncoding, Matrix.mul_apply, Matrix.conjTranspose_apply]
