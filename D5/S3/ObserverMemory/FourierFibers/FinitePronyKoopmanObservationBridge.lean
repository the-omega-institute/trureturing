...
/- Library-search audit trail (2026-09-03):
   ...
   * The Prony owners added in this branch supply the frozen reconstruction,
     shifted-Hankel and matrix-pencil views. The statements below are
...
theorem finite_prony_moment_eq_crystal_time_sample {m : ℕ}
    ...
    pronyMoment nodes weights time =
...
theorem finite_prony_shifted_hankel_entry_eq_transported_sample {m n : ℕ}
  ...
  simp [finitePronyShiftedHankel, pronyMoment,
...
theorem finite_prony_first_window_eq_crystal_time_window {m : ℕ}
    ...
    (fun time : Fin m =>
      pronyMoment nodes weights (time : ℕ)) =
...
theorem finite_prony_first_window_injective {m : ℕ}
    ...
    Function.Injective
      (fun weights : Fin m → ℂ =>
        fun time : Fin m =>
          pronyMoment nodes weights (time : ℕ)) := by
...
theorem finite_prony_koopman_observation_package {m : ℕ}
    ...
    Function.Injective
      (fun amplitudes : Fin m → ℂ =>
        fun time : Fin m =>
          pronyMoment nodes amplitudes (time : ℕ)) :=
