...
/-
  ...
  not bounded enumerations or certified instances. `two_mul_totativeSum`,
  `odd_totativeSum_iff`, and `result` are general theorems, not checkers or
  numeric reductions. Thus none of the five declarations belongs to any of the
  four computational-content classes.
-/
...
private lemma totativeSum_eq_filtered_range {n : ℕ} (hn : 1 < n) :
    totativeSum n = ∑ k ∈ (range n).filter (n.Coprime ·), k := by
  rw [totativeSum, range_add_one, filter_insert]
  simp [show n ≠ 1 by omega]

theorem two_mul_totativeSum {n : ℕ} (hn : 1 < n) :
    2 * totativeSum n = n * n.totient := by
  ...
  rw [totativeSum_eq_filtered_range hn]
  change 2 * (∑ k ∈ A, k) = _
  rw [two_mul]
  nth_rw 2 [hsym]
  rw [← sum_add_distrib]
...
#print axioms two_mul_totativeSum
