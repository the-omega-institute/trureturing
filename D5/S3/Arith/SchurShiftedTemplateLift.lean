...
Because the carrier is the total function type `Nat -> Fin k`,
`HasSchurColoring 0 0` is false: evaluating a putative coloring at `0` would
produce an element of `Fin 0`. Thus the zero-color, zero-length convention here
differs from finite-interval colorability represented by an empty function.
All published theorems below either have at least one color or are unaffected
by this convention. -/
