/- GID: D5/S3/Arith/Radical/GoldenRadicalDividedDifference
   ...
   anchors: []
   utility: none
   digest: A golden lift gives an integral divided sum and a quadratic equation. -/
...
theorem result {L : Type*} [Field L] [CharZero L]
  ...
  have hny : (n : L) * y = powerSum n theta (ι a) := by
    dsimp [y, dividedPowerSum]
    field_simp [hn0]
  have hxy : (theta - ι a) * y = (n : L) * ι b := by
    apply mul_left_cancel₀ hn0
    calc
      (n : L) * ((theta - ι a) * y) = ((n : L) * y) * (theta - ι a) := by ring
      _ = powerSum n theta (ι a) * (theta - ι a) := by rw [hny]
      _ = theta ^ n - (ι a) ^ n := (Commute.all theta (ι a)).geom_sum₂_mul n
      _ = (n : L) * ((n : L) * ι b) := by rw [hpow]; ring
  have hsecond_mul : secondDifference n theta (ι a) * (theta - ι a) =
      powerSum n theta (ι a) - (n : L) * (ι a) ^ (n - 1) := by
    calc
      secondDifference n theta (ι a) * (theta - ι a) =
          ∑ i ∈ range n, (ι a) ^ (n - 1 - i) * (theta ^ i - (ι a) ^ i) := by
        rw [secondDifference, Finset.sum_mul]
        apply Finset.sum_congr rfl
        intro i hi
        rw [mul_assoc]
        exact congrArg ((ι a) ^ (n - 1 - i) * ·)
          ((Commute.all theta (ι a)).geom_sum₂_mul i)
      _ = powerSum n theta (ι a) - powerSum n (ι a) (ι a) := by
        simp only [mul_sub, Finset.sum_sub_distrib, powerSum]
        congr 1 <;> apply Finset.sum_congr rfl <;> intro i hi <;> ring
      _ = powerSum n theta (ι a) - (n : L) * (ι a) ^ (n - 1) := by
        unfold powerSum
        rw [geom_sum₂_self]
  have hquad : y ^ 2 = (ι a) ^ (n - 1) * y + ι b * secondDifference n theta (ι a) := by
    apply mul_left_cancel₀ hn0
    calc
      (n : L) * y ^ 2 = ((n : L) * y) * y := by ring
      _ = powerSum n theta (ι a) * y := by rw [hny]
      _ = (secondDifference n theta (ι a) * (theta - ι a) +
            (n : L) * (ι a) ^ (n - 1)) * y := by
        rw [hsecond_mul]
        ring
      _ = secondDifference n theta (ι a) * ((theta - ι a) * y) +
            (n : L) * ((ι a) ^ (n - 1) * y) := by ring
      _ = (n : L) * ((ι a) ^ (n - 1) * y + ι b * secondDifference n theta (ι a)) := by
        rw [hxy]
        ring
  refine ⟨hxy, hquad, ?_⟩
  have hphi : IsIntegral ℤ phi := by
    ...
    refine ⟨P, hP, ?_⟩
    simp [P, phi_sq]
  have hgolden (z : GoldenInt) : IsIntegral ℤ z := by
  ...
  have ha : IsIntegral ℤ (ι a) := map_isIntegral_int ι (hgolden a)
  have hb : IsIntegral ℤ (ι b) := map_isIntegral_int ι (hgolden b)
  have htheta : IsIntegral ℤ theta := by
    apply IsIntegral.of_pow hn
    rw [hroot]
    exact map_isIntegral_int ι hphi
  ...
  let c₁ := (ι a) ^ (n - 1)
  let c₀ := ι b * secondDifference n theta (ι a)
  have h₁ : IsIntegral ℤ c₁ := ha.pow (n - 1)
  have h₀ : IsIntegral ℤ c₀ := hb.mul hsecond
