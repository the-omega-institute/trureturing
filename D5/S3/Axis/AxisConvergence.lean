...
private local instance zeckendorfRelTrans : IsTrans ℕ (fun a b => b + 2 ≤ a) where
  trans _ _ _ hab hbc := hbc.trans (le_self_add.trans hab)

private local instance zeckendorfRelIrrefl : Std.Irrefl (fun a b : ℕ => b + 2 ≤ a) where
  irrefl _ h := by omega
...
/-- The origin counterexample packages its exact values with its divergence. -/
theorem axisPartialSum_zero_zero_package :
    (∀ K : ℕ, axisPartialSum 0 0 K = (Nat.fib (K + 1) : ℝ)) ∧
      Tendsto (axisPartialSum 0 0) atTop atTop :=
  ⟨axisPartialSum_zero_zero, axisPartialSum_zero_zero_tendsto_atTop⟩
...
#print axioms axisPartialSum_zero_zero_package
