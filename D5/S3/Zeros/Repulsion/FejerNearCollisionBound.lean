...
private theorem fejer_energy_nonnegative {ι : Type*} [Fintype ι]
    {M : ℕ} (hM : 1 ≤ M) (γ : ι → ℝ) :
    0 ≤ ∑ i, ∑ j, fejerKernel M (γ i - γ j) := by
  classical
  exact Finset.sum_nonneg fun i _ =>
    Finset.sum_nonneg fun j _ => fejer_nonnegative hM (γ i - γ j)
...
private theorem repeated_value_energy_lower_bound
    ...
    (M : ℝ) *
      ...
      ∑ i, ∑ j, fejerKernel M (γ i - γ j) := by
  classical
  let fiber : Finset ι := Finset.univ.filter (fun i => γ i = v)
  let allPairs : Finset (ι × ι) := Finset.univ.product Finset.univ
  have hblock :
      (M : ℝ) * (fiber.card : ℝ) ^ 2 =
        ∑ p ∈ fiber.product fiber, fejerKernel M (γ p.1 - γ p.2) := by
    symm
    calc
      (∑ p ∈ fiber.product fiber, fejerKernel M (γ p.1 - γ p.2)) =
          ∑ p ∈ fiber.product fiber, (M : ℝ) := by
        apply Finset.sum_congr rfl
        intro p hp
        rw [show γ p.1 - γ p.2 = 0 by
      ...
      _ = (M : ℝ) * (fiber.card : ℝ) ^ 2 := by
        simp [pow_two, mul_comm]
  have hsubset : fiber.product fiber ⊆ allPairs := by
    intro p hp
    exact Finset.mem_product.mpr
      ⟨(Finset.mem_filter.mp (Finset.mem_product.mp hp).1).1,
  ...
  have htoAll :
    ...
    intro p hp hpn
    exact fejer_nonnegative hM _
  have hprod := hblock.le.trans htoAll
  dsimp [allPairs] at hprod
  rw [Fintype.sum_prod_type] at hprod
  simpa [fiber] using hprod
