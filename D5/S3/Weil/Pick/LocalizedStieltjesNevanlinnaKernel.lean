...
theorem raw_nevanlinna_difference_quotient_eq_mass_kernel
  ...
  have hw' : (support : ℂ) - conj w ≠ 0 := by
    simpa [Complex.star_def] using hw
  have hcross' : z - conj w ≠ 0 := by
...
theorem normalized_diagonal_reads_mass_and_support
  ...
  have hmassValue :
      (atomicMassKernel mass support).value
          (normalizedUpperSample support) (normalizedUpperSample support) =
        (mass : ℂ) := by
    simp only [atomicMassKernel, hfeature, Complex.star_def, Complex.conj_I]
    simp [mul_assoc, Complex.I_mul_I]
  have hsupportValue :
      (atomicSupportKernel mass support).value
          (normalizedUpperSample support) (normalizedUpperSample support) =
        ((mass * support : ℝ) : ℂ) := by
    simp only [atomicSupportKernel, hfeature, Complex.star_def, Complex.conj_I]
    simp [mul_assoc, Complex.I_mul_I]
