...
theorem active_coordinate_negative_index_eq_active_orbit_count
  ...
  simpa using Fintype.card_congr
    (activeOrbitEquivFiltered delta gamma time)
...
theorem active_coordinate_quadratic_neg
  ...
  have hsum :
      (∑ a in (Finset.univ : Finset (ActiveOrbit delta gamma time)), term a) <
        ∑ _a in (Finset.univ : Finset (ActiveOrbit delta gamma time)), (0 : ℝ) := by
