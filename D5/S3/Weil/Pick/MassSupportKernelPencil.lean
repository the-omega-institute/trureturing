...
theorem mass_support_kernel_pencil_factorization {n : ℕ}
  ...
  classical
  ext i j
  simp only [massSupportKernelPencil, supportGramMatrix, massGramMatrix,
    HermitianKernel.gramMatrix, finiteSupportKernel, finiteMassKernel,
    atomicSupportKernel, atomicMassKernel, Matrix.sub_apply,
    Matrix.smul_apply, smul_eq_mul]
  rw [Finset.mul_sum, ← Finset.sum_sub_distrib, Matrix.mul_apply]
  apply Finset.sum_congr rfl
  intro a _
  rw [shiftedSupportWeightMatrix, Matrix.mul_diagonal]
...
theorem mass_gram_mulVec_of_dual {n : ℕ}
  ...
  simp [massWeightMatrix, cauchyFeatureMatrix, cauchyAtomVector,
    Matrix.mulVec, dotProduct, Matrix.diagonal_apply,
    mul_ite, ite_mul, mul_comm, mul_left_comm, mul_assoc]
