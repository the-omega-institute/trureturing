...
private def altAlternatingTerm (s : ℂ) (n : ℕ) : ℂ :=
  (-1 : ℂ) ^ n * ((n + 1 : ℂ) ^ (-s))

private def altEtaPairTerm (n : ℕ) (s : ℂ) : ℂ :=
  (((2 * n + 1 : ℕ) : ℝ) : ℂ) ^ (-s) -
    (((2 * n + 2 : ℕ) : ℝ) : ℂ) ^ (-s)

private noncomputable def altPairedEta (s : ℂ) : ℂ :=
  ∑' n : ℕ, altEtaPairTerm n s

private noncomputable def altZetaEtaFactor (s : ℂ) : ℂ :=
  (1 - 2 * (2 : ℂ) ^ (-s)) * riemannZeta s

private lemma altAlternatingTerm_pair (s : ℂ) (n : ℕ) :
    altAlternatingTerm s (2 * n) + altAlternatingTerm s (2 * n + 1) = altEtaPairTerm n s := by
  simp [altAlternatingTerm, altEtaPairTerm, pow_add]
  rw [show (2 * (n : ℂ) + 1 + 1) = 2 * (n : ℂ) + 2 by ring]
  rfl

private lemma altAlternating_sum_even (s : ℂ) (N : ℕ) :
    (∑ n ∈ Finset.range (2 * N), altAlternatingTerm s n) =
      ∑ n ∈ Finset.range N, altEtaPairTerm n s := by
  induction N with
  | zero => simp
  | succ N ih =>
      rw [show 2 * (N + 1) = 2 * N + 1 + 1 by omega,
        Finset.sum_range_succ, Finset.sum_range_succ, Finset.sum_range_succ, ih]
      rw [add_assoc, altAlternatingTerm_pair]

private lemma altAlternating_sum_odd (s : ℂ) (N : ℕ) :
    (∑ n ∈ Finset.range (2 * N + 1), altAlternatingTerm s n) =
      (∑ n ∈ Finset.range N, altEtaPairTerm n s) + altAlternatingTerm s (2 * N) := by
  rw [Finset.sum_range_succ, altAlternating_sum_even]

private lemma altEtaPairTerm_norm_le {delta radius : ℝ} (hdelta : 0 < delta)
  ...
  have hmv' :
    ...
    rw [← norm_neg]
    simpa [a, b] using hmv
  simpa [altEtaPairTerm, a, b] using hmv'

private lemma altEtaPairTerm_summable {s : ℂ} (hs : 0 < s.re) :
    Summable fun n : ℕ => altEtaPairTerm n s := by
  let delta : ℝ := s.re / 2
  let radius : ℝ := ‖s‖
  ...
  intro n
  have hraw := altEtaPairTerm_norm_le hdelta (s := s) (by simp [delta]; linarith)
...
private lemma differentiableOn_altPairedEta :
    DifferentiableOn ℂ altPairedEta {s : ℂ | 0 < s.re} := by
  intro z hz
  change 0 < z.re at hz
  let delta : ℝ := z.re / 2
  let radius : ℝ := ‖z‖ + 1
  ...
  have hmajorant :
    ...
    simpa only [Function.comp_apply, Nat.cast_add, Nat.cast_one, add_comm, one_div] using
      hshift.mul_left radius
  have hterm (n : ℕ) : DifferentiableOn ℂ (altEtaPairTerm n) U := by
    intro w hw
  ...
  have hterm_bound (n : ℕ) (w : ℂ) (hw : w ∈ U) :
      ‖altEtaPairTerm n w‖ ≤ radius * ((n + 1 : ℕ) : ℝ) ^ (-delta - 1) := by
    have hraw := altEtaPairTerm_norm_le hdelta (s := w) hw.1.le hw.2.le n
    have hcast : ((n + 1 : ℕ) : ℝ) ≤ ((2 * n + 1 : ℕ) : ℝ) := by
      exact_mod_cast (by omega : n + 1 ≤ 2 * n + 1)
    have hpow : ((2 * n + 1 : ℕ) : ℝ) ^ (-delta - 1) ≤
        ((n + 1 : ℕ) : ℝ) ^ (-delta - 1) :=
  ...
  have hdiff : DifferentiableOn ℂ altPairedEta U := by
    change DifferentiableOn ℂ (fun w => ∑' n : ℕ, altEtaPairTerm n w) U
...
private lemma altPairedEta_eq_zeta_factor_of_one_lt_re {s : ℂ} (hs : 1 < s.re) :
    altPairedEta s = (1 - 2 * (2 : ℂ) ^ (-s)) * riemannZeta s := by
...
private lemma altPairedEta_eq_altZetaEtaFactor_upper_half_plane :
    Set.EqOn altPairedEta altZetaEtaFactor
  ...
  have hpaired_diff : DifferentiableOn ℂ altPairedEta U :=
    differentiableOn_altPairedEta.mono inter_subset_left
  have hfactor_diff : DifferentiableOn ℂ altZetaEtaFactor U := by
    intro s hs
    have hs1 : s ≠ 1 := by
      intro h
      subst s
  ...
  have hnear : altPairedEta =ᶠ[nhds (2 + I : ℂ)] altZetaEtaFactor := by
    filter_upwards [(isOpen_lt continuous_const continuous_re).mem_nhds
      (by norm_num : (1 : ℝ) < (2 + I : ℂ).re)] with s hs
    exact (altPairedEta_eq_zeta_factor_of_one_lt_re hs).trans rfl
  change Set.EqOn altPairedEta altZetaEtaFactor U
...
private lemma altPairedEta_eq_altZetaEtaFactor_lower_half_plane :
    Set.EqOn altPairedEta altZetaEtaFactor
...
private lemma altPairedEta_eq_altZetaEtaFactor_of_real {x : ℝ}
    (hx : 0 < x) (hx1 : x ≠ 1) :
    altPairedEta (x : ℂ) = altZetaEtaFactor (x : ℂ) := by
  ...
  have hseq_mem (n : ℕ) :
    ...
    · change 0 < (seq n).im
      ...
      · positivity
  have hseq_eq (n : ℕ) : altPairedEta (seq n) = altZetaEtaFactor (seq n) :=
    altPairedEta_eq_altZetaEtaFactor_upper_half_plane (hseq_mem n)
  have hpaired_cont : ContinuousAt altPairedEta (x : ℂ) := by
    have hxmem : (x : ℂ) ∈ {s : ℂ | 0 < s.re} := by simpa using hx
    exact ((differentiableOn_altPairedEta (x : ℂ) hxmem).differentiableAt
      ((isOpen_lt continuous_const continuous_re).mem_nhds hxmem)).continuousAt
  have hfactor_cont : ContinuousAt altZetaEtaFactor (x : ℂ) := by
  ...
  have hleft' : Tendsto (altZetaEtaFactor ∘ seq) atTop
      (nhds (altPairedEta (x : ℂ))) := by
    apply hleft.congr'
    exact Eventually.of_forall hseq_eq
  exact tendsto_nhds_unique hleft' hright

private lemma altAlternatingTerm_tendsto_zero {s : ℂ} (hs : 0 < s.re) :
    Tendsto (altAlternatingTerm s) atTop (nhds 0) := by
  rw [tendsto_zero_iff_norm_tendsto_zero]
  have h := (tendsto_rpow_neg_atTop hs).comp tendsto_natCast_atTop_atTop
  have h' := (Filter.tendsto_add_atTop_iff_nat 1).2 h
  convert h' using 1
  funext n
  rw [altAlternatingTerm, norm_mul, norm_pow, norm_neg, norm_one, one_pow, one_mul,
    show (n : ℂ) + 1 = ((((n + 1 : ℕ) : ℝ) : ℂ)) by norm_num,
    Complex.norm_cpow_eq_rpow_re_of_pos (by positivity)]
  simp

private lemma tendsto_alternating_partialSums_altPairedEta (s : ℂ) (hs : 0 < s.re) :
    Tendsto (fun N => ∑ n ∈ Finset.range N, altAlternatingTerm s n)
      atTop (nhds (altPairedEta s)) := by
  have hpair := (altEtaPairTerm_summable hs).hasSum.tendsto_sum_nat
  have hterm := altAlternatingTerm_tendsto_zero hs
  ...
  · have hk : max K₁ K₂ ≤ k := by omega
    rw [altAlternating_sum_even]
    exact (hK₁ k (le_trans (le_max_left _ _) hk)).trans (by linarith)
  · have hk : max K₁ K₂ ≤ k := by omega
    have hk₂ : K₂ ≤ 2 * k := by omega
    rw [altAlternating_sum_odd]
    calc
      dist ((∑ n ∈ Finset.range k, altEtaPairTerm n s) + altAlternatingTerm s (2 * k))
          (altPairedEta s) ≤
          dist (∑ n ∈ Finset.range k, altEtaPairTerm n s) (altPairedEta s) +
            dist (altAlternatingTerm s (2 * k)) 0 := by
              simpa only [add_zero] using
                (dist_add_add_le (∑ n ∈ Finset.range k, altEtaPairTerm n s)
                  (altAlternatingTerm s (2 * k)) (altPairedEta s) 0)
      _ < ε / 2 + ε / 2 := add_lt_add
        (hK₁ k (le_trans (le_max_left _ _) hk))
        (hK₂ (2 * k) hk₂)
      _ = ε := by ring

private lemma altPairedEta_eq_altZetaEtaFactor {s : ℂ}
    (hs : 0 < s.re) (hs1 : s ≠ 1) :
    altPairedEta s = altZetaEtaFactor s := by
  rcases lt_trichotomy s.im 0 with him | him | him
  · exact altPairedEta_eq_altZetaEtaFactor_lower_half_plane ⟨hs, him⟩
  · have hs_eq : s = (s.re : ℂ) := by
      apply Complex.ext
      · simp
      · simpa using him
    rw [hs_eq]
    apply altPairedEta_eq_altZetaEtaFactor_of_real (by simpa using hs)
    intro hre
    apply hs1
    rw [hs_eq, hre]
    norm_num
  · exact altPairedEta_eq_altZetaEtaFactor_upper_half_plane ⟨hs, him⟩

private lemma altTwo_mul_two_cpow_neg (s : ℂ) :
    2 * (2 : ℂ) ^ (-s) = (2 : ℂ) ^ (1 - s) := by
...
theorem tendsto_alternating_partialSums_eta_of_ne_one
  ...
  have ht := tendsto_alternating_partialSums_altPairedEta s hs
  rw [altPairedEta_eq_altZetaEtaFactor hs hs1, altZetaEtaFactor,
    altTwo_mul_two_cpow_neg] at ht
  simpa only [altAlternatingTerm] using ht
...
theorem alternating_partialSums_eta_atom_fails_at_one :
  ...
  have hreal :
    ...
    have hre := (Complex.continuous_re.tendsto 0).comp hfalse
    change Filter.Tendsto
      (fun N ↦ (∑ n ∈ Finset.range N,
        (-1 : ℂ) ^ n * ((n + 1 : ℂ) ^ (-(1 : ℂ)))).re)
      Filter.atTop (nhds 0) at hre
    have hterm (n : ℕ) :
        ((-1 : ℂ) ^ n * ((n + 1 : ℂ) ^ (-(1 : ℂ)))).re =
          (-1 : ℝ) ^ n * (((n + 1 : ℕ) : ℝ))⁻¹ := by
      have hsign : ((-1 : ℂ) ^ n).re = (-1 : ℝ) ^ n := by
        have hpow : (-1 : ℂ) ^ n = (((-1 : ℝ) ^ n : ℝ) : ℂ) := by
          simpa using (Complex.ofReal_pow (-1 : ℝ) n).symm
        exact (congrArg Complex.re hpow).trans (Complex.ofReal_re _)
      rw [Complex.cpow_neg_one]
      norm_num [Complex.inv_re, Complex.normSq_apply, hsign]
    simpa only [Complex.re_sum, hterm] using hre
  have hantitone : Antitone (fun n : ℕ ↦ (((n + 1 : ℕ) : ℝ))⁻¹) := by
    intro a b hab
    apply inv_anti₀ (by positivity)
    exact_mod_cast Nat.add_le_add_right hab 1
  have hbound := hantitone.alternating_series_le_tendsto hreal 1
  norm_num at hbound
...
theorem riemannZeta_ne_zero_of_real_mem_Ioo
  ...
  intro hz
  rcases
      (D5.S3.Analytic.Isolation.RiemannZetaPositiveRealSign.riemannZeta_ofReal_sign
        h0 (ne_of_lt h1)).2 with hneg | hpos
  · rw [hz] at hneg
    norm_num at hneg
  · linarith [hpos.1]
