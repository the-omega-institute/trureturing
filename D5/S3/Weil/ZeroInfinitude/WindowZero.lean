...
theorem zero_summand_norm_le
  ...
  have hp :
      ‖Zeta23.paperFT (CosinePacket.packetSquare : ℝ → ℂ)
          (Zeta23.gammaOf ρ + T)‖ ≤
        K / (1 + ((ρ : ℂ).im + T) ^ 2) := by
...
theorem gamma_term_re_lower_log :
  ...
  refine ⟨δ / (8 * Real.pi),
    ∫ r : ℝ, (Zeta23.paperFT (CosinePacket.packetSquare : ℝ → ℂ) r).re,
    T₁, by positivity, fun T hT => ?_⟩
  ...
  calc
    δ / (8 * Real.pi) * Real.log (T + 3) -
        ∫ r : ℝ, (Zeta23.paperFT (CosinePacket.packetSquare : ℝ → ℂ) r).re =
        δ / 2 * ((1 / (4 * Real.pi)) * Real.log (T + 3)) -
          ∫ r : ℝ,
            (Zeta23.paperFT (CosinePacket.packetSquare : ℝ → ℂ) r).re := by ring
    _ ≤
        δ / 2 * (Zeta23.mu (T - δ) + 1) -
          ∫ r : ℝ,
            (Zeta23.paperFT (CosinePacket.packetSquare : ℝ → ℂ) r).re := by
      exact sub_le_sub_right hscale
        (∫ r : ℝ,
          (Zeta23.paperFT (CosinePacket.packetSquare : ℝ → ℂ) r).re)
    _ ≤ _ := by simpa using harch
