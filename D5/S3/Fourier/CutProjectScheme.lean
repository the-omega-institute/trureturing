...
/-- Top-level restatement: model-set construction is monotone in the internal
window. -/
theorem scheme_modelSet_mono
    {Physical : Type u} {Internal : Type v}
    [AddGroup Physical] [AddGroup Internal]
    (scheme : Scheme Physical Internal) {left right : Set Internal}
    (h : left ⊆ right) :
    scheme.modelSet left ⊆ scheme.modelSet right :=
  Scheme.modelSet_mono scheme h

#print axioms scheme_modelSet_mono
