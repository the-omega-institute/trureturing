...
private lemma wedgePair_order (i : Fin 6) :
  ...
  let orderedPair : Fin 2 → Fin 4 := ![wedgeFirst i, wedgeSecond i]
  have hmem : ∀ j, orderedPair j ∈ (wedgePair i).val := by
    intro j
    fin_cases i <;> fin_cases j <;>
      simp [orderedPair, wedgePair, wedgeFirst, wedgeSecond]
  have hmono : StrictMono orderedPair := by
    intro a b hab
    fin_cases i <;> fin_cases a <;> fin_cases b <;>
      simp_all [orderedPair, wedgeFirst, wedgeSecond]
  have hordered :=
    Finset.orderEmbOfFin_unique (wedgePair i).prop hmem hmono
  constructor
  · simpa [orderedPair] using congrFun hordered.symm (0 : Fin 2)
  · simpa [orderedPair] using congrFun hordered.symm (1 : Fin 2)
