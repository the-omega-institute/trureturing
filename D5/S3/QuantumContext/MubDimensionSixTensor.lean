...
/- Library-search audit trail (2026-09-05):
   ...
   * Pinned Mathlib supplies `Fintype.sum_prod_type`, `Finset.sum_mul_sum`, complex
     conjugation/norm identities, and finite-index case analysis, which are reused below.
   * Pinned Mathlib has matrix Kronecker and inner-product tensor APIs, but none states
     this coordinate-family certificate or evaluates the explicit qutrit overlap table.
-/
...
/-- Two coordinate orthonormal bases are mutually unbiased when every squared
cross-overlap is the reciprocal of the coordinate cardinality. -/
def MutuallyUnbiased {ι : Type*} [Fintype ι] [DecidableEq ι]
    (b c : ι → ι → ℂ) : Prop :=
  CoordinateOrthonormalBasis b ∧
    CoordinateOrthonormalBasis c ∧
...
private theorem coordinateOrthonormalBasis_tensor_aux
  ...
  intro i j
  rw [show coordinateInner (tensorBasis b b' i) (tensorBasis b b' j) =
      coordinateInner (b i.1) (b j.1) *
        coordinateInner (b' i.2) (b' j.2) by
    exact coordinateInner_tensorVector_aux _ _ _ _]
...
private theorem mutuallyUnbiased_tensor_aux
  ...
  refine ⟨coordinateOrthonormalBasis_tensor_aux hbc.1 hbc'.1,
    coordinateOrthonormalBasis_tensor_aux hbc.2.1 hbc'.2.1, ?_⟩
  intro i j
  rw [show coordinateInner (tensorBasis b b' i) (tensorBasis c c' j) =
      coordinateInner (b i.1) (c j.1) *
        coordinateInner (b' i.2) (c' j.2) by
    exact coordinateInner_tensorVector_aux _ _ _ _]
  rw [norm_mul, mul_pow, hbc.2.2 i.1 j.1, hbc'.2.2 i.2 j.2]
...
coordinate orthonormality and mutual unbiasedness. -/
...
private theorem pairwiseMUB_tensor_aux
  ...
  · intro r s hrs i j
    have hpairA : MutuallyUnbiased (bs r) (bs s) :=
      ⟨hbs.1 r, hbs.1 s, hbs.2 r s hrs⟩
    have hpairB : MutuallyUnbiased (cs r) (cs s) :=
      ⟨hcs.1 r, hcs.1 s, hcs.2 r s hrs⟩
    exact (mutuallyUnbiased_tensor_aux hpairA hpairB).2.2 i j
...
private def omega : ℂ := Complex.exp (2 * Real.pi * Complex.I / 3)
...
example : MutuallyUnbiased (qutritBases 0) (qutritBases 1) :=
  ⟨qutrit_three_mubs.1 0, qutrit_three_mubs.1 1,
