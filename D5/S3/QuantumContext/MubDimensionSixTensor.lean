...
/- Library-search audit trail (2026-09-05):
   ...
   * Pinned Mathlib `dotProduct` is the existing finite coordinate sum, so
     `coordinateInner` is only the pointwise-star specialization of that definition.
   * A def-level search found generic `Function.uncurry` and matrix-level
     `Matrix.kroneckerMap`, but no named Pi-vector tensor product; `tensorVector`
     remains the typed coordinate-vector operation while `tensorBasis` binds
     `Matrix.kronecker` directly.
-/
...
private theorem coordinateInner_tensorVector_aux
  ...
  classical
  simp only [coordinateInner, dotProduct, Pi.star_apply, tensorVector,
...
private theorem qubitBases_orthonormal :
  ...
  fin_cases r <;> fin_cases i <;> fin_cases j <;>
    simp [coordinateInner, dotProduct, qubitBases, qubitZ, qubitX, qubitY,
      Fin.sum_univ_two] <;>
    ring_nf <;>
...
private theorem qubit_three_mubs_aux : PairwiseMUB qubitBases := by
  ...
  fin_cases r <;> fin_cases s <;> fin_cases i <;> fin_cases j <;>
    simp_all [coordinateInner, dotProduct, qubitBases, qubitZ, qubitX, qubitY,
      Fin.sum_univ_two] <;>
    ring_nf <;>
...
private theorem qutritBases_orthonormal :
  ...
  intro r i j
  fin_cases r <;> fin_cases i <;> fin_cases j <;>
    simp [coordinateInner, dotProduct, qutritBases, qutritZ, qutritFourier,
...
theorem qutrit_three_mubs : PairwiseMUB qutritBases := by
  ...
  rw [sq_norm_eq_iff_mul_star]
  fin_cases r <;> fin_cases s <;> fin_cases i <;> fin_cases j <;>
    simp_all [coordinateInner, dotProduct, qutritBases, qutritZ, qutritFourier,
      qutritQuadratic, Fin.sum_univ_succ] <;>
    ring_nf <;>
