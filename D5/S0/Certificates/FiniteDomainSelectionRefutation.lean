...
abbrev Assignment (colors count : Nat) := Fin count → Fin colors

/-- Allowed values for every transition-table or trace variable. -/
abbrev Domains (colors count : Nat) := Fin count → Finset (Fin colors)
...
theorem restrict_preserves {colors n : Nat} {D : Domains colors n}
  ...
  by_cases hw : w = v
  · subst w
    simpa only [restrict, if_pos rfl, if_true, Finset.mem_inter] using And.intro (hD v) h
...
inductive Refutation (colors n m : Nat)
  ...
  | split (schedule : List (Instruction colors m)) (v : Fin n)
