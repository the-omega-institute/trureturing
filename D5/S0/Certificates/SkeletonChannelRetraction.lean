...
theorem evalFrom_mapChannels (K : Skeleton Output State) (f g : Output → Output)
  ...
  | cons block blocks ih =>
      cases block with
      | zero =>
          cases hs : K.zeroStep q with
          | none => simp [Skeleton.evalFrom, mapChannels, hs]
          | some next => simpa [Skeleton.evalFrom, mapChannels, hs] using ih next
      | oneZero =>
          ...
          | some p =>
              ...
              cases hn : next with
              | none => simp [Skeleton.evalFrom, mapChannels, hs, hn]
              | some target =>
                  simpa [Skeleton.evalFrom, mapChannels, hs, hn] using ih target
