/- GID: D5/S0/Asymptotics/EscapeProbability/DiploidSelectionRevealOrder
   ...
   anchors: []
   digest: Reuse the frozen diploid selection theorem with positive-dominance guards. -/
...
/- Library-search audit trail (2026-09-02):
   ...
   * Pinned Mathlib owns `analyticOrderAt` and `Asymptotics.IsBigO`; no result is
     reproved here. This wrapper restores the old atom's explicit `0 < h`
     branch while reusing the stronger imported result.
...
quadratic rare-allele signal, while a positive heterozygote effect exposes a
...
theorem diploid_selection_reveal_order
    ...
    (0 < dominance →
      (fun x => selectionChange dominance x - (-(dominance * selection) * x))
        =O[𝓝 0] (fun x => x ^ 2)) ∧
    (0 < dominance → analyticOrderAt (selectionChange dominance) 0 = 1) := by
  dsimp
  ...
  · intro dominancePos
    exact exposedOrder (mul_ne_zero (ne_of_gt dominancePos) selectionNe)

/- Reverse probe for CAS-A5: the public result yields a concrete nontrivial
order-one selection signal at `s = h = 1`. -/
example :
    analyticOrderAt
      (fun x : Real =>
        (x ^ 2 * (1 - 1) + (1 - x) * x * (1 - 1 * 1)) /
