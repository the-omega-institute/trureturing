...
open D5.S0.Automata.TypedPartialDFAOOverBase in
...
theorem eval_padSkeleton (skeleton : Skeleton Output State) (Extra : Type w)
  ...
  | cons block blocks ih =>
      ...
      | zero =>
          ...
          | some next =>
              simp only [Skeleton.evalFrom, padSkeleton, hs, Option.map_some,
                Option.bind_some]
              exact ih (Sum.inl next)
      | oneZero =>
          ...
          | some signature =>
              ...
              | some next =>
                  simp only [Skeleton.evalFrom, padSkeleton, paddingSignature, hs, hr,
