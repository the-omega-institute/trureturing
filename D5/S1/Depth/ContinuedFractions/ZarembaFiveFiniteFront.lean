...
/- Library-search audit trail (2026-09-05):
   * Pinned Lean core supplies the exact membership projection `List.all_eq_true`;
     `List.all_eq_true.mp h x hx` replaces a local list induction.
   * Repository and in-flight searches found no existing Zaremba checker, witness
     table, public finite certificate, or theorem covering the two target atoms. -/
...
private instance (A a q : Nat) : Decidable (ZarembaWitness A a q) := by
...
/-- Public finite certificate: kernel reduction checks every row of the 1025-entry
witness table, with rows zero and one discharged by the explicit guard. -/
theorem zarembaFiveCertificate :
    (List.range 1025).all
      (fun q => decide (q < 2) || zarembaCheck 5 (zarembaFiveNumerator q) q) = true := by
  decide

private theorem zaremba_five_upto
    (checkerSound : ∀ A a q : Nat, zarembaCheck A a q = true -> ZarembaWitness A a q)
    (q : Nat) (h2 : 2 ≤ q) (hQ : q ≤ 1024) :
  ...
  simp [hnot] at hrow
  exact ⟨zarembaFiveNumerator q, checkerSound 5 (zarembaFiveNumerator q) q hrow⟩
...
theorem zaremba_five_upto_certified :
  ...
  exact ⟨
    zaremba_five_upto cfDigits_checker_sound.2.2,
...
#print axioms zarembaFiveCertificate
