...
/- Library-search audit trail (2026-09-05):
   ...
   * Public theorems classify all naturals; only private helpers enumerate, so kind=none.
...
private theorem two_pow_length_dvd_prod_of_all_even (l : List ℕ)
  ...
  simpa using Multiset.prod_dvd_prod_of_dvd (S := (l : Multiset ℕ))
    (fun _ => 2) id (by simpa using heven)
