...
theorem window_cylinder_partition :
    ...
    (∀ m, 1 ≤ m → eMinus m ≠ ePlus m ∧
      ∀ x, phase x = E m ↔ x = eMinus m ∨ x = ePlus m) ∧
    ∀ L, 1 ≤ L →
      ...
      (∀ p : X L, ∃ i j : ℕ, 1 ≤ i ∧ i ≤ G L ∧ 1 ≤ j ∧ j ≤ G L ∧
        (ell p : Circle)=E i ∧ (upper p : Circle)=E j ∧
        C p = phase ⁻¹' A p ∪ {ePlus i,eMinus j}) ∧
  ...
  have seam_interior (w : List Block) : seam w ∈ Set.Ioo a b := by
    ...
    have hr := ((signed_series_fibres.1 w).2 (rightStream w)).mpr (Or.inr rfl)
    have hb := bounds (leftStream w)
    rw [hl] at hb
    have hna : seam w ≠ a := by
      intro h
      exact (signed_series_fibres.1 w).1
        (((signed_series_range.2.1 _).mp (hl.trans h)).trans
          ((signed_series_range.2.1 _).mp (hr.trans h)).symm)
    have hnb : seam w ≠ b := by
      intro h
      exact (signed_series_fibres.1 w).1
        (((signed_series_range.2.2 _).mp (hl.trans h)).trans
          ((signed_series_range.2.2 _).mp (hr.trans h)).symm)
    exact ⟨lt_of_le_of_ne hb.1 hna.symm, lt_of_le_of_ne hb.2 hnb⟩
  have index_inj : Function.Injective seamIndex := by
    intro w z he
    apply signed_series_fibres.2.1
    apply (AddCircle.coe_eq_coe_iff_of_mem_Ico
      (show seam w ∈ Set.Ico a (a+1) from ⟨(seam_interior w).1.le, by linarith [(seam_interior w).2]⟩)
      (show seam z ∈ Set.Ico a (a+1) from ⟨(seam_interior z).1.le, by linarith [(seam_interior z).2]⟩)).mp
    rw [seam_phase, seam_phase, he]
  have value_range (N s : ℕ) (hN : 1 ≤ N) (hs : s < G N) :
      ∃ x : LegalDigits, V (P N x) = s := by
    by_cases h : s < G N-1
    · obtain ⟨x,hx,_⟩ := ((window_successor_graph N hN).1 s (s+1)).mpr (Or.inl ⟨h,rfl⟩)
      exact ⟨x,hx⟩
    · have he : s = G N-1 := by omega
      obtain ⟨x,hx,_⟩ := ((window_successor_graph N hN).1 s 0).mpr (Or.inr (Or.inl ⟨he,rfl⟩))
      exact ⟨x,hx⟩
  ...
  have value_formula (N : ℕ) (x : LegalDigits) :
      V (P N x) = ∑ j ∈ Finset.range N, if x.val j then Nat.fib (j+2) else 0 := by
    change (∑ i : Fin N, Nat.fib (i.val+2) * (if x.val i then 1 else 0)) = _
    rw [Fin.sum_univ_eq_sum_range (fun j => Nat.fib (j+2) * (if x.val j then 1 else 0)) N]
    apply Finset.sum_congr rfl
    intro j _
    cases x.val j <;> simp
  have index_exists (m : ℕ) (hm : 2 ≤ m) : ∃! w : List Block, seamIndex w = m := by
    have hex : ∃ t, m ≤ G (t+1) := by
      refine ⟨m+2, ?_⟩
      have h := Nat.le_fib_self (n := m+5) (by omega)
      change m ≤ Nat.fib (m+5)
      omega
    let t := Nat.find hex
    have ht : m ≤ G (t+1) := Nat.find_spec hex
    have he : ∃ w : List Block, seamIndex w = m := by
      by_cases ht0 : t = 0
      · have hm2 : m = 2 := by norm_num [ht0,G] at ht; omega
        refine ⟨[],?_⟩
        norm_num [seamIndex,wordValue,len,digitsOf,
          D5.S0.Automata.BinaryZeckendorfBlockSkeleton.expand,G,hm2]
      · have htpos : 0 < t := by omega
        have hlow : G t < m := by
          have h := Nat.find_min hex (show t-1 < t by omega)
          have he : t-1+1=t := by omega
          rw [he] at h
          omega
        have hrec : G (t+1) = G t + G (t-1) := by
          dsimp [G]
          rw [show t-1+2=t+1 by omega]
          simpa only [Nat.add_assoc,Nat.reduceAdd] using (Nat.fib_add_two (n := t+1)).trans (Nat.add_comm _ _)
        let N := G (t+1)-m
        have hN : N < G (t-1) := by dsimp [N]; omega
        by_cases ht1 : t = 1
        · have hm3 : m = 3 := by norm_num [ht1,G] at ht hlow; omega
          refine ⟨[.zero],?_⟩
          norm_num [seamIndex,wordValue,len,digitsOf,
            D5.S0.Automata.BinaryZeckendorfBlockSkeleton.expand,G,hm3]
        · obtain ⟨x,hx⟩ := value_range (t-1) N (by omega) hN
          ...
          have hv : wordValue w = N := by
            ...
            have hb := hwbits (t-1) (by omega)
            simp only [y,pad,lt_self_iff_false,↓reduceDIte] at hb
            rw [hb]
          ...
          omega
    obtain ⟨w,hw⟩ := he
    exact ⟨w,hw,fun z hz => index_inj (hz.trans hw.symm)⟩
  have einj : Function.Injective E := by
    intro m n he
    by_contra hmn
    have hd : (m : ℤ)-(n : ℤ) ≠ 0 := by omega
    have hi := Real.goldenRatio_irrational.intCast_mul hd
    have he' : (((m : ℝ)*Real.goldenRatio : ℝ) : Circle) =
        (((n : ℝ)*Real.goldenRatio : ℝ) : Circle) := by
      apply neg_injective
      simpa [E,neg_mul] using he
    have hz : (((((m : ℤ)-(n : ℤ) : ℤ) : ℝ)*Real.goldenRatio : ℝ) : Circle) = 0 := by
      push_cast
      rw [sub_mul,AddCircle.coe_sub,he',sub_self]
    obtain ⟨k,hk⟩ := (AddCircle.coe_eq_zero_iff (1 : ℝ)).mp hz
    exact hi.ne_int k (by simpa using hk.symm)
  have hone : ((1 : ℝ) : Circle) = 0 :=
    (AddCircle.coe_eq_zero_iff (1 : ℝ)).mpr ⟨1,by simp⟩
  have haid : a = 1-Real.goldenRatio := by
    dsimp [a,alpha]
    rw [Real.inv_goldenRatio]
    linarith [Real.goldenRatio_add_goldenConj]
  have hac : (a : Circle) = E 1 := by simp [haid,E,AddCircle.coe_sub,hone]
  have hbc : (b : Circle) = (a : Circle) := by
    have h : b = a+1 := by linarith
    rw [h,AddCircle.coe_add,hone,add_zero]
  have real_fibre (t : ℝ) (ht : t ∈ Set.Ioo a b) (x : LegalDigits) :
      phase x = (t : Circle) ↔ signedValue x = t := by
    constructor
    · intro hx
      have hxb : signedValue x ≠ b := by
        intro hb
        have he : (t : Circle) = (a : Circle) := by
          rw [← hx]
          change (signedValue x : Circle) = (a : Circle)
          rw [hb,hbc]
        have he' := (AddCircle.coe_eq_coe_iff_of_mem_Ico
          (show t ∈ Set.Ico a (a+1) from
            ⟨ht.1.le,by linarith [ht.2]⟩)
          (show a ∈ Set.Ico a (a+1) from ⟨le_rfl,by linarith⟩)).mp he
        exact (ne_of_gt ht.1) he'
      exact (AddCircle.coe_eq_coe_iff_of_mem_Ico
        (show signedValue x ∈ Set.Ico a (a+1) from
          ⟨(bounds x).1,by have h := lt_of_le_of_ne (bounds x).2 hxb; linarith⟩)
        (show t ∈ Set.Ico a (a+1) from
          ⟨ht.1.le,by linarith [ht.2]⟩)).mp hx
    · intro hx
      exact congrArg (fun t : ℝ => (t : Circle)) hx
  have phase_seam (w : List Block) (x : LegalDigits) :
      phase x = E (seamIndex w) ↔ signedValue x = seam w := by
    rw [← seam_phase]
    exact real_fibre _ (seam_interior w) x
  have seam_word (m : ℕ) (hm : 2 ≤ m) : seamIndex (seamWord m) = m := by
    have he : ∃ w, seamIndex w = m := (index_exists m hm).exists
    simp only [seamWord,dif_pos he]
    exact Classical.choose_spec he
  have endpoint_fibres (m : ℕ) (hm : 1 ≤ m) : eMinus m ≠ ePlus m ∧
      ∀ x, phase x = E m ↔ x = eMinus m ∨ x = ePlus m := by
    by_cases hm1 : m ≤ 1
    · have hmE : m=1 := by omega
      have hne : v ≠ u := by
        intro h
        have ht := congrArg (fun x : LegalDigits => x.val 0) h
        simp [u,v] at ht
      simp only [eMinus,ePlus,if_pos hm1]
      refine ⟨hne,?_⟩
      intro x
      rw [hmE,← hac]
      constructor
    ...
    · have hwm := seam_word m (by omega)
      have hf (x : LegalDigits) : phase x = E m ↔
          x = leftStream (seamWord m) ∨ x = rightStream (seamWord m) := by
        have hs := phase_seam (seamWord m) x
        rw [hwm] at hs
        exact hs.trans ((signed_series_fibres.1 (seamWord m)).2 x)
      have hne := (signed_series_fibres.1 (seamWord m)).1
      by_cases he : len (seamWord m) % 2 = 0
  ...
  have value_bound (N : ℕ) (x : LegalDigits) : V (P N x) < G N := by
    ...
    · have hpos : 0 < G N := Nat.fib_pos.mpr (by omega)
      have hb : beta N < G N := by
        have h1 : 0 < Nat.fib (N+1) := Nat.fib_pos.mpr (by omega)
        have h2 := Nat.fib_mono (show N+1 ≤ N+2 by omega)
        dsimp [beta,G]
        omega
      have he := ((window_successor_graph N (by omega)).1
        (V (P N x)) (V (P N (T x)))).mp ⟨x,rfl,rfl⟩
  ...
  have word_band (w : List Block) (ht : 0 < len w) :
      G (len w) < seamIndex w ∧ seamIndex w ≤ G (len w+1) := by
    let x := prependWord w u
    have hzero : x.val (len w-1) = false := by
      rw [prepend_digits,if_pos (show len w-1 < len w by omega)]
      have hz := word_last w
    ...
    have hv : wordValue w = V (P (len w-1) x) := by
      rw [value_formula]
      unfold wordValue
      have he : len w = (len w-1)+1 := by omega
      conv_lhs => rw [he,Finset.sum_range_succ]
      have hz : (digitsOf w)[len w-1]?.getD false = false := by
        simpa [List.getLast?_eq_getElem?,len] using word_last w
      rw [hz]
      simp only [Bool.false_eq_true,↓reduceIte,add_zero]
      apply Finset.sum_congr rfl
      intro j hj
      have hj' : j < len w := by have := Finset.mem_range.mp hj; omega
      rw [prepend_digits,if_pos hj']
    have hb := value_bound (len w-1) x
    rw [← hv] at hb
    have hrec : G (len w+1) = G (len w) + G (len w-1) := by
      dsimp [G]
      rw [show len w-1+2=len w+1 by omega]
      simpa only [Nat.add_assoc,Nat.reduceAdd] using
        (Nat.fib_add_two (n := len w+1)).trans (Nat.add_comm _ _)
    dsimp [seamIndex]
    omega
  have gmono : StrictMono G := by
    apply strictMono_nat_of_lt_succ
    intro n
    exact Nat.fib_lt_fib_succ (by omega)
  have word_threshold (L : ℕ) (hL : 1 ≤ L) (w : List Block) :
      seamIndex w ≤ G L ↔ len w < L := by
    by_cases ht : len w = 0
    · have hw : w = [] := by
        cases w with
        | nil => rfl
        | cons b w => cases b <;> simp [len,digitsOf,
            D5.S0.Automata.BinaryZeckendorfBlockSkeleton.expand] at ht
      have hG : 2 ≤ G L := by
        have := gmono.monotone hL
        norm_num [G] at this ⊢
        exact this
      subst w
      norm_num [len,digitsOf,D5.S0.Automata.BinaryZeckendorfBlockSkeleton.expand,
        seamIndex,wordValue,G] at hG ⊢
      exact ⟨fun _ => hL,fun _ => hG⟩
    · have hb := word_band w (by omega)
      constructor
      · intro h
        by_contra hn
        have := gmono.monotone (show L ≤ len w by omega)
        omega
      · intro h
        exact hb.2.trans (gmono.monotone (by omega))
  have prepend_append (w z : List Block) (x : LegalDigits) :
      prependWord (w++z) x = prependWord w (prependWord z x) := by
    induction w with
    | nil => rfl
    | cons b w ih => simp only [List.cons_append,prependWord,ih]
  have pair_windows (L : ℕ) (w : List Block) :
      P L (leftStream w) ≠ P L (rightStream w) ↔ len w < L := by
    constructor
    · intro h
      by_contra hn
      apply h
      apply Subtype.ext
      funext j
      change (leftStream w).val j = (rightStream w).val j
      simp only [leftStream,rightStream,prepend_append]
      simp only [prepend_digits,if_pos (show j.val < len w by omega)]
    · intro ht he
      have hd := congrArg (fun p : X L => p.val ⟨len w,ht⟩) he
      change (leftStream w).val (len w) = (rightStream w).val (len w) at hd
      simp only [leftStream,rightStream,prepend_append] at hd
      simp only [prepend_digits,lt_self_iff_false,↓reduceIte,Nat.sub_self] at hd
      norm_num [len,digitsOf,D5.S0.Automata.BinaryZeckendorfBlockSkeleton.expand] at hd
  have endpoint_split (L : ℕ) (hL : 1 ≤ L) (m : ℕ) (hm : 1 ≤ m) :
      P L (eMinus m) ≠ P L (ePlus m) ↔ m ≤ G L := by
    by_cases hm1 : m ≤ 1
    · have hmE : m=1 := by omega
      have hG : 1 ≤ G L := Nat.fib_pos.mpr (by omega)
      have hne : P L v ≠ P L u := by
        intro h
        have hh := congrArg (fun p : X L => p.val ⟨0,by omega⟩) h
        simp [P,u,v] at hh
      simp [eMinus,ePlus,hm1,hmE,hG,hne]
    · have hwm := seam_word m (by omega)
      have he : len (seamWord m) < L ↔ m ≤ G L := by
        have hs := word_threshold L hL (seamWord m)
        rw [hwm] at hs
        exact hs.symm
      by_cases hp : len (seamWord m) % 2 = 0
      · simp only [eMinus,ePlus,if_neg hm1,if_pos hp]
        rw [ne_comm,pair_windows]
        exact he
      · simp only [eMinus,ePlus,if_neg hm1,if_neg hp,pair_windows]
        exact he
  have interval_subset (L : ℕ) (hL : 1 ≤ L) (p : X L) : I p ⊆ Set.Icc a b := by
    rw [← (partition L hL).1]
    exact Set.subset_iUnion (fun p : X L => I p) p
  have interval_ends (L : ℕ) (hL : 1 ≤ L) (p : X L) :
      ell p < upper p ∧ ell p ∈ I p ∧ upper p ∈ I p := by
    have hg := geometry L hL p
    have hl : ell p < upper p := by linarith [hg.2.2.1,hg.2.2.2.1]
    rw [hg.2.1]
    exact ⟨hl,⟨le_rfl,hl.le⟩,⟨hl.le,le_rfl⟩⟩
  have no_other (L : ℕ) (hL : 1 ≤ L) (p q : X L) (hpq : p ≠ q) :
      Disjoint (Set.Ioo (ell p) (upper p)) (I q) := by
    apply Set.disjoint_left.mpr
    intro t hp hq
    rw [(geometry L hL q).2.1] at hq
    ...
    apply Set.disjoint_left.mp ((partition L hL).2 p q hpq)
  ...
  have shared_end (L : ℕ) (hL : 1 ≤ L) (p q : X L) (hpq : p ≠ q)
      (t : ℝ) (hp : t ∈ I p) (hq : t ∈ I q) : t = ell p ∨ t = upper p := by
    by_contra h
    push Not at h
    have hp' := hp
    rw [(geometry L hL p).2.1] at hp'
    exact Set.disjoint_left.mp (no_other L hL p q hpq)
      ⟨lt_of_le_of_ne hp'.1 h.1.symm,lt_of_le_of_ne hp'.2 h.2⟩ hq
  have boundary_other (L : ℕ) (hL : 1 ≤ L) (p : X L) (t : ℝ)
      (ht : t=ell p ∨ t=upper p) (ha : a<t) (hb : t<b) :
      ∃ q : X L, q ≠ p ∧ t ∈ I q := by
    have : Finite (X L) := by unfold X; infer_instance
    let J : Set ℝ := ⋃ q : X L, ⋃ (_ : q ≠ p), I q
    ...
    have covers (s : ℝ) (hs : s ∈ Set.Icc a b) (hn : s ∉ I p) : s ∈ J := by
      rw [← (partition L hL).1] at hs
      obtain ⟨q,hq⟩ := Set.mem_iUnion.mp hs
      have hqp : q ≠ p := by rintro rfl; exact hn hq
      exact Set.mem_iUnion.mpr ⟨q,Set.mem_iUnion.mpr ⟨hqp,hq⟩⟩
    have htJ : t ∈ J := by
      rcases ht with ht | ht
      · have hsub : Set.Ioo a t ⊆ J := by
          intro s hs
          apply covers s ⟨hs.1.le,(hs.2.trans hb).le⟩
          rw [(geometry L hL p).2.1]
          intro hn
          linarith [hn.1,hs.2]
        have htcl : t ∈ closure (Set.Ioo a t) := by
          rw [closure_Ioo ha.ne]
          exact ⟨ha.le,le_rfl⟩
        exact hJ.closure_eq ▸ (closure_mono hsub htcl)
      · have hsub : Set.Ioo t b ⊆ J := by
          intro s hs
          apply covers s ⟨(ha.trans hs.1).le,hs.2.le⟩
          rw [(geometry L hL p).2.1]
          intro hn
          linarith [hn.2,hs.1]
        have htcl : t ∈ closure (Set.Ioo t b) := by
          rw [closure_Ioo hb.ne]
          exact ⟨le_rfl,hb.le⟩
        exact hJ.closure_eq ▸ (closure_mono hsub htcl)
    obtain ⟨q,hq⟩ := Set.mem_iUnion.mp htJ
    obtain ⟨hne,htq⟩ := Set.mem_iUnion.mp hq
    exact ⟨q,hne,htq⟩
  have index_two (w : List Block) : 2 ≤ seamIndex w := by
    by_cases ht : 0 < len w
    · have hb := (word_band w ht).1
      have hg : 0 < G (len w) := Nat.fib_pos.mpr (by omega)
      omega
    · have hw : w=[] := by
        cases w with
        | nil => rfl
      ...
      subst w
      norm_num [seamIndex,wordValue,len,digitsOf,
        D5.S0.Automata.BinaryZeckendorfBlockSkeleton.expand,G]
  have cuts (L : ℕ) (hL : 1 ≤ L) : actualCuts L = B L := by
    ...
    · rintro ⟨p,hz⟩
      ...
      have htb := interval_subset L hL p htp
      by_cases hta : t=a
      · refine ⟨1,⟨le_rfl,?_⟩,?_⟩
      ...
      · refine ⟨1,⟨le_rfl,?_⟩,?_⟩
        · exact Nat.fib_pos.mpr (by omega)
        · rw [hzt,htb',hbc,hac]
      obtain ⟨q,hqp,htq⟩ := boundary_other L hL p t ht
        (lt_of_le_of_ne htb.1 (fun h => hta h.symm)) (lt_of_le_of_ne htb.2 htb')
      rw [← (geometry L hL p).1] at htp
      rw [← (geometry L hL q).1] at htq
      obtain ⟨x,hx,hxt⟩ := htp
      obtain ⟨y,hy,hyt⟩ := htq
      have hxy : P L x ≠ P L y := by
        change P L x=p at hx
        change P L y=q at hy
        rw [hx,hy]
        exact hqp.symm
      have hseam : t ∈ Set.range seam := by
        by_contra hn
        obtain ⟨s,_,hu⟩ := signed_series_fibres.2.2 t htb hn
        exact hxy (congrArg (P L) ((hu x hxt).trans (hu y hyt).symm))
      obtain ⟨w,hw⟩ := hseam
      have hxm : phase x = E (seamIndex w) := by
        change (signedValue x : Circle) = _
        rw [hxt,← hw,seam_phase]
      have hym : phase y = E (seamIndex w) := by
        change (signedValue y : Circle) = _
        rw [hyt,← hw,seam_phase]
      ...
      have hsplit : P L (eMinus (seamIndex w)) ≠ P L (ePlus (seamIndex w)) := by
        intro he
        apply hxy
        rcases hxF with hxF | hxF <;> rcases hyF with hyF | hyF
        ...
        · exact (congrArg (P L) hxF).trans (congrArg (P L) hyF).symm
      have hm := (endpoint_split L hL _ (by have := index_two w; omega)).mp hsplit
      refine ⟨seamIndex w,⟨by have := index_two w; omega,hm⟩,?_⟩
      rw [← seam_phase,hw,hzt]
    · rintro ⟨m,hm,rfl⟩
      by_cases hm1 : m=1
      · let p := P L u
        have hpa : a ∈ I p := by
          rw [← (geometry L hL p).1]
          exact ⟨u,rfl,(signed_series_range.2.1 u).mpr rfl⟩
        have hl := (interval_subset L hL p ((interval_ends L hL p).2.1)).1
        have hpa' := hpa
        rw [(geometry L hL p).2.1] at hpa'
        have he : ell p = a := le_antisymm hpa'.1 hl
        exact ⟨p,Or.inl (by rw [he,hm1,hac])⟩
      · let w := seamWord m
        have hw : seamIndex w=m := seam_word m (by have := hm.1; omega)
        have hx := (endpoint_fibres m hm.1).2 (eMinus m) |>.mpr (Or.inl rfl)
        have hy := (endpoint_fibres m hm.1).2 (ePlus m) |>.mpr (Or.inr rfl)
        have hxs : signedValue (eMinus m)=seam w := by
          apply (phase_seam w _).mp
          rwa [hw]
        have hys : signedValue (ePlus m)=seam w := by
          apply (phase_seam w _).mp
          rwa [hw]
        have hp : seam w ∈ I (P L (eMinus m)) := by
          rw [← (geometry L hL _).1]
          exact ⟨eMinus m,rfl,hxs⟩
        have hq : seam w ∈ I (P L (ePlus m)) := by
          rw [← (geometry L hL _).1]
          exact ⟨ePlus m,rfl,hys⟩
        have hd := shared_end L hL _ _ ((endpoint_split L hL m hm.1).mpr hm.2) _ hp hq
        refine ⟨P L (eMinus m),?_⟩
        have he := seam_phase w
        rw [hw] at he
        rw [← he]
        exact hd.imp (congrArg (fun t : ℝ => (t : Circle)))
          (congrArg (fun t : ℝ => (t : Circle)))
  have first_side (x : LegalDigits) :
      (x.val 0 = false → q ≤ signedValue x) ∧ (x.val 0 = true → signedValue x ≤ q) := by
    constructor
    · intro hx
      have hv := tail_value x 1
      simp [term,hx,r] at hv
      have hu := (bounds (shift x 1)).2
      dsimp [b] at hu
      have hmul := mul_le_mul_of_nonpos_left hu (neg_nonpos.mpr hp.le)
      dsimp [q]
      nlinarith
    · intro hx
      have hx1 : x.val 1 = false := by
        ...
        · rfl
        · exact False.elim (x.property 0 ⟨hx,hh⟩)
      have hv := tail_value x 2
      norm_num [term,hx,hx1,r,Finset.sum_range_succ] at hv
      have hu := (bounds (shift x 2)).2
      dsimp [b] at hu
      have hmul := mul_le_mul_of_nonneg_left hu (sq_nonneg alpha)
      have he : alpha ^ 4 + alpha ^ 3 = alpha ^ 2 := by
        nlinarith [congrArg (fun t : ℝ => alpha ^ 2*t) ha,
          congrArg (fun t : ℝ => alpha*t) ha]
      dsimp [q]
      nlinarith
  have real_sides (w : List Block) (x : LegalDigits)
      (hprefix : ∀ j, j < len w → x.val j = (digitsOf w)[j]?.getD false) :
      (x.val (len w) = false →
    ...
    have hv : signedValue x = S w + r ^ len w * signedValue (shift x (len w)) := by
      rw [tail_value]
      congr 1
      apply Finset.sum_congr rfl
      intro j hj
      dsimp [term]
      rw [hprefix j (Finset.mem_range.mp hj)]
    have hr : r ^ len w = if len w % 2 = 0 then alpha ^ len w else -(alpha ^ len w) := by
      rw [r,neg_eq_neg_one_mul,mul_pow,neg_one_pow_eq_ite]
      simp [Nat.even_iff]
    have hf (hx : x.val (len w)=false) : q ≤ signedValue (shift x (len w)) :=
      (first_side (shift x (len w))).1 (by simpa [shift] using hx)
    have ht (hx : x.val (len w)=true) : signedValue (shift x (len w)) ≤ q :=
      (first_side (shift x (len w))).2 (by simpa [shift] using hx)
    by_cases he : len w % 2 = 0
    · have hpow : 0 ≤ r ^ len w := by rw [hr,if_pos he]; exact (pow_pos hp _).le
      simp only [if_pos he]
      constructor
      · intro hx
        have h := mul_le_mul_of_nonneg_left (hf hx) hpow
        dsimp [seam,f]
        linarith
      · intro hx
        have h := mul_le_mul_of_nonneg_left (ht hx) hpow
        dsimp [seam,f]
        linarith
    · have hpow : r ^ len w ≤ 0 := by rw [hr,if_neg he]; exact neg_nonpos.mpr (pow_pos hp _).le
      ...
      constructor
      · intro hx
        have h := mul_le_mul_of_nonpos_left (hf hx) hpow
        dsimp [seam,f]
        linarith
      · intro hx
        have h := mul_le_mul_of_nonpos_left (ht hx) hpow
        dsimp [seam,f]
        linarith
  have left_side (L : ℕ) (w : List Block) (hL : len w < L) (x : LegalDigits)
      (hx : P L x = P L (leftStream w)) :
    ...
    have hpref (j : ℕ) (hj : j < len w) : x.val j = (digitsOf w)[j]?.getD false := by
      have he := congrArg (fun p : X L => p.val ⟨j,by omega⟩) hx
      change x.val j = (leftStream w).val j at he
      rw [leftStream,prepend_append,prepend_digits,if_pos hj] at he
      exact he
    have hbit : x.val (len w) = false := by
      have he := congrArg (fun p : X L => p.val ⟨len w,hL⟩) hx
      change x.val (len w) = (leftStream w).val (len w) at he
      rw [leftStream,prepend_append,prepend_digits,if_neg (lt_irrefl _)] at he
      simpa [prependWord,prependBlock] using he
    exact (real_sides w x hpref).1 hbit
  have right_side (L : ℕ) (w : List Block) (hL : len w < L) (x : LegalDigits)
      (hx : P L x = P L (rightStream w)) :
      if len w % 2 = 0 then signedValue x ≤ seam w else seam w ≤ signedValue x := by
    have hpref (j : ℕ) (hj : j < len w) : x.val j = (digitsOf w)[j]?.getD false := by
      have he := congrArg (fun p : X L => p.val ⟨j,by omega⟩) hx
      change x.val j = (rightStream w).val j at he
      rw [rightStream,prepend_append,prepend_digits,if_pos hj] at he
      exact he
    have hbit : x.val (len w) = true := by
      have he := congrArg (fun p : X L => p.val ⟨len w,hL⟩) hx
      change x.val (len w) = (rightStream w).val (len w) at he
      rw [rightStream,prepend_append,prepend_digits,if_neg (lt_irrefl _)] at he
      simpa [prependWord,prependBlock] using he
    exact (real_sides w x hpref).2 hbit
  have oriented_sides (L m : ℕ) (hL : 1 ≤ L) (hm : 2 ≤ m) (hmL : m ≤ G L)
      (x : LegalDigits) :
      (P L x = P L (eMinus m) → signedValue x ≤ seam (seamWord m)) ∧
      (P L x = P L (ePlus m) → seam (seamWord m) ≤ signedValue x) := by
    have hw := seam_word m hm
    have ht : len (seamWord m) < L := (word_threshold L hL _).mp (by rw [hw]; exact hmL)
    have hm1 : ¬m ≤ 1 := by omega
    by_cases he : len (seamWord m) % 2 = 0
    · simp only [eMinus,ePlus,if_neg hm1,if_pos he]
      exact ⟨fun hx => by simpa [he] using right_side L _ ht x hx,
        fun hx => by simpa [he] using left_side L _ ht x hx⟩
    · simp only [eMinus,ePlus,if_neg hm1,if_neg he]
      exact ⟨fun hx => by simpa [he] using left_side L _ ht x hx,
        fun hx => by simpa [he] using right_side L _ ht x hx⟩
  have oriented_formula (L : ℕ) (hL : 1 ≤ L) (p : X L) :
      ∃ i j : ℕ, 1 ≤ i ∧ i ≤ G L ∧ 1 ≤ j ∧ j ≤ G L ∧
        (ell p : Circle)=E i ∧ (upper p : Circle)=E j ∧
        C p = phase ⁻¹' A p ∪ {ePlus i,eMinus j} := by
    have hcutl : (ell p : Circle) ∈ actualCuts L := ⟨p,Or.inl rfl⟩
    have hcutr : (upper p : Circle) ∈ actualCuts L := ⟨p,Or.inr rfl⟩
    rw [cuts L hL] at hcutl hcutr
    obtain ⟨i,hi,hie⟩ := hcutl
    obtain ⟨j,hj,hje⟩ := hcutr
    have hlo := (interval_subset L hL p ((interval_ends L hL p).2.1)).1
    have hhi := (interval_subset L hL p ((interval_ends L hL p).2.2)).2
    have hlen := (interval_ends L hL p).1
    have hxl : ell p ∈ signedValue '' C p :=
      (geometry L hL p).1.symm ▸ (interval_ends L hL p).2.1
    have hxr : upper p ∈ signedValue '' C p :=
      (geometry L hL p).1.symm ▸ (interval_ends L hL p).2.2
    obtain ⟨xl,hxl,hxlv⟩ := hxl
    obtain ⟨xr,hxr,hxrv⟩ := hxr
    have hxli : phase xl=E i := by change (signedValue xl : Circle)=_; rw [hxlv,← hie]
    ...
    have left_owner : ePlus i ∈ C p := by
      ...
      · have hminus : eMinus i ∈ C p := he ▸ hxl
        ...
        · have hs : seam (seamWord i)=ell p := by
            have hv := (phase_seam (seamWord i) xl).mp (by rwa [seam_word i (by omega)])
            exact hv.symm.trans hxlv
          have hb := (oriented_sides L i hL (by omega) hi.2 xr).1
            ((show P L xr=p from hxr).trans (show P L (eMinus i)=p from hminus).symm)
          exfalso
          linarith
      · exact he ▸ hxl
    have right_owner : eMinus j ∈ C p := by
      ...
      · have hplus : ePlus j ∈ C p := he ▸ hxr
        ...
        · have hs : seam (seamWord j)=upper p := by
            have hv := (phase_seam (seamWord j) xr).mp (by rwa [seam_word j (by omega)])
            exact hv.symm.trans hxrv
          have hb := (oriented_sides L j hL (by omega) hj.2 xl).2
            ((show P L xl=p from hxl).trans (show P L (ePlus j)=p from hplus).symm)
          exfalso
          linarith
    have not_left_minus : eMinus i ∉ C p := by
      intro hx
      exact ((endpoint_split L hL i hi.1).mpr hi.2)
        ((show P L (eMinus i)=p from hx).trans (show P L (ePlus i)=p from left_owner).symm)
    have not_right_plus : ePlus j ∉ C p := by
      intro hx
      exact ((endpoint_split L hL j hj.1).mpr hj.2)
        ((show P L (eMinus j)=p from right_owner).trans (show P L (ePlus j)=p from hx).symm)
    refine ⟨i,j,hi.1,hi.2,hj.1,hj.2,hie.symm,hje.symm,?_⟩
    ext x
    constructor
    · intro hx
      have hv : signedValue x ∈ I p := by
        rw [← (geometry L hL p).1]
        exact ⟨x,hx,rfl⟩
      rw [(geometry L hL p).2.1] at hv
      ...
      · exact Or.inl ⟨signedValue x,
          ⟨lt_of_le_of_ne hv.1 (fun h => hl h.symm),lt_of_le_of_ne hv.2 hr⟩,rfl⟩
    · rintro (hx | hx)
      · obtain ⟨t,ht,htx⟩ := hx
        have htI : t ∈ Set.Ioo a b := ⟨hlo.trans_lt ht.1,ht.2.trans_le hhi⟩
        have hval : signedValue x=t := (real_fibre t htI x).mp htx.symm
        have hxI : signedValue x ∈ I (P L x) := by
          rw [← (geometry L hL (P L x)).1]
          exact ⟨x,rfl,rfl⟩
        change P L x=p
        by_contra hne
        exact Set.disjoint_left.mp (no_other L hL p (P L x) (fun h => hne h.symm))
          (hval.symm ▸ ht) hxI
      · simp only [Set.mem_insert_iff,Set.mem_singleton_iff] at hx
        rcases hx with rfl | rfl
        · exact left_owner
        · exact right_owner
  refine ⟨index_exists,endpoint_fibres,?_⟩
  intro L hL
  have hf : (Set.univ : Set (X L)).Finite := by
    unfold X
    exact Set.toFinite _
  exact ⟨hf,geometry L hL,(partition L hL).1,(partition L hL).2,cuts L hL,oriented_formula L hL,endpoint_split L hL⟩
