...
private def listedBlocks (n : Nat) : List (Nat × List Nat) :=
  (((List.range ((n - 1) / 2)).map (fun j => j + 1)).filter (fun lam => 2 <= d n lam)).map
    (fun lam => (lam, (List.range (d n lam - 1)).map (fun j => blockStart n lam + j)))
...
example : listedBlocks 15 = [(1, [4])] := by decide +kernel
example : listedBlocks 21 = [(1, [6]), (2, [4])] := by decide +kernel
example : listedBlocks 33 = [(1, [10, 11]), (2, [8]), (3, [6])] := by decide +kernel
