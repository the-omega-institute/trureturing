...
theorem fibPair_append_digit (w : List (Fin 2)) (a : Fin 2) :
  ...
  | cons d w ih =>
      ...
      have hf : Nat.fib (w.length + 1 + 3) =
        ...
        convert Nat.fib_add_two (n := w.length + 2) using 1 <;> congr 1 <;> omega
      rw [hf]
...
theorem endpoint_certificate (q : Fin 21) (a : Fin 2) (t : Fin 21)
  ...
  all_goals
    dsimp [lowerPair, upperPair, point, errorStep]
    norm_num <;> nlinarith only [hl, hu, hs]
...
theorem step_preserves_cell {q t : Fin 21} {a : Fin 2} {e : Real}
  ...
  · subst q
    ...
    all_goals subst t
    · simp [Cell, errorStep]
    · dsimp [Cell, point, lowerPair, upperPair, errorStep]
      norm_num
  ...
  | cons a word ih =>
      ...
      | some c =>
          ...
          have htType : stateType t = c := by
            ...
            change binaryZeckendorfBase.step (stateType q) a = some (stateType t) at typed
            rw [hs] at typed
