...
theorem four_pow_mod_five_reduce_six (n : Nat) :
  ...
  conv_lhs => rw [hn]
  have period : (4 : ZMod 5) ^ 6 = 1 := by decide
  rw [pow_add, pow_mul, period, one_pow, mul_one]
...
theorem four_pow_mod_three (n : Nat) : (4 : ZMod 3) ^ n = 1 := by
  have base : (4 : ZMod 3) = 1 := by decide
  rw [base, one_pow]
