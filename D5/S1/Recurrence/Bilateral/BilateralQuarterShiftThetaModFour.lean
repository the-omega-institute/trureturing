/- GID: D5/S1/Recurrence/Bilateral/BilateralQuarterShiftThetaModFour
   generality: I
   mirror-B: D5/B/S1/Recurrence/Bilateral/BilateralQuarterShiftThetaModFour
   mirror-E: none(waiver:unbounded-symbolic-proof)
   anchors: []
   utility: none
   digest: Paired bilateral terms give an integral degree-contracting theta equation. -/
...
namespace D5.S1.Recurrence.Bilateral.BilateralQuarterShiftThetaModFour
...
private theorem negative_order (A : PowerSeries R) (m : ℕ) :
  ...
  exact dvd_mul_of_dvd_left (dvd_mul_of_dvd_right
    (pow_dvd_pow _ (by nlinarith : m + 1 ≤ m ^ 2 + 3 * m + 1)) _) _
...
private theorem reciprocal_sub (A B : PowerSeries R) (m : ℕ) :
    reciprocal A m - reciprocal B m =
      -4 * X ^ (m + 3) * reciprocal A m * reciprocal B m * (A - B) := by
...
private theorem paired_agree {d : ℕ} {A B : PowerSeries R} (h : Agree d A B) (m : ℕ) :
  ...
  have hp : X ^ d ∣ (4 * A + X ^ m) ^ (m + 2) - (4 * B + X ^ m) ^ (m + 2) := by
  ...
  have hy : (X : PowerSeries R) ∣ (-1) ^ (m + 2) * X ^ (m ^ 2 + 3 * m + 1) :=
    dvd_mul_of_dvd_right (by simpa only [pow_one] using
      (pow_dvd_pow (X : PowerSeries R) (by omega : 1 ≤ m ^ 2 + 3 * m + 1))) _
...
private theorem paired_zero (m : ℕ) : pairedTerm (0 : PowerSeries R) m = 0 := by
  ...
  have he : m + 1 + m * (m + 2) = m ^ 2 + 3 * m + 1 := by ring
...
private theorem paired_linear (A : PowerSeries R) (h16 : (16 : PowerSeries R) = 0) (m : ℕ) :
    pairedTerm A m =
      (-1) ^ (m + 1) * 4 * (m + 2 : PowerSeries R) * X ^ ((m + 1) ^ 2) * A +
      (-1) ^ (m + 1) * 4 * (m + 1 : PowerSeries R) * X ^ ((m + 2) ^ 2) * A := by
  have ha : (4 * A) ^ 2 = 0 := by linear_combination A ^ 2 * h16
  have hb : (4 * A * X ^ (m + 3)) ^ 2 = 0 := by rw [mul_pow, ha, zero_mul]
  have hi : reciprocal A m = 1 - 4 * A * X ^ (m + 3) := by
    linear_combination (1 - 4 * A * X ^ (m + 3)) * reciprocal_spec A m +
      reciprocal A m * hb
  have hip : reciprocal A m ^ (m + 1) =
      1 - (m + 1 : PowerSeries R) * (4 * A * X ^ (m + 3)) := by
    rw [hi, sub_eq_add_neg, square_zero_pow _ _ (by simpa using hb) m]
    simp [sub_eq_add_neg]
  have hbase : (X : PowerSeries R) ^ (m + 1) * (X ^ m) ^ (m + 2) =
      X ^ (m ^ 2 + 3 * m + 1) := by
    rw [← pow_mul, ← pow_add]
    congr 1
    ring
  have hlin : (X : PowerSeries R) ^ (m + 1) * (X ^ m) ^ (m + 1) =
      X ^ ((m + 1) ^ 2) := by
    rw [← pow_mul, ← pow_add]
    congr 1
    ring
  have hneg : (X : PowerSeries R) ^ (m ^ 2 + 3 * m + 1) * X ^ (m + 3) =
      X ^ ((m + 2) ^ 2) := by
    rw [← pow_add]
    congr 1
    ring
  have hp : positiveTerm A m =
      (-1) ^ (m + 1) * X ^ (m ^ 2 + 3 * m + 1) +
      (-1) ^ (m + 1) * 4 * (m + 2 : PowerSeries R) * X ^ ((m + 1) ^ 2) * A := by
    calc
      _ = (-1) ^ (m + 1) * (X ^ (m + 1) * (X ^ m) ^ (m + 2)) +
  ...
  have hn : negativeTerm A m =
      (-1) ^ (m + 2) * X ^ (m ^ 2 + 3 * m + 1) -
      (-1) ^ (m + 2) * 4 * (m + 1 : PowerSeries R) * X ^ ((m + 2) ^ 2) * A := by
    unfold negativeTerm
    rw [hip, ← hneg]
    ring
...
private theorem square_approximation (N : ℕ) :
  ...
  · subst n
    ...
    apply Finset.sum_eq_zero
    intro m hm
    rw [if_neg (Ne.symm (pow_ne_zero 2 (Nat.succ_ne_zero m)))]
  · rw [if_neg h0]
    by_cases hs : IsSquare n
    · obtain ⟨k, hk⟩ := hs
      obtain ⟨m, rfl⟩ := Nat.exists_eq_succ_of_ne_zero (show k ≠ 0 by
        intro he; simp [he] at hk; exact h0 hk)
      have he : n = (m + 1) ^ 2 := by nlinarith [hk]
      have hm : m ∈ Finset.range (N + 1) := by simp only [Finset.mem_range]; nlinarith
      rw [if_pos ⟨m + 1, hk⟩, Finset.sum_eq_single m]
      · simp [he]
      · intro j hj hjm
    ...
    · rw [if_neg hs]
      ...
      apply Finset.sum_eq_zero
      intro m hm
      rw [if_neg (by intro he; apply hs; exact ⟨m + 1, by nlinarith⟩)]

private theorem square_mul_coeff (A : PowerSeries R) (N : ℕ) :
    coeff N (squareSupport * A) =
      ∑ m ∈ Finset.range (N + 1), coeff N (X ^ ((m + 1) ^ 2) * A) := by
...
private theorem theta_expansion : ThetaSelfCompositionModFour.thetaSeries =
    1 + 2 * (squareSupport : PowerSeries ℤ) := by
  classical
  ext n
  simp only [ThetaSelfCompositionModFour.coeff_thetaSeries, map_add,
...
private theorem remainder_mod_sixteen (A : PowerSeries ℤ) :
  ...
  let t (m : ℕ) : PowerSeries (ZMod 16) := (-1) ^ (m + 1) * X ^ ((m + 1) ^ 2) * B
  ...
  have hz : coeff N (t (N + 1)) = 0 := by
    apply (X_pow_dvd_iff (n := (N + 1 + 1) ^ 2)).mp ?_ N (by nlinarith)
    exact dvd_mul_of_dvd_left (dvd_mul_left _ _) _
  ...
  have hs (m : ℕ) : (8 : ZMod 16) * coeff N (t m) =
      8 * coeff N (X ^ ((m + 1) ^ 2) * B) := by
    have hh : (8 : PowerSeries (ZMod 16)) * t m = 8 * (X ^ ((m + 1) ^ 2) * B) := by
      dsimp only [t]
...
theorem cancellation_identity : ∃ Q : PowerSeries ℤ,
    1 = generatingSeries * ThetaSelfCompositionModFour.thetaSeries + 4 * Q := by
  let H := remainder generatingSeries -
    4 * generatingSeries * (ThetaSelfCompositionModFour.thetaSeries - 1)
  ...
  have hc : (4 : PowerSeries ℤ) =
      4 * (generatingSeries * ThetaSelfCompositionModFour.thetaSeries + 4 * Q) := by
    dsimp only [H] at hq
...
theorem mod_four_identity : generatingSeries.map (Int.castRingHom (ZMod 4)) =
    ThetaSelfCompositionModFour.thetaSeries.map (Int.castRingHom (ZMod 4)) := by
  let f := Int.castRingHom (ZMod 4)
  let T := ThetaSelfCompositionModFour.thetaSeries.map f
...
theorem hanna_conjecture (n : ℕ) (hn : 1 ≤ n) :
  ...
  change (a n : ZMod 4) = ((coeff n ThetaSelfCompositionModFour.thetaSeries : ℤ) : ZMod 4) at hc
  rw [ThetaSelfCompositionModFour.coeff_thetaSeries, if_neg (by omega : n ≠ 0)] at hc
  by_cases hs : IsSquare n
  · rw [if_pos hs] at hc
...
end D5.S1.Recurrence.Bilateral.BilateralQuarterShiftThetaModFour
