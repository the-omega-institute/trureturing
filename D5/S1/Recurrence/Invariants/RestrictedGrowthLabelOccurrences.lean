...
/-- The chronological source predicate for positive-integer restricted-growth functions. -/
def IsRestrictedGrowth (f : List ℕ) : Prop :=
  f = [] ∨ (f.head? = some 1 ∧ ∀ j (hj : j < f.length),
    1 ≤ f[j] ∧ f[j] ≤ 1 + (f.take j).foldr max 0)

/-- Membership in the reverse-chronological generator is exactly the source
restricted-growth predicate on the chronological reversal. -/
theorem mem_words_iff (n : ℕ) (w : List ℕ) :
    w ∈ words n ↔ (w.length = n ∧ IsRestrictedGrowth w.reverse) := by
  have rg_append_singleton (f : List ℕ) (x : ℕ) :
      IsRestrictedGrowth (f ++ [x]) ↔
        IsRestrictedGrowth f ∧ 1 ≤ x ∧ x ≤ 1 + f.foldr max 0 := by
    constructor
    · intro h
      obtain ⟨hhead, hbounds⟩ := h.resolve_left (by simp)
      have hrg : IsRestrictedGrowth f := by
        by_cases hf : f = []
        · exact Or.inl hf
        · refine Or.inr ⟨?_, ?_⟩
          · simpa [List.head?_append_of_ne_nil f hf] using hhead
          · intro j hj
            have hbound := hbounds j (by simpa using Nat.lt_succ_of_lt hj)
            simpa [List.getElem_append_left hj,
              List.take_append_of_le_length (Nat.le_of_lt hj)] using hbound
      have hx := hbounds f.length (by simp)
      exact ⟨hrg, by simpa using hx⟩
    · rintro ⟨hrg, hxlo, hxhi⟩
      refine Or.inr ⟨?_, ?_⟩
      · by_cases hf : f = []
        · subst f
          have hx : x = 1 := by simp at hxhi; omega
          simp [hx]
        · obtain ⟨hhead, _⟩ := hrg.resolve_left hf
          simpa [List.head?_append_of_ne_nil f hf] using hhead
      · intro j hj
        by_cases hjf : j < f.length
        · have hf : f ≠ [] := by
            intro h
            simp [h] at hjf
          obtain ⟨_, hbounds⟩ := hrg.resolve_left hf
          have hbound := hbounds j hjf
          simpa [List.getElem_append_left hjf,
            List.take_append_of_le_length (Nat.le_of_lt hjf)] using hbound
        · have hjle : j ≤ f.length := by
            simp only [List.length_append, List.length_singleton] at hj
            omega
          have hfj : f.length ≤ j := Nat.le_of_not_gt hjf
          have hj_eq : j = f.length := Nat.le_antisymm hjle hfj
          subst j
          simpa using And.intro hxlo hxhi
  have foldr_max_acc (v : List ℕ) (a : ℕ) :
      v.foldr max a = max (v.foldr max 0) a := by
    induction v with
    | nil => simp
    | cons y v ih => simp [ih, max_assoc]
  have reverse_prefix_max (v : List ℕ) :
      (v.reverse).foldr max 0 = maxLabel v := by
    induction v with
    | nil => rfl
    | cons y v ih =>
      rw [List.reverse_cons, List.foldr_append]
      simp only [List.foldr_cons, List.foldr_nil, max_eq_left (Nat.zero_le y)]
      rw [foldr_max_acc, ih]
      simp [maxLabel, max_comm]
  have mem_succ (k : ℕ) (x : ℕ) (v : List ℕ) :
      x :: v ∈ words (k + 1) ↔
        v ∈ words k ∧ 1 ≤ x ∧ x ≤ maxLabel v + 1 := by
    constructor
    · intro h
      change x :: v ∈ (words k).biUnion (fun u =>
        (Finset.Icc 1 (maxLabel u + 1)).image fun y => y :: u) at h
      obtain ⟨u, hu, hi⟩ := Finset.mem_biUnion.mp h
      obtain ⟨y, hy, heq⟩ := Finset.mem_image.mp hi
      obtain ⟨rfl, rfl⟩ := List.cons.inj heq
      exact ⟨hu, (Finset.mem_Icc.mp hy).1, (Finset.mem_Icc.mp hy).2⟩
    · rintro ⟨hv, hxlo, hxhi⟩
      change x :: v ∈ (words k).biUnion (fun u =>
        (Finset.Icc 1 (maxLabel u + 1)).image fun y => y :: u)
      apply Finset.mem_biUnion.mpr
      exact ⟨v, hv, Finset.mem_image.mpr
        ⟨x, Finset.mem_Icc.mpr ⟨hxlo, hxhi⟩, rfl⟩⟩
  induction n generalizing w with
  | zero =>
    constructor
    · intro hw
      have hw' : w = [] := by simpa [words] using hw
      subst w
      simp [IsRestrictedGrowth]
    · rintro ⟨hlen, _⟩
  ...
  | succ n ih =>
    ...
    | cons x v =>
      ...
      constructor
      · rintro ⟨hv, hxlo, hxhi⟩
        obtain ⟨hvlen, hrg⟩ := (ih v).mp hv
        refine ⟨by simpa using congrArg Nat.succ hvlen, ?_⟩
        rw [List.reverse_cons, rg_append_singleton, reverse_prefix_max]
        exact ⟨hrg, hxlo, by simpa [Nat.add_comm] using hxhi⟩
      · rintro ⟨hlen, hrg⟩
        have hvlen : v.length = n := by simpa using hlen
        rw [List.reverse_cons, rg_append_singleton, reverse_prefix_max] at hrg
        exact ⟨(ih v).mpr ⟨hvlen, hrg.1⟩, hrg.2.1,
          by simpa [Nat.add_comm] using hrg.2.2⟩
