/- GID: D5/S1/Recurrence/NormOneCriticalPrimes
   ...
   anchors: []
   digest: Prime fixed points of the actual norm-one Lucas recurrence are exactly the prime divisors of a-2; an infinite parameter family refutes the literal restriction in Benfield-Lippard Conjecture 6.5(v). -/
...
theorem prime_fixed_iff_parameter_two (a : ℤ) (p : ℕ) (hp : p.Prime) :
  ...
  constructor
  · intro h
    have hu : companion (a : ZMod p) (1 : (ZMod p)ˣ) ^ p = 1 := by
      rw [← h]
