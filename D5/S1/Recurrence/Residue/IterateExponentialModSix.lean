/- GID: D5/S1/Recurrence/Residue/IterateExponentialModSix
   generality: I
   mirror-B: D5/B/S1/Recurrence/Residue/IterateExponentialModSix
   mirror-E: none(waiver:evidence-not-specified-by-formal-manifest)
   anchors: []
   utility: none
   digest: A396806 coefficients equal their indices modulo six. -/
...
open private stepK stepK_map approximationK parity_mod_two from
  D5.S1.Recurrence.Residue.IterateExponentialParity
open D5.S1.Recurrence.Residue.QuarticEGFModFour (F)
open private coeff_F_pow eCoeff_F composition_assoc composition_identity_right composition_identity_left from
  D5.S1.Recurrence.Residue.QuarticEGFModFour

namespace D5.S1.Recurrence.Residue.IterateExponentialModSix

theorem result (n : ℕ) (hn : 1 ≤ n) :
    Nat.ModEq 6 (D5.S1.Recurrence.Residue.IterateExponentialParity.aK 6 n) n := by
  ...
  have sixth_support (q : ℕ) :
      iterate (fun n : ℕ => (n : ZMod 3)) 6 (3 * q) = 0 ∧
        iterate (fun n : ℕ => (n : ZMod 3)) 6 (3 * q + 1) = if q = 0 then 1 else 0 := by
    let l (n : ℕ) : ℤ := n
    have hmap (f : ℕ → ℤ) (n : ℕ) :
    ...
    have hz (k : ℕ) : iterate l k 0 = 0 := by
      cases k <;> simp [iterate, identity, composition, l]
    have commute (k : ℕ) : composition (iterate l k) l = composition l (iterate l k) := by
      induction k with
      | zero =>
        rw [iterate, composition_identity_left l rfl, composition_identity_right]
      | succ k ih =>
        change composition (composition l (iterate l k)) l =
          composition l (composition l (iterate l k))
        rw [composition_assoc l (iterate l k) l (hz k) rfl, ih]
    have hiter (j n : ℕ) :
        ((iterate l (j + 1) n : ℤ) : ZMod 3) = (T^[j] (fun n : ℕ => (n : ZMod 3))) n := by
      induction j generalizing n with
      | zero => simp [iterate, composition_identity_right, l]
      | succ j ih =>
        change ((composition l (iterate l (j + 1)) n : ℤ) : ZMod 3) = _
        rw [← commute (j + 1), hmap]
        rw [show (fun n => ((iterate l (j + 1) n : ℤ) : ZMod 3)) =
          T^[j] (fun n : ℕ => (n : ZMod 3)) from funext ih]
        rw [Function.iterate_succ_apply']
    have hT (j q : ℕ) :
        (T^[j] (fun n : ℕ => (n : ZMod 3))) (3 * q) = 0 ∧
        (T^[j] (fun n : ℕ => (n : ZMod 3))) (3 * q + 1) = (j + 1 : ZMod 3) ^ q := by
      induction j generalizing q with
      | zero =>
        simp only [Function.iterate_zero_apply, Nat.cast_mul, Nat.cast_add,
          Nat.cast_ofNat, Nat.cast_one, Nat.cast_zero, show (3 : ZMod 3) = 0 by decide,
          zero_mul, zero_add, one_pow, and_self]
      | succ j ih =>
        rw [Function.iterate_succ_apply']
        constructor
        · rw [T_zero_residue]
        ...
        · rw [T_one_residue]
          simp_rw [(ih _).2]
          have hs := (add_pow (j + 1 : ZMod 3) 1 q).symm
          simpa only [one_pow, mul_one, one_mul, mul_comm, Nat.cast_add,
            Nat.cast_one, Nat.succ_eq_add_one] using hs
    have hsix : iterate (fun n : ℕ => (n : ZMod 3)) 6 =
        T^[5] (fun n : ℕ => (n : ZMod 3)) := by
      funext n
      have hm := iterate_map (Int.castRingHom (ZMod 3)) l 6 n
      change ((iterate l 6 n : ℤ) : ZMod 3) =
        iterate (fun n : ℕ => (n : ZMod 3)) 6 n at hm
      exact hm.symm.trans (hiter 5 n)
    rw [hsix]
    refine ⟨(hT 5 q).1, ?_⟩
    rw [(hT 5 q).2, show ((5 : ℕ) : ZMod 3) + 1 = 0 by decide]
    exact zero_pow_eq q
  have linear_fixed :
      stepK 6 (fun n : ℕ => (n : ZMod 3)) = fun n : ℕ => (n : ZMod 3) := by
    have he (q : ℕ) := E_support (iterate (fun n : ℕ => (n : ZMod 3)) 6)
      (fun q => (sixth_support q).1) (fun q => (sixth_support q).2) q
    funext n
    cases n with
    | zero => simp [stepK]
    | succ n =>
      change (n + 1 : ZMod 3) * E (iterate (fun n : ℕ => (n : ZMod 3)) 6) n = _
  ...
  have endpoint_of_linear_fixed
      (hf : stepK 6 (fun n : ℕ => (n : ZMod 3)) = fun n : ℕ => (n : ZMod 3))
      (n : ℕ) : Nat.ModEq 3 (aK 6 n) n := by
    have ha (d n : ℕ) : (approximationK 6 d n : ZMod 3) = n := by
      induction d generalizing n with
      | zero => rfl
      | succ d ih =>
        change (Nat.castRingHom (ZMod 3)) (stepK 6 (approximationK 6 d) n) = _
        rw [stepK_map]
        change stepK 6 (fun j => (approximationK 6 d j : ZMod 3)) n = _
        rw [show (fun j => (approximationK 6 d j : ZMod 3)) = (fun j : ℕ => (j : ZMod 3))
          from funext ih, hf]
    exact (ZMod.natCast_eq_natCast_iff _ _ _).mp (ha (n + 1) n)
  have h3 : Nat.ModEq 3 (aK 6 n) n := endpoint_of_linear_fixed linear_fixed n
  have h2 : Nat.ModEq 2 (aK 6 n) n := parity_mod_two 6 n hn
  exact (Nat.modEq_and_modEq_iff_modEq_mul (m := 2) (n := 3)
    (by decide : Nat.Coprime 2 3)).mp ⟨h2, h3⟩

set_option pp.fullNames true in
#print axioms result

end D5.S1.Recurrence.Residue.IterateExponentialModSix
