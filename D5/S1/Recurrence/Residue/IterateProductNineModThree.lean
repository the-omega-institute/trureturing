/- GID: D5/S1/Recurrence/Residue/IterateProductNineModThree
   generality: I
   mirror-B: D5/B/S1/Recurrence/Residue/IterateProductNineModThree
   mirror-E: none(waiver:unbounded-symbolic-proof)
   anchors: []
   utility: none
   digest: A coefficient perturbation and mod-nine lifting prove Hanna A396793. -/
...
namespace D5.S1.Recurrence.Residue.IterateProductNineModThree
...
private theorem product_top {n : ℕ} {f g : PowerSeries R}
    ...
    coeff (n + 1) (f * iterate f 2) - coeff (n + 1) (g * iterate g 2) =
      3 * (coeff n f - coeff n g) := by
  rw [mul_top hf (iterate_normal hg hg1 2).1 hf1 (iterate_normal hg hg1 2).2
    h (iterate_agree hf hg h 2), iterate_top hn hf hg hf1 hg1 h]
  norm_num
  ring
...
theorem lift_mod_nine (f : PowerSeries ℤ)
    (hf : constantCoeff f = 0) (h3 : ∀ n, 3 ∣ coeff n (f - X)) :
    ∀ n, 9 ∣ coeff n (f * iterate f 2 - X ^ 2) := by
  let b : PowerSeries ℤ := mk fun n => coeff n (f - X) / 3
  have hb : f = X + C 3 * b := by
    ext n
    have hh := Int.mul_ediv_cancel' (h3 n)
    simp only [map_sub] at hh
    simp only [map_add, coeff_C_mul, b, coeff_mk, map_sub]
    linear_combination -hh
  let hom := Int.castRingHom (ZMod 9)
  have hz : constantCoeff (f.map hom) = 0 := by
    rw [← coeff_zero_eq_constantCoeff, coeff_map, coeff_zero_eq_constantCoeff, hf, map_zero]
  have hm : f.map hom = X + C 3 * b.map hom := by
    rw [hb]
    simp only [map_add, map_mul, map_X, map_C]
    norm_num [hom]
  have hi : (iterate f 2).map hom = iterate (f.map hom) 2 := by
    ...
    exact aux 2
  have hn := nilpotent_iterate (3 : ZMod 9) (by decide) (b.map hom) (hm ▸ hz) 2
  have he : (f * iterate f 2 - X ^ 2).map hom = 0 := by
    rw [map_sub, map_mul, map_pow, map_X, hi, hm, hn]
    norm_num only [Nat.cast_ofNat, show (2 : ZMod 9) * 3 = 6 by decide]
    have hsum : C (3 : ZMod 9) + C 6 = 0 := by
      rw [← map_add, show (3 : ZMod 9) + 6 = 0 by decide, map_zero]
    have hprod : C (3 : ZMod 9) * C 6 = 0 := by
      rw [← map_mul, show (3 : ZMod 9) * 6 = 0 by decide, map_zero]
...
private noncomputable def rhs : PowerSeries ℤ := X ^ 2 + C 9 * X ^ 3

private theorem error_divisible (f : PowerSeries ℤ)
    (hf : constantCoeff f = 0) (h3 : ∀ n, 3 ∣ coeff n (f - X)) (n : ℕ) :
    9 ∣ coeff n (rhs - f * iterate f 2) := by
  have h := lift_mod_nine f hf h3 n
  have he : coeff n (rhs - f * iterate f 2) =
      9 * coeff n (X ^ 3 : PowerSeries ℤ) - coeff n (f * iterate f 2 - X ^ 2) := by
...
private theorem extend_three {n : ℕ} {f : PowerSeries ℤ}
    (hf : constantCoeff f = 0) (h3 : ∀ k, 3 ∣ coeff k (f - X)) :
    ∀ k, 3 ∣ coeff k (extend n f - X) := by
  have hc : 3 ∣ correction n f := by
    obtain ⟨q, hq⟩ := error_divisible f hf h3 (n + 1)
    refine ⟨q, ?_⟩
  ...
  have he : coeff k (extend n f - X) =
    ...
    ring
  rw [he]
  exact dvd_add (h3 k) (by split_ifs <;> simp_all)

private theorem extend_correct {n : ℕ} {f : PowerSeries ℤ}
    (hn : 1 < n) (hf : constantCoeff f = 0) (hf1 : coeff 1 f = 1)
    (h3 : ∀ k, 3 ∣ coeff k (f - X))
    (he : Agree (n + 1) (f * iterate f 2) rhs) :
    Agree (n + 2) (extend n f * iterate (extend n f) 2) rhs := by
  have hz := extend_normal hn hf hf1
  have ht := product_top hn hz.1 hf hz.2 hf1 (extend_agree n f)
  have hc : 3 * correction n f = coeff (n + 1) (rhs - f * iterate f 2) :=
    Int.mul_ediv_cancel' (dvd_trans (by norm_num : (3 : ℤ) ∣ 9)
      (error_divisible f hf h3 (n + 1)))
  ...
  intro k hk
  by_cases hkn : k < n + 1
  · exact (agree_mul hz.1 (iterate_normal hf hf1 2).1 (extend_agree n f)
      (iterate_agree hz.1 hf (extend_agree n f) 2) k hkn).trans (he k hkn)
...
private theorem approximation_spec (j : ℕ) :
    ...
    (∀ k, 3 ∣ coeff k (approximation j - X)) ∧
    Agree (j + 3) (approximation j * iterate (approximation j) 2) rhs := by
  induction j with
  | zero =>
  ...
  | succ j ih =>
    ...
    exact ⟨hz.1, hz.2, extend_three ih.1 ih.2.2.1,
...
private theorem exists_series : ∃ f : PowerSeries ℤ,
    f * iterate f 2 = X ^ 2 + 9 * X ^ 3 ∧ constantCoeff f = 0 ∧ coeff 1 f = 1 := by
  ...
  change limitSeries * iterate limitSeries 2 = rhs
  ext n
  exact (agree_mul hz (iterate_normal (approximation_spec n).1
    (approximation_spec n).2.1 2).1 (limit_agree n)
    (iterate_agree hz (approximation_spec n).1 (limit_agree n) 2) n (by omega)).trans
...
theorem generating_equation :
    generatingSeries * iterate generatingSeries 2 = X ^ 2 + 9 * X ^ 3 ∧
...
private theorem solution_three (f : PowerSeries ℤ)
    (he : f * iterate f 2 = rhs) (hf : constantCoeff f = 0) (hf1 : coeff 1 f = 1) :
    ∀ n, 1 < n → 3 ∣ coeff n f := by
  intro n
  induction n using Nat.strong_induction_on with
  | h n ih =>
    intro hn
    ...
    have hd := error_divisible p hp0 h3 (n + 1)
    rw [map_sub, ht] at hd
    obtain ⟨q, hq⟩ := hd
    exact ⟨q, by omega⟩

theorem hanna_conjecture (n : ℕ) (hn : 1 < n) : 3 ∣ a n :=
  solution_three generatingSeries generating_equation.1 generating_equation.2.1
    generating_equation.2.2 n hn

#print axioms lift_mod_nine
#print axioms generating_equation
#print axioms generating_unique
#print axioms hanna_conjecture

end D5.S1.Recurrence.Residue.IterateProductNineModThree
