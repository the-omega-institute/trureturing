...
/-- `a` is the bilateral sequence with values `x` and `y` at indices zero and one that satisfies the
Fibonacci recurrence at every integer index. -/
def IsBilateralFibonacci (x y : ℤ) (a : ℤ → ℤ) : Prop :=
  a 0 = x ∧ a 1 = y ∧ ∀ k : ℤ, a (k + 2) = a (k + 1) + a k

/-- Position zero is a global absolute-value minimizer; ties are allowed. -/
def min0 (x y : ℤ) : Prop :=
  ∀ a : ℤ → ℤ, IsBilateralFibonacci x y a → ∀ k : ℤ, |a 0| ≤ |a k|

private def closedForm (x y : ℤ) (k : ℤ) : ℤ :=
...
noncomputable def boundedMin0Probability (N : ℕ) : ℚ :=
  by
    classical
...
def claim : Prop :=
  ∀ ε : ℚ, 0 < ε → ∃ N₀ : ℕ, ∀ N : ℕ, N₀ ≤ N →
    |boundedMin0Probability N - 1 / 4| < ε

/-- The bounded-initialization probability does not converge to one quarter. -/
theorem result : ¬ claim := by
  have closedForm_isBilateral (x y : ℤ) : IsBilateralFibonacci x y (closedForm x y) := by
    refine ⟨?_, ?_, ?_⟩
    · simp [closedForm]
    · simp [closedForm]
    · intro k
      have hf0 : Int.fib (k + 1) = Int.fib (k - 1) + Int.fib k := by
        convert Int.fib_add_two (k - 1) using 1 <;> ring
      have hf1 : Int.fib (k + 2) = Int.fib k + Int.fib (k + 1) :=
        Int.fib_add_two k
      simp only [closedForm]
      rw [show k + 2 - 1 = k + 1 by ring, show k + 1 - 1 = k by ring]
      calc
        x * Int.fib (k + 1) + y * Int.fib (k + 2) =
            x * (Int.fib (k - 1) + Int.fib k) + y * (Int.fib k + Int.fib (k + 1)) := by
          exact congrArg₂ (· + ·) (congrArg (fun z => x * z) hf0)
            (congrArg (fun z => y * z) hf1)
        _ = _ := by ring
  have closedForm_values (x y : ℤ) :
      closedForm x y 0 = x ∧
        closedForm x y 1 = y ∧
        closedForm x y 2 = x + y ∧
        closedForm x y (-1) = y - x ∧
        closedForm x y (-2) = 2 * x - y := by
    have hf : Int.fib (-3) = 2 := by decide
    simp [closedForm, hf]
    constructor <;> ring
  have cone (x y : ℤ) (hm : min0 x y) (hx : 0 < x) :
      3 * x ≤ y ∨ y ≤ -2 * x := by
    obtain ⟨h0, h1, h2, hn1, hn2⟩ := closedForm_values x y
    have hmin := hm (closedForm x y) (closedForm_isBilateral x y)
    have h1' := hmin 1
    have h2' := hmin 2
    have hn1' := hmin (-1)
    have hn2' := hmin (-2)
  ...
  have min0_neg (x y : ℤ) (h : min0 x y) : min0 (-x) (-y) := by
    intro a ha k
    have hneg : IsBilateralFibonacci x y (fun j => -a j) := by
      refine ⟨by simp [ha.1], by simp [ha.2.1], fun j => ?_⟩
      simp [ha.2.2 j]
      ring
    simpa only [abs_neg] using h (fun j => -a j) hneg k
  have sum_cast (n : ℕ) :
      2 * (∑ i ∈ Finset.range n, (i : ℤ)) = (n : ℤ) * ((n : ℤ) - 1) := by
    have hz := congrArg (fun m : ℕ => (m : ℤ)) (Finset.sum_range_id_mul_two n)
    simp only [Nat.cast_mul, Nat.cast_sum, Nat.cast_ofNat] at hz
    rcases n with _ | n
    · simp
    · have hsub : ((n + 1 - 1 : ℕ) : ℤ) = (n : ℤ) := by simp
      rw [hsub] at hz
      push_cast at hz ⊢
      linarith
  have down_count (t : ℕ) :
  ...
  have bound_probability (t : ℕ) (ht : 3 ≤ t) :
      boundedMin0Probability (6 * t) ≤ 2 / 9 := by
    ...
    calc
      boundedMin0Probability (6 * t) =
          (((good (6 * t)).card : ℚ)) / (12 * (t : ℚ) + 1) ^ 2 := by
        simp only [boundedMin0Probability, good, square]
        congr 1
        push_cast
        ring
      _ ≤ (30 * (t : ℚ) ^ 2 + 10 * (t : ℚ) + 1) /
...
example : IsBilateralFibonacci 1 1 (fun k => Int.fib (k + 1)) := by
  refine ⟨by simp, by simp, fun k => ?_⟩
  change Int.fib (k + 2 + 1) = Int.fib (k + 1 + 1) + Int.fib (k + 1)
  rw [show k + 2 + 1 = (k + 1) + 2 by ring, Int.fib_add_two]
  ring
