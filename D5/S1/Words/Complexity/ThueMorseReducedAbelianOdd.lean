...
def runCompress : List Bool -> List Bool
  | [] => []
  | [a] => [a]
  | a :: b :: word =>
      if a = b then runCompress (b :: word) else a :: runCompress (b :: word)
...
private theorem parikh_runCompress_cons (a : Bool) (word : List Bool) :
  ...
  | cons b word ih =>
      simp only [runCompress, listRuns]
      split_ifs with h
      · subst a
        simpa using ih b
...
private theorem reducedAbelianComplexity_two : R 2 = 3 := by
