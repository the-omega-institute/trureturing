...
theorem result : (∀ (s : Bool) (n : ℕ),
  ...
  have hequiv : ∀ (s : Bool) (n : ℕ),
    ...
    have certificates (x : Factors s n) :
      ...
      · let k := n - m
        let α : Equiv.Perm (Fin m) := x.2.1.val
        let β : Equiv.Perm (Fin k) := x.2.2.val.val
        change descents (blockSum s α β) = descents α + descents β + if s then 1 else 0
        have leftval (i : Fin m) :
            (blockSum s α β (Fin.castAdd k i)).val =
              (if s then k else 0) + (α i).val := by
          cases s <;> simp [blockSum, Equiv.sumCongr, Equiv.sumComm, Nat.add_comm]
        have rightval (i : Fin k) :
          ...
          cases s <;> simp [blockSum, Equiv.sumCongr, Equiv.sumComm]
        have hleft (i : Fin m) :
            descentAt (blockSum s α β) (Fin.castAdd k i) =
              descentAt α i + if i.val + 1 = m then (if s then 1 else 0) else 0 := by
          ...
          · have hib : i.val + 1 = m := by omega
            ...
            cases s <;> simp only [Bool.false_eq_true, ↓reduceIte, Nat.zero_add]
            · rw [if_neg (by have := (α i).isLt; omega)]
            · rw [if_pos (by have := (β ⟨0, hk⟩).isLt; omega)]
        have hright (i : Fin k) :
            descentAt (blockSum s α β) (Fin.natAdd m i) = descentAt β i := by
        ...
        have boundary : (∑ i : Fin m, if i.val + 1 = m then (if s then 1 else 0) else 0) =
            (if s then 1 else 0) := by
