/- GID: D5/S1/Words/Patterns/ShiehYangYuMachineConvergence
   generality: G
   mirror-B: D5/B/S1/Words/Patterns/ShiehYangYuMachineConvergence
   mirror-E: none(waiver:unbounded-symbolic-proof)
   anchors: []
   utility: none
   digest: Every permutation reaches a fixed point of the Shieh-Yang-Yu 21-dot machine. -/
...
/-!
# Convergence of the Shieh-Yang-Yu 21-dot machine

The definitions and Conjecture 6.2 are from Yang, Shieh, and Yu,
*Stack-Sorting with Dotted-Pattern-Avoiding Stacks*, arXiv:2411.11914v2.
The source defines s_{21̇} through Baril's dotted patterns and PROVES the closed form
of Proposition 3.5; this module takes that proved right-hand side (reverse each
valley run) as the definition of `r`. The identification of the dotted-pattern
stack with valley-run reversal is the source's own theorem and is NOT formalized here.

West's map `s` uses its pop/push/flush algorithm. Reversed words, ordered
lexicographically, give a nondecreasing potential for `M` on words without
repetitions. A maximal potential on the finite reachable orbit forces two
consecutive iterates to coincide. All statements are unbounded symbolic results;
no bounded enumeration, checker, numerical reduction, or certified instance is used.
-/

namespace D5.S1.Words.Patterns.ShiehYangYuMachineConvergence
...
/-- Every positive-length permutation eventually reaches a fixed point of the 21-dot machine. -/
...
end D5.S1.Words.Patterns.ShiehYangYuMachineConvergence
