...
private theorem golden_word_phase (i : ℕ) :
    goldenWord i = true ↔
...
private theorem golden_word_phase_strict (i : ℕ) :
    goldenWord i = true ↔
      1 - goldenMechanicalSlope < goldenFractionalPart (i + 1) := by
...
private theorem golden_phase_step (i d : ℕ) :
...
private theorem interval_run_bound {lo hi step : ℝ} {x : ℕ → ℝ} {length : ℕ}
...
private theorem interval_run_bound_reverse {lo hi step : ℝ} {x : ℕ → ℝ}
...
private theorem paired_rotation_bound {lo step : ℝ} {x : ℕ → ℝ} {length : ℕ}
...
private theorem paired_rotation_bound_reverse {lo step : ℝ} {x : ℕ → ℝ}
...
private theorem golden_map_right_small (i d length : ℕ) (symbol : Bool)
    (hpos : 0 < length)
  ...
  · have hmem (k : ℕ) (hk : k < length) :
    ...
    linarith [hslope_half]

private theorem golden_map_left_small (i d length : ℕ) (symbol : Bool)
    (hpos : 0 < length)
...
private noncomputable def goldenStepDistance (d : ℕ) : ℝ :=
  min (Int.fract ((d : ℝ) * Real.goldenRatio))
    (1 - Int.fract ((d : ℝ) * Real.goldenRatio))

private theorem golden_map_small_step (i d length : ℕ) (symbol : Bool)
...
private theorem golden_map_large_step (i d length : ℕ) (symbol : Bool)
    (hd : 0 < d) (hpos : 0 < length)
  ...
  have htrue : symbol = true := by
    by_contra hb
    have hbfalse : symbol = false := by cases symbol <;> simp_all
    have hword0 : goldenWord i = false := by
      simpa [hbfalse] using hmap 0 (by omega)
    have hword1 : goldenWord (i + d) = false := by
      simpa [hbfalse] using hmap 1 (by omega)
...
private theorem golden_slope_identity :
...
private theorem golden_integer_form_nonzero (d : ℕ) (p : ℤ) (hd : 0 < d) :
...
private theorem golden_small_arithmetic (d N : ℕ) (p : ℤ)
    (hd : 0 < d)
...
private theorem golden_step_nearest_integer (d : ℕ) :
...
private theorem golden_step_positive (d : ℕ) (hd : 0 < d) :
...
private theorem golden_map_small_conjecture (i d length : ℕ) (symbol : Bool)
    (hd : 0 < d) (hpos : 0 < length)
    (hsmall : goldenStepDistance d < 1 - goldenMechanicalSlope)
    (hmap : ∀ k < length, goldenWord (i + k * d) = symbol) :
...
private theorem golden_approx_lower (q : ℕ) (p : ℤ) (hq : 0 < q)
...
private theorem golden_large_arithmetic (d length : ℕ) (hd : 7 ≤ d)
    (δ : ℝ) (hδpos : 0 < δ)
...
private theorem golden_double_step_nearest (d : ℕ) :
...
private theorem golden_map_large_conjecture_of_seven (i d length : ℕ) (symbol : Bool)
...
private theorem golden_small_denominator_windows :
...
private theorem golden_map_large_conjecture_below_seven (i d length : ℕ) (symbol : Bool)
...
private theorem golden_map_all_runs_bound (i d length : ℕ) (symbol : Bool)
    (hd : 0 < d) (hpos : 0 < length)
    (hmap : ∀ k < length, goldenWord (i + k * d) = symbol) :
    ((length - 1 : ℕ) : ℝ) < (3 - Real.goldenRatio) * d := by
