/- GID: D5/S1/Words/BarkerEvenLengthModFourObstruction
   ...
   anchors: []
   digest: Mod-four congruences obstruct even Barker lengths; finite words witness fidelity. -/
...
private theorem two_step_telescoping (f : Nat -> Int) (m : Nat) :
  ...
  calc
    (∑ i ∈ Finset.range m, (f i - f (i + 2))) =
        ∑ i ∈ Finset.range m,
          ((f i + f (i + 1)) - (f (i + 1) + f (i + 2))) := by
      apply Finset.sum_congr rfl
      intro i _hi
      ring
    _ = (f 0 + f 1) - (f m + f (m + 1)) := by
      simpa using Finset.sum_range_sub' (fun i => f i + f (i + 1)) m
    _ = f 0 + f 1 - f m - f (m + 1) := by ring
...
/-- No even length congruent to two modulo four and greater than two supports a Barker
sequence. -/
theorem no_even_barker_of_mod_four_eq_two :
    ∀ (a : Nat -> Int) (n : Nat),
      Even n -> 2 < n -> n % 4 = 2 -> ¬IsBarker a n := by
  intro a n hn hnlarge hnmod hb
  have hdiv := (even_barker_length_mod_four (a := a) (n := n) hn hnlarge).1 hb
  omega
...
#print axioms no_even_barker_of_mod_four_eq_two
