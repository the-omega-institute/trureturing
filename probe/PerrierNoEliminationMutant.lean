...
-- Witness-bypass mutant: no coordinate-elimination lemma, no induction on j.
namespace PerrierNoEliminationMutant
...
/-- The two preregistered identities, universally over recurrence solutions. -/
    ...
    have heq : R r (Fin.last d) =
      ...
      calc
        _ = X^d * (1 + X * R r (Fin.last d)) + A + M * R r (Fin.last d) := by
          ...
          calc
            T + R r (Fin.last d) =
                X^d * R r 0 + (A + T + M * R r (Fin.last d)) := htel.symm
...
end PerrierNoEliminationMutant

#print axioms PerrierNoEliminationMutant.result
