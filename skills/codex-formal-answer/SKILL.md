...
2. `F`: trace useful concepts to exact declarations and statement shapes in `D5/`, rendered meanings in `Blueprint/`, and pinned abstractions in `.lake/packages/mathlib/Mathlib/`. Search the proposed conclusion, its negation, and counterexample shapes. Follow `CLAUDE.md` item 11's repository, mathlib, third-party, local-proof order. Stop external theorem search at an exact repository hit, but continue inferential completion unless that one declaration closes the complete proposition. Reuse an exact declaration; never reprove it or add a renamed copy.
...
Complete `agents/echo-template.md` for exactly one auditable bridge:
...
The renderer is one-way and strength-monotone. Its only epistemic input is the immutable Step 5 register; it may not draft claims afresh from search hits, `G`, or `S`. Draft ordinary prose freely without a phrase table, then reverse-echo every item of communicated content in every heading, lead, body paragraph, and final sentence into a proposition, including at minimum assertive sentences, presuppositions, definite descriptions, rhetorical questions, imperatives, sentence fragments, and conventional implicatures. Match each epistemic proposition to the unique active Step 5 record for its stable assertion key, never to a void record, and require that active record to entail it with no hidden premise. Match each repository-action proposition to the Step 6 `side_effects` record. Reject and redraft every unmatched or stronger item. For this audit, communicating any proposition stronger than an active `open` record's `P` counts as asserting `P`. The answer may not be emitted until the stronger-or-unmapped set is empty.
...
- For an unsettled proposition, communicate neither `P` nor its negation through an assertive sentence, presupposition, definite description, rhetorical question, imperative, sentence fragment, or conventional implicature, including after "my judgment is." Ground any practical recommendation separately and say that it does not settle the question.
