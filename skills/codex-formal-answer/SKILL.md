...
1. `reuse-complete`: one existing declaration already proves the whole reusable `G` or canonical `P`, including every clause and boundary. Retain the compiled exact `S` application from Step 4 as `tracked-S` with its canonical `Describe`, cite the retained `project-source`, verify the retained tree with `make lean`, and stop persistence. Multiple adjacent hits that still require a deduction do not qualify.
2. `persist-generalization`: a new reusable `G` closes a genuine inferential gap, passes the `generalization-audit`, has a semantically distinct specialization or nontrivial countermodel, reuses existing declarations, and compiles without `sorry` or nonstandard axioms. Retain `G` as `tracked-lean` and its applying `S` as `tracked-S`, each in its canonical module with a matching canonical `Describe` source; the descriptions state the model's interpretive and empirical limits and include enough ordinary concept vocabulary for the same or an analogous future question to find them during `C`. Keep the required import-graph changes and verify the retained tree with `make lean`; use `make lean-report` only when the answer needs machine-readable closure.
3. `retain-specialization-open`: `S` successfully compiles exact `P`, but no existing complete `project-source` supplies the reusable inference and no new `G` passes the `generalization-audit`. Retain `S` as `tracked-S` with its canonical `Describe`, record why the reusable obligation failed, verify the retained tree with `make lean`, and keep the formal result `open`; retention records the exact proof without misclassifying it as reusable completion.
