...
codex-formalize owns digestion atoms and their deposit and coverage workflow. This skill owns the user's clause-complete `P`/`G`/`S` bridge, inferential completion, assertion register, renderer, and reusable formal-library growth. Do not import freezing, deposit, coverage, or truth-DAG publication machinery. Every formalizable gap must end in one of three states: existing results supply the complete inference and are reused, an admissible new abstract `G` is retained as canonical project source, or the gap remains `open`. Every claimed exact `S` requires a successful current cache-guarded compile and accepted axiom closure; compilation alone does not entitle it to retention or close a missing reusable-library obligation.

**Admission and retention precedence (`CLAUDE.md` §3.2).** For this skill's ordinary formal-library work, that section governs every new declaration and every retention instruction below, including `tracked-lean` and `tracked-S`. A bind-only specialization/application is transient compiled evidence, not new delivered content: use a temporary exact-check `example` or a current build of an existing module that already checks the exact application. Do not add a tracked named declaration, canonical `Describe`, or audit wrapper solely to retain an existing result's application; private declarations and retained `example` packaging cannot evade the ban. Retain real new mathematical declarations only when individually admissible under §3.2 (including its explicitly allowed external open-problem resolution), with the same-delivery Reg audit proofs required by §3.9 for newly authored public theorems/lemmas in selected D5 source modules. Bind-only checks of a newly proved `G` also remain transient. This preserves clause-complete compiled evidence without creating a new persistence class or ledger; it does not alter the assertion-record or plain-answer procedures.
...
3. `in-repository`: the thinking that produces every answer is done inside the repository worktree, as files, not in prose. Every `formalizable` assertion requires exact Lean evidence judged by a scoped build before Step 5 (a full-project `make lean` is optional): on the reuse route, an `S` applies the cited results to exact `P` through a transient exact check or an existing module check; on the new-`G` route, the admissible reusable `G` is written under `D5/` in its canonical mathematical subject domain and `S` applies it to exact `P`. Prose that was never compiled yields no formal outcome, and reading a module is not the same act as compiling against it (`rendered_formal_claim_is_compiled`). Step 6 retains admissible new mathematical declarations and their canonical Describe sources and required Reg audit proofs under the admission and retention rule above; retained specializations individually admissible under §3.2 use `tracked-S`, while a bind-only `S` remains transient and a missing reusable abstraction remains `open`.
...
Those sections own the same-delivery author and reviewer duty for every newly authored public `theorem` or `lemma` declaration in a selected D5 source module selected by the existing delta/first-pin scope. D5 content-template declarations remain in scope. A helper declaration in `Reg`, `Interface` or `Impl` is outside the selected D5 source set, creates no recursive Reg/D5 audit target, and continues to receive its existing kernel, `sorry` and axiom checks. Whenever this skill writes and retains such a declaration, deliver at least one `declared_validated` four-slot registration in `Reg/D5/<mirrored source path>.lean`, with compiled Lean proof terms and current binding evidence. Validate the actual realization bridge, lawful enrolled-template application, and any variation/sensitivity proof required by that bridge. `escape continues` must be an `EscapeResidualWitness`, `EscapeResidualEmpty`, or literal `open`; `open` records only an unknown residual, proves neither undecidability nor inexhaustibility, and cannot replace the bridge or other required evidence. Missing or invalid registration evidence leaves delivery incomplete even if CI is green or DTR emits only Observe warnings. Do not backfill historical theorems. A reuse-only answer or other zero-new-Lean path creates no audit target and must not add a bind-only wrapper; the required Reg evidence is internal completion evidence even when the final answer remains plain prose.
...
Use the repository's cache-guarded build entry with explicit module targets. Populate `answer_modules` with the actual dotted module names obtained from the selected `.lean` paths (remove `.lean` and replace `/` with `.`). For a retained newly authored public theorem/lemma declaration in a selected D5 source module, also populate its mirrored `Reg.D5...` target and build both through the wrapper; a helper declaration in `Reg`, `Interface` or `Impl` creates no recursive Reg/D5 target, and the plain root `lake build Reg...` form is wrong. Run:

```sh
/bin/bash tools/scripts/worktree/lean-cache-run.sh --build "${answer_modules[@]}" "${reg_modules[@]}"
```

The equivalent canonical door is `make lean LEAN_TARGETS="D5.<dotted.module> Reg.D5.<mirrored.dotted.module>"`. When no new public declaration is written, omit `reg_modules` and retain the zero-new-Lean reuse path; do not manufacture an audit wrapper.

The target list must be nonempty: bare `lake build` selects the full project. This is the same cache wrapper used by `make lean`; keep its pin, cache, and writer checks. A broader build is needed only when the requested verification scope or changed shared infrastructure calls for it, or when a separate integration check requires it.

Apply this scope to Steps 4–6. Exact-statement and accepted axiom-closure checks still cover every claimed result; when a machine-readable report is needed, the existing inspector may inspect only the selected modules. Do not invoke a full `make lean-report` solely to obtain those scoped checks. Reuse a successful current build for settlement and retention while its source and dependency inputs are unchanged. Record the selected modules, command, exit code, and axiom evidence; disclose targeted success without claiming whole-repository health. Required CI and admission checks keep their own scope.
...
Search chooses only the implementation route: reuse existing results that supply the complete reusable `G` or exact `P`, with bind-only applications checked as transient compiled `S` evidence and no new tracked declaration, Describe, or audit wrapper; otherwise compile an admissible new load-bearing abstract `G` for canonical retention; or, for a `not-formalizable` record only, produce no Lean evidence. There is no successful thin-only route: when existing results do not cover the complete inference and no reusable `G` can be built, the formal result remains `open`. Failed proof, unavailable compiler, elapsed effort, and convenience do not revise Step 1; they leave the internal result unsettled.
...
Before writing either `G` or `S`, complete a `placement-audit`: choose its `canonical-domain` from the theorem's `mathematical-subject`, the strongest reused or imported declaration's `dependency-neighborhood`, and adjacent `D5/` and `Blueprint/` modules. The workflow that produced a file does not own its mathematical source. `D5/S3/ConceptDynamics/Answering/` is reserved for mathematics about answerability, settlement, assertion registers, and answer rendering; it is not a default destination for Lean produced by this skill. Place each source in the smallest established subject domain, co-locating it with its primary abstraction when that agrees with the current taxonomy, and record the path decision before compilation.

Implement the missing composition through this ordered completion. Before writing a newly authored public theorem or lemma declaration in a selected D5 source module in `G` or `S`, reserve its mirrored Reg path and the same-delivery four-slot audit evidence required above; helper declarations in `Reg`, `Interface` or `Impl` do not create a recursive Reg/D5 target and keep their existing kernel, `sorry` and axiom checks. This requirement does not turn a reuse application into a new source obligation.
...
2. Use `G` to implement only the missing composition. If existing reusable `project-source` and upstream results do not already supply it and it is individually admissible under §3.2, write `G` as `tracked-lean` in the smallest canonical module and run the `generalization-audit`; a conjunction of unrelated hits, a renamed theorem, a conclusion repeated as a hypothesis, hard-coded scenario data, or definitions chosen to make the conclusion reflexive is not completion.
3. Use `S` to apply `G` to exact `P` and pass exact-statement, successful-build, and axiom-closure checks. A bind-only application is transient evidence; `tracked-S` additionally requires individual admission under §3.2. For a broad question, separate interpretations and prove their boundary instead of forcing an ambiguous yes/no predicate.

On the reuse route, carry out Step 2's reuse decision: compile the exact `S` application through the current cache-guarded build entry as a temporary exact-check `example` or check an existing module that already establishes that exact application. Add no retained specialization module, Describe, renamed `G`, or audit wrapper for this bind-only check. On the new-`G` route, run `make help`, inspect current project structure, and measure compiler capability with the current cache-guarded build entry and exit codes; never use a cold bare Lake command. Write admissible `G` directly in the smallest canonical module with a matching canonical `Describe`, connect it to the import graph, and compile both `G` and exact `S`; retain `S` in a canonical specialization module with a matching Describe only if independently admissible under §3.2, otherwise check it transiently. Before settlement, compile either a semantically distinct specialization of `G` or a nontrivial inhabited countermodel, verify that scenario-specific names and constants occur only in `S` or examples, and remove any unused hypothesis exposed by the audit. Apply the same admission rule to those checks. If that audit cannot distinguish `G` from a question-specific wrapper, keep the assertion `open` and retain only independently admissible mathematical content; a successful bind-only check does not justify retaining a wrapper or calling the reusable gap complete. On the no-Lean-evidence route, which exists only for `not-formalizable` records, write no ornamental definition or scenario wrapper.

Capability failure is not evidence for or against `P`. A failed or unavailable compile leaves the formal result unsettled.

Postcondition: the record contains either exact reused sources plus compiled exact `S` evidence, the smallest admissible substantive compiled abstract `G` ready for canonical retention plus compiled exact `S` evidence, an independently admissible retained exact specialization whose reusable gap remains `open`, or explicit evidence for no formal result. If a newly authored public theorem or lemma declaration in a selected D5 source module is delivered, the record also contains its mirrored Reg registration, compiled Reg proof and current binding-evidence check; helper declarations in `Reg`, `Interface` or `Impl` do not create that target and remain covered by their existing kernel, `sorry` and axiom checks. A green D5 build or Observe warning alone cannot satisfy this postcondition. Bind-only applications remain transient compiled evidence; only admissible new mathematical declarations remain in project source, and exact `S` evidence alone never masquerades as a missing reusable inference.

### 5. Settle outcomes and freeze the answer register

Use a successful current build covering the claimed declarations under Compilation scope, together with their exact compiled statements, as the authority for any formal result. Reject evidence with `sorry`, a statement mismatch, a failed command, or a nonstandard axiom closure. Apply the first matching rule:
...
For a formalizable assertion not covered by existing results under the reuse rule, Step 4's compiled abstract `G`, compiled exact applying `S`, and passed `generalization-audit` are preconditions for a settled formal result. Compilation of `S` alone guarantees neither retention nor settlement: admission still follows §3.2, and the reusable gap remains `open`.
...
1. `reuse-complete`: existing declarations already supply the whole reusable `G` or canonical `P`, including every clause and boundary, directly or through §3.2's bind-only operations. Cite the existing reusable `project-source` or upstream declarations and verify the exact `S` application under Compilation scope (full-project `make lean` is optional). Because §3.2 excludes new bind-only declarations, this `S` is not newly retained as `tracked-S`. Remove any temporary exact-check source after validation; add no tracked declaration, Describe, or audit wrapper and stop persistence. A hit list with an unproved inferential gap does not qualify.
2. `persist-generalization`: an individually admissible new reusable `G` closes a genuine inferential gap, passes the `generalization-audit`, has a semantically distinct specialization or nontrivial countermodel, reuses existing declarations, and compiles without `sorry` or nonstandard axioms. Retain `G` as `tracked-lean`; retain an applying `S` as `tracked-S` only if individually admissible under §3.2, otherwise remove its transient exact-check source after validation. Each retained mathematical module has a matching canonical `Describe` source; the descriptions state the model's interpretive and empirical limits and include enough ordinary concept vocabulary for the same or an analogous future question to find them during `C`. Keep the required import-graph changes and same-delivery Reg audit proofs for newly authored public theorem/lemma declarations in selected D5 source modules, and verify the retained modules under Compilation scope (full-project `make lean` is optional); helper declarations in `Reg`, `Interface` or `Impl` do not create recursive Reg/D5 targets and retain their existing kernel, `sorry` and axiom checks. Obtain machine-readable closure only when needed and keep its module scope explicit.
3. `retain-specialization-open`: `S` successfully compiles exact `P` and is individually admissible under §3.2, but no existing results supply the reusable inference and no new `G` passes the `generalization-audit`. Retain that admissible `S` as `tracked-S` with its canonical `Describe` and, when it contains newly authored public theorem/lemma declarations in selected D5 source modules, their required Reg audit proofs; record why the reusable obligation failed, verify the retained modules under Compilation scope, and keep the formal result `open`; retention records the exact proof without misclassifying it as reusable completion.
4. `discard-thin`: the only new code is a non-admissible bind-only wrapper, an import wrapper, ornamental definition, hard-coded question data that does not implement `P`, a theorem whose conclusion is assumed, or other `failed-or-uncompiled` material with no reusable value. Record the rejection evidence and remove those artifacts even if a bind-only check compiled; keep any unresolved inferential gap `open`. A successfully compiled, individually admissible exact `S` never enters this route merely because it is scenario-specific.
5. `open-compile`: otherwise, including an unavailable compiler, failed elaboration, unresolved model, or nonstandard axiom closure. Preserve the exact source, command, exit code, and diagnostics in the current-run record as `open`; never report the proposition as proved.

Compare the worktree with its pre-run state. Recheck the `placement-audit` against the final imports, and relocate any misplaced admissible successful source together with its canonical Describe rather than deleting it or routing it through `discard-thin`. Retain successful individually admissible new `G` and `S` declarations, their canonical Describe sources, required import changes, and same-delivery Reg audit registrations/proofs for newly authored public theorem/lemma declarations in selected D5 source modules; helper declarations in `Reg`, `Interface` or `Impl` do not create recursive Reg/D5 targets and retain their existing kernel, `sorry` and axiom checks. Retain literal `open` residual evidence when the other slots validate, without treating it as a theorem of undecidability or inexhaustibility. Remove transient exact-check sources and non-admissible bind-only wrappers, as well as failed, uncompiled, statement-mismatched, or ornamental artifacts with no reusable value; never remove pre-existing project source as part of this cleanup. Useful admissible generalization is not discarded merely because prose can answer or the current user did not ask to see code, and a specialization acquires neither `G` status nor `tracked-S` retention merely by compiling. If authorization does not permit a commit or pull request, leave all admissible retained source present in the worktree and disclose the integration state; do not delete it to make the tree appear clean. Commit the coherent local unit only when repository policy and the user's request allow; never open or advance a pull request unless requested.

Emit an internal `side_effects` record containing the final project paths, the selected module targets, exact build command and exit code, the axiom output or report or an explicit reason none was produced, any other verification commands and exit codes, whether anything was committed, and the plain verification result. If nothing was written, record that fact and an empty path set without inventing a mutation.

Postcondition: persistence is settled, the final tree is accounted for, transient checks and non-admissible wrappers are not delivered, every discarded content candidate is non-admissible or unsuccessful and has no reusable value, and every compiler failure is evidence-complete. Every claimed exact specialization has compiled evidence; only individually admissible new mathematical declarations are retained, with required Reg audit proofs for newly authored public theorem/lemma declarations in selected D5 source modules. Helpers in `Reg`, `Interface` and `Impl` keep their existing kernel, `sorry` and axiom checks without creating recursive Reg/D5 targets. Every successfully closed reusable gap is reachable through existing results or admissible retained canonical `G` source so a later run can reuse it. `side_effects` describes the state and integration status that Step 7 will report.

### 7. Render the plain answer

For this skill's output, default to ordinary conversational prose. By default omit `P`/`G`/`S` labels, GIDs, proof-provenance module paths, build receipts, axiom closures, search traces, and the outcome-vocabulary words. This is not a repository-wide register policy. The rendering rule cannot hide required internal source or Reg audit evidence: keep those artifacts and checks in the run record, and disclose them when proof review or the user's request requires the record.
