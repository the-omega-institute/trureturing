...
Implement the missing composition through this ordered completion. Before writing a new public theorem or lemma in `G` or `S`, reserve its mirrored Reg path and the same-delivery four-slot audit evidence required above; this requirement does not turn a reuse application into a new source obligation.
...
Postcondition: the record contains either exact reused sources plus compiled exact `S` evidence, the smallest admissible substantive compiled abstract `G` ready for canonical retention plus compiled exact `S` evidence, an independently admissible retained exact specialization whose reusable gap remains `open`, or explicit evidence for no formal result. If a new public theorem or lemma is delivered, the record also contains its mirrored Reg registration, compiled Reg proof and current binding-evidence check; a green D5 build or Observe warning alone cannot satisfy this postcondition. Bind-only applications remain transient compiled evidence; only admissible new mathematical declarations remain in project source, and exact `S` evidence alone never masquerades as a missing reusable inference.
