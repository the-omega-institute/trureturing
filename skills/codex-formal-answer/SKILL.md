...
2. Use `G` to implement only the missing composition. If existing results do not already supply it and it is individually admissible under §3.2, write `G` as `tracked-lean` in the smallest canonical module and run the `generalization-audit`; a conjunction of unrelated hits, a renamed theorem, a conclusion repeated as a hypothesis, hard-coded scenario data, or definitions chosen to make the conclusion reflexive is not completion.
