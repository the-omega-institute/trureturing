...
Use the repository's cache-guarded build entry with explicit module targets. Populate `answer_modules` with the actual dotted module names obtained from the selected `.lean` paths (remove `.lean` and replace `/` with `.`). Populate `reg_modules` only from this delivery's actual retained Reg sources, including mirrors and support/enrollment sources. When no valid Reg source can be delivered, omit that target and use the linked-issue exception; do not invent a mirror or placeholder target. Helpers create no recursive audit target, and the plain root `lake build Reg...` form is wrong. Run:

```sh
/bin/bash tools/scripts/worktree/lean-cache-run.sh --build "${answer_modules[@]}" "${reg_modules[@]}"
```

For a retained mirror, the equivalent canonical door is `make lean LEAN_TARGETS="D5.<dotted.module> Reg.D5.<mirrored.dotted.module>"`. When no Reg source is retained, use only the actual mathematical targets. Retain successful audit sources, compiled proofs and current binding evidence. Remove this attempt's failed audit source additions and imports if they would break the normal build; do not remove pre-existing or successful sources to hide failures. Every retained Reg source still requires compilation and its existing kernel, `sorry` and axiom checks.
...
Implement the missing composition through this ordered completion. For each new public D5 theorem/lemma in `G` or `S`, attempt the faithful mirrored audit described above; a concrete registration obstacle takes the linked-issue route without stopping this completion. Reuse applications and helpers create no artificial audit target.
...
Capability failure is not evidence for or against `P`. A failed or unavailable mathematical compile leaves the formal result unsettled. A registration-only failure follows the audit exception and neither proves nor refutes `P`.

Postcondition: the record contains either exact reused sources plus compiled exact `S` evidence, the smallest admissible substantive compiled abstract `G` ready for canonical retention plus compiled exact `S` evidence, an independently admissible retained exact specialization whose reusable gap remains `open`, or explicit evidence for no formal result. For each delivered public D5 theorem/lemma in scope, the record also contains either its mirrored Reg registration, compiled proof and current binding-evidence check, or the linked issue and explicit unfinished-audit disclosure required above. Bind-only applications remain transient compiled evidence; only admissible new mathematical declarations remain in project source, and exact `S` evidence alone never masquerades as a missing reusable inference.
