...
- **Not a standalone target — bind-only corollaries** (`CLAUDE.md` 5⁗): a statement whose proof, once every `import`/`open`/frozen-theorem application is stripped, is one application of a frozen theorem (including discharging a hypothesis with a frozen witness), an `obtain ⟨x⟩` followed by one frozen theorem, a `.1`/`.2`/`.mp`/`.mpr` projection or a composition of frozen iffs, a `Finset.sum_empty`/`simp`/`ring`/`linarith`-level rewrite, or a TFAE/conjunction packaging of frozen iffs. Such a statement is digestion (if a frozen theorem covers the atom verbatim, `make cover` it — zero Lean) or a companion lemma inside a content-bearing module; it is never the freeze target of its own deposit. If every open atom in reach is bind-only, the valuable output is `open` naming the missing analytic input, not a deposit.
...
If the dispatched sandbox forbids git writes, state that constraint explicitly and hand the exact `git push` and `make pr-open` invocations above, with substituted arguments, to the caller; do not report `success` as though the work landed. Otherwise report `success` only with the opened pull request, touched paths, door-produced commit subjects, every relevant exit code, completed fidelity-gate evidence, and `proof_shape` (`content` | `bind-only`, with the non-bind line count) for every public theorem. Or report `open`, naming the stopping step and carrying every evidence class reached; mark each unreached class not run and explain why. There is no third outcome.
...
- **Bind-only deposits are thin deposits (escape content).** Strip each public theorem's proof of every `import`, `open`, and frozen-theorem application and count what remains. Record `proof_shape: content | bind-only` and the number of non-bind lines for every public theorem in the result envelope and in a PR-body section 「逃逸内容」. A deposit whose freeze target is `bind-only` must not be deposited: stop before `make deposit` and report `open` with reason `proof_shape: bind-only`. Landed precedent (2026-09-04): across five RH-route addenda, 11 of 12 obligations (W-10…W-21) were de-hypothesizations, `.1` projections, `obtain ⟨Z⟩`-then-one-line, or `activePrimePowers = ∅`; only W-12 (an integrability proof) carried content. The owner judged the sequence 平推 and the fifth same-shape lane was stopped unlanded. Bind-only corollaries may ride in the same PR as the content theorem they package; they may not stand alone.
...
- Never deposit a module whose freeze target is a bind-only corollary of frozen theorems; `CLAUDE.md` 5⁗ owns this rule (report `open` with `proof_shape: bind-only` instead).
