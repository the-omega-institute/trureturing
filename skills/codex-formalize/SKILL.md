...
- **Not a standalone target — bind-only corollaries** (`CLAUDE.md` 5⁗): after inlining local aliases, private helpers, tactic expansions and definitional equalities, the statement follows from frozen public theorems only by instantiation (substituting parameters), discharging a frozen hypothesis with a frozen witness (de-hypothesization), logical projection or repackaging (`.1`/`.2`/`.mp`/`.mpr`, conjunctions, TFAE), or normalization that introduces no new mathematical fact. Non-exhaustive landed examples: one application of a frozen theorem; `obtain ⟨x⟩` then one frozen theorem; projections or compositions of frozen iffs; `Finset.sum_empty`/`simp`/`ring`/`linarith`-level rewrites; TFAE/conjunction packaging. A probe may classify a candidate; once it is bind-only it is digestion (if a frozen theorem covers the atom verbatim, `make cover` it — zero Lean) or a named companion lemma inside the same new frozen module that carries a content theorem — never the sole content of a new frozen module. If every open atom in reach is bind-only, the valuable output is `open` naming the missing analytic input, not a deposit. Bridges, instantiations and characterizations that an atom explicitly requires, connect previously unconnected concepts and have a named downstream use remain Good odds; the thinnest honest upstream wrapper required by 第 11 条 is likewise not bind-only.
...
- **Bind-only modules are thin deposits (escape witness).** For every public theorem, record in the Step 8 report: `proof_shape: content | bind-only`, its frozen public dependencies, and an `escape_witness` — one precisely stated new intermediate proposition that is NOT obtained from those dependencies by instantiation, projection or normalization, together with the declaration that proves it. No witness ⇒ `bind-only`. Source-line counts are non-load-bearing readings, never the criterion. A new frozen module none of whose public theorems carries an escape witness must not be deposited: stop before `make deposit` and report `open` with reason `proof_shape: bind-only`; the `make deposit` anchor GID does not change the module-level classification. Landed precedent (2026-09-04, itemized): across five RH-route addenda (W-10…W-21, PRs #5177/#5186/#5207/#5219), W-12 (an integrability proof) was the analytic escape; W-18's helper `activePrimePowers_eq_empty_of_exp_lt_two` and the W-14/W-15 companion `exists_supportRadius` were small new lemmas whose sufficiency is judged by this criterion; W-10/11/13/14/15/16/17/19/20/21 were de-hypothesizations, projections, `obtain ⟨Z⟩`-then-one-line or compositions of frozen iffs. The owner judged the sequence 平推 and the fifth same-shape lane was stopped unlanded. Bind-only corollaries may ride as named companions inside the content-bearing module; they may not stand alone.
...
- Never deposit a new frozen module none of whose public theorems carries an escape witness; `CLAUDE.md` 5⁗ owns this rule (report `open` with `proof_shape: bind-only` instead).
