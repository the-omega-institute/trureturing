...
Those sections own the author and reviewer obligation for a newly authored public `theorem` or `lemma` declaration in a selected D5 source module selected by the existing delta/first-pin scope. D5 content-template declarations remain in scope. A helper declaration in `Reg`, `Interface` or `Impl` is outside the selected D5 source set, creates no recursive Reg/D5 audit target, and continues to receive its existing kernel, `sorry` and axiom checks. The same delivery must include at least one `declared_validated` four-slot registration in `Reg/D5/<mirrored source path>.lean`, with compiled Lean proof terms and current binding evidence. Check the actual realization bridge, lawful enrolled-template application, and any variation/sensitivity proof required by that bridge. `escape continues` must be an `EscapeResidualWitness`, `EscapeResidualEmpty`, or literal `open`; `open` truthfully records an unknown residual and proves neither undecidability nor inexhaustibility, and it never substitutes for the bridge or missing evidence. Missing or invalid registration evidence makes the delivery incomplete even when CI is green or DTR reports only Observe warnings. Do not backfill historical theorems. Reusing existing Lean without a new declaration creates no audit target and must not be satisfied by an artificial bind-only wrapper; registration proofs are audit evidence in `Reg`, not a new mathematical GID/Scribe/deposit obligation.
...
Before creating the new module, set `lean_dir` to its target directory and `blueprint_dir` to the corresponding `Blueprint/` mirror directory, then read the live limit from its owner (`DirectoryFileLimit` in `tools/StrataLint.Engine/Rules/RepositoryRules.Structure.cs`; never a number remembered from this file), measure both, and confirm that adding one counted file to each stays within that limit:

```sh
limit=$(sed -nE 's/.*DirectoryFileLimit = ([0-9]+);.*/\1/p' tools/StrataLint.Engine/Rules/RepositoryRules.Structure.cs); test -n "$limit"; lean_count=$(git ls-files "$lean_dir" | awk 'END { print NR+0 }'); blueprint_count=$(git ls-files "$blueprint_dir" | awk '!/\.md$/ { n++ } END { print n+0 }'); printf 'limit %s\n%s %s\n%s %s\n' "$limit" "$lean_dir" "$lean_count" "$blueprint_dir" "$blueprint_count"; test $((lean_count + 1)) -le "$limit" && test $((blueprint_count + 1)) -le "$limit"
```
...
When this delivery adds a newly authored public theorem or lemma declaration in a selected D5 source module, compile its D5 target and the mirrored Reg target together through the governed entry (the plain root `lake build Reg...` form is not valid); helper declarations in `Reg`, `Interface` or `Impl` do not create a recursive Reg/D5 target and retain their existing kernel, `sorry` and axiom checks:
...
The compiled Reg proof and current binding-evidence check are required completion evidence; retain the Reg source/proof with the delivery for review, and treat a successful D5 build alone as insufficient.
...
If the dispatched sandbox forbids git writes, state that constraint explicitly and hand the exact `git push` and `make pr-open` invocations above, with substituted arguments, to the caller; do not report `success` as though the work landed. Otherwise report `success` only with the opened pull request, touched paths, door-produced commit subjects, every relevant exit code, completed fidelity-gate evidence, and, for every newly authored public theorem/lemma declaration in a selected D5 source module, its `Reg/D5/<mirrored source path>.lean` registration path, compiled Reg target exit code, validated four-slot/binding-evidence result and continuation kind. Helpers in `Reg`, `Interface` or `Impl` do not create that target and remain covered by their existing kernel, `sorry` and axiom checks. `proof_shape` (`content` | `bind-only`), direct frozen public dependencies (GID + `statement_id`), `escape_witness` (or `bind-only`) and the module's `admission_basis` remain required for the mathematical declaration. A green build or Observe warning alone is never completion evidence. Or report `open`, naming the stopping step and carrying every evidence class reached; mark each unreached class not run and explain why. There is no third outcome.
...
- **A `FromLiterature` Describe drags in a Library note, and that note has a machine-checked shape (`incomplete-library-locator`, RED; landed 2026-09-06).** Whenever a Describe carries `AssessedProvenance.FromLiterature(<bibkey>)` (or an acknowledgement reference), the note at `Library/<Area>/<bibkey>.md` is pulled into the check. Owner: `DescribeContentGovernance.ValidateReferencedNoteLocators` in `tools/StrataLint.Scribe/Verification/DescribeContentGovernance.cs`. It is a **RED inside `DESCRIBE_STATUS`**, so it fails the `Canonical Lean report production` required check — **not** the engineering one, which is where you would look first if you only read the check name.
  **The three conditions, verbatim from the source:** the note text must contain `\n## Verified locator\n` (or `\n## Locator\n`); the section body, taken up to the next `\n## `, must be **non-blank**; and that body must contain every declared locator: the frontmatter `doi:` value (case-insensitive) when it is not `null`, and the `url:` value (byte-exact) when present. Putting a locator only in the frontmatter **is not enough** — the frontmatter is what the checker compares *against*, not what satisfies it.
  **Frontmatter is a closed alphabet** (`LibraryNoteCatalog.RequiredKeys`): exactly `bibkey`, `authors`, `year`, `title`, `doi`, `claim`, `strata_touched`, `license`, `triage`, plus the optional `url`. Write `doi: null` when the source has no DOI; give a DOI, a stable HTTPS `url`, or both. Required lines: `bibkey`, `title`, `authors`, `claim`, `license`, `triage`, and `year` as an integer in 1000–9999. Copy a current tracked neighbour in the same `Library/<Area>/` and change the values.
...
- Never deposit a first-freeze module without `admission_basis: escape-witness`; `CLAUDE.md` §3.2 owns the rule. A wholly bind-only module has no admission basis, even with an atom requirement or promised downstream consumer.
