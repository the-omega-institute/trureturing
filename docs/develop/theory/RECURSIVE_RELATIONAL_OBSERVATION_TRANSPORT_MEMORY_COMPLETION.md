...
若下一动作由全部既有可见档案确定，则两个同商初态具有相同档案，策略下一步选同一动作，CE.19 又给相同下一读数。逐步归纳，任何这样的自适应确定协议仍不能区分它们。如果使用与初态独立的随机种子，固定同一种子后同一论证成立，因此观察分布也相同。

这要求合法动作菜单、合法性、费用、停止规则以及额外报告都只依赖可见档案。随机种子须与初态独立，且同一来源族使用相同的种子分布；若种子本身携带源信息，它就是另一个观察。若“动作是否合法”或执行耗时泄露了隐藏态，该报告就是新增接口，不能仍称只观察 $q$。
