...
## 26.99 追加锚
## 27. 有限窗口的总状态容量、交换实现与可逆循环

**定义 27.1（全部来源的一次准备合同）。** 固定非空有限集 $S$、总更新 $D:S\to S$、有限字母表 $A$、读出 $q:S\to A$ 和整数 $n\ge0$。记

$$
w_n(s)=(q(s),q(Ds),\ldots,q(D^ns)),\qquad
W_n=w_n(S),\qquad N_n=|W_n|,\qquad R_n=\ker w_n.
\tag{27.10}
$$

一个实现是有限完整状态集 $M$ 及总映射 $I:S\to M$、$F:M\to M$、$g:M\to A$，满足

$$
g(F^jI(s))=q(D^js)\qquad(s\in S,\ 0\le j\le n).
\tag{27.11}
$$

$F,g$ 固定而不随时刻变化。准备 $I$ 可一次读取整个来源，此后只有内部更新，没有再次访问来源、免费外置时钟或额外停止动作；时刻 $n$ 之后的输出在本合同中自由。容量为总数 $|M|$，包括未被 $I$ 直接取到的状态，以及所有周期相位、控制器和历史。全源准备是允许的操作，不附取得成本结论；固定转移规则的描述长度也不是这里的优化量。这延续定义26.4的有限任务，而将优化目标固定为总载体。

$I$ 总定义是基本要求；另加 $I(S)=M$ 称满射准备，另加 $I\circ D=F\circ I$ 称交换实现，二者是不同条件。称 $F$ 为置换的实现可逆；未另加条件的实现称自由实现。$S$ 非空且 $q$ 存在保证 $A$ 非空。

这里的未约束未来是输出规格的留白。作为模型比较，Renkin 等，*Effective Reductions of Mealy Machines*，[arXiv:2206.10228v1](https://arxiv.org/abs/2206.10228v1)，定义1–3（第2–3页）研究部分输入转移、允许输出集合及其行为包含意义下的特化；Giantamidis–Tripakis，*Learning Moore Machines from Input-Output Traces*，[arXiv:1605.07805v2](https://arxiv.org/abs/1605.07805v2)，§§3.1–3.2、5.1（第3–5、8页）的有限迹从固定 $q_0$ 开始，LMoMIO 也不要求得到最小机器。它们的输入与初态合同不提供式(27.11)中全部来源分别准备的总状态最小值；以下在本合同内直接证明。

**定理 27.2（自由确定实现的精确容量）。** 式(27.11)的最小总状态数恰为 $N_n$，且可由满射准备达到。具体地，对每个 $w\in W_n$ 选一个 $r_w\in S$ 满足 $w_n(r_w)=w$，取

$$
M=W_n,\qquad I(s)=w_n(s),\qquad g(w)=w_0,\qquad
F(w)=w_n(Dr_w).
\tag{27.12}
$$

这个实现从任一状态开始的每个长 $n+1$ 输出窗口都属于 $W_n$，但不必与实际来源 $D$ 交换。代表无关的来源下降
...
存在当且仅当 $R_n=R_{n+1}$；相等时式(27.12)的任意代表选择都给这个同一下降。

证明。若 $I(s)=I(t)$，确定性及式(27.11)给 $w_n(s)=w_n(t)$。从每个实际词的来源纤维中取一个元素，其初始状态两两不同，故 $|M|\ge|I(S)|\ge N_n$。

只需一个重合观察来验证达到构造。设任意状态集 $X$ 上有更新 $U$ 和长 $n+1$ 词标签 $L(x)$，并且

$$
L(Ux)_k=L(x)_{k+1}\qquad(0\le k<n).
\tag{27.14}
$$

以首符号读出，则对 $0\le j\le n$，反复应用该等式得到 $L(U^jx)_0=L(x)_j$：$j=0$ 是恒等，$j>0$ 时依次把下标从零移到 $j$。因此状态 $x$ 的实际输出窗口就是 $L(x)$。$n=0$ 时重合区间为空，结论仅要求当前首符号，仍成立。

式(27.12)在 $W_n$ 上总定义，因为 $D r_w\in S$；对 $k<n$，其后继标签的第 $k$ 项为 $q(D^{k+1}r_w)=w_{k+1}$。用式(27.14)便得式(27.11)，并且 $I$ 满射、$|M|=N_n$。对任何后继状态再用同一观察，它的输出窗口等于自身的 $W_n$ 标签；故不论继续多少步，窗口都仍在 $W_n$。这不要求它们是原来那个来源的后续窗口。

来源下降的闭性机制归属既有 [GradedPredictionShift](../../../D5/S3/ObserverMemory/Refinement/GradedPredictionShift.lean) 的 `graded_prediction_shift`、`closedUpdate`。在当前记号下，式(27.13)良定恰要求 $sR_nt\Rightarrow DsR_nDt$。直接按坐标有

$$
R_{n+1}=R_0\cap(D\times D)^{-1}R_n\subseteq R_n.
\tag{27.15}
$$

若上述稳定性成立，由 $R_n\subseteq R_0$ 得反向包含，故连续两核相等；反之，相等和式(27.15)给稳定性。稳定时任取同词代表都给相同后继，故式(27.12)就是该下降。没有稳定性时，式(27.12)仍是合法的有限窗口综合，式(27.13)却不是良定函数。证毕。

**定理 27.3（交换条件恢复完整未来的约束）。** 令完整行程及其有限实际像为

$$
\iota(s)=(q(D^js))_{j\ge0},\qquad T=\iota(S),\qquad K=|T|,
\qquad \sigma(t)_j=t_{j+1}.
\tag{27.16}
$$

尾移 $\sigma:T\to T$ 总定义。对任意 $n\ge0$，交换实现的最小总状态数恰为 $K$。交换可逆实现存在当且仅当 $\sigma$ 单射，等价地双射；存在时最小总状态数也为 $K$。两种达到构造均可取满射准备。此外

$$
K=N_n\quad\Longleftrightarrow\quad R_n=R_{n+1}.
\tag{27.17}
$$

这里完整行程与尾移的对应归属 [FutureItineraryShift](../../../D5/S3/ObserverMemory/Trajectories/FutureItineraryShift.lean) 的 `future_itinerary_shift`；稳定有限商与完整商的对应归属 [GradedPredictionShift](../../../D5/S3/ObserverMemory/Refinement/GradedPredictionShift.lean) 的 `stableCompletionEquiv`。来源族的残余并见 [PROCESS_GEOMETRY 定理6.2(c)](RECURSIVE_RELATIONAL_OBSERVATION_PROCESS_GEOMETRY.md)。[ReachableBehaviorMinimality](../../../D5/S3/ObserverMemory/PredictionFactors/ReachableBehaviorMinimality.lean) 的 `finite_state_minimality` 要求一个指定初态及从它出发的可达性；本定理不把它当作全部准备状态的下界，而作如下直接计数。

证明。$\iota(Ds)=\sigma\iota(s)$，所以 $T$ 对尾移封闭。交换给 $F^jI(s)=I(D^js)$；将式(27.11)的 $j=0$ 用于来源 $D^js$，得到

$$
g(F^jI(s))=q(D^js)\qquad\text{对所有 }j\ge0.
\tag{27.18}
$$

因而即使原任务只要求零时刻，交换条件也强制全部未来正确。若 $I(s)=I(t)$，则两来源完整行程相等；每个不同行程至少需要一个不同初始状态，故 $|M|\ge|I(S)|\ge K$，任意额外状态或历史都不能降低此数。反向，取 $M=T,I=\iota,F=\sigma,g(t)=t_0$，便是 $K$ 状态的满射准备交换实现。

若候选 $F$ 是置换，令 $P=I(S)$。交换使 $F(P)\subseteq P$；有限性及 $F$ 单射使 $F|_P$ 双射。把 $p\in P$ 送到它的完整输出行程，得到满射 $b:P\to T$，且 $bF=\sigma b$。任给 $t\in T$，先取 $p$ 使 $b(p)=t$，再取 $p'\in P$ 使 $F(p')=p$，则 $\sigma b(p')=t$。所以 $\sigma$ 满射，在有限 $T$ 上即双射。这个证明容许多个状态有同一行为，也容许 $M\setminus P$ 非空；复制行为状态或添加未用状态都不能绕过必要性。反向，$\sigma$ 双射时上述 $T$ 构造已经可逆，仍达到 $K$。

最后，取前 $n+1$ 个符号给满射 $\pi_n:T\to W_n$。若 $R_n=R_{n+1}$，定理27.2的稳定性使 $sR_nt$ 蕴含 $D^jsR_nD^jt$ 对每个 $j$ 成立，从每次的首坐标得 $\iota(s)=\iota(t)$。故 $\pi_n$ 单射，$K=N_n$。反向，$K=N_n$ 使此有限满射成为双射，故 $R_n=\ker\iota$；而 $\ker\iota\subseteq R_{n+1}\subseteq R_n$，两核相等。证毕。

**定义 27.4（词边的整数循环量）。** 以下缩写 $W=W_n$。取顶点集 $V=A^n$、带不同词标签的边集 $E=A^{n+1}$；词 $e=e_0\cdots e_n$ 从前缀 $p(e)=e_0\cdots e_{n-1}$ 指向后缀 $s(e)=e_1\cdots e_n$。当 $n=0$，$V=\{\epsilon\}$，每个字母各给一条有标签的自环。对每条边给出现次数 $c_e\in\mathbb Z_{\ge0}$，称其平衡若

$$
\sum_{e:p(e)=v}c_e=\sum_{e:s(e)=v}c_e\qquad(v\in V).
\tag{27.19}
$$

每个出现副本分别计数；自环在两边各计一次。经典平衡与环分解机制见 Bang-Jensen–Gutin，*Digraphs: Theory, Algorithms and Applications*，2007年8月15日稿，定理6.8.1（第318页）。下文直接给出含自环、平行副本的状态对应。

**定理 27.5（自由可逆实现与整数循环量）。** 自由可逆实现的最小总状态数恰为

$$
P_n=\min\left\{\sum_{e\in E}c_e:
c\text{ 满足式(27.19)},\quad c_w\ge1\ (w\in W),\quad
c_e\in\mathbb Z_{\ge0}\ (e\in E)\right\}.
\tag{27.20}
$$

该集合非空，且 $P_n=N_n$ 当且仅当所需词的指示次数 $c_e=\mathbf1_W(e)$ 平衡。此处允许多个周期，不要求所有边拼成一条巡回，也不要求支撑图强连通。

证明。给可逆实现，令每个状态的实际窗口为

$$
B(m)=(g(m),g(Fm),\ldots,g(F^nm)),\qquad
c_e=|\{m\in M:B(m)=e\}|.
\tag{27.21}
$$

由 $B(I(s))=w_n(s)$，每个所需词至少出现一次，且 $\sum c_e=|M|$。又 $s(B(m))=p(B(Fm))$；$F$ 双射使两端按 $m$ 计数相等，给每个顶点的式(27.19)。这一步统计全部状态，未把没有直接初始化的相位删掉。

反向，给式(27.20)的整数解，设

$$
M_c=\{(e,k):e\in E,\ 1\le k\le c_e\}.
\tag{27.22}
$$

在每个顶点 $v$，分别把终点为 $v$ 的出现副本列为 $\operatorname{In}(v)$，起点为 $v$ 的出现副本列为 $\operatorname{Out}(v)$。平衡给一个双射 $\phi_v:\operatorname{In}(v)\to\operatorname{Out}(v)$。对副本 $x=(e,k)$ 置 $F(x)=\phi_{s(e)}(x)$。所有入集分割 $M_c$，所有出集也分割 $M_c$，故各 $\phi_v$ 合在一起是 $M_c$ 的置换。以 $e$ 作副本标签、$g(e,k)=e_0$，相邻标签满足式(27.14)，所以每个副本的实际长 $n+1$ 窗口恰为 $e$。对每个 $w\in W$ 选一个副本 $x_w$，令 $I(s)=x_{w_n(s)}$，便验证式(27.11)，总状态数恰为 $\sum c_e$。

空入出集只需空双射；自环副本可属于同一顶点的两张表而仍各计一次，平行副本由 $k$ 区分。置换自然分成若干不交周期，这些周期不必合并。$n=0$ 时重合条件为空，每个副本按其字母读出，同一构造适用。

令所有 $e\in E$ 的次数为一即可行：$n\ge1$ 时每个顶点各有 $|A|$ 条入边和出边；$n=0$ 时全为自环。故有有限可行总数，非空整数总数集合取得最小值。每个所需词至少一次给 $P_n\ge|W|=N_n$；若等号成立，只能恰取所需词各一次、其他词零次，因而其指示次数平衡；反向，平衡的指示次数由上述构造达到 $N_n$。证毕。

这里使用的是经典重叠图及平衡环分解在式(27.11)下的对应。Chung–Diaconis–Graham，*Universal cycles for combinatorial structures*，Discrete Mathematics 110 (1992), 43–59，§3（第45–47页）以连通条件取得一条遍历全部边的 Euler 巡回，本定理允许多周期。Bernardini 等，*Making de Bruijn Graphs Eulerian*，CPM 2022，12:1–12:18，摘要及第12:3页 Problems 1、2 都允许添加合法新边，两者区别在于允许的顶点范围，并非“仅复制旧边”与“允许新边”之别；其整体 Euler 巡回要求也不同于式(27.20)。这些一条巡回的问题不为本定理提供复杂性结论。

**定理 27.6（重复局部合法与满射准备的分别约束）。** 对可逆实现，另定义重复局部合法为

$$
B(F^jI(s))\in W\qquad(s\in S,\ j\ge0),
\tag{27.23}
$$

其中 $B$ 如式(27.21)。它只要求每个滑动窗口都是某个实际来源的窗口，不要求全部窗口来自同一个来源的完整行程。记实际来源纤维大小 $a_w=|w_n^{-1}(w)|$。

(a) 满足式(27.23)的最小总状态数，由式(27.20)另加 $c_e=0\ (e\notin W)$ 得到；该约束可行当且仅当每条 $w\in W$ 都位于只使用 $W$ 边的一个有向环中。

(b) 满射准备的可逆实现自动满足式(27.23)，其最小总状态数由式(27.20)另加

$$
c_e=0\ (e\notin W),\qquad 1\le c_w\le a_w\ (w\in W)
\tag{27.24}
$$

得到。任一约束下没有可行整数循环量，即没有相应的可逆实现。总量 $\sum c_e\le|S|$ 不能单独代替满射准备的条件。

证明。(a) 从一个满足式(27.23)的实现中，只删除与 $I(S)$ 完全不交的整个置换周期。准备及其全部未来不受影响；保留下来的每个状态都在某个 $F^jI(s)$ 上，因为有限周期从任一相位向前遍历全部相位。因此保留部分的全部窗口在 $W$，而每个所需词仍有初始化副本。其状态数不大于原数，式(27.21)给支撑限于 $W$ 的可行循环量。这只删完全未初始化的周期，不能删被访问周期中未直接初始化的相位。反向，支撑限于 $W$ 的循环量由定理27.5构造出的每个状态窗口都在 $W$，故全部初始化轨道满足式(27.23)，并达到同一总数。两方向给所述最小值。

若这样的循环量存在，出现副本上的置换将任一所需边放进一条闭合有向游走。对该边的终点到起点的返回段删去重复顶点，可取得含该边的有向环；自环自身已经是环，所有边仍在 $W$。反向，若每条所需边都有这样的环，为每条边选一个含它的环，再将这些有限多个环的边次数相加。每个环入出平衡，总和亦平衡；它只使用 $W$，且覆盖每条所需边至少一次。这证明可行性充要条件。

(b) 若 $I$ 满射，每个 $m\in M$ 都可写作 $I(s)$，于是 $B(m)=w_n(s)\in W$；将此用于任意未来状态便得式(27.23)。对固定 $w$，$I$ 把来源纤维 $w_n^{-1}(w)$ 映入状态纤维 $B^{-1}(w)$，并满射到它：该状态纤维的任一元素取一个 $I$ 原像，由式(27.11)其来源词必为 $w$。故 $1\le c_w=|B^{-1}(w)|\le a_w$，所有其他次数为零。

反向，给满足式(27.24)的循环量，先按定理27.5构造副本状态、置换及读出。对每个 $w$，因 $1\le c_w\le a_w$，可将 $w_n^{-1}(w)$ 满射分配到其 $c_w$ 个副本：给每个副本分配一个不同来源，再把余下来源任意分配。各来源纤维互不相交，合起来得到总映射 $I:S\to M_c$，且它满射；副本的实际窗口已证明等于词标签，故仍满足式(27.11)。这说明容量约束逐来源纤维承担，而非仅计可分配来源的总数。下述七状态实现虽有 $|M|<|S|$，却有一个任何来源都不能准备到的窗口，直接排除只看总量的判据。证毕。

**命题 27.7（同一实际来源的 $6/7/8$ 分离）。** 取 $A=\{0,1\}$ 及两个不交的四周期

$$
S=\{x_0,x_1,x_2,x_3,y_0,y_1,y_2,y_3\},\qquad
Dx_i=x_{i+1},\quad Dy_i=y_{i+1}\quad(i\bmod4),
$$

$$
(q(x_0),q(x_1),q(x_2),q(x_3))=0001,\qquad
(q(y_0),q(y_1),q(y_2),q(y_3))=0011,\qquad n=2.
\tag{27.25}
$$

普通自由实现的最小总状态数为 $6$；自由可逆实现为 $7$；重复局部合法的可逆实现、满射准备的可逆实现、交换确定实现及交换可逆实现的最小总状态数均为 $8$。

证明。依次列出两组来源的窗口：

$$
(w_2(x_0),w_2(x_1),w_2(x_2),w_2(x_3))=(000,001,010,100),
$$

$$
(w_2(y_0),w_2(y_1),w_2(y_2),w_2(y_3))=(001,011,110,100).
\tag{27.26}
$$

故 $W=\{000,001,010,100,011,110\}$ 按所列次序的纤维大小为 $(1,2,1,2,1,1)$，且 $N_2=6$。取这六个词为状态、首符号为读出、$I=w_2$，并规定

$$
000\mapsto001,\quad001\mapsto010,\quad010\mapsto100,\quad
100\mapsto000,\quad011\mapsto110,\quad110\mapsto100.
\tag{27.27}
$$

它是在 $001$、$100$ 处分别选择 $x_1$、$x_3$ 的式(27.12)，其余词只有一个来源。定理27.2给有限任务正确性及六状态最优性；$I$ 满射，$F$ 却非单射。相同窗口的 $x_1,y_0$ 后继窗口分别是 $010,011$，所以该选择不能是来源更新的规范下降，且 $R_2\ne R_3$。

自由可逆的七状态实现取 $M=\mathbb Z/7\mathbb Z$、$F(i)=i+1$，循环读出为 $0001101$。相位 $0,\ldots,6$ 的长三窗口依次为
...
将 $x_0,x_1,x_2,x_3$ 准备到 $(0,1,5,6)$，将 $y_0,y_1,y_2,y_3$ 准备到 $(1,2,3,6)$；按这些相位读取式(27.28)，所得词与式(27.26)逐项相同，故全部来源在 $0,1,2$ 时刻的输出正确。相位4的 $101$ 不在 $W$，也没有被直接初始化；它在这个七周期中实际被访问，必须计入总数。因此该实现既不满足重复局部合法，也不可能满射准备。它的总数 $7\le8=|S|$ 并未消除这项障碍。

为排除六状态可逆实现，对 $W$ 每条词边各计一次。顶点 $01$ 的出度减入度为 $+1$，顶点 $10$ 为 $-1$，$00,11$ 为零；故所需指示次数不平衡，定理27.5排除 $P_2=6$，上述七状态构造达到最小值。它增加的词边 $101:10\to01$ 正好补平两个差额。

若支撑只能使用 $W$，逐顶点平衡等价于

$$
c_{001}=c_{100}=c_{010}+c_{011},\qquad c_{110}=c_{011}.
\tag{27.29}
$$

于是全部词均须出现时，总数为 $c_{000}+3c_{010}+4c_{011}\ge8$。原来的两个四周期给出总数8、次数恰为 $(1,2,1,2,1,1)$ 的平衡实现；取 $M=S,I=\mathrm{id},F=D,g=q$ 即同时满足满射准备、重复局部合法及交换。它也在每个纤维上满足式(27.24)。定理27.6因而给前两种受限可逆合同的最小值8。
