...
$$
即测试预测与其极限之差只含训练瞬态的速率，不存在独立于训练瞬态、由零空间分量单独贡献的慢衰减模态。这不排除测试误差进入某容差的时刻可以任意晚：$\|\varphi(x)\|$ 越大，同一瞬态跨过同一容差越晚（例如 $\Phi=(1,0)$、$y=0$、$\theta(0)=(1,0)$、$\varphi(x)=(M,0)$、$y(x)=0$ 时训练损失为 $\tfrac12e^{-2t}$ 而测试损失为 $\tfrac12M^2e^{-2t}$）。
