...
\tag{32.109}
$$
若 $A_j>B_j$，该不等式链不可能，故完整可行集为空；不能把倒置区间静默投影为一个非空区间。若顺序正确，（32.109）等价于 $|g_p(r_j)-c_j|\le w_j$。由 $|r_j-\widetilde r_j|\le\epsilon_r$ 及共同 $L_g$ 界，三角不等式给（32.108）的残差。两个完整候选在同一测得位置的预测差各至多 $2(w+L_g\epsilon_r)$，直接应用定理29.3的解析比较和定理29.4（29.119）的相同计算即得直径界。
