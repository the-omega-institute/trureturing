...
本节使用以下已有数学结果：Context43.2–43.4、43.6–43.7 与 43.10 提供有限依赖、形式行恒等式、首列互逆、共同实现和完整档案运输；Context46.2–46.3 的式（46.3）、（46.4）、（46.6）、（46.8）、（46.9）、（46.12）提供非负递推、共同全负包络、系数式、二阶递推与闭式归属；Context47.1–47.2、47.9–47.11 提供加权空间、临界阈值、有限来源消失尾、距离下界与实际观察者合同；Context48.2–48.3、48.14 提供全幅度前缀阈值、有理达到条件及实际像/有限记录限制；Context49.3–49.6、49.11–49.15、49.18 提供分次相位等变、实复相位排除、正尾分类、相位度量、临界拓扑与完整档案。下文只在这些实际陈述的假设内调用它们；稀疏支持判据、十一阶全局最小值和阻尼—相位路径极限是本节新增的完整普通证明。

前述临界分类已经给出同一递归实际像中的相位刚性、有限来源距离与完成拓扑。下面固定这份关系，进一步区分三个问题：有限来源需要保留多少非零坐标；两个不同相位最少能相距多远；从圆盘内部同时改变阻尼与相位时，全层距离沿什么路径趋近。
...
这正是前述临界分类的实际递归与共同包络；以下不另造一个环境数组模型。有限支持块取实来源 $K_A=K_A^{\mathbb R}$，相位与阻尼块取同一个递归的复来源 $K_A^{\mathbb C}$。记
$$
a_i^\zeta=-A\zeta^{i+1},\qquad T^\zeta=\mathcal E(a^\zeta),\qquad |\zeta|=1.
$$
已有分次相位等变给 $T^\zeta(n,k)=-\zeta^{n+k+1}u_k$。有限来源的消失尾、相位族的正尾 $c$、首列逆映射及相位距离均沿用前述临界结果；具体优化需要的附加估计在下文证明。完整实际来源与档案合同统一放在第50.24节。
...
$$
连续前缀至少需要八个坐标；允许间隔以后，**最小非零坐标数恰为五**。一个达到者的支持是
$$
...
每个有限来源均有消失加权尾。为写清本稿所需的局部依据，若 $a$ 支持于 $\{0,\ldots,L-1\}$，令
$$
...
\tag{50.9}
$$
这也是已给临界正尾分类在有限来源上的特例；本稿不需要重新使用或重证整份分类。
...
共同最优性使反例同时覆盖同支持的任意实系数和任意符号。它不反驳 $W_S\ge1$ 的必要性，也不反驳已证的连续前缀定理。缺少的正是递推进入全部延迟之前的有限初值约束；（50.14）将该缺项明确补齐。
...
连续负前缀 $S=\{0,1,2\}$ 的权重为 $19/16>1$。取 $H=2$，有
...
\tag{50.26}
$$
对非零的有限多个行，这是前述形式行恒等式的直接展开；其他行等于零。形式逆的系数由常数项一及正递推唯一确定，因此多项式卷积与逐列递推给出同一批有限系数。包络可独立用
$$
...
以下先恢复任意 $0<\rho<1$ 及第50.1节对应的 $A,c$；十一阶实例另在第50.17节证明。按同一递归的分次运输与同一加权范数，已经得到
$$
\|T^\zeta-T^\xi\|_\rho
...
如果 $\omega$ 的阶为偶数，其幂周期性经过 $-1$，因权重趋于一而有 $F_\rho(\omega)\ge2$；事实上第一次这样的幂已给严格大于二。若 $\omega$ 为无限阶，任意尾部的幂在圆周稠密，故
...
\tag{50.31}
$$
序列 $L_q$ 随 $q$ 严格增加并趋于二。这里只需要供应结果中的圆周稠密性与有限周期事实，不需要任何对不同相位独立赋权的概率律。
...
\tag{50.42}
$$
既有 $\sqrt3\,c$ 一致分离常数因而对每个固定正 $\rho$ 都不是最佳常数。结合有限候选上界，有
$\sqrt3<M(\rho)<2$。
...
本部分重新固定第50.1节任意 $A>0$ 及其临界 $\rho$；以下变化阻尼和相位参数，保持 $A,\rho,c,b$ 不变。对复数 $w$ 满足 $|w|\le1$，定义
$$
(D_w a)_i=w^{i+1}a_i,
...
对 $\tau>0$，阻尼相位来源 $D_{e^{-\tau+i\theta}}a^\zeta$ 仍在同一自由来源域内；其首项幅度严格小于 $A$，因此按临界来源分类属于消失尾实际像。
...
\tag{50.54}
$$
距离与共同相位 $\zeta$ 无关；这一点来自实际等距运输。逐项复共轭还给 $F_A(\tau,-\theta)=F_A(\tau,\theta)$，所以只取相位与阻尼的绝对比值时，符号交替不改变距离。每个固定有限坐标在 $(\tau,\theta)\to(0,0)$ 时都趋向原坐标。
...
综合（50.59）与（50.60），对任意这样的路径，只要
$|\theta_j|/\tau_j\to\kappa\in[0,\infty]$，就有
$$
\boxed{F_A(\tau_j,\theta_j)\to cG(\kappa).}
\tag{50.61}
$$
每个 $d\in[c,2c]$ 都可由一条实际来源路径得到：$d<2c$ 时取唯一的有限 $\kappa\ge0$ 使 $cG(\kappa)=d$，并令 $\theta_j=\kappa\tau_j$；$d=2c$ 时可取 $\theta_j=\sqrt{\tau_j}$。全部路径在每个固定有限坐标上都收敛到同一相位数组。
...
这些是**距离值**的不同极限，不是同一个范数收敛过程拥有多个极限对象。不存在范数收敛至目标，因为所有极限距离至少为 $c>0$。径向的全幅度缩小 $(1-\varepsilon)a^\zeta$ 与本稿的逐级阻尼 $D_{e^{-\tau}}a^\zeta$ 也是不同路径；不能因两者距离极限同为 $c$ 就将两种作用认作相同。

### 50.24 同一关系中的不同资源与完整观察者

支持基数、最远位置、相位阶数、阻尼与相位的比值都来自同一递归，但各自控制不同的问题。五点来源通过更远的位置降低非零坐标数；有限奇数阶归约寻找整个相位族的最小分离；混合路径公式描述逐坐标趋近时仍留下的全层距离。三类结论不能通过同一个“接近”或“简洁”用语互相替换，也不合并成一个忽略前提的更强定理。
...
$$
首列读出给逆映射，并逐字保留同一个档案。不把实际像扩为未经证明的笛卡儿积，不假设档案与边界独立，不删除原始记录，也不把数学工作边界认作完整观察者。这是前述临界观察者接口在本节的共同适用合同。

**稀疏来源。** 自由类中的“五点达到”只在同一实际纤维提供合法五点见证时运输过去；已经在整个自由类中证明的“四点不可能”，在任意子来源类中仍是下界。五点构造并不保留任意既有读数：它在下标四的边界值为零，而全负目标为 $-A$。若档案已记录该精确读数，两者不能产生同一相容档案。前缀最近点的有限记录重放定理不能直接用于任意稀疏支持；重放要求该次实际查询的全部必要来源坐标仍被保留。基数最优不等于档案约束下最优，也不等于未来全部实验等价。
...
本节仅解决指定模型和资源问题。没有给所有 $A>0$ 的最小稀疏基数闭式、全部最优支持分类、五点来源最小截止十二的必要性、任意目标的稀疏最近点问题、有限比特取得成本、随机查询界或约束档案纤维中的最优构造。截止十二只是当前五点见证的属性，不宣称所有五点见证必须如此远。没有给一般最优相位阶数的闭式、全部阶数转换参数或最优搜索复杂度。阻尼—相位混合公式针对当前相位目标、临界权重和自由来源域，不扩张到一般来源、其他递推、非临界权重或任意受限来源。
