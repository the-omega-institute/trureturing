...
\tag{51.5}
$$
同一加权识别把它识别为 $c_0(\mathbb N^2)$。有限三角是有限索引集；截断加上尾部小于任意给定的半径，因而 $B_\rho^0$ 在 $B_\rho$ 中范数闭。
...
\tag{51.30}
$$
因此这里失败的是有限值与完整下确界的交换以及共同完整见证；这条论证没有声称第一目标的正下确界等于 $c_A$，也没有声称它不达到。相同截断作为完整来源的有限优化器满足窗口误差为零而完整误差至少 $c_A$，所以 Context45.5 所需的来源到完整范数连续性不能无条件搬到 $X_{\rm fin}$。
