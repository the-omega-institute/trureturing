...
一个具体例子是位置删除 $\sigma x=(x_{j+1})_j$：$m(n)=n+1$，$\overline\sigma_n$ 删除输入前缀的首位。这不同于式（36.1）删除末位的限制箭头，也不同于 Zeckendorf 算术后继。主卷定理20.5、20.7、20.9已经证明后一后继的行程复杂度和零熵，以及位置删除的 $\log\phi$ 熵；编码载体相同不消去操作差别。
