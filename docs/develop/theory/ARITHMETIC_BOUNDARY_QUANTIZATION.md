...
**31.1 定理：结果及前节引理的定义域审计。** 对步长 \((\log2,\log3,\log7)\),每个有限 \(T\ge\log2\) 的零形状两角点实薄层
...
证明的记号与适用假设见 31.2；两项缩放界分别由 31.13、31.17 证明。

**31.2 定义：两个预算域、端点角点和同一归一化。** 全部对数为自然对数,固定
\[
...
**31.4 定理：每个正高度的零形状贪心次序。** 对任意 \(T>0\) 和 \(h>0\),
\[
\sigma_T(h)=\frac{f(T+h)-f(T)}h=\frac1h\int_0^h f'(T+s)\,ds,
...
**31.7 定理：全部十八个整数 eligibility 与稳定反射槽。** 对 \(j=1,\ldots,6\),令
...
**31.8 定理：九节点的精确距离与贪心权重。** 下表给 31.6 距离公式的完整代入。
...
\]
权重只依赖本表的相对容量,对每个正高度均最优。

**31.9 定义与定理：有限带符号测度与绝对收敛矩展开。** 对每个保留节点,令
\[
...
**31.11 定理：三个严格对数区间与九行半径/原子证书。** 令 \(d=10^{15}\),固定
\[
\begin{array}{c|r|r}
...
\(e^{-\lambda}<l_*\)、\(e^{-\eta}<r_*\)。

数学引文（经典工具）：
NIST DLMF [4.6.E1](https://dlmf.nist.gov/4.6.E1.tex) 的对数级数

本节的精确专门公式由上面的自足证明和证书承担。

**31.12 定理：每行的两个质量不等式及严格首矩裕量。** 依数值
...
**31.13 定理：九节点推出每个有限高度的全薄层界。** 31.10-31.12 给九节点同时满足
\[
...
**31.14 定理：预算对应保留角点与可行混合,不保留原 cutoff。**
对任意本节形状和宽域薄层写
\[
...
**31.15 定理：投影和距离的统一扰动界。** 以 27.3 的通常欧氏投影
...
**31.16 定理：两个非负增量给传递常数 2。** 置 \(z=e^{-T}\)、\(K_z=z/(1-z)\)。
...
**31.17 定理：闭半径 \(1/320\) 的整箱严格裕量。** 31.13-31.16 给
\[
...
**31.18 定理：齐次逼近给无界的实际 (2,3,7) 素数幂箱体。**
...
数学引文（经典工具）：
Encyclopedia of Mathematics [Dirichlet theorem](https://encyclopediaofmath.org/wiki/Dirichlet_theorem)
的 Diophantine approximations 小节中多实数版本,本节只用 \(Q=N^2\) 的抽屉特例。

**31.19 定理：原严格域非空的充要阈值与缺失素数 5 的等号排除。**
...
**31.20 定理：缩放极限与未覆盖的形状。** 对固定相对薄层,31.9 给
...
闭半径不覆盖任意相对形状,也不解决其它素数、一般素数或 第三素数射线。
固定实际 5040、共同平移 5040、密度次序不属于本层。
