...
**32.1 定理：本层的命题。** 只研究原始固定箱体 \(p=(2,3,5,7)\)、\(b=(4,2,1,1)\),
...
**32.2 定义与命题：普通指数、所选内积与编码的区别。** **证明。** 唯一素因数分解给出
\[
...
**数学引文。** QUANTUM-RH.md 的“5040 还给出一个直接的维数障碍”已明确四个普通指数及可逆编码语义。

**32.3 定义与命题：固定端点、预算平移与实际角点。** 所有对数均为自然对数。置
\[
h_i=\log p_i,\quad c_i=(b_i+1)h_i,\quad d_i=(b_i+2)h_i,
\quad C_i=\{c_i,d_i\},
\]
...
**证明。** 唯一分解保证 16 个不同 mask 的乘积互异。原始可容许域准确为
\[
...
**32.4 定义与命题：两个上界及比较的归一化。** 令 \(u=M_1\)、\(f(x)=\log(1-e^{-x})\),
...
\]
**证明。** 该最优值满足 \(W\le D\),因为每个可行整数选择都是下述分数可行点。
\(D\) 是分数松弛的精确值,一般不是 \(W\) 的精确值;
严格比较两个上界不等于证明 Robin 不等式。

**数学引文。** 第 26 节的一般有限集/等号定理和第 18-19 节的同一比较归一化。
ZECKENDORF_EULER_5040.md 定理 12.3 已给连续—整数差额的精确 KL 分解,
其第 11 章已有另一个 5040 单元的 Robin 证书;此处不把 \(G\) 当作该 KL 差额。

**32.5 引理：全实域的正支撑。** **证明。** 取薄层内任一实际角点 \(x\),记
...
所有后续饱和区间都保留实际角点,故逐点继承正域。

**32.6 引理：包络的严格性质与零半径。** 在凸域
\(\Omega=\{(\mu,r):r\ge0,\ \mu-r>0\}\) 上置
\(F(\mu,r)=f(\mu+3r)+3f(\mu-r)\)。**证明。** 由于
\[
...
**包络依据。** [既有固定有理包络证据](../../reports/quantized-gh/actual-5040-fixed-box-0910.md#standalone-certificate)。

**32.8 定理：分数背包的值、价格与端点。** 对 \(u\ge A\),分数问题为
\[
\max_{0\le y_i\le1,\ \sum_i h_i y_i\le u-A}
\left(\sum_i f(c_i)+\sum_i v_i y_i\right).
\]
**证明。** 对任意可行 \(y\) 及 \(\lambda\ge0\),
\[
\sum_i v_i y_i
\le\lambda(u-A)+\sum_i\max(0,v_i-\lambda h_i).
\]
...
**数学引文。** HKUST [Lecture 14: Greedy Algorithms](https://home.cse.ust.hk/~dekai/271/notes/L14/L14.pdf)
PDF pp.4-7,支持按 value/weight 递减、最后一项可部分取入及交换论证;
本节自己给出精确价格证明。

**32.9 引理：下预算饱和及其等号。** **证明。** 对任意 \((M_0,u)\in\mathscr S_{5040}\),取
...
**32.10 引理：唯一相关的向下距离切换。** **证明。** 固定饱和 \(a\),写
...
**数学引文。** Boyd/Vandenberghe [Convex functions](https://web.stanford.edu/class/ee364a/lectures/functions.pdf)
PDF pp.6,16,26,27 (slides 3.4,3.14,3.24,3.25),支持范数凸性、Jensen 和受单调性控制的复合。
严格性、插值正域、端点最大化及零半径处理由本节证明,不冒充文献已经给出本 \(G\) 的符号。

**32.12 定理：无界上尾与全实薄层支配。** **证明。** 若 \(u\ge\beta_{15}\),
...
**32.13 定义与命题：原始 71 槽的精确资格。** 相邻 raw slot \(r=0,\ldots,14\) 表示
...
\end{array}
\]
**证明。** 由指数函数严格单调,它们分别等价于 cutoff、\(c_i<M_0/4\)、
...
不活跃数 \(1+4+3+30+0+17=55\)。

**32.14 命题：两个反射节点的整数证书与有理端点。** **证明。** 两节点的严格 guards 可逐项核对:
...
**32.15 定理：全部 48 个节点值包络。** 下表每个端点分子均除以 \(10^{12}\),
表示闭有理区间。\(G\) 从未先舍入的 \(D-\Psi\) 直接包络,不是两个打印区间相减后强行缩窄。
...
**证明。** 所有 \(G_+\) 都严格小于 \(-10^{12}/1250=-800000000\),
...
\]
**证明。** 四个 \(z_i\) 都严格正且恰为此节点的行距离,不只由数值交叠猜测。
...
**32.17 定理：整个实域的唯一性及等号资格。** **证明。** 严格内部凸性、严格上尾和实际角点处再饱和
...
**32.18 定义与更正命题：级数方法与勘误。** 对数余项含压缩串 `9/(41293^129)`。这是历史 raw 表达式;
其数学纠正是 `9/(4*129*3^129)`,即
\[
\frac{9}{4\cdot129\cdot3^{129}}.
\]
**证明。** 对 \(1\le t\le2\),令 \(z=(t-1)/(t+1)\le1/3\),则
\[
\log t=2\sum_{j=0}^{63}\frac{z^{2j+1}}{2j+1}+R,
\qquad0\le R\le\frac{2z^{129}}{129(1-z^2)}
\le\frac{9}{4\cdot129\cdot3^{129}}.
\]
最后一步是确切的 \(2(1/3)^{129}/[129(1-1/9)]\)。

**定义（原包络方法）。** 在 \(2^{-160}\) 网格上逐运算向外取整,以 \(q=2^e t,1\le t<2\)
归约有理数对数,另用 \(t=2\) 求 \(\log2\)。平方根由整数平方根向外包络。


**命题与证明（指数余项）。** 对需计算的 \(0<x<16\),令 \(t=x/16\),用
\(S_{80}(t)\le e^t\le S_{80}(t)+2/81!\):
首个遗漏项至多 \(1/81!\),后续项比例至多 \(1/82\),故该界有效。
再取 16 次幂并倒数求 \(e^{-x}\)。距离用 interval min/max 处理并列。


**支撑范围。** 十六个保留节点满足 \(0<L\le H<14<16\)，解析证明见 32.22；\([3571708000370,5592595433915]/10^{12}\) 的紧包络见 32.23–32.24。

**数学引文。** NIST [DLMF 4.6.4](https://dlmf.nist.gov/4.6#E4) 支持上述奇次幂对数级数,
64 项截断的具体常数由 32.18 推出。

**32.21 定义（结论的适用范围）。** 本节仅比较两个上界,不是 RH 等价判据、Robin 证书、RH 进展、新 GH 定义。
其它指数箱体、任意形状/素数、正高度 17 节点、全高度密度与平移全域结论
都在本层之外;\(G\) 本身的固定裕量不能外推为无界共同高度族的裕量。

**32.22 定理：固定十六节点的解析支撑范围。** 取 32.3 的固定四维箱体
\(c=(\log32,\log27,\log25,\log49)\)、
\(d=(\log64,\log81,\log125,\log343)\)，所有对数均为自然对数。
实际角点满足 \(x_i\in\{c_i,d_i\}\)。仍用严格原域
\(A=\log1058400<M_0<M_1<\infty\)，闭薄层包含至少两个实际角点。
令 \(u=M_1\)、\(I=[M_0/4,u/4]\)、\(\mu=u/4\)，并定义
\[
\delta_i=\min\{\operatorname{dist}(c_i,I),\operatorname{dist}(d_i,I)\},
\quad V_0=\sum_{i=1}^4\delta_i^2,\quad
\rho=\sqrt{V_0/12},\quad L=\mu-\rho,\quad H=\mu+3\rho.
\]
对 32.13–32.14 的十六个保留节点，均有
\[
0<\log25\le L\le H
\le\log(222264000/15625)<14\log2<14<16.
\]

**证明。** 32.3 的有序角点预算为
\(\beta_r=\log(1058400q_r)\)，其中
\((q_0,\ldots,q_{15})=(1,2,3,5,6,7,10,14,15,21,30,35,42,70,105,210)\)。
相邻槽 \(r=1,\ldots,14\) 的上端为 \(u=\beta_{r+1}\le\beta_{15}\)；
保留反射槽的严格护栏为 \(\beta_j<u<\beta_{j+1}\)，\(2\le j\le14\)，
故也有 \(u<\beta_{15}=\log222264000\)。这里使用既有节点定理的护栏，
不把闭角点边界当作新的反射节点；被严格截断排除的槽 0 不在量词内。

取节点薄层内任一实际角点 \(x\)，则
\(\bar x=\frac14\sum_i x_i\in I\)、\(m=\min_i x_i\ge\log25\)。
因为 \(x_i\) 是第 \(i\) 坐标的一个端点，
\(\delta_i\le\operatorname{dist}(x_i,I)\le|x_i-\bar x|\)。
令 \(w_i=x_i-m\ge0\)、\(s=\sum_iw_i=4(\bar x-m)\)，则
\(s^2-\sum_iw_i^2=2\sum_{i<j}w_iw_j\ge0\)，所以
\[
V_0\le\sum_i(x_i-\bar x)^2
=\sum_iw_i^2-s^2/4\le3s^2/4=12(\bar x-m)^2.
\]
取非负平方根，得到 32.5 的
\(0\le\rho\le\bar x-m\le u/4-\log25\)。从而
\(L\ge\log25>0\)、\(H-L=4\rho\ge0\)，并且
\[
H=u/4+3\rho\le u-3\log25
\le\log222264000-\log15625=\log(222264000/15625).
\]
严格整数比较
\(222264000<15625\cdot16384=256000000\) 与对数的严格递增性给出
\(\log(222264000/15625)<\log16384=14\log2\)。
由 \(\log2=\int_1^2dt/t\)，被积函数为正且在 \((1,2]\) 上严格小于 1，
故 \(0<\log2<1\)。所有严格界因而成立。
证明允许角点位于闭薄层端点、距离并列、零距离及 \(V_0=0\)；
\(L=H\) 当且仅当 \(V_0=0\)，没有除以 \(\rho\) 的步骤。

这里的统一上界仅约束十六个固定节点，因此足以满足 32.18 指数余项的
\(0<x<16\) 前提。它不约束无界薄层尾部：固定
\(M_0=\beta_{14}>A\)，任取有限 \(u\ge\beta_{15}\)，仍含两个实际角点，
而 \(H=u/4+3\rho\ge u/4\) 可任意大。
全实域的正支撑仍由 32.5 给出，全实域符号及最大值仍分别依赖 32.12、32.15–32.17。

**32.23 引理：有理区间支撑证书的可靠性。** 以闭有理区间 \([a_-,a_+]\)
包含相应精确实数。若 \(a\in[a_-,a_+]\)、\(b\in[b_-,b_+]\)，则
\[
a+b\in[a_-+b_-,a_++b_+],\quad
a-b\in[a_--b_+,a_+-b_-],
\]
\[
\min(a,b)\in[\min(a_-,b_-),\min(a_+,b_+)],\quad
\max(a,b)\in[\max(a_-,b_-),\max(a_+,b_+)].
\]
正有理数倍逐端点缩放。对 \(0\le a_-\le a_+\)，平方区间为
\([a_-^2,a_+^2]\)，平方根区间为
\([\sqrt{a_-},\sqrt{a_+}]\) 的任意向外有理包络。

**证明。** 加法、正数倍、最小值、最大值及非负轴上的平方、平方根都逐变量递增；
减法对第二变量递减，所列端点界随即成立，不要求区间互不相交。
写 \(v=M_0/4\)、\(w=M_1/4\)。由于 \(v<w\)，对任意实数 \(e\)，
\[
\operatorname{dist}(e,[v,w])=\max(e-w,v-e,0).
\]
在 \(e<v\)、\(v\le e\le w\)、\(e>w\) 三种情形，该式分别为
\(v-e,0,e-w\)，也涵盖两个端点。先将每个端点距离按上述最大值包络，
再取 \(c_i,d_i\) 两个距离区间的最小值，便包住精确 \(\delta_i\)。
最大值含精确零区间，故这些有理下端非负；平方不会误用跨零区间的单调性。
四个平方相加后除以 12 再取非负平方根，便包住 \(\rho\)，最后两次线性运算
分别包住 \(L,H\)。共享输入所带来的相关性可能扩大区间，但不会破坏包含关系。

正有理输入的对数和非负有理输入的平方根，只需要各自可靠的向外包络。
即使有理数转入球区间时不能精确表示，也先包含该有理数；
对数、平方根的包络性质继续包含精确函数值。取球的向下、向上端点得到精确二进有理数，
再以有理算术执行上述运算，不引入舍入。平方根下端可与零取最大值，因为真值非负。
最后对固定正整数 \(N\)，把 \([a_-,a_+]\) 扩成
\([\lfloor Na_-\rfloor,\lceil Na_+\rceil]/N\) 仍然可靠。
由此，32.24 的逐行有理包络及其整数比较构成有限命题的证书；
区间相交本身不能证明待比较的不等式，也不从有限包络推出新的连续量词。

**32.24 定理：十六节点的紧支撑有理包络。** 对 32.22 的同一十六节点，有
\[
\frac{3571708000370}{10^{12}}<L\le H<\frac{5592595433915}{10^{12}}.
\]
下表给出精确 \(E_0=e^{M_0},E_1=e^{M_1}\)；两支撑列中每个整数对均除以
\(N=10^{18}\)，并表示闭区间。最后一列“内”表示该行的两个完整区间均严格包含于上述紧区间的内部。

\[
\begin{array}{r r r l l c}
\text{槽} & E_0 & E_1 & L\text{ 的分子区间} & H\text{ 的分子区间} & \text{紧区间比较} \\
1 & 2116800 & 3175200 & [3571708000370205849,3571708000370205850] & [4255757179340375123,4255757179340375124] & \text{内} \\
2 & 3175200 & 5292000 & [3656087431865280954,3656087431865280955] & [4513444508621140490,4513444508621140491] & \text{内} \\
3 & 5292000 & 6350400 & [3672351920444274689,3672351920444274690] & [4646972599678113912,4646972599678113913] & \text{内} \\
4 & 6350400 & 7408800 & [3709281801213438733,3709281801213438734] & [4690333637197880086,4690333637197880087] & \text{内} \\
5 & 7408800 & 10584000 & [3805416993823084069,3805416993823084070] & [4758603003307676456,4758603003307676457] & \text{内} \\
6 & 10584000 & 14817600 & [3906887374518372080,3906887374518372081] & [4790664097843025352,4790664097843025353] & \text{内} \\
7 & 14817600 & 15876000 & [3924307344577077266,3924307344577077267] & [4807397059153861248,4807397059153861249] & \text{内} \\
8 & 15876000 & 22226400 & [4035465149539971906,4035465149539971907] & [4810395880886390258,4810395880886390259] & \text{内} \\
9 & 22226400 & 31752000 & [4139340862663385094,4139340862663385095] & [4855443685454883071,4855443685454883072] & \text{内} \\
10 & 31752000 & 37044000 & [4167384397859439993,4167384397859439994] & [4925463759693976678,4925463759693976679] & \text{内} \\
11 & 37044000 & 44452800 & [4211686767642371492,4211686767642371493] & [4974878207139136808,4974878207139136809] & \text{内} \\
12 & 44452800 & 74088000 & [4345560437173103411,4345560437173103412] & [5084082822312931734,5084082822312931735] & \text{内} \\
13 & 74088000 & 111132000 & [4407480363376819653,4407480363376819654] & [5303788151809947391,5303788151809947392] & \text{内} \\
14 & 111132000 & 222264000 & [4542260329528568813,4542260329528568814] & [5592595433914645221,5592595433914645222] & \text{内} \\
23 & 3175200 & 17592186044416/3175200 & [3667629269505833276,3667629269505833277] & [4524706955669101113,4524706955669101114] & \text{内} \\
37 & 7408800 & 95367431640625/7408800 & [3861593487709095353,3861593487709095354] & [4785798744716525144,4785798744716525145] & \text{内} \\
\end{array}
\]

**证明（有理包络）。** [支撑区间证据](../../reports/quantized-gh/actual-5040-fixed-box-0910.md#i35-fixed-support-certificate)
给出各行的四个距离、\(V_0,\rho,L,H\) 包络。其输入就是表中固定端点，
向外运算与精确有理端点符合 32.23，因而逐行包含相应精确支撑。
表中最小的 \(L\) 下端在槽 1，最大的 \(H\) 上端在槽 14，故全部支撑位于
\[
[3571708000370205849,5592595433914645222]/10^{18}.
\]
把所需紧区间的分母从 \(10^{12}\) 化到 \(10^{18}\)，两边的严格余量分别为
\[
3571708000370205849-3571708000370000000=205849>0,
\]
\[
5592595433915000000-5592595433914645222=354778>0.
\]
