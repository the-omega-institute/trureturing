...
使用同一 $G$，上述字典序最优列表及 $K_x(z)$ 都置换不变。
群在大小 $q$ 支持上传递，故任意置换不变策略的积分成功率对支持相同。
平均此恒等值等于其先验 Bayes 成功率，规范最优列表又逐纤维取得上界，因而

```math
\sup_{\substack{\mathcal L\ {\rm 置换不变}\\
 |\mathcal L_x(y)|\le K_x(z)}}
 \Pr_{S,G}\{R(S,\mathscr X)\in\mathcal L_{\mathscr X}(Y)\}
