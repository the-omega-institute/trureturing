...
$W_β$ 消去 $N_∞$，其二次型在商 $V/N_∞$ 上良定义并正定。若要把它写成一个规范的算子，可在给定内积下识别商与 $N_∞^⊥$；未经这一识别时，应把商上的对象表述为正定二次型。定义
