...
若两个合法字串在压缩后相同，并且在同一后续输入协议、同一初态约定下的未来响应差异超过

$$
\varepsilon_{\mathrm{task}}
$$

，则 74.2 的条件失败；若它们的全部指定响应差异被残差界覆盖，则可以在该有限任务中共享一个预测接口。
...
同样，若把 Zeckendorf 合法字串映为输入坐标，两个字串被同一个 $$P$$ 合并，只能说明它们在当前投影坐标中相同。只有当目标任务的读出满足因子化，并且后续动力学满足下降条件时，这种合并才是该任务中的精确压缩。Fibonacci 数
