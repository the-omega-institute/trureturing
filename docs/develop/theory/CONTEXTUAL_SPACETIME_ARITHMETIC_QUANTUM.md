...
第 82 节给出了完整量子载体的维数界，但实际记录通常被分成多个上下文：同一系统可以在不同设置、不同基底或不同局部协议下留下不同结果。设上下文集合为 $X$，上下文 $x$ 有 $m_x$ 个归一化结果。由于概率总和固定为一，每个上下文最多提供
...
这不是说一个上下文的所有结果都没有物理意义；被去掉的最后一个结果由其余结果和归一化条件决定。这里计数的是能独立增加状态区分能力的实线性方向。仓库的 `SingleContextVisibleRemainderDimension` 对一个秩一基底上下文给出了相同的几何边界：单个完整基底的概率方向只有 $d-1$ 个独立中心化方向，剩余的 trace-zero Hermitian 方向仍需其他上下文或其他效应补足。

### 83.2 联合信息完整性的下界

`multi_context_budget_lower_bound` 的假设是：每个上下文的效果满足归一化关系

$$
\sum_{j=1}^{m_x}E_{x,j}=0
$$

（这里已把单位方向和中心化处理吸收进效果空间），并且所有上下文的联合读数对密度态是单射的。若输入 Hilbert 空间维数为 $d\ge1$，则定理给出
...
第一，**上下文专用记录**可以在设置 $x$ 已知后，只保存该上下文的 $m_x-1$ 个独立结果；代价按上下文分开计算。

第二，**共同经典记录**必须在设置尚未揭示时同时支持所有上下文的联合读数。第 78 节已经证明，$X/Z$ 二元查询不存在对未知量子比特完全精确的单一父 POVM；这里的多上下文下界说明，即使放宽到一般维数，独立结果预算也不能凭空消失。
...
多上下文预算不推翻第 80 节关于中心和块的区分，也不替代第 81 节的恢复误差下界。它补充的是另一项必要条件：即使没有任何隐藏回流，联合完整记录本身也需要足够多的独立方向。
