...
下方二进制逼近的严格误差界及式 (22.9) 的逐精度首位反例由取整界和首位机械词定义直接推出。`MechanicalDyadicBoundary.dyadic_upper_eventually_word_eq` 已证明上方二进制逼近在任意固定相位、固定有限词长最终逐位正确，包括整数命中相位；起始精度可依赖相位。`finite_word_stable_off_integer_hits` 为没有正时间整数命中的固定有限前缀构造正半径，同时保持累计取整和实际词，因此任意收敛到该斜率的参数序列最终给出同一有限词。
