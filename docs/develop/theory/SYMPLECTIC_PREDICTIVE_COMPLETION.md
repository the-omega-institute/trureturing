...
<a id="sec-open-sensor-design"></a>
## 12. 外部开放问题的定向推进：测量设计能否提高稳定秩，以及提高以后保留了什么

### 12.1 原文问题、量词与所证明的范围

[EYWR18] 的期刊接受稿 §V“Conclusions and Open Problems”第二条，明确提出能否通过选择标量测量函数提高动力系统的稳定秩；作者在该处给出否定方向的猜测。本章使用其 §III.A 的测量族定义、A1–A3 和式 (14)–(16)，处理一个固定光滑系统、固定采样间隔及固定两个延迟的测量族设计子问题。定义和原文开放问题已核对期刊接受稿，而非仅依摘要立题。

重要量词区别：原文先固定有限基函数族 $h_1,\ldots,h_P$，令 $H=(h_1,\ldots,h_P)^T$，再选一个标量传感器 $h_\alpha=\alpha^TH$。式 (16) 的稳定秩依赖整个 $H$，并不依赖固定族内的 $\alpha$。本章允许设计基函数族 $H$；它不把改变 $\alpha$ 说成改变该定义下的稳定秩，也不把多通道 $H$ 直接当成原文随机标量传感器。

取原状态圆周 $\mathcal A=\{x(\varphi)=(\cos\varphi,\sin\varphi)\}$，固定一步流 $\varphi\mapsto\varphi+\theta$、$\theta=\pi/6$，固定 $M=2$。可将其实现为谐振 Hamilton 系统的固定能量圆，也可实现为径向吸引极限环上的相位流；本章只使用圆周上这个已指定的微分同胚和欧氏弦距离，不从吸引性引入其他假设。

对 $H:\mathcal A\to\mathbb R^P$，令
\[
 X_H(\varphi)=\begin{pmatrix}H(\varphi)^T\\H(\varphi-\theta)^T\end{pmatrix},
 \qquad
 R_H=\inf_{\varphi\not\equiv\psi}
 \frac{\|X_H(\varphi)-X_H(\psi)\|_F^2}
      {\|X_H(\varphi)-X_H(\psi)\|_{\mathrm{op}}^2}.
\]
本章所有测量族都是光滑嵌入，满足原文 A1–A3；固定微分增益 $\|dH/d\varphi\|=1$。研究结果包括：相同最优全局上下弦增益下的严格提升；保留全局分离裕量时的定量上界；固定十维测量族中的未达到上确界与阶匹配障碍；同一构造下任何判别器的精确 Gaussian 噪声风险。

这些结论给出原文可设计性问题的限定构造性答案，没有给出任意吸引子的最优传感器。$M=2$ 不被声称满足 [EYWR18] 随机稳定嵌入主定理的数值充分条件；本章也没有反驳该定理。原文已经分别跟踪测量映射的双 Lipschitz 常数与稳定秩，本章检验的是只优化后者的局限。具体结果尚无独立优先权审阅，不登记外部问题机器结算。

### 12.2 同维数、同微分增益、同全局距离界的严格提升

记 $c_k(\varphi)=(\cos k\varphi,\sin k\varphi)$。对成对谐波族 $H=(a_kc_k)_k$，写 $d=\varphi-\psi$。两个延迟差行的 Gram 矩阵为
\[
 \begin{pmatrix}A(d)&B(d)\\B(d)&A(d)\end{pmatrix},
 \quad A(d)=4\sum_k a_k^2\sin^2(kd/2),
 \quad B(d)=4\sum_k a_k^2\cos(k\theta)\sin^2(kd/2).
\]
这是每个平面谐波的旋转保持长度、且相邻旋转的内积为长度平方乘余弦的结果。因此该弦的稳定秩恰为 $2/(1+|B(d)|/A(d))$。

**定理 12.1（固定几何增益下的测量族设计）。** 两个四维测量族
\[
 H_{12}=\left(\sqrt{3/52}\,c_1,\sqrt{49/208}\,c_2\right),
 \qquad H_{23}=\frac1{\sqrt{13}}(c_2,c_3)
\]
具有相同最优下弦增益 $l=\sqrt{3/52}$、相同最优上弦增益 $U=1$、相同处处微分增益 1，但
\[
 \boxed{R_{H_{12}}=\frac4{2+\sqrt3}<R_{H_{23}}=\frac43.}
\]

**证明。** 置 $x=\cos^2(d/2)$。测量距离平方与原弦距离平方之比分别为
\[
 \frac{3+49x}{52},\qquad
 \frac{16x^2-4x+1}{13}
 =\frac3{52}+\frac{16}{13}(x-1/8)^2.
\]
两者在 $[0,1]$ 的最小值为 $3/52$，上确界为 1。导数平方分别为 $3/52+4(49/208)=1$ 和 $(4+9)/13=1$。正下界给单射，非零导数和紧性给到其像的光滑嵌入；故 A1–A3 都成立。

在 $H_{12}$ 中，$B/A$ 是 $\cos\theta=\sqrt3/2$ 和 $\cos2\theta=1/2$ 的非负加权平均。$d=\pi$ 时第二谐波差消失，最大值达到 $\sqrt3/2$。在 $H_{23}$ 中相应余弦为 $1/2$ 和 0；$d=2\pi/3$ 消去第三谐波而保留第二谐波，最大值达到 $1/2$。代入两行 Gram 特征值即得结果。证毕。

提升不是整体放大信号，也不改变状态流、延迟数或这些最优距离界。它证明原稳定秩在明确的非线性测量族设计中确实可以提高；没有证明全部其他几何参数或随机标量读数的恢复误差也保持不变。

### 12.3 全局分离裕量阻止稳定秩达到二延迟上限

下述结论不要求测量族由谐波构成，也不限制其有限输出维数。设 $H$ 满足
\[
 l\|x(\varphi)-x(\psi)\|\le\|H(\varphi)-H(\psi)\|
 \le U\|x(\varphi)-x(\psi)\|,\qquad 0<l\le U.
\]

**定理 12.2（十二周期轨道的定量障碍）。** 仍固定 $\theta=\pi/6,M=2$，则
\[
 \boxed{R_H\le\frac{2}{1+(l/U)^2/[2C_0]}<2,
 \qquad C_0=9+8/\sqrt3.}
\]

**证明。** 记 $\eta=2/R_H-1\in[0,1]$。对任一差矩阵的两行 $a,b$，两行 Gram 特征值公式给
\[
 \sqrt{(\|a\|^2-\|b\|^2)^2+4\langle a,b\rangle^2}
 \le\eta(\|a\|^2+\|b\|^2),
\]
故 $|\langle a,b\rangle|\le\eta(\|a\|^2+\|b\|^2)/2$。

固定任一相位，取完整十二点轨道 $h_j=H(\varphi+j\theta)$，减去其算术均值。令离散 Fourier 系数 $v_k=12^{-1}\sum_jh_je^{-ikj\theta}$、$w_k=\|v_k\|^2$、$W=\sum_kw_k=12^{-1}\sum_j\|h_j\|^2$。有 $w_0=0,w_k=w_{12-k}$。两两距离的方差恒等式与上弦界给 $W\le U^2$。

对偏移 $m$ 定义平均平方差及相邻内积
\[
 A_m=12^{-1}\sum_j\|h_j-h_{j+m}\|^2,
 \quad B_m=12^{-1}\sum_j\langle h_j-h_{j+m},h_{j+1}-h_{j+1+m}\rangle.
\]
逐行使用上述内积界并移位平均，得 $|B_m|\le\eta A_m\le4\eta W$。Parseval 和移位公式给
\[
 A_m=2\sum_kw_k(1-\cos km\theta),\quad
 B_m=2\sum_kw_k\cos(k\theta)(1-\cos km\theta).
\]
对 $m$ 再作十二点 Fourier 变换，$k\ne0$ 的系数为 $-2w_k\cos(k\theta)$。所以 $w_k|\cos(k\theta)|\le2\eta W$。除 $k=3,9$ 外，这个余弦绝对值至少为 $1/2$；更精确地，对
$\mathcal B=\{1,2,4,5,6,7,8,10,11\}$，有
\[
 \sum_{k\in\mathcal B}\frac1{|\cos(k\theta)|}=C_0,
 \qquad\sum_{k\in\mathcal B}w_k\le2C_0\eta W.
\]
另一方面，四步偏移的原圆周弦长平方为 3，故
\[
 3l^2\le A_4
 =3\sum_{k\not\equiv0\pmod3}w_k
 \le3\sum_{k\in\mathcal B}w_k
 \le6C_0\eta U^2.
\]
整理得到 $\eta\ge(l/U)^2/(2C_0)$ 和所述上界。证毕。

这是一条对所有满足该全局几何前件的有限维测量族都成立的障碍，不是仅对一组 Fourier 权重的计算。它表明固定非零全局分离裕量时，二延迟稳定秩不能任意逼近 2。常数未证明最优；下面证明关于 $(l/U)^2$ 的阶不能统一加强。

### 12.4 固定十维测量族的上确界及阶匹配构造

取 $0<\varepsilon\le1/10$，定义
\[
 Z_\varepsilon=9+(26+20\sqrt3)\varepsilon,
\]
\[
 H_\varepsilon=Z_\varepsilon^{-1/2}
 \big(\sqrt\varepsilon c_1,\sqrt{\sqrt3\varepsilon}c_2,
 c_3,\sqrt{\sqrt3\varepsilon}c_4,\sqrt\varepsilon c_5\big).
\]
这是固定十维、最高谐波次数为五的测量族，没有随 $\varepsilon$ 增加维数或频带。

**定理 12.3（未达到的上确界与最佳可能阶）。** 每个 $H_\varepsilon$ 是满足 A1–A3 的光滑嵌入，最优上弦增益及处处微分增益都为 1。其最优下弦增益 $l_\varepsilon$ 满足
\[
 \frac{\varepsilon}{Z_\varepsilon}\le l_\varepsilon^2
 \le\frac{2(1+\sqrt3)\varepsilon}{Z_\varepsilon},\qquad
 l_\varepsilon^2\sim\frac{2(1+\sqrt3)}9\varepsilon.
\]
稳定秩满足
\[
 \boxed{\frac2{1+2\sqrt3\varepsilon}\le R_{H_\varepsilon}
 \le\frac2{1+18\sqrt3\varepsilon/Z_\varepsilon},}
\]
\[
 \boxed{2-R_{H_\varepsilon}\sim4\sqrt3\varepsilon
 =\Theta(l_\varepsilon^2).}
\]
因此，在固定圆周流、固定两个延迟、固定十维且单位微分增益的 A1–A3 测量族中，稳定秩上确界为 2，但没有测量族达到 2。定理 12.2 的平方阶障碍是阶匹配的，常数最优化仍未解决。

**证明。** 第一谐波具有正权重，所以单射。导数平方是
$[\varepsilon+4\sqrt3\varepsilon+9+16\sqrt3\varepsilon+25\varepsilon]/Z_\varepsilon=1$。
每个谐波差满足 $|\sin(kd/2)|\le k|\sin(d/2)|$，故上弦增益至多为 1，短弦极限给恰为 1。

令 $x=\cos^2(d/2)$，测量弦比平方为 $P_\varepsilon(x)/Z_\varepsilon$，其中
\[
 P_\varepsilon(x)=(4x-1)^2+\varepsilon B(x),
\]
\[
 B(x)=1+(16x^2-12x+1)^2
 +\sqrt3\,[4x+16x(2x-1)^2].
\]
$B(x)\ge1$ 给下界；$x=1/4$ 给上界。极小点在 $\varepsilon\to0$ 时必趋向 $1/4$，因为第一项在紧区间有唯一零点。以 $x=1/4$ 作比较并用 $B$ 连续，得到
$\min P_\varepsilon/\varepsilon\to B(1/4)=2(1+\sqrt3)$。正下弦界、非零导数和紧性给 A1–A3。

对两延迟差行，省去共同的 $4/Z_\varepsilon$ 因子。平方范数为
\[
 d_\varepsilon(d)=\sin^2(3d/2)
 +\varepsilon[\sin^2(d/2)+\sin^2(5d/2)]
 +\sqrt3\varepsilon[\sin^2d+\sin^2(2d)].
\]
内积因子通过三角恒等式精确化为
\[
 c_\varepsilon(d)=-\sqrt3\varepsilon\sin^2(3d/2)
 (1+\cos d)(2\cos d-1).
\]
$|(1+y)(2y-1)|\le2$ 对 $-1\le y\le1$ 成立，且 $d_\varepsilon\ge\sin^2(3d/2)$，因此 $|c_\varepsilon|/d_\varepsilon\le2\sqrt3\varepsilon$，得到稳定秩下界。短弦极限则给
\[
 \lim_{d\to0}\frac{|c_\varepsilon(d)|}{d_\varepsilon(d)}
 =\frac{18\sqrt3\varepsilon}{Z_\varepsilon},
\]
得到上界。两边夹逼给精确一阶稳定秩缺口。结合 $l_\varepsilon^2$ 的等价式和定理 12.2，得到上确界不达到及平方阶的最优性。证毕。

构造机制是三次谐波在该采样间隔下旋转四分之一周，使两个差行趋向等长正交；微弱的其他谐波负责区分三次绕行所合并的相位。平衡第二、第四和第一、第五谐波的权重，使相关内积在潜在重合弦处具有二阶零点。这个抵消是上界阶为 $O(\varepsilon)$ 的承重步骤，不能仅由缩放不变性或原文定义推出。

### 12.5 同一个高稳定秩构造的精确噪声不可辨识代价

将整个二延迟矩阵按固定顺序向量化为 $\mathcal F_\varepsilon(\varphi)\in\mathbb R^{20}$。给出比原文单标量传感器更丰富的全部基函数读数，并假设噪声在这二十个坐标中为独立 $N(0,\sigma^2)$，$\sigma>0$ 固定。这个测量噪声模型与稳定秩的归一化必须同时保持，不能随 $\varepsilon$ 重新放大数据而保留原噪声。

**定理 12.4（远隔状态的最优判别错误率）。** 对同一轨道上的等先验两状态 $\varphi_0=0$、$\varphi_1=2\pi/3$，原状态距离为 $\sqrt3$，但
\[
 \|\mathcal F_\varepsilon(\varphi_0)
       -\mathcal F_\varepsilon(\varphi_1)\|^2
 =\frac{12(1+\sqrt3)\varepsilon}{Z_\varepsilon}.
\]
全部可测判别器的最小错误率恰为
\[
 \boxed{p_\varepsilon^*=\Phi\!\left(-\frac1\sigma
       \sqrt{\frac{3(1+\sqrt3)\varepsilon}{Z_\varepsilon}}\right)
 \longrightarrow\tfrac12,}
\]
其中 $\Phi$ 是标准 Gaussian 分布函数。对任意固定标量传感器 $h_\alpha=\alpha^TH_\varepsilon$、$\|\alpha\|\le1$，在两个标量延迟具有同方差 Gaussian 噪声时，最小判别错误率至少为上式。

**证明。** 这两个相位的第三谐波相同，第一、第二、第四、第五谐波的差长平方都为 3；两个延迟的差长相同，代入权重得到精确距离。等协方差 Gaussian 两假设的似然比是沿均值差方向的一维阈值；阈值在中点，对一维正态尾积分得 $\Phi(-d/(2\sigma))$。标量系数范数约束使每一行差的投影不增加长度，同样的阈值公式给下界。证毕。

因此，即使稳定秩任意接近其上确界、微分增益处处固定为 1，远隔相位的辨识也可以在固定噪声下趋于随机猜测。这没有否定原文完整稳定嵌入定理；该定理本来还依赖双 Lipschitz 常数、reach 等几何因素。它否定的是仅凭稳定秩增大就给实际重建能力排序的推断。

这里的 $H_\varepsilon$ 是可设计测量函数族，不能由已有的有噪单标量读数凭空计算得到。实现需要对应传感器或可计算的状态特征；本章没有把后处理说成获得新信息。最大判别能力的限制已经允许观察全部二十维读数，故不依赖某种网络架构或训练失败。

### 12.6 可实现的验收量与本轮验证

一种有明确含义的设计约束是保留全局分离裕量 $l_H/U_H\ge\lambda_0>0$，再优化稳定秩或具体任务风险。定理 12.2 给出该约束下不能跨越的稳定秩上界。对有确定下弦增益 $l_{\mathcal F}>0$ 的实际延迟映射，最近像点重建在观测误差范数不超过 $\delta$ 时满足状态误差至多 $2\delta/l_{\mathcal F}$，这是三角不等式的既有稳定逆结论；它不另列为本章新增定理。有限采样检查不能认证所有弦的下界，需要解析证明或独立的全局覆盖预算。

同一 $H_\varepsilon$ 模型在 $\sigma=0.05$ 下的结果如下。稳定秩列是定理 12.3 的解析下界，错误率列是定理 12.4 的精确值；无须以有限网格猜测全局秩。

| $\varepsilon$ | 稳定秩已证下界 | 两个远隔相位的二延迟距离 | 最优判别错误率 |
|---:|---:|---:|---:|
| $10^{-2}$ | 1.9330376128 | 0.1847372379 | 0.0323465947 |
| $10^{-3}$ | 1.9930957139 | 0.0601527583 | 0.2737443228 |
| $10^{-4}$ | 1.9993074196 | 0.0190795221 | 0.4243430166 |
| $10^{-6}$ | 1.9999930718 | 0.0019085884 | 0.4923862960 |

本轮程序的 42 个分组检查通过：精确弦增益多项式与权重抵消、两组最小稳定秩见证、任意十二点向量配置的 Fourier 恒等式、全局分离障碍的有限实例、构造族的解析上下界，以及 Gaussian 最优判别。每个噪声设置另用 200000 个独立样本在等价的一维充分判别统计量上检验公式。数值极值另存为诊断值，不替代解析下确界。没有训练或比较神经网络、硬件实验或独立同行核验。

### 12.7 文献归属、实际回答与剩余问题

[EYWR18] 出处见 §11.9。这里核对的是 APS 接受稿 https://link.aps.org/accepted/10.1103/PhysRevE.97.022222 的 §III.A A1–A3、式 (14)–(16)、§V 第二条开放问题；同时核对 arXiv:1609.06347v2。原论文已解释测量映射的几何失真与稳定秩是不同因素，本文不将这一区分本身认领为新发现。

[UGV11] L. C. Uzal, G. L. Grinblat, P. F. Verdes. *Optimal reconstruction of dynamical systems: A noise amplification approach*. Physical Review E 84, 016223 (2011), DOI: 10.1103/PhysRevE.84.016223; arXiv:1205.3196。该文已经使用噪声放大来选择重建方法并给出实验，不把“重建应考虑噪声”认领为本章贡献。
