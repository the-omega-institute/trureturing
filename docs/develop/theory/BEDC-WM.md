...
**候查 B9(切投影词典:双速架构 ↔ cut-and-project,v0.4)**。词条:快系统↔物理空间(展嵌入)、慢库/harness↔内部空间之窗(接纳域)、τ↔内部空间距离、晋升↔投影登记、虚构(T5)↔无提升点(phason 缺陷)、灾难性遗忘↔影子发散(非 Pisot,共轭出盘,E6 刚性之病灶像)、WmAtomizer 局部文法↔Penrose 匹配规则(局部约束强制全局序)。按名界(S2),名须绑检验面:**形式面**——于 MinkowskiModelSet 之窗上构造"harness 接纳 ⟺ 窗内"之显式对应〔Lean可关〕;**经验面**——B6 晋升原型中,祖先门拒收项与"无提升"判据之重合率〔runner可关〕。状态:唱名后首次登记,残差未测,**禁作同构宣称**;组件 literature-attested(de Bruijn 1981 切投影、Shechtman 1982 准晶、phason 文献;Zurek 量子达尔文另属唱名,不随本条入账)。〔候查;双检验面各自可关〕
