...
**定义 36.1（同一后验上的阈值场）。** 取定义 35.1 的非格点情形，固定
...
固定互异内点 $`s_0\lt s_j\lt s_1`$、$`1\le j\le d`$，以及 $`A\gt0`$，置

```math
\nu_{M,j}=\mathbb E_S N_M(s_j),\qquad
X_{M,j}=-\frac{N_M(s_j)-\nu_{M,j}}{\sqrt q},\qquad
...
$`I_{M,j}(a)=A_M(s_j)\triangle T_{M,j}(a)`$，
$`e_{M,j}(a)=\mathrm{sgn}(N_M(s_j)-m_{M,j}(a))`$，定义
...
p_j=\frac1{1+e^{-s_j}},\quad v_j=p_j(1-p_j),\qquad
H(s)=B^\circ(\Gamma(s)).
```
...
$`(a\mapsto H(s_j))_{j=1}^d`$。阈值场独立于粗尺度原点和全部细尺度曲线，且
...
固定排名半径 $`H_0\gt0`$，对每个 $`s_j`$ 取阈值两侧各前
...
再将有限多个细尺度过程放入同一联合极限。因 $`s_j`$ 互异，可先在它们周围取
...
还须证明不同阈值使用同一个粗尺度原点。对任意两个固定 $`s_j,s_k`$，
$`N_M(s_j)-N_M(s_k)`$ 除符号外，是固定得分区间中的实际混合行计数。
固定宽度局部界与背景换测度使其比较均值为 $`O(q/\sqrt\lambda)`$。
(35.8) 因而给出

```math
\mathrm{Var}_S\bigl(N_M(s_j)-N_M(s_k)\bigr)
\le C\left(\frac q{\sqrt\lambda}
