# Wall-Sun-Sun Golden-Unit Research

> **Machine contract.** This volume uses the `generic-v1` atomizer and is
> append-only after admission. It is reference input: Lean declarations and
> their kernel-checked proof terms remain the repository's only mathematical
> truth source. Later corrections belong after the append anchor, with the
> original text retained.

## 1. Position, status, and provenance

**Reference-input status.** This volume collects the ordinary mathematical
arguments developed around the Wall-Sun-Sun golden-unit problem. The arguments
are paper proofs, not Lean certification. The current formal result in
`D5/S3/Arith/Lattices/PrimeCyclotomicTraceImage.lean` is narrower than the
surrounding PH, CG, FPD, THS, ROC, SGN, STL, CTG, HCR, SJC, TBN, DCE, and GPC
developments; the broader WSS existence question remains open.

**Provenance.** The source is the repository's single WSS research series,
authored under Lexa's repository account with Mac Studio implementation commits
from 2026-09-15 through 2026-09-24. It was developed as a sequence of ordinary
mathematics continuations in the same owner document. Exact model-seat metadata
is not present in the committed source, so that part of the production record
is `ASSUMED-UNVERIFIED`. This relocation preserves the mathematical text in one
theory volume and leaves the literature problem dossier as its stable index.

**Source boundary.** The original literature problem is registered under
`shi2026second`, DOI `10.48550/arXiv.2603.25343`. Later sections cite their own
primary sources inline. Literature attribution supports only the ranges stated
at each citation; repository-derived arguments and literature results are not
interchanged.
...
## 10. PH. 固定黄金 Pell 曲线与初始 WSS 深度的高度预算
...
**theorem 10.1 (PH1).** $A_\ell,B_\ell$ 是互素奇数，且 $\gcd(M_\ell,10)=1$。每个 $p\mid A_\ell$ 恰有 $r(p)=\ell$；每个 $p\mid B_\ell$ 恰有 $r(p)=2\ell$。对两种情形均有
...
**theorem 10.4 (PH4).** 假设 $\mathrm{PH}(\kappa)$，取其常数 $H,\ell_0$。令
...
**corollary 10.5 (PH5).** 如果对每个 $\kappa>1$ 都有 $\mathrm{PH}(\kappa)$，则
...
**theorem 10.6 (PH6).** 若对某个 $1<\kappa<2$ 有 $\mathrm{PH}(\kappa)$，则每个足够大的素数指标 $\ell$ 都有一个素数 $p$，满足
...
**corollary 10.6a (PH6 channel separation).** 在 PH6 的第二种假设下，这两族见证彼此无重复。Lucas 通道的每个见证均在原黄金域中分裂，并满足 $s_\ell\equiv1\pmod{2\ell}$。Fibonacci 通道的见证满足 $p_\ell\equiv1\pmod4$，但没有由此确定它分裂还是惰性。
...
**theorem 10.7 (PH7).** 固定整数 $d\ge2$。若一个无界素数指标子族的 $M_\ell$ 的全部素因子均有 $h_p\ge d$，则沿该子族
...
**proposition 10.8 (PH scope).** PH1–PH3 和 PH7 无需高度猜想；PH4–PH6 明确依赖 $\mathrm{PH}(\kappa)$。这些结论均允许原 WSS 零集合为空，也允许它非空。上述条件性非零见证不能推出存在某个 $q_p=0$。
...
**proposition 11.1 (CG finite norm-one group).** $|T(\mathbb F_p)|=N$，且对每个正整数 $n$，
...
**theorem 11.2 (CG unique period-preserving lift).** 在模 $p^2$ 的范数一群中，固定 $u\bmod p$，其全部提升恰可写成
...
**theorem 11.3 (CG p-adic orbit closure).** $\log$ 给出群同构 $T_1\cong p\mathbb Z_p d$。令 $\beta_p=\log(u^N)$，则 $v_p(\beta_p)=h_p$。映射
...
**corollary 11.3a (CG orbit index criterion).** WSS 条件等价于上述真实 p-进轨道闭包在 $T_1$ 中具有非平凡有限指数。局部流的完整描述以实际 $h_p$ 为参数，尚未决定变动素数 $p$ 上哪些 $h_p\ge2$。该流的参数空间是 $\mathbb Z_p$，没有把它作为实连通时间区间到全不连通空间的非平凡连续路径。

### CG.4 Fibonacci 多项式的单根与固定参数偏移

**定义。** 令 $U_0(X)=0,U_1(X)=1$、$V_0(X)=2,V_1(X)=X$，两列都满足 $P_{n+2}(X)=XP_{n+1}(X)+P_n(X)$。

**proposition 11.4 (CG Fibonacci polynomial derivative).** 对所有 $n\ge0$，在 $\mathbb Z[X]$ 中有
...
**theorem 11.4a (CG fixed-parameter root).** 对每个 $p>5$，
...
**theorem 11.5 (CG mapping-torus homology).** 对每个正偶数 $n$，
...
**corollary 11.5a (CG homology criterion).** 原 WSS 条件等价于这个指定三维覆盖的一阶同调中存在阶为 $p^2$ 的元素。等价式依赖随 $p$ 变化的同一个覆盖次数 $N$；它没有从实双曲伸缩或三维拓扑分类中得到额外的素数深度约束。
...
**proposition 11.6 (CG arithmetic differential).** 对任意 $x,y\in\mathcal O_p$，
...
**corollary 11.6a (CG differential criterion).** $q_p=0$ 当且仅当 $\delta_p(\phi)=0\pmod p$。另一方面，
...
**proposition 11.7 (CG circle readout).** $w_p=0$ 当且仅当 $p$ 为原 WSS 素数。圆周上 Haar 等分布的性质本身，不推出这种零点至少出现一次。
...
**proposition 11.8 (CG scope).** CG.1–CG.7 保留原黄金单位，但没有强制任何变动素数满足 $h_p\ge2$。其中局部流、唯一提升、根位置和三维同调都容许 $h_p=1$ 与 $h_p\ge2$；将它们互相代换不产生新的素数存在结论。
...
## 12. FPD. Recurrence-level square transport and maximal-prime descent
...
**theorem 12.1 (FPD1).** （文献：T. Lengyel, Fibonacci Quart. 33 (1995) 234–239；L. A. Medina, E. Rowland, Fibonacci Quart. 53 (2015) 265–271, Theorem 1.4） For every prime p, positive n, and natural k, if
p|F_n, then
...
**lemma 12.2 (FPD2).** If m>0 has no prime divisor greater than five, then
...
**lemma 12.3 (FPD3).** Suppose ell>=7 is prime and p is a prime divisor of
...
**theorem 12.4 (FPD4).** （方法先例：N. Robbins, Fibonacci Quart. 21 (1983) 215–218；V. Andrejić, Univ. Beograd Publ. Elektrotehn. Fak. Ser. Mat. 17 (2006) 38–44） If m>0, m is outside E, and F_m is powerful, then
...
**corollary 12.5 (FPD consequence).** The following two assertions are equivalent:
...
## 13. THS. Exact theta sieving and the principal-orbit coefficient
...
**theorem 13.1 (THS1).** For every positive integer d,
...
**theorem 13.2 (THS2).** If chi_5(ell)=-1, then T_ell V_d=0. Consequently V_p-V_(p^2), viewed at common level 20p^4, is killed by every such good inert operator with ell not dividing 10p.
...
**corollary 13.3 (THS3).** If a simultaneous good-Hecke Maass cusp eigenform g has a nonzero Petersson pairing with V_d, its cuspidal representation satisfies
...
**theorem 13.4 (THS4).** The exact Fourier difference
...
**corollary 13.4a (THS depth specialization).** With the actual initial depth h_p=v_p(F_r), one has
...
**theorem 13.5 (THS5).** For a>=3, the series Z_a(s)=sum_(k>=1)F_(ak)^(-s), initially on Re(s)>0, has the representation
...
**theorem 13.6 (THS6).** For every p>5 and k>=2, the number of pairs (v,w) modulo p^k with
...
## 14. ROC. Fixed-golden orders, mixed-conductor kernels, and zero-parameter dihedral forms
...
**theorem 14.1 (ROC1).** There is a natural isomorphism
...
**theorem 14.2 (ROC2).** For p>5, a>=1, and chi_p=(5/p), the group C_(p^a) is cyclic of order (p-chi_p)p^(a-1). With the actual h_p and
...
**corollary 14.3 (ROC3 stabilization).** The p-primary parts of P_(p^a) stabilize to a cyclic group of order p^(h_p-1). This assertion concerns the nonmaximal orders O_(p^a) in K; it makes no claim about the full class group of the different field K(zeta_p).

**Proof.** The prime-to-p factor is M_p, and min(a-1,h_p-1) becomes constant at a=h_p.

### ROC.3 The mixed-conductor kernel

**theorem 14.4 (ROC4).** For coprime f,g, both coprime to ten, there is a natural exact sequence
...
**theorem 14.5 (ROC5).** Fix p>5 and m coprime to 10p. Put b=v_p(r(m)). For every a>=1,
...
**proposition 14.6 (ROC6).** The increase H(mp^2)>H(mp) at a mixed conductor does not imply p is WSS. In fact
...
**theorem 14.7 (ROC7).** For all p>5 and k>=1,
...
**theorem 14.8 (ROC8).** Every representation in D_(p,k) has weight zero, Laplace eigenvalue 1/4, central character chi_5, and exact arithmetic conductor 5p^(2k+2). Moreover
...
**corollary 14.8a (ROC theta-level realization).** Forms in D_(p,1), when they exist, can be viewed as oldforms at the theta-sieve level 20p^4. ROC8 does not count all cusp forms, all dihedral forms from other quadratic fields, or forms induced from characters with nontrivial archimedean parameter.
...
**theorem 14.9 (ROC9).** The coefficient in THS4 has the exact expression
...
## 15. SGN. Spectral Galois arithmetic, exact congruence ideals, and packet norms
...
**theorem 15.2 (SGN2).** Choose a primitive mth root of unity zeta, identify the image of eta with its powers, and put
...
**theorem 15.3 (SGN3).** Put b(n)=sum_(a|n)chi_5(a). Then for (n,5p)=1,
...
**corollary 15.4 (SGN4).** As the auxiliary good rational prime ell varies, the set on which the discrepancy in SGN2 has finite l_m-valuation p^j has Dirichlet density
...
**theorem 15.5 (SGN5).** Let t=zeta+zeta^(-1). The matrices
...
**lemma 15.6 (SGN6).** For Re(s)>0, with the standard modified Bessel function K_0,
...
**theorem 15.7 (SGN7).** With N=N_k and the exact Fourier normalization SGN1,
...
**proposition 15.8 (SGN8).** In the standard K_0 convention, the coefficient 2^(s-2) in Tanaka, arXiv:2601.21588v3, Lemma 4.2, must be replaced by 2^(s-3) when nu=0. For the family in SGN1 the cosine normalization Theta_eta=g_eta/2 therefore has
...
**theorem 15.9 (SGN9).** The Gram matrix is diagonal and
...
**corollary 15.10 (SGN WSS-threshold determinant).** At the WSS threshold k=1, put d=(p-1)/2. The reflection field satisfies
...
## 16. STL. Saturated spectral trace lattices and exact loss under reduction
...
**lemma 16.1 (STL1).** One has O_B=Z[t]. The Z-span of A_eta(n) for n=1 and good split primes is O_B. More precisely, write t_j=zeta^j+zeta^(-j). There are distinct good split primes ell_1,...,ell_(d-1) such that
...
**theorem 16.2 (STL2).** The rational-coefficient part of the real span of the g_sigma is exactly V_Q, and
...
**theorem 16.4 (STL4).** Suppose k=1 and d=(p-1)/2. With t_j as in STL1, the trace-dual basis is
...
**theorem 16.4a (STL4A).** Write n=d-1 and regard H as a map on Z x Z^n. For an integer vector y, the following conditions are equivalent:
...
**theorem 16.5 (STL5).** For a good split prime ell, write eta(q)=zeta_p^b and interpret b modulo p. The prime coefficients of the integral basis f_j=F_(beta_j) are
...
**corollary 16.5a (STL threshold quotient).** At k=1 the ordinary trace lattice misses exactly (Z/pZ)^(d-1) inside the full coefficient-integral lattice. Modulo p its inclusion has one-dimensional image. The ordinary trace Gram matrix modulo p has rank one.
...
**theorem 16.6 (STL6).** The integral algebra generated by the good Hecke operators on V_Q is O_B acting by a mapping to a*A_eta(ell). It preserves L_int. Its action on L_int/pL_int is the regular action of
...
**theorem 16.7 (STL7).** The greatest common divisor of the Fourier coefficients of the raw trace F_1 is exactly p^(k-1). Its primitive normalization is F_(1/p^(k-1)), whose first coefficient is (p-1)/2. For a good split prime with rotation exponent b, the raw coefficient is
...
**theorem 16.8 (STL8).** Let G_eig be the SGN Gram matrix in the normalized eigenbasis. In any Z-basis of L_tr and any Z-basis of L_int, respectively, the Petersson Gram determinants are
...
**proposition 16.9 (STL scope).** STL constructs and saturates the integer-coefficient structure of an existing golden dihedral packet. It does not change the existence condition h_p>=k+1, and supplies no new actual WSS example or non-WSS prime family. In particular L_int being nonzero was derived from the preselected eta, rather than proved without it.
...
## 17. CTG. Complete-tower gluing and exact rational Eisenstein congruence depth
...
**theorem 17.3 (CTG3).** The map a to F_a identifies the rational-coefficient part of the real tower span with E_k and
...
**theorem 17.4 (CTG4).** The good-Hecke algebra on the tower plus this scalar system is
...
**theorem 17.5 (CTG5).** The first Fourier coefficient gives an isomorphism
...
**theorem 17.6 (CTG6).** Every unital homomorphism R_k -> Z/p^r has t=2 modulo p. The number of such homomorphisms is p^(r-1) for 1<=r<=k and zero for r>k. For the augmented algebra Rtilde_k the number is
...
**corollary 17.7 (CTG7).** Fix a prime p>5 and take the complete finite p-power ring-class tower in ROC, whose largest character order is p^(h_p-1). If h_p=1 there is no such cuspidal tower. Otherwise the maximum rational p-adic Eisenstein congruence depth of a primitive integer-coefficient vector in this tower is exactly h_p-1.
...
## 18. HCR. One-prime Hecke reconstruction and the exact observable subfield
...
**lemma 18.1 (HCR1).** The exact prime-power coefficients and their generating function are
...
**theorem 18.2 (HCR2).** For c in B, let R_c(X) be the unique polynomial of degree less than d with
...
**theorem 18.3 (HCR3).** Given arbitrary integers y_0,...,y_(d-1), define, for 0<=j<=d-1,
...
**theorem 18.4 (HCR4).** The prime-power sequences of L_int are in bijection with the integer functions b on Z/mZ satisfying
...
**theorem 18.5 (HCR5).** Starting from any prescribed b_1,...,b_d, the only missing positive representatives in HCR5 are r=d+a, 1<=a<=(q-1)/2. They are given by
...
**theorem 18.6 (HCR6).** Let ell be any good split prime for an existing order-p^k packet, and suppose eta(q_ell) has exact order p^s, with 1<=s<=k. Put
...
**proposition 18.7 (HCR7).** At k=1, let y_j=[ell^j]F_c, 0<=j<d, be the single-prime prefix of a coefficient-integral form. Then
...
## 19. SJC. Predictive completion of the original golden orbit at square precision
...
**proposition 19.1 (SJC1).** Z with its ordinary absolute-value metric is complete, whereas its p-adic completion is Z_p. The projection O/p^2O -> O/pO is a noninjective ring map and cannot have a unital ring section. In particular its source cannot be replaced as a ring by (O/pO)[epsilon]/(epsilon^2).
...
**theorem 19.2 (SJC2).** There is a unique multiplicative section s:C -> X_2 whose image consists of r-torsion. Put xi=s(v mod p). Every state of X_2 has a unique expression
...
**theorem 19.3 (SJC3).** For any prescribed gamma_j in F_p on j in Z/rZ, consider T_gamma(j,z)=(j+1,z+gamma_j). A fiberwise translation z'=z+f(j) changes the drift to
...
**proposition 19.4 (SJC4).** All lifts v_c=xi(1+pc*delta), c in F_p, have the same complete history after reduction modulo p. Their refined return behaviors differ. The original integer recurrence selects exactly one of them by the fixed polynomial equation v_c^2-3v_c+1=0, and
...
**theorem 19.5 (SJC5).** Let R=Z/p^aZ for a>=1 and p>5. For x=A+Bphi in O/p^aO, write y_0=Tr(x) and y_1=Tr(vx). Then
...
**corollary 19.5a (SJC phase-return trace).** At the phase return r,
...
**theorem 19.6 (SJC6).** In O_p=O tensor Z_p there is a unique norm-one torsion lift xi of v modulo p with order r. There is a unique a_p in Z_p such that
...
**theorem 19.7 (SJC7).** In the coefficient basis (1,phi), multiplication by phi and v has matrices
...
**theorem 19.8 (SJC8).** Let r be the norm-one period in SJC.2. For a primitive rth root of unity zeta_r let Psi_r be the minimal polynomial of zeta_r+zeta_r^(-1), and set G_r=Psi_r(3). Then G_r is a positive integer and
...
**corollary 19.8a (SJC norm converse).** Conversely, for any r>=3 and prime p>5 with p not dividing r, p^2|Psi_r(3) implies that the order of v modulo p is r and that p is WSS.
...
## 20. TBN. A fixed-golden split tower and simultaneous initial-depth balances
...
**theorem 20.1 (TBN1).** Among square integers d>=4, the equation D(d)=5 holds only at d=4. In particular D(p^(2a))!=5 for every odd prime p and positive a.
...
**theorem 20.2 (TBN2).** The following exact identities hold:
...
**theorem 20.3 (TBN3).** If p is a prime divisor of B_j, then
...
**theorem 20.4 (TBN4).** The prime factors of each B_j satisfy
...
**theorem 20.5 (TBN5).** If every prime factor of B_j is WSS, then
...
**corollary 20.6 (TBN infinite odd-depth support).** There are infinitely many distinct split primes p with odd h_p, p=3 modulo4, and rho(p)=2*3^s for some s>=2. Within this restricted set, either infinitely many have h_p=1 or infinitely many have odd h_p>=3, or both. At least J distinct primes of those two types combined are at most B_J.
...
**theorem 21.2 (DCE2).** For every j>=1, retain the actual block B_j=L_(3^j)^2+3 and its original depths from TBN. Then
...
**corollary 21.3 (DCE3).** Fix any real bound X and any integer e>=2. There is a split prime p>X with
...
**theorem 21.4 (DCE4).** Let H>=2 be an integer. If every prime divisor of B_j has h_p>=H, then B_j has at least two distinct prime factors and
...
**corollary 21.4a (DCE depth-budget contrapositive).** For every H>=2, the exact inequality B_j<b(r_j)^(2H+1) guarantees some actual prime p|B_j with h_p<H. For example the fixed fourth block satisfies
...
**theorem 21.5 (DCE5).** Suppose all prime factors of B_j are WSS and Omega(B_j)=5. With the forced factorization B_j=P^2 Q^3, one has
...
**theorem 21.6 (DCE6).** For every odd prime p and every integer a>=2, McConnell's dimension parameter satisfies
...
## 22. GPC. Golden prime clocks, exact interlevel jets, and quadratic reciprocity
...
**theorem 22.1 (GPC1).** If p is any prime divisor of B_j, then
...
**theorem 22.2 (GPC2).** For every j>=1 and every 1<=i<j,
...
**theorem 22.3 (GPC3).** For every 1<=i<j,
...
**corollary 22.4 (GPC4).** For every i<j there are primes p|B_i and q|B_j with both h_p,h_q odd and (q/p)=-1. For every fixed i, at least one such p has infinitely many distinct later witnesses q. In particular there are infinitely many split primes q with odd h_q, (q/19)=-1, and rho(q)=pi(q)=2*3^s for some s>=3.
...
**theorem 22.5 (GPC5).** Each s_j is a nontrivial squarefree integer, the s_j are pairwise coprime, and
...
**corollary 22.6 (GPC6).** If B_j has exactly one prime Q of odd initial depth, then Q splits completely in E_j. In particular, if B_j is an all-WSS block of total multiplicity five, DCE4 forces B_j=P^2 Q^3 with distinct primes and
...
**theorem 22.7 (GPC7).** Fix j>=2 and its actual earlier supports. The set of rational primes Q satisfying
