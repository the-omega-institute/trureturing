...
## SM.1 从群作用到读数，不从名称推出本体

本补编区分三个问题：群如何作用在数学载体上；一个读数保留哪些群表示或动力学信息；指定投影之后还有哪些历史影响返回。Monster（魔群）提供高度受约束的群作用，但并不自动指定某个系统的时间演化或观察者。《利维坦》提供残留运动的历史哲学解释，也不是关于 Monster 的数学来源。两条研究的连接须经过明确的载体、算子与观察，而不能从“巨大”“隐藏”或“对合”这些字眼推出。

Monster 的 Griess 实现作用于一个 196884 维代数；其月光模是一种具有 Monster 作用的分次顶点算子代数 [SM1,SM2]。采用通常的非负共形分次记号，月光迹写作

$$
T_g(q)=\sum_{n\ge0}\operatorname{Tr}(g|V_n^{\natural})q^{n-1}.
$$

其中 $g=1$ 时，系数是各级的维数；一般 $g$ 的系数是字符值。它们不是特定向量的坐标。完整字符在特征零中约束表示的同构类型，却不指定一个被观察的实际向量；一个由能量分次构成的迹也不能无条件恢复相对于任意观察者的耦合。这里的 $q$ 是分次生成变量，不能直接叫做物理时间。

当前 dev 的 `MonsterPrimitiveMobiusRecovery.monster_primitive_mobius_recovery` 接收任意整数系数族 $c$ 和归一化二元形式级数 $D$，并显式要求

$$
-\log D=\sum_{k\ge1}\frac1k H_c(p^k,q^k),
\qquad H_c(p,q)=\sum_{m,n>0}c(mn)p^mq^n.
$$
...
这是实际二元形式级数上的 Möbius 恢复，不是已经构造了 Monster 群、指定的月光分母或任何记忆动力学。函数名 `logarithmicHistory` 中的 history 指同次幂代换的级数叠加，不等于时间有序轨迹。若需使用实际月光模，仍须给出具体分次对象、群作用以及上述对数展开的实例证明。

## SM.2 对称平均可以消除不相关方向而不产生记忆

取有限群 $G$ 的特征零表示 $\rho$，并令 $K\rho(g)=\rho(g)K$。平均投影

$$
P_G=\frac1{|G|}\sum_{g\in G}\rho(g)
$$

满足 $P_G^2=P_G$，因为对每个 $h\in G$，乘积 $g_1g_2=h$ 恰有 $|G|$ 对解；它也与 $K$ 交换。因此置 $Q_G=I-P_G$，有

$$
Q_GKP_G=P_GKQ_G=0.
$$

相对于这个特定不变量投影，所有隐藏往返核均为零。这是普通群平均的直接推论，不另立 Lean 包装，也不能解释成“所有隐藏信息都不存在”。它只说明被群平均遗忘的方向对保留的不变量动态没有反馈。

尤其要区别：$P$ 与全部群作用交换、$K$ 与全部群作用交换，并不总推出 $PK=KP$。在复数域的等型分解

$$
V=\bigoplus_\lambda W_\lambda\otimes M_\lambda
$$

中，群作用在不可约因子 $W_\lambda$ 上，而交换群作用的动力学可在重数空间 $M_\lambda$ 内混合。保留整个等型分量与只保留它的某个副本是不同的观察。下面的显式两副本定理正处理后者，不依赖先构造不可约分解。

MF.1 中的 $2P-I$ 是由投影构造的线性对合。它是不是某个具体 Monster 元素的表示，必须另证；不能仅凭阶为二，就将它归入 Monster 的某个对合共轭类。

## SM.3 定理：全部带群插入的动力迹仍不决定观察记忆

令 $W$ 为任意非零有限维实空间，观察只保留 $W\oplus W$ 的第一个副本，记
...
取任意实数 $a,b$，记 $c=(a+b)/2,d=(a-b)/2$，定义
...
其中 $M_j(K)=(PKQ)(QKQ)^j(QKP)$。当 $a\ne b$ 时，零滞后核已经不同。

**证明。** Kronecker 乘法规则直接给出群因子上的交换性。归纳得 $K_0^k=\operatorname{diag}(a^k,b^k)\otimes I$，且 $K_1^k$ 的对角项为 $(a^k+b^k)/2$、非对角项为 $(a^k-b^k)/2$。取迹并使用 tensor 迹相乘，得到全部带插入迹。

$K_0$ 的 $PK_0Q$ 为零。对 $K_1$，实际分块是 $A=cI,B=dI,C=dI,D=cI$，故 $BD^jC=d^2c^jI$。源码不假设这个核，而是算出实际矩阵恒等式 $DC=cC$ 后对全部 $j$ 归纳，再乘回 $B$。证毕。

Lean 承载为 `TwinedTraceMemoryObstruction.twined_traces_and_exact_memory`，定理甚至允许插入任意 $R$，不只表示像中的 $\rho(g)$。若给定 Monster 的一个实际有限维实表示，它可作为 $W$ 被代入；本模块没有构造 Monster，也没有声称这两副本是某个预定共形级的自然动态。

两个 $K$ 彼此正交共轭，但比较时 $P$ 被固定。若将状态、动力学、观察和群作用一起作同一基变换，记忆核当然相应运输；这里刻画的是在保持观察接口不动时改变动态与接口的相对位置。故不是把换基误称为物理差异。选 $0<b<a<1$ 可同时使两者自伴且严格收缩，但它们不被宣称为随机转移矩阵。

这给出明确的逆问题障碍：即便知道每个群元素插入后的全部动力矩，仍可能不知道指定观察者的记忆核。月光形式级数的系数恢复不能越过这个障碍，除非加入其未包含的观察接口数据。

## SM.4 定理：完整可见响应可以因果恢复实际记忆核

现在保留比迹更丰富的读数。沿用 MF.2 的实际分块更新，从初态 $(v,0)$ 出发，定义

$$
R_n(v)=\operatorname{pr}_V T^n(v,0).
$$

这是每个输入向量的完整输出，而非 $\operatorname{Tr}(R_n)$。有 $R_0=I,R_1=A$。MF1 的消元等式给出
...
则对任意环上的模、任意实际分块映射和全部 $n\ge0$，有 $\widehat M_n=BD^nC$。

**证明。** 当 $n=0$，和为空，消元公式即给出 $R_2-R_1^2=BC$。假设所有较小指标已正确恢复，代入右侧有限和，消元恒等式剩下的唯一项就是 $M_n$。每个递归指标 $n-1-i$ 严格小于 $n$；所有组合次序都保留，没有使用交换乘法。证毕。

源码 `CausalMemoryRecovery.recover_actual_memory` 直接使用实际 `evolve` 与已有 `exact_memory_equation`，并对递归恢复函数作强归纳。第 $n$ 个核只需要响应直到 $n+2$。两个系统的全部完整可见响应相同，必有相同记忆核；它们的隐藏维数、初始隐藏状态和非最小实现仍可不同。

对应形式生成函数在可见端同态环中满足

$$
R(z)=(I-zA-z^2M(z))^{-1},\qquad M(z)=\sum_{j\ge0}M_jz^j.
$$

这是 $R_0=I$ 的形式幂级数恒等式；此表达是上述递推的普通推导，本模块未另立幂级数定理。取迹通常不能保留乘积，因此不能用迹序列代替恢复算法的算子输入。

Monster 模块里的 Möbius 恢复与此处的因果恢复都研究“由聚合读数还原结构”，但两者的索引代数不同：前者使用正整数除数关系及同步幂代换，后者使用有序时间卷积与算子组合。没有证明两种逆运算相同，也不能把对数级数称为已经发生的物理记忆。

## SM.5 《利维坦》原文中的动力学与遮蔽

《利维坦》（1651）第一部第二章从物体运动的持续讨论感觉之后仍留下的内部运动，并用风停后波浪仍继续的例子解释残留；他把想象和记忆看作对同一过程的不同考察，原文为“Imagination and Memory, are but one thing”。同章也区分感觉的变弱与其运动的消失：新的强烈感觉能遮蔽旧印象，如白日阳光使星光难以看见。第三章进一步讨论思想相继发生的秩序。[SM3]

这段原文支持一种动力学的哲学解释，而不支持“记忆只是随意编造”或“别的未实现现实推动当前状态”。历史文本没有给出 $P,K$、概率空间或精确衰减律，也不是现代神经科学的验证。其对当前数学研究有用的区分是：系统中保留的影响、观察下的可见程度、以及观察者对过去的重构，三者需要分别建模。

MF1 的 $BD^nh_0$ 是停止外部驱动后仍可存在的初始残留影响；$BD^jC$ 是一个不同对象，描述可见历史注入隐藏通道后的返回规律。一个是给定初态的响应，一个是与该初态无关的反馈核。不能因为都与过去有关，就把它们合成同一“想象”变量。

## SM.6 定理：实际状态严格衰减，不阻止其观察重新显现

为分清衰减与可见性，取实际二模态自主演化

$$
x_0=(1,-1),\qquad x_{n+1}=(a x_n^{(1)},b x_n^{(2)}),\quad0<b<a<1.
$$

观察 $y_n=x_n^{(1)}+x_n^{(2)}$。这里分量是有符号振幅，不是概率；$\|x\|^2$ 只是欧氏平方幅度，不命名为脑内能量。
...
**证明。** 对实际递归归纳得状态公式。由 $0<b<a$ 得 $b^n<a^n$（$n>0$），故后续读数为正。平方幅度每步减少

$$
(1-a^2)a^{2n}+(1-b^2)b^{2n}>0.
$$

读数一阶差由直接展开得到；两个公比都在 $(0,1)$ 中，故分别趋零，再相减。证毕。源码为 `DampedObservationRevival.decay_and_visibility_are_distinct`。

于是“当前读数为零”“实际内部状态为零”“实际状态随时间衰减”是三件不同的事。整个状态在持续衰减时，当前取消的读数也会在下一步显现。例子采用内部抵消，而不是直接建模霍布斯所说的外部新感觉竞争；后者还需加入输入或非线性读出。
...
可见性先增强并不要求状态重新被外部赋能。这个数学结果不证明任何具体人类回忆过程符合该二模态模型。

## SM.7 继续推进的逆问题

前三个定理分别完成了数据不足的构造性障碍、足够数据的实际逆算法，以及衰减与观察显现的分离。它们将当前问题收紧为：给定具有实际群作用的模型，哪些观察者相关矩阵元足以恢复指定的 $BD^nC$，以及在有限精度与有限滞后下可以恢复到什么程度？

第一项应保留同一观察投影，而不是只比较字符或谱。SM1 已排除“全部带群迹自动足够”；SM2 则给出完整响应足够的端点，中间需要研究更小的可测量矩阵元族。第二项是有限阶尾部与扰动稳定性：若已知隐藏实现的维数、最小多项式或谱隙，哪些有限响应可以约束全部后续记忆？必须引入这些假设后再证明，不能从有限拟合推出无限时间一致性。

对 Monster 具体化还缺真实的 $V^\natural$、群作用、演化和观察接口的联合定义。对称平均若已经与动态交换，会给出零反馈；非零研究对象需要明确的重数内耦合或不同观察。不能只把任意二模态模型改名为 Monster 动力学。当前结论既不解决月光模唯一性问题，也不把月光迹当成人类记忆的机制。

对 #6881 原始进位及既有随机 Zeckendorf 游戏目标，需先证明相关群作用是否真的保持数位域、合法动作、概率和观察；若没有这样的作用，就继续使用原始进位的实际纤维与共同见证，不能为了引入魔群而更换问题。SM2 的直接应用范围仍是线性分块演化，而不是任意非线性进位。不同表示只有通过真实载体映射和交换图才连接。

## SM.8 来源、形式化位置与证明边界

本补编 SM1–SM3 分别由 `TwinedTraceMemoryObstruction`、`CausalMemoryRecovery`、`DampedObservationRevival` 及其同名 Scribe 承载。没有 Monster 类型、VOA 构造或心理学公理作为新前提。群平均、字符解释和生成函数等普通说明不另包装为新形式化成果。新 Lean 是待实际 elaboration 与 kernel 检查的证明源码；普通证明及有限精确诊断不替代机器认证。

[SM1] G. Höhn, M. Seysen. *The Order of the Monster Finite Simple Group*. arXiv:2508.01037v1, 2025. https://arxiv.org/html/2508.01037v1 。Griess 代数与月光模的实际 Monster 作用；不提供本补编中记忆模型的生物学或动力学实例。

[SM2] K. Ono, L. Rolen, S. Trebat-Leder. *Classical and Umbral Moonshine: Connections and p-adic Properties*. arXiv:1403.3712. https://arxiv.org/abs/1403.3712 。用于分次迹与月光函数的含义，不把字符值解释成一个实际状态的坐标。
