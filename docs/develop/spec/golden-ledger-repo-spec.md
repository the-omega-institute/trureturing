...
**封闭 front matter 恰五键**：`slug`（须等于文件名主干且合 canonical slug）、`bibkey`（须为 canonical bibkey）、`doi`（**完整 DOI**，由 `Doi.TryCreate` 判语法，其正则恰为 `^10\.[0-9]{4,9}/\S+$`，故裸 arXiv id、`arXiv:` 前缀与 `https://doi.org/` 前缀皆被拒）、`triage`（封闭字母表恰为 `theorem|window|wall`，与 §11.20.2 Theorist 契约同表）、`motivation_gids`（非空、无重复的 GID 列表）。缺键、未知键、非规范值、BOM、CR、非 strict UTF-8 均 fail-closed。**owner 2026-09-06 裁决**：外部开放问题的准入身份是 **DOI**，不是 arXiv id——arXiv 只是外部发表面之一，DOI 是这些面共用的那一个；故独立键 `arxiv_id` 由 `doi` 取代，二者不并存。**expand–migrate–contract 三段已闭合（第 6 条只禁永久 expand，不禁三段式）**：J2a（#5964）令 loader 暂时同时接受旧五键集，C1a（#6099 / #6106）迁完全部卷宗，**J2b（#6233，2026-09-07 合入 `219117a391`）删除了 `LegacyRequiredKeys`、`ArxivIdPattern`、`ProblemCandidate.ArxivId`、`RequireDoiForChangedDossiers` 与 `ValidateProblemSource` 的 `OrdinalIgnoreCase` 分支**，比较自此无条件为 `Ordinal`。`RequireDoiForChangedDossiers` 一并移除**不是**因为它形似迁移期机器，而是因为其性质被封闭键集包含：后者在 `ProblemCandidateCatalog.cs:157` 对**每一份**卷宗拒 `arxiv_id`，而前者只看新增或字节改动者。**唯一的行为变化**是：卷宗与所绑笔记的 `doi` 仅大小写不同者，此前接受、此后拒绝。该 contract 已合入，故本节所述的封闭键集现为唯一形；`ProblemCandidateCatalog.RequireDoiForChangedDossiers` 只保证「新增或改动的卷宗必须用 `doi`」，不保证存量已迁——存量已迁由本轮的实际字节承担，不由该谓词承担。
...
**引用机械可判（第Ⅵ节）。** 每条 `motivation_gids` 必须在树上解析，悬空即 `dangling-problem-gid`；`bibkey` 必须解析到现役 `Library/**` 笔记，否则 `dangling-problem-bibkey`；`doi` **不是第二真源**——笔记持有该论文的身份，故候选的 `doi` 必须与该笔记 front matter 的 `doi` **逐字节相等**（`StringComparison.Ordinal`），不一致或笔记无 DOI 即 `problem-source-mismatch`。三者由 `DescribeRepositoryValidator` 与既有 `dangling-library-gid` 同处执法，不新增门。**格式校验不算指向校验**：`Doi.TryCreate` 的正则只判语法（且 `IgnoreCase`），其指向由上述逐字节比对承载——正是后者把大小写也钉住。**残余（如实）**：旧形卷宗的比对仍走 `OrdinalIgnoreCase`，该分支随 J2b 一并删除。
