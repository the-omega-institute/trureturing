...
FILEMAP `schema_version = 4` 的每条资源登记含 `cache_activation` 表，键集合须与 `cache_layers` 完全一致。阶段仅为 `stage-start` 或 `lean-production`，后者只用于 current；同层在多条资源中的阶段须一致。缺项、额外项、未知阶段和冲突由 C# loader 与轻量 planner 同样拒绝；无工作仍须验证完整登记。现役 Actions current 在 `stage-start` 恢复报告、elan 与检查证据，报告种子经 `STRATALINT_REPORT_CACHE_ROOT` 恢复至 `.lake` 外。dependency/project 的下载、校验和安装仅在共用报告 prepare 给出 `needs_lean_build=true` 或已选择独立 `lean` step 后启动。`lean_actions.py --stage current --phase lean-production` 自行校验准备输入，workflow 输出只控制调用，不取得选择权威。

需要 Lean/report 的路由在命中后仍进入共用增量调度入口，缓存只提供起点。`make lean-report-prepare` 调用同一报告入口的 `--prepare`，严格输出单个 `{needs_lean_build: bool}`；显式 `STRATALINT_LEAN_REPORT_PREPARATION` 目录位于 `.lake` 外，绑定候选输入、实际 runtime、分区、完整增量计划和固定种子。prepare 不创建 `.lake`、不发布成功报告；后续 `make current` 仍只执行一次原报告入口，校验准备状态后 resume。没有显式准备目录的本地入口在内部执行同一 prepare/resume 算法。准备状态缺失、损坏或候选/runtime/分区变化为输入失败，不回退到另一条 CI 算法；固定种子在 `.lake` 替换后仍可用于增量合并。零待检模块执行复用、候选绑定及最终验证，不恢复重缓存、不启动 Lake build 或 Inspector；需要编译时才调用共用缓存 reader 初始化，随后允许 Lake 配置和构建写入 `.lake`。已有目录与缺 stamp 的保护不放宽，缓存初始化本身不构成 build、报告或检查成功。有待检模块时用显式 `+Module` 目标交给 Lake 构建其依赖,全量重检时构建完整模块集。只有种子或增量合并材料不可用才允许全量生产;真实 cache-bootstrap/utility/build/inspect/compact 失败保留原始退出码并阻断,不得重试全量后覆盖该失败。报告增量器保留模块增删、source hash、反向依赖闭包与材料完整性失效检查;这是增量生产验证,不作为 CI 归属或输入的动态发现器。producer 兼容性只取 A14.9 的显式版本 token；代码字节变化不自动改变该 token。版本、Lean 选项或显式登记的实际语义环境变化在增量器内触发必要重算,不得进入远端兼容分区。相同 mathlib 下源码变化只重算受影响闭包;不影响登记语义输入的 metadata 变化为零 Lean 模块重编、零报告模块重检;同环境增量结果必须等于干净生产。

current 报告生产只有在最终绑定、验证并发布后才原子写 `result.json`，其中实际 `lean_build_executed`/`lean_build_succeeded` 与报告 input/runtime/partition/sha 绑定。现役 Actions 的 `lean_actions.py --stage current --bounded-cache` 将 dependency/project 的构建结果与本轮已接受的 current 交接复核放进既有 bounded worker，与材料制作共用同一剩余缓存窗口，不重跑报告语义验证；独立 `lean` 则使用相同候选/run/attempt 下已接受且执行成功的 `lean` step。零编译、失败或不匹配结果不制作或保存重缓存；报告与检查证据仍按各自验证结果保存。通用显式 `--layers` 运输接口保留原生产成功契约，不冒领为 current 的准入门。
