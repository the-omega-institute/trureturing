...
- **v7.13 R2**(2026-07-18):L 平面首次容量压力按 2.3 只裂不迁:根桶达到 12 后新增的两篇 Zeros 文献进入已登记的 `Library/Zeros/` 兄弟桶,旧 `D5/L/<bibkey>` 地址全数保留;L 双射扩展为受控 `<Domain>` 两段路径,route、typed ref、跨桶 catalog(bibkey 全局唯一)、FILEMAP 与 `Library/MAP.md` 同步执法。
