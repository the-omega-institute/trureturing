...
## 11.21 回填溯源清单(消化完整性)
`Meta/BACKFILL.yaml` 是 **Digestion Ledger** 唯一真源,现役且仅现役 schema 为 `schema_version: 3` / `ledger: theory-digestion-v1`;旧 anchor/disposition 格式经一次迁移只存于 git 历史,运行时无兼容读者、无双读。每个 source 恰含 `{source_id,path,atomizer,entries}`,其中 `source_id` 全局唯一、文件正名且文件名禁空格;每个原子 entry 恰含必选字段 `{atom_id,boundary:{ast_path,start_byte,end_byte},fingerprints:{raw_sha256,normalized_sha256},coverage_gids,receipts,status}`,并可选含 `cas_ref`。canonical 账本现状为 933/933 条 entry 均携 `cas_ref`;THEORY-ERRATUM(11.27)的案件主键 `(cas_ref, atom_id)` 只对携 `cas_ref` 的理论收据成立,无 `cas_ref` 收据不在该事件类内。raw 指纹绑定原始字节,normalized 指纹只容许 UTF-8 BOM、CRLF/CR→LF 与 Unicode NFC 的受限规范化;二者均为 `sha256:<64 lowercase hex>`。
...
- **v7.13 R8**(2026-07-18):PR #204 pass3 终修将 TheoryErratum 结案收紧为“三环证据链闭合 ∧ coverage 所指 witness 经 truth DAG 判 `Closed` ∧ 送达工件在案”的显式清单,并规定无 witness 路径不因既有 `migration: absorbed` 闭合;同时补列 11.21 entry 的 `cas_ref`,字段表按现役 loader 接受域如实标注为可选,canonical 数据现状另述。
