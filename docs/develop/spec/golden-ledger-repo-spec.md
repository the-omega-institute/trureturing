...
**A5.2 Native decide source prohibition (SL-035; #6388)** The candidate-owned Block rule selects distinct paths in `Changes` that are present in the candidate, start with `D5/`, end with `.lean`, and have absent same-path baseline source or different `RawBytes`. It scans the entire selected file, including anonymous examples and previously frozen modules; headers, reports, named declarations, freeze events and edited hunks do not limit this surface. Deletions, equal bytes and unchanged history are outside the delta, including when judge implementation inputs change. Rename/copy destinations are selected by the same same-path byte predicate.

The bounded lexical spelling policy rejects exact bare `native_decide` code tokens, including syntax quotations and terms inside standard `s!`, `m!` and `f!` interpolated strings (including nested terms). It ignores line comments, nested block comments, inert ordinary/raw string chunks, character and name literals, escaped identifiers, and longer or qualified identifiers. The shared `LeanSourceTokenizer` owns these lexical boundaries, using the pinned Lean v4.33.0 `isIdFirst`/`isIdRest` identifier ranges; proposition extraction retains literal spelling rather than receiving the extra interpolation-term tokens. This is a spelling gate, not a Lean elaborator or proof of tactic execution: aliases, custom syntax/interpolation, generated code, qualified/escaped spellings and other compiler-trusted mechanisms are not ruled out. Lean/kernel validation and axiom-closure checks remain separate obligations.

Each token finding has source `Path` and message `NATIVE_DECIDE_SOURCE line=N: bare native_decide token is forbidden in changed D5 Lean source`, where `N` is one-based. Recognized unterminated comments/literals/interpolations/escaped identifiers and malformed character/delimiter input fail closed with `NATIVE_DECIDE_LEXICAL_ERROR line=N: <reason>` at the lexical error or unmatched opening; malformed input never becomes an empty successful scan. Other Lean syntax errors remain the Lean inspector's responsibility. Normal rendering is `SL-035 <Path>: <message>`. Unit fixtures and production admission must demonstrate rejection with an otherwise admissible empty anonymous-declaration report, paired `decide` acceptance, delta selection and execution dispatch; the integration trigger PR and final required checks remain the authoritative rollout evidence.
