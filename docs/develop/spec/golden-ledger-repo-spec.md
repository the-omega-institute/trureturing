...
`Meta/Digestion/backfill/` 是 **Digestion Ledger** 唯一现役目录真源;旧 `Meta/BACKFILL.yaml` aggregate 与更早 anchor/disposition 格式经一次迁移只存于 git 历史,运行时无兼容读者、无双读。candidate/current 目录账本中每个 `source.toml` 恰含必填 `{source_id,path,atomizer,genre_registry_check,unregistered_genres}`;`acknowledged_stale` 仅在非空时出现,出现则元素非空且编码 canonical。其中 `genre_registry_check ∈ {collected,no-registry}`,`unregistered_genres` 为排序、去重且无空 token 的 raw token 列表;`no-registry` 必配空列表,`collected` 可空或非空,缺必填字段、额外字段或非 canonical 编码均拒。差分命令的 baseline view 只投影 source identity、stale acknowledgements 与 atoms;其 genre registry 投影在类型上恒为 `Unavailable`,历史 baseline 的 `source.toml` 可同时缺少两个 genre 字段,但 loader 不为此合成 `no-registry` 或空 token 集,且任何读取 baseline genre marker 的路径必须显式失败。该 baseline view 不构成 candidate/current 运行时旧格式读者。`source_id` 全局唯一、文件正名且文件名禁空格;每个原子 entry 的共同必选字段为 `{atom_id,fingerprints:{raw_sha256,normalized_sha256},cas_ref,coverage_gids,receipts,status}`,边界字段互斥二选一:嵌套 `boundary:{ast_path,start_byte,end_byte}` 或顶层 `ast_path`,无 entry 级可选字段。canonical 账本 3460 条 entry 中 3448 条采用顶层 `ast_path`、12 条采用嵌套 `boundary`,且 loader 强制 3460/3460 均携 `cas_ref`;故 THEORY-ERRATUM(11.27)的案件主键 `(cas_ref, atom_id)` 对每条已摄入理论收据成立。raw 指纹绑定原始字节,normalized 指纹只容许 UTF-8 BOM、CRLF/CR→LF 与 Unicode NFC 的受限规范化;二者均为 `sha256:<64 lowercase hex>`。

`digest-status --formalize-candidates` 的 `ledger_sha256` 只从一份 canonical、按完整 repo-relative path ordinal 排序的目录账本 preimage 定义。preimage 逐文件纳入 path 长度、path 的 strict UTF-8 字节、content 长度与 canonical content 字节;文件集合恰为每个 `source.toml` 与每个 atom YAML,故 `genre_registry_check` 或 `unregistered_genres` 任一字段单独变化均必改变账本地址。不存在聚合账本 serializer 或第二套 hash material。
