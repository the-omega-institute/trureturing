...
2. P0-F1 先把三个 `disposable-projection` 迁出保护面前缀；它是 PR-A 的独立前哨与居所子集，范围更小、收益立即且不依赖 P0-2 的任何前置，完成后不替代 PR-A 的七项 run-local protocol。
...
**父原则：治理按「物理居所」而非「语义权威」分类对象。** 仓库的物理居所、committed 字节、保护等级与写者拓扑，没有服从对象的语义权威、所有权、可再生性与可逆性：本应由程序与权威输入决定的可再生投影，被路径或 committed 副本反向赋予承重地位；本应由数据所有者维护的数据被编码进程序；具有真实权威的账本被实现成多写者共享线性尾。R1、R2、R3 的既有判词全部保留，并降为该父原则的三种已冻结表现；该父原则解释 R1、F1、F5、F6、F7，不单独解释 R2、R3，也不覆盖 F2/F3。
...
**F1 居所不变量：**`disposable-projection` 不得住于保护面前缀之下；判据是语义（可由已被 harness 的输入确定性重算）而非路径，其一致性由重算比对保证，那是验算不是治理；实测三个计算物仅因住 `Meta/StrataLint/` 前缀下而每次触发 `conservative 529s`，同类的 `Generated/DAG.md` 住顶层则从不触发。
...
**范围边界：**F2/F3（多驱动者调度的收敛性与失败隔离）是独立因果支，不属第十二部，交独立 issue 处理；具体为 `is_derived_conflict` 白名单不覆盖 ceremony 产物、FIFO 头阻塞整条队列（实测 `#903` 卡 2h52m，`#904`/`#914` 在其后 waiting），且它们不会因 PR-A 落地而自动消失，故不得把第十二部误作已覆盖。
...
- **v7.14 R3**(2026-08-08):`PROJECTION-RESIDENCY` 以「治理须按语义权威、所有权、可再生性与可逆性，而非物理居所」统摄 R1/R2/R3 的三种已冻结表现，保持三项既有判词与商映射不变；在 R1 下补 F1 投影不得住保护面、F6 投影不得依赖兄弟投影、F7 病因消除即按 §12.9 删除补偿面，并明确 F5 并入 F6。F2/F3 多驱动者调度收敛与失败隔离划为独立因果支，不因 PR-A 自动消失；实施顺序新增独立前哨 `P0-F1`，在 PR-A 前先迁出三个保护面内 disposable projection，不改门级、`emit-check` 或禁兼容垫层条款。
