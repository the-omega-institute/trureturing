...
每个保留文件必须在 `Meta/FILEMAP.toml` 以完整路径单独登记；`docs/reports/**` 或目录通配符不能替代登记。`filemap-conform` 对未登记文件报 `FILEMAP-REPORT-UNREGISTERED`，对仅有通配覆盖的文件报 `FILEMAP-REPORT-NONEXACT`。实验程序记 `kind=program`，数据和许可证记 `kind=data`，均属内容面；登记本身属判官面。

依 SL-029，先通过独立的判官拉取请求登记精确路径，再通过内容拉取请求添加材料。精确登记允许暂时无文件，供预登记及删除后的分步清账使用；不赋予同目录其他路径准入。删除材料后再以判官拉取请求移除登记。
...
**双轴状态由机器派生,status 只是受检投影,禁手写冒领。**迁移轴为 `{residual,partial,absorbed,nonpropositional,upstream}`:仅完成 extract/identify 而无语义目标或收据进展者为 residual;已识别目标 GID 或已有迁移收据但合取未齐者为 partial;原子本地收据与全部 `chain_atoms` 均闭合者才为 absorbed;经 canonical `receipts.nonpropositional` 判定无可覆盖命题者为 nonpropositional。真值轴为 `{closed,tail,open,inapplicable}`:对承载 coverage 的 `{residual,partial,absorbed}` 三种迁移态而言,Lean 闭包 Closed 才是 closed;`inapplicable` 表示经 `receipts.nonpropositional` 判定无可覆盖之命题,与 Lean 闭包无关,亦不计入已证;Tail 只有在 migration 已 absorbed 且 `tools/Authorizations/digestion-tail/<atom_id>.json` 之 canonical 工件逐字绑定 atom 与全部 Tail GID 时才投影为 **absorbed-tail**,否则一律 open;Tail 不计已证。新增轴值的合法新对恰为两对:`nonpropositional-inapplicable` 与 `upstream-closed`(后者见下款「上游命题终态」);loader 对 `residual-inapplicable`、`nonpropositional-open`、`upstream-open` 等其他组合一律 fail-closed。SL-016 对 source 结构、内容指纹、目标 GID、收据、双轴重算一致性逐项 fail-closed,任一 stored status 与派生不同即红。
...
**上游命题终态(owner 2026-09-15 定,补 CLAUDE.md 第 3.2 条具名缺口 `open(BIND-ONLY-PROPOSITIONAL-SETTLEMENT)`;案 #8049)。**命题 atom 的全部子句若仅由钉版上游(`lake-manifest.json` 所钉 Mathlib 及其传递依赖)的既有声明经实例化、投影与规范化推出,且仓内无可用冻结 GID,则按第 3.2 条不得新增 bind-only 项目声明取 GID,而以 `receipts.upstream` 进入第五终态 `upstream-closed`。该收据键集恰为 `{justification, declarations, mathlib_rev, probe_sha256, probe_axioms, previous_atom_id, next_atom_id}`:`justification` 为非空 scalar(逐子句说明由哪些上游声明经何种实例化/规范化闭合);`declarations` 为非空、ordinal 排序、无重复的上游全限定声明名列表;`mathlib_rev` 为写入时 manifest 中 mathlib 的 40 位小写十六进制 rev;`probe_sha256` 为 `sha256:<64 lowercase hex>`,绑定 canonical 探针源 `Meta/Digestion/upstream/<atom_id>.lean`(kind=data,唯一 producer 为 `settle-upstream`,与 atom YAML 同一原子写入落地);`probe_axioms` 为探针全部声明公理闭包之并的 ordinal 排序列表,须 ⊆ 公理白名单 `{propext, Classical.choice, Quot.sound}`;`previous_atom_id`/`next_atom_id` 语义与 `receipts.nonpropositional` 相同。探针源只许 import 钉版上游包(`Mathlib…` 及 manifest 闭包内其他包),不得 import 任何 `D5.` 模块——需要项目声明者走 cover,两者不混用;探针须以 `theorem`/`example` 逐子句陈述该 atom 的形式命题并给出无 `sorry` 证明。该收据与非空 `coverage_gids`、`receipts.quarantine`、`receipts.cover_disposition`、`receipts.nonpropositional` 互斥;loader 对共存、错形、`declarations` 乱序或重复、`probe_sha256` 与探针文件字节不符一律 fail-closed;`mathlib_rev` 与当前 manifest 不同**不得**成为 Block(避免 mathlib 升版即全仓自锁,#1947 同形);当前 harness 对该漂移**无消费者**——既无 `UPSTREAM-REV-DRIFT` Observe,`settle-upstream` 亦拒绝 `--reverify`(`ARGUMENTS_INVALID`)——升版后的探针重验与 rev 刷新记 `open`(#8049 后续层),本款不冒领该机器已存在。`settle-upstream` 是唯一 writer。**set:**`settle-upstream --request FILE --base REV` 读取 strict TOML,键集恰为 `{atom_id, justification, declarations, probe, previous_atom_id, next_atom_id}`(`probe` 为工作树内探针源路径,前后邻居为同形 id 或字面量 `source-boundary`);检查次序与 `settle-atom` 相同,可写性先于邻居(`ATOM_ABSENT`/`ATOM_AMBIGUOUS` → `CHAIN_PARENT` → 动词专属可写性 `NOT_RESIDUAL_OPEN`、`COVERAGE_PRESENT`、`QUARANTINE_PRESENT`、`DISPOSITION_PRESENT`、`NONPROPOSITIONAL_PRESENT`、`UNRESOLVED_SUBITEMS_PRESENT` → 邻居上下文复算 `CONTEXT_MISMATCH`),再依次探针面 `PROBE_PATH_INVALID`(request 的 `probe` 不是工作树内的相对常规文件路径)、`PROBE_IMPORT_SYNTAX`(不可识别的 import 行)、`PROBE_IMPORTS_PROJECT`(import 了 `D5.` 或钉版 manifest 闭包之外的包)、`PROBE_DECLARATION_UNSUPPORTED`(探针含有界方言之外的命令 token)、`PROBE_FAILED`(源含 sorry/axiom 等禁用形式,或 `lake env lean` 非零退出)、`PROBE_AXIOMS`(缺具名 `theorem` + `#print axioms` 形状,或闭包越出白名单)、`DECLARATION_UNRESOLVED`(某声明名在钉版上游不能解析);同值重放为零字节变化,内容不同的重设以 `UPSTREAM_CONFLICT` 拒;全部通过后原子写入:探针源复制到 canonical 路径、收据写入、entry 由 `residual-open` 迁至 `upstream-closed`,同值重放为零字节变化。**clear:**`--clear ATOM_ID` 原路退回 `residual-open` 并删除探针源,无收据时以 `UPSTREAM_ABSENT` 拒绝。该终态对链闭合视同已闭(与 `absorbed`、`nonpropositional` 并列);对项目 Lean 真值零主张,不计入项目已证与可形式化分母,不进入 `formalization_frontier`,在 `digest-status` 中单列计数。判形真实性(全部子句确为 bind-only 于所列上游声明、探针陈述忠实于 atom 文本)仍是第 3.2 条的软评审边界,机器只判形状、编译、公理与 import 面,不冒称已机器判判形。SL-016 对收据形状、目录↔派生态、探针文件存在与 `probe_sha256` 相符逐项 fail-closed(delta-only,对存量盲)。

**链闭合规则。**`DeriveMigration` 固定点与 `CompleteChainGaps` 共用同一谓词:子项迁移态为 `absorbed`、`nonpropositional` 或 `upstream` 即闭;父项 `LocalComplete` 要求不变。零 coverage 的父项仍为 `residual-open`,只失去对应子项的 `chain-migration-incomplete` gap。
