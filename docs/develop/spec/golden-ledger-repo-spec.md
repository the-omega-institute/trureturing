...
**coverage→Scribe 收据的 delta 门与 `--seed-missing` 机械登记(总单 #5533 第 8 层)。**准入时 SL-016 把候选的每条 coverage 边 `(gid, target_statement_id)` 与 protected-base 上同一 atom 的账目比对:**新增或变更**的边必须恰有一条匹配该 GID 的 Scribe 收据,否则产 blocking finding `coverage-scribe-receipt-required`;base 上未变的边缺收据只作可观察的 `scribe-receipt-missing`,不阻断无关 delta(含 producer 的全量扫描)——第 20‴ 条 delta 作用域,无 `changes==null` 全量回退。`align-scribe-receipt --seed-missing --atom <id> --gid <gid> --base <rev>` 只在该 atom **零条**匹配收据、恰一条已有 coverage 边、目标声明在冻结账本中唯一解析且 Closed、当前存在 Scribe 定义与已验证的模块发射(声明 GID 另需已验证的声明引用)时,机械登记首条收据;cover 与 seed 共用同一个 receipt builder,首次 cover 时的收据与 seed 的收据字节同形。`--pairs FILE` 接受严格 UTF-8、LF 结尾、两列 `atom_id<TAB>gid` 且无重复的 TSV,**全部合格才原子写入**,任一不合格即整批拒绝零写入;`--dry-run` 对每对给出 `eligible / missing-definition / missing-emission / missing-declaration-reference / ambiguous-edge` 五态之一且不写树;`make scribe-seed PAIRS=file [BASE=rev]` 为批量入口。登记不产生新权威:迁移/真值状态仍只由 evaluator 派生,且 seed 在**同一事务**内投影受影响的整个 `chain_atoms` 祖先闭包的派生状态(子 atom 补齐首条收据使父 atom 由 partial 变 absorbed 时,父的存储状态一并更新,不重投影无关存量债);unresolved subitems 原样保留;写入前的最终收据校验覆盖**完整计划写集**(被 seed 的 atom + 状态将迁移的全部祖先),任一收据校验失败即整体拒绝、零写入(`--dry-run` 用同一校验并报告将被拒绝的原因);不持久化任何时间戳。
...
**deposit 在 freeze 后组合 canonical cover(2026-09-05 定)。**`make deposit ATOM_ID=x GID=g` 在 lean-report → header-check → emit → freeze 之后,对同一 `(ATOM_ID, GID)` 立即调用 canonical `cover-atom` 并重新 `emit`;`cover-atom` 仍是 `coverage_gids` 的唯一写者,shell 层永不写边;Scribe 收据有两个 writer——`cover-atom`(首次 cover 与 refresh)与 `align-scribe-receipt --seed-missing`(对已有边补登首条收据)——二者经同一个 receipt builder 在相同谓词下产出同形收据。已冻结与已覆盖均为可重放的跳过(`PLAYBOOK_SKIP … coverage-already-applied`)。若 cover 不能使残余原子进入 deletable `closed`(如多子句守门),冻结的定理保留、CLI 已写入的 `receipts.cover_disposition` 为权威记录,工作流以 `PLAYBOOK_DEPOSIT_FROZEN_UNCOVERED atom_id=… gid=… reason=…` 非零退出,不得把「已冻结」报成「已消化」。
