...
## 11.13 实验材料与历史记录
`docs/reports/` 只保留有明确用途的实验程序、实验数据及正式源码引用的第三方许可证。正式成果归入对应的代码、理论正文或文献条目；实施日记、构建日志、席位回执、转录和重复快照不入仓，历史由 Git 和 PR 保存。
...
依 SL-029，先通过独立判官 PR 登记精确路径，再通过内容 PR 添加材料。精确登记允许暂时无文件，供预登记及删除后的分步清账使用；不赋予同目录其他路径准入。删除材料后再以判官 PR 移除登记。立新案前仍须检索正式成果与 Git/PR 中的历史证据。
