...
Report artifacts are isolated by Lake traces and `report_cache_release_semantic_version`; configuration identity belongs to the aggregate. An enrollment plan is a compile product of its owner module, contains no manifest-derived value, and is invalidated only by Lake import traces, including its imported judge.
