...
**A14 版本码** 版辑 tag `E<n>`+Zenodo DOI;工具链钉版。升级触发为季度机器定时**检查**(不是季度强制升级)+事件触发(某个已准入第三方包要求的最小 pin 在当前组合下不可解);无更新或所得 pins 相同即机器 no-op。候选必须在同一 PR 原子更新 `lean-toolchain`/`lakefile.toml`/`lake-manifest.json`,禁止部分升级;验证闭包依次为全量 clean `lake build` → 产 canonical Lean report → 声明 identity 不变检查(升级 PR 只改证明不改陈述,SL-014 diff 检查)→ base-owned admission → 对全部环境受影响的 active 冻结条目执行既有 `ledger-reattest`(catalog-wide,当前量级 463 且动态增长),因为每个冻结 witness 均含 toolchain/manifest blob。纯环境升级不触发 C0 renewal/TOWER renewal/conservative replay,除非同时改了它们自然拥有的 harness/controller 路径。四态失败归位:机器可判且全绿即自动合入;版本/证明不兼容或无兼容解则保持旧 pins、拒绝候选并登记具名 `open`;当前 pins 是可验证初始态,持续有效直到候选完整通过;网络/上游 release/资源不可用则记为等灯亮的 `open` 约束,灯亮自动重试,其余 lane 继续推进。任何失败绝不留下部分 pins 或部分 reattest,绝不静默撤后代。

**A15 提交与 PR 文法** `COMMIT := <官>"("<GID>"): "<动词短语>`;PR 模板 = 四段判词(立了什么/依赖什么/试了什么死了什么/账平声明勾选:无既有 closed 被推翻)。

**A17 Scribe:文档即代码(v7.14)** 叙事层(Blueprint/Papers,渐及 spec)的 canonical 源=C# 类型化文档 AST(`Meta/StrataLint/StrataLint.Scribe`):文件头/章节/段落/公式(自建封闭 Formula AST→total `LatexWriter`,逐节点构造式确定性发射,不解析 LaTeX 文本)/GID 引用(经 Engine Gid 构造期解析,悬空即失败——取代已废的行号/片段哈希位置锚)。正式陈述统一为 `DocumentBlock.Describe`:文档内唯一的 typed local `DescribeId` 与 kind/statement/provenance 均构造期必填,kind 封闭为 `{definition,theorem,proposition,lemma,example,remark}`,statement 必为 `Formula` 或 `LeanDeclarationRef`,provenance 必为 `{literature-attested,repo-derived,suspected-novel,unassessed}`;旧 `Proposition/Theorem/ComputedValue/RenderedStatement` 类型一次迁移后从程序集删除,不得留兼容读者。学术发射以 `DocumentHeader.Digest` 生成 Abstract,Describe 按类型化 AST 深度优先序自动编号为 `1.n`,定理类生成 `Theorem/Proposition/Lemma` 陈述及显式 `Proof`(Lean declaration+axiom badge+终止符),原人类散文归 Commentary;`literature-attested` 以类型强制携 A12 的 L 引用并投影 author-year-title-DOI citation,其余三态投影规范 Source 且不携文献元数据。Markdown 与 QuestPDF 使用同一编号、陈述、证明、citation 和 commentary 数据流。红项=缺 kind/statement/provenance、公式字段非 Formula、TextRun 裸 `$`/`\\(`/`\\[` LaTeX 定界符、L/GID 悬空、citation 元数据缺位、DOI 语法或唯一性坏、发射漂移及旧节点残留;Observe=纯文本/Unicode 疑似公式、代码跨度、Lean docstring 公式、DOI 在线解析与标题一致性,Observe 永不使离线硬门出网。`scribe describe-report [--json]` 离线读取预计算 Lean material 验 selector,以案号 `DESCRIBE-NODES` 发射机器账:逐节点 `GID#describe/DescribeId` 稳定 ID、kind/provenance 统计、`suspected_novel` Papers 候选清单及 `open_count=unassessed` 存量;人工/文献勘正只需改 typed provenance,报告自动消化 open。`DocumentHeader.Anchors` 为 `Anchor` 封闭联合类型,Lean 六行头仍以 `anchors: [string, ...]` 作序列化边界;canonical MD/catalog writer 确定性字节,由 writer 的 first/second 双次序列化断言自洽;**markdown 不再逐字节钉定提交树**(v7.16 R1):它四项合取全真即投影,而投影的字节钉定按 CLAUDE.md 第〇节必须删除——f 与真源都已被守,再守一遍不增信息;提交的 md 是人读快照,`consumed_by = ["reader"]` 故陈旧无害于任何判决;PDF 经 QuestPDF(钉版,社区条款,许可年检入账)。依赖闸门在 Lean 与 NuGet 两侧共用同一许可/证据原则:Markdig/CSharpMath/MathNet.Symbolics/AngouriMath 未准入(证据闸门),iText7 永拒(AGPL);markdown/PDF 自此为构建投影(1.1 三定律与 CLAUDE.md 第 7 条之工程兑现)。理论锚统一外壳为 `anchor := scheme "/" payload`;仓内 scheme 为 `{gict,pzg,spec,lit}`,外部 scheme 泛化为 Lake 解析的外部 Lean 包,各 scheme 保持自身 sealed payload 语法,共同要求 ASCII、ordinal、严格 round-trip,禁宽松归一。外部锚**不设注册表、包目录或 anchor catalog**:某个仓内模块是否依赖某个外部模块,权威是 **Lean import 图**(kernel 验),版本权威是 `lake-manifest.json`,直接依赖声明在 `lakefile.toml`;Lean 头部声明的外部包模块锚必须在该文件的仓内 import 闭包中可达,import 图判不了的锚形一律拒。手维护的 `anchor-catalog.v1.json` 及其 typed manifest 已随之退役——它曾以 0.51% 的覆盖率(198 个被 import 的 mathlib 模块中登记 1 个)和一串无人对账的手抄 revision,构成同一批事实的第二真源,任何手抄依赖注册表/包目录/anchor catalog 同判。第三方 Lean 包的准入谓词仅为可离线客观判定的合取:公开可解析源、不可变 rev、许可与本仓分发相容、在钉定 toolchain/mathlib 下可解析、clean build 通过、axiom/sorry 闭包不扩张;满足即机器准入,否则拒绝或按四态记账。
...
**A19.1 负例地板与观察期义务(v7.14)** 负例地板只覆盖 `RuleLifecycle.Active` 且 `AdmissionEffect` 为 `Block` 或 `HumanGate` 的规则:此类规则每条都必须在基拥有 Golden corpus 中有真实 blocking witness,缺失即 infrastructure fail-closed。`Observe` 规则仍属 active、照常执行并报告 warning,但其语义结构上不参与 admission 阻断,故与 SL-007/009/014 的 deferred 规则同样递延地板义务;不得为了凑红例把构造器、typed loader 或 emitter 的独立 fail-closed 冒充该规则的 blocking witness。规则从 `Observe` contract 为 `Block` 时,contract PR 必须同时加入该规则的基拥有 blocking fixture;合入后下一基线立即把它纳入地板。该递延只校正不可满足的见证义务,不关闭、不软化任何检测路径。
...
warn→required 的升红义务逐预算结算,只许另开独立 PR/check,不得批量升格:owner 须签署该预算与修复动作;同一完整栏键须覆盖至少 30 个日历日且有至少 30 个 `passed` 样本;连续 14 天滚动误报率 `<5%`;相对同栏 baseline 超界 `>=20%`;独立复跑 2/2 仍越界;复审日未过期。误报定义为 WARN 经独立同栏 2/2 复跑未复现,或证实来自预算未登记的宿主/负载漂移。任一收据缺失即继续 warn-only,与 #272 的 Observe 地板递延同构:能力/证据地板未满足是显式 deferred,不是降低地板或让待升规则提前承重。required 后任一撤销判据成立——连续 14 天误报率 `>=5%`、独立复跑不再 2/2 一致、cohort 身份漂移、owner 撤签/空缺、复审到期未续,或 check 阻断一项经语义门证明有效的变更——必须在独立回滚 PR 退回 warn-only;不得调整 admission 掩盖预算误报。**P2 优化收据(待升提,非现役)**:性能优化 PR 只有在 before/after 同 cohort、同 workload、同统计口径,各自原始事件集合与汇总内容寻址,并同时记录失败/副作用与 commit 身份时,方可声称“已获益”;无收据只能称候选或实验,不得以单次最快值、跨机样本或不同缓存态领功。P1 未实现 CI 回流、canonical snapshot、retention 或 required check;P2 的机器、snapshot 与 receipt 仍未建立。
...
- **v7.14 R4**(2026-07-20):性能账 P1 建立三条 P0 可观测的外置 warn-only p95 预算,数据住 `Golden/perf-budgets.toml`,由 strict loader 逐项验证完整 cohort/workload/stage、owner、复审日、修复动作、误报率槽、回滚判据与测量来源;只进 caller-held summary 的 conservative 暂不伪造独立预算。`make perf-report` 在既有确定性汇总后逐完整栏键对照,超界发 `WARN` 但 exit 0;no-data 不跨栏补数。升 required 必须逐预算另开 check,结清 30 天/30 样本、14 天误报率、20% 超界、2/2 复跑、owner 签署与未过期复审收据;任一撤销判据成立即退 warn-only,#272 Observe 地板作为证据不足时递延而不降格的同构判例。admission、CI 回流与 required checks 均未改动。
- **v7.14 R4**(2026-07-20):修复 #246 后门自锁:conservative negative floor 从“全部 active descriptor”精化为“active 且 effect 可阻断”,Observe/warn 规则继续执行与报告但地板义务递延;Block/HumanGate 仍逐规则强制基拥有红例。`SL-023` expand 期因此不再被要求提供结构上不可能的 blocking witness;后续 contract 升 Block 必须与其 Golden 红例同 PR,非法 LaTeX 的 typed 构造失败不得冒充 SL-023 witness。此变更只恢复元层自改通道,不降低检测或既有拒绝能力。
