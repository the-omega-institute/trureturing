...
**A14.9 Inspector-owned report artifacts.** `lean-report-inputs.json` is the sole FILEMAP-registered report input manifest. Its strict JSON schema selects report modules, Inspector sources, configuration and the `lean-report` / `scribe-content` producer scopes through explicit include/exclude patterns. The optional `dependency_sources` path set registers additional local Lean sources that may occur in reported modules' native compiler dependencies; absence declares no additional local inputs. It is an allowed source inventory, not a producer compatibility fingerprint. The current `native_inputs: {"kind":"lake-fetched"}` registration additionally selects the native Lake-resolved package input population; it declares an input kind, not a package-name catalog. Repository-local inputs, including `leanInspector`, retain their existing registrations. Required registrations without files, malformed or duplicate fields, unsafe paths and unsupported schema identifiers fail with a named diagnostic. The required `report_semantic_version` is a developer-controlled positive JSON integer, separate from `schema_version` and all report/wire schemas. Missing, duplicate, noninteger, boolean, zero or negative values fail before accepting artifacts. The current value is 7, including the required native fetched-source bindings. As an explicit developer maintenance requirement, compatible producer refactors retain this value, incompatible report semantics require a strictly upward bump, and the coordinate must never decrease or be reset. The implemented checks require a positive manifest integer and current-version equality for module evidence; they do not compare historical values. Declared-binding module evidence carries this same value in `compatibility_version`; neither judge source bytes nor a producer implementation digest defines evidence validity. Missing required evidence or a different module evidence version rejects; there is no older-version compatibility path. The reader `tools/scripts/report/lean-report-selection.py` expands registered path sets and validates the native input kind; Lake supplies native ownership and dependency resolution, with no independent import parser, MSBuild evaluation or script/call-graph discovery. The existing Scribe projection and describe checks consume the registered scope, including deleted paths.
...
The local dependency-free `tools/lean-inspector` Lake package owns per-module complete row/material bundles and their aggregate under root `.lake/build/lean-inspector`. Native module report facets trace the semantic compatibility token, source bytes, private/server/import/transitive compiler artifacts, each module's authoritative utility record, the fixed judge driver and external claim artifact closures. Registered configuration bytes and the native package/configuration/source-namespace population bind aggregate acceptance; that population is not inserted into every row's trace. The Inspector executable must compile successfully through native Lake dependencies, but its job trace and registered producer implementation bytes are not report regeneration dependencies. Same-version compatible Lean/Python/C#/script or Inspector Lake configuration edits therefore reuse valid module and aggregate artifacts. A semantic version bump invalidates every required module report, without itself invalidating compiler artifacts or changing statement identity or frozen pins. Utility input is generated once per invocation. Current module membership determines aggregation, including removals. Native Lake trace and artifact reuse select misses. The Inspector asks Lake through its native `noBuild` artifact operation, preserves each module's input/output traces, and composes the selected work through a shared native job. Expected internal misses become continuations; other failures and a caller's actual `--no-build` propagate. Batching selected misses and exact validator rejections shares import work without a second invalidation engine or per-miss blocking promise wait. Ordinary root defaults and audit builds remain required. A complete-entry miss need not extract any unchanged row. A validation batch may reuse already verified material identities within that invocation while reading, CRC-checking and hashing every actual material stream again; it retains no report cache or validation state across invocations. New aggregate artifacts still cross the complete canonical bundle validator.
...
Inspector privately stages, canonically validates and publishes raw-v2 rows, all declaration materials including excluded helpers, ZIP digests, provenance-v2 and current-input attestation. A rejected optional artifact rebuilds privately with shared blob writes disabled; invalid required inputs and Lean failures remain failures. The existing coordinate/attestation wire fields `producer_sha256` and `repository_inspector_sha256` carry the semantic compatibility token `SHA256("schema=stratalint-lean-report-compatibility\nversion=<positive integer>\n")`; they are not executable-byte evidence. Report source coordinates select only `report_modules`; Inspector sources remain registered producer provenance and native compiler inputs. Configuration digests retain their registered byte preimage. Each native module bundle includes a strict origin record containing `module`, its singleton raw `report_sha256`, `compatibility_sha256`, `producer_sources_sha256`, `inspector_executable_sha256`, `input_sources` and `external_inputs`, captured at generation. `input_sources` binds the module's own source plus its local non-report transitive dependencies to SHA-256 bytes, constrained by `report_modules` and `dependency_sources`; unregistered local inputs fail explicitly. Other reported modules are bound by the complete report source address, avoiding quadratic closure duplication. Each `external_inputs` row binds `owner`, `module`, `package` (the workspace-relative package directory), `path` (the package-relative regular source), `mode` and `sha256`. Lake's `Module.pkg`, `Workspace.findModules` and `transImports` supply unique owners and the full native import closure, including private transitive inputs, the fixed judge driver and its imports, and utility claims and their imports even when outside the result's import closure. Native acceptance compares the captured dependency set and source bindings with that invocation's Lake-resolved inputs; publication staging and the shell input verifier recheck the bindings against current sources and the native owner inventory. Producer-only files do not become input dependencies merely by being registered. Bundles without a required captured binding fail closed and must reconstruct through native facets; current source bytes cannot retroactively certify old output. The source fingerprint hashes the registered producer path/content digest inventory, including the canonical report-policy projection. Aggregate provenance-v2 retains these exact records in `module_origins`, keyed by module and checked against the canonical singleton row bytes and current compatibility. Mixed generation origins are valid; publication may update only invocation mode and never relabel a reused row as produced by the current executable. Legacy bundles without required origin evidence are rejected and rebuilt privately. The public shell input helper exposes `address`, `verify`, `modules`, `producer-paths`, `scribe-producer-paths` and canonical `coordinates`; it does not own artifact reuse. The thin pair interface invokes one candidate Inspector producer.

Aggregate `native_inputs` captures the native package descriptors, root-manifest pins, resolver configuration, source namespaces and Git snapshot evidence once at aggregate/entry scope. Lake library roots and globs supply the source population, including default/audit Lean inputs; actual Lake build-directory exclusions do not exclude source merely because a directory is named `build` or `bin`. Materialized packages are checked for source/configuration bytes and modes, namespace additions and deletions (including ignored sources), and unique module ownership. Source/configuration paths must remain contained, regular and free of symlink traversal. Package roots retain Lake’s lexical `Package.relDir`; source-directory components are checked before normalization can erase aliases. Resolver capture binds the actual `Workspace.packageOverridesFile` path and its absence/presence, bytes and mode. Resolver configuration changes or unaccounted package overrides/alternate source maps cannot obtain complete-entry reuse. A missing current descriptor with any fetched checkout present requires Lake resolution; optional provenance/native JSON with invalid shapes is a miss, while malformed authored registration remains a failure. Git eligibility checks the captured HEAD against the native pin and anchors tree/blob membership to that same commit, including relevant source/configuration bytes and executable modes; a nonempty revision string is insufficient. Source freshness is separate: changed local dependency bytes may produce an accurate current report while failing immutable pinned-source eligibility. Compilation and report acceptance alone grant neither the immutable-source shortcut nor A17 external-library, license or toolchain admission.

The optional `report_execution` manifest declaration names the supported tools (`lake`, `lean`), platform coordinates (`system`, `machine`) and explicit semantic environment variables. Root inputs are captured before ensure; external inputs are captured after provisioning/native resolution and before default/report output, then rechecked at acceptance, private publication and seal. The normal Inspector entry may seal a `.reuse.json` sidecar only after successful native `:report` (including ordinary defaults/audits), complete publication and eligible native inputs. It binds the complete declared source/producer/configuration inventory, native population, file modes, execution environment and five-file report. Reusing this successful invocation requires unchanged registered roots, pins, configuration and environment, and the complete current-source/material validator to pass on a private snapshot. A previously accepted complete-entry bundle may reuse its producer-bound immutable Git snapshot for a fetched package only when that package directory is wholly absent; a present but incomplete, changed or unsafe checkout cannot use this absence rule. Missing source evidence is a cache miss, never an invented snapshot. Custom defaults or additional target/plugin inputs not accounted for by the native population make the shortcut ineligible and still execute through Lake. An absent, old, damaged or mismatched receipt requests native Lake work; invalid registration and actual build/check failures remain failures. Producer implementation changes prevent reuse of the complete invocation, but do not change the semantic version or invalidate compatible per-module artifacts: the native incremental path retains its existing semantics.
