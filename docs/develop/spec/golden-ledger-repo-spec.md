# trureturing / D5 —— 仓库规范全卷 v7.37(定本:宪法·地层·编码·执法·八官·管线·治理·引导)
...
├── Meta/{FILEMAP.toml, domains.yaml, Digestion/}
...
## 2.3 生长律
目录 >12 文件或文件 >400 行 ⟹ **局部分裂**(按子疆域,组名先入词表;分裂工具(`StrataLint split` 子命令,D5-T0004;成熟前以 git mv+手工 import 重写代行,SL-003 机器执法)单 PR 完成 mv+import 重写);**只裂不迁,永不全局重排;深度对数增长,结构演化 append-only。** 深度无上限:满桶在任一层皆可就地裂出子疆域,SL-003 只数目录内文件、不数子目录,故裂的代价恒为一层而非一次全局重排(2026-09-08:实现侧原将坐标钉死为三或四段,压力遂横向外溢,`D5/S3/ConceptDynamics` 由此长出120 个平铺兄弟子疆域;放开后旧地址一条不翻,属保守扩展)。 容量只约束骨骼:JSON(含 JSONL/NDJSON/JSON5/JSONC)、YAML/YML、TOML、CSV/TSV、XML/TRX、Base64(`.b64`) 数据文件豁免 SL-003 文件行数软硬上限,扩展名不区分大小写,目录容量仍按既有规则计算。Blueprint Markdown 是 FILEMAP `kind=generated` 的人读投影,不另计容量——同一文档的结构名额已由其 `.scribe.cs` 定义支付;文档 GID 须指向既存 Lean 模块,且真实树上的 `.scribe.cs` 定义、反射发现项与同 stem `.md` 路径保持双射,若把源与投影重复计数,合法的十二模块 Lean 桶在第七个蓝图化模块处即溢出其 Blueprint 镜像(2026-07-30,#499/#543 撞墙先例)。该排除与双射只守结构槽及镜像存在性,不校验 `.md` 内容,也不赋予其内容或历史权威。
...
**A2 全域标识符 GID(v7.11 规范虚拟地址)** `GID := THEORY "/" [PLANE "/"] PATH ["." DECL] ["--" TAG]`,PLANE∈{F(省),B,E,C,L,P};每个 PATH segment 必须非空且不得为 `.`/`..`;GID 与**语义目标**立总双射,逐平面唯一反解:F:`D5/<S 层>/<疆域>[/<子疆域>]*/<模块>[.<DECL>]` ↔ `D5/<S 层>/<疆域>[/<子疆域>]*/<模块>.lean` 中之文件或声明(子疆域链长度不设上限,兑现 2.3「深度对数增长」;每段 CamelCase,且**相邻两个桶段**(疆域与其后各子疆域,不含 `<S 层>` 与 `<模块>`)不得同名——重复段不携带坐标信息,地址即不再是其位置的最短名;模块名不参与该比较,故 `D5/S0/Carrier/Algebra/Algebra` 合法(桶对为 `Carrier`/`Algebra`,末段是模块),非相邻的重复亦合法(如 `D5/S0/Carrier/Algebra/Carrier/Ring`));B:`D5/B/<PATH>` ↔ `Blueprint/D5/<PATH>.md`;E:`D5/E/<PATH>.<DECL>--<KIND>` ↔ **唯一单文件** `Evidence/D5/<PATH>.<DECL>.<KIND>`(目录永不充当 E 目标,同一选择子只许一种工件类型),全局常数表为唯一专例 `D5/E/values--json` ↔ `Evidence/D5/values.json`;C:`D5/C/<YYYY-MM-DD>/<slug>` ↔ `Chronicle/<YYYY>/<MM>/<DD>-<slug>.md`;L 根桶:`D5/L/<bibkey>` ↔ `Library/notes/<bibkey>.md`,容量压力裂出的受控疆域桶:`D5/L/<Domain>/<bibkey>` ↔ `Library/<Domain>/<bibkey>.md`,其中 `<Domain>` 必须先入 `Meta/domains.yaml`,且既有根桶地址不迁;P:`D5/P/<paper-id>` ↔ `Papers/recipes/<paper-id>.yaml`,`D5/P/<paper-id>--frozen` ↔ 该冻结包唯一 `manifest.sha256`。F 层工件 GID 即字面 Lean 路径去 `.lean` 后缀,`.DECL` 是该文件内声明选择子;其余平面 GID 是虚拟地址,不得与物理路径混写。例:`D5/S3/Spectral/GapLabeling.gap_label_mem`、`D5/E/S3/Analytic/Cphi.result--json`、`D5/E/values--json`、`D5/C/2026-07-06/r168`、`D5/L/Zeros/coffey2007theta`、`D5/P/D5-P001--frozen`。**papergen/blueprint 只接受全 GID;跨库引用自带理论坐标。** **M0 admission 精确主张**:给定一个受支持且 machine-decide 判词为 admit 的语义 manifest,至多存在一种规范表示与恰一次 admission;不受支持或机器判词非 admit 的 manifest 按 fail-closed 得零次 admission。受 manifest 路由的 JSON/YAML 结构化语义工件现役强制 UTF-8、禁 BOM、对象键字典序、禁行尾空白且末尾恰一 LF;完整 Unicode NFC、默认值与 tag 顺序规范化延后 D5-T0015,故字节规范不得报 full active。
...
**A5.2 Information registration and declared templates.** A declared readout names exactly one enrolled template and its ordered arguments. Templates, enrollments and binding declarations are content-plane inputs to the judge; no content declaration, Boolean, callback or family table grants admission. The judge applies the same finite grammar to every module and never widens it for a corpus module. The command forms are:

```lean
register_information_template T
register_information_template T constructors 1 [AST]
register_information_theorem thm in A readout via (T args) primitives bundle realization bridge
register_information_theorem thm in A readout via (T args) primitives bundle realization inline realizationTerm := proof
register_information_theorem thm via (ReifierTemplates.pointwise f g) in A readout via (T args)
register_information_theorem thm in A object_arena O catalog C readout via (T args) primitives bundle realization bridge
register_information_theorem thm in A object_arena O catalog C readout via (T args) primitives bundle realization inline realizationTerm := proof
information_theorem thm in A readout via (T args) primitives bundle : statement := proof
```

`T args` denotes an ordinary elaborated Lean application, including its implicit arguments. The `information_theorem` form also permits `object_arena O catalog C`; primitive registration forms retain their optional `variation` and `sensitivity` witnesses, and the reifier form retains optional `output_evidence`. A legacy realization is either an existing theorem name or the disjoint inline form `inline realizationTerm := proof`. The inline form elaborates `realizationTerm` at `PrimitiveRealization A.signature`, elaborates `proof` at the exact `LegacyPrimitiveRealization A (typeOf thm) realizationTerm`, and emits the registration-owned theorem companion with its complete universe telescope. The companion passes the same kernel, closedness, axiom, statement, arena, primitive-bundle, unit, witness, occurrence and transaction checks as the named form. Its proof implementation remains opaque under E5: it adds no proof-root field, proof dependency traversal or semantic hash; freshness continues to use the complete module source closure. `readout via` supplies binding metadata to the existing registration transaction; it does not replace the theorem, Law, primitive bundle, bridge or their semantic checks. Registration without a declaration emits `undeclared` evidence and is subject to A5.5; it does not select an alternative readout checker. Declarations are inline in their Reg registration owner; the former binding-sidecar command is retired.

**A5.3 Template enrollment judgment (E1–E8).** Enrollment checks a definition once and publishes a private checked plan. Occurrence assessment selects that one plan by canonical Name and instantiates its retained obligations; it does not search the template population or re-audit the selected definition's body. The byte-radix index bounds lookup by the selected key length, independently of the number of enrolled templates. Unknown forms fail closed; a frozen, governed or upstream identity alone supplies no grammar rule.

- **E1 — telescope and result.** The template is a safe, nonrecursive `def` or `abbrev` with an available body, rigid universe parameters and an ordered Pi telescope retaining every `binderInfo`; a domain may refer only to earlier slots. Admitted slots are carriers in successor universes, data, functions, predicates, `Fintype`/`DecidableEq` dictionaries, indexed decision families, proofs of admitted propositions, and explicit arena/signature interfaces. All domains and the result satisfy E2; the result interface is exactly `PrimitiveRealization signature` or `StructuralPrimitiveRealization arena signature`. Arbitrary `Prop` parameters, unindexed `Decidable φ`, arbitrary typeclass slots, Laws, bridges, theorem units, Name/String/Expr/hash/certificate inputs and unrelated return interfaces have no rule. `DecidablePred` and `DecidableRel` undergo the same indexed-dictionary judgment as explicit Pi syntax: an index must survive typing normalization of the proposition. Raw domains are checked and retained before that test. A quantified proposition used as a parameter type is a proof slot, not a function slot. These typing operations do not authorize binding-comparator normalization.
- **E2 — types, propositions and dictionaries.** The closed grammar admits symbolic slots, universe sorts, Pi/arrow over admitted domains, `Unit`, `PUnit`, `Bool`, `Nat`, `Fin n`, `Prod`, `Sum`, `Option`, `Subtype`, and checked arena/signature/realization interface types and projections, including `State`, `Index`, `Output`, `AnchorIndex` and `PrimitiveAxis`. Propositions admit symbolic predicate applications, equality on admitted data, `True`, `False`, `And`, `Or`, `Not`, `Iff`, Pi/Exists over admitted data, and Nat index bounds. Every child is checked. Dictionaries are symbolic slots or the fixed constructive Unit/PUnit/Bool/Fin/Prod/Sum/Option/Subtype combinators. A combinator matches only its pinned standard Name, universe arity, proof-erased raw type/body identities and actual declaring module; a same-typed instance does not match. Missing pins are incomplete and mismatched identities are unclassified. Arbitrary closed proposition constants, statement-bearing opaque carriers, closed decisions and classical choice have no rule. The lowering table belongs to judge grammar, never content enrollment data.
- **E3 — bodies.** Admitted nodes are typed variables, admitted constructors and structural index literals, lambdas, retained metadata, retained lets with checked type/value/body, applications of admitted functions/predicates/dictionaries, checked interface-record construction, fixed interface projections, and field projections of constructor-enrolled types (`E3.constructor_projection`: a raw projection node or a projection-function application saturated through the self argument compiles to the structural read of that field, function-valued fields included; recursors, `casesOn` and `match` on those types stay rejected under E4c, as do class projections and projections of types not enrolled by E4c). Constructor parameters, motives, instances and every field are checked, including fields later discarded by extraction. Explicit Nat telescope positions, implicit constructor arguments that occur in the constructor's result type (such as the cardinality of `Fin.mk`) and the propositions of proof leaves provide structural-index context; other positions reject non-index numeric literals. Strings, names and quoted expressions have no rule. Executable `decide φ d` requires symbolic object dependence in both the proposition and its decision family; a closed decision fails with `E3.closed_decision`.
- **E4 — eliminations.** The base grammar admits Prod/Subtype projections, Unit/PUnit elimination, Bool/Option/Sum cases, `Fin.elim0` at its kernel type, and the fixed nonrecursive constructor cases with checked motives and branches. Every branch is checked, including dead branches. General recursion, `Nat.rec`, Eq/cast elimination into executable data, choice, Quot elimination and proof-to-data elimination have no rule. A generated matcher must expose an admitted kernel form. The explicit `constructors 1 [...]` extension admits non-mutual, unindexed inductive descriptions with direct strictly positive recursive fields; E4c checks non-parameter constructor fields and direct recursive arguments, while actual uses still pass the shared expression compiler. Only an original AST slot or a direct recursive constructor field supplies descent authority to its kernel recursor; motives and branches are checked and recursive results are symbolic slots. An enrolled inductive whose constructors all have zero fields is an enumeration: its recursor (`E4c.enumeration_dispatch`, reached also through `casesOn` and compiled `match`) is finite dispatch admitted on any admitted major premise with a constant motive, exactly as Bool/Option/Sum cases are; enrolled types with fields keep the descent-authority requirement, and dependent motives and non-enrolled types stay rejected. Indexed/GADT families, nested or function-valued recursion, arbitrary fixpoints/evaluators and unknown extension versions are unsupported.
- **E5 — expansion and proof leaves.** Safe saturated definition applications expand by bounded capture-avoiding substitution of their full telescope. All raw arguments are checked before expansion, including unused arguments. Public Pi domains, body-lambda domains, instantiated helper result types and checked argument obligations remain in the plan even when typing considers them definitionally equal. Recursive/mutual and opaque definitions have no expansion rule. At a proof-valued subterm, check its entire proposition type in type position and retain `ProofLeaf(type)`; the proof implementation is opaque: it is not entered, contributes no dependency, is not compared and is not hashed (τ=0 owner ruling 2026-09-18: the judge judges data, proofs are irrelevant). A proposition expression is not erased merely because its type is `Prop`. Two proof leaves are equal exactly when their propositions are raw-equal; proof-to-data eliminations remain E4 rejections. Independent subtype bounds/signature proof fields are permitted, but their implementations cannot be observed.
- **E6 — theorem independence.** Proof subterms are erased to their propositions before identity checks; raw identity rejection on data precedes projection reduction and definition substitution. The executable/body-type graph rejects registered theorem/proof identities, companions, certificates, statement/hash/query APIs, identity-bearing carriers and theorem-specific decisions. Arbitrary closed theorem constants used as executable sources lack a rule even before registration; the same constants in proof position are opaque leaves whose propositions are checked. The common judge-identity predicate also rejects plans, frames, indexes, dependency identities, occurrence events/keys, binding records/claims/results/certificates, joined records and P1 semantic certificates, including constructors and named/raw projections; selected-plan lookup, observation, inventory, assessment and export are not content primitives. A negative identity test grants no admission. Retained lets check the raw value identity before the complete type/value/body. Proof implementations remain outside traversal; only their propositions pass the occurrence and type checks.

**Independent infinite sources.** E2 also admits a carrier constant in occurrence arguments and forwarding-helper types when its declaring module is independent of the target theorem's module under the import graph; its arguments are checked in type position. This gives an infinite source type its own identity without equating it to a finite `Arena.State`. The pinned standard `Classical.decEq` is an additional E2 dictionary combinator when applied in type position. E5 also admits a safe, nonrecursive, unapplied source definition of function type as an atomic readout when its complete type passes this compiler, an input binder contains an independently declared non-finite carrier, its result has no loose binders, and its declaring module is independent of the target theorem's module. A carrier occurring only in the output or a dictionary binder does not suffice. Its native type and body identities remain dependencies. The rule does not apply to template enrollment. The source must be outside the registration and judge packages and outside the target theorem's module; its imports must not reach that theorem's module. Its body is not classified by E3-E4, so this rule certifies source independence and binding identity, not the internal algorithm of an analytic readout. All other E6 identity exclusions still apply to raw arguments and checked types. This extension does not admit a local or target-dependent function through an opaque-source exception.

**One raw-argument compiler.** Enrollment, declared supplied arguments, forwarding-helper type checks and literal-receiver checks use the same E2–E5 compiler and checked constructor descriptions. Occurrence argument checking adds the prohibition on the target theorem, retained raw statement, its proof/decision/certificate and identity-bearing carriers. No whole-readout allowance table supplies a declared-template rule. All retained type nodes, proof-leaf types, public/body domains and audit inputs undergo slot/universe substitution in their original binder contexts and occurrence-relative identity checks, including on an identical-descriptor comparison path. Unknown identity is unclassified, exact target identity is forbidden, and inference or budget failure is incomplete (`dtr.instantiated_type`). A symbolic structural type instantiated to the target statement cannot escape through proof erasure, an unused parameter or an intermediate beta step.

**E7 — producer evidence.** The private `CheckedTemplatePlan` constructor alone publishes enrollment evidence. Its data retains schema/grammar/compiler/toolchain identities, definition and enrollment owners, rigid telescope and slot types, constructor descriptions, proof-erased raw type/body SHA-256 identities, dependency inputs, checked body/type plans, retained proposition/type obligations, charged work and rule IDs. Raw identity uses domain-separated, length-prefixed Expr/Level serialization with anonymous binder names and positional universe parameters; it preserves binderInfo, metadata, let structure, instances, literals and argument order, erases proof subterms to their propositions, and does not reuse a pretty-printed statement hash. Dependency identities contain canonical Name, actual declaring module, raw type identity and executable body identity; proof implementations contribute neither a dependency nor a body identity. Native coherence checks bind evidence to the actual compiled producers. Plans retain no source-file byte hashes; their identities and assessment depend on compiled information. Lake traces govern recompilation and reissue evidence.

The persistent `TemplatePlanFrame` contains canonical Name bytes, a token-interned payload and its SHA-256 identity. The encoder, decoder and private extension use one current frame layout and version tuple, distinct from the module report release version in A14.9. The decoder accepts exactly that layout and tuple. Literal tokens use canonical decimal UTF-8 byte length, a colon and exact bytes; repeated tokens use backward-only `@<first-literal-index>:` references. Duplicate literals, leading zeros, invalid UTF-8, unknown markers/references and trailing bytes reject. The fixed-width charged-work literal is outside interning. Both serialization passes debit token processing and emitted bytes and must agree on work. Imported ownership is checked against the actual extension container and Lean declaration-to-module table, not supplied owner strings. No older frame layout is an alternative reader path.

**E8 — bounds and failure.** A template frame's key, payload, digest and framing together must fit 65,536 bytes; canonical Name length is at most 1,024 bytes, telescope slots at most 64, visited definition constants at most 4,096, and charged expression/construction/byte work at most 524,288 units. Structural depth is at most 256, including Name/Level encoding. An imported environment retains at most 8,388,608 frame bytes; limits are checked before decoding, and the stream stops at its first failure. Decoding bounds tokens, counts, nesting and allocation to at most 32 times payload bytes. Substitution, abstraction, de Bruijn lifting, universe instantiation, materialization, identity encoding and comparison share the caller's remaining quota, charging before construction; compression grants no extra budget. Universe substitution permits only Lean's cheap `mkLevelMax'`/`mkLevelIMax'` simplifications, not term normalization. Enrollment and occurrence assessment each impose a cumulative `maxHeartbeats` ceiling of 100,000 public-option units (1,000 internal allocation heartbeats per unit), also lowering `Core.Context.maxHeartbeats` and respecting smaller nonzero enclosing limits. Nested checks cannot reset the outer total; it is checked after the final subcall. Options can only lower these limits.

Unsupported/malformed forms yield `unclassified_form`; established E6 identities yield `forbidden_dependency`; missing bodies/owners/inputs, failed inference, metavariables, stale evidence or exhausted bounds yield `incomplete_closure`. Available unsafe/opaque/recursive definitions are unsupported, not missing input. For established simultaneous causes the priority is incomplete, then forbidden, then unclassified. Failed enrollment emits no plan; it does not create a review-pending admission state.

**A5.4 Declared readout evidence and IE-C050.** Binding comparison preserves the origins `template-body`, `supplied-argument`, `actual-extraction` and `proof-leaf`. Extraction reads explicit native realization fields, the realization argument in a `LegacyPrimitiveRealization` or `EscapeRecord.EscapePrimitiveRealization` bridge's type, or the declared structural realization; it does not inspect the bridge proof or `TheoremUnit.proof`. A literal interface record or safe saturated closed forwarding definition may expose the realization. Forwarding must retain every parameter exactly once, in order, as a whole argument; raw arguments, complete literal receivers and helper types are audited before selecting fields, with proof parameters and arguments opaque while their propositions still pass the common compiler. Unsafe/recursive/mutual/extern/implemented-by extraction definitions are unsupported.

Comparison permits only binder-name anonymization, positional rigid universe alignment, fixed projections on audited interface constructors, expansion of the selected checked plan or admitted forwarding form, and capture-avoiding beta substitution at plan-created lambda sites. A named extraction's nonempty universe telescope must match the occurrence arity and is instantiated positionally; it is not solved by type equality. Complete dependent signatures, readouts and all anchor functions must match by raw `Expr.equal` after these rewrites and after proof erasure. Supplied-argument nodes are immutable. Metadata, lets, binderInfo, literals, dictionary expressions and rigid levels remain significant; proof expressions are compared by their propositions only. There is no general `whnf`/`isDefEq`, eta/zeta reduction, metadata stripping, arbitrary iota simplification, sample enumeration or discarded-argument reduction in this comparator. “Defeq-free” describes comparison, not Lean typing. A frozen realization still needs the selected plan to match.

Every successful registration emits a separate occurrence event and `BindingRecord`. Its exact key is `(root, registration_module, theorem, object_arena, catalog)`; the record retains statement/type identity, rigid parameters, unit/realization names, registration source identity, arena, optional descriptor and binding owner. The result alternatives are `undeclared`, `declared_unresolved(diagnostic)` and `declared_validated(certificate)`; only the last carries a declared-template certificate. Missing/unregistered template heads, unsupported bindings and exhausted binding checks are evidence failures, not module-build errors. Syntax/type errors and existing semantic insertion failures retain their throwing behavior. Transactions roll back generated declarations, enrollment, events, records, certificates and diagnostics on exception.

The same `TemplateBinding.assess` function serves registration and authoritative assessment. The latter loads the complete governed registration inputs and joins claims by exact occurrence key, deriving origins from actual native extension containers. Duplicate events/claims, dangling keys, wrong owners, changed arenas/catalogs or retargeted statements/units/realizations fail closed. A generated unit must be declared in its registration module; a named realization may come from that module's actual transitive import closure. C# resolves `unit_name` and `realization_name` as compiler display spellings: exactly one local unit declaration of kind `def` and exactly one realization declaration in the import closure must match. Occurrence-key components retain strict round-trippable Name grammar. Seal-generated unit abbreviations do not register occurrences merely by their suffix.

The module wire field `information_templates` has exactly `{schema_version,compatibility_version,inventory,registered,records}` with schema 1 and compatibility read from `report_cache_release_semantic_version`. Every record has exactly `{key,registration_source_path,statement_identity,unit_name,realization_name,binding_source_path,state,diagnostic,certificate,escape_from,escape_continues,bridge_kind}`. A validated certificate has exactly `{key,evidence_ref,plan_identity,descriptor_identity,actual_identity,argument_inputs,extraction_inputs}` and no diagnostic. Other states have no certificate; unresolved declarations require a diagnostic and binding owner, while undeclared records have no binding owner and the exact occurrence-addressed missing-declaration diagnostic. Certificate dependency entries have exactly `{name,owner,type_identity,body_identity}`. The evidence reference (`DTR-binding-evidence-v2`) hashes a domain-separated, length-prefixed occurrence tuple, statement identity, selected plan, descriptor, actual extraction, all escape identities and bridge kind, and separately sized dependency arrays. `escape_from` is null or `{name,type_identity,object_identity}` with the canonical object head and raw type/object identities. `escape_continues` is null or `{kind,declaration_name,statement_identity,chain_name}`: `kind` is `witness`, `empty` or `open`; only `open` has all three certificate fields null. `bridge_kind` is `legacy` (including exact native Law registration) or `forward`. Certificate and named-chain dependencies participate in compile-time assessment invalidation. It cannot be copied to another occurrence.

The native exporter and strict `InformationTemplateEvidence` reader reconcile independent compiler registration keys, command events and binding records by exact key-set equality, with complete governed-source coverage. Missing evidence is never an empty inventory. Unknown fields/states, contradictory results, wrong source ownership and copied certificates reject. Module evidence and records contain no raw-source input hash lists; the row's own `source_path` and `source_sha256` are the sole module source coordinates. Report reuse follows A14.9's Lake traces and `report_cache_release_semantic_version`; retained report fields depend only on those traced inputs and the release version.

For admission, the strict reader applies these checks only to A5.5's selected registration owners and their binding claims. Selection precedes template-evidence validation, including when a producer also carries claims for unselected owners.

Authoritative assessment may cache only validated results in a private, nonpersistent environment cache. Reuse requires the exact occurrence/statement/levels, claim and owner, selected plan, options, registration context, unchanged native constant objects/owners and relevant class/reducibility/projection/extern/implemented-by metadata, plus native coherence. Invalid cache inputs cause reassessment. Mathematical seal publication consumes joined producer-issued metadata in its immutable environment; it issues no binding certificate or subsequent file-freshness claim. An unresolved binding does not invalidate an already valid mathematical seal or certify template completeness. The registration owner supplies its binding declaration; mathematical source owners remain independent. P1 semantic insertion, reflexive-source exclusion, certificate validation, C048/C049 variation/sensitivity obligations and rollback remain independent conjuncts; the new binding record never enters the existing semantic throw-on-insert branch.

Declared-binding diagnostics use IE-C050 with the implemented reason priority: `incomplete_closure`, then `forbidden_dependency`, then `unclassified_form`. Its declared-binding diagnostics use these coupled trigger/message/payload contracts:

| Table | Current declared-binding contract |
| --- | --- |
| Trigger | Missing declaration; unresolved template head; failed E1–E8 or constructor rule; forbidden argument/type identity; unsupported extraction or raw mismatch; incomplete current evidence; exhausted bound. |
| Message | `IE-C050 ClosedTruthReadout key=<root>/<catalog>/<theorem> reason=<reason> rule=<rule> site=<json-string> readout=<json-string> provenance=<payload>` |
| Payload | Canonically ordered `{argument_inputs,extraction_inputs,plan_identity,rule,site,template_key}`. Missing declarations use `unclassified_form`, `dtr.missing_declaration`, empty site/readout and input arrays, and null template/plan. Incomplete provenance is null. |

Diagnostic input arrays describe direct raw-argument roots and native extraction identities; they neither expand the selected template body nor assert an admitted closure. Grammar failures name their E-rule; binding/extraction failures name their dtr rule. The strict reader requires the exact missing-declaration diagnostic for the same occurrence; null or retargeted text is invalid. The diagnostic is metadata, not evidence of a validated binding.

**A5.5 Declared-template judgment (SL-031).** `DeclaredTemplateBindingRule` is a named obligation of SL-031, dispatched by `UtilityAdmissionRule` alongside the utility obligation in A5.1. Its registration domain is present Reg Lean modules that are candidate-new or byte-changed relative to the protected base. Rename/copy destinations participate. Separately, `RepositoryRules.ChangedOrFirstPinD5Modules` selects changed, new or first-pinned D5 source owners for newly public theorem lookup; D5 template payloads are never registration evidence. Judge, FILEMAP, compiler or toolchain changes alone do not select unchanged registration owners. An empty selection produces no declared-template findings. Every newly public `theorem`/`lemma` in a selected D5 source (no public theorem/lemma with that name in the base bytes of the same path) must have at least one `declared_validated` four-slot registration in its exact `Reg/D5/<source path>.lean` mirror or a Reg module explicitly imported by that mirror. The theorem's compiler source owner, not its namespace spelling, determines the mirror. Only matching theorem occurrences in that import closure are assessed; an unimported registration cannot supply coverage. A first pin alone creates no new authored theorem. Missing coverage emits `DTR-Unregistered` (Observe). Private and internal-detail declarations, `def`, named source `instance`, `example`, and machine-generated companions are outside this obligation. Explicitly authored theorem names do not acquire a suffix-based exemption. Base bytes are read only through the repository snapshot gateway; base code is never executed. Known limitation: anonymous proposition-valued instances elaborate as theorems and cannot be distinguished in the report. The finding names are exactly `DTR-Undeclared`, `DTR-Evidence`, `DTR-Unregistered` and `DTR-Declared`; every one of them is Observe. The obligation collects registration state as warnings and never blocks admission; changes to this judge remain owned by the registration research line (#5214), while theorem authors and reviewers own the delivery evidence described below.

**Author delivery and registration completion.** For each newly authored public `theorem`/`lemma` in the selected D5 source scope, authors and reviewers must attempt faithful registration in the same delivery. Completion requires at least one `declared_validated` four-slot registration in the source owner's mirrored Reg change, compiled Lean proof terms and current binding evidence. D5 content-template theorems/lemmas are included. Helpers in `Reg`, `Interface` and `Impl` create no recursive D5 audit target and retain their existing kernel, `sorry` and axiom checks. CI success or an Observe warning is not registration evidence; missing, invalid or uncompiled evidence leaves the audit unfinished. Registration proofs create no mathematical GID, Scribe, deposit or utility obligations, and zero-new-Lean reuse creates no artificial audit wrapper.

**Registration obstacle exception (CLAUDE.md §3.9).** If a concrete obstacle in template expressiveness, interfaces, or registration proof/evidence production or processing prevents completion, open or reuse a relevant issue identifying the original theorem and source, actual failure and missing evidence or obligation. Link it in delivery and explicitly state that the escape audit is unfinished; mathematical development and delivery may continue without waiting for registration repair. This narrow exception takes precedence over CLAUDE.md §5.4/5.5: exhaustive encodings, repeated retries, an immediate hotfix/repair lane and a separate waiver are not prerequisites. The issue is not proof, `declared_validated` or a new machine status. Wrong templates, weakening the original theorem and bind-only wrappers remain forbidden; Lean/kernel, `sorry`, axiom, freeze and all other required checks remain required. This introduces no historical backfill or new debt ledger and changes none of the four Observe findings, delta selection or judge ownership. Owner instruction, verbatim: 「改一下claude md 跟形式化skills, 如果无法登记, 则留个issues, 然后继续即可.」

Report artifacts are isolated by Lake traces and `report_cache_release_semantic_version`; configuration identity belongs to the aggregate. An enrollment plan is a compile product of its owner module, contains neither source-file byte hashes nor manifest-derived values, and is invalidated only by Lake import traces, including its imported judge.

Registration ownership determines the evidence domain. The reader collects each selected owner's current producer evidence and its inline claims, then performs A5.4's strict join and certificate checks. Owner addresses are inspected to route claims before their payloads are validated; either a selected registration source or a selected registration-module address retains a record, so inconsistent addresses cannot hide a selected claim. Other modules cannot overlay a selected registration; their unrelated registrations are outside its evidence domain. Outside the selection, registration payloads are not read as evidence, judged or used as a failure cause at the report loader, evidence reader or rule layer. Missing, malformed, unresolved, stale or incompatible template evidence for unselected registrations cannot fail admission.

Whole-artifact integrity remains global: canonical UTF-8 JSON bytes and report structure, complete module membership, source path/hash structure, declaration material content addresses, and the bundle provenance and attestation checks in A14.9 still apply. These checks establish the integrity of the producer artifact consumed by the repository; they do not validate its unselected registrations. `RawLeanReportArtifact` retains template payloads without interpreting them, and `InformationTemplateEvidence` projects the selected owners before applying the strict template reader. Global artifact validation must not turn an unselected registration's template evidence into an admission obligation.

The closed finding-name allowlist for this obligation is exactly the following table. Evidence-reader diagnostics are message details under `DTR-Evidence`, not additional finding names.

| Finding | Effect | Judgment |
| --- | --- | --- |
| `DTR-Undeclared` | Observe | A selected occurrence remains `undeclared` or lacks `escape from` / `escape continues` after joining claims. |
| `DTR-Evidence` | Observe | Selected producer evidence is missing, malformed, unresolved, inconsistent or lacks a valid declared-template certificate; current-file digest comparison is not a DTR-Evidence check. |
| `DTR-Unregistered` | Observe | A new public theorem has no validated four-slot registration. |
| `DTR-Declared` | Observe | A selected occurrence is `declared_validated` with all four slots; reports `escape_from`, `escape_continues`, `bridge_kind`. |

To be reported `DTR-Declared`, each selected occurrence declares four slots: (1) `escape from (<term>)` supplies a constant or binder type occurring in the raw elaborated statement and definitionally identifies what the closed arena's State represents; absent occurrence gives `dtr.escape_from_absent`, wrong State gives `dtr.escape_from_state`. (2) `readout via (<T args>)` passes the existing enrolled-template E1–E8 and exact extraction checks. (3) The realization bridge retains exact statement/arena/primitive-bundle checks; `EscapeRecord.EscapePrimitiveRealization` proves only statement ⇒ Law and requires valid variation and sensitivity (`dtr.forward_bridge_requires_sensitivity`), while the legacy equivalence and exact native-Law forms remain available. `CounterexampleRecord.WitnessPrimitiveRealization` over a `WitnessArena` is the witness bridge and reports `bridge_kind=witness`. Its registered statement is `¬ c` for a closed, universe-parameter-free `Prop` definition `c` that is definitionally `∀ d : arena.Domain, arena.predicate d`; the theorem type is definitionally `¬ c` and its proof term is checked against that type (`dtr.witness_statement_identity`). The judge inspects the named claim and the bridge arguments, never the refutation's proof body. For this bridge the `escape from` term is identified against `arena.Domain` instead of a State. The bridge's actual realization is definitionally the arena-computed `WitnessArena.realization`, or shares its `PrimitiveRealization.readout` (`dtr.witness_readout_tie`). The named variation is `Law actual ∧ ¬ Law constantTrue` for that same actual over the derived law arena (`dtr.witness_bridge_requires_positive_variation`, `dtr.witness_bridge_requires_negative_variation`), and `FiniteSlotSensitivity` of the derived law arena is mandatory (`dtr.witness_bridge_requires_sensitivity`). An explicit-object occurrence keys on the derived finite arena and keeps the original witness arena for the statement and Domain checks. (4) `escape continues (<term>)` supplies a named `EscapeRecord.EscapeResidualWitness` value or an `EscapeRecord.EscapeResidualEmpty` proof for a named `LayerChain` over that same arena (`dtr.escape_continues_arena` rejects another arena), or the literal `open`. Membership is kernel-checked through the witness field; the judge checks types and names without evaluating states, chains or certificates. Literal `open` is a truthful unknown-residual marker without a certificate: it proves neither undecidability nor inexhaustibility and cannot replace the realization bridge, any required variation/sensitivity proof, template application or binding evidence. When all other required evidence validates, an explicitly open residual is still a complete honest registration.

The two optional escape clauses follow variation/sensitivity (or output evidence), and precede the statement colon for `information_theorem`. All new words are non-reserved. Old-form registrations keep their parser and producer status; the delta rule imposes the four-slot obligation. Valid registration completion requires at least one complete verified registration per new public D5 theorem/lemma in scope, with its compiled proof and current binding evidence in the mirrored Reg change; delivery with an unfinished audit follows the exception above. Reuse a lawful existing template; if none applies, add a meaningful parameterized content template with shared enrollment in `Reg/Support`, subject to the same concrete-obstacle exception. No forced template fit, judge relaxation or bind-only D5 wrapper is allowed. Multiple registrations on different arenas coexist under their original occurrence keys; there is no completeness or naturalness judgment. A selected Reg module with an empty inventory still requires current producer evidence; new public D5 theorems use the separate mirror lookup. Other admission obligations remain independent.

Owner rulings (τ=0, 2026-09-18), verbatim: 「你就只判delta就可以, 很简单新定理需要给出几个东西, 原来逃逸在哪里, 把逃逸怎么处理的, 出来什么新信息, 新信息在哪里继续逃逸.」 「应该至少一种吧, 就是这个本质上就是你写了能过机器验证肯定是对的, 至于有没有其他种类, 那你写两种就有两种逃逸方式?」 Owner ruling (τ=0, 2026-09-20), verbatim: 「把判官从block 改成warning … 把改的权限全部收到你这边来吧, 否则太乱了. 你只要收集他们的warning就可以了.」

**A5.6 Declaration package and dependency direction.** Escape declarations are metadata about mathematics and live apart from it. The D5 package holds mathematics and the content-level arenas, templates and record types; it requires Mathlib only and imports neither a judge module nor a declaration module. The judge is two Lake packages, each with its own lakefile and manifest: the interface package (the declaration command syntax, the record of one declaration and the stable types an author needs) and the implementation package (E1–E8, gates, assessment and evidence), the implementation requiring the interface. Declarations live in a third Lake package, `Reg`, with its own lakefile and manifest, requiring the D5 package and the judge packages. The dependency direction D5 ← judge ← `Reg` is one-way; no edge points from D5 to the judge or to `Reg`.

The declarations for the theorems of `D5/<path>.lean` are written in `Reg/D5/<path>.lean`, the same path shape; catalog roots, seals and shared enrollments are `Reg` modules that import those leaves. A declaration names a theorem of an imported module and uses the four-slot grammar of A5.5 unchanged. The verdict of a declaration is the compile product of its `Reg` module under the judge it imports, and Lake's import trace is the only invalidation of that verdict: a judge change rebuilds the `Reg` modules that import it and no D5 module. There is no manifest of module names, no additional cache and no registration-specific freshness mechanism.

`DeclaredTemplateBindingRule` stays delta-only and observe-only (A5.5). Its selection covers changed `Reg` modules; a newly authored public theorem/lemma declaration in a changed selected D5 source module is looked up in its mirror `Reg` module by path. The resulting `DTR-Unregistered`/`DTR-Undeclared` readings remain warning-only for admission. Authors must attempt the same-delivery registration and compiled evidence described in A5.5; its linked-issue exception permits delivery while the audit remains explicitly unfinished, without changing registration completion or runtime assessment. Helper declarations authored in `Reg`, `Interface` or `Impl` do not create another D5 selection or recursive Reg/D5 audit target; their existing kernel, `sorry` and axiom checks remain. `Reg` modules are report modules of `lean-report-inputs.json`; the report's version and reuse mechanisms (A14.9) are not changed by this architecture.

For author delivery, compile all Reg sources actually retained by that delivery and retain successful audit proofs and binding evidence. If no valid Reg source can be delivered, do not invent a mirror or require a nonexistent build target. Remove this attempt's failed audit source additions and imports if they would break the normal build; do not remove pre-existing or successful sources to hide failures. Report the actual obstacle under A5.5. Every retained Reg source keeps its existing build, report, kernel, `sorry` and axiom obligations.

A frozen D5 module may be edited or moved to take declarations out of it; mathematical statements and proofs do not change, and pins are regenerated by the canonical writer. Until the migration completes, the D5 modules that import the judge are base-owned debt under SL-001: a candidate-new edge from D5 to the judge or to `Reg` is rejected, the candidate's edge set is a subset of the protected base's, a change to an indebted module strictly reduces its debt, and the same rule judges the full tree once the set is empty. The D5→judge/Reg edge set is empty. The sidecar command `declare_information_template_binding` and its cross-module overlay reader are retired; the grammar rejection has a negative test.

Owner rulings (τ=0, 2026-09-20), verbatim: 「我觉得用户写D5定理, 然后写个类似于元数据的东西, 把逃逸怎么逃逸的登记清楚. 编译的时候, 会load 判官跑一下, 但是判官怎么改不应该引起二次编译什么的.」 「单独建一个项目, 这个项目依赖判官跟D5, 在这个项目里面按照同样的目录写逃逸声明? 判官应该也分interface跟implement两个工程, 然后依赖反转一下.」 「独立manifest, 不要把定理跟这些混在一起. 而且依赖更干净.本来判官也是独立 manifest 的」 「改吧, 彻底改, 冻结的也能改, 又不是区块链.」

**Reg package admission**
...
`make lean` without explicit targets demands root, Impl and Reg defaults in dependency order. Root `lakefile.toml` requires only Mathlib and owns only `Trureturing`/D5. Interface has no package dependency beyond Lean. Impl requires root D5 and Interface, owns `LeanInformationAudit` (including its generic tests) and the explicit `LeanInformationAuditAnalysis` library, and builds in `.lake/build/lean-inspector/producer`. Its manifest inherits the same pinned git dependencies as root and shares `.lake/packages`. Reg owns the mirrored declarations and the actual downstream `LeanInformationAuditRegTests` defaults. `LeanInformationAuditRegAnalysis` stays explicit; the physical `tools/lean-inspector` directory does not imply Impl library ownership.

Explicit `LEAN_TARGETS` retain their scope: Reg module/package targets and `LeanInformationAuditRegTests`/`LeanInformationAuditRegAnalysis` go to Reg; judge module/package targets go to Impl; D5 and other root targets go to root. A failure in any demanded build fails the entrypoint. `make lean-report` uses the Reg workspace: its report facet produces registered report modules and the caller supplies selected registered program targets (all registered targets for an unscoped direct call); successful whole-report reuse still builds selected program targets, with no program build when that selection is empty. After `make lean-cache-ensure`, package-cwd Lake builds are supported. The cache reader/writer validates shared Reg/root manifest agreement before either supported entrypoint lets Lake materialize dependencies. Lake imports own compile invalidation; no runtime module inventory or extra cache is introduced.
...
Generic seal and companion consumers resolve this contract by the compiling root's identity. Production contracts and the independently authored snapshot/baseline rows live in Reg/Support and are installed explicitly by Reg/Catalogs. Impl starts with no implicit catalog contracts and contains no production FixedSnapshot/FrozenBaseline supplier or seed acquisition path. Contract declaration acquires expected, source and baseline provenance before crossing the olean boundary. Revalidation and sealing consume only that compiled evidence; missing or unsupported evidence is rejected. A registration module can publish imported-theorem companions under its own qualified prefix, for example `companionPrefix := some rootId`; downstream modules refer to those genuine declarations and compiler ownership stays with the registration module. Without an explicit public prefix, imported companions remain private to their compilation. An imported root contract does not select a downstream root, and duplicate declarations of a root contract are rejected.

The sidecar grammar and cross-owner binding overlay are retired. Registration bindings belong to their Reg occurrence owner. Delta selects changed Reg owners plus exact mirrors and explicitly imported variants for newly public D5 theorems; it does not scan other modules for claims. All four DTR findings remain Observe, and unselected malformed registration payloads remain outside the evidence domain. RawLeanReportArtifact's current-source-byte check, report release version 9, export reassessment and existing trace/reuse semantics are unchanged.
...
**A12 文献码** bibkey `<姓><年><首词>`;根桶 `Library/notes/<bibkey>.md` 或容量分裂后的受控疆域桶 `Library/<Domain>/<bibkey>.md` 之 canonical YAML front matter schema 为 `{bibkey,authors,year,title,doi:DOI|null,url?:HTTPS,claim,strata_touched:[GID],license,triage:anchor|task(GID)|rejected(理由)}`。`authors` 是非空规范署名串,`year` 是四位整数;文件名、`bibkey` 必须逐字一致,bibkey 在全 L 平面唯一,分裂桶名必须已入 `Meta/domains.yaml`;DOI 用统一 typed parser 验语法并按大小写无关键全 L 平面唯一;`task(GID)` 必须解析 canonical GID 并参与悬空检查。authors/year/title/DOI/URL 只在该 note 定义,Describe 与发射投影只携对应的 `D5/L/<bibkey>` 或 `D5/L/<Domain>/<bibkey>` typed 引用及 `lit/<bibkey>` 锚,禁内联复制文献元数据。`literature-attested` provenance 必携可解析 note 的 `LibraryNoteRef`,其学术发射至少携 DOI 或稳定 HTTPS URL 之一，二者可并存，并由 note 投影 author-year-title-source citation，并存时依次列 DOI 与 URL；无 DOI 时写 `doi: null` 加 `url`；凡给出的 `url` 均通过 `System.Uri` 验证绝对 HTTPS、无凭据、规范字节；被引用 note 的 locator 段须包含其声明的每个来源值，DOI 按大小写无关比较，URL 按字节比较；URL 的持久性由来源核实承担，离线语法检查不声称证明其长期可达性;悬空 L/GID 或 citation 元数据缺位即红;硬门全程离线,DOI 在线解析与标题一致性只列 Observe。
...
**A14.9 Inspector-owned report artifacts.** `lean-report-inputs.json` is the sole FILEMAP-registered report input manifest. Its strict JSON schema selects report modules, Inspector sources, configuration and the `lean-report` / `scribe-content` producer scopes through explicit include/exclude patterns. The optional `dependency_sources` path set registers additional local Lean sources that may occur in reported modules' native compiler dependencies; absence declares no additional local inputs. It is an allowed source inventory, not a producer compatibility fingerprint. Required registrations without files, malformed or duplicate fields, unsafe paths and unsupported schema identifiers fail with a named diagnostic. The required `report_cache_release_semantic_version` is a developer-controlled positive JSON integer, separate from `schema_version` and all report/wire schemas. Missing, duplicate, noninteger, boolean, zero or negative values fail before accepting artifacts. The current value is read only from that manifest. As an explicit developer maintenance requirement, compatible producer refactors retain this value, incompatible report semantics require a strictly upward bump, and the coordinate must never decrease or be reset. The implemented checks require a positive manifest integer and current-version equality for module evidence; they do not compare historical values. Declared-binding module evidence carries this same value in `compatibility_version`; neither judge source bytes nor a producer implementation digest defines evidence validity. Missing required evidence or a different module evidence version rejects; there is no older-version compatibility path. The reader `tools/scripts/report/lean-report-selection.py` expands only those registrations, with no MSBuild evaluation or script/call-graph discovery. The existing Scribe projection and describe checks consume the registered scope, including deleted paths.

For admission, these template-evidence requirements apply only to A5.5's selected registration owners and the producers of their selected binding claims.

The local dependency-free `tools/lean-inspector` Lake package owns per-module complete row/material bundles and their aggregate under root `.lake/build/lean-inspector`. Native module report facets trace the explicit cache release compatibility token, source bytes, private/server/import/transitive compiler artifacts, each module's authoritative utility record and external claim artifact closure; registered configuration identity belongs to aggregation. The Inspector executable must compile successfully through native Lake dependencies, but its job trace and registered producer implementation bytes are not report regeneration dependencies. Same-version compatible Lean/Python/C#/script or Inspector Lake configuration edits therefore reuse valid module and aggregate artifacts. A semantic version bump invalidates every required module report. Utility input is generated once per invocation. Current module membership determines aggregation, including removals. Native Lake trace and artifact reuse select misses. The Inspector asks Lake through its native `noBuild` artifact operation, preserves each module's input/output traces, and composes the selected work through a shared native job. Expected internal misses become continuations; other failures and a caller's actual `--no-build` propagate. Batching selected misses and exact validator rejections shares import work without a second invalidation engine or per-miss blocking promise wait. Ordinary root defaults and audit builds remain required. A validation batch may reuse already verified material identities within that invocation while reading, CRC-checking and hashing every actual material stream again; it retains no report cache or validation state across invocations. New aggregate artifacts still cross the complete canonical bundle validator.

Lean-cache publication invokes the existing current Inspector entry once before packing. Native `:report` supplies ordinary default compiler obligations and report work; required input, build or complete report validation failure prevents publication, including when a run snapshot already exists. The root buildDir archive carries native Inspector artifacts and the canonical report/material/origin/attestation outputs. Publication retains the resolved mathlib/platform partition and run/attempt snapshots; published draft state and transferred inventories remain validated. Legacy Lean-only and report-bearing v1 snapshots are seeds under the same partition rule: their declared config/source/optional snapshot addresses bind manifests to tags, never select candidate compatibility. Lake traces and `report_cache_release_semantic_version` decide report reuse after restore; validators check structure and artifact integrity only. Optional transport failures cannot turn a required build or report failure into success.

Inspector privately stages, canonically validates and publishes raw-v2 rows, all declaration materials including excluded helpers, ZIP digests, provenance-v2 and current-input attestation. A rejected optional artifact rebuilds privately with shared blob writes disabled; invalid required inputs and Lean failures remain failures. The existing coordinate/attestation wire fields `producer_sha256` and `repository_inspector_sha256` carry the semantic compatibility token `SHA256("schema=stratalint-lean-report-compatibility\nversion=<positive integer>\n")`; they are not executable-byte evidence. Report source coordinates select only `report_modules`; Inspector sources remain registered producer provenance and native compiler inputs. Configuration digests retain their registered byte preimage. Each native module bundle includes a strict origin record containing `module`, its singleton raw `report_sha256`, `compatibility_sha256`, `producer_sources_sha256` and `inspector_executable_sha256`, captured at generation. Origin sidecars contain no raw-source hash map. Lake's normal `transImports` supplies the dependency closure, constrained by `report_modules` and `dependency_sources`; unregistered local inputs fail explicitly during production. Fetched packages remain pinned by the registered toolchain/Lake configuration. Publication staging and the shell input verifier check origin integrity, module membership and source-path ownership. Producer-only files do not become input dependencies merely by being registered. The source fingerprint hashes the registered producer path/content digest inventory, including the canonical report-policy projection. Aggregate provenance-v2 retains these exact records in `module_origins`, keyed by module and checked against the canonical singleton row bytes and current compatibility. Mixed generation origins are valid; publication may update only invocation mode and never relabel a reused row as produced by the current executable. Legacy bundles without required origin evidence are rejected and rebuilt privately. The public shell input helper exposes `address`, `verify`, `modules`, `producer-paths`, `scribe-producer-paths` and canonical `coordinates`; it does not own artifact reuse. The thin pair interface invokes one candidate Inspector producer.

The optional `report_execution` manifest declaration names the supported tools (`lake`, `lean`), their required registered identity file (`toolchain: "lean-toolchain"`), platform coordinates (`system`, `machine`) and explicit semantic environment variables. Report reuse binds the authored pin and those coordinates; it does not execute compiler version commands or require installed compiler binaries. Missing, changed or excluded pin registration cannot yield reusable evidence. CI resolves the optional report seed before restoring or installing the toolchain. Selected program builds and report misses still require the normal toolchain; if publication rejects a seed after planning, the normal entry obtains the missing toolchain and builds, retaining build failures. The normal Inspector entry first seals a `.reuse.json` sidecar after a successful native `:report` invocation with the selected registered program targets and complete publication. A receipt hit privately validates and republishes the report while separately fulfilling the selected program-build obligations. It binds the report semantic version, the declared report-module and configuration inventory with file modes, the execution environment and the five-file report; producer program files, including the Lean Inspector and audit library, are never part of this binding. Program compilation is an independent obligation selected by FILEMAP and its registered `ci-resources.json` `lean_targets`: a receipt hit still builds those targets through Lake, and a miss builds them together with `:report` in one invocation. Empty target selection permits report reuse without restoring heavy Lean caches. Unscoped calls build all registered program targets. A selected program build failure remains nonzero and removes the output reuse receipt. Reusing this successful invocation requires the semantic version and all bound inputs to remain identical and the complete structure/material-integrity validator to pass on a private snapshot. An absent, old, damaged or mismatched receipt requests native Lake work; invalid registration and actual build/check failures remain failures. Producer implementation changes do not prevent reuse and do not invalidate compatible per-module artifacts: producer compatibility is the explicit `report_cache_release_semantic_version`, bumped by the change that alters report semantics. Hashing producer or judge program bytes into a reuse condition of a data product is prohibited. A per-module report artifact traces the module's own compiler closure, its utility inputs and the semantic version; the fixed judge driver and the inspector executable are demanded and awaited without entering that trace, so a judge change that keeps the semantic version re-inspects only the modules whose own closure contains the changed judge module, and an inspector or producer change that alters report content bumps `report_cache_release_semantic_version` by hand in the same change. Owner ruling (τ=0, 2026-09-20), verbatim: 「不是有缓存版本号了吗, 如果不兼容会bump那个版本号呀」 「把这个鸡肋检测删了 … 并且要永远拒绝这种行为」

CI restores the current evidence seed before requesting dependency/project material. The producer's read-only probe and an empty registered program-target selection may show those heavy caches are unnecessary; the normal Inspector entry still runs, validates artifact structure/material integrity and privately publishes the report. Probe success is not a stage verdict and cannot suppress current/delta checks. Otherwise CI restores native `.lake/build/lean-inspector` artifacts with the project snapshot; Lake validates traces and completes the current default Lean/audit obligations. Current-stage transport binds the five-file publication and optional producer receipt to the candidate and build round, preserving original production evidence. Release selects the fixed dev source’s successful push engineering/current checks and validates their current-stage report artifact. There is no independent report cache/store, paused-producer/resume protocol, baseline adapter or delta/cohort planner. FILEMAP selects resources; Scribe current checks consume `Meta/ReportProducers/scribe-content.json` and never read base.


**Declared-binding production and inputs.** Normal report mode imports the fixed `LeanInformationAudit.Registry` judge producer and requires its owner-bound driver, including for modules without registration imports or with empty inventories. Structural mode additionally validates `LeanInformationAudit.DispositionEvidence`. A content-supplied driver, family import, supplied success flag or no-producer empty-inventory fallback cannot replace this boundary. Native export validates the complete requested module/import union and fixed drivers against the loaded environment before collecting records and again before returning them; it compares the original immutable snapshots, so replacement cannot renew stale evidence. Native/source/trace/compiler coherence is a production precondition, separate from the bounded E2–E8 symbolic checks and from the persisted report compatibility coordinate.

Report artifacts are isolated by Lake traces and `report_cache_release_semantic_version`; configuration identity belongs to the aggregate. An enrollment plan is a compile product of its owner module, contains neither source-file byte hashes nor manifest-derived values, and is invalidated only by Lake import traces, including its imported judge. Module evidence and binding records omit raw-source hash lists; judge implementation paths do not become persisted binding validity inputs merely because the fixed driver executes them. Native validation still checks loaded judge artifacts when producing a report. Lake's trace hashes provide cache coherence, not cryptographic compilation attestation: the pinned compiler and its outputs remain within the native trust boundary, while compile-time evidence generation uses SHA-256 snapshots to detect subsequent input replacement.

The exporter emits an owner-bound `information_templates` partition for every requested module. Native row reuse and final publication validate complete current membership, evidence structure and material integrity; for a present `information_templates` partition, native validation checks its shape and manifest-version equality, while the C# consumer rejects missing evidence and malformed records within A5.5's selected registration-owner scope. `inspect.sh` delegates both unchanged and changed work to native Lake report facets and propagates phase failures. C# separately performs A5.4's complete occurrence/sidecar/ownership join for the selected owners and A5.5's delta judgment. Selected records check fields, canonical occurrence keys, ordering, source ownership and certificate consistency. Stored hashes are not compared with current snapshot bytes; Lake traces and the explicit cache release version govern report reuse. The explicit `--statements-only` fixture operation omits `information_templates` and cannot satisfy the declared-template consumer for selected owners.

Census keeps `information_template_bindings` separate from mathematical disposition rows. It discovers occurrence events and binding claims by their actual containing modules, imports all scoped evidence modules, obtains one authoritative joined snapshot and reconciles selected keys against the finite/structural registries. Missing discovery/sidecars, inventory mismatch or out-of-scope binding owners make the query incomplete; absence of an occurrence does not invent an undeclared record. Each query carries `{schema_version,compatibility_version,query_completed,diagnostic,records,source_inputs}` with schema 1 and its current census transport coordinate 5. That wrapper is not module report evidence and cannot substitute for the current manifest-version partition. Complete queries have a null query diagnostic; incomplete queries have a diagnostic and empty records/inputs. Only individually validated records carry certificates. Per-key cache reuse checks the wrapper, selected record fields, cache-input/query correspondence and hashes of listed source inputs; complete record and ownership validation belongs to the authoritative producer and strict module reader, and cache reuse does not attest source-to-native compilation.

**A15 提交与 PR 文法** `COMMIT := <官>"("<GID>"): "<动词短语>`;PR 模板 = 四段判词(立了什么/依赖什么/试了什么死了什么/账平声明勾选:无既有 closed 被推翻)。


**A17 Scribe:文档即代码(v7.14)** 叙事层(Blueprint/Papers,渐及 spec)的 canonical 源=C# 类型化文档 AST(`tools/StrataLint.Scribe`):文件头/章节/段落/公式(自建封闭 Formula AST→total `LatexWriter`,逐节点构造式确定性发射,不解析 LaTeX 文本)/GID 引用(经 Engine Gid 构造期解析,悬空即失败——取代已废的行号/片段哈希位置锚)。正式陈述统一为 `DocumentBlock.Describe`:文档内唯一的 typed local `DescribeId` 与 kind/statement/provenance 均构造期必填,kind 封闭为 `{definition,theorem,proposition,lemma,example,remark}`,statement 必为 `Formula` 或 `LeanDeclarationRef`,provenance 必为 `{literature-attested,repo-derived,suspected-novel,unassessed}`;旧 `Proposition/Theorem/ComputedValue/RenderedStatement` 类型一次迁移后从程序集删除,不得留兼容读者。学术发射以 `DocumentHeader.Digest` 生成 Abstract,Describe 按类型化 AST 深度优先序自动编号为 `1.n`,定理类生成 `Theorem/Proposition/Lemma` 陈述及显式 `Proof`(Lean declaration+axiom badge+终止符),原人类散文归 Commentary;`literature-attested` 以类型强制携 A12 的 L 引用并投影 author-year-title-source citation,其余三态投影规范 Source 且不携文献元数据。Markdown 与 QuestPDF 使用同一编号、陈述、证明、citation 和 commentary 数据流。红项=缺 kind/statement/provenance、公式字段非 Formula、TextRun 裸 `$`/`\\(`/`\\[` LaTeX 定界符、L/GID 悬空、citation 元数据缺位、DOI 语法或唯一性坏、producer 契约漂移及旧节点残留;Observe=纯文本/Unicode 疑似公式、代码跨度、Lean docstring 公式、DOI 在线解析与标题一致性,Observe 永不使离线硬门出网。`scribe describe-report [--json]` 离线读取预计算 Lean material 验 selector,以案号 `DESCRIBE-NODES` 发射机器账:逐节点 `GID#describe/DescribeId` 稳定 ID、kind/provenance 统计、`suspected_novel` Papers 候选清单及 `open_count=unassessed` 存量;人工/文献勘正只需改 typed provenance,报告自动消化 open。`DocumentHeader.Anchors` 为 `Anchor` 封闭联合类型,Lean 六行头仍以 `anchors: [string, ...]` 作序列化边界;canonical MD writer 由 first/second 双次序列化断言同进程确定性。跨版本 producer 契约只对测试自有的固定合成 Formula/Document 语料作长度分帧聚合 SHA-256,该指纹路径不发现或读取仓库 Scribe 数据;独立覆盖证明读取真实 Scribe 语料,并要求其 Describe `(kind,provenance,statement)` 元组及 Formula 父/子渲染上下文均为固定合成语料组合的子集,缺项判词点名具体组合。两者均不读取 tracked Markdown;`Blueprint/**/*.md` 是 FILEMAP `kind=generated` 的可刷新人读投影,`emit --check` 只作作者 freshness 便利,不进入 CI、admission 或 producer 契约。PDF 经 QuestPDF(钉版,社区条款,许可年检入账)。依赖闸门在 Lean 与 NuGet 两侧共用同一许可/证据原则:Markdig/CSharpMath/MathNet.Symbolics/AngouriMath 未准入(证据闸门),iText7 永拒(AGPL)。现役 `Anchor` 封闭联合仅有两型:文献 `lit/<bibkey>` 与外部 Lean 模块族 `mathlib/{module,decl}/<Lean.Name>`;二者均由 typed parser 强制 ASCII、ordinal 与严格 round-trip。`gict`/`pzg` 的理论卷、章节与编号只作 authoring provenance 注记,不是 Anchor scheme,机器不解析。`spec/<payload>` 在 `Anchor` parser、`AnchorScheme`、`Anchors/` 实现、D5 Lean 头及 Scribe 实例中均无现役定义或实例;其历史意图无法由仓内事实恢复,故记 `open(SPEC-ANCHOR-CLASSIFICATION)`,且在该 open 闭合前不得作为 Anchor 输入。现役 Lean 头成员资格进一步收窄为可由 import 图判定的 `mathlib/module/<Lean.Module>`;`lit` 与 `mathlib/decl` 虽是现役 typed Anchor 形,但 SL-017 对 Lean 头 fail-closed 拒收,其各自现役消费边界见 11.23。外部锚**不设注册表、包目录或 catalog**:依赖权威是 Lean import 图,版本权威是 `lake-manifest.json`,直接依赖声明在 `lakefile.toml`。手维护的 `anchor-catalog.v1.json` 及其 typed manifest 已退役:历史证据精确为 0.51% 覆盖率、198 个被 import 的 mathlib 模块、登记 1 个。任意 Lake-resolved package anchor 泛化为 `open`;仅由独立 harness change 解锁,该 change 必须实现 manifest/lakefile-derived generic external package anchor type/parser、package-to-module resolution、import-closure validation 与 red/green tests。第三方 Lean 依赖现役可机器验证的事实仅为:在 pinned toolchain/mathlib 下可 resolution、现行 build 通过、axiom/sorry closure 不扩张;现行 build 可复用 cache,不等同于 A14 所要求的 clean build 谓词。public source、immutable rev、license compatibility 均为 `open`;其机器谓词落地前不能驱动 automatic admission,且永不委派给人。
...
**A16 零信任合并门(v7.12,CLAUDE.md 第 19 条之 spec 形)** 提交者身份(维护者/agent/fork)与准入无关,一切 PR 过同一道纯机器门。CI/preflight 的目标检查集、current/delta 分工、候选与 base 身份、最终 required check 名及迁移证据统一由 A22 定义;本次规范修订不表示新 workflow 或 required set 已上线。门脚本、判官与 PR workflow 取自候选合并树,使用只读 `pull_request`,不再保证 workflow 文本来自 base 侧;base 仅以数据参与候选判官的跨树检查。候选可削弱自己判官的残余仍在;SL-029 仅报混面警告,SL-022 保护面标注、dev push 检测与独立评审继续适用,不冒领为准入时点的独立保证。绿=auto-merge;红=按纪律不得绕过(`enforce_admins=false` 当前不冒领机器锁)。`strict` 已按 2026-08-10 τ=0 裁决禁用,保持 `strict=false`。**exit 语义**:0=检查通过;1=违规;2=输入/基础设施失败(含快照拒非常规 git 条目);3=SL-022 保护面变更标注。归一与摘要由 A22 共享入口拥有,标注不豁免其余检查,也不要求重跑已成功的 current/Lean。人审与 AI 审=质量增益,非准入权威;削弱门=元层自改,须付 τ=0 成本(CLAUDE.md 第 21 条)。**base 判官永久禁止(v7.21,τ=0 owner 2026-09-03 裁决)**:凡为判决候选而 checkout / restore / 编译 / 执行受保护 base(或任何非受审 HEAD 修订)之**代码**的机制——base worktree、base ref checkout、`git show <base>:<脚本>` 交给解释器、指向非候选内容寻址构建的 `--judge-dll`、「base 拥有的 derive / verify / classify / floor」步骤——一律禁止;base 只以**数据**(SHA、字节快照)参与判词。候选可削弱自己判官的残余逃逸由 owner 明示接受,现役 SL-029 按下款端点分树分类并报非阻断警告,允许判官与内容以一个完整 PR 交付;SL-022 保护面标注、dev push 检测与独立评审保留,不强制隔离同一 PR 的判官/内容修改。定义、案由(#4380 隔离未交付;2026-08-13 两次全仓死锁;裁决落地前后三次复辟:#5210→#5319 与 #5285 建于落地裁决的同一 PR 内,`7ffc6b054a` 所移除的 pure-revert 分类器立于裁决前两日)与守护分栏见 CLAUDE.md 第 19 条。**机器投影(v7.22)**:准入规则 **SL-030「Judge surface reads no other revision」**(Block,trust,delta-scoped)——对候选 delta 内的判官面文件(`.github/**`、`tools/scripts/workflow/**`)逐行扫描能物化修订文件的 git 动词(`show <rev>:<path>`、`cat-file` 内容模式、`archive <rev>`、`worktree add … <commit-ish>`、`checkout <rev>`、`restore --source`、`read-tree`、`checkout-index`),修订不是字面 `HEAD`(精确允许集 `HEAD`、`HEAD:<path>`、`HEAD^{tree}`、`HEAD^{commit}`)即拒,修订为变量、未知选项、缺操作数一律 fail-closed;workflow 中 `ref:` 指向 `base_ref` / `pull_request.base` 亦拒;参数在首个 shell 重定向或 `#` 处截断;`cat-file -e/-t/-s` 一类元数据模式不判。**SL-032「Scribe narrative provenance」**(Block,repository,delta-scoped;2026-09-07,#6134/#6156)——对候选 `Changes` 内的 `Blueprint/**/*.scribe.cs`(不用 `IsBaseFactAffected`,判官实现变更时存量仍不判;同一 changeset 内一删一加且 RawBytes 相等的搬迁不判)逐文件扫描:①`CjkSourceCharacter`——整文件 Unicode 标量(含注释与标识符)落在钉死的 CJK 块并集(U+2E80–2EFF、2F00–2FDF、3000–303F、31C0–31EF、3200–33FF、3400–4DBF、4E00–9FFF、F900–FAFF、FE10–FE1F、FE30–FE4F、FE50–FE6F、FF00–FFEF、20000–2FA1F、30000–323AF)即拒;②三个词汇类只对文本载体(Roslyn 注释 trivia 与字符串字面量,含转义解码与相邻 `+` 拼接)判、不判标识符:`DigestionLedgerReference`(账本记号 `Meta/Digestion|atom_id|coverage_gids|chain_atoms|cas_ref|residual-open|partial-closed|absorbed-closed|nonpropositional-inapplicable|accepted-event|subitems?|digestion|backfill`、atom id 形 `atom (sha256:)?<40–64 hex>`/`*-residual-<hex>`、AST 定位 `theorem/1.2` 形,以及只锚文档专属名词与固定习语的 atom 关系文法:`… atom asserts`、`… atom carries a|the|no (pre-committed) receipt|numerical certificate|clause|claim`、`closes|closing|discharges|formalizes|digests + 限定词 + ≤3 个文档修饰词 + atom`、`(closes|covers) only the … clause|subitem|claim|statement of the … atom`、`multi-clause (corollary) atom`、`atom's proof skeleton|claims|clauses|subitems|statements|registration|…`、`stated|asserted (elsewhere) in that atom`、`registration statements in that atom`、`clauses|subitems in|of that|the source atom`——**不含任何有数学读法的动词或名词**(states/claims/reports/closed/closure/covers/carries/absorbs/families/ends 均不作关系证据,召回缺口逐句登记为残余)、`TheoryVolumeReference`(`docs/develop/theory`、`theory volume`、`PZG|GICT|DECT|BEDC|FPOD|QDO|CONE` + 引用语法或 `-vN`;裸缩写与标识符放行)、`GovernanceProcessReference`(`issue #?NNN`、`PR #N`、`pull request`、`panel|dispatch brief`、`orchestrator`、`search`∧`duplicate` 同句、`six-route`、`proof_shape`、`admission_basis`、`escape witness`、`bind-only`、`postmortem`、`git-history`)。全部模式 `RegexOptions.NonBacktracking`。判据是词法类,不证明散文由 Lean 蕴涵;精确率优先(Block 门不得咬数学句),四轮 sshx 评审构造的 60+ 句数学句全部作为放行夹具钉住。它是早反馈而非「不存在 base 判官」的证明——反例集合(面外脚本、`bash -c`/`eval` 拼接、`curl` 取脚本、C# 进程内读者、判官被同一 PR 改弱)写在 CLAUDE.md 第 19 条守护段;base 数据由候选判官的快照读者消费;报告与缓存的目标契约统一见 A22,旧 workflow 文本缓存地址不再作为兼容选择器。

**SL-029 混面警告与删除路径分类(τ=0 owner 2026-09-24 裁决)**:合法混面 changed set 恰报一条 `ADMISSION-PLANE-MIXED`,为 `DisplaySeverity.Warning` + `AdmissionEffect.Observe`,随后继续全部普通验证。完整 `check` 与 `check-delta` 均在普通结果中保留该警告;不得提前返回成功、以仅含 Observe 的结果拒绝或吞掉其它 Block。单面不报该警告。混面不削减适用的完整 engineering 义务,生产 CI 的测试/检查选择仍由现役资源计划承担。SL-029 不再强制分面,不能防止同一 PR 改弱判官并修改内容;此残余不由警告消除。缺失、畸形、歧义、不安全的 FILEMAP、错误的策略文件分类及无效删除/base 来源仍作基础设施失败。变更路径只有在候选原始快照中缺席、且在不可变 protected-base 原始快照中在场时,才按 protected-base 的 FILEMAP 分类;候选中找不到登记不构成删除证据。每个删除路径必须在 base FILEMAP 中恰有一条有效登记,base FILEMAP 缺失、无法解析、登记缺失或多重匹配均 fail-closed。非空 delta 的候选 FILEMAP 仍须通过准入分类元数据校验,即使所有变更路径均已删除;未涉及实际删除时不要求 base FILEMAP。存续、新增与修改路径保留既有候选 FILEMAP 分类语义。重命名的删除端与新增端分别参与分类,复制源不作为变更端。删除判官文件可同时移除其字面登记;删除内容文件与判官变更同交仍报 `ADMISSION-PLANE-MIXED` 警告并继续验证。候选准入使用这一分类器,base 仅作为字节数据读取。engineering 与 current 不读取 base;基线登记测试项目须在 candidate 继续登记为 CI 成员;本次资源计划选中的项目由 delta 消费本轮已接受 TRX 核对,不恢复按 base 选测。
...
**A18 FILEMAP 文件与 Evidence 政策（schema 5）** `Meta/FILEMAP.toml` 是唯一 manifest 权威。`files` 条目是唯一的路径成员源，声明五类 `{truth,program,data,generated,ledger}`、`admission_plane`、producer/consumer/verifier、runtime disposition 与适用的生成物、symlink 字段；全体 tracked/unignored 文件必须恰命中一条 pattern，根文件与 agent 章程使用字面条目，reports 仍要求字面登记。成员资格之后必须通过 canonical path/GID/domain 检查，glob 命中本身不能放行错误坐标。`evidence.artifact_kinds.<kind>` 严格声明 `{profile,selectors,path_selectors}`；七种已登记格式（含当前无实例的保留格式）只在此声明，缺表、未知 profile、非法 scope、重复或大小写冲突均 fail-closed。`Meta/domains.yaml` 仍为独立的 strict 词汇真源。`digestion_source = true` 在对应 files 条目上承担非理论源与理论源族的摄入资格，无平行文档清单。Engine 提供纯模型、strict loader 和确定性 TOML writer，CLI 取字节并 join domains，Scribe 只作投影。canonical snapshot schema 2 以 `filemap_sha256` 绑定规范政策，changed FILEMAP/domain/structured Evidence 的字节与重编码不动点在写入门检验；无关 delta 不重放已入库的非规范历史字节。历史 admission-plane 与 symlink reader 只取同快照自身 FILEMAP，不把旧快照强制送入当前写 schema。FILEMAP conformance 保留 actor/verifier、generated inventory、目录纯度、路径引用与 run-local 守卫，缺失政策无默认、无旧源回落。

The exact rows partition remains registered when empty, both before seeding and after discharge. This population exception requires all the above fields and a tracked, separately registered judge-plane activation authority; it grants no exception to other empty globs or to activation/seed/residual/deletion checks. The store and rule verifier names resolve through the FILEMAP implementation registry to production program paths. Missing or dangling implementations retain the existing `FILEMAP-DATA-VERIFIER` / `FILEMAP-DATA-VERIFIER-DANGLING` failures.
...
**A20 Lane hygiene 与 gate 承载力(v7.13)** 新建 lane 分支的 creation grammar 由 `WorktreeCommand` 唯一规定为 `<creation-namespace>/<kind>/<task-code>`,其中当前 creation namespace 只取 `WorktreeCommand.CreationNamespace`;精确 kind 词表只在同一 C# 所有者中定义;该 grammar 只约束产出侧。新建分类与生命周期 ownership 必须正交:前者严格分类新 lane,后者须持续管理 `WorktreeCommand.LifecycleNamespaces` 中每个 namespace 的任何非空子路径,不得因 creation 词表变化而缩域。historical `harness/*` 仅属 lifecycle ownership(可回收),不是可创建 alias。本次文法收口是前向修复,只作用后续新建,不清理任何存量分支。`WorktreeCommand.IsManagedBranch` 对上述任何非空 lifecycle 子路径为真,不要求三段。`make -C tools clean-lanes` previews cleanup as JSONL; `FORCE=1` applies it. Registered linked worktrees, including detached HEAD and branches outside the managed namespaces, are reclaimable when their last Git update is at least 24 hours old and `git rev-list --count HEAD..BASE` is at least 300. Uncommitted changes, untracked files, merge/PR state, never-worked status and process occupancy do not prevent removal; status, GitHub and lsof are not queried. The last update is the maximum timestamp across the worktree's own `logs/HEAD` and the HEAD commit's committer timestamp; source file mtimes are irrelevant. The reflog must be nonempty, parseable and end at the observed HEAD. Future timestamps, invalid counts and failed reads retain the worktree. The main checkout, current worktree, locked worktrees and unreadable paths are retained. Children are evaluated before parents; a retained registered descendant prevents recursive deletion of its parent. Immediately before removal the command rechecks HEAD, branch, lock, update time and behind count, then runs `git worktree remove --force -- <path>`. Managed local branches are deleted using `git update-ref -d <ref> <observed-tip>`; other local branches and all remote refs are retained. Worktree removal failure emits `action=partially_removed,reason=worktree_remove_failed_state_indeterminate`; ref deletion failure emits `action=partially_removed,reason=branch_ref_retained`. Both contribute to `partial_count` and exit 2; the sweep continues. Managed orphan branches still require ancestry in BASE. Registered temporary worktrees use the same inactivity policy and cannot bypass it through the temporary-directory sweep. Unregistered temporary snapshots retain the same-repository pointer or complete gitless-project-marker requirements; report directories, symlinks and foreign repositories are retained.


**A20.1 点名删除已注册 worktree** 根入口为 `make worktree-remove NAMES="trureturing-foo trureturing-bar"`,唯一程序实现为 `StrataLint worktree remove --names "trureturing-foo trureturing-bar"`。单个与多个名称均用 `NAMES` / `--names`,没有 `NAME` 别名。名称是 `git worktree list --porcelain -z` 中注册路径的完整末级目录名,按 Ordinal 精确匹配;不剥前缀、不读创建时 NAME、不匹配分支,也不按模糊、后缀或完整路径回退。空白分隔名称,含空白的目录名不在本参数文法内;重复名称只处理一次。Make 以 `unexport NAMES` 阻止命令行变量被隐式导出时展开,再由目标专属 `export WORKTREE_REMOVE_NAMES := $(value NAMES)` 捕获原始字节;配方仅引用环境变量,不把 NAMES 字节展开为 make/shell 命令。

一次读取完整清单,复用 `RegisteredWorktreeInventory.ReadWorktrees(..., resolveGitDirectories: false)`,不逐树探测 gitdir。先对所有名称解析:匹配数为 0 → `not_found`;大于 1 → `ambiguous`;唯一匹配是清单首项(主检出) → `main_worktree`;唯一匹配含 Locked → `locked`。主检出参与匹配计数,不得先排除再消歧。清单不可读或为空一律拒绝。任一名称拒绝则整批零删除,按输入顺序返回首个失败的分类码,并逐项输出所有失败;其余已解析对象输出 `batch_refused`。locked 必须先显式 `git worktree unlock <path>`,本命令不解锁。
...
CI/preflight 的阶段、候选报告/DLL/工程证据交接、退出与缓存统一由 A22 定义。原整报告精确输入地址命中短路、失配即完整生产、共享检查隐式取 `HEAD^1`、固定 `check`→`filemap-conform` 顺序及旧 gate 接口退役配方由该契约替代;报告在所有命中后仍进入增量 producer,base 只作 delta 的数据。接口与操作说明随对应实现层同步,本规范层不声称旧入口已退役。
...
消化 atoms/backfill 的路径显式登记 `current-metadata`、`delta-metadata` 与 `filemap`。push 只检查 SL-003/015/019 与 filemap;PR 的 `delta-metadata` 另要求正常 Lean report,供 SL-016 等现役 delta 谓词消费;仅有消化元数据不要求 engineering,不附带其它 current 谓词或 Scribe。delta 谓词集合保持不变;混入 Lean、冻结或判官等登记路径时按资源并集恢复相应完整义务,不以 metadata 标签豁免其它路径。

filemap 的检查范围由本轮 build 绑定的完整变更路径与 `Meta/ci-checks.json` 中 `filemap.delta_scope` 决定。普通路径只检查候选中现存的变更文件正文;删除端保留作登记人口与产物存在性检查。关联范围仅按 `related` 的显式输入/目标 glob 与 `trigger` 展开: `changed` 接受所有变更端点,`removed` 仅接受完整变更清单中已不在候选路径索引的删除/重命名旧端点;缺失或未知 trigger 失败。登记为 `removed` 的产物正文修改或新增不扫描未变消费者;删除仍扫描登记的未变消费者和尚未出现的登记产物,不从正文发现影响范围。`whole_tree_inputs` 明列需要整体复验的政策/判官输入,`actor_inputs` 明列需要核对声明名称的输入,`inventory_inputs` 明列需要核对完整 producer 产物目录的输入;产物目录复验不扩大正文读取范围。路径索引可完整读取,不由此扩大正文检查;无范围的显式完整入口仍检查全树。current 与 build 的计划必须相同,包括有无计划;执行前、封存与下游接受均校验。局部检查证据同时绑定完整路径范围、登记与实际选中的候选正文材料,不得复用成另一范围或全量的成功。scope 缺失/损坏须失败,不得静默退回全量或免跑;CI 与 preflight 使用同一选择与检查入口,current 判官不读取 base。
...
engineering 与 Scribe 不按 base 选测。当前项目与检查义务由候选登记确定;登记输入、编译依赖、材料、mode、执行环境及原成功证据验证一致时,允许增量复用原执行结果,并在本轮候选身份下接受。输入改变、证据损坏或无有效证据时执行本次资源计划选中的登记项目/检查;被选中的 selftest 与两项反证仍须结算实际执行和诊断义务。工程执行器、程序集运输和 TRX 校验消费同一已验证计划,不能将全体 `ci=true` 项目替代选中集合;未选中项目不得伪造成功执行。delta 将 base 的测试项目集合仅作为数据读取,所有基线 CI 项目须在 candidate 保留合法 CI 登记及既有拓扑约束;**本次选中的项目**必须具有本轮候选已接受的成功覆盖。没有 engineering 资源时,delta 不读取或要求其工件;有资源时缺少任何选中项目的成功 TRX 仍阻断。没有显式计划的完整 engineering/preflight 入口仍执行全体 CI 项目。新执行的 TRX 或经登记输入与完整性验证后接受的原成功 TRX 均须保留原执行来源及本轮候选绑定;裸历史 TRX、另一候选的绿、项目被删、漏跑或失败不能补证。delta 只验证证据,不重跑测试或引入 base 判官。base 尚无 manifest 时,候选 current/historical 显式登记寻址历史项目字节,不运行旧发现器。

Scribe Markdown 的范围由同一完整计划与 `scribe-markdown.markdown_scope` 登记决定：`changed_inputs` 选择变更端点，保留删除和重命名端点，由现役 Scribe 判官映射 `.scribe.cs` 到对应 `.md`；`whole_tree_inputs` 命中时以及显式全量诊断时，检查 `path_inventory` 的完整登记范围。不得因选中 Scribe 资源就展开全部 Markdown。增量计划缺少范围登记须失败，成功证据绑定实际范围及全量/局部身份，不能复用于另一范围；输入、报告和材料完整性验证保持。
...
**测试分析退役。** 测试归属、编译输入及影响范围按 FILEMAP 或显式 manifest 登记,不以 Roslyn 调用图、仓库路径或 IO 效果推导补全。Scribe 方法映射从未承担当前测试调度权威;其唯一规则消费者 SL-003 的 parser-unknown 债务分支及 280/281 容量、身份收缩条款退役。SL-003 直接执行原有行数和目录容量谓词,其 no-growth、作用域与容量棘轮不变,测试项目拓扑及本次选中项目的执行证据约束保留。仅为映射推导服务的编译元数据导出、运输和加载一并退役;候选 DLL/二进制材料封印、轮次和候选身份、locked restore/build、真实 Scribe 内容编译、工程测试、selftest、两项反证编译和 TRX 校验继续执行。登记缺口由后续真实 CI 暴露后补最小登记与原生行为测试,不重建分析器。

共享 build 的预期项目集合为已验证资源计划的项目根及其显式登记引用闭包,排除 `compile-fail-proof`;无资源计划的完整入口保留完整工程构建。执行计划分别绑定 `projects` 构建根与 `tests` 执行集合:后者只取资源直接列出的 `ci=true` 项目,编译引用中的测试程序集不自动取得执行义务。检查、测试项目及 current 步骤必须登记在对应执行阶段,错配在规划时失败。仅选 selftest/反证的工程轮次不启动测试运行器,不生成当前测试成功证据。逐项读取 `build/ci/build-outputs/<仓根相对项目路径>.outputs`,缺失即失败;收据声明的项目路径和 PE 程序集身份必须匹配登记。邻近未选中收据不参与运输。编译器 FileWrites、TargetPath 与 ref 输出只验证已声明项目的产物。收据第四行 `packages=$(NuGetPackageRoot)` 提供同一绝对包根;`Meta/package-materials.json` 只展开一次,不以 assets.json 的包或文件清单发现运输材料。

部分工程执行后保存种子时保留未选中项目的完整、成功 TRX 及原执行来源;损坏行独立丢弃,不连带丢失其它有效项目。该保留只提供未来执行的可选起点,不代表本轮已接受未选中的项目;未来选中时仍核对其登记输入、程序集、运行环境与材料。仅 guard 的轮次可运输这类种子,但不得冒领当前测试执行。缓存缺失、损坏或保存失败不影响已经成立的检查结论,实际选中测试缺失或失败仍阻断。

工程测试执行器对已选中且未接受缓存证据的项目最多同时运行两个完整测试进程。准备与种子校验先完成,各项目使用独立 TRX 目录;等待全部进程结束后按登记路径顺序封存证据,再统一验证本轮输入和结果。失败项目不重试、不回退到旧证据,其余已选中项目仍完整执行;并发只改变调度,不改变白名单或项目内用例集合。
该上限是临时 `policy-override`,不是容量派生值:2026-09-20,域为工程测试项目进程,案号 #8989,owner 为 CI harness 维护者。正向读数为并发进入、成功/非零/异常全部 join 的三项行为回归通过;反向成本读数为集成 push 35498282221 串行测试1596.992秒后在产物传输阶段触及30分钟job限额。尚无各测试项目固定CPU/内存强限或隔离峰值材料,不声称2是资源安全上限或最优并发度。最终CI交付前及runner/测试任务画像变化时复审该override;取得具名容量材料后替换为登记的派生配置或撤销,不得以即时负载调节。

delta 消费资源计划时,build 与 current 必须绑定同一份 canonical PR 计划,其 candidate 和 protected base 必须对应本次检查;父入口和候选判官分别验证。相同候选的 push/current 范围不能替代 PR 范围。无计划的显式完整入口仍保留全部义务。测试源码、项目配置及反证夹具按 FILEMAP 同时选择目标项目和已知的架构扫描/覆盖消费者;不会由编译引用动态扩张测试执行集合。
...
切换时原子迁移 watcher、`AdmissionTopology`、FILEMAP 与报告消费者，删除旧 job 名、旧拓扑及重复逻辑。临时 PR wrapper 的触发在正式入口安装时一并移除；同一候选不产生两套同名 required checks。上游独立 truth release 发布 workflow 及其 Mac 验证作业已退役；下游 Pages 只消费上游成功 CI 的报告工件并在 GitHub-hosted Linux 发布，不依赖独立 truth release。报告消费须核对来源 commit、成功的 engineering/current、run/attempt 与完整材料绑定，不得追逐移动 dev tip、等待 push delta 或借用另一 commit 报告，不在消费者重产报告。可复用的 truth/export、来源验证和 bundle 验证工具及测试保留；定时 Release 缓存发布不变。报告 producer 的入口、项目、引用、脚本、材料与语义配置输入由 FILEMAP/登记 manifest **显式声明**,随接口同步维护;禁止从程序调用闭包、调用图或 workflow 文本切片动态推导,不得保留发现回退。
...
**集成发布验证。** `ci-publication-verify.yml` 仅由 `integration-ci-*-tests` 原生 push 的发布程序、配置和 workflow 路径触发,独立于 required CI;普通内容与 no-resource 文档不增加发布作业。路径过滤只选择该验证事件,不代替 required CI 的完整 FILEMAP 规划。cache 作业使用生产对应 runner 和时限,仅为隔离测试发布申请 contents-write,以 actions-read 查询真实运行身份。该 workflow 只保留 Linux cache 作业；Mac truth 作业与独立 truth publisher 已删除。上述显式来源验证入口仍可复用，删除作业不代表已运行该验证。

缓存验证用既有 `make lean-cache-to-github-without-mathlib` 的 `LEAN_CACHE_MODE=verification LEAN_CACHE_SOURCE_REF=<ref> LEAN_CACHE_SOURCE_COMMIT=<sha>` 参数,只接受真实 integration push 与干净 HEAD,核对 API 的仓库、受保护来源、run/attempt/ref/SHA。它复用生产 build、归档、manifest、分片、上传及恢复实现,标签固定为独立的 `lean-cache-verify-v1-<partition>-<run>-<attempt>`;上传后必须向空的私有目标完整下载、核验并安装,才能报告 published。对应 fetch 参数仅消费本轮精确标签,不受 Actions 命中短路,不替换已有 warm 目标。验证不覆盖生产标签、不执行生产 prune,生产 fallback 不选择验证标签。它证明集成事件下的缓存发布运输与来源消费，不冒领 schedule/dev、生产 prune 或 truth assembly 已验证；缓存的 schedule/dev 与生产 prune 仍须在相应真实事件核验。
...
Actions 仅恢复所选候选资源声明的层；需要原生报告时，由 `lean` 前置声明 dependency/project/elan，`.lake/build/lean-inspector` 随 project 一起运输，不再存在独立 report 层、prepare/resume 协议或晚启动缓存阶段。stage-start 先恢复 current 证据；A14.9 的报告 probe 命中且所选 `lean_targets` 为空时不下载 dependency/project；报告未命中或目标非空时按原登记恢复。filemap 等无 Lean 资源的路径仍不物化 Lean 缓存。

需要 Lean/report 的路由在命中后仍进入共同增量入口。`make lean-report` 的正常 producer 入口按 A14.9 校验报告复用证据；命中时仍对 FILEMAP 选中的 `lean_targets` 执行 Lake 增量构建，未命中时在一次 `lake build :report <targets>` 中履行报告与程序构建义务。无 scope 的直接入口执行全部登记程序目标；实际 Lean 重编由 Lake traces 决定。Lake traces 跟踪模块源码、utility claim 和传递依赖；复用验证器检查结构与工件完整性；允许恢复的输入范围由显式清单限定，不动态生成 CI 归属。producer 兼容性只取 `lean-report-inputs.json` 的显式 `report_cache_release_semantic_version`，代码字节本身不改变语义版本，复用行保留实际来源。登记配置文件字节和实际模块环境参与原生失效，不参与远端分区。相同 mathlib 的源码变更只做原生依赖要求的工作；不影响已登记报告/编译输入的 metadata 可零模块重编/重检。配置文件字节变化不冒称零报告重检。同环境增量结果须等于规范完整生产，比较时分别核对实际来源字段。

下载前的 report probe 只核对登记报告输入、环境、receipt 与材料哈希；资源规划在 probe 未命中或所选 `lean_targets` 非空时请求 Lake 缓存;它不接受报告或签发检查成功。正常 reuse 对私有快照执行完整 publication 校验,失败即进入实际生产;probe 后材料变化、哈希自洽的坏报告与构建失败均不得假绿。完整语义校验不在 probe 重复执行。
...
| 混分区、非法首次冻结、棘轮违规、base CI 测试项目登记丢失或选中项目无候选接受的成功覆盖 | delta 阻断,合法例放行,delta-only 存量作用域不变。 |
...
每个真实 run 核对实际事件、入口及 reusable workflow 的路径/不可变修订、候选 M/H、PR base B、checks 和 artifact 身份;正式 `pull_request` 的 workflow 随候选合并修订取源,不提供 base 侧 workflow 文本保证,旧入口的绿不算新入口版本证据。可达替代事件只证明其实际版本/权限边界,原生事件覆盖缺口不得用条数抵销。性能以可比载荷、runner、工具链、缓存条件下的总耗时、关键路径、阶段耗时与实际重编/重检量判断,本机读数不外推为 CI 性能。交付前核对原始 lane 最终净改动与 integration 已验证候选的路径、mode、内容一致性;正式交付经 lane→dev 普通 PR,不合入 integration 镜像历史。合入后观察首个 dev push 与适用 PR 新入口,按真实名称更新 required set 并复核 `strict=false`。只保留必要测试程序、结果数据、现行结论与验证缺口,记录形式服从 CLAUDE.md §2.10;实现存在、本地测试通过或单轮绿均不代表完整目标完成。
...
整个系统遵守 `CLAUDE.md` 第 2.10 条，只留结果，不留过程。`docs/reports/` 只保留有明确用途的实验程序、结果数据及正式源码引用的第三方许可证。正式成果归入对应的代码、理论正文或文献条目；实施日记、构建日志、席位回执、转录和重复快照直接删除，不搬到其他目录，也不在正文保留历史链接。
...
**居所恰为 `Problems/<slug>.md`，不住 `Library/`。** 三处现役机器同向裁决：`LibraryNoteCatalog` 枚举 `Library/` 下**每个**桶的每个 `.md` 并要求其为A12 闭合元数据的文献笔记（`Library/notes/` 只是众桶之一，`Analytic`/`Arith`/…/`Words` 同为笔记桶），故问题档案入任一 `Library/` 桶必被该 loader 判红；`Meta/FILEMAP.toml` 的 `Library/*/*.md` 已覆盖该形状，再加一条即 `FILEMAP-AMBIGUOUS`；`RepositoryPathPolicy` 的 `Library/([A-Z][A-Za-z0-9]*)/` 不接受小写桶名。故另立顶层地址。`Problems/` 不进 SL-022 保护面、不产 GID、不入真值 DAG——它是**输入**，不是被冻结的真。
...
**封闭 front matter 的必需键为五个**：`slug`（等于文件名且合 canonical slug）、`bibkey`（canonical bibkey）、`doi`（完整 DOI 或 null）、`triage`（`theorem|window|wall`）、`motivation_gids`（非空且无重复的 GID 列表）。仅允许一个可选键 `url`：给出时必须是规范绝对 HTTPS URL，使用 A12 同一 parser，禁止凭据；`doi: null` 时必须给出。DOI 与 URL 至少其一，二者可并存；未知键、缺键、非规范值、BOM、CR、非 strict UTF-8 均拒绝。DOI 继续由 `Doi.TryCreate` 判语法，裸 arXiv id 和 DOI URL 前缀仍不作为 DOI 接受；`arxiv_id` 旧键仍拒绝。2026-09-08 的 OEIS 实施请求需要引用明确发表但没有 DOI 的陈述，故将此前 DOI 唯一路径扩展为 DOI 或稳定 HTTPS URL，不把 URL 伪装成 DOI。2026-09-06 至 2026-09-07 的 DOI 迁移及 J2b 删除旧 arXiv 键的历史不变。
...
**引用机械可判（第Ⅵ节）。** 每条 `motivation_gids` 必须在树上解析，悬空即 `dangling-problem-gid`；`bibkey` 必须解析到现役 `Library/**` 笔记，否则 `dangling-problem-bibkey`。笔记持有来源身份，候选给出的每个 DOI 与 URL 都必须与笔记的同种来源值逐字节相等（`StringComparison.Ordinal`）；笔记缺该种来源或值不匹配均为 `problem-source-mismatch`；笔记多出的来源不要求候选重复。这些检查仍由 `DescribeRepositoryValidator` 执行，不新增门。DOI/URL 语法检查不证明链接的实际指向或开放状态；正文来源核实与独立评审承担这些事实，硬门不出网。

**动机链的成员资格强度已收口（原第三条 open 已闭合，2026-09-04 `fa917c80ef`）。** `ValidateProblemMotivationGid` 现役判词为三项合取：①该 GID 必须选中 Formal 平面（`IsFormalDeclaration` 或 `IsFormalModule`），否则红；②其 host selector 必须是 `Golden/Frozen/state/` 状态表成员（`FrozenStateCatalog.Records.ContainsKey(reference.Path)`，catalog 由 `Golden/Frozen/state/**/*.json` 经 `SnapshotDecoder` 装载），否则红；③在此之后才走既有 `ValidateGid`（正名存在，有 Lean 报告时并解析该声明）。故 §11.20.2 对 Theorist motivation 所要求的 **active Frozen 成员资格现已由机器执法**，v7.16 R15 记的「第三条 open」与「Scribe 今日无 frozen-ledger 依赖」两句**已被实现推翻，此处撤回**；下文 v7.16 R15 与 #3686 勘注保留为审计记录，其结论不再描述现状。**强度边界仍如实分栏**：状态片存在即成员，判词到此为止；它不判该模块的证明是否忠实于该论文的命题，也不判该问题在世界上是否仍未解决（后者是闭世界，见 §11.20.5）。
...
### 11.20.5 THEORY-GENERATION-P-RESOLVED（外部开放问题的解决绑定）

§11.20.3 定的是**池**（哪些外部问题在案），本节定的是**边**（本仓的哪条定理声称解决了池中的哪一个）。案由是一次实际发生的事：仓内已有三个外部已发表开放问题的形式化定理（Bosma 等 Conjecture 17、Thue–Morse 约化阿贝尔复杂度奇指标递推、Archer–Bourne 2143 猜想），而全仓**没有任何机器可查的地方记着这件事**——问「系统解决了哪些外部 open problem」只能靠人翻散文。边不在工件上即视同不存在（第 9 条）。

**居所恰为 Scribe 的 `Describe` 节点，不入 Lean（owner 2026-09-06 裁决：「这种信息也没必要在 lean, 只改 scribe 就可以」）。** 理由与 §11.20.3 的居所裁决同形，但由另外两处机器逼定：①`UtilityAdmissionRule` 的 `UTILITY-RATCHET` 令已冻结模块的 `utility:` 头部字节不可改动，故已解决问题的绑定**无法**追加进那些模块的 Lean 头部；②`Problems/<slug>.md` 增设 `resolution:` 字段是 SL-006 明禁的手写状态，且 §11.20.3「无 status 字段」一款正是为此不设该字段。故解决状态必须是**派生**的：它恰等于「存在一个绑定该 slug 的 `Describe` 节点」，无人手写、无第二真源。

**封闭形状恰三项**：`problem_slug`（`ProblemSlugRef`，须合 `ProblemPoolPaths.IsCanonicalSlug`）、`declaration_gid`（解决该问题的声明，取自宿主 `Describe` 的 `DescribeStatement.LeanDeclaration` 原值，不重排不加前缀）与 `resolution_kind`（封闭字母表恰为 `proved|refuted`，`ResolutionKind` 枚举，构造期越界即 `ArgumentOutOfRangeException`）。**勘正（owner 2026-09-07，撤回 2026-09-06 的「不补 GID」裁决）**：owner 后一句指令是「我要在这个页面, 看到何时哪个定理解决了哪个开放问题」——两项形状撑不起「哪个定理」，故补第三项。**它不是第二真源**：该 GID 已由 `ValidateResolutionSource` 判为「恰一条当前冻结的定理式声明」，marker 只是把这个**已被校验**的值原样投影出去；判定权仍只在宿主节点，claim 不新增任何信任。落地时树内 marker 计数实测为 **0**（`git grep -c` 对 `Blueprint/**` 与 `.scribe.cs` 皆为 0），故不 bump 版本名、不设双读——两者都会是给一个没发生过的过去建垫层（第 6 条）。

**单值（v1）及其预登记升级条件。** 一个 slug 至多一条 claim。该选择由读数而非投票定：立本节时池内 7 份卷宗**零**条解决绑定，而三个已解决的外部问题**各由恰好一条具名定理承载**——故支持多值的已验证实例数为 **0**，按第 8 条（第二个实例才上收）与第 20″ 条（防的必须是发生过的事）取单值。**升级条件预登记**：一旦出现一个外部问题需要**两条互不可由对方 bind-only 得到**的定理共同承载，即把 claim 由单值改为具名集合，并同步 `duplicate-problem-resolution-claim` 的判据；在此之前多值是臆想，不建。

**五条 fail-closed 判词（与 §11.20.3 同处执法，零新门；`DescribeRepositoryValidator`）**：①`dangling-problem-slug`——该 slug 必须解析到**恰一份**现役 `Problems/*.md`（0 份与 >1 份同红）；②`duplicate-problem-resolution-claim`——同一 slug 的第二条 claim 即红，判词点名第一条的位置；③`invalid-problem-resolution-source`——宿主 `Describe` 的 statement 必须是 `LeanDeclaration`、必须是 canonical formal GID、必须由 `FrozenStatementIndex` 解析到**恰一条当前冻结声明**、且在 Lean 与 Scribe **两侧同时是定理式**（Lean 侧 `LeanDeclarationKind.Theorem`，Scribe 侧 `Theorem|Proposition|Lemma`）；④`missing-problem-resolution-lean-report`——无编译产物报告时不得判绿（不猜、不放行）；⑤上列任一解析抛 `InvalidOperationException` 亦落 ③，不静默吞。

**投影：Markdown 标记。** `CanonicalMarkdownWriter` 在该 `Describe` 的正文后发射一句人读的 `*Resolves.* ` 与一条机读注释 `<!-- scribe-open-problem-resolution-v1 {"problem_slug":…,"declaration_gid":…,"resolution_kind":…} -->`，键集恰为上述三项，且那句人读散文与 marker 点名**同一个**声明，故读者与机器不会各读到一个答案。它是 `.md` 投影的一部分——`Meta/FILEMAP.toml` 记 `Blueprint/**/*.md` 为 `kind=generated`、`produced_by=ScribeEmitter`、`verified_by=ScribeEmitter`——故**不是**第二真源（第〇节四项合取：producer 受治理、输入闭包受治理、可由 `make emit` 逐字节重建、无独立权威）。**如实**：`emit-check` 目标已随投影守卫整族退役，故标记与 claim 的一致由「重跑 `make emit` 即得同字节」承担，不由一道常驻门承担；手改 `.md` 是第 6 条所禁的第二真源，正解是改 `.scribe.cs` 后重发。

**外部消费者如实记账（不在本仓治理面内）。** `the-omega-institute/trureturing-mdbook` 的 `scripts/open_problems.py` 按 `dev` 分支的 `Problems/`、`Library/` 与 `Blueprint/**/*.md` 生成站点页 `open-problems.html`，读的就是上述 Markdown 标记。**该选择由一条硬事实定**：该站点的 CI 只装 mdbook 与 python3，**无 .NET、无 Lean**，故它结构上不可能消费类型化 AST 或跑本仓判官；`.md` 标记是它唯一可达的通道。**故该页面的强度须如实分栏，且其页面文字已如此声明**：它显示的是「本仓在这份 Markdown 快照里记了什么」，**不**是「该问题在世界上是否仍未解决」，**不**是「该 Lean 证明是否忠实于该论文的命题」，**也不**校验仓库有效性、声明身份或 Lean 证明。该仓不在本 spec 的治理面内，本节只记这条边存在。

**三条 open，如实分栏（不冒领）**：①**语义忠实性上界**——没有任何机器能判定一条 Lean 定理与一篇论文的自然语言命题等价；claim 只断言「本仓把这条定理登记为该问题的解决」，那是**登记**不是**等价性证明**，忠实性归内容面对抗评审（与 §11.21 的 `cover` 不判忠实性同形）。②**标记可伪造，判为已接受的限制（#6227，2026-09-07 关单）**——散文可以写出逐字节相同的 HTML 注释；上述五条判词守的是**类型化 AST 侧**的 claim，而外部消费者读的是 `.md` 侧的注释，二者之间没有机器等价性证明。该性质由 `honesty` 评审席**构造性演示**:一个只含从散文抄来的 marker 与合成冻结状态片、**无 Lean 亦无类型化 resolution 源**的仓库，两版渲染器皆计入一条 resolution。**但演示可能 ≠ 发生过:本仓伪造案底 0 次**，立门即第 20″ 条所禁的臆想防御。两条候选机制亦经权衡而不可取:发射侧的认证值可被复制（writer 的输入全为公开仓库字节）；令站点改读类型化工件亦不可行（其 CI 只装 mdbook 与 python3，**无 .NET 无 Lean**，结构上消费不了 AST——这正是当初选 Markdown 通道的原因）。**故记为已接受的限制而非未偿债务**:页面以读者的语言明示该限制，且伪造记录会出现在 `Blueprint/**` 的 diff 里为评审者可见。**重开条件**:一条真实的伪造或错误 marker 进入 `dev`——届时它是有案号的事件，账算法随之改变。③**闭世界**——「无记录」只等于「本仓未登记」，不等于「无人解决」；页面上的 `0 recorded markers` 是本仓账面的真实状态，不是关于世界的断言。

**卷宗原子性仍未机器可判（承 §11.20.3 的 loader 注释）。** 「一份卷宗恰锚定一个显式命题」这条判据当前只靠散文与评审：`ProblemPoolPaths` 只判两条路径正则、`ProblemCandidateCatalog` 只判封闭键集与八节非空，**都不判锚定命题的个数**；而自由的非空锚定字符串在原理上立不了原子性——须先定义一个**封闭的单锚形式与可判边界**，才谈得上执法。**现存违例已点名**：`wall-sun-sun-golden-unit-lift`（Existence 与 Stronger 两个锚）、`random-zeckendorf-game-gaussianity`（Conjecture 1.7 / Question 6.5 / Question 6.6，其中只有一个「should be」半锚）；`mub-six-fourth-basis` 提到四条命题但**不**违例——其正文明写「本卷宗刻意只锚定 Problem 10.2」，故判据是「有没有恰一个显式锚」，**不是**「散文里出现了几条命题」。该门记 `open`。
...
**双轴状态由机器派生,status 只是受检投影,禁手写冒领。**迁移轴为 `{residual,partial,absorbed,nonpropositional}`:仅完成 extract/identify 而无语义目标或收据进展者为 residual;已识别目标 GID 或已有迁移收据但合取未齐者为 partial;原子本地收据与全部 `chain_atoms` 均闭合者才为 absorbed;经 canonical `receipts.nonpropositional` 判定无可覆盖命题者为 nonpropositional。真值轴为 `{closed,tail,open,inapplicable}`:对承载 coverage 的 `{residual,partial,absorbed}` 三种迁移态而言,Lean 闭包 Closed 才是 closed;`inapplicable` 表示经 `receipts.nonpropositional` 判定无可覆盖之命题,与 Lean 闭包无关,亦不计入已证;Tail 只有在 migration 已 absorbed 且 `tools/Authorizations/digestion-tail/<atom_id>.json` 之 canonical 工件逐字绑定 atom 与全部 Tail GID 时才投影为 **absorbed-tail**,否则一律 open;Tail 不计已证。新增轴值的合法新对恰为一对:`nonpropositional-inapplicable`;loader 对 `residual-inapplicable`、`nonpropositional-open` 等其他组合一律 fail-closed。SL-016 对 source 结构、内容指纹、目标 GID、收据、双轴重算一致性逐项 fail-closed,任一 stored status 与派生不同即红。

**非命题终态(owner 2026-09-06 定,#5770 方向③)。**`receipts.nonpropositional` 的键集恰为 `{justification, previous_atom_id, next_atom_id}`,后两者为裸 64 位小写十六进制 atom_id 或 `null`(`null` 只表示已核实的源首/尾边界);该收据与非空 `coverage_gids`、`receipts.quarantine`、`receipts.cover_disposition` 互斥,loader 对任何共存或错形一律 fail-closed。该状态是无需 coverage 的终态,不再承担形式化义务,亦不计入可形式化分母。`settle-atom` 是唯一 writer。两种动词形态的共同检查次序先以 atom_id 定位唯一 entry(`ATOM_ABSENT` / `ATOM_AMBIGUOUS`),随后凡 target 携非空 `chain_atoms` 即以 `CHAIN_PARENT` 拒绝,且该判断先于邻居上下文查询与任何动词专属检查;settlement target 必须是 `chain_atoms: []` 的 leaf,链父项由其子项闭合。**set:**`settle-atom --request FILE --base REV` 读取 strict TOML,键集恰为 `{atom_id, justification, previous_atom_id, next_atom_id}`;`atom_id` 为裸 64 位小写十六进制 id,前后邻居为同形 id 或字面量 `source-boundary`。其专属前置依次为 stored state 恰为 `residual-open`(`NOT_RESIDUAL_OPEN`;已有 nonpropositional 收据却位于错误 residual 目录者亦为该码)、`coverage_gids` 为空(`COVERAGE_PRESENT`)、无 `receipts.quarantine`(`QUARANTINE_PRESENT`)、无 `receipts.cover_disposition`(`COVER_DISPOSITION_PRESENT`)、`unresolved_subitems` 为空(`UNRESOLVED_SUBITEMS_PRESENT`)。此后 `DigestionAtomContextProjection` 才复算真实邻居,其 `ATOMIZER_NONE` / `SOURCE_MISSING` / `OCCURRENCE_MISSING` / `OCCURRENCE_AMBIGUOUS` 原码传播;request 的 previous / next 必须逐字相等,且 `source-boundary` 必须对应真实源边界,否则为 `CONTEXT_MISMATCH`。成功打印 `SETTLED_NONPROPOSITIONAL atom_id=… path=…`。**clear:**`settle-atom --clear ATOM_ID --base REV` 的起态为携带 `receipts.nonpropositional` 的 `nonpropositional-inapplicable`;收据缺席即 `NONPROPOSITIONAL_ABSENT`。clear 只删除该收据、派生 `residual-open` 并把 shard 移回该目录,成功打印 `SETTLE_CLEARED atom_id=… path=…`。无 `--replace`;传入它即 `ARGUMENTS_INVALID`,更正须先 clear 再重新 set。两种转换均经 `BackfillInventoryWriter.WriteAtom` 序列化、由 `BackfillInventoryLoader` 重载并要求逐字节相同的 replay(`ROUND_TRIP_FAILED`),随后恰调用一次 `IngestCommand.ApplyLedgerUpdatesAtomically`,携带旧 shard 删除与新 shard 写入两条路径;set 的 residual shard 不唯一即 `SHARD_AMBIGUOUS`。任一转换成功后,若任一传递链祖先(其 `chain_atoms` 直接或间接包含 target 的 entry)携非空 `coverage_gids`,命令另打印 `SETTLE_ALIGN_REQUIRED ancestors=<sorted ids>`;这是祖先派生 migration 可能改变的 advisory,内容 PR 须跑 `align-digestion-status`,跳过后 stored / derived 不一致仍由 SL-016 fail-closed 检出。loader 读取错误、`ledger changed under us`、IO 等未分类失败统一渲染 `SETTLE_INVALID INFRASTRUCTURE <detail>`;其余拒绝统一渲染 `SETTLE_INVALID <CODE> <detail>`,CODE 的封闭列表恰为 `ARGUMENTS_INVALID, ATOM_ABSENT, ATOM_AMBIGUOUS, ATOMIZER_NONE, CHAIN_PARENT, CONTEXT_MISMATCH, COVERAGE_PRESENT, COVER_DISPOSITION_PRESENT, INFRASTRUCTURE, NONPROPOSITIONAL_ABSENT, NOT_RESIDUAL_OPEN, OCCURRENCE_AMBIGUOUS, OCCURRENCE_MISSING, QUARANTINE_PRESENT, REQUEST_ENCODING_INVALID, REQUEST_KEYS_INVALID, REQUEST_PATH_INVALID, REQUEST_TOML_INVALID, REQUEST_UNREADABLE, REQUEST_VALUE_BLANK, ROUND_TRIP_FAILED, SHARD_AMBIGUOUS, SOURCE_MISSING, UNRESOLVED_SUBITEMS_PRESENT`。`quarantine-atom` 对携带 `receipts.nonpropositional` 的 entry 以 `NONPROPOSITIONAL_PRESENT` 拒绝;`cover-atom` 继续拒绝 zero-coverage 且 truth 非 Open 的 entry。

**纯上游闭合的命题 atom(owner 2026-09-15 裁决,案 #8049)。**全部子句仅由钉版上游(`lake-manifest.json` 所钉 Mathlib 及其传递依赖)的既有声明经实例化、投影与规范化推出、且仓内无可用冻结 GID 的命题 atom,**不新增终态、不新增 writer、不做机器判形**:直接以 `settle-atom` 写 `receipts.nonpropositional`,其 `justification` 须写明由哪些上游全限定声明名闭合、仓内无 GID;收据形状、邻居复算与目录一致性检查照上款,判形真实性只由 PR 正文与评审判(CLAUDE.md 第 3.2 条软边界)。同日早先拟定的第五终态 `upstream-closed`/`receipts.upstream`/`settle-upstream`/`Meta/Digestion/upstream/**` 已撤回(机器 #8169 精确逆 revert;dev 上从未落地实例)。

**链闭合规则。**`DeriveMigration` 固定点与 `CompleteChainGaps` 共用同一谓词:子项迁移态为 `absorbed` 或 `nonpropositional` 即闭;父项 `LocalComplete` 要求不变。零 coverage 的父项仍为 `residual-open`,只失去对应子项的 `chain-migration-incomplete` gap。

**邻居上下文查询。**`atom-context --atom-id ID`(`make atom-context ATOM_ID=ID`)是只读的查询时投影:按 `source.toml` 登记的 atomizer 重切源卷,顶层 claim 按字节序排列,凡账本中带 `chain_atoms` 的父项均以其已验证的子句计划子片递归替换,父项永不与自己的子项相邻。目标按 raw 指纹定位,同一指纹出现多处时 fail-closed;输出 PREVIOUS / CURRENT / NEXT 三者的 atom_id、账本状态目录(未登记者为 `unregistered`)与全文。树中不持久化任何位置数据,与 `atom_id` 不存位置元数据之律一致;`show-atom` 保持只读 CAS 查询,不重放源。

**残余的查询时分区与 `formalization_frontier`。**持久化 `residual-open` 只保持「无迁移进展且真值 open」之义；可形式化前沿不是持久化账本状态,而是 Engine 对 residual-open 的具名查询时投影,永不对应目录或写回树。查询时分区只作用于 `residual-open`;`nonpropositional-inapplicable` 自然不在其中,亦不进入 `formalization_frontier`。投影先为全部 atomizer 可产出的 content kind 作全函数角色分类,再以 `quarantined(blocker_class) > withheld(cover-disposition|acknowledged-stale|malformed-status-marker|qualified-closed-status) > chain-child > not-formalizable(kind) > formalizable-claim` 的严格优先级把每条 residual-open 恰分入一类；`is_chain_child` 与按序 parent atom IDs 同时作正交事实输出。`formalization_frontier` 恰为 primary disposition 等于 `formalizable-claim` 的条目。content kind 字母表由 Engine 独占；未登记的编号 claim 以 `unregistered:<token>` 归 `not-formalizable`,无 kind 的 scaffold/none 归 `not-formalizable(none)`,而代码或 atomizer 数据引入却未扩表的新 kind 是 developer error,不是可持久化 ledger state:**准入时由 SL-016 判**——候选判官对本次变更的内容处置依赖闭包(变更的账目条目、源字节、atom CAS、atomizer 数据与 Engine 构建输入所触及的源)内每条无处置映射的 atom 产 blocking finding `content kind '<k>' has no disposition (atom <atom_id>, source <source_id>)`;共享映射或其 producer 变更时该闭包可跨越全语料,但**不以「规则实现变更」为全量重算的开关**(CLAUDE.md 第 20‴ 条),未触及的既有条目不重判。quarantine 明细(`blocker_class` / `justification` / `reentry_condition`,`blocker_class` 非空且限于封闭字母表)在 residual summary 的 `## frontier` 节下以 `### quarantined residuals` 子节按源渲染,不因被排除出残余清单而失去可见性。readiness、`digest-status --formalize-candidates`、residual summary 与 status JSON 只消费这一投影；旧 `needs-routing` 分类退役。
**retry 与状态标记的归属**:`DigestionFrontierProjection` 是唯一分区所有者;`--retry-dispositions` 只绕过 `cover-disposition` 这一种 withheld 原因,`malformed-status-marker` 与 `qualified-closed-status` 两种 withheld 不受 retry 影响;`formalization_frontier` 在同一组 retry/标记参数下恰等于 `formalizable-claim` 类,`formalize-candidates` 只渲染该投影、不再自行判定成员——前文「`--retry-dispositions` 只在 `digest-status --formalize-candidates` 下合法,仅对带 disposition 的 atom 绕过该 withhold」之条款由此在投影层实现,语义不变。
**残余 atom 的年龄由 git 派生,只作查询投影。**`digest-status --residual-summary` 与 `--json` 按需从 git 历史派生每条 residual atom 的 `first_seen_date`:取 HEAD 可达提交中**相对任一父提交**新增该 atom 不可变 CAS 路径(`Meta/Digestion/atoms/sha256/<atom_id>`)的最小 committer 时间(`%ct`),含根提交的新增;同一路径多次新增(删除后重加)取最小值,merge 父提交的新增保留侧分支日期而非 merge 日期。历史读取是**单次有界**的 `git log --full-history --diff-merges=separate --root --format=%x1e%ct --name-only --diff-filter=A --no-renames HEAD -- Meta/Digestion/atoms/sha256/`——显式的 merge/root 模式使结果不依赖宿主 `log.diffMerges`(`off`/`first-parent`)与 `log.showRoot` 配置。年龄按 UTC 日历日分桶(`0-6 / 7-13 / 14-29 / 30-89 / 90+` 日),输出按源直方图与按 primary disposition(§11.21 查询时分区)的交叉计数;**树中不持久化任何年龄或时间戳字段**(owner 2026-09-02:边即数据,不记动作),readiness 不读历史。浅克隆(即使每条 atom 都有新增记录)、缺失新增记录、超时、畸形或截断的历史输出一律 fail-closed,以 `DIGEST_AGE_HISTORY_UNAVAILABLE` 返回且不产出部分结果;`echo-verify` 与 `digest-status` 共用残余派生,故同样依赖历史可用性——不可用时在写任何分片之前以 exit 2 与 `ECHO_VERIFY_INFRASTRUCTURE` 退出,其发射的分片不含年龄节。
**Canonical CAS 分解(总单 #5533 第 6 层)。**`DigestionDecomposition` 是 Engine 内子句计划、精确铺砌校验与子 atom 物化的**唯一权威**:输入为父 entry、其不可变 CAS 字节与登记的 atomizer;计划把父的完整 `[Start, End)` 字节区间划分为闭合段字母表 `{claim, structural}`,无缝、无重叠、无丢弃字节、无歧义子区间;只有 claim 段物化为子 CAS 对象与 `chain_atoms`,structural 段不产生子 entry、不入任何分母。**已持久化的语法即 canonical 语法**:首个非空行之后的粗体标记开启新子,首段(标题/导语)保留为 child 0;无此类粗体时全部列表标记参与切分(不论缩进层级),child 0 含导语与首项,末子含收尾散文;代码块内的粗体按 ingestor 已物化者保留——对任一已入账父 atom,计划算出的子集(数量、次序、字节)必须与其持久化 `chain_atoms` 逐字节相等,structural 段只允许存在于从未物化过子的位置——对当前全部已入账 atomizer(generic-v1、pzg-v1 等)该类别为**空**:原契约「标题/导语/收尾为 structural 段、不入分母」的承诺被持久化语法**取代**而非叠加,分母恰为 claim 子集;已持久化的导语/收尾字节仍是 claim 字节,structural 分类在任何情况下不得改变已物化的子集——本条如实记录该契约变更。计划与字节区间按需计算,不作为聚合元数据持久化;子 atom 的 content kind 经既有 Engine 分类所有者继承。`decompose-atom --atom <id> --base <rev> [--dry-run]`(`make decompose ATOM_ID=x [BASE=…] [DRY_RUN=1]`)恰解析一条 entry,对被覆盖、已隔离、缺 CAS、gap/overlap 等情形 fail-closed;子 CAS 创建与账目更新在一个事务内,任一失败即回滚本次新建的 CAS blob;`align-digestion-status` 仍只是验证器,`RejectsNewAbsorption` 守卫不变——未分解的多子句父仍被拒绝吸收,分解后其子按各自路径 cover。

**Canonical chain reconciliation.** `decompose-atom --reconcile-chain` permits a lossless regrouping of an existing nonempty chain. The ordinary `Plan` authority selects the registered canonical plan, or, when no canonical plan exists, the existing `--split-at BYTE_OFFSET` interface; canonical-plan conflicts, UTF-8 boundaries, unique subspans and exact full tiling remain mandatory. Ordinary decomposition without this flag still rejects any differing nonempty chain. Reconciliation requires the parent to have no coverage, unresolved subitems, tail authorization, quarantine, cover disposition or nonpropositional receipt, even for a no-op plan. Its old direct chain must reconstruct the entire parent CAS. Old descendants and proposed existing descendants must resolve to unique entries with matching atom IDs, CAS bytes and both fingerprints; each nested chain must pass the same `Plan`/ordinary `Materialize` authority. Every old direct child must remain reachable in the new chain. Reachability alone cannot admit a malformed nested chain, missing child, cycle or changed identity. Existing descendants, coverage, receipts and state directories are retained byte-for-byte; the parent retains its state and receives only the validated chain. Serialization, loader round-trip, optimistic ledger check, atomic CAS/ledger writer and rollback of only newly created CAS are unchanged. Split points are not persisted and no new schema or status is introduced. This is a graph operation, not semantic entailment or absorption evidence; source/coverage review and the parent's own coverage plus complete child closure remain required.
**无标记 CAS 的显式分解。**当登记 atomizer 与 canonical 标记语法均无计划时，`decompose-atom --atom ID --base REV --split-at BYTE_OFFSET [--split-at BYTE_OFFSET ...] [--dry-run]`（`make decompose ATOM_ID=ID SPLIT_AT="偏移 ..."`）接受相对父 CAS 的 UTF-8 字节切点。切点须严格递增、位于首尾之间且落在字符边界；所有片段均为 claim，完整保留标题、条件、公式及收尾字节，并沿用无损铺砌与唯一子区间校验。显式计划不得替换既有 canonical 计划；已物化的不同子链仅可经上款 `--reconcile-chain` 的全部守卫替换；相同计划重跑为字节 no-op。只持久化既有子 CAS 与 `chain_atoms`，不增加切点或计划字段；后续分解、alignment、邻居查询及内容分类在 canonical 计划缺席时由子 CAS 按链顺序重建计划，核验身份、完整铺砌及子项存在性，孩子沿已验证链继承父内容类别。未知方言、歧义及非法计划不进入此回退。子句的语义划分与覆盖忠实性仍由逐子句核对保证，字节校验不证明语义独立；父项仍须自身覆盖与全部子项闭合后才能被吸收。

**Scribe 收据机制已整体退役(owner 2026-09-07 裁决 #5942:「改动都在 git,不需要纠结。把预防"有人偷偷改的"删了吧」)。**此处原记述总单 #5533 第 8 层的 `coverage→Scribe 收据` delta 门与 `align-scribe-receipt --seed-missing` 机械登记,二者连同 `receipts.scribe` 字段本身**均已不存在**:L1 停执法、#6130 停 writer、#6168 使 producer 能够到已提交字节、#6323–#6331 清空全部 28,205 条、#6334 使 loader 对该键 fail-closed 并删除模型与 `strip-scribe-receipts`。**现役形态**:account 的收据面只剩 `unresolved_subitems`、`chain_atoms`、`tail_authorization`、`quarantine`、`cover_disposition`、`nonpropositional`;带 `receipts.scribe` 的条目按未知键被拒。历史形态由 git 查证(第 6 条:现状唯一,历史归 git)。

**atom↔GID 覆盖边为 M:N,不是所有权。** atom 是启发式切片,故同一 atom 可由多个 declaration GID 合取覆盖,同一 GID 也可覆盖多个 atom;baseline 已出现某 GID、或另一 atom 已登记该 GID,均不构成拒绝理由,仓内不建立全局 `GID→atom` 映射。保留的结构不变量恰为:`atom_id` 全局唯一;单 entry 的 `coverage_gids[].gid` 无重复;每条 `(atom_id,GID)` 边只保存 `{gid,target_statement_id}`。`target_statement_id` 由 GID 在 active frozen ledger 中唯一解析,模块 GID 取模块 `statement_id`,声明 GID 取对应 `declaration_statement_ids[].statement_id`,不绑定 Lean 文件原始字节;GID 当前不可解析、目标未冻结或其它 target validation 未解析时必须为 `null`,并使 truth 状态派生为 `Open`。`cover-atom` 对本次 GID 逐项要求当前 report 中唯一存在、为 Closed 且满足公理白名单,再把 coverage 边写入工作树;GID 缺失、非唯一或非 Closed 均 fail-closed。`align-digestion-status` 每次从当前 report 与冻结账本直接刷新所有边的 target,不留旧值;写回前须按最终候选相对基线的有效 delta 迭代派生迁移态及其 `chain_atoms` 依赖至固定点,再一次性写回,二次运行必须零字节变化。首次 cover 可一次登记一个或多个 GID;追加边只需在命令中给出本次 GID,不要求重报已有 coverage 首项。

**全局对齐预览。**`align-digestion-status --base REV --plan`（`make align-digestion-status BASE=REV PLAN=1`）执行普通全局对齐的同一计算、coverage 刷新、固定点迭代、Lean、Scribe、receipt 与最终 backfill 校验，只在最终输出处分支。成功只输出一个 JSON 对象及 LF：`schema="digestion-alignment-plan-v1"`、`baseline_revision=REV`、`applied=false`、`writes`、`validation`、`diagnostics`。`validation` 为 `{fixed_point:"converged",scope:<实际 FullScan 或 ChangedSet>,lean:"accepted",scribe:"accepted",backfill:"accepted"}`，不表示 admission 或 CI 通过。失败保持 `INGEST_INVALID` 错误契约，成功输出为空，不发布部分计划。

`writes` 是所有来源的完整持久化文件写集合。每行 `{domain,action,path,before,after,durability_order}`：`domain` 为 `cas` 或 `ledger`，`action` 为 `create`、`change` 或 `delete`，`path` 为 canonical 仓库相对路径。`before`/`after` 为 `null`（文件不存在，区别于空文件）或 `{byte_length,sha256,base64}`，分别给出精确字节长度、64 位小写十六进制 SHA-256 与 base64 字节。CAS 行先于账本行，复用 writer 对实际磁盘的 pending 检查：缺失对象为 create，before=null，after 为对象原字节，durability_order=null；已存在且相同者省略，冲突字节拒绝，无 CAS change/delete。账本行来自最终 `LedgerUpdates`，before 取捕获的 current raw 原字节，after 取既有更新字节或 null；包含 source.toml、新 atom YAML、coverage 修复和全部状态搬移，省略未变文件，按 DurabilityOrder 后 ordinal Path 排序。搬移表示目标 create 与旧路径 delete，两行各保留其字节与哈希，不引入 rename 操作；逻辑来源或 atom 删除仍由既有 writer 拒绝。存在账本更新时复用 writer 的陈旧账本检查。
...
对象键集封闭为 `{outcome,gids,gaps}`:`outcome` 是 canonical 双轴状态;`gids` 非空、逐项为 canonical GID、ordinal 排序且无重复;`gaps` 每项键集封闭为 `{code,detail}` 并按 `(code,detail)` ordinal 排序。未知键、非法/乱序/重复 GID 或乱序 gap 均 fail-closed。失败落账只写 selector 数据:原 entry 的 coverage 与 projected `status` 保持不变,故 SL-016/admission 的派生状态、gaps 与 deletable 判词加字段前后逐字等价;`cover_disposition` 也不得与非空 coverage 共存。下一次显式重试若仍失败,以该次精确结果替换当前终判词,历史由 git 保存;若成功,与 coverage 边同一原子写入中清掉旧终判词。
...
**quarantine 收据键集封闭为三键(2026-09-05 定,总单 #5533 第 2 层)。**`receipts.quarantine` 的键集恰为 `{justification, reentry_condition, blocker_class}`,三者皆为非空 scalar;`blocker_class` 不再可选,取值限于 Engine 封闭字母表 `DigestionQuarantine.BlockerClasses`(`already-covered` / `missing-prerequisite` / `multi-clause-guard`),loader 对缺键、空值与未知取值一律 fail-closed,writer 恒写出该键;历史上唯一一条无类收据(`pzg-v170` 原子 `0e09132a1f3ec5f44216dd5019605bfe47f975bddbb39c65892dc0e98ba12df2`)已经 `quarantine-atom --replace` 归入 `missing-prerequisite`,故不设兼容分支(第 6 条)。

**deposit 在 freeze 后组合 canonical cover(2026-09-05 定)。**`make deposit ATOM_ID=x GID=g` 在 lean-report → header-check → emit → freeze 之后,对同一 `(ATOM_ID, GID)` 立即调用 canonical `cover-atom` 并重新 `emit`;`cover-atom` 仍是 `coverage_gids` 的唯一写者,shell 层永不写边。已冻结与已覆盖均为可重放的跳过(`PLAYBOOK_SKIP … coverage-already-applied`)。若 cover 不能使残余原子进入 deletable `closed`(如多子句守门),冻结的定理保留、CLI 已写入的 `receipts.cover_disposition` 为权威记录,工作流以 `PLAYBOOK_DEPOSIT_FROZEN_UNCOVERED atom_id=… gid=… reason=…` 非零退出,不得把「已冻结」报成「已消化」。
...
**已有来源登记投影刷新。**`make refresh-source-registry SOURCE=已有source_id或完整源路径 BASE=REV` 经既有 report-consumer 调用 `align-digestion-status --base REV --refresh-source SELECTOR --plan`。selector 必须恰解析一个已登记来源；缺失、未知、歧义或无确定 atomizer 均零写入。由登记 atomizer、当前完整源字节及 atomizer 数据重新派生该来源的 `GenreRegistryProjection`，仅允许修改其既有 `source.toml`；身份与 acknowledgment 不变，不注册新来源、不物化 atom/CAS/chain、不对齐 coverage 或 status。真正候选由既有 loader/writer 构造，完整字节差分若超出该 metadata 即拒绝。候选通过当前 Lean report/closure、Scribe、FullScan digestion 与 source/backfill integrity 后，输出完整 write set（before/after base64 与 SHA256）、完整 current/baseline/candidate 的 canonical snapshot 哈希、Lean report 哈希和 plan 哈希；普通 GAP 保留其既有观察语义。`PLAN_HASH=该哈希` 使用同一入口重新验证并核对绑定，任何输入或计划变化均拒绝；既有 ledger 陈旧检测之后，原子 writer 在任何目录创建或账本写入之前再次核对完整输入前置条件：重读并核对绑定的 Lean report、baseline 与完整 current snapshot（current 最后读取，包含选定源、atomizer 数据及所有其他仓库路径）。任何变化均在零账本写入时拒绝；空 write set 与 plan 输出也须核对此前置条件。只将这份已验证计划交既有原子 writer，保留其失败回滚与串行调用边界。plan 与 apply 均不取得 admission、CI 或提交授权证明，输出不写入 canonical 账本。
...
**术语对照表**〔勘注 2026-08-17·open(`D5-T0044`)〕原表述称 `Meta/glossary.csv` 为双语项目之唯一译名权威,由 Blueprint 与 papergen 引用,译名漂移判 lint 红。实测三项皆不成立:全仓 `.csv` 文件数为 0,该文件不存在;papergen 已退役(其退役票为 `D5-T0006`);`glossary` 在 `tools/` 零命中,不存在任何译名漂移 lint。本卷因此不再声称存在译名权威;是否建立由 `D5-T0044` 承接。`Meta/FILEMAP.toml` 的 `evidence.artifact_kinds` 保留 `csv`——它是 Evidence GID `--tag` 的封闭字母表成员,属保留坐标,不因无对象而删。
...
**触发与边界**:机器发现一条**已摄入**的 claim **涉嫌**数学错误、内部矛盾、空洞性问题或与已冻结 Lean 真值冲突之一,始得归本类;每条已摄入 claim 均由 11.21 的 loader 不变量携 `cas_ref`,不存在需另行排除的无 `cas_ref` 收据。证据门裁决前只称“涉嫌”,不得断言问题存在。四项排除各归既有路径:风格措辞不是理论错;普通未证猜想照常以 `open` 消化;外部文献勘误走 L 平面路径;harness/Lean 自身证明洞走既有后代撤销路径。须分清两层状态:全局 truth DAG 为 `closed/open/tail/semantic` 四态;`BACKFILL.status.truth` 在命题子域仅为 `closed/tail/open` 三态投影,全局 `semantic` 在 BACKFILL 投影为 `open`,不得把 `semantic` 写成 BACKFILL 第四值;非命题终态的 `inapplicable` 位于该命题子域之外,只表示无可覆盖命题,不映射为全局 truth DAG 新值。〔守护:**硬+评审**·机器保证 claim 已摄入及 `cas_ref`/`atom_id` 合规;四类涉嫌触发、排除项及忠实分类当前由评审守护〕
...
“consumer 为空”只能由已注册 scope 的成功 base 侧 query 证明,且其结果 digest 须等于 base judge 注入的 `EXPECTED_EXTERNAL_SCOPE_RESULT_SHA256`;缺 scope、query 失败或注入值与重算值不等均为 `unknown`/exit `3`。〔勘注 v7.21:本段与 AU-EXT-1 所依赖的「base 侧 query / base judge 注入」是 base 判官形态,已按 A16 禁令作废;若重启 P0-2,承重结构改为「判词输入取自 protected-base 的数据 + 候选判官」;SL-029 现为混面警告,不提供分区保证,不得再要求 base 侧执行。〕
...
3. `TE2-1` 所称 obligation catalog 应放入 artifact manifest 的方向若指生产 disposition manifest，则会重造第二真源。fix pass 4 进一步删除内部 catalog：P0-0 由既有 base judge 冻结完整顶层 authority roots〔勘注 v7.21:base judge 形态已按 A16 禁令作废,冻结动作归候选判官〕，候选 manifest 仅可留审计副本且不参与判词；FILEMAP 继续独占生产 disposition。
...
- **v7.37**(2026-09-22,#8931):A5.6 对齐 D5/Interface/Impl/Reg 的最终包所有权、编译证据和显式目标选择；生产目录契约归 Reg，sidecar 与跨 owner overlay 退役，DTR 四态保持 Observe。
- **v7.36**(2026-09-22,#8931):A5.6 定义稳定记录与存储接口、独立 Reg 构建/报告入口、仓库根定位、目录根契约和 canonical writer 的登记地址退役及内容保全边界。
- **v7.35**(2026-09-22,#8931):A5.6 定义独立 Reg 包配置、manifest 准入、源码族与报告义务；缺失任一包配置拒绝，零源码与包缺失分开判定；局部 FILEMAP population 使用完整登记上下文。

- **v7.34**(2026-09-15,案 #8049,owner 同日追加裁决「直接用现有机制就完事了 … 没必要机械检测, 文本标记即可 … 都没必要加新的终态」):撤回 v7.32 同日拟定的第五终态 `upstream-closed`/`receipts.upstream`/`settle-upstream`/`Meta/Digestion/upstream/**`(机器 #8169 精确逆 revert,dev 从未落地实例);纯上游闭合的命题 atom 改用现有 `settle-atom` 收据以 justification 文本终结;§11.21 迁移轴回到四值,链闭合与查询时分区措辞同步;CLAUDE.md 第 3.2/4.7 条同批改写。
- **v7.33**(2026-09-07,#6233 落地后的收口):§11.20.3 记 expand–migrate–contract **三段已闭合**(J2b 删 `LegacyRequiredKeys`/`ArxivIdPattern`/`ProblemCandidate.ArxivId`/`RequireDoiForChangedDossiers`/`OrdinalIgnoreCase` 分支,净 −159 行),撤回「机器上仍有一个旧形入口」;唯一行为变化是 `doi` 仅大小写不同者由接受转为拒绝。§11.20.5 的「标记可伪造」由 `open` 改记为**已接受的限制**(#6227):该性质经评审席构造性演示,但**本仓伪造案底 0 次**,立门即第 20″ 条所禁的臆想防御;两条候选机制(发射侧认证值、站点改读类型化工件)经权衡皆不可取,后者受制于该站 CI 无 .NET 无 Lean 的硬约束。重开条件为一条真实伪造 marker 进入 dev。**另据独立 fidelity 评审席勘正一处文献描述**:arXiv:2503.04122v1 的 Conjecture 6 **不是**印刷版 Conjecture 17 的同一陈述,其周期分支界为 `z > g+d`,在第三个种子处为假,印刷版改为 `z >= g+d`;锚定与 Lean 皆对印刷版。该席对两条现役 claim 判 `equivalent modulo a stated convention`,对 `archer-bourne-cube-2143-count` 的弃权判 `correct`。
- **v7.32**(2026-09-07,#6134 lane → integration-scribe-prose-0907 → dev #6156):A16 增 **SL-032「Scribe narrative provenance」**(Block,repository,delta-scoped):候选 delta 内的 `Blueprint/**/*.scribe.cs` 不得含 CJK 字符,其注释与字符串载体不得含消化账本/理论卷/治理过程叙事(词汇类只锚文档专属名词与固定习语,精确率优先,召回缺口登记为残余);评估器只遍历 `Changes`,不用 `IsBaseFactAffected`(判官实现变更时不重判存量),RawBytes 相等的一对一搬迁豁免;架构棘轮 `BaseFactScopeProbe(32)` 差分探针在案。存量(3345 文件)命中 169 文件,清理为内容面工作,记 #6157。
- **v7.32**(2026-09-07,owner 后续指令,勘正 v7.31 同日的一处描述):§11.20.5 的 marker 封闭键集由两项改为**三项**(加 `declaration_gid`),并**撤回** 2026-09-06 的「不补 GID」裁决——owner 后一句「我要在这个页面, 看到何时哪个定理解决了哪个开放问题」使两项形状不足。**不 bump 版本名、不设双读**:落地时 `git grep -c` 对 `Blueprint/**` 与 `Blueprint/**/*.scribe.cs` 皆为 **0**,无旧形 marker 可迁、无已部署读者可护,二者都会是给没发生过的过去建垫层(第 6 条)。该 GID **不新增信任**:`ValidateResolutionSource` 早已判它为「恰一条当前冻结的定理式声明」,marker 只投影这个已校验值。**首两条真实 claim 同批落地**:`greedy-three-sumfree-two-parameter` 与 `thue-morse-reduced-abelian-odd` 各绑一条冻结定理(`proved`),`archer-bourne-cube-2143-count` **刻意不绑**——冻结的是旋转幂判据,猜想的计数等式仍缺 Archer–Bourne 自己已证的分解双射(候签 6.222 未形式化)。外部消费者 `trureturing-mdbook` 同步到三键并把卷宗/笔记/定理页全改为站内链接,日期由 git 派生(树内仍不存时间戳);该仓不在本 spec 治理面内。本轮零判官面改动、零门级变更。
- **v7.32**(2026-09-15,案 #8049,owner 2026-09-15 裁决「立案并开 worktree 补 Mathlib-only 结算工具」):§11.21 增第五终态 `upstream-closed` 与 `receipts.upstream`(七键封闭、绑定 canonical 探针源 `Meta/Digestion/upstream/<atom_id>.lean` 与公理白名单)、唯一 writer `settle-upstream`(set/clear;检查次序同 `settle-atom`,再判探针 import 面/编译/公理/声明解析)、链闭合并列、`UPSTREAM-REV-DRIFT` 只 Observe;补 CLAUDE.md 第 3.2 条具名缺口 `open(BIND-ONLY-PROPOSITIONAL-SETTLEMENT)`。本条只定契约,机器实现另按层落地(L1a 读侧、L1b writer),落地前不冒称现役。
- **v7.31**(2026-09-07,按用户已授权方案原位修订):A22 改定默认 push 的 engineering/current 与显式 PR 候选的附加 delta、类型化 context 和谓词分拆、候选绑定的单次测试/报告/工程证据、共享验证/退出/摘要、ci-push/ci-pr 与消费者迁移、resolved mathlib revision + OS/arch 增量缓存分区及分层真跑验收;A16/A20 删除直接冲突的旧阶段、base 选测和精确报告短路约定。此条记录目标契约,程序、缓存、workflow、集成/合入后验证与 required-set 更新尚待完成;不沿用旧方案的实施或共识声明。
- **v7.30**(2026-09-06,#5770 方向③,owner 裁决):第四种消化终态 `nonpropositional-inapplicable`、`receipts.nonpropositional`、`settle-atom` / `atom-context` 与链闭合谓词落入 §11.21;查询时前沿投影不变。
- **v7.31**(2026-09-07,#5964 落地后的内容面对齐,owner 2026-09-06 三项裁决):§11.20.3 的 `arxiv_id` 键由**完整 DOI** 键 `doi` 取代(owner:「open problems 一定要外部的, 有 DOI 的这些」「arxiv_id 不必须, 有 DOI 就行」),语法由 `Doi.TryCreate` 判、指向由与所绑 `Library` 笔记 `doi` 的**逐字节**比对承载(`Ordinal`,旧形的 `OrdinalIgnoreCase` 随 J2b 删除);七份现役卷宗于本轮迁完,expand–migrate–contract 的 contract 步(删 `LegacyRequiredKeys`/`ArxivIdPattern`/`ProblemCandidate.ArxivId`/旧比对分支)登记为**必须收尾**的判官面义务。**撤回一处已被实现推翻的规范文字**:v7.16 R15 记的第三条 open「动机链未按 active Frozen 成员资格执法」与「Scribe 今日无 frozen-ledger 依赖」,自 2026-09-04 `fa917c80ef` 起为假——`ValidateProblemMotivationGid` 现役判词为「Formal 平面 ∧ host selector ∈ `Golden/Frozen/state/` ∧ 既有 `ValidateGid`」三项合取;旧文保留为审计记录。**新增 §11.20.5 `THEORY-GENERATION-P-RESOLVED`**:把「本仓哪条定理解决了池中哪个外部问题」这条边立为类,居所为 Scribe `Describe` 节点的 `OpenProblemResolutionClaim`(owner:「这种信息也没必要在 lean, 只改 scribe 就可以」;另由 `UTILITY-RATCHET` 与 SL-006 两处机器逼定),封闭形状恰 `problem_slug`+`resolution_kind∈{proved,refuted}`,v1 单值并预登记升级条件,五条 fail-closed 判词与 §11.20.3 同处执法零新门,`.md` 侧发射 `scribe-open-problem-resolution-v1` 标记。**三条 open 如实分栏**:语义忠实性无机器上界、标记可被散文伪造(非 AST-exclusive 通道)、闭世界(无记录≠无人解决);卷宗原子性仍无机器判据,两个现存违例已点名。外部消费者 `trureturing-mdbook` 只读该 `.md` 标记(其 CI 无 .NET 无 Lean),不在本 spec 治理面内,只记该边存在。本轮零判官面改动、零门级变更、零 producer 变更。
- **v7.x**(2026-09-05,本 PR):`quarantine-atom` 成为 `receipts.quarantine` 的唯一 set/replace/clear writer；strict TOML request、residual-open/无 coverage/无 cover_disposition 前置、封闭 blocker alphabet、冲突与 absent 具名拒绝、同值零写及单 shard writer→loader round-trip 均由命令与 `make quarantine`/`make quarantine-clear` 承载，直接 YAML receipts 编辑退役。

- **v7.30**(2026-09-08,收据退役收口):**撤销 v7.29 所记的整套 Scribe 收据机制**——owner 2026-09-07 裁决 #5942 判其为「预防有人偷偷改的」而应删。§11.21 的 coverage→Scribe 收据 delta 门、`align-scribe-receipt --seed-missing`、`make scribe-seed` 与 `receipts.scribe` 字段本身**全部不再存在**;落地链:#6130 停 writer → #6168 producer 按 committed 字节选择 → #6323–#6331 清空 28,205 条 → #6334 loader 拒键并删模型与 `strip-scribe-receipts`。本次按第 6 条**在原文撤销**(不并列新节、不留兼容记述);历史形态由 git 查证。普查读数:`git grep -lE '^  scribe:' -- Meta/Digestion/backfill` 为 0。
...
- **v7.26**(2026-09-05,总单 #5533 第 3 层配套):§11.21 将 residual-open 的内容处置统一归 Engine 查询投影:五类互斥穷尽分区(`quarantined > withheld > chain-child > not-formalizable > formalizable-claim`,retry 只绕过 cover-disposition 一种 withheld)与 `formalization_frontier`,反向 `chain_atoms` 索引给出 chain-child 及 parent IDs,readiness/formalize-candidates/residual summary/status JSON 共用同一结果;无处置映射的 content kind 由 SL-016 在候选变更的依赖闭包内产 blocking finding(不以规则实现变更为全量开关);`needs-routing` 退役;quarantine 明细以 `### quarantined residuals` 子节保留;不新增目录、持久状态、时间戳或生成 aggregate。

- **v7.22**(2026-09-05):删除 A16.1;理论卷由 `docs/develop/theory/` 路径规则治理;不存在 two-PR 登记义务;裁决:owner 2026-09-05。

- **v7.22**(2026-09-05):A16 增 base 判官禁令的机器投影 **SL-030「Judge surface reads no other revision」**(Block,trust,delta-scoped;判官面 = `.github/**` + `tools/scripts/workflow/**`;物化非字面 HEAD 修订文件的 git 动词即拒,变量/未知选项/缺操作数 fail-closed;元数据模式不判;早反馈,反例集合见 CLAUDE.md 第 19 条)。ci.yml 的 workflow 文本缓存 key 改为 blob OID(#5486)。三轮 sshx 评审(9 席次)抓出 7 条同类 blocking 后收敛,记于 PR #5492。
- **v7.21**(2026-09-05):A16 增 **base 判官永久禁止**(τ=0 owner 2026-09-03 裁决):为判决候选而 checkout / restore / 编译 / 执行 base 树代码的一切机制作废,base 只以数据参与;当时以分区门 SL-029 接住残余逃逸(历史语义;2026-09-24 改为 Warning/Observe,现役边界见 A16)。第十二部凡以「base judge 注入 / base 侧 query」为承重的条款(2.2 AU-EXT-1、12.6.2 external scope、12.10 完成条件、12.11 TE2-1)加勘注作废其 base 侧执行形态,不改写原文。案由与守护分栏见 CLAUDE.md 第 19 条。
...
- **v7.25**(2026-09-05,总单 #5533 第 2 层配套):§11.21 quarantine 收据键集封闭为三键,`blocker_class` 必填且限于封闭字母表;唯一无类历史收据经 canonical 写入器归类,不留可空分支。
...
**A18.2 定理逃逸登记与共享模板投影（PROPOSED / NOT IMPLEMENTED）**

本节是待实施的 graph-v2 契约；A18.1 仍是现役格式。本节不安装运行时、判官、Lean 声明或新冻结规则。目标是在同一可丢弃的图投影中回答「这个精确定理有哪些已选中的登记路线，各路线应用哪个参数化模板，现有证据说残余是什么」。模板是共享的参数化数学读出构造器；模板复用、证明依赖、经证明的信息细化是三种不同关系。Lean 声明、证明项及公理闭包仍是数学真源，投影不成为登记账本或证明证书。

**A18.2.1 输入与首期边界。** 复用 A5.2–A5.6 的 occurrence、绑定证据与 delta 选择，A14.9 的编译报告复用，以及 A18.1 的图生产链。当前源码接口与未来最小变化如下；链接指接口所有者，不声称这些变化已落地。

| 输入 / 所有者 | 当前能力与首期所需变化 |
| --- | --- |
| [TruthDagProjectionAssembler](../../../tools/StrataLint.Scribe/Projection/TruthDagProjection.cs)、[TruthGraphModelBuilder](../../../tools/StrataLint.Scribe/Writers/TruthGraphModelBuilder.cs) | 保留 module-import truth DAG、四态、blocker、最长路径 depth；在 builder 接收可选的同候选已选中观察批次。 |
| [稳定记录](../../../tools/lean-inspector-interface/LeanInformationAuditInterface/Records.lean)、[Registry exporter](../../../tools/lean-inspector/LeanInformationAudit/Registry.lean) | occurrence 五元组、绑定状态、四槽及 certificate 已存在；certificate 增加下述来源绑定的 `template_ref`。 |
| [TemplatePlanData](../../../tools/lean-inspector/LeanInformationAudit/RegistryTypes.lean)、[Enrollment](../../../tools/lean-inspector/LeanInformationAudit/Registry/Enrollment.lean) | 已保留模板 Name、definition/enrollment owner 与 checked plan identity；从所选 plan 导出，不另做模板普查。 |
| [InformationTemplateEvidence](../../../tools/StrataLint.Engine/RepositoryIo/InformationTemplateEvidence.cs)、[DeclaredTemplateBindingRule](../../../tools/StrataLint.Engine/Rules/TheoryGeneration/DeclaredTemplateBindingRule.cs) | 前者检查 certificate 的 plan/descriptor/actual identities 与依赖，但现有 occurrence DTO 未保留这些字段；未来保留 typed 值，并让后者一次消费的结果可交给投影。 |
...
| [EscapeRecord](../../../D5/S3/ConceptDynamics/InformationEscape/EscapeRecord.lean)、[kernel projection](../../../tools/lean-inspector/LeanInformationAudit/Projection/ProjectionSchema.lean) | 前者只给具名链的 witness/empty，后者是有 scope 的 certified analysis；首期不产生二者之间的新桥或 residual-handling 边。 |
...
1. caller 用既有 repository gateway、accepted compiled-artifact report 和 `TruthContext` 固定 candidate snapshot、report digest、report release version。delta 调用另持受保护 baseline snapshot **数据**；不编译或运行 baseline，不取 baseline report。selection 所需 baseline identity 用既有 snapshot materializer 生成，只有使用 baseline 时才写入。
2. 复用 [InformationTemplateSelection](../../../tools/StrataLint.Engine/RepositoryIo/InformationTemplateSelection.cs) 与 [theorem mirror selection](../../../tools/StrataLint.Engine/RepositoryIo/InformationTemplateTheoremSelection.cs)：改动 Reg owner 选其现有完整局部 inventory；changed/first-pin D5 仅识别新增 authored public theorem，并按 compiler source owner 到精确镜像及其既有 Reg import 搜索域选匹配 theorem。保留现役可读的路由地址检查；不得借此严格读取未选中的 payload，D5 payload 永不作登记证据。
3. Engine 在现役 DTR 消费路径一次收集 typed 结果、各次选择的 inventory/失败、新增定理目标及原判词，形成非持久 `SelectedInformationTemplateObservations`。它携同一 candidate snapshot/report/release 与 baseline identity；不是从告警字符串反解析出的记录。每次 `Collect` 的成功/失败边界不扩大、不合并；重叠选择可引用同一个 occurrence。
4. 同轮请求图导出的 caller 将该批次连同原 `TruthContext` 交给 Scribe，再到 `TruthGraphModelBuilder` / `DagEmitter` / canonical writer。batch 身份必须与当前输入完全一致。Scribe 只解析声明地址、投影 typed 事实；不得调用 `assessJoined`、重新执行 DTR、调用无选择的 `InformationTemplateEvidence.Collect` 或扫描整个 report 的登记 payload。writer/reader 不启动报告生产或评估。
5. 普通 [dag-render](../../../tools/StrataLint.Cli/Commands/Dag/DagRenderCommand.cs) 没有 delta context/观察批次时输出 `scope.mode=not-assessed`，其登记相关数组为空；这表示没有本次审计范围，不是全局未登记。不得为了填满图而补跑 assessment。delta 已执行且选择为空时输出 `mode=delta` 和空选择，仍不证明全库覆盖。
6. 缺 producer、坏选中 payload 或过期 batch 在对应选择上显示 unavailable；保持原 DTR 输出，不新造一次 DTR-Evidence。若 caller 只有过期批次，不得将其判词/记录冒充本轮已评估结果；本轮 selection 可由当前 context 给出，inventory 置 null。独立可用的选择继续投影。无任何本轮 context 则按第 5 项处理。

D5 拥有数学、舞台、模板与记录类型；Interface 拥有稳定 transport records；Impl 拥有 E1–E8、绑定评定与 checked-plan 导出；Reg 拥有 enrollment 和登记。Engine 只按现役选择消费证据，Scribe 组装，Trureturing.Truth 拥有 DTO/序列化/结构校验。保持 D5 ← judge ← Reg 单向依赖；元数据导航边不成为 Lean import。不得另设全局状态库、文件新鲜度服务或并行 registration manifest。

**A18.2.3 身份与 join。** 所有身份限制在根 provenance 指向的一个 accepted report；图与同级 `Generated/DAG.md`、旧 graph、诊断文本互不作为身份输入。
...
- `descriptor_identity` 相等不是数学等价或相同 kernel 的证明；依赖输入清单也不是完整参数列表。A5.4 的 proof-erased raw `Expr.equal` 及其有限允许改写保持不变，不引入 `isDefEq`、归一化、语义 hash 或反向查 hash 得模板名称。证明实现不同但原 comparator 擦除后相同，不在此虚造参数差异。

**最小 producer 扩展。** 新版 validated certificate 在现有七字段之外只增 `template_ref`，闭集为 `{name_key,definition_owner,enrollment_owner}`；NameKey 用 inspector 现有结构编码，owners 用现有 canonical module Name。Impl 在 `TemplateBinding.assess` 取得所选 plan 时填入，与原 `plan_identity` 一起传至 `certificateJson`；不读 diagnostics 的 `template_key`，也不按 descriptor hash 搜索 plan。当前 typed C# consumer 同步保留这三项和原 certificate 的四个 identity/两个依赖数组。certificate.key 必须等于 record.key 后才允许在图的内嵌应用中省去这份重复 key。
...
| 基础类型 | wire / 身份约束 |
| --- | --- |
| `Path` / `Name` / `NameKey` | canonical 仓内相对路径 / 既有严格可 round-trip Lean Name 字符串 / report 的无损结构 Name 编码字符串；均非空，不能互当 display label。 |
| `H` / `StatementId` / `SnapshotDigest` | `H` 是 64 个小写 hex 字符；StatementId 是现有 tagged `sha256:` 加 H；SnapshotDigest 完全复用 A18.1 `snapshot.content_digest` 格式和 materializer。不同域不得互换。 |
| `DeclKey` | `{source_path:Path,name_key:NameKey}`，compiler owner 与结构 Name 的有序元组。 |
...
| `TemplateRef` / `Dependency` | `{name_key:NameKey,definition_owner:Name,enrollment_owner:Name}` / A5.4 `{name:Name,owner:Name,type_identity:H,body_identity:""或H}`。`body_identity` 的 wire 域只有空字符串或 64 位小写 hex；精确的生产条件见下文，读者不得从空字符串反推语义。不得补造摘要，非空而非 `H` 的值拒绝。 |

**Dependency identity 的 producer/reader 边界。** `Dependency.body_identity` 的生产契约归 `LeanInformationAudit/Registry/Assessment.lean` 的 `inputIdentity`：对每个依赖，`isProp info.type` 或 `info.value? = none` 时原样发出 `""`；否则必须对可执行 value 计算并发出其 `rawIdentity`（64 位小写 hex）。因此 Prop 常量和无 value 的常量是合法空值，带 executable body 却发出空值是 producer 违约。现有 `InformationTemplateEvidence.CheckDependencies` 只检查该字段为字符串、非空时符合 hash 形状并且依赖名不重复；它不检查空值为何产生，也不得仅凭空值判定 Prop 或 bodyless。未来 canonical projection 绑定这份未改变的 producer evidence；本 SPEC 不新增 C# 语义分类器、整源重哈希或 freshness 系统。若 producer 以后演化，仍由 `inputIdentity` 及其 typed producer fixtures 在这个依赖证据范围内维持上述条件，reader 的形状检查边界不被偷换成语义保证。

| 对象 | 全部字段及类型 |
| --- | --- |
| root | `{schema:string,schema_version:integer,provenance:A18.1,truth:A18.1,documents:A18.1,joins:A18.1,escape:Escape,deferred_layers:string[]}`；两个 schema 值固定如上。 |
| `Escape` | `{scope:Scope,declarations:Declaration[],occurrences:Occurrence[],templates:Template[],dtr_observations:DtrObservation[],diagnostics:Diagnostic[]}`。 |
| `Scope` | `{mode:"not-assessed"或"delta",baseline_snapshot_digest:SnapshotDigest?,report_semantic_version:positive integer,selections:Selection[],new_theorems:DeclKey[]}`。 |
| `Selection` | `{key:SelectionKey,inventory:OccurrenceKey[]?,availability:"available"或"unavailable"}`；唯一 key 为 SelectionKey。 |
| `Declaration` | `{key:DeclKey,statement_id:StatementId}`；唯一 key 为 DeclKey，其 source_path 恰命中一个 formal truth node。 |
| `Template` | `{key:TemplateKey,versions:TemplateVersion[]}`；key 唯一，versions 非空且 version key 唯一。 |
| `Occurrence` | `{key:OccurrenceKey,declaration:DeclKey?,registration_source_path:Path,binding_source_path:Path?,statement_identity:H,unit_name:string,realization_name:string,binding_state:BindingState,binding_diagnostic:string?,four_slots:boolean,escape_from:EscapeFrom?,residual:Residual,bridge_kind:BridgeKind,application:Application?,availability:Availability}`；key 唯一。 |
| `Application` | `{template_ref:TemplateRef,plan_identity:H,descriptor_identity:H,actual_identity:H,evidence_ref:H,argument_inputs:Dependency[],extraction_inputs:Dependency[]}`；其隐式 occurrence key 不重存。 |
| `EscapeFrom` | `{name:Name,type_identity:H,object_identity:H}`，原 `escape_from` 值。 |
| `Residual` | `{kind:"absent"或"open"或"witness"或"empty",declaration_name:Name?,statement_identity:H?,chain_name:Name?}`；三个证书字段在 absent/open 时全 null，在 witness/empty 时全非 null。 |
| `DtrObservation` | `{selection:SelectionKey?,declaration:DeclKey?,occurrence:OccurrenceKey?,verdict:DtrVerdict,detail:string}`；仅拷贝本轮真实评估的 typed 输出，不从图状态合成。 |
| `Diagnostic` | `{selection:SelectionKey?,declaration:DeclKey?,occurrence:OccurrenceKey?,reason:ExportReason,detail:string}`；这是投影诊断，无 admission effect。 |

| 闭合枚举 / 交叉约束 | 合法取值与含义 |
| --- | --- |
| `BindingState` | 原标签 `undeclared`、`declared_unresolved`、`declared_validated`；不加入 stale/error 状态。 |
| `BridgeKind` / `Availability` | `legacy`、`forward`、`witness` / `available`、`unavailable`。availability 说的是导出所需证据，不是数学真假或 residual 状态。 |
| `DtrVerdict` | 原四名 `DTR-Undeclared`、`DTR-Evidence`、`DTR-Unregistered`、`DTR-Declared`；全部保留 Observe。 |
| `ExportReason` | `missing-producer`、`invalid-selected-evidence`、`incompatible-evidence-version`、`snapshot-mismatch`、`declaration-unresolved`、`statement-mismatch`、`template-reference-unavailable`、`conflicting-selected-identity`。不增加 DTR 判词。 |
| scope | not-assessed 时 baseline=null，selections/new_theorems 及 Escape 其余数组全空；delta 时 baseline 非 null。release version 必须等于 accepted report 的实际版本。 |
| inventory | available 选择必须有非 null、严格已核对的 inventory（可以 `[]`）；unavailable 选择必须为 null 并有具名 Diagnostic。不同选择交集可非空，禁止借 union 隐去某次选择失败。 |
| occurrence | 只为成功解析的原记录建行，key 集合恰等于 available selection.inventory 的并集。坏 key 不造占位 occurrence。原 binding diagnostic、binding owner 与 application 的 nullability 保持 A5.4；generated unit/realization 使用 report 中唯一的非空 compiler display spelling，不强套 Name 语法。 |
| 四槽与 join | `four_slots` 复用现役 `HasFourSlots`，不是新的评定器；只有它为 true、binding_state=declared_validated、application 有效且 availability=available 时 declaration 才非 null。其他情形 declaration=null；四槽缺失本身不把一个可读记录改成 unavailable。 |
| template 端点 | 每个 application 的 `(template_ref.definition_owner,template_ref.name_key)` 及 `(template_ref.enrollment_owner,plan_identity)` 恰命中一个 Template/version；templates 恰为 application 端点集合，不导出未使用的模板普查结果。 |
| declaration 端点 | declarations 恰为 scope.new_theorems、非 null occurrence.declaration 及 observation/diagnostic declaration 引用的并集。新定理即使无 occurrence 仍有 Declaration 和原 DTR-Unregistered observation，不捏造五元 key。 |
| observation | DTR-Unregistered 只有 declaration 目标；occurrence 判词携 selection 和 occurrence；一次选中读取失败的 DTR-Evidence 可只有 selection。selection 引用必须存在，失败时 occurrence 可以无对应行；detail 保存原诊断，不解析其字符串补身份。 |
| diagnostic | selection/declaration 至少一个非 null；occurrence 若有只作失败定位，可没有对应行。可解析行 join 失败则行 availability=unavailable、declaration=null，并给 Diagnostic；这不覆盖原 DTR 判词。 |

**顺序、去重与编码。** root 的 `deferred_layers` 固定为 `["digestion","residual-handling"]`；其余 A18.1 字段与内部数组排序保持原契约。新实体数组按上表 key 的组件逐项 `StringComparer.Ordinal` 升序，null 排在字符串前；OccurrenceKey 按五元次序，SelectionKey 按 path/filter，Template/version 按各自表列 key。`new_theorems` 与 inventory 按其 key 严格排序。DtrObservation 按 `(selection,declaration,occurrence,verdict,detail)`，Diagnostic 按 `(selection,declaration,occurrence,reason,detail)` 排序，嵌套 key 按上述次序；重复完整行拒绝。argument/extraction inputs 保持证书给定的顺序，不为图改排或去重。

所有复合 key 由字段/结构元组比较，不以斜杠拼接或 display label 编码；无需新的 opaque ID/hash。assembler 对重叠选择中的同一 occurrence 只在记录逐字段一致时合成一行，保留各 selection 的 inventory。同 key 冲突时，将 inventory 含该 key 的所有选择置 unavailable/null 并报 `conflicting-selected-identity`，随后只保留仍由 available inventory 支持的行；不得 last-write-wins 或随输入顺序挑选证据。保留原 DTR observations，不因此重评；重复同 SelectionKey 的一致结果只保留一次，冲突同样 unavailable。最终 wire 内重复实体 key、悬空有效端点、未知/缺失字段、非法 null、矛盾状态均由 strict reader 拒绝。canonical bytes 继续由 `StructuredCanonicalWriter.WriteJson` 产生并由 reader 重写逐字节核对，不另写 JSON 标准或第二套摘要。

投影自身的 Diagnostic.detail 只由 reason、受影响 key 与已有输入身份确定，不写时间、绝对工作树路径、随机 ID 或平台异常栈。原 binding diagnostic/DTR detail 按 accepted report/实际观察保留。确定性前提是 candidate、report、baseline、选择及观察批次相同；改变选择范围会合法改变图，不能把零行相等当作覆盖证据。

**A18.2.5 关系与正交状态。** 不同时存嵌套端点和另一份 edges 数组。以下分型关系由字段确定，客户端可反向索引；反向导航不是新增证明边。

| 关系 | 唯一来源 / 认证条件 | 不得推出 |
| --- | --- | --- |
| `declared_in`：Declaration → module | DeclKey.source_path → truth.nodes.repo_path。 | theorem-level proof dependency、单独的 declaration 冻结状态。 |
| `registers`：Occurrence → Declaration | 非 null occurrence.declaration，满足四槽 join 条件。 | 登记的数学自然性、语义忠实性完备证明或 proof novelty。 |
| `applies`：Occurrence → 内嵌 Application | 原 source-bound application；认证导航须满足同一四槽 join 条件。 | 参数改写的自由等价、共同实现。 |
| `uses_template`：Application → checked version → definition | template_ref + plan_identity；认证边继承 occurrence 的可用性与四槽条件。 | 相同信息 kernel、strict refinement、相同 capture 或新增数学内容。 |
| `continues_at`：Occurrence → 具名链/证书引用 | witness/empty 原字段，随 source-bound 四槽证据关联；open/absent 没有证书端点。 | 另一个登记已处理此残余、跨 arena 运输。 |

truth module 的 `closed/open/tail/semantic`、原 binding state、实际 DTR 判词、residual kind 与 export availability 各自独立。可读的 `declared_validated` 旧式缺槽记录可以 four_slots=false、DTR-Undeclared；不能把 binding label 改写为 DTR-Declared。若源 DTR 已报 Declared 而导出 join 信息不可用，保留该判词，同时禁发认证关联；不重新裁决它。

`residual.absent` 只表示原 continuation 字段缺席；`open` 只是一项无证书的 continuation 声明，**不证明不可判定、不证明逃逸无穷或永远无法闭合**。`witness` 证明具名链的某对状态仍未分开；`empty` 只证明指定 arena 上指定链的 residual 为零，不能升级为整个理论、所有读出或未来所有链已经穷尽。坏证据中的字面标签不因可展示而成为证明。

**A18.2.6 不可用、陈旧与图边界。** selected evidence 的错误按现役读取事务边界隔离：读取失败的整个 selection inventory=null，不从失败 payload 抢救部分「已认证」行；另一独立成功选择不受连坐。缺 producer 与有效空 inventory 不等价。未选历史 payload 保持 opaque，其畸形不得影响导出或 admission；outer report 的 canonical bytes、内容地址、封套 schema 错误仍全局拒绝。坏外层 graph、unsupported graph dialect、重复 key 或结构自相矛盾同样拒绝加载，不能降级成空图。

provenance 复用 snapshot.content_digest、lean_report_digest、truth_root_sha256、dependency_granularity；图消费端要作「当前」关联必须取得 expected snapshot/report/version 并核对。旧图作为自身历史快照可以独立加载，但没有 expected provenance 就不能声称当前，不能与另一快照拼接。candidate/report/batch 不匹配时禁止当前 join，并按当前已知 selection 给 snapshot-mismatch；不以重新 hash 源文件给登记另建 freshness 机制。

只有 `truth.edges` 的 module-import 关系参与 truth 拓扑、最长依赖路径 depth、blocker、撤销后代、冻结和公理闭包；没有前置为 0，否则 `1+max`。`documents.document_edges.dependency` 继续必须无环；`narrative_reference` 继续允许环且不承重。metadata 层和可反向遍历的整个多关系图不要求 DAG；有限遍历使用 `(节点种类,完整 key,provenance)` visited 集合和调用者边界，不能计算一个混合 depth。

现有 kernel-analysis DAG 保留自己的 root/catalog/arena、relation certificate、strict-refinement 与 cover 语义；node_key 不获得全局身份。把它附到本投影前，必须有实际兼容证据将 registration root/occurrence/realization、analysis sealing root/catalog/arena、具名链及同一 report 对齐；arena 同名、模板相同或计数相同都不够。首期不导出该 attachment 字段，禁止从 sibling projection 反向补证；未来新增须版本化并明确哪些边承载经证明的细化。

**A18.2.7 第二期 deferred：残余处理的证明义务。** 首期 wire 中没有 `handles_residual` 边，也没有伪造的当前 certificate 类型。以下只是未来**充分**义务，不声明已有证明，不要求为了 SPEC 新造 bind-only Lean 包装。

设同一实际 arena/realization 上源具名链为 `C`，末层 kernel 为 `K_C`，非对角 residual 为 `R_C`，目标登记经证据绑定的实际读出为 `q_T`。未来 certificate 必须绑定源/目标 occurrence、声明与报告身份、精确链、所用读出及量词域；不能只给两个模板名。

| 未来 claim | 需要的 Lean 证据 | 精确结论边界 |
| --- | --- | --- |
| 处理一对 | 给定或存在 `(x,y)∈R_C`，并证明 `q_T(x)≠q_T(y)`；若引用原 residual witness，须证明是同一对。 | 只分开该对；不推出整个 residual 消失。 |
| 处理子集 | 给定 `S⊆R_C`，证明 `∀ (x,y)∈S, q_T(x)≠q_T(y)`；声称正增益另需 `S≠∅`。 | 空子集的真命题不冒充有增益；未覆盖部分仍保留。 |
| 处理所有残余 | 证明 `∀ (x,y)∈R_C, q_T(x)≠q_T(y)`，并给追加读出后的 kernel 为 `K_C ∩ ker(q_T)` 的兼容证据。 | 该共同实现的联合读出消除此 R_C；不推出 q_T 单独优于 C，或任意 arena 上消除逃逸。 |
| strict refinement | 对所声称的新旧 kernels 证明包含关系并给严格见证。若是追加观察，证明 `K_C ∩ ker(q_T) ⊊ K_C`；若声称目标自身更细，必须另证 `ker(q_T) ⊊ K_C`。 | 分开一对不足以证明替换读出的全局包含；all-residual 在 R_C 为空时也不自动严格。 |
| 跨 arena / common realization | 明列源状态到目标状态/共同实现的映射及适用假设；证明在所声称 pair/subset/all 域上的合法性、非对角与 residual 对应、读出相容和所需分离性质。 | 若声称双向等价或反向覆盖，另需相应反射/满射/逆映射义务；不同边缘分布或各自最优不能拼成共同实现。 |

多目标读出一起处理 residual 时，必须证明同一实现上的联合分离；例如 `∀p∈R_C, ∃i, separates(q_i,p)` 与「某一固定 i 分开所有 p」量词不同。有限已列链、一个样本或数值计数不推出统一无界结论。原 `EscapeResidualWitness` / `EscapeResidualEmpty`、literal open、template reuse 和 analysis 数值均不能替代这些桥梁。

**A18.2.8 版本切换与可证伪验收。** 实施时在同一原子改动中更新 Interface/Impl 证书与 release version、Engine typed 消费、Scribe 装配、Truth DTO/writer/reader、所有图消费者和 FILEMAP 的 run-local 路径归属；`Generated/truth-graph.v2.json` 继承 A18.1 产物职责。删除现役 v1 输出/读者路径，不保留永久双格式兼容或旁路 sidecar；历史 v1 由历史 revision 的 reader 读取。当前 v1 必须继续拒绝 v2 字段。仅提交本 SPEC 时不改 FILEMAP、报告版本或运行时。

以下为未来实现验收，不是本次已运行测试。`E` 指现有 `InformationTemplateEvidenceTests` / `DeclaredTemplateBindingRuleTests`，`G` 指 `TruthGraphJsonTests` 及 Scribe graph assembly 测试，`L` 指 inspector 的 enrollment/binding/evidence fixtures；owner 应在相应现役项目加入有意义的用例。

| 情形 | 预期可观察结果 | 未来 test owner |
| --- | --- | --- |
...
| 选中 producer 缺失 / 新定理无任何 registration key | 前者 unavailable+null inventory；后者保留 Declaration 和实际 DTR-Unregistered，不造 occurrence。 | E + G |
| residual absent / open / witness / empty | 各状态及字段约束保持；open 无证书，无 undecidability/unbounded 推论；empty 仅具名链。 | L + G |
| 原 binding validated 但四槽缺项 | four_slots=false，保留实际 DTR-Undeclared；无 DTR-Declared 认证 join。 | E + G |
| copied certificate、owner/statement/unit 不符、缺 template_ref | 选中证据不可用或对应 join 不可用；无认证边、无新阻断规则；原 binding/DTR 状态不得伪改。 | L + E + G |
| 未选历史记录畸形 / 被选记录畸形 | 前者不读不判且不影响图；后者按原读取范围 unavailable，其他独立选择照常。 | E + G |
| outer report/graph 畸形、未知 dialect、缺字段、悬空有效端点 | strict loading 失败，不能以 unavailable 或空数组掩盖外层损坏。 | E + G |
| candidate snapshot、report digest、release 或 baseline 与批次不符 | 禁发当前认证 join；原历史图可自述其 snapshot，但不可混入当前图。 | E + G |
| 重排输入实体、重复选择 / 同 key 冲突 | 相同语义输入得到逐字节相同 canonical JSON；重叠一致行合一、冲突具名 unavailable；wire 重复 key 拒绝。 | G |
| metadata 反向导航成环、文档 narrative 成环 | visited 遍历终止，truth depth/状态/blocker 不变；truth import 或 document dependency 成环仍拒绝。 | G |
| 两 arena 的模板/计数/hash 相同；仅有 residual witness | 零 fabricated refinement/transport/handles-residual 边；deferred_layers 明示 residual-handling。 | L + G |
| 增加导出调用前后比较 DTR | 相同 candidate/base/report 的选择、四种 Observe 输出与 admission effect 不变；显示不增加 assessment 调用。 | E + G |

本节的完成标准是上述首期接口可按闭合字段、选择与失败边界实现，并有明确的后期证明义务；运行时是否满足这些用例须由后续实施验证。当前文档交付只做 diff/链接校验及适用文档 preflight，不把设计中的测试或 runtime cutover 报成已完成。

**冻结面条款:**`D5/X_Frontier/Hearts.lean` 的 Modified/Deleted 一律拒绝;Hearts 的规则变更须与规则实现同改并经过 SL-022 保护面,`HeartsAuthorizations.md` 不构成改动豁免。`Golden/Frozen/accepted/*.json` 保存当前接受的 Freeze 快照,由 canonical writer 替换或删除;历史版本保留在 Git,不要求当前树重复保存旧快照。`ledger-align --retire-registration D5/.../Missing.lean` 显式退役已迁出的纯登记地址,并重建保留后代的当前依赖快照;它不伪造 Revoke 证据、不为 Reg 创造 GID 或 state pin。参数边界、回滚和内容保全义务见 A5.6。L3e 起(#5344):`Golden/Frozen/state/<module>.lean.json` 为冻结成员的唯一真源(键集恰 `{statement_id}`,片存在 = 成员);SL-008 只判**当前态一致性**——对受影响集合内每片判 C1 路径可解码为唯一 canonical 仓内 `.lean` selector(含根 `Trureturing.lean`)、C2 schema 封闭、C3 模块存在、C4 TruthState=Closed、C5 stored pin == 当前 report 模块 statement_id(`CanonicalStatementWriter`,由声明集合派生,不含头部注释);pin 改变以 `FROZEN_PIN_CHANGE selector= old= new=` Observe 点名而不拒绝。受影响集合 = 改动的状态片 ∪ 改动 canonical D5 模块在「当前 report import 图 ∪ baseline 源解析 import 图」上的反向传递闭包(baseline 边由 baseline snapshot 源经 canonical `ParseImports` 得:admission 不 checkout、不编译 base,无 baseline report)∪ 当 `RuleImplementationChanged` 或任一 canonical Lean-report producer 输入(`IsLeanReportProducerInput`,包含输入登记 `lean-report-inputs.json` 与现有判官/环境输入)改动时的全部当前成员;SL-008 的 RuleCatalog 外层 affectedness 同含该 producer 谓词。SL-008 不再读取 `Golden/Frozen/accepted/`;CLI 冻结账本 admission sidecar 已删除。accepted 事件目录与 `ledger-align` 的 `--from-accepted` 为 contract 相位前的过渡形态,由 #4687 的 owner 裁决收口。
