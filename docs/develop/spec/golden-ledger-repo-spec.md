...
**形式化覆盖是两个 PR 的权威事务。**PR-1 的 `make deposit` 产生 Lean 声明及 canonical `Meta/Digestion/formalizations/<atom_id>.v1.json`(`schema: digestion-formalization-v1`)并先行落入受保护基线;PR-2 必须从该落地基线的新 lane 运行 `make cover`。receipt 逐字绑定 `atom_id`、原子 `cas_ref`/`raw_sha256`、primary GID 及 primary/hosted declaration 的 `{name_key,kind,type}` 签名。覆盖权威只来自候选 fork point 中已存在的 receipt;同 PR 或 candidate-only receipt 均无权威。SL-016 必须独立于 cover CLI 所收参数,重新验证每个相对 fork point 新增的 coverage GID 已获该 base-owned receipt 授权,且候选当前 Lean declaration 签名与其 pin 完全相等。
