...
**Finite kernel structural helpers.** A safe singleton definition whose actual kernel body, after its lambda telescope, is headed by `List.brecOn` or `Nat.brecOn` may be checked in a scoped structural grammar. The compiler checks that entire unspecialized body before applying inputs; the major must be an original source binder. Pinned `brecOn`, `below` and nonrecursive `casesOn` operations retain every motive, discriminant and alternative. Function-valued recursive results, `PProd` dictionary projections and partially applied finite helpers are checked inside that scope. Generated suffixes and source-equation metadata provide no authority; there is no unchecked recursive self leaf. The resulting existing plan is `audit(checked body and inputs, atom(original occurrence))`, so semantic checking never authorizes replacing the original raw call during comparison. Public/body domains, rigid levels, binder information, arguments (including unused arguments), helper result types and dependencies remain obligations. Raw E4c recursors retain their original major restrictions. Unsupported schemes remain diagnostic failures and all E8 quotas are unchanged.

**Standard Nat operations.** Producer-pinned logical `Nat.add`, `Nat.sub`, `Nat.div`, `Nat.decLt`, `Nat.decLe`, `Nat.bitwise`, `Nat.shiftRight`, `Nat.hasNotBit`, constructive And/Or decision combinators and `ite` check all operands and branches. Nat division uses its total logical semantics, including division by zero, not an arbitrary extern implementation. The pinned bitwise operation applies its checked Bool function to corresponding operand bits; shift-right uses repeated natural division by two, and `hasNotBit m n` is the proposition `Nat.land 1 (Nat.shiftRight m n) ≠ 1`. Lean's sparse constructor matchers use this logical bitmap guard. The pin binds each operation's actual kernel type, body, levels and owner; it grants no rule to a same-typed replacement or an arbitrary extern. Standard homogeneous add/subtract/divide notation and List append notation require exact typed standard dictionaries; an alternative dictionary has no rule. These are operation productions, not whole-program admissions. Structural helper Nat arguments (including `optParam Nat`) receive index context after typing normalization while their original domains remain checked. Closed executable `decide` and proof-to-data elimination remain rejected.

**Transient argument audit.** Supplied-argument traversal retains the full grammar, raw argument, public/body-domain, dependency and occurrence-identity checks. Its private discarded plan need not abstract binder fvars or compact repeated audit nodes; exported enrollment plans still do both. Successful occurrence-identity checks may be reused within one query, with at most 256 retained raw expressions and charged full raw comparison, including rigid levels, binder information and immutable fvar identities. Separately, at most 64 successful complete closed kernel-body audits may be reused at the identical depth and type position in structural scope, after charged full raw equality; expressions containing local variables, loose binders or metavariables are ineligible, and metadata has no equality shortcut. The fixed query binds the theorem, environment and constructor descriptions. Raw call arguments, public/body domains and helper result obligations still run at every use. Neither cache supplies an exported plan or changes extraction/comparison. Empty universe substitution reuses the unchanged raw expression. Argument dependency identities are produced and charged once by this traversal and consumed directly by assessment; no quota is reset or increased.
