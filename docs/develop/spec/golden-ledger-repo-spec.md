...
Normal Lean-cache publication, scheduled or manual, invokes the existing Inspector entry before packing or returning an existing-tag success. Its native `:report` target supplies the ordinary default compiler obligations and report work once; required input, build or full report validation failure prevents publication. The root buildDir archive carries native Inspector artifacts plus canonical report/material/origin/attestation outputs. Release tags append the existing shared `build-snapshot-address` to the Lean-only identity and manifests retain its full `build_snapshot_sha256`; no separate compatibility version is introduced. Legacy Lean-only releases remain prefix or explicitly allowed same-toolchain compilation seeds, never complete exact hits. Fetch validates resolved tag/manifest addresses and existing archive content/provenance constraints; native traces and complete current-input/material validation decide report reuse after restore. Native executables may rebuild for the target platform.
...
CI saves the existing native `.lake/build` snapshot after report production and preserves the successful Lean build snapshot when reporting fails. The existing snapshot generation consumes the report semantic coordinate, so same-version compatible report-only edits preserve its key and a version bump changes it; the ordinary Lean source/config address and helper/key/writer/restore-prefix ownership remain intact. Ordinary validated `raw-lean-reports` artifacts transfer exact-source results to Release, which validates them or invokes the same Inspector. There is no independent report cache/store, remote report publisher, baseline adapter or delta/cohort planner. Donor warm requires only Lean products and does not claim report hits without report products. Lean-cache release publication requires the complete current report as specified above.
...
**Workflow 与消费者。** `ci-push.yml` 承载 `push` 到 `dev` 与 `workflow_call`,运行 engineering/current。PR 调用固定传入候选 SHA,被调用 jobs 检出该候选,workflow 配置为 PR 只读行为(含缓存只读)。`ci-pr.yml` 只承载 `pull_request`,目标分支保持 `dev` 与 `integration-**`;每轮使用 native `github.sha=M` 及其第一父 `B=M^1`,将不可变身份传给调用的 push 流程与 delta。所有检查 jobs 均检出同一个 M,B 只供 delta 使用;各 job 不得重新解析可变 ref;合并冲突会阻止该 workflow 运行。delta 依赖两个共享阶段的成功与产物交接,不再运行 engineering/current。
...
每层在 integration 上通过检查后,还须以最小触发 PR 在真实事件中验证新判据的具名拒绝或放行;测试载荷验完关闭不合并。特别是 native `pull_request` 从 PR merge tree 取 workflow,必须核对 run **实际执行的 workflow revision**,合并冲突会阻止该 workflow 运行;同轮复核本地 merge-tree 与 GitHub M 的 tree 一致。向 dev 开 PR 前,将最新 dev 合入 integration,在含其新增量的集成树上复测 engineering/current 及真实 PR delta,不豁免分区、首次冻结或棘轮。留存对应候选/base、workflow revision、具名 checks、报告/工程证据与 run 链接;本地行为测试不能替代权限、缓存只读、拓扑、artifact 传递与报告发布的真实集成验证。合入后观察首个新 dev push 与后续 PR 的新 checks,确认实际名称与判词后更新 required set 并复核 `strict=false`;失败须按仓库流程修复或撤因,不作 admin bypass。规范合入、程序完成或某一轮绿均不单独代表完整目标完成。
