...
The same manifest's `source_patterns` selects report sources and module enumeration, currently root `Trureturing.lean` followed by sorted `D5/**/*.lean`. Their path/content manifest is hashed separately from the compatibility token. The compiled Lean cache keeps its existing broader source scope, including Inspector Lean; both consumers share the unchanged `lean_config_sha256` preimage: `lean-toolchain`, `lake-manifest.json`, then each present `lakefile.toml` and `lakefile.lean`. Source changes, additions and deletions still drive the existing reverse-import/refutation-claim delta closure. Report/sidecar/provenance/material validation and C# source/path/symlink/UTF-8 validation retain their existing consumers and failure boundaries. CI report addressing and provenance use report-source/config outputs; compiled-cache outputs only address the compiled cache.

The manifest uses a closed, dependency-free TOML subset: unique top-level assignments to `compatibility_version`, `source_patterns` and optional `scribe_check_inputs`; the integer uses `[1-9][0-9]*`, arrays use JSON-compatible double-quoted relative path patterns without trailing commas, and whitespace and `#` comments are allowed. Unknown keys and duplicate or escaping patterns fail. `source_patterns` uses recursive path globs and rejects overlapping selected files. `scribe_check_inputs` is mandatory for the existing Scribe selector and lists supplemental shell path patterns, including deleted paths; it selects the unchanged `projections --check` and `describe-report --check` commands. Its bytes do not enter report compatibility. The reader exposes `address`, `verify`, `modules`, `compatibility-token` and `scribe-input-patterns`; no report producer-closure discovery remains.
