...
每个保留文件必须在 `Meta/FILEMAP.toml` 登记：以完整路径单独登记，或由**以扩展名收尾的通配条目**登记（`docs/reports/<目录>/*.py`、`docs/reports/**/*.json`），使同一条目匹配的每个文件共享一个登记的 `kind`。不以扩展名收尾的 catch-all（如 `docs/reports/**`）不构成登记。`filemap-conform` 对未登记文件报 `FILEMAP-REPORT-UNREGISTERED`，对仅由 catch-all 覆盖的文件报 `FILEMAP-REPORT-UNSCOPED`。实验程序记 `kind=program`，数据和许可证记 `kind=data`，均属内容面；登记本身属判官面。

依 SL-029，新的路径或扩展名族先通过独立的判官拉取请求登记，再通过内容拉取请求添加材料；已登记的扩展名通配条目覆盖的新文件不需要改动 `Meta/FILEMAP.toml`。精确登记允许暂时无文件，供预登记及删除后的分步清账使用；不赋予同目录其他路径准入。删除某族全部材料后再以判官拉取请求移除其登记。
