...
current 报告生产只有在最终绑定、验证并发布后才原子写 `result.json`，其中实际 `lean_build_executed`/`lean_build_succeeded` 与报告 input/runtime/partition/sha 绑定。现役 `lean_actions.py --stage current` 的 dependency/project 快照在派发 bounded worker 前复核此结果和本轮已接受的 current transport；独立 `lean` 则使用相同候选/run/attempt 下已接受且执行成功的 `lean` step。零编译、失败或不匹配结果不制作或保存重缓存；报告与检查证据仍按各自验证结果保存。通用显式 `--layers` 运输接口保留原生产成功契约，不冒领为 current 的准入门。
