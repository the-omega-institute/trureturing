...
`DeclaredTemplateBindingRule` stays delta-only and observe-only (A5.5). Its selection covers changed `Reg` modules; a new public theorem of a changed D5 module is looked up in its mirror `Reg` module by path; `DTR-Unregistered` is the work list of the registration line (#5214) and places no obligation on the authoring lane. `Reg` modules are report modules of `lean-report-inputs.json`; the report's version and reuse mechanisms (A14.9) are not changed by this architecture.

A frozen D5 module may be edited or moved to take declarations out of it; mathematical statements and proofs do not change, and pins are regenerated by the canonical writer. Until the migration completes, the D5 modules that import the judge are base-owned debt under SL-001: a candidate-new edge from D5 to the judge or to `Reg` is rejected, the candidate's edge set is a subset of the protected base's, a change to an indebted module strictly reduces its debt, and the same rule judges the full tree once the set is empty. The sidecar command `declare_information_template_binding` is retired when no module uses it.
