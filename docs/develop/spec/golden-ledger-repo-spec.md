...
**动机链的成员资格强度如实分栏（第三条 open）。** `dangling-problem-gid` 的现役判词只到「该 GID 在树上解析」（并在有 Lean 报告时一并解析该声明），**不**要求其 module path 属于 `Golden/Frozen/state/` 状态表成员——而 §11.20.2 对 Theorist Frontier 的 motivation 要求的正是后者。这条强度差记 `open(案号待开)`：`FrozenLedgerBaseViewReader` 在 Engine 已现役，故强谓词可达，但 Scribe 今日无 frozen-ledger 依赖，接线是真工作量而非一行。**实测缓解（读数，非假设；附采集方法）**：现役六个实例共 53 条引用、35 个相异 GID，35/35 正名 `.lean` 存在、35/35 所属模块无 `sorry`、35/35 作为 `descriptor_selector` 出现于 `Golden/Frozen/accepted/` 的事件中。该读数由 accepted 事件集**手算**得出，未经 `FrozenLedgerBaseViewReader` 的 `ActiveByPath` 投影；二者今日重合是因为当前账本恰无 Revoke 或 Supersede 事件（1079 Freeze + 1028 Reattest + 1 Genesis），而这是账本此刻的状态、不是谓词的性质。故不满足强性质的是**类**，不是当前实例；在该 open 闭合前，不得声称本类的动机链已按 active Frozen 成员资格执法。
...
**Scribe 收据刷新事务(总单 #5533 第 9 层)。**`align-scribe-receipt --refresh --documents FILE --base REV [--dry-run]`(`make scribe-refresh DOCUMENTS=file [BASE=rev] [DRY_RUN=1]`)从严格 UTF-8、LF 终止、逐行 canonical Formal 模块 GID 的选择文件,经账本依赖投影展开为去重排序的 `(atom_id,gid)` 对;直接 pair 入口与 document 入口共用同一个 planner。每对必须在当前账本与 protected base 各恰有一个 atom、同 target 的 coverage 边及同 GID 收据;GID 可选 Lean 模块或 declaration,后者另须 verified declaration reference;任一 identity 缺失、目标非 Closed、发射未验证或定义/发射路径不匹配即整批拒绝、零写。刷新只替换已有 Scribe 收据,绝不增删或改写 coverage 边;新收据必须由 cover/seed 共用的 `DigestionReceiptBuilder` 从当前 definition bytes 与 verified emission 构造,不得由 refresh 自算 fingerprint 或以旧收据 `with` 复制出第二套 payload policy。事务以 `DigestionStatusEvaluator.ExpandStatusAuthorityChanges` 唯一派生被选 atom 的传递 `chain_atoms` 祖先闭包,只在该闭包内重投影 evaluator 派生状态,并以精确 write set 覆盖所有状态迁移 shard;闭包外状态变化即 `status-change-outside-refresh-closure` fail-closed。最终 receipt-integrity 与 projected-status 校验覆盖选择项、同文档依赖项、机械派生祖先及实际写集,随后才作一次并发字节复核和原子写入;`--dry-run` 走同一 planner/校验并只向 stdout 投影 expanded pair、`needed|noop`、entry path 与 write set,树中零写且不持久化时间戳。`cover-atom --align-scribe-receipt` 的 already-covered resume 分支消费同一 transaction;alignment 返回拒绝时必须保留其诊断并返回 `cover=resumed align=failed`,不得把零写拒绝冒充 `align=passed`。
...
- **v7.x**(2026-09-07,总单 #5533 第 9 层实现):§11.21 增 `align-scribe-receipt --refresh` / `make scribe-refresh` 的 document→pair 规划、protected-base identity、module/declaration GID、canonical receipt builder、chain 祖先状态重投影、精确 write set、闭包终验与 dry-run stdout 契约;already-covered 的组合 cover 对 refresh 拒绝 fail-closed 传播。
