...
**G · harness 存规则,不存代表元(商结构律,用户 2026-08-08 定)。** harness 是**商集上的函数** `H: X/~ → 判词`,不是**代表元上的函数**外加一张实例表。**判据**:改一个实例,若被迫编辑 harness,则该 harness 里存的是数据而非结构。现状反例即在信任根:`TOWER.yaml` 的 `c0` 组件**手工声明 36 条 `(path, git-sha1)`**(其中 21 条集中于 `StrataLint.Cli/Conservative/**`),故改任一 controller 的一个字节即令其中一条过期、必须编辑 TOWER 并重做约 25 min 仪式(2026-08-08 实测:止血仅改 `scripts/local-harness-gate.sh` 一个文件即触发)。**应然形**:TOWER 声明 controller 闭包的**成员规则**(目录闭包 + 具名 entrypoint 清单)与该闭包的**单个派生 digest**;实例字节变化由派生吸收,harness 一行不改,变更可否接纳交 base judge 判。防候选自证由**规则本身 base-owned 且不可被候选覆写**保证,而非由抄写 36 个哈希保证。本条与 F 互补:F 治"同一输入不该重算",G 治"改实例不该改 harness"。〔守护:**软 + 硬投影**·"声明式实例表 vs 规则+派生 digest"可由 schema 机器判;成员规则是否恰当靠 base 侧评审〕
...
**执法分级(R8，实测逼定)**：上段的完整 env 矩阵按第 20 条分为两道，**检测机制不降级，只按成本分层**：

- **required lane（每 PR 必跑，接入 `make gate`/`make preflight`、CI `candidate-engineering`、`baseline-admission`、C0 两阶段与双 required checks）**：全部 192 个 packaging/consumer 协议 case（不含重建，成本可忽略），加 **恰 2 次真重建**——两个独立 clean checkout 在**唯一 canonical env**（locale `C`、timezone `UTC`、顺序 `canonical`、并发 `1`、`SOURCE_DATE_EPOCH=0`）各 rebuild 一次，逐字节比较七项 artifact 的 identity/path/mode/sha256/bytes。这一道承载**出库所依赖的那条命题本身**：七项投影可从源在独立 clean checkout 中确定性重建。任何不符 fail-closed。
- **canary lane（周期性，非 required）**：其余 94 次真重建覆盖 locale/timezone/顺序/并发/clock 的完整 metamorphism，带外调度；发现的不一致按第 20 条**入案号追踪至闭合**，不阻断 PR。

**分级依据是实测而非偏好**：2026-08-08 于实施机测得单次真重建 ≈ **165 s**，96 次真重建下界 **≥ 4 h 24 min**（不含 setup 与 192 次发布/消费）。本仓 `dev` 平均前进间隔约 **50 min**——**一个耗时超过主干平均前进间隔的 required check 永远无法通过**：分支在检查跑完前必然 stale，须重新 merge、重算派生物、重跑，形成不收敛的循环。故完整矩阵作 required 不是"慢"，是**结构上不可满足**；必须分层。

**禁静默截断**：verifier 的 JSON 输出必须显式记录 `lane`、`real_rebuilds_run`、`canary_deferred_count` 与被推迟的 env 元组集合。少跑了什么必须写在读数里，不得让"通过"看起来像"全覆盖"。canary lane 的存在是本分级成立的前提——**取消 canary 即取消检测，属第 20 条红线，禁止**。PR-A 同 PR 迁移全部已证 consumer、停止跟踪七项 bytes、加精确 ignore；不动 Blueprint、BACKFILL、ledger。
...
- **v7.14 R9 续 G**(2026-08-08):补商结构律。用户指出 harness 本质是商余结构——有共性的才是 harness,若"文件改了 hash 变了还得改 harness",那存的就是数据不是结构。实测证实信任根即反例:`TOWER.yaml` 的 `c0` 组件手工声明 36 条 `(path, git-sha1)`,改任一 controller 一个字节即须编辑 TOWER 并重做约 25 min 仪式。应然形为"成员规则 + 单个派生 digest",实例变化由派生吸收;防候选自证靠规则 base-owned 不可覆写,而非抄写哈希表。与 F 互补:F=不重算已判过的输入,G=不为实例变更改 harness。
