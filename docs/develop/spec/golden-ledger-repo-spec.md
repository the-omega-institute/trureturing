...
治理指令 `CLAUDE.md`、`AGENTS.md`、`skills/**` 由 `repository` 的具名仓库检查验证,同时执行 `current-metadata`、`delta-data`、filemap;普通 `agents/**` 只执行后三项。已登记的 `merge-gate.sh`、`openproblem/erdos617.py`、`openproblem/standing-check.py` 与 `TARGET-GATES.md` 只需 `current-data`、`delta-data`、filemap,不宣称拥有现役 CI 行为测试。`Meta/registry.yaml` 与 `Meta/domains.yaml` 以 `repository` 验证实际登记契约,以 `current`/Scribe 验证候选语义,以 `delta-registry` 保留 SL-016 的真实报告证据;不请求无关固定夹具、selftest 或反证编译。当前容量材料包含上述受管文本与脚本字节,有效缓存不能掩盖新违规。
