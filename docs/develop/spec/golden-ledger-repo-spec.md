...
The fixed Registry also records the raw `lean-report-inputs.json` SHA in each real module's `information_templates` inputs. Changing those bytes invalidates the captured evidence even when `report_semantic_version` is unchanged; a compatible native trace cannot override source-bound evidence validation. The explicit statements-only test mode omits this partition and does not establish reuse of real template evidence after manifest edits. Collection repairs privately reconstruct rejected row artifacts and validate all reconstructed files in one native invocation before aggregation; standalone public facets validate their own reconstruction immediately. Any rejected reconstructed material fails the stage.
...
The optional `report_execution` manifest declaration names the supported tools (`lake`, `lean`), platform coordinates (`system`, `machine`) and explicit semantic environment variables. Its optional `toolchain` registration declares an exact `pin` matching the registered `lean-toolchain` input and platform-specific complete tool version strings. Executed production verifies those strings against the actual tools before sealing reuse evidence. When tools are absent, probe and normal reuse compare the prior receipt against candidate-declared identities; donor-reported versions are never the authority. Unregistered platforms, mismatched pins or semantic environments request normal production. The common producer activates unavailable tools only after rejecting reuse, including corruption discovered after a successful probe; installation and actual compilation failures remain failures. The normal Inspector entry may seal a `.reuse.json` sidecar only after successful native `:report` (including ordinary defaults/audits) and complete publication. It binds the complete declared source/producer/configuration inventory, file modes, execution environment and five-file report. Reusing this successful invocation requires all registered inputs to remain identical and the complete current-source/material validator to pass on a private snapshot. An absent, old, damaged or mismatched receipt requests native Lake work; invalid registration and actual build/check failures remain failures. Producer implementation changes prevent reuse of the complete invocation, but do not change the semantic version or invalidate compatible per-module artifacts: the native incremental path retains its existing semantics.
...
治理指令 `CLAUDE.md`、`AGENTS.md`、`skills/**` 由 `policy` 的具名政策检查验证,同时执行 `current-metadata`、`delta-data`、filemap;普通 `agents/**` 只执行后三项。已登记的 `merge-gate.sh`、`openproblem/erdos617.py`、`openproblem/standing-check.py` 与 `TARGET-GATES.md` 只需 `current-data`、`delta-data`、filemap,不宣称拥有现役 CI 行为测试。`Meta/registry.yaml` 与 `Meta/domains.yaml` 以 `policy` 验证实际登记契约,以 `current`/Scribe 验证候选语义,以 `delta-registry` 保留 SL-016 的真实报告证据;不请求无关固定夹具、selftest 或反证编译。当前容量材料包含上述受管文本与脚本字节,有效缓存不能掩盖新违规。`policy` 与 `repository` 是独立完整测试项目,政策文本不请求 C# 源图/Lean 预算检查,普通 Lean/Blueprint 不请求政策契约。既有 formal-answer 锚点检查单列 `anchors`,由 D5 Lean 与该 skill 显式请求,不复制检查或按用例过滤;完整 `engineering` 登记三者。
...
工程分类、程序集身份、CI 执行成员、生产属主、测试债务分区、项目引用和 Compile 源 include/exclude（含共享链接）的唯一数据真源为 FILEMAP 登记的 `Meta/engineering-projects.json`。engineering、拓扑与候选工程证据消费同一严格 reader；禁止从项目/目录名称、`IsTestProject`、xUnit 引用、SDK、MSBuild 求值或源语义自行发现这些事实。登记 glob 可在已跟踪源上展开；缺失、重复、未覆盖输入明确失败并补登记，不作全仓兜底。完整测试项目并发上限只取候选清单必填正整数 `test_parallelism`，不探测宿主 CPU；各项目独占预分配 TRX 地址，全部进程结束后按项目原确定顺序验证与写证据，失败或取消不产生成功工程封印；base 项目数据投影不要求该候选调度字段。当前登记值 2 是 policy-override，不是容量派生值（2026-09-19；域=登记的完整工程测试项目并发；永久案 [#8640](https://github.com/the-omega-institute/trureturing/pull/8640)，owner=仓库 τ=0 owner）：原生阻塞握手已验证 1/2 槽及非法配置拒绝，既有串行 CI engineering 1783 秒、其中测试执行 1598 秒；并行真实负载已在集成运行验证，完整项目覆盖与证据保留；内存峰值尚未测量。runner 资源或完整项目负载登记变化，或出现 OOM、并发冲突、调度超时即复审，按实测调整或撤销该登记值。拓扑债务身份、集合包含/严格收缩棘轮、ScriptTests CI 排除与两项反证编译保持。
...
共同检查的 `Meta/ci-checks.json` 使用 `ci-check-input-registration-v3`；每项以必填非空 `program_inputs` 显式登记影响本项判词的程序路径或 glob。`program_projects` 保留已登记项目、引用及构建配置的身份，不把项目全部源码自动并入检查指纹。每个 `report_inputs` 必须显式登记 `producer`、`consumer`、`artifact`、`materials`。`consumer` 指向 FILEMAP 登记的 `report-consumer-inputs-v2` 清单，其封闭字段为 `schema/producer/projects/program_inputs/materials`：producer 须与引用一致，项目须已登记，非空程序输入与材料须显式存在、无重复；缺项、冲突、未知字段或必需字面路径缺失在执行前失败，不动态补全或退回全项目。共享 consumer 的程序输入按同一规则展开，不经其 projects 重新带入全部源码。

检查指纹绑定本项登记、显式程序与数据输入、路径集合、项目构建配置、已验证报告内容及执行环境。改同程序集内仅被 A 登记的源码只使 A 失效；共享源码只影响其显式消费者；程序输入的新增、删除与 mode 变化仍参与计算。完整编译、测试项目覆盖、候选 DLL、round 与材料验证不变，复用单元保留原执行来源；新执行失败不得由旧成功证据替代。新指纹契约不接受旧契约收据。报告消费者不绑定仅负责生产或传输报告的脚本字节；生产清单仍完整验证，报告源码/内容/材料/兼容性/运行时身份验证不变。原生 `lean-report-inputs.json` 和消费者接受材料仍进入消费地址，Scribe 下游引用须与 describe 单元的 producer 和 consumer 一致，保留其实际材料与原执行来源校验。
...
Actions current 的物理缓存分为完整 `current` 与无报告 `checks` 两种 profile，二者共用上述 mathlib/OS/arch 分区规则，各有独立 key、目录与运输身份。`filemap`、`current-data`、`current-metadata` 只请求 `checks`；`lean-report` 自己声明 `current`，不借 filemap 携带报告种子。混合资源按登记取并集。`checks` 仅保存 `Meta/ci-checks.json` 中 `report_inputs` 为空的 current 检查及其原始材料，不保存 producer report 或报告绑定检查；导入仍校验候选登记输入、环境、原执行来源及全部材料。小检查保存不得替换或遮蔽完整报告种子；完整 profile 保持既有报告验证与增量 producer。普通 artifact 与这两种可选 seed 分别运输，不把普通运行时材料计作报告开销。本地 `automatic` 导出仍保留完整 current 种子；CI 使用 `deferred` 交给上述 profile 运输，不改变检查集合或报告判词。
...
FILEMAP `schema_version = 4` 的每条资源登记含 `cache_activation` 表，键集合须与 `cache_layers` 完全一致，支持 `stage-start` 及仅限 current 的 dependency/project/elan 的 `report-miss`。同一 stage 内同层阶段须一致；不同 stage 可独立登记。缺项、额外项、未知阶段或阶段/层不符由 C# loader 与轻量 planner 同样拒绝；无工作仍须验证完整登记。Actions 先恢复 stage-start 的 current 证据，再由 A14.9 的正常 producer probe 计算已登记资源的激活集合；可复用时不下载 report-miss 的 dependency/project/elan，也不安装 Lean 工具。缺失或拒绝种子时恢复对应层并进入正常生产；probe 后损坏由共用 producer 入口激活缺失工具并重建。`.lake/build/lean-inspector` 随 project 一起运输，无独立 report 缓存层。filemap 等无 Lean 资源的路径仍不物化 Lean 缓存。
