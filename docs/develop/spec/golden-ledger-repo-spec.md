...
**E7 — producer evidence.** The private `CheckedTemplatePlan` constructor alone publishes enrollment evidence. Its data retains schema/grammar/compiler/toolchain identities, definition and enrollment owners, rigid telescope and slot types, constructor descriptions, raw type/body SHA-256 identities, dependency inputs, checked body/type plans, retained proof/type obligations, charged work and rule IDs. Raw identity uses domain-separated, length-prefixed Expr/Level serialization with anonymous binder names and positional universe parameters; it preserves binderInfo, metadata, let structure, instances, literals and argument order, and does not reuse a pretty-printed statement hash. Dependency identities contain canonical Name, actual declaring module, raw type identity and executable body identity; independent proof implementations contribute no body identity. Source and imported-artifact checks bind evidence to the actual producers. Recompilation reissues evidence; rehashing an old plan against current source cannot certify it.

The persistent `TemplatePlanFrame` contains canonical Name bytes, a token-interned payload and its SHA-256 identity. The current marker is `DTR-checked-plan-v3`, with extension identity `checkedPlanFramesV3`; the decoded plan's schema, grammar and constructor-recursion versions are each 1, and its private compatibility coordinate is 5. These are distinct from the module report semantic coordinate in A14.9. The decoder accepts exactly this layout and version tuple. Literal tokens use canonical decimal UTF-8 byte length, a colon and exact bytes; repeated tokens use backward-only `@<first-literal-index>:` references. Duplicate literals, leading zeros, invalid UTF-8, unknown markers/references and trailing bytes reject. The fixed-width charged-work literal is outside interning. Both serialization passes debit token processing and emitted bytes and must agree on work. Imported ownership is checked against the actual extension container and Lean declaration-to-module table, not supplied owner strings. No older frame layout is an alternative reader path.
