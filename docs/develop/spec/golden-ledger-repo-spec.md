...
The two optional escape clauses follow variation/sensitivity (or output evidence), and precede the statement colon for `information_theorem`. All new words are non-reserved. Old-form registrations keep their parser and producer status; the delta rule imposes the four-slot obligation. At least one complete verified registration is required for each newly authored public theorem/lemma declaration in a selected D5 source module selected by the existing delta/first-pin scope, and the author must deliver that registration, its compiled proof and current binding evidence in the same mirrored Reg change. D5 content-template declarations remain in this scope; helper declarations in `Reg`, `Interface` or `Impl` do not create a recursive Reg/D5 audit target and retain their existing kernel, `sorry` and axiom checks. A lawful existing template must be reused; if none applies, the D5 content owner adds a meaningful parameterized template and its shared enrollment in `Reg/Support` before the mirror registration. No forced template fit, judge relaxation or bind-only D5 wrapper is allowed. Multiple registrations on different arenas coexist under their original occurrence keys; there is no completeness or naturalness judgment. A selected Reg module with an empty inventory still requires current producer evidence; new public D5 theorems use the separate mirror lookup. Other admission obligations remain independent.
