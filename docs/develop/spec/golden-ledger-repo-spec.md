...
CI restores and saves native `.lake/build/lean-inspector` artifacts with the project snapshot in the resolved mathlib revision partition. Native Lake validates traces and completes the current default Lean/audit targets on every report invocation; a seed never skips that obligation. Current-stage transport binds the complete five-file publication to the candidate and build round. Release selects the fixed dev source’s successful push engineering/current checks and validates their current-stage report artifact. There is no independent report cache/store, report preparation shortcut, baseline adapter or delta/cohort planner. FILEMAP selects resources; Scribe current checks consume `Meta/ReportProducers/scribe-content.json` and never read base.
