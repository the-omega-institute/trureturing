...
**封闭 front matter 恰五键**：`slug`（须等于文件名主干且合 canonical slug）、`bibkey`（须为 canonical bibkey）、`doi`（**完整 DOI**，由 `Doi.TryCreate` 判语法，其正则恰为 `^10\.[0-9]{4,9}/\S+$`，故裸 arXiv id、`arXiv:` 前缀与 `https://doi.org/` 前缀皆被拒）、`triage`（封闭字母表恰为 `theorem|window|wall`，与 §11.20.2 Theorist 契约同表）、`motivation_gids`（非空、无重复的 GID 列表）。缺键、未知键、非规范值、BOM、CR、非 strict UTF-8 均 fail-closed。**owner 2026-09-06 裁决**：外部开放问题的准入身份是 **DOI**，不是 arXiv id——arXiv 只是外部发表面之一，DOI 是这些面共用的那一个；故独立键 `arxiv_id` 由 `doi` 取代，二者不并存。**expand–migrate–contract 的中间态如实记账（第 6 条只禁永久 expand，不禁三段式）**：J2a（#5964）令 loader 暂时同时接受把 `doi` 换成 `arxiv_id` 的旧五键集，本轮（C1a）迁完全部现役卷宗，**J2b 必须删除 `ProblemCandidateCatalog.LegacyRequiredKeys`、`ArxivIdPattern`、`ProblemCandidate.ArxivId` 与 `ValidateProblemSource` 的 `OrdinalIgnoreCase` 分支**。在该 contract 合入之前，机器上仍有一个旧形入口，**本节所述的封闭键集不得称已唯一**；`ProblemCandidateCatalog.RequireDoiForChangedDossiers` 只保证「新增或改动的卷宗必须用 `doi`」，不保证存量已迁——存量已迁由本轮的实际字节承担，不由该谓词承担。
