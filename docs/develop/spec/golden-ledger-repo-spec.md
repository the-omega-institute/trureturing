...
**A16 零信任合并门(v7.14,CLAUDE.md 第 17/19/21 条之 spec 形)** 提交者身份(维护者/agent/fork)与准入无关,一切 PR 过同一道纯机器门:dev 分支 `enforce_admins` + **双 required check(strict)**——① engineering(build --warnaserror + 全测试 + selftest 字节比对 + 能力链编译证明);② baseline admission(`pull_request_target` 固定取 base 侧 workflow 与 gate)。**内容层与元层严格分路**:`check --protected-base` 对所有候选始终由内容寻址的 dev-baseline `JUDGE_DLL` 执行;rc≠3 时永不执行 candidate verifier,故内容 PR 的提交者改不了判内容的法官。只有旧法官已把变更完整分类为 rc=3/SL-022 后,base gate 才构建 `CANDIDATE_DLL`,并由它驱动 `verify-conservative`,同时逐字传入 `baseline-harness=JUDGE_DLL`、`candidate-harness=CANDIDATE_DLL`、两侧冻结 identity/report 与 base-owned corpus;这是第17条“新 harness 判前 harness”的元层路径,不参与普通内容 admission。候选驱动不等于候选可任意放行:机器在冻结 base-owned corpus 的每个 case 上强制 `base admit => candidate admit`,并要求每条 active `Block/HumanGate` 规则保留至少一个 base-owned blocking witness;候选翻旧 admit 或丢尽红例均出 `CONSERVATIVE_VIOLATION`,输入/case/root/replay 漂移均 infrastructure fail-closed。绿=auto-merge;红=无人可合。**exit 语义**:0=内容全验;1=违规或保守性破坏;2=基础设施;3=SL-022 保守扩展证书已发射,由 local/CI 外层按既有协议记为元层成功。人审与 AI 审=质量增益,非准入权威;本驱动器选择自身属 τ=0 bootstrap,只备分支与全验材料,须 owner 解锁后再经正常全成本门合入,不得以 admin/protection 绕过。
...
**A19.1 负例地板与观察期义务(v7.14)** 负例地板只覆盖 `RuleLifecycle.Active` 且 `AdmissionEffect` 为 `Block` 或 `HumanGate` 的规则:此类规则每条都必须在 base-owned Golden corpus 中有真实 blocking witness,缺失即 infrastructure fail-closed。`Observe` 规则仍属 active、照常执行并报告 warning,但其语义结构上不参与 admission 阻断,故与 SL-007/009/014 的 deferred 规则同样递延地板义务;不得为了凑红例把构造器、typed loader 或 emitter 的独立 fail-closed 冒充该规则的 blocking witness。规则从 `Observe` contract 为 `Block` 时,contract PR 必须同时加入该规则的 base-owned blocking fixture;合入后下一基线立即把它纳入地板。该递延只校正不可满足的见证义务,不关闭、不软化任何检测路径。
...
诚实分栏声明:P0 的 local gate/preflight 自动落本机账;现行 CI 直接运行 base-owned shared gate,不把 runner artifact 回流至该账,故 `venue=ci` schema 能力不等于已有 CI 样本,`make perf-report` 当前不得冒称 CI 基线。**P1 预算义务会计(待升提,非现役)**:预算实例必须登记 cohort/workload/stage、metric、owner、修复动作、误报率窗、临时撤销/回滚判据;先由只读 comparator 发 warn,数据成熟后才建独立 PR check,再以至少 30 天/30 个成功同栏样本、连续 14 天误报率 <5%、相对同栏 baseline 超界 ≥20% 且独立复跑 2/2 仍越界为义务收据,逐个预算升 required;任一义务缺失只许 warn,不得批量升红或并入 admission。**P2 优化收据(待升提,非现役)**:性能优化 PR 只有在 before/after 同 cohort、同 workload、同统计口径,各自原始事件集合与汇总内容寻址,并同时记录失败/副作用与 commit 身份时,方可声称“已获益”;无收据只能称候选或实验,不得以单次最快值、跨机样本或不同缓存态领功。P1/P2 的机器、预算数据、snapshot 与 receipt 本轮均未建立。
...
## 11.21 回填溯源清单(消化完整性)
`Meta/BACKFILL.yaml` 是 **Digestion Ledger** 唯一真源,现役且仅现役 schema 为 `schema_version: 3` / `ledger: theory-digestion-v1`;旧 anchor/disposition 格式经一次迁移只存于 git 历史,运行时无兼容读者、无双读。每个 source 恰含 `{source_id,path,atomizer,entries}`,其中 `source_id` 全局唯一、文件正名且文件名禁空格;每个原子 entry 的共同必选字段为 `{atom_id,fingerprints:{raw_sha256,normalized_sha256},coverage_gids,receipts,status}`,边界字段互斥二选一:嵌套 `boundary:{ast_path,start_byte,end_byte}` 或顶层 `ast_path`;另可选含 `cas_ref`。canonical 账本现状另述:933 条 entry 中 921 条采用顶层 `ast_path`、12 条采用嵌套 `boundary`,且 933/933 均携 `cas_ref`;THEORY-ERRATUM(11.27)的案件主键 `(cas_ref, atom_id)` 只对携 `cas_ref` 的理论收据成立,无 `cas_ref` 收据不在该事件类内。raw 指纹绑定原始字节,normalized 指纹只容许 UTF-8 BOM、CRLF/CR→LF 与 Unicode NFC 的受限规范化;二者均为 `sha256:<64 lowercase hex>`。
...
- **v7.14 R4**(2026-07-20):关闭 exit-3 判官自锁:内容 `check` 继续只由 base `JUDGE_DLL` 判;仅旧判官已返回 SL-022/rc=3 后,改由 `CANDIDATE_DLL` 驱动双 harness 的 base-owned conservative replay,兑现“新 harness 判前 harness”。negative floor 同时从“全部 active descriptor”精化为“active 且 effect 可阻断”;Observe/warn 继续执行与报告但地板义务递延,Block/HumanGate 仍逐规则强制 base-owned 红例。门级身份测试锁住 base-content/candidate-meta 边界,verifier/command 红例锁住旧 admit 翻转与 blocking witness 丢失;本变更只恢复元层证明通道,不改变 rc≠3 内容准入或既有拒绝能力。
