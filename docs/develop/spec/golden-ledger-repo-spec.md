...
**A5.5 Declared-template debt (SL-031).** `DeclaredTemplateBindingRule` is a named obligation of SL-031, alongside the utility obligation in A5.1; its domain is defined here independently. It is affected when activation exists in protected or candidate data and the rule implementation, D5 paths, debt paths, frozen-state paths or FILEMAP change. `Golden/InformationTemplateDebt/activation.json` has exactly `{schema_version:1,seed_base:<lowercase 40-hex immutable commit>,activated:<Boolean>}`. The candidate rule and seed writer read effective activation and seed authority only from protected-base data. Candidate bytes cannot choose the current run's authority. Installation from a base without activation permits only an inactive, row-free mechanism. After installation, missing/malformed activation fails closed; seed retargeting, deactivation and noncanonical changes reject. The only activation transition is the canonical active encoding of the protected seed.

The authority order is **inactive row-free mechanism → independent seed-only content change → judge activation → content declarations/discharges**. Once activation exists in protected-base data, inactive row-free evaluation requires complete current producer evidence and no declarations. First installation checks only the inactive, row-free mechanism. Seeding reconstructs the original seed's complete occurrence inventory; every original occurrence must be undeclared and have an exact row binding its statement, registration source and content inputs. Initialization cannot replace rows, enroll, discharge or run while active. The initialization writer may take the exact current immutable commit OID as its installed protected base, including a clean checkout, because it creates the first delta; other revisions and writer operations retain ordinary ancestor/non-vacuous preparation. This is not an admission exemption. Requested activation with empty rows still reconstructs original, protected and candidate evidence: only an originally empty inventory can activate without seed rows. An active empty debt set cannot be initialized again.

Rows occupy `Golden/InformationTemplateDebt/rows/<key>.json`, where `<key>` is lowercase SHA-256 of `DTR-occurrence-v1\0` followed by the canonical JSON array `[root,registration_module,theorem,object_arena,catalog]` without a newline. Each row has exactly `{schema_version,key,seed_base,base_statement_identity,base_registration_source_sha256,base_content_inputs,reason}`; schema is 1, reason is `undeclared`, and every seed equals protected activation's seed. Hashes are lowercase, Name components canonical, and inputs nonempty, unique and strictly ordered. The strict store checks canonical UTF-8 JSON bytes, filename/key identity and immutable seed input hashes. The writer accepts only the registered activation path and row layout, rejects links/unexpected entries, rechecks the complete input snapshot under its writer lock, and performs atomic per-file replacements with batch rollback on failure. An interrupted partial set cannot satisfy admission's residual equality.

When active, the candidate debt key set must be a subset of protected debt, surviving rows must be byte-identical, and removing a registration does not discharge its enduring obligation. Debt must equal the independently collected set of candidate `undeclared` occurrences. Every candidate-new occurrence requires a validated declaration; every unresolved declaration rejects. Every touched debt occurrence must discharge and any such migration must strictly shrink the debt set. A touch includes changed statement/state/binding owner, a changed binding source, changed/missing retained content inputs, or first freeze of the registration source or an authoritative content input, including an imported theorem owner. Judge/compiler/toolchain changes alone refresh evidence without becoming content touches.

While debt remains, the selected certificate-checking domain contains new occurrences, touched debt, changed content/binding inputs and occurrences affected by first freeze; `Golden/Frozen/state/<module>.lean.json` maps back to its source module. When debt is empty, the same rule selects the complete occurrence universe automatically. No completion flag, compatibility branch, corpus exemption or second activation authority is permitted. Current and historical evidence must pass the same source-bound reader; historical content stays immutable while configuration/toolchain and producer inputs come from the current candidate, with base used only as data.

The closed allowlist below contains 12 direct SL-031 finding names and the `DTR-DebtSchema` inner diagnostic nested under `DTR-Evidence`; none is a new SL rule number. All failure findings Block. The outer evidence boundary may report `DTR-Evidence` carrying a named inner failure; `DTR-Inactive` and `DTR-Domain` only Observe scope.

| Finding | Judgment |
| --- | --- |
| `DTR-DebtSchema` | Canonical schema, key/path, reason or input format/availability is invalid. |
| `DTR-Activation` | Installation, authority, activation order or permitted transition is violated. |
| `DTR-Seed` | Rows do not exactly bind the protected original undeclared inventory and seed inputs. |
| `DTR-Inventory` | Registration/event/record keys, owners or governed-source coverage disagree, or an enduring occurrence is removed. |
| `DTR-New` | A candidate-new occurrence lacks a validated declaration. |
| `DTR-Subset` | New/replacement debt appears or a surviving row changes. |
| `DTR-Touched` | Touched debt survives or its migration does not strictly shrink debt. |
| `DTR-Residual` | Debt differs from independently collected undeclared occurrences. |
| `DTR-Evidence` | Current/historical evidence is missing, malformed, unresolved, stale or lacks a selected source-bound certificate. |
| `DTR-Routing` | The complete batch is not content-only under both protected and candidate FILEMAP, or the classifications disagree. |
| `DTR-Required` | The installed activation mechanism is missing. |
| `DTR-Inactive`, `DTR-Domain` | Observe the inactive phase or completed consumer scope; neither authorizes admission. |

Whenever D5 content or debt rows are touched, routing evaluates the complete changed-path set under both protected and candidate FILEMAP; both must admit it as content-only with identical classification. It also checks the content subset under both policies. Judge-only installation/activation remains a separate batch. Template declarations and debt rows are content-plane data; the activation file is judge-plane data, as specified in A18.
...
The persisted binding input closure is the requested content source closure plus the configuration/toolchain inputs specified in A5.4. Judge implementation paths are excluded from that closure and do not become binding validity inputs merely because the fixed driver executes them. Native validation still checks loaded judge artifacts when producing a report. Historical reports are read against immutable historical content with current producer/configuration/toolchain inputs; rehashing current files around an old loaded environment does not supply source-to-native evidence. Lake's trace hashes provide cache coherence, not cryptographic compilation attestation: the pinned compiler and its outputs remain within the native trust boundary, while SHA-256 snapshots detect subsequent input replacement.

The exporter emits an independently source-bound `information_templates` partition for every requested module. Native row reuse and final publication validate complete current membership, captured source inputs and material integrity; for a present `information_templates` partition, native validation checks its top-level shape and current input hashes, while the required C# consumer rejects missing evidence and malformed records. `inspect.sh` delegates both unchanged and changed work to native Lake report facets and propagates phase failures. C# separately performs A5.4's complete occurrence/sidecar/ownership join and A5.5's debt judgment. Input digests may be reused only within one immutable `RepositorySnapshot` identity; replacing a snapshot requires a fresh index, and every record still checks fields, ordering, availability, symlink exclusion and digest equality. No repository-path/revision-only digest cache may certify changed inputs. The explicit `--statements-only` fixture operation omits `information_templates` and cannot satisfy the declared-template consumer.
...
Declared templates and their binding declarations belong to the content plane; their Lean files retain FILEMAP's Lean-content kind. `Golden/InformationTemplateDebt/rows/*.json` is `kind=data`, `admission_plane=content`; `Golden/InformationTemplateDebt/activation.json` is separately registered `kind=data`, `admission_plane=judge`. Both use `InformationTemplateDebtWriter` as producer and `InformationTemplateDebtStore` plus `DeclaredTemplateBindingRule` as consumers/verifiers, with `artifact_id=none` and `runtime_disposition=committed-source`. A5.5 owns their authority order and checks the complete content batch under both FILEMAP policies. The specification itself is `kind=program`, `admission_plane=judge`; it cannot share a PR with template declarations or debt rows.
...
current 对原生报告的五个发布材料、可选 `.reuse.json` 完整调用证据及本轮成功步骤作候选/run/attempt 绑定。Actions 的 dependency/project 快照只有在本轮接受的 current 证据证明所选 `lean` 或 `lean-report` 步骤成功后才获授权；逐项检查材料哈希与身份，不读取已退役的报告准备收据，也不把 Lean 重编数量当作构建义务是否执行。缓存检查与制作放入共用 bounded worker，失败或超出剩余窗口仅跳过可选保存，不能改变已经验证的业务结果。检查证据优先保存，其后项目与依赖层；通用显式运输接口保留自身生产成功契约。
