...
**非命题终态(owner 2026-09-06 定,#5770 方向③)。**`receipts.nonpropositional` 的键集恰为 `{justification, previous_atom_id, next_atom_id}`,后两者为裸 64 位小写十六进制 atom_id 或 `null`(`null` 只表示已核实的源首/尾边界);该收据与非空 `coverage_gids`、`receipts.quarantine`、`receipts.cover_disposition` 互斥,loader 对任何共存或错形一律 fail-closed。该状态是无需 coverage 的终态,不再承担形式化义务,亦不计入可形式化分母。`settle-atom` 是唯一 writer:其 strict TOML request 键集恰为 `{atom_id, justification, previous_atom_id, next_atom_id}`,源首/尾边界在 request 中写 `source-boundary`;写入前置为当前 entry 恰处于 `residual-open`、coverage 为空、无 quarantine / cover_disposition、`unresolved_subitems` 为空,且全部 `chain_atoms` 子项已为 absorbed 或 nonpropositional。writer 以 `DigestionAtomContextProjection` 复算真实邻居,任一 request 邻居不等即 `SETTLE_INVALID CONTEXT_MISMATCH`;有效写入经 `BackfillInventoryWriter` 序列化、loader 逐字节回放,再由 `ApplyLedgerUpdatesAtomically` 一次完成旧 shard 删除与新 shard 写入。`--clear` 只删除该收据并原路退回 `residual-open`;无 `--replace`。若任一链祖先承载 coverage,writer 打印 `SETTLE_ALIGN_REQUIRED ancestors=…`,内容 PR 须先跑 `align-digestion-status`,祖先 stored 与 derived 不等仍由 SL-016 判红。
...
- **v7.28**(2026-09-06,#5770 方向③,owner 裁决):第四种消化终态 `nonpropositional-inapplicable`、`receipts.nonpropositional`、`settle-atom` / `atom-context` 与链闭合谓词落入 §11.21;查询时前沿投影不变。
