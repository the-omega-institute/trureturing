...
**A14.8（retired historical record）升级门许可集勘正(v7.16 R21)** `Supersede` 的新闭包判据已原位改为 candidate `axiom_closure ⊆ LeanAxiomFacts.StandardAxioms`;历史 protected-base 闭包既不参与 admit/reject,也不因 unknown 而拒绝。合成测试已钉住 8 个许可子集全部放行、每个子集加入非标准字符串均拒绝、窄 base `{}` 到 `c={propext}` 放行、生产入口的合法/非法可达性、历史闭包 8 子集+空值+unknown 的 metamorphic 不变性,以及缺失 candidate 闭包的显式 fail-closed;这些是许可集谓词、可达性与失败传播的证明,不是跨时间证据。

退役时本条仍记 `open(F4[2]-REAL-PIN-BUMP-E2E)`:当前 dev `a965c9737` 语料实测 `Supersede=0`,`Revoke=0`,所以合成正负例**尚无真实 pin bump 的端到端收据**,不得表述为「已在真实语料验证」;建议另开新单并在正文引用 #1947,不得复活该已卡死工单。以下两项只作 `ASSUMED-UNVERIFIED`,不冒领为结论:① candidate `payload.AxiomClosure` 与新环境独立重算结果之间的不可伪造性尚缺真实重算/绑定收据;②所有 pin/material 变更路径是否都到达该门尚缺全路径枚举与生产运行收据。治理级反例亦保留:若同时修改 `StandardAxioms` 与 `TruthDagTests.StandardAxiomAlphabetIsPinned` 的期望值,单元钉子不能拦住,只能由 τ=0 评审约束。
...
**A21 性能账(v7.14,P0+P1 warn-only 现役;P2 待升提)** P0 事件封套唯一 schema 为 `stratalint-perf-event-v1`:`{schema,run_id,ts,cohort:{venue,os,arch,cpu_class,runner_class?},context:{commit,base,workload_id,cache_state?,loadavg_per_cpu,host_concurrency},kind,stage,status,elapsed_seconds?,resources:{disk_free_gb?,fd_peak?,rss_peak_mb?},role?,pid?,elapsed_ms?,rc?,fd_peak?,rss_peak_kb?,concurrency_count?}`;尾部七字段仅供逐进程 `kind=resource` 事件保留宿主原始读数,与 `resources` 中的跨 workload 汇总读数并存。`venue∈{local,ci}`,`kind∈{timing,resource}`,`status∈{passed,failed,skipped,observation}`。`commit`、`loadavg_per_cpu`、`host_concurrency` 任一缺失时,唯一 writer 必把 status 强制改为 `observation`;不可比读数可以留账,不得进入 p50/p95 或回归判词。比较栏键为完整 cohort `(venue,os,arch,cpu_class,runner_class)` × `workload_id` × `kind` × `stage`;**禁跨栏基线、百分位或回归**。只有 `passed` 是可比延迟样本;`failed`、`skipped` 与 `observation` 各自单列,不得用 skipped 的零秒或失败前耗时压低分位数。`make -C tools perf-report` 只读运行账,以 nearest-rank 汇总各栏 p50/p95、可比样本数、最近 N 趋势与三类排除计数;算法只以合成事件作确定性单测,测试套不运行实测负载。

P0 运行账唯一位为 `$HOME/.stratalint-perf/events.jsonl`,由 gate/preflight timing spool 与 report supervisor 的逐进程 resource spool 经同一 C# writer 验必填字段、canonicalize 后追加;writer 以单写者句柄追加,写入/flush 失败必须截回初始长度。采集、spool、锁或 writer 失败均静默降级并保留原 gate/worker rc,不得把观测器变成新红门。运行时不得把性能事件、临时 spool 或派生报告写进工作树。此账是 host-local、非 canonical、单 epoch append-only 原始事实;P0 不自动淘汰,也不声称跨主机完整。gate/preflight 的 `run_id` 标识一次顶层运行身份且其 stage 共享身份;report resource 的 `run_id` 标识一个受监督进程。`commit`/`base` 绑定被测树与比较起点。schema 演化须另开 epoch 判例 PR,一次迁移 reader/writer,禁双读历史兼容垫层。P1 才可引入 canonical 快照:受信 importer 必按 schema/cohort/时间窗选择事件,记录源账 SHA-256、规范事件 bytes SHA-256、选择谓词、run 去重集与导入 commit,写入 `Meta/performance/snapshots/sha256/<content-address>.events.jsonl` 并由同地址 receipt 互证;仅在快照已验证、可重建后方可按完整 epoch 淘汰外置原始账。本 P0 未实现 importer、snapshot、retention daemon 或 CI 回流。

诚实分栏声明:P0 的 local gate/preflight 自动落本机账;现行 CI 由 base 侧 workflow 直接运行候选树中的 shared gate,不把 runner artifact 回流至该账,故 `venue=ci` schema 能力不等于已有 CI 样本,`make -C tools perf-report` 当前不得冒称 CI 基线。**P1 预算义务会计(v7.14,现役 warn-only)**:预算数据唯一位为 `Golden/perf-budgets.toml`,schema=`stratalint-perf-budget-v1`;每个实例登记完整 cohort/workload/kind/stage、metric=`p95_seconds`、limit、owner、到期复审日、修复动作、误报率窗与当前误报率槽、撤销判据及测量来源,且 mode 必为 `warn-only`。`make -C tools perf-report` 在 P0 汇总后以完整栏键逐字匹配预算,发射 `stratalint-perf-budget-result-v1`;超预算行为 `level=WARN`,但命令仍 exit 0。无同栏成功样本明确报 `no-data`,不得借邻栏或 observation 填数;预算解析/数据本身损坏可令显式 report 命令失败,但 comparator 不接 gate、admission 或 CI,故不成新红门。

P1 初值只登记 P0 外置账实际可产出的 `local-harness-gate/{total,admission}` 与 `preflight/total` 三栏。shared gate 的 `conservative` 阶段已随保守扩展重放机器整体退役,不再产生可登记的 timing;本 session 的 worker `~180s` 上限只记作 admission 来源旁证,不得伪造为独立 conservative p95 预算。待另案建立不触碰 admission rc 的独立事件生产后,才可登记该栏预算。

warn→required 的升红义务逐预算结算,只许另开独立 PR/check,不得批量升格:预算与修复动作须有内容寻址的 owner receipt;同一完整栏键须覆盖至少 30 个日历日且有至少 30 个 `passed` 样本;连续 14 天滚动误报率 `<5%`;相对同栏 baseline 超界 `>=20%`;独立复跑 2/2 仍越界;复审日未过期。误报定义为 WARN 经独立同栏 2/2 复跑未复现,或证实来自预算未登记的宿主/负载漂移。任一收据缺失即继续 warn-only,与 #272 的 Observe 地板递延同构:能力/证据地板未满足是显式 deferred,不是降低地板或让待升规则提前承重。required 后任一撤销判据成立——连续 14 天误报率 `>=5%`、独立复跑不再 2/2 一致、cohort 身份漂移、owner receipt 缺失或失效、复审到期未续,或 check 阻断一项经语义门证明有效的变更——必须由机器判词退回 warn-only;不得调整 admission 掩盖预算误报。该 required transition 及 owner receipt verifier 尚未实现,当前为 `deferred`;不得以现役流程冒领。**P2 优化收据(待升提,非现役)**:性能优化 PR 只有在 before/after 同 cohort、同 workload、同统计口径,各自原始事件集合与汇总内容寻址,并同时记录失败/副作用与 commit 身份时,方可声称“已获益”;无收据只能称候选或实验,不得以单次最快值、跨机样本或不同缓存态领功。P1 未实现 CI 回流、canonical snapshot、retention 或 required check;P2 的机器、snapshot 与 receipt 仍未建立。
...
**atom↔GID 覆盖边为 M:N,不是所有权。** atom 是启发式切片,故同一 atom 可由多个 declaration GID 合取覆盖,同一 GID 也可覆盖多个 atom;baseline 已出现某 GID、或另一 atom 已登记该 GID,均不构成拒绝理由,仓内不建立全局 `GID→atom` 映射。保留的结构不变量恰为:`atom_id` 全局唯一;单 entry 的 `coverage_gids` 无重复;每条 `(atom_id,GID)` 边各自持有匹配该 atom raw fingerprint 与 active 冻结命题 id 的 coverage receipt、各自持有匹配当前 Scribe definition/emission 的 Scribe receipt。cover 的每个宿主模块必须已在冻结账本 active 集合中,未冻结即 fail-closed 并要求先 deposit 再 cover。**预承诺执法按写入面分栏,不是存量全账迁移的冒领:**SL-016 对 baseline→candidate 新出现的每条边,要求 baseline canonical `digestion-formalization-v1` receipt 绑定该 `atom_id` 与 atom raw fingerprint、登记该 GID,且其 declaration signature 匹配 candidate raw Lean report;直接手改 BACKFILL 与把同一 GID 伪植到第二 atom 均由此 fail-closed。`cover-atom` 则在写入前验证**结果 target entry 的全部边**(既有∪新增),所以 legacy partial-closed host 若含未预承诺旧边,不得借一次合法追加把它继续带过。未被本次 candidate 新增、也未进入 cover 结果 entry 的历史边保持 grandfathered:SL-016 的 delta 判词不对它们作追溯式 formalization-receipt 全扫;故现役声明不得写成「仓内每条历史边都已预承诺」。首次 cover 可一次登记一个或多个 GID;追加边只需在命令中给出本次 GID,不要求重报已有 coverage 或 receipt 首项。
...
只读核实（retired historical premise）：PR-C 原以 `Genesis/Freeze/Reattest/Supersede/Revoke` 五类和线性 `previous_hash` 为迁移前提。该前提已退役：现役读取面消费 content-addressed `Genesis/Freeze/Reattest/Revoke` 历史事件集，candidate 写入面只接受 `Freeze/Revoke`；不得复活 Supersede 或 V1 replay envelope。

候选 lane 只可提交 `Golden/Frozen/intents/<case_sha256>/<intent_sha256>.json`，绝不可修改 accepted ledger 或提交 `events.jsonl`。封闭 schema 为 `{schema:"frozen-intent-v2",base_snapshot_sha256,case_sha256,operation,payload_sha256,lean_report_sha256,evidence_sha256,reason_sha256,producer_build_sha256,previous_case_event_sha256,intent_sha256}`；operation 枚举仅 `Genesis|Freeze|Reattest|Revoke`，其中 candidate 写入只允许 `Freeze|Revoke`；`intent_sha256=sha256(UTF8("frozen-intent-v2") || 0x00 || JCS(除自身外全部字段))`。sha256 均为 64 lowercase hex，Genesis 的 case 固定为 64 个 `0`、previous 固定为 64 个 `0`；其它 operation 禁零 case。仅完整 intent SHA 相同才可能幂等。
...
`Reattest→Reattest`：全字段相同为 I；否则仅新 evidence/Lean 有效且 previous 连续时 A。该转移只描述历史读取/reducer 语义，不授权已退役的 `ledger-reattest` 写入命令。`Supersede` 转移与 A14 写入门均 retired。`Revoke→Freeze` 对同一 case 永久为 R；后续 Freeze 必须使用从其 typed input 重新导出的新 `case_sha256`。这是现有 `allCaseIds` 永久集合的保守保持：candidate validator 对 `allCaseIds.Add` 失败即拒绝，history validator同样拒绝；Revoke 仅从 active 集移除，不从 `allCaseIds` 移除。
...
- **v7.14 R4**(2026-07-20,退役历史措辞):性能账 P1 建立三条 P0 可观测的外置 warn-only p95 预算,数据住 `Golden/perf-budgets.toml`,由 strict loader 逐项验证完整 cohort/workload/stage、owner、复审日、修复动作、误报率槽、回滚判据与测量来源;只进 caller-held summary 的 conservative 暂不伪造独立预算。`make perf-report` 在既有确定性汇总后逐完整栏键对照,超界发 `WARN` 但 exit 0;no-data 不跨栏补数。当时写作 owner 签署;该旧式措辞不构成现役要求,现役状态以 A21 的 `deferred` machine receipt verifier 为准。admission、CI 回流与 required checks 均未改动。
...
**冻结面条款:**SL-008 只读候选树与 changeset 状态:`D5/X_Frontier/Hearts.lean` 的 Modified/Deleted 一律拒绝;`Golden/Frozen/accepted/*.json` 只许 Added,既有 accepted 文件不可 Modified/Deleted;新增事件再由 content-addressed loader 与 candidate validator fail-closed 校验,写入面只接受 Freeze/Revoke。`ledger-reattest`、`ledger-sync`、`ledger-supersede` 及 Supersede 升级门均 retired,不构成 source 修改授权。Hearts 的规则变更须与规则实现同改并经过 SL-022 保护面;`HeartsAuthorizations.md` 保留 canonical 格式校验,不构成 Hearts 改动豁免。
