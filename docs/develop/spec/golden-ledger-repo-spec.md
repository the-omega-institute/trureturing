# trureturing / D5 —— 仓库规范全卷 v7.18(定本:宪法·地层·编码·执法·八官·管线·治理·引导)
...
| H12 | 在仓 TASK token 与异常账合规 | SL-016/SL-019 |
...
`docs/MISSION.md`:北极星 = 两颗心脏(可仰望不可硬攻);价值序 = **理解 > 数量,诚实 > 速度,负知识与正结果等价记账**;WorthVector 恰含四因子:新颖性、依赖就绪度、结构变现潜力(可开支票否)、收据潜力。每因子的状态恰为 `measured(value, receipt_ref)` 或 payload-free `open` 二选一；open 不再携带仓内案件锚点。

`docs/MISSION.md` 的唯一机读面是恰一个 `mission-v1` fenced block,由 `MissionFileLoader` 以 strict UTF-8、无 BOM、LF 与封闭 JSON schema 解析为 typed WorthVector;文件缺失、不可解析、未知/缺失因子、状态字段混用或 open 携带已退役的 `case_id` 均返回 typed error,禁止默认填 `0`、`1` 或任何常数。任一因子为 `open` 时,禁止乘法合成标量,禁止输出或自称完整 worth score/argmax;此时唯一合法的选择标签是 `bootstrap eligibility order`,且 tie-break 恰按 `canonical candidate id` 稳定破。同一输入的 typed 结果及该退化序必须逐字节可重放。仅当四因子全部携真实 `measured(value, receipt_ref)` 时,完整 worth argmax 才进入合法域;P0 不测量、不产生标量、不枚举或排序候选。**现役 P0 边界:**`measured` 分支保留为 typed schema 的可表示状态,但其 machine-replayable measurement receipt contract 与 resolver 落地前,任一 `measured` 实例均由 `MissionFileLoader` typed fail-closed;故 `CompleteWorthArgmax` 在 P0 结构上不可达,不得以非空 `receipt_ref` 字符串冒充收据权威。

仓内 `TASK D5-Tnnnn` token 仍是 SL-016/SL-019 等现役机制可机器派生并消费的案件地址，但不再参与 MISSION WorthVector 的 open 解析或 Frontier owner 判定；档案迁至权威 GitHub issue 后，仓内载体可以退役且不负永久保留义务；TASK 的有无、数量或散文内容均不得作为资格分类器。
...
**P4-A 交付陈述同一性。** `SL-027` 的发现集是两类路径之并：受保护 baseline 中存在、candidate 中删除的 `D5/X_Frontier/*.lean` 集合差；以及上段逐字定义的 existing V2 模块 raw-byte 变化路径。两类皆为空即返回。删除类不以 candidate 自报的契约块产生。`SL-002` 的治理 source 删除授权是五项合取：source 在 baseline 存在且在 current 缺失、protected baseline typed owner 恰为 `governance`、baseline 载体无 V1/V2 marker、current MISSION 完整加载、current owner 条目缺失；否则只有 current MISSION typed owner 明示为 `retired` 且携 `delivery_gids` 的条目进入退役证据验证。`SL-027` 使用另一个独立的三项 delivery-identity 分流判据：protected baseline typed owner 恰为 `governance`、baseline 载体无 V1/V2 marker、current MISSION 完整加载；它不要求 current owner 条目缺失。两者不得共享判据：governance 载体没有定理交付，故不适用交付声明与被退役模块的身份对应校验；即使 current owner 明示 `retired`，其 `delivery_gids` 仍由 `SL-002` 逐项验证为 active Frozen 声明。其余进入 `SL-027` 同一性验证的删除项，必须从 baseline 侧源码恰解析一个完整 `THEORIST_FRONTIER_CONTRACT_V2` 块；V1 判 legacy，V2 块缺失、重复、JSON 不可解析、schema/封闭键/`exact_statement.statement_sha256` 非 canonical 均阻断。`SL-027` 要求至少一个已解析交付声明的 candidate raw Lean report `.type` 经同一 `CanonicalStatementWriter.StatementTypeAddress` 得到的地址，逐字等于 baseline 契约的 `exact_statement.statement_sha256`。对 theorem/axiom，量词、前提、domain 或结论的任一 elaborated type byte 改变都会使地址不等并阻断；对 def/opaque，Inspector 所编码的 value bytes 也参与地址。module path、声明名与 `statement_id` 不参与跨模块比较。SL-027 的 affectedness 恰由 Frontier 删除或 existing V2 模块 bytes 变化触发；两侧 bytes 相同、baseline 无该文件的新 Frontier、MISSION、普通非 Frontier Lean/report 输入或 accepted ledger event 单独变化不唤醒本门。MISSION、candidate report 与 Frozen ledger 仍只由删除类的退役校验加载，修订类不读取三者。
...
**立案**:以 `(cas_ref, atom_id)` 二元组唯一定址 claim;重复发现必须先按该二元组检索并复用唯一案件,只追加证据、尸检或处置记录,不得另立平行案。GitHub issue 是案件档案的权威载体,须持久记录同一组 `case_id`、`cas_ref`、`atom_id`;仓内若已存在 TASK 载体,则 TASK 应引用其 issue,issue 也应引用 TASK GID,但本节不产生新建、搬迁或永久保留仓内 TASK 的义务。`case_id`→TASK GID 映射由 SL-016 从全仓 `D5/**/*.lean` 的 TASK token 现算派生(不再有手工镜像文件),该映射不得冒充 claim 身份或案件双向绑定。本类只定义既有载体的处置规范:零新状态、零新 schema、零新 workflow、零新服务。〔守护:**硬+评审**·`atom_id` 唯一性、以及派生的 `case_id`→TASK GID 映射对**全仓** TASK(不限 X_Frontier)的覆盖由 SL-016 机器判;二元组检索复用、issue 档案及存在 TASK 时的三字段双向引用当前由评审守护,机器化待升提〕
...
**案件生命周期**:GitHub issue 是案件历史账的权威载体;仓内若仍有 X_Frontier `TASK`,truth DAG 因其路径/`TASK` 标记恒判 `open`,该 `open` 只是载体结构态,不得用来冒充案件处置状态。SL-013 现为 deferred `NoFindings`,不产生禁删 finding;仓内 TASK 的新增、搬迁或保留义务均不得由该规则冒领,相关历史处置由评审守护。四种归宿逐案追加留痕:(1)送达成功——记录送达工件引用;仅当双轴结算清单 (a)–(c) 同时满足时案件闭合,issue 作为历史账保留,任何既有 `status.migration: absorbed` 均不替代 (a) 或 (b);(2)送达失败——记录失败收据,issue 标 `open` 并重试,不得称闭合;(3)反证链接误配——依 11.14 追加勘正工件并重开案件,旧 CAS、issue 与错误链接均不改写;(4)反证本身被勘误——走既有后代撤销路径,追加勘误工件并重开案件,其后只接受新证据重新结算。现役载体尚不能机器区分这四种案件归宿,**暂由评审守护,机器化待升提**,不得以仓内 TASK 的结构性 `open` 或存在与否冒充专用生命周期 schema。

**反馈闭环**:立 issue、携 `case_id`、`cas_ref`、`atom_id` 与证据 GID 通知理论作者、取得送达收据及失败重试,均为本类的**规范性动作**;**当前未机器执法,由评审守护,机器化待升提律**。证据不足而只有 `open` 异常、尚无 witness GID 时,仍须在权威 issue 记录该异常并明确“证据不足”;仓内若已有 TASK 可引用该 issue,但不构成新建或搬迁 TASK 的义务。此路由不改 `coverage_gids`、不增 coverage 收据、无任何 coverage 状态变更。送达收据结清的只是通知义务,作者回应绝非结案前置;送达失败时 issue 保持 `open` 并重试。作者修订投卷仍走既有 `theory-ingest`:新 atom 按现役 ingest 入账,旧 CAS 与旧案均保留,修订历史归 git;现役 ingest 不投影 `supersedes` 边,故不得冒领该关系。〔守护:**评审**·issue 创建、权威档案、存在 TASK 时的双向引用、通知内容、无 GID 路由、送达收据、重试、作者修订路由及“不声称 supersedes”当前全由评审守护,专用机器执法待升提〕
...
- **v7.18 R1**(2026-08-29):退役 `D5/X_Frontier/GovernanceDeferrals.lean` 治理载体及其仓内 TASK 锚点。MISSION 四因子统一为 payload-free `open`，current loader 对旧 `case_id` unknown-property fail-closed，保留 P0 measured 与 open/argmax 阻断；baseline Frontier ownership 改由 typed `frontier_eligibility` 窄投影读取。P2 仅在 baseline owner 恰为 governance、无 V1/V2 契约、current MISSION 完整加载且 source/owner 同时缺失时放行删文件+删 owner，数学/Theorist 退役仍须 active formal delivery。TASK block lexical scanner 随仓内治理载体退役，SL-019 保留完全相同 token regex；历史条目不改写。
