# trureturing / D5 —— 仓库规范全卷 v7.19(定本:宪法·地层·编码·执法·八官·管线·治理·引导)
...
**L2 schema 生命周期(expand→migrate→contract;owner 2026-09-03 定)**:**L2a expand(本 PR)** 在 candidate 与 protected-base 两个读取面共用同一份严格旧形投影,按 `coverage_gids` 元素类型区分旧形字符串与新形对象;旧形还可含 `receipts.coverage[]{gid,source_sha256,target_statement_id[,statement_id_history]}` 与 `cover_disposition.recorded_at_utc`,投影时合并 GID、保留 receipt target,删除 `receipts.coverage`、`source_sha256`、`statement_id_history` 与 `recorded_at_utc`,保留 `receipts.scribe`。字符串与对象混排、两种 schema 之外的键或形状一律 fail-closed;空 `coverage_gids: []` 两形逐字节相同,仅在出现旧形专属 receipt/disposition 字段时走旧形投影。L2a writer 只写对象形,并保留一次性 `migrate-digestion-coverage` 工具供下一步使用。**L2b migrate** 运行该工具,只迁移非空旧形条目;空列表条目保持零字节改动。**L2c contract** 在存量旧形清零后删除 candidate 与 protected-base 的旧形投影、`DigestionCoverageSchemaMigrator`、`migrate-digestion-coverage` CLI 动词及迁移专用测试,loader 随即只接受对象形。L2c 未完成前不得把 L2a 双读冒充永久兼容面。
...
- **v7.19**(2026-09-03,L2a expand):coverage 边持久键保持 `coverage_gids`,只把非空元素由 GID 字符串归一为 `{gid,target_statement_id}` 对象;同名查询字段仍投影为排序后的纯 GID 列表。L2a 为 candidate/protected-base 增加严格旧形投影、令所有 writer 只产对象形并暂留 L2b 一次性迁移器;L2b 只迁移非空旧形、空列表零字节改动;L2c 必须删除两侧旧形投影、迁移器、CLI 动词与迁移专用测试,收口为对象形单读。
