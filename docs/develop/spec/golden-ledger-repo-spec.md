...
- **v7.14 R4**(2026-07-20):`SCRIBE-LATEX-EPOCH` migrate 与 Scribe 学术体例同段落地:当前 60 个 theorem/proposition/lemma Describe 全部携与 Lean 陈述逐项对照的 `LatexStatement`;初裁 28 文件完成 16 个实迁+12 个 no-op 审计,并动态纳入初裁后 7 个新增源文件。发射体例由字段标签升级为 Digest→Abstract、typed AST 深度优先 `1.n` 自动编号、正式 Definition/Theorem/Proof/Commentary,`literature-attested` 从 L note 唯一投影 author-year-title-DOI citation;Markdown 与 QuestPDF 同源编译。SL-023 缺位 Observe 已为零,本段不做 contract 升红。
