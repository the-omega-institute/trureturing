...
The optional `report_execution` manifest declaration names the supported tools (`lake`, `lean`), platform coordinates (`system`, `machine`) and explicit semantic environment variables. The normal Inspector entry may seal a `.reuse.json` sidecar only after successful native `:report` (including ordinary defaults/audits) and complete publication. It binds the report semantic version, the complete report-module source and configuration inventory with file modes, the execution environment and the five-file report; no program that produces or judges the report, the Lean inspector and judge library included, is part of this binding. An inspector or producer change that alters report content bumps `report_semantic_version` by hand in the same change; a change that does not alter report content neither bumps it nor invalidates any cache. Reusing this successful invocation requires the semantic version and all bound inputs to remain identical and the complete current-source/material validator to pass on a private snapshot. An absent, old, damaged or mismatched receipt requests native Lake work; invalid registration and actual build/check failures remain failures. Producer implementation changes do not prevent reuse and do not invalidate compatible per-module artifacts: producer compatibility is the explicit `report_semantic_version`, bumped by the change that alters report semantics. Hashing producer or judge program bytes into a reuse condition of a data product is prohibited. Owner ruling (τ=0, 2026-09-20), verbatim: 「不是有缓存版本号了吗, 如果不兼容会bump那个版本号呀」 「把这个鸡肋检测删了 … 并且要永远拒绝这种行为」
...
Every declaration row requires a source-bound JSON boolean `generated_companion` in both the Inspector spool and compact report. Missing or nonboolean values reject; there is no old-wire fallback. A true value requires positive compiler-generation provenance or exact membership retained by a fixed registration/seal producer at successful theorem insertion. Fixed builder queries are read-only and owner-bound; membership matches the declaration owner, universe parameters, type and proof in the loaded environment. Missing or mismatched membership grants no exemption; a compiler-shaped name or parent alone does not establish that provenance. Explicit source `theorem`/`lemma` declarations still override the exemption, and unknown provenance remains selected. The field does not participate in statement material, declaration identity or module statement identity.
