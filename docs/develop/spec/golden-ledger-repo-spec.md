...
**fatal receipt-integrity 判据一名一址。** Engine 中与 `DigestionGap` 同居的 `DigestionReceiptIntegrity` 是 fatal 身份、严重度与差分判词的唯一权威；CLI consumer 不得按 code 字面另立清单。fatal code 恰为 `coverage-receipt-mismatch`、`scribe-definition-mismatch`、`scribe-emission-mismatch`，精确身份为 `(atom_id, code, detail)`；`missing`、`unverified` 及其余 gap 均不因本判据升级为 fatal。`digest-status`、`theory-candidates`、residual-frontier/truth-release 与 SL-016 均消费该 Engine 判词：前三者见 fatal 即 fail-closed；SL-016 对 fork-point→candidate 新增身份 `Block`，因规则实现变更触发全扫时对不在该候选差分内的存量身份显式 `Observe`。

**账本 writer 只阻断 delta，不以存量自锁。** `cover`、`ingest`、`align-scribe-receipt` 均须携显式 fork-point `--base`，并在任何 CAS/账本字节落盘前完成两道同源检查：先对 fork-point→当前 candidate 的 changed-set 求 fatal 身份，任一新增或身份恶化即拒；再把计划更新路径并入同一 change-set，对当前内存账本与最终内存账本求身份集差，任一 writer 新造身份即拒。基线已有且身份三元组逐字不变者保持 Observe，修复令 fatal 集严格缩小者允许。`align-scribe-receipt` 的第一道门只可豁免它正要修复的精确 `(atom_id,GID)` Scribe definition/emission mismatch，不得豁免 coverage 或 sibling 身份；第二道门除不得新增任何身份外，还须证明该目标精确身份已消失。所有判决发生在写调用之前，阻断路径须保持账本与 CAS 字节不变。
...
- **v7.16 R24**（2026-08-24，#2919）：将三类 fatal receipt-integrity 的严重度、`(atom_id,code,detail)` 身份与差分判词收归 Engine `DigestionReceiptIntegrity` 单一 helper。`cover`、`ingest`、`align-scribe-receipt` 在任何字节写入前统一执行 fork-point candidate delta 与 planned-state delta；基线存量保持 Observe、缩小 fatal 集的修复继续可写，align 只豁免其精确目标 Scribe mismatch 且最终必须消除该目标。`digest-status`、`theory-candidates`、residual-frontier/truth-release 与 SL-016 改接同一判词，SL-016 规则变更全扫把非候选增量的存量身份渲染为 Observe。三类 writer×三类 fatal、存量 Observe、missing/unverified 负例、身份三元组、只读链与 CLI 禁私有 code fork 均由具名测试及变异证明守住；账本 schema 与既有 golden 输入不变。
