...
Module rows and enrollment plans bind their own source closure and the semantic version; configuration identity belongs to the aggregate.
...
| `DTR-Evidence` | Observe | Selected producer evidence is missing, malformed, unresolved, stale, inconsistent or lacks a valid source-bound certificate. |
