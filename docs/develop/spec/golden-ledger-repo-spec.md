...
R1：可再生全局 aggregate 入库，令独立 source change 争用相同路径。其字面强化实例曾是本 spec 的 worked examples 以绝对 `start_byte`/`end_byte` 保存：在 spec 中间插入 bytes 会使其后所有边界整体位移，即“派生数据入库”与脆弱位置锚合流。该 12 条 example、`atomizer:none` 分支与全部 boundary 机制已在 v7.14 R11 删除；消化状态现与 source 字节位置无关。
...
- **v7.14 R11**(2026-08-09):`DIGESTION-DIRECTORIES` 将 34,266 行 `Meta/BACKFILL.yaml` 的 2,030 条原子记录一步迁为 `Meta/Digestion/backfill/<source_id>/<migration>-<truth>/<atom_id>.yaml`,以路径表达状态并令状态转换成为独立文件移动。迁移前实测 atom_id 2,030/2,030 唯一、ast_path 1,878 唯一(152 碰撞),状态组合为 residual-open 1,888、partial-closed 112、partial-open 30；1,877 条无 coverage 且无非空 receipt。删除 2,030/2,030 为空的 `chain_atoms` 与 `tail_authorization`,删除 `golden-ledger-spec-v7.11` 的 12 条 worked example、`atomizer:none`、absolute byte boundary/relocator 与 byte-match 检查,最终 2,018 个状态文件；source 元数据分置 `source.toml`,TASK 映射分置 `ticket-index.toml`,CAS 内容地址文件不移动。candidate 只读新布局,旧 baseline 无新目录时按空历史账加载,不设双读或 contract follow-up；SL-003 仅排除 backfill 目录容量计数。
