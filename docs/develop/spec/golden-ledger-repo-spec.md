...
**A14.9 Inspector-owned report artifacts.** `lean-report-inputs.json` is the sole FILEMAP-registered report input manifest. Its strict JSON schema selects report modules, Inspector sources, configuration and the `lean-report` / `scribe-content` producer scopes through explicit include/exclude patterns. Required registrations without files, malformed or duplicate fields, unsafe paths and unsupported schema identifiers fail with a named diagnostic. The schema identifier describes the wire format; there is no manually advanced producer compatibility generation. The reader `tools/scripts/report/lean-report-selection.py` expands only those registrations, with no MSBuild evaluation or script/call-graph discovery. The existing Scribe projection and describe checks consume the registered scope, including deleted paths.

The local dependency-free `tools/lean-inspector` Lake package owns per-module complete row/material bundles and their aggregate under root `.lake/build/lean-inspector`. Native module/package report facets trace source bytes, private/server/import/transitive compiler artifacts, the Inspector executable, registered Python/C#/script/configuration inputs, each module's authoritative utility record and external claim artifact closure. Utility input is generated once per invocation. Current module membership determines aggregation, including removals. Native Lake trace and artifact reuse select misses; batching selected misses shares import work without a second invalidation engine. Ordinary root defaults and audit builds remain required.

Inspector privately stages, canonically validates and publishes raw-v2 rows, all declaration materials including excluded helpers, ZIP digests, provenance-v1 and current-input attestation. A rejected optional artifact rebuilds privately with shared blob writes disabled; invalid required inputs and Lean failures remain failures. Public producer/resident digests bind registered producer bytes, while source and configuration digests retain their separate preimages. The public shell input helper exposes `address`, `verify`, `modules`, `producer-paths`, `scribe-producer-paths` and canonical `coordinates`; it does not own artifact reuse. The thin pair interface invokes one candidate Inspector producer.

CI saves the existing native `.lake/build` snapshot after report production and preserves the successful Lean build snapshot when reporting fails. The snapshot generation also accounts for registered report producers; the ordinary Lean source/config address and helper/key/writer/restore-prefix ownership remain intact. Ordinary validated `raw-lean-reports` artifacts transfer exact-source results to Release, which validates them or invokes the same Inspector. There is no independent report cache/store, remote report publisher, baseline adapter or delta/cohort planner. Donor warm and Lean publication require only Lean products and do not claim report hits without report products.
