...
**勘正（v7.16 R23，2026-08-23；上述可见性 open 闭合）。** 上段保留为 R18 当时的记录，不再描述现役行为：`Admitted` 路径自提交 `9e20d3680`（2026-08-19）起已在 `DEFERRED` 后渲染 `OBSERVED`；`ProtectedSurfaceChange` 路径现已把 `AdmissionEngine` 产出的 observations 带入 outcome，并在 `DEFERRED` 后复用同一 renderer 输出 `OBSERVED`，退出码仍为 3。至此两条路径均把 `SL-028` advisory 送达 CLI 读者，R18 所记 `open(案号待开)` 就此闭合。
...
- **v7.16 R23**（2026-08-23，#2612）：闭合 §11.20.4 R18 所记的 `SL-028` CLI 可见性 `open(案号待开)`。`Admitted` 路径的 `OBSERVED` 渲染自 `9e20d3680`（2026-08-19）起已经存在；本修订补齐 `ProtectedSurfaceChange` 的 observations 载体与渲染，在 SL-022 与 `DEFERRED` 之后复用同一 observation renderer，且退出码保持 3。原五项合取回归拆为 protected 放行侧、SL-028 载体、`DEFERRED`→`OBSERVED` 同序渲染、退出码四个具名测试，并分别以可编译变异取得独立红；`Admitted` 路径实现不改。
