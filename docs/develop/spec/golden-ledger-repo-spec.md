...
The persistent `TemplatePlanFrame` contains canonical Name bytes, a token-interned payload and its SHA-256 identity. The current marker is `DTR-checked-plan-v4`, with extension identity `checkedPlanFramesV4`; the decoded plan's schema, grammar and constructor-recursion versions are each 1, and its private compatibility coordinate is 6. These are distinct from the module report semantic coordinate in A14.9. The decoder accepts exactly this layout and version tuple. Literal tokens use canonical decimal UTF-8 byte length, a colon and exact bytes; repeated tokens use backward-only `@<first-literal-index>:` references. Duplicate literals, leading zeros, invalid UTF-8, unknown markers/references and trailing bytes reject. The fixed-width charged-work literal is outside interning. Both serialization passes debit token processing and emitted bytes and must agree on work. Imported ownership is checked against the actual extension container and Lean declaration-to-module table, not supplied owner strings. No older frame layout is an alternative reader path.
...
The module wire field `information_templates` has exactly `{schema_version,compatibility_version,inventory,registered,records,inputs}` with schema 1 and current report compatibility 6. Every record has exactly `{key,registration_source_path,statement_identity,unit_name,realization_name,content_inputs,binding_source_path,state,diagnostic,certificate}`. A validated certificate has exactly `{key,evidence_ref,plan_identity,descriptor_identity,actual_identity,argument_inputs,extraction_inputs}` and no diagnostic. Other states have no certificate; unresolved declarations require a diagnostic and binding owner, while undeclared records have no binding owner and the exact occurrence-addressed missing-declaration diagnostic. Certificate dependency entries have exactly `{name,owner,type_identity,body_identity}`. The evidence reference hashes a domain-separated, length-prefixed occurrence tuple, statement identity, selected plan, descriptor, actual extraction and separately sized dependency arrays. It cannot be copied to another occurrence.
...
| --- | --- | --- |
| `DTR-Undeclared` | Block | A selected occurrence remains `undeclared` after joining its registration and binding claims. |
...
| `DTR-Declared` | Observe | A selected occurrence is `declared_validated` and has a source-bound certificate. |

To pass this obligation, a content author uses an enrolled template satisfying E1–E8, supplies `readout via` on the registration or an exact `declare_information_template_binding` sidecar, and produces current source-bound report evidence under A5.4 and A14.9. Every selected occurrence must resolve to `declared_validated` with its certificate. A selected module without registrations still requires valid producer evidence of its empty inventory. `DTR-Declared` records the successful binding check; the other admission obligations remain independent.
