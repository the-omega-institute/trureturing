...
- **v7.15 R1**(2026-08-08):勘正 A17.1:生产代码从未存在 `LatexStatement` 类型;本修订由机器读数得到 220 个 theorem/proposition/lemma Describe,其中 9 个已投影派生、0 个手写且可投影、211 个手写且机器不可投影。多数现存节点晚于 v7.14 R5 的 77 节点复核批次,未曾受其人工逐项流程约束;现以强制 StatementProjection 溯源取代人工席,可投影即 Block 手写来源,不可投影逐节点 open。truth-graph v1 同步原位增加 Describe 节点与 Describe 身份锚。
