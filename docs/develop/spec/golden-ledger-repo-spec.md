...
`DeclaredTemplateBindingRule` stays delta-only and observe-only (A5.5). Its selection covers changed `Reg` modules; a new public theorem of a changed D5 module is looked up in its mirror `Reg` module by path. The resulting `DTR-Unregistered`/`DTR-Undeclared` readings remain warning-only for admission, but the authoring delivery still must carry the same-diff `Reg/D5/<mirrored source path>.lean` registration and compiled evidence required by A5.5. `Reg` modules are report modules of `lean-report-inputs.json`; the report's version and reuse mechanisms (A14.9) are not changed by this architecture.
