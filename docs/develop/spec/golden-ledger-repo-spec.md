# trureturing / D5 —— 仓库规范全卷 v7.22(定本:宪法·地层·编码·执法·八官·管线·治理·引导)
...
declare_information_template_binding thm in A readout via (T args)
declare_information_template_binding thm in A object_arena O catalog C readout via (T args)
```

`T args` denotes an ordinary elaborated Lean application, including its implicit arguments. The `information_theorem` form also permits `object_arena O catalog C`; primitive registration forms retain their optional `variation` and `sensitivity` witnesses, and the reifier form retains optional `output_evidence`. `readout via` supplies binding metadata to the existing registration transaction; it does not replace the theorem, Law, primitive bundle, bridge or their semantic checks. Registration without a declaration emits `undeclared` evidence and is subject to A5.5; it does not select an alternative readout checker. A binding sidecar identifies an existing occurrence by theorem, canonical object arena and, when supplied, catalog; it cannot create a registration or retarget one.
...
**A5.3a Dependent-family registration.** `dependent-family-v1` is a sibling of `fixed-state-v1`, not a fallback after fixed-State rejection. A plan, occurrence key, certificate key and cached assessment retain the mode. The plan frame includes the mode token after its version tuple. Occurrence keys have exactly `{mode,root,registration_module,theorem,object_arena,catalog}`. Unknown or missing modes, cross-mode plan selection, and stale mode bindings reject. Existing fixed-State grammar, `FiniteLawVariation`, `FiniteSlotSensitivity`, finite catalogs and seals keep their semantics.

```lean
register_information_template T dependent_family
register_information_family source in A readout via (T args) realization registration
  escape from coordinates [0, 1, 11]
  state ["body", ...] output ["body", ...]
  escape continues (open)
declare_information_family_binding source in A readout via (T args) realization registration
  escape from coordinates [0, 1, 11]
  state ["body", ...] output ["body", ...]
  escape continues (open)
```

The second form creates a family occurrence in the declaring sidecar; it does not edit the source theorem or replace a fixed occurrence. `A` has type `DependentFamily.Arena`. Its `Signature` supplies an ordered dependent index telescope Θ, actual fibers `State : Θ → Type` and `Output : Role → Θ → Type`, a finite nonempty role type, and a finite anchor type that may be empty. A `Realization` supplies complete readout families `∀ role θ, State θ → Output role θ` and complete anchors `∀ anchor θ, State θ`. Θ is never substituted for State. The intervention domain is all well-typed complete families. There is no naturality, transport or completeness gate.

`GlobalFamilyVariation A` requires good and bad complete realizations with opposite Law truth. `FamilyRoleSensitivity A` quantifies over every role and anchor: changing a readout role fixes every other complete readout family and all complete anchors; changing an anchor fixes all complete readouts and every other complete anchor. A typed `Registration A statement` retains realization, `statement → A.Law realization`, variation and sensitivity as proof fields. The producer kernel-checks those fields and the original named theorem at the source's exact ordered rigid levels, checks the source statement against both the registration proposition and the actual realized Law by kernel definitional equality, and checks the bridge result against that Law, and restricts all source/registration/arena axiom closures to `propext`, `Classical.choice`, and `Quot.sound`. The literal registration constructor is the extraction boundary; arbitrary opaque record providers have no rule. No finite theorem unit, catalog or seal is synthesized. Continuation is literally `open` on this route.

Source coordinates and executable template arguments are disjoint inputs. The source resolver retains the full raw theorem type, all binder domains and BinderInfo, rigid levels, caller-selected State/Output AST paths, and both complete enclosing contexts, including lambda/forall distinctions and let values/nondependency flags. Paths use only `fn`, `arg`, `domain`, `body`, `type`, and `value`; the producer never searches for an alternative occurrence. Coordinates form an ordered injective dependency-closed map of data/carrier binders. Proofs and dictionaries cannot become coordinates. Each selected coordinate must be reached through its original leading forall-body path; a same-typed nested binder in an earlier domain cannot replace it. The resolver projects de Bruijn variables through the map and checks inverse embedding against the exact raw occurrence, retaining nested cutoffs. It builds the selected telescope as dependent pairs (the empty telescope is Unit and the final coordinate is its domain), then kernel-checks that the actual signature fields are precisely the extracted State/Output fibers; the selected Output occurrence must match each role. Descriptor universes are inferred against the original source-level realization type; generic templates may use a different universe arity. This source relation does not weaken the limited raw template/extraction comparator. A source dictionary needed by Law stays in the full Law and is not thereby admitted as an executable argument.

Family content explicitly imports `LeanInformationAudit.Registry.Family`; the fixed/finite seal import closure excludes this module, `Registry.SourceScope`, and `DependentFamilyRealization`. The registry loads only the fixed `FamilyRegistration.driver` API after checking its type owner, implementation owner and native source/dependency coherence; missing or retargeted drivers fail closed. Family enrollment uses the same E1–E8 compiler and obligation accounting, with an explicit family return interface. Its additional interface constructors and projections are pinned by the trusted family producer; shared primitives remain pinned by Syntax. Dependent-pair and empty-type construction are confined to family mode. `Fintype.instEmpty` is a pinned constructive empty-anchor dictionary. The only additional numeric projection rule is the pinned `Fin.val` from `Init.Prelude`, with exact raw type/body, universe arity and matching index. Named `Fin.val n i` and raw field 0 are permitted only at type/index sites with neutral `i : Fin n`; value-position use, proof fields, index aliases and missing pins reject. There is no MeasurableSpace or blanket dictionary rule. All ordinary supplied arguments retain independent E5/E6 checks, and extracted result types and proof-leaf propositions retain occurrence-relative statement-independence checks. Source projection, inverse embedding, substitution, plan construction and serialization debit the existing 524288 work / 256 depth bounds; source/argument telescopes have at most 64 entries.

The family content-input walk retains content declarations and stops at native-validated judge and pinned external package owners; their own source/version/pin closure remains a separate obligation. A family record has the fixed record fields plus `family_binding`. `bridge_kind` is `family-forward`; `unit_name` and `realization_name` identify the typed registration definition, not a finite TheoremUnit. Its `escape_from` is `{kind:"source-occurrence",source,scope_identity}`. Validated `family_binding` has exactly `{identity,material}`. Material binds source name/path/hash, statement and raw-type identities, rigid levels, ordered binder-domain identities, coordinates, State/Output paths and context/raw/fiber identities, actual signature/State/Output/Law/realization/registration identities, the typed registration name (equal to both row names and present in the extraction inputs), bridge/variation/sensitivity proof-field-and-proposition identities, literal continuation, selected plan, descriptor and actual extraction identities, and a separate ordered executable-argument map. `identity` is SHA-256 of `DTR-family-evidence-v1:` followed by sorted-key compact UTF-8 JSON material, using `Lean.Json.compress` string encoding (including unescaped supplementary scalars). The normal binding identity includes this identity and mode. The strict family reader recomputes the complete `DTR-binding-evidence-v3` framing, including the occurrence, extraction and dependency arrays; rehashing family material alone cannot retain a copied native certificate. Failed family rows carry no validated family material. The strict managed reader checks all fields, bounds, source inputs, maps, identity and certificate links after delta selection. Missing/unknown/copied/stale/cross-mode evidence fails closed. Family registrations participate in the existing independent native inventory and DTR join, with the same four verdict names and delta owner selection.

The selected family join also requires `family_registration` on the actual resolved raw declaration report. The fixed owner-bound report driver independently enumerates native module declarations and reads `ConstantInfo` for definitions whose actual type is `DependentFamily.Registration`; it does not derive this field from occurrence rows or certificates. The exact object is `{schema,owner,type_identity,body_identity,registration_identity,level_arity}`, with schema `dtr-family-declaration-v1`. Type and body use the existing typed proof-erasure identity domain `DTR-proof-erased-expr-v2`; the named registration constant uses `DTR-raw-expr-v1` with positional rigid levels. These are not normalized `statement-v1` material hashes. Inspector, compaction and raw report loading retain this field separately from `information_templates`. After delta selection, the strict join requires its actual module owner, exact extraction type/body, constant identity and universe arity to match the family material and unique extraction dependency. Missing declaration evidence, a different actual type, and coherent certificate retargets reject even when all supplied family and certificate hashes have been recomputed. The optional field is not a fallback: selected validated family evidence must have it; unrelated declarations and unselected registration payloads are not interpreted by this join.
...
Authoritative assessment may cache only validated results in a private, nonpersistent environment cache. Reuse requires the exact occurrence/statement/levels, claim and owner, selected plan, options, registration context, unchanged native constant objects/owners and relevant class/reducibility/projection/extern/implemented-by metadata, plus current selected input hashes and native coherence. Invalid cache inputs cause reassessment. Mathematical seal publication consumes joined producer-issued metadata in its immutable environment; it issues no binding certificate or subsequent file-freshness claim. An unresolved binding does not invalidate an already valid mathematical seal or certify template completeness. A sidecar can supply the original occurrence's binding overlay without rewriting or resealing frozen proofs. P1 semantic insertion, reflexive-source exclusion, certificate validation, C048/C049 variation/sensitivity obligations and rollback remain independent conjuncts; the new binding record never enters the existing semantic throw-on-insert branch.
...
**A5.5 Declared-template judgment (SL-031).** `DeclaredTemplateBindingRule` is a named obligation of SL-031, dispatched by `UtilityAdmissionRule` alongside the utility obligation in A5.1. Its domain is the deduplicated selection from `RepositoryRules.ChangedOrFirstPinD5Modules`: present D5 Lean modules that are candidate-new, byte-changed relative to the protected base, or receive their first `Golden/Frozen/state/<module>.lean.json` pin relative to that base. Rename/copy destinations participate; byte changes participate even when the module is already frozen. The selection is independent of A5.1's unfrozen-content restriction. Judge, FILEMAP, compiler or toolchain changes alone do not select unchanged registration owners. An empty selection produces no declared-template findings. This obligation has no ledger, switch or grace mode. Within the selection, every public `theorem`/`lemma` declaration that is new relative to the protected base (no `theorem`/`lemma` of the same name in the base bytes of the same path; every public theorem of a candidate-new module) must have at least one `declared_validated` binding carrying all four escape slots — a `readout via` registration in any candidate module or a `declare_information_template_binding` sidecar — or the rule emits `DTR-Unregistered` (Block). Private and internal-detail declarations, `def`, named source `instance`, `example`, and machine-generated companions are outside this obligation. Explicitly authored theorem names do not acquire a suffix-based exemption. Base bytes are read only through the repository snapshot gateway; base code is never executed. Known limitation: anonymous proposition-valued instances elaborate as theorems and cannot be distinguished in the report. The finding names are exactly `DTR-Undeclared`, `DTR-Evidence`, `DTR-Unregistered` (Block) and `DTR-Declared` (Observe).

Registration ownership determines the evidence domain. The reader collects each selected owner's current producer evidence and any sidecar claims for its occurrences, then performs A5.4's strict join and certificate checks. Owner addresses are inspected to route claims before their payloads are validated; either a selected registration source or a selected registration-module address retains a record, so inconsistent addresses cannot hide a selected claim. A sidecar's claims for selected owners do not bring its own unrelated registrations into scope. Outside the selection, registration payloads are not read as evidence, judged or used as a failure cause at the report loader, evidence reader or rule layer. Missing, malformed, unresolved, stale or incompatible template evidence for unselected registrations cannot fail admission.

Whole-artifact integrity remains global: canonical UTF-8 JSON bytes and report structure, complete module membership, source path/hash bindings, declaration material content addresses, and the bundle provenance and attestation checks in A14.9 still apply. These checks establish the integrity of the producer artifact consumed by the repository; they do not validate its unselected registrations. `RawLeanReportArtifact` retains template payloads without interpreting them, and `InformationTemplateEvidence` projects the selected owners before applying the strict template reader. Global artifact validation must not turn an unselected registration's template evidence into an admission obligation.
...
| --- | --- | --- |
| `DTR-Undeclared` | Block | A selected occurrence remains `undeclared` or lacks `escape from` / `escape continues` after joining claims. |
| `DTR-Evidence` | Block | Selected producer evidence is missing, malformed, unresolved, stale, inconsistent or lacks a valid source-bound certificate. |
| `DTR-Unregistered` | Block | A new public theorem has no validated four-slot registration. |
...
To pass this obligation, each selected occurrence declares four slots: (1) `escape from (<term>)` supplies a constant or binder type occurring in the raw elaborated statement and definitionally identifies what the closed arena's State represents; absent occurrence gives `dtr.escape_from_absent`, wrong State gives `dtr.escape_from_state`. (2) `readout via (<T args>)` passes the existing enrolled-template E1–E8 and exact extraction checks. (3) The realization bridge retains exact statement/arena/primitive-bundle checks; `EscapeRecord.EscapePrimitiveRealization` proves only statement ⇒ Law and requires valid variation and sensitivity (`dtr.forward_bridge_requires_sensitivity`), while the legacy equivalence and exact native-Law forms remain available. (4) `escape continues (<term>)` supplies a named `EscapeRecord.EscapeResidualWitness` value or an `EscapeRecord.EscapeResidualEmpty` proof for a named `LayerChain` over that same arena (`dtr.escape_continues_arena` rejects another arena), or the literal `open`. Membership is kernel-checked through the witness field; the judge checks types and names without evaluating states, chains or certificates. `open` is a declaration without a certificate.

The two optional escape clauses follow variation/sensitivity (or output evidence), and precede the statement colon for `information_theorem`; a sidecar places them after `readout via`. All new words are non-reserved. Old-form registrations keep their parser and producer status; the delta rule imposes the four-slot obligation. A sidecar applies the same checks against the original theorem and arena and cannot replace an existing inline claim. At least one complete verified registration is required for each new public theorem. Multiple registrations on different arenas coexist under their original occurrence keys; there is no completeness or naturalness judgment. A selected module with an empty inventory still requires current producer evidence and its new public theorems are still checked. Other admission obligations remain independent.

Owner rulings (τ=0, 2026-09-18), verbatim: 「你就只判delta就可以, 很简单新定理需要给出几个东西, 原来逃逸在哪里, 把逃逸怎么处理的, 出来什么新信息, 新信息在哪里继续逃逸.」 「应该至少一种吧, 就是这个本质上就是你写了能过机器验证肯定是对的, 至于有没有其他种类, 那你写两种就有两种逃逸方式?」
...
The persisted binding input closure is the requested content source closure plus the configuration/toolchain inputs specified in A5.4. Judge implementation paths are excluded from that closure and do not become binding validity inputs merely because the fixed driver executes them. Native validation still checks loaded judge artifacts when producing a report. Lake's trace hashes provide cache coherence, not cryptographic compilation attestation: the pinned compiler and its outputs remain within the native trust boundary, while SHA-256 snapshots detect subsequent input replacement.

The exporter emits an independently source-bound `information_templates` partition for every requested module. Native row reuse and final publication validate complete current membership, captured source inputs and material integrity; for a present `information_templates` partition, native validation checks its top-level shape and current input hashes, while the required C# consumer rejects missing evidence and malformed records within A5.5's selected registration-owner scope. `inspect.sh` delegates both unchanged and changed work to native Lake report facets and propagates phase failures. C# separately performs A5.4's complete occurrence/sidecar/ownership join for the selected owners and A5.5's delta judgment. Input digests may be reused only within one immutable `RepositorySnapshot` identity; replacing a snapshot requires a fresh index, and every selected record still checks fields, ordering, availability, symlink exclusion and digest equality. No repository-path/revision-only digest cache may certify changed inputs. The explicit `--statements-only` fixture operation omits `information_templates` and cannot satisfy the declared-template consumer for selected owners.

Census keeps `information_template_bindings` separate from mathematical disposition rows. It discovers occurrence events and binding claims by their actual containing modules, imports all scoped evidence modules, obtains one authoritative joined snapshot and reconciles selected keys against the finite/structural registries. Missing discovery/sidecars, inventory mismatch or out-of-scope binding owners make the query incomplete; absence of an occurrence does not invent an undeclared record. Each query carries `{schema_version,compatibility_version,query_completed,diagnostic,records,source_inputs}` with schema 1 and its current census transport coordinate 5. That wrapper is not module report evidence and cannot substitute for the module partition at the current manifest `report_semantic_version`. Complete queries have a null query diagnostic; incomplete queries have a diagnostic and empty records/inputs. Only individually validated records carry certificates. Per-key cache reuse checks the wrapper, selected record fields, cache-input/query correspondence and hashes of listed source inputs; complete record and ownership validation belongs to the authoritative producer and strict module reader, and cache reuse does not attest source-to-native compilation.
...
- **v7.21**(2026-09-05):A16 增 **base 判官永久禁止**(τ=0 owner 2026-09-03 裁决):为判决候选而 checkout / restore / 编译 / 执行 base 树代码的一切机制作废,base 只以数据参与;残余逃逸由分区门 SL-029 接住。第十二部凡以「base judge 注入 / base 侧 query」为承重的条款(2.2 AU-EXT-1、12.6.2 external scope、12.10 完成条件、12.11 TE2-1)加勘注作废其 base 侧执行形态,不改写原文。案由与守护分栏见 CLAUDE.md 第 19 条。
