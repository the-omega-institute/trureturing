...
**A14.9 前置坐标重锚(2026-08-30)** schema-v5 现役候选联合扩为 `Freeze|Reanchor`;本条不恢复 A14 已退役的 Reattest/Supersede 或环境升级协议。`Reanchor` payload 闭集为 `{declaration_statement_ids,descriptor_selector,prerequisite_frozen_node_ids,previous_event_hash,statement_id}`;`previous_event_hash` 必须指向同 case 恰为当前 active 的事件,descriptor、statement 与逐声明身份必须全等,前置坐标必须有变化,且新 payload 必须逐字段等于 fresh candidate Lean report 重算的完整 material。`ledger-append` 是唯一 writer,按依赖优先追加事件;既有 accepted 字节不可改删。受信 replay 只以因果链末端为 active:仅已有合法 Reanchor 后继的历史节点可不再承担 active dependency edge,每个末端节点仍须形成 closed DAG。故该扩展只修复同一真命题的引用坐标,不授权 source、statement、声明集、axiom closure 或 environment 漂移。
...
**冻结面条款:**SL-008 只读候选树与 changeset 状态:`D5/X_Frontier/Hearts.lean` 的 Modified/Deleted 一律拒绝;`Golden/Frozen/accepted/*.json` 只许 Added,既有 accepted 字节不可改删。candidate accepted event 的现役联合仅为 `Freeze|Reanchor`;candidate Reattest/Supersede 一律拒绝,已提交的 Reattest v2/v3/v4 只由受信历史读侧消费。Reanchor 只可按 A14.9 修正同一 case 的前置坐标,不构成 frozen source Modified、statement drift 或 pin bump 的授权;Reattest/Supersede 仍非现役写面。Hearts 的规则变更须与规则实现同改并经过 SL-022 保护面;`HeartsAuthorizations.md` 保留 canonical 格式校验,不构成 Hearts 改动豁免。
