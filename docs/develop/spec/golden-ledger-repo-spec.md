...
IE-C050 keeps its identifier and established reason priority, and takes precedence over IE-C021 when both apply. Its declared-binding diagnostics use these coupled trigger/message/payload contracts:
...
The authority order is **inactive row-free mechanism → independent seed-only content change → judge activation → content declarations/discharges**. Inactive row-free evaluation still requires complete current producer evidence and no declarations. Seeding reconstructs the original seed's complete occurrence inventory; every original occurrence must be undeclared and have an exact row binding its statement, registration source and content inputs. Initialization cannot replace rows, enroll, discharge or run while active. The initialization writer may take the exact current immutable commit OID as its installed protected base, including a clean checkout, because it creates the first delta; other revisions and writer operations retain ordinary ancestor/non-vacuous preparation. This is not an admission exemption. Requested activation with empty rows still reconstructs original, protected and candidate evidence: only an originally empty inventory can activate without seed rows. An active empty debt set cannot be initialized again.
...
The finding allowlist is closed; these are named SL-031 findings, not new SL rule numbers. All failure findings Block. The outer evidence boundary may report `DTR-Evidence` carrying a named inner failure; `DTR-Inactive` and `DTR-Domain` only Observe scope.
