...
Every declaration row requires a source-bound JSON boolean `generated_companion` in both the Inspector spool and compact report. Missing or nonboolean values reject; there is no old-wire fallback. A true value requires positive compiler-generation provenance for the automatic-companion exemption; a compiler-shaped name or parent alone does not establish that provenance. Explicit source `theorem`/`lemma` declarations still override the exemption, and unknown provenance remains selected. The field does not participate in statement material, declaration identity or module statement identity.
