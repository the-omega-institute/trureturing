...
仓内 `TASK D5-Tnnnn` 仍是其他规则的永久案件地址，但不再参与 MISSION WorthVector 的 open 解析或 Frontier owner 判定；TASK 的有无、数量或散文内容均不得作为资格分类器。
...
**P4-A 交付陈述同一性。** `SL-027` 的发现集是两类路径之并：受保护 baseline 中存在、candidate 中删除的 `D5/X_Frontier/*.lean` 集合差；以及上段逐字定义的 existing V2 模块 raw-byte 变化路径。两类皆为空即返回。删除类不以 candidate 自报的契约块产生；仅当 protected baseline typed owner 恰为 `governance`、baseline 载体无 V1/V2 marker、current MISSION 完整加载且 current source 与 owner 条目均缺失时，才从该门分流；其余删除中只有 candidate MISSION typed owner 明示为 `retired` 且携 `delivery_gids` 的条目进入同一性验证。`TheoristFrontierContractValidator` 与 `TheoristFrontierRevisionValidator` 调用同一具名治理删除豁免判据，五项必须同时成立；后者只跳过满足该判据的治理、无契约载体之交付身份检查，不改变数学/Theorist 退役证据要求。每项必须从 baseline 侧源码恰解析一个完整 `THEORIST_FRONTIER_CONTRACT_V2` 块；V1 判 legacy，V2 块缺失、重复、JSON 不可解析、schema/封闭键/`exact_statement.statement_sha256` 非 canonical 均阻断。`SL-002` 仍逐项验证全部 `delivery_gids` 可解析为 active Frozen 声明；`SL-027` 另要求其中至少一个已解析声明的 candidate raw Lean report `.type` 经同一 `CanonicalStatementWriter.StatementTypeAddress` 得到的地址，逐字等于 baseline 契约的 `exact_statement.statement_sha256`。对 theorem/axiom，量词、前提、domain 或结论的任一 elaborated type byte 改变都会使地址不等并阻断；对 def/opaque，Inspector 所编码的 value bytes 也参与地址。module path、声明名与 `statement_id` 不参与跨模块比较。SL-027 的 affectedness 恰由 Frontier 删除或 existing V2 模块 bytes 变化触发；两侧 bytes 相同、baseline 无该文件的新 Frontier、MISSION、普通非 Frontier Lean/report 输入或 accepted ledger event 单独变化不唤醒本门。MISSION、candidate report 与 Frozen ledger 仍只由删除类的退役校验加载，修订类不读取三者。
