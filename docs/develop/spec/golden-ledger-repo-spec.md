...
`digest-status` 等全账读侧仍加载并投影全部 entry,闭合仍要求 entry 的全部 GID 边与 Lean/Scribe 条件齐备;但 protected-base 查询的**收据完整性升级域**不是全体 entry,而是与 SL-016 准入共用的、由 current/baseline 值比较派生的受影响 entry 依赖闭包。Scribe verifier 与 evaluator 的收据核验必须消费这同一闭包;CAS 重哈希与 handwritten projected-status replay 仍各消费其既有原始 delta,不得借收据闭包缩窄。故只改 target Lean 文件中的无害注释、且 formal header 与 statement resolution 均未改变时,存量 `scribe-declaration-reference-missing` 可继续作为非阻断 gap 被查询,但不得升级为 `DIGEST_STATUS_INVALID`;定义、发射、header、resolution 或受影响 chain ancestor 的值变化仍重新裁决相应 entry。若 frozen/report-derived authority 在构造依赖闭包时不可读取,不得吞错后坍缩为空集:resolver 必须保留候选 dependant 为 affected,由 applicability classifier 产 blocking `scribe-applicability-invalid`。producer/gate 持有 candidate Lean report 时仍对真实全账重算并校验全部 persisted/derived status;该全量事实不写回聚合物。
...
- **v7.30**(2026-09-07,总单 #5533 第 10 层配套):§11.21 区分全账查询投影与收据完整性升级域:`digest-status` 的 Scribe verifier、receipt verification 与 SL-016 共用 current/baseline 值派生的受影响 entry 闭包,无害 Lean 注释不再把存量 declaration-reference gap 升级为查询失败,CAS 与 projected-status 仍保留原始 delta;frozen/report authority 构造失败保留 dependant 为 affected 并由 classifier 产 `scribe-applicability-invalid`,不得静默不裁决。真实全账 persisted/derived 一致性仍由持 candidate Lean report 的 producer/gate 校验,不进入无该报告的 engineering test job,不持久化 aggregate。
