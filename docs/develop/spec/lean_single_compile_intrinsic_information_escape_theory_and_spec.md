...
每个 theorem 族（按 typeclass 约束族或载体族划分）定义一个标准 arena 集合，含实例 arena（如 `Fintype α` 族取 `Fin 2`／`Fin 3`／`Fin 4`）与 §6.2 各逃逸处上的 structural arena；同族 theorem 均在标准集合上登记，使同核 theorem 在共同 arena 上成为 peers 并由 IE-C008 识别，不依赖 CIRPT-IE-022 搬运；未在标准集合上登记是登记缺口，census 报告该缺口而不判 trivial。标准集合自身由参数族生成时（如 `Fin n` 的 $n$），该参数轴作为上一层 structural arena 登记其 §6.2 逃逸处，由同一判官递归判定；递归无封顶，顶层形式不可判者标 open。任何 readout 都不能区分的 theorem（bind-only 传输、无见证的纯存在、高阶或元命题）为 transparent：只作传输边，不登记、不判 trivial 亦不判 positive，census 以 report-only 行入账，不计入 certified 分母。本规范中一切 zero／trivial 判词均相对于已登记的观测集合（arenas 与逃逸处），positive 判词由具体 occurrence 见证而不依赖集合选择；不存在「绝对无逃逸」判词。
