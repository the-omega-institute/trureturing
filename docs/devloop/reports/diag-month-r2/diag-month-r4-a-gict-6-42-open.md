...
Those clauses cannot be omitted, supplied by a different carrier, or installed
by new definitions without weakening or fabricating the source claim.
...
| `任一 m-柱回归时间恰取两...值` | Universal quantification over every positive golden factor/cylinder and exact cardinality two of its first-return set | Covered on the word/rotation carrier by `golden_return_words_encard_eq_two_of_pos`, `golden_occurrence_gap_set_encard_eq_two`, and `golden_arc_first_return_gap_set_encard_eq_two`. Return-word length bijects to occurrence gaps via `golden_return_words_length_bijOn`. |
...
The unresolved set is therefore substantive: adjacent Fibonacci index pairing,
return-frequency multiplicities on the cylinder carrier, the matrix direct-sum
carrier, the cyclic-to-golden mechanism bridge, the three-gap finite-image
bridge, the concrete return certificate, and the decimal contract.
...
and a `13 -> 21` substitution prefix is not a return spectrum.
...
identifying each positive factor cylinder's return set with one consecutive
Fibonacci pair; frequency counts on that same carrier (or a proved bridge to
the existing golden-name counts); the cyclic-to-double-clock mechanism map;
and a public finite return certificate specifying the cylinder, ratio
orientation, and decimal rounding rule.
...
- **Clause fidelity:** the dropped-or-weakened set for any available partial
  bind is nonempty, as enumerated in the clause table. Deposit is blocked.
...
- **Witness vs universal:** a concrete factor or one return gap cannot replace
  the source's `任一 m-柱`; universal carrier coverage remains required.
