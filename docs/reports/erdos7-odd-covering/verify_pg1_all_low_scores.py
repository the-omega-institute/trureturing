...
def main():
    ...
    parser.add_argument('--source', type=Path, default=HERE / 'mod3_conditioned_geometry_certificate.json')
    parser.add_argument('--fixed-certificate', type=Path, default=HERE / 'exact_signed_digit_dp_certificate.json')
    parser.add_argument('--certificate', type=Path, default=HERE / 'pg1_all_low_scores_certificate.json')
    parser.add_argument('--binary', type=Path)
    parser.add_argument('--write', action='store_true')
    args = parser.parse_args()
    started = time.monotonic()
    fixed_raw = args.fixed_certificate.read_bytes()
    fixed = json.loads(fixed_raw)
    require(sha256(args.source.read_bytes()).hexdigest() == fixed['source_sha256'],
            'existing fixed-A certificate binds the actual PG1 source')
    auxiliary = fixed['joint_auxiliary_example']
    require(auxiliary['depth'] == [1, 1, 1] and auxiliary['formula'] ==
    ...
    certificate = {'schema_version': 1,
                   ...
                   'fixture_formula': auxiliary['formula'],
    ...
    if args.write:
        ...
        require(json.loads(args.certificate.read_text()) == certificate, 'deterministic certificate matches exact recomputation')
