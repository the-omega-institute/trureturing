...
def compute_certificate():
    ...
    transport_coefficient = F(57, 4) * seven_positive_weight * eleven_complete_weight
    require(transport_coefficient == F(1159, 90), 'Positive-seven layout transport mismatch')
    ...
    require(objective == vector({'t': F(704, 6075), '35': F(1159, 90),
    ...
    rows = [
        ('append_7', vector({'35': 1 - metrics[(3, 5)][0] / 5, '357': -1}), F(0), F(2135, 108)),
        ...
        ('append_11', vector({'357': 1 - F(1649, 360) / 9}), F(1), F(66606, 1591)),
    ...
    require(head == F(5015891, 47730) < F(168332, 1591), 'New exact head failed')
    ...
    return {
        ...
        'head_bound': str(head), 'previous_head_bound': '168332/1591',
        'head_improvement': str(F(168332, 1591) - head), 'bridge': bridge,
...
def main():
    data = json.loads(read_artifact_text(Path(__file__).resolve().parent / 'certificates/common_density_head_certificate.json'))
