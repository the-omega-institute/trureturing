...
CERTIFICATE = 'certificates/source_norms/j-source/j_face_complete_survival_linear_heads.json'
...
def scan_job(base_string, job):
    ...
    provider = module('j_additional_scanner',base/'frontier/j-source/j_face_coupled_seven_heads.py')
...
def calculate(base, workers=2):
    ...
    predecessor = json.loads(io.read_artifact_bytes(base/'certificates/source_norms/j-source/j_face_coupled_seven_heads.json'))
    pins = dict(PINS)
