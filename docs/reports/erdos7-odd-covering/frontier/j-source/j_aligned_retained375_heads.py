...
prefix=read(args.prefix);heavy=read(B/'certificates/source_norms/j-source/j_face_retained375_heavy_heads.json');zero_data=p.zero_induction(data,heavy);zero=set(zero_data['zero_columns']);co={int(t):F(v)for t,v in prefix['coefficients'].items()};p.require(co==p.coefficients(B,prefix['name']),'Exact original own-load coefficients');target=F(prefix['target_scheduling_upper']);constant=sum(co.values())*(data['complete_zero7_tail']+data['complete_positive7_tail']);leaves=[]
