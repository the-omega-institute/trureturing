#!/usr/bin/env python3
"""Complete J comparison with the retained375 AP11-0 source bound."""
import argparse
...
CERTIFICATE = 'certificates/source_norms/j-source/j_face_retained375_ap11_complete_moment_cost_comparison.json'
PINS = {'frontier/j-source/j_face_higher_pure_hinge_complete_moment_cost_comparison.py': '02b7f5ca0a2af1ef809d719fa637552b5a7a56be00832fe6d32ee592e4f0c2f3', 'certificates/source_norms/j-source/j_face_higher_pure_hinge_complete_moment_cost_comparison.json': '856041c459fe1d47b2b1113b4dda8e03d99f38ba0248b1f45afd4a16c2cc2f01', 'profile-notes/j-source/291-two-higher-pure-hinges-improve-the-complete-j-comparison.md': '5ba10dd25a7fa32a91f4f41738f087356e9eb77faac0e9c385fd94ed080b245e', 'frontier/j-source/j_face_retained375_ap11_heads.py': '85453d3df342a60ac3589d610e8330ebd56d987ea074464834ccc1e25186803e', 'certificates/source_norms/j-source/j_face_retained375_ap11_heads.json': '97e17693e88033c9c6c932b445a0a85dcab8c70dbd05dc46df245a7d0a3c96b8', 'profile-notes/j-source/292-retaining375-strengthens-the-complete-j-ap11-head.md': '0300d5c6d7a3c05d56f7d3daea19e6f1dddf31ac19c8426f0ec1f03eeb657928'}
...
def inputs(base):
    ...
    prior=module('retained375_ap11_comparison291',base/'frontier/j-source/j_face_higher_pure_hinge_complete_moment_cost_comparison.py')
    ...
    old=read('j-source/j_face_higher_pure_hinge_complete_moment_cost_comparison')
    heads=read('j-source/j_face_retained375_ap11_heads')
    pins=dict(data['pins'])
    ...
    require([r['name']for r in old['basis']]==names and [F(r['upper'])for r in old['basis']]==bounds
            ...
            'The exact final291 baseline retains all34 observations and all52 original costs')
    pos=names.index('AP11-0');prior_bound=bounds[pos];upper=F(heads['complete_AP11_upper'])
    require(heads['schema']=='erdos7-j-face-retained375-ap11-heads-v1'
    ...
    require(len(rows)==len(leaves)==c['lp_remaining']==heads['residual_leaf_count']
            ...
            and F(ledger['fixed_pruning_benchmark'])==fixed,
    ...
    co={int(t):F(a)for t,a in heads['coefficients'].items()}
    require(heads['name']=='AP11-0' and heads['block']==0 and F(heads['at_one'])==0
            and co=={1:F(1355,263538),2:F(20425,263538),3:F(25,363),5:F(28,33)}
            and sum(co.values())==F(heads['complete_late_slope'])==1,
            'The unchanged original complete AP11-0 positive hinge combination, including H1')
    seeds=heads['seed_results']
    require(len(seeds)==50 and len({tuple(r['projection441_735'])for r in seeds})==50
            and all(0<=r['projection441_735'][0]<5 and 0<=r['projection441_735'][1]<2
                    and 0<=r['projection441_735'][2]<5 and F(r['complete_tail_constant'])==F(774979,19845000)for r in seeds)
            and fixed==max(F(r['complete375_upper'])for r in seeds),
            'All50 original266 seed completions determine the fixed full-domain pruning benchmark')
    require(upper==max([fixed]+[F(r['adopted_upper'])for r in rows])
            and heads['rational_column_checks']==heads['independent_direct_column_checks']==12941*heads['distinct_dual_count']
            and 0<upper<prior_bound==F(heads['previous_complete_AP11_upper'])
            and F(heads['improvement_over_previous'])==prior_bound-upper
            and polynomials[pos]==(-sum(t*a for t,a in co.items()),F(1),F(0))
            and all(functions[pos](n)==sum(a*max(n-t,0)for t,a in co.items())for n in range(1,TAIL_ENTRANCE)),
            'The same whole-positive-integer AP11-0 function receives a strict uniform source improvement')
    bounds[pos]=upper
    data.update(pins=pins,names=names,functions=functions,polynomials=polynomials,bounds=bounds,old=old,
                changed_basis_bounds=[{'name':'AP11-0','previous_upper':prior_bound,'updated_upper':upper,'source':'292'}],added_basis_bounds=[])
    return data
...
def calculate(base, proof_data):
    ...
    for proof, (name, target, target_polynomial) in zip(proof_data, targets):
        ...
        require(upper <= rational(old_targets[name]['upper']), 'Each new envelope is no worse than its complete291 bound')
        witness = {}
    ...
    result = {'schema': 'erdos7-j-face-retained375-ap11-complete-moment-cost-comparison-v1', 'source_sha256': data['pins'],
