#!/usr/bin/env python3
"""Complete J comparison with joint raw and survivor factorial-cross bounds."""
import argparse
...
CERTIFICATE = 'certificates/source_norms/j-source/j_face_joint_factorial_cross_complete_moment_cost_comparison.json'
PINS = {'certificate_io.py': '287582353eeb0674f4e80530ebf268228b023f6088d14c819488a56111d0b232', 'frontier/j-source/j_face_shifted_factorial_complete_moment_cost_comparison.py': '7a4fddb5772f8afb833c92430c0c12087f0416125f4a3548091eb76c3a09ba48', 'certificates/source_norms/j-source/j_face_shifted_factorial_complete_moment_cost_comparison.json': '269ea14c090b913196f1545340ce01c45b81c33491fffe2c5b94ea3dfbdfa1ff', 'profile-notes/j-source/271-three-shifted-factorial-costs-improve-the-complete-j-comparison.md': '61ac7b0960d0693db948ffce7b72e637e60a54054bdf7d07ffa7d56b157ecf28', 'frontier/j-source/j_face_joint_factorial_cross_heads.py': 'ecdc89af4c5228693f3aa17211b5d13836c5f58e9f436e05b42e768527d84aba', 'certificates/source_norms/j-source/j_face_joint_factorial_cross_heads.json': 'ed567355b0475ab7f5712dca3fda30866638452efaab1bfbcadab5f3e7f33fdb', 'profile-notes/j-source/272-joint-raw-and-survivor-crosses-strengthen-three-original-j-costs.md': '410ff46a80f194974fb03f0bbf9c5b1aa6d5fccfdf4fa668d69b5c9d4f7364fe'}
...
def inputs(base):
    ...
    prior_module = module('joint_factorial_cross_j_envelope_previous', base/'frontier/j-source/j_face_shifted_factorial_complete_moment_cost_comparison.py')
    ...
    old = read('j-source/j_face_shifted_factorial_complete_moment_cost_comparison')
    heads = read('j-source/j_face_joint_factorial_cross_heads')
    pins = dict(data['pins'])
    ...
    require([row['name'] for row in old['basis']] == names
            ...
            'The exact complete271 baseline with all25 moment constraints')
    require(heads['original_cost_indices'] == [41, 47, 48]
            ...
            and heads['positive7_cross_cost_indices'] == [41, 47, 48] and heads['zero7_cross_cost_indices'] == [41, 48]
            and [row['retained_zero7_cross'] for row in heads['results']] == [True, False, True],
            'The complete272 unchanged original costs, all source columns and all exponent tails')
    ...
    for row in heads['results']:
        ...
        require(complete == F(scan['complete_cost_upper']) and upper == min(complete, prior)
                ...
                'The complete272 source bound improves its own original271 target and retains every containing choice')
        require(name in names, 'Every new cost already belongs to the complete25-observation basis')
        k = names.index(name)
        require(functions[k] == fn and polynomials[k] == poly and prior <= bounds[k] and upper < bounds[k],
                'The same original basis function improves; its old envelope can already be below its old basis bound')
        changed.append({'name': name, 'previous_upper': bounds[k], 'updated_upper': upper, 'source': '272'})
        bounds[k] = upper
    require(len(changed) == 3 and [row['name'] for row in changed] == ['cost41', 'cost47', 'cost48']
            and not added and sum(a != b for a, b in zip(bounds, prior_bounds)) == 3
            and len(names) == len(functions) == len(polynomials) == len(bounds) == 25,
            'All25 prior observations remain, with exactly three stronger original-cost bounds')
...
def calculate(base, proof_data):
    ...
    for proof, (name, target, target_polynomial) in zip(proof_data, targets):
        ...
        require(upper <= rational(old_targets[name]['upper']), 'Each new envelope is no worse than its complete271 bound')
        witness = {}
    ...
    result = {'schema': 'erdos7-j-face-joint-factorial-cross-complete-moment-cost-comparison-v1', 'source_sha256': data['pins'],
              ...
              'scope': 'Complete original52-cost comparison on both entire actual saturated J faces. All25 previous scalar basis functions remain; only original cost41/cost47/cost48 bounds improve using272 actual common raw and survivor factorial crosses. Every original function keeps independently chosen original test labels. All59 exact whole-integer-load envelopes and59 fresh independent finite moment witnesses are checked against all25 updated constraints. Exact mass3/20, all signed payments, all four AP11 blocks, AP13 and the complete count tail are recomputed. The witnesses constrain only this updated scalar-moment method and need not be actual source configurations or coexist on one actual family. The old271 method bracket is not reused. No actual attainment, off-face/global join, unrestricted Erdos7 result or Lean verification is asserted.'}
