...
CERTIFICATE = 'certificates/source_norms/j-source/j_face_alignment.json'
PINS = {
...
def calculate(base):
    ...
    prior = module('j_face_prior', base/'frontier/endpoint_linear_neighborhood.py')
    for path, pin in prior.PINS.items():
        require(sha256((base/path).read_bytes()).hexdigest() == pin, 'Inherited input '+path)
    saturation = prior.load(base, 'frontier/source_barrier_saturation.py', 'j_face_source')
    ...
    constructor = prior.load(base, 'frontier/sharp_source_mass_endpoints.py', 'j_face_constructor')
