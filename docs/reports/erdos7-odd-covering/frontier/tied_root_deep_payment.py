...
CERTIFICATE = 'certificates/source_norms/tied_root_deep_payment.json'
PINS = {
...
def calculate(base):
    ...
    previous = read('certificates/source_norms/quadratic_marked_global.json')
    for path, pin in previous['source_sha256'].items():
        ...
        used[path] = pin
    prior_module = module('tied_deep_previous', base/'frontier/quadratic_marked_global.py')
    ...
    schedule = module('tied_deep_schedule', base/'frontier/ap_schedule.py')
    fixed = module('tied_deep_fixed', base/'frontier/fixed_cost.py')
