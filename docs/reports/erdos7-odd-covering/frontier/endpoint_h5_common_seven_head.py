...
CERTIFICATE = 'certificates/source_norms/endpoint_h5_common_seven_head.json'
PINS = {
...
def calculate(base):
    ...
    used = dict(PINS)
    for previous in (read('certificates/source_norms/endpoint_square_positive5.json'),
                     read('certificates/source_norms/endpoint_survival_scalar_barrier.json')):
        for path, pin in previous['source_sha256'].items():
            ...
            used[path] = pin
    linear = module('h5_seven_linear', base/'frontier/endpoint_linear_numerator.py')
    head = module('h5_seven_head', base/'frontier/endpoint_square_coherent_head.py')
    ...
    require(encode(weighted_descendant) == read('certificates/source_norms/endpoint_square_positive5.json')['descendant5_cell_caps'],
    ...
    old_scalar = read('certificates/source_norms/endpoint_survival_scalar_barrier.json')
