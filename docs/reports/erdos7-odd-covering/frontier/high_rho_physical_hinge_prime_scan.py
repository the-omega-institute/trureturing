...
DEFAULT_PROOF='profile-notes/331-complete-physical-hinges-continue-finite-sources-through43.md'
...
def calculate(base,proof):
    ...
    h330=module('hingeprime_330',base/'frontier/high_rho_survival_through29.py')
    parent_path=base/'certificates/source_norms/j_whole_face_high_rho_finite_guard.json'
    ...
    paths=[base/'certificate_io.py',base/'verify_joint_frontier.py',parent_path,parent_proof,parent_producer,
           base/'frontier/high_rho_survival_through29.py',base/'profile-notes/330-a-finite-high-surplus-source-survives-through29.md',
           base/'profile-notes/302-a-positive-actual-source-neighborhood-keeps-j-below403.md',
           base/'profile-notes/311-imperfect-j-source-alignment-forces-a-sharp-sector-surplus.md',
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--base',type=Path,default=Path(__file__).resolve().parents[1]);p.add_argument('--proof',type=Path);p.add_argument('--certificate',type=Path)
