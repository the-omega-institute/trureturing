"""Consume full absorbed margins on the same actual source-mass interval.

Both S=D and S=s are evaluated. The exact full margin values improve the
predecessor epsilon lower bounds; every fixed target uses the endpoint
selected by its coefficient sign. Original labels and tails are unchanged.
"""
...
def consume(source, core, kc, encode, previous, pure, rows):
    ...
    for index, (parameter, row) in enumerate(zip(parameters, rows)):
        ...
        mg, Mq, Ml, m25, m4, m5 = (F(row[k]) for k in
            ('source_margin', 'Mquad', 'M41', 'survival_hinge_margin',
             'full_absorbed_hinge_4_margin', 'full_absorbed_hinge_5_margin'))
        e4, e5 = (F(row['absorbed_hinge_'+str(t)+'_epsilon']) for t in (4, 5))
        raw4, raw5 = (source.raw357(F(t), dat) for t in (4, 5))
        old4, old5 = (F(row['absorbed_hinge_'+str(t)+'_margin']) for t in (4, 5))
        gain4, gain5 = (F(row['full_absorbed_hinge_'+str(t)+'_gain']) for t in (4, 5))
        require(e4 >= 0 and e5 >= 0 and old4 == D-raw4+e4 and old5 == D-raw5+e5
                and m4 == old4+gain4 and m5 == old5+gain5 and min(gain4, gain5) > 0,
                'Full margins strictly improve the same certified predecessor lower bounds')
        Ms = m4/6+F(4, 33)*m5+m25/22
        old_Ms = old4/6+F(4, 33)*old5+m25/22
        delta = Q*D+Ms
        old_delta = Q*D+old_Ms
        require(delta == old_delta+gain4/6+F(4, 33)*gain5 and old_delta > 0 and delta > old_delta,
                'Exact raw survival improvement with unchanged fully charged constants')
        ...
        for endpoint in (D, s):
            prior_delta = Q*endpoint+old_Ms
            require(prior_delta > 0, 'Positive predecessor denominator at both source-mass endpoints')
            for key in keys:
                prior = constants[key]+(slopes[key]*endpoint-corrections[key])/prior_delta
                old_targets[key] = prior if old_targets[key] is None else max(old_targets[key], prior)
            prior_rho = prior_delta/endpoint
        ...
        current.append({'index': index, 's': s, 'D': D, 'Delta': delta,
                        ...
                        'survival_hinge_margin': m25, 'full_absorbed_hinge_4_margin': m4,
                        'full_absorbed_hinge_5_margin': m5, 'gain4': gain4, 'gain5': gain5,
                        'absorbed_hinge_4_margin': old4, 'absorbed_hinge_5_margin': old5,
    ...
    require(all(old_targets[k] == F(previous[k]) for k in keys) and old_rho == F(previous['rho']),
            'Published profile42 targets and controlling survival reconstructed at both endpoints')
    ...
    require(all(targets[key] < F(previous[key]) for key in keys) and rho > F(previous['rho']),
            'All numerator targets and actual survival improve the published profile42 bounds')
    return {**targets, 'rho': rho, 'q_effective': Q,
            ...
            'scope': 'Full h4/h5 absorption margins and the h(5/2) margin use the same actual S in [D,s], with all barriers unchanged. Every fixed target selects its endpoint by coefficient sign. Full values replace the predecessor epsilon lower bounds. Independent original labels and complete tails retained; ordinary proof plus exact arithmetic, no Lean or unrestricted Erdos7 endpoint.'}
