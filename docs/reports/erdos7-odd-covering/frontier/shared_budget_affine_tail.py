...
CERTIFICATE = 'certificates/source_norms/shared_budget_affine_tail.json'
PINS = {
...
def calculate(base):
    ...
    cost = module('affine_tail_cost', base/'frontier/complete_off_face_cost.py')
    for path, pin in cost.PINS.items():
        require(sha256(io.read_artifact_bytes(base/path)).hexdigest() == pin, 'Pinned cost input '+path)
    finite = module('affine_tail_finite', base/'frontier/finite_source_face_transport.py')
    mean = module('affine_tail_mean', base/'frontier/joint_deep_mean_transport.py')
    tails = module('affine_tail_complete', base/'frontier/complete_off_face_omitted_tails.py')
