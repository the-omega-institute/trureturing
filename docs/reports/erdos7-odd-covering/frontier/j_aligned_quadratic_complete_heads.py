...
CERTIFICATE='certificates/source_norms/j_aligned_quadratic_complete_heads.json'
...
def encode(x):
 ...
 return x
def context(B):
 p=mod('retained_quadratic_actual375',B/'frontier/j_aligned_quadratic_sharp_source.py');data=p.build(B);lp,io=data['lp'],data['io'];load=lambda path:json.loads(io.read_artifact_bytes(path),object_pairs_hook=io._unique)
 heavy=load(B/'certificates/source_norms/j_face_retained375_heavy_heads.json');zero=p.zero_induction(data,heavy)
 zero['new_explicit_E5_zero_columns']=sorted({850+5*c+s for c in range(5)for s in range(5)if s!=3}-set(zero['zero_columns']))
 aligned=mod('retained_quadratic_actual313',B/'frontier/j_aligned_joint_selected_heads.py');small=aligned.strengthen(aligned.build_source(B,'retained_quadratic_actual_coarse'));j=small['j']
 moment=mod('retained_quadratic_moment',B/'frontier/j_face_shared_square_factorial.py');mom=moment.JMomentHead(j)
 mom.descendant=tuple(F(2,27)if i==14 else v for i,v in enumerate(mom.descendant))
 coherent=mod('retained_quadratic_raw_caps',B/'frontier/j_face_coherent_positive7_pairs.py')
 ...
 raw314=load(B/'certificates/source_norms/j_aligned_raw_prime_path_moments.json')
...
def conditional_raw(m):
 ...
 paths=mod('conditional_actual_paths',B/'frontier/j_face_raw_prime_path_pairs.py');moment=mod('conditional_actual_moment',B/'frontier/j_face_shared_square_factorial.py');mom=moment.JMomentHead(j)
...
def source_pins(m):
 paths=('frontier/j_aligned_quadratic_complete_heads.py','frontier/j_aligned_quadratic_sharp_source.py','frontier/j_aligned_retained375_source.py','frontier/j_aligned_joint_selected_heads.py','frontier/j_face_shared_square_factorial.py','frontier/j_face_coherent_positive7_pairs.py','frontier/j_face_raw_prime_path_pairs.py','certificates/source_norms/j_aligned_raw_prime_path_moments.json','certificates/source_norms/j_aligned_retained375_heavy553_heads.json','certificates/source_norms/j_face_retained375_heavy_heads.json',m.aligned.ORIGINAL)
 pins={path:sha256(m.io.read_artifact_bytes(m.B/path)).hexdigest()for path in paths}
 for path in('certificates/source_norms/j_aligned_joint_selected_heads.json','certificates/source_norms/j_aligned_raw_prime_path_moments.json'):
...
def main():
 parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--base',type=Path,default=Path(__file__).resolve().parents[1]);parser.add_argument('--certificate',type=Path);parser.add_argument('--proposal-directory',type=Path)
