...
CERTIFICATE = 'certificates/source_norms/whole_block_ap11_survival.json'
PINS = {
...
def calculate(base):
    ...
    for prior in (previous, stop, survival):
        for path, pin in prior['source_sha256'].items():
            ...
            used[path] = pin
    common = module('ap11_block_common', base/'frontier/whole_cost_common_stop_loss.py')
    bridge = module('ap11_block_bridge', base/'frontier/k_face_common_seven_hinges.py')
    law = module('ap11_block_law', base/'frontier/whole_face_ap11_survival.py')
