...
CERTIFICATE = 'certificates/source_norms/whole_face_stop_loss_generator.json'
PINS = {
    ...
    'certificates/source_norms/vector_face_complete_ratio.json': '321fea6802b6920457002abef9cc2c88c603d85d82f8ffcee2e67c45dc0560de',
...
def calculate(base):
    ...
    previous = read('certificates/source_norms/vector_face_complete_ratio.json')
    survivor = read('certificates/source_norms/k_face_surviving_tail_ratio.json')
    old_hinges = read('certificates/source_norms/k_face_common_seven_hinges.json')
    for record in (previous, survivor, old_hinges):
        for path, pin in record['source_sha256'].items():
            ...
            used[path] = pin
    old84 = read('certificates/source_norms/k_face_complete_ratio.json')
    bridge = module('stop_loss_bridge', base/'frontier/k_face_common_seven_hinges.py')
    profile = stop_loss_profile(bridge, old_hinges, survivor)
    engine = module('stop_loss_engine', base/'frontier/source_barrier_saturation.py').Experiment(base)
    ...
    consumer = module('stop_loss_consumer', base/'frontier/vector_face_complete_ratio.py')
    majorant = module('stop_loss_majorant', base/'frontier/endpoint_numerator_common_costs.py')
