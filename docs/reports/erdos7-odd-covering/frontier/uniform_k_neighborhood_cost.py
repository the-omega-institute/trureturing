...
PINS = {
    ...
    'frontier/actual_five_slot_source_modulus.py': 'dab8cbce31dd85d8a6476c1b13a8146159ff748fa6ce94d78775febf4f3832c0',
}
...
def calculate(base):
    ...
    cost = module('uniform_k_cost', base/'frontier/complete_off_face_cost.py')
    for path, pin in cost.PINS.items():
        require(sha256(io.read_artifact_bytes(base/path)).hexdigest() == pin, 'Pinned cost input '+path)
    finite = module('uniform_k_finite', base/'frontier/finite_source_face_transport.py')
    affine = module('uniform_k_affine', base/'frontier/shared_budget_affine_tail.py')
    mean = module('uniform_k_mean', base/'frontier/joint_deep_mean_transport.py')
    source = module('uniform_k_source', base/'verify_joint_frontier.py')
    capacity = module('uniform_k_capacity', base/'frontier/broad_weighted_identity_source.py')
