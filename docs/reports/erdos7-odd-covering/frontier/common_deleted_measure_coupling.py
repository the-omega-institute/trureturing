...
def calculate(base):
    parent = load(base, 'frontier/source_cost_endpoint_attainment.py')
    source = parent.load(base, 'verify_joint_frontier.py', 'deletion_source')
    constructor = parent.load(base, 'frontier/sharp_source_mass_endpoints.py', 'deletion_constructor')
...
def main():
    ...
    if args.check:
        certificate = args.base/'certificates/source_norms/common_deleted_measure_coupling.json'
