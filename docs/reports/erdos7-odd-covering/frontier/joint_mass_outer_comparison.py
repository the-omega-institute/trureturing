...
CERTIFICATE = 'certificates/source_norms/joint_mass_outer_comparison.json'
PINS = {'certificate_io.py': '287582353eeb0674f4e80530ebf268228b023f6088d14c819488a56111d0b232', 'frontier/product_source_outer_comparison.py': 'f906face2a53dade240f652877fe62d31eb258ef62f4e8e6d934af043b7805e3', 'certificates/source_norms/product_source_outer_comparison.json': '03af583245956a8be39b972abd16dbafa5d049f2a56f7aa4855b7c44f41a9404', 'frontier/joint_orientation_carrier_mass.py': '893e2c06ee89d1f456a96ec1fc6504ee4a91ccd1a8ad6c75a437f3e255bb94ee', 'certificates/source_norms/joint_orientation_carrier_mass.json': '69d914c78bf157754b651adbe2f3587cad93b56964bbc52024cdda266f5df997'}
...
def calculate(base):
    ...
    old = module('joint_outer_prior', base/'frontier/product_source_outer_comparison.py')
    previous = old.calculate(base)
    require(previous == json.loads(io.read_artifact_bytes(base/'certificates/source_norms/product_source_outer_comparison.json')),
            'Reconstruct149 including original46 costs, complete tails, outside branches and shared residual')
    mass = module('joint_outer_mass', base/'frontier/joint_orientation_carrier_mass.py')
    require(mass.calculate(base) == json.loads(io.read_artifact_bytes(base/'certificates/source_norms/joint_orientation_carrier_mass.json')),
