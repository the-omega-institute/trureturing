#!/usr/bin/env python3
"""Independent lifted-profile directed audit, no candidate arithmetic imports.

Product convolution is evaluated by each output integer and its divisors.
Head moments use the exact atomic laws. Every full prime-stage triple is
compared, and all required checks remain active under Python -O.
"""
import argparse
...
SOURCES=('certificate_io.py', 'problem-details/54-depth-profile-head-laws-with-unrestricted-original-tails.md', 'frontier/source-budgets/depth_profile_head_input.json', 'frontier/source-budgets/depth_profile_tail_budget.py', 'certificates/source_norms/source-budgets/depth_cap_bellman.json', 'certificates/source_norms/source-budgets/depth_profile_tail_budget.json')
...
def calculate(base):
    io=load_module('independent_tail_io',base/'certificate_io.py')
    given=json.loads(io.read_artifact_bytes(base/'certificates/source_norms/source-budgets/depth_profile_tail_budget.json'),object_pairs_hook=io._unique)
    profile_input=json.loads(io.read_artifact_bytes(base/'certificates/source_norms/source-budgets/depth_cap_bellman.json'),object_pairs_hook=io._unique)
    arithmetic_input=json.loads(io.read_artifact_bytes(base/'frontier/source-budgets/depth_profile_head_input.json'),object_pairs_hook=io._unique)
    require(given['schema']=='depth-profile-tail-budget-v1' and
            profile_input['schema']=='depth-cap-bellman-v1','current certificate schemas')
    for record,path in ((given,'frontier/source-budgets/depth_profile_tail_budget.py'),
                        (profile_input,'frontier/source-budgets/depth_cap_bellman.py')):
        require(record['producer_sha256']==sha256((base/path).read_bytes()).hexdigest(),
                'current certificate producer: '+path)
        for name,digest in record['source_sha256'].items():
            require(sha256(io.read_artifact_bytes(base/name)).hexdigest()==digest,
                    'current certificate dependency: '+name)
    B=16384
    scale=10**18
    cap=(2*B+1)//5
    ...
    require(digest==given['step_digest'] and [list(row) for row in steps]==given['stages'],'every directed stage agrees')
    ...
    result=dict(scope='Independent exact directed audit of this fixed154 head with uniform extra digits; arbitrary finite head exponents in original tail cofactors are covered by the geometric auxiliary domination.',
    ...
    # Independently subtract the exact geometric remainder from each full mean.
    # This differs from the producer's explicit sum through the finite cutoff.
    full=exact_head_mean
    cut=F(1)
    for p,h,r in zip(arithmetic_input['prime_order'],heights,profiles):
        full_factor=1+sum(r[1:],F(0))+r[h]/(p-1)
        remaining=r[h]/((p-1)*p**(5-h))
        cut*=full_factor-remaining
    error=full-cut
    extra=given['high_head_cutoff']
    require(error==F(extra['union_loss_upper']) and error<F(7,400),'independent high-head union loss')
    require(epsilon+C+error+(J-1)/(short_threshold-1)==F(extra['consumer_score_upper'])<1,
            'independent high-head sufficient score')
    require(1-epsilon-C-error==F(extra['survival_lower'])>F(9,80),'independent enlarged surviving mass')
    require(1+(J-1)/F(extra['survival_lower'])==F(extra['Gamma_upper'])<F(719929,9)<short_threshold,
            'independent enlarged complete-layout moment bound')
    result['high_head_cutoff']=dict(extra)
    return result
...
def main():
    ...
    path=args.base/'certificates/source_norms/source-budgets/depth_profile_tail_verification.json'
    if args.write:
