...
head solver is not called: every actual local leaf is evaluated and allocated
...
CERTIFICATE = 'certificates/source_norms/source-budgets/balanced_profile_head_verification.json'
...
CANDIDATE = 'frontier/source-budgets/balanced_profile_head.py'
CANDIDATE_CERTIFICATE = 'certificates/source_norms/source-budgets/balanced_profile_head.json'
SOURCES = ('certificate_io.py',
           'problem-details/57-balanced-depth-profile-and-degree-seven-frontier.md',
...
def calculate(base):
    ...
    candidate=bind_certificate(base,io,source_bytes,CANDIDATE_CERTIFICATE,CANDIDATE,'balanced-profile-head-v1')
    ...
    expected = Fraction(39891291142993164384829733829906791483779,
                        130822105133669014006347656250000000000000)
    require(actual == expected == Fraction(budget['epsilon_exact']), 'independent balanced-profile154 optimum exact equality')
    ...
    require(actual < Fraction(61,200), 'verified balanced head mass')
    ...
    result = dict(schema='balanced-profile-head-verification-v1',
...
def main():
    ...
    path=args.base/'certificates/source_norms/source-budgets/balanced_profile_head_verification.json'
    if args.write:
    ...
    print(json.dumps({key:result[key] for key in ('schema', 'epsilon_exact', 'states', 'actual_coordinate_leaf_evaluations')},indent=2))
