#!/usr/bin/env python3
"""Fresh complete original-head bound at source radius1/25 and residual1/1000."""
import argparse
...
CERTIFICATE = 'certificates/source_norms/source-budgets/fresh_full_slot_source_comparison.json'
HEAD_CERTIFICATE = 'certificates/source_norms/source-budgets/fresh_full_slot_source_heads.json'
DELTA, RHO = F(1, 25), F(1, 1000)
TARGET = F(101857, 200)
PINS = {
...
def parameters_and_guards(study):
    ...
    require(d == F(1, 25) < F(2, 27) and rho == F(1, 1000) and r == F(1, 200),
    ...
    gap = min(gaps)
    require(total_loss == F(1, 24) and Delta == F(1, 96) < F(1, 18)
            and D == F(3, 292) <= Delta and F(3, 4)+d/4 == F(19, 25) < F(4, 5),
            'Actual r=0 source lies inside162; exposed price bounds the same p+a+b')
    require(gap == F(151, 9000) > 0 and rho/gap == F(9, 151) < F(1, 5),
            'All five general first-label guards and their full common defect simplex')
    require(F(1, 4)-3*d/4 == F(11, 50) > F(1, 20),
    ...
    require(min(polynomials.values()) > 0 and 6*d-23 < 0,
            'Every source domain polynomial stays positive throughout[0,1/25]')
    ...
    require(par['v0'] == F(49, 2400) and par['v1'] == F(365, 14304),
            'Reconstructed full-r cap increments on the new source radius')
    P = max(hbar/(5*gap), F(65, 9))
    require(hbar/(5*gap) == F(904, 151) and P == F(65, 9)
...
def calculate(base, scan=False, write_heads=False):
    ...
    if scan:
        ...
        print('Source1/25 original heads elapsed_seconds='+str(perf_counter()-started), flush=True)
        if write_heads:
    ...
    require(result['comparison_upper'] < 509 < TARGET and heads['original_head_evaluations'] == 9750000,
    ...
    return study.uniform.encode({'schema': 'erdos7-fresh-full-slot-source-comparison-v1', 'source_sha256': pins,
        ...
        'scope': 'Ordinary complete actual-source bound for both K orientations, sigma<=1/25 and rho<=1/1000, with only the actual r<=5rho. Re-establishes every actual first-label/gap/heavy guard and evaluates9.75 million new original heads with312 independent rational LP comparisons. All52 independent original costs, five denominator objectives, actual mass and every infinite tail remain. No new global K bound, Lean verification, unrestricted Erdos7 result or actual attainment of the relaxed maxima is asserted.'})
...
def main():
    ...
    print('PASS: fresh sigma1/25, rho1/1000, no independent r cutoff; K='+result['comparison']['comparison_upper'])
