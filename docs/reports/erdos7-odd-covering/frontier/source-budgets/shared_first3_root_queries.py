"""First3-root restricted queries under the unchanged complete actual survivor law."""
...
CERTIFICATE='certificates/source_norms/source-budgets/shared_first3_root_queries.json'
SOURCES=('certificate_io.py', 'frontier/source-budgets/adaptive_phase_io.py', 'problem-details/67-shared-root-prefix-bounds-for-the-actual-survivor-law.md', 'problem-details/65-independent-survivor-marginal-ceiling-at-the-fixed-cutoff.md', 'frontier/source-budgets/uniform_phase_capacity_input.json', 'certificates/source_norms/source-budgets/adaptive_core7_policy.json', 'certificates/source_norms/source-budgets/adaptive_core7.json', 'frontier/source-budgets/adaptive_core7.py', 'certificates/source_norms/source-budgets/survivor_cylinder_queries.json', 'frontier/source-budgets/survivor_cylinder_queries.py', 'certificates/source_norms/source-budgets/survivor_cylinder_literals.json', 'frontier/source-budgets/verify_survivor_cylinder_literals.py')
...
def calculate(base):
    ...
    old=ctx.fresh('certificates/source_norms/source-budgets/survivor_cylinder_queries.json','frontier/source-budgets/survivor_cylinder_queries.py')
    ...
    capacity={'capacity_exact':old['absolute_E154_capacity']}
    START=time.monotonic();SECONDS=120;LIMIT=2_000_000
    class Stop(Exception):pass
    def check_budget():
        if time.monotonic()-START>=SECONDS:raise Stop('120-second root-query budget')
    ...
    old_by={r['modulus']:r for r in old['records']}
    exact={r['modulus']:list(map(F,r['all_residue_masses'])) for r in literal['records']}
    require(states[root]['axis']==0 and P[0]==3,'actual policy reads3 first')
    records=[]
    for m in sorted(x for x in coeff if x%3==0):
        check_budget()
        oldnum=value(root,m)
        left=m;power=1
        ...
        values=[max(bins[r::3]) for r in range(3)]
        require(values[0]==0 and max(values)==oldnum,'root0 killed; restricted roots recover scalar maximum')
        require(F(oldnum,den[127])==F(old_by[m]['survivor_max_upper']),'same prior relaxed upper')
        upper=[F(n,den[127]) for n in values]
        if m in exact:
            for r in range(3):
                actual=max(exact[m][r::3])
                require(actual<=upper[r],'exact root-restricted literal upper')
                upper[r]=actual
        records.append({'modulus':m,'restricted_upper':[str(x) for x in upper],'old_scalar_upper':str(max(upper)),'uses_exact_literal':m in exact})
    result={'schema':'actual64-first3-root-restricted-cylinder-v1','status':'PASS','records':records,'queries':len(records),'states':query_states,'head_survival':str(h),'common_denominator':str(den[127]),'scope':'First3 root retained globally; all remaining query phases are history-relaxed exactly as in65. Same actual law and H. Exact literal controls override four moduli.'}
    require(len(records)==2662 and sum(r['uses_exact_literal'] for r in records)==4,'all restricted moduli and all four literal replacements')
