...
def calculate(base):
    ...
    previous = read('certificates/source_norms/broad_marked_identity_global.json')
    for path, pin in previous['source_sha256'].items():
        ...
        used[path] = pin
    current74 = read('certificates/source_norms/global_k_face_gain.json')
    old49 = read('certificates/source_norms/full_linear_carrier_frontier.json')
    broad = read('certificates/source_norms/broad_five_slot_tradeoff.json')
    source = module('weighted_marker_source', base/'verify_joint_frontier.py')
    schedule = module('weighted_marker_schedule', base/'frontier/ap_schedule.py')
    fixed = module('weighted_marker_fixed', base/'frontier/fixed_cost.py')
