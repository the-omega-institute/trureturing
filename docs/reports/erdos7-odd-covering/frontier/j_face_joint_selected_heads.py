...
class JointSelectedJHead:
    def __init__(self, base, j, bank=None, proposer=None):
        ...
        raw = module('j_joint_raw', base/'frontier/joint_selected_source_comparison.py')
        retained = module('j_joint_retained', base/'frontier/retained_deletion_heavy_comparison.py')
...
def calculate(base, bank=None, proposer=None):
    ...
    previous = json.loads(io.read_artifact_bytes(base/'certificates/source_norms/j_face_coupled_seven_heads.json'),object_pairs_hook=unique)
    pins = dict(PINS)
    ...
    j = module('j_joint_original',base/'frontier/j_face_coupled_seven_heads.py')
    problem = JointSelectedJHead(base,j,bank,proposer)
    face = json.loads(io.read_artifact_bytes(base/'certificates/source_norms/j_face_alignment.json'),object_pairs_hook=unique)
