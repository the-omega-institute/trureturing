...
def calculate(base):
    ...
    previous = json.loads(io.read_artifact_bytes(base/'certificates/source_norms/endpoint_linear_complete_pure3_deletion.json'))
    for path, pin in previous['source_sha256'].items():
        ...
        used[path] = pin
    geometry = module('joint15_25_previous', base/'frontier/endpoint_linear_complete_pure3_deletion.py')
    require(encode(geometry.calculate(base)) == previous, 'Reproduced full77 geometry arithmetic and complete load')
    linear = module('joint15_25_source_tables', base/'frontier/endpoint_linear_numerator.py')
