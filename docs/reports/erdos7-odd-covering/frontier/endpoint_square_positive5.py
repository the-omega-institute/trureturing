...
def calculate(base):
    ...
    spec = importlib.util.spec_from_file_location('positive5_deep', base/'frontier/endpoint_square_common_pure3.py')
    ...
    linear = deep.load(base, 'frontier/endpoint_linear_numerator.py', 'positive5_linear')
    head = deep.load(base, 'frontier/endpoint_square_coherent_head.py', 'positive5_head')
    old = json.loads((base/'certificates/source_norms/endpoint_square_common_pure3.json').read_text())
...
def main():
    ...
    if args.check:
        expected = args.base/'certificates/source_norms/endpoint_square_positive5.json'
