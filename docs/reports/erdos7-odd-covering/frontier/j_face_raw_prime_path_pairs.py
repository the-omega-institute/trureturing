...
CERTIFICATE='certificates/source_norms/j_face_raw_prime_path_pairs.json'
PINS={'certificate_io.py': '287582353eeb0674f4e80530ebf268228b023f6088d14c819488a56111d0b232', 'frontier/j_face_coherent_positive7_pairs.py': 'd643cc68b0c0d076a29246e78c75d1f6d6778617a852fa0bb738a18d2554619c', 'certificates/source_norms/j_face_coherent_positive7_pairs.json': '1840efa1ced6f86beb990cb03ee4cce3be8c3e04d1508268c598c7f0a5076845', 'profile-notes/258-independent-positive-seven-depths-share-one-complete-raw-source.md': '2d2ebe6ca7762c9b7da6b8900c2412667f2df303303fe4a3ae5b5d60951c5090', 'frontier/j_face_pure_path_square.py': '056391a969873856f4b15ca3eb1e0a24fd2635b06cc057383b60c587419baf88', 'profile-notes/253-two-complete-prime-path-blocks-sharpen-the-j-square.md': '8fe6ab808afc912aa3e38e16ed3e3a460234982d815204fa6710f210f42cb3ab'}
...
def calculate(base):
    ...
    io=module('raw_path_io',base/'certificate_io.py');previous=module('raw_path_previous',base/'frontier/j_face_coherent_positive7_pairs.py')
    ...
    j=module('raw_path_source',base/'frontier/j_face_coupled_seven_heads.py');moment=module('raw_path_moment',base/'frontier/j_face_shared_square_factorial.py');joint=module('raw_path_head',base/'frontier/j_face_joint_retained_square.py');tails=module('raw_path_tails',base/'frontier/complete_off_face_factorial_tail.py')
...
def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--base',type=Path,default=Path(__file__).resolve().parents[1]);modes=parser.add_mutually_exclusive_group();modes.add_argument('--write',action='store_true');modes.add_argument('--check',action='store_true');args=parser.parse_args()
    result=calculate(args.base);io=module('raw_path_writer',args.base/'certificate_io.py');old=module('raw_path_json',args.base/'frontier/j_face_coherent_positive7_pairs.py')
