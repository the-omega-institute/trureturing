...
CERTIFICATE = 'certificates/source_norms/global_k_face_gain.json'
PINS = {
...
def calculate(base):
    ...
    old = read('certificates/source_norms/allocated_seven_thresholds.json')
    faces = read('certificates/source_norms/global_control_faces.json')
    local = read('certificates/source_norms/endpoint_k_face_neighborhood.json')
    used = dict(PINS)
    ...
    schedule = module('global_k_schedule', base/'frontier/ap_schedule.py')
    fixed = module('global_k_fixed', base/'frontier/fixed_cost.py')
    prior49 = read('certificates/source_norms/full_linear_carrier_frontier.json')
