...
CERTIFICATE = 'certificates/source_norms/whole_block_mean_survival.json'
PINS = {
...
def calculate(base):
    ...
    for data in (prior, current, stop):
        for path, pin in data['source_sha256'].items():
            ...
            used[path] = pin
    common = module('mean_block_common', base/'frontier/whole_cost_common_stop_loss.py')
    bridge = module('mean_block_bridge', base/'frontier/k_face_common_seven_hinges.py')
    mean = module('mean_block_engine', base/'frontier/whole_cost_mean_stop_loss.py')
