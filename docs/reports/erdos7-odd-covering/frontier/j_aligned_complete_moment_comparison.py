...
CERTIFICATE = 'certificates/source_norms/j_aligned_complete_moment_comparison.json'
ORIGINAL = 'certificates/source_norms/j_face_heavy_positive175189_complete_moment_cost_comparison.json'
SOURCE_NAMES = (
...
HELPERS = (
    ...
    'frontier/j_face_complete_moment_cost_comparison.py',
    'frontier/j_face_complete_survival_linear_heads.py',
    'frontier/j_aligned_own_test_interface.py',
    'frontier/j_aligned_joint_selected_heads.py',
    'frontier/j_aligned_remaining_hinges.py',
    'frontier/j_aligned_retained375_source.py',
    'frontier/j_aligned_retained375_prefix.py',
    'frontier/j_aligned_retained375_heads.py',
    'frontier/j_aligned_quadratic_sharp_source.py',
    'frontier/j_aligned_quadratic_complete_heads.py',
)
...
def calculate(base):
    ...
    docs = {name: read('certificates/source_norms/'+name+'.json') for name in SOURCE_NAMES}
    original = read(ORIGINAL)
    paths = [ORIGINAL, *HELPERS, *('certificates/source_norms/'+n+'.json' for n in SOURCE_NAMES)]
    ...
    algebra = module('aligned_complete_envelopes', base/'frontier/j_face_complete_moment_cost_comparison.py')
    jobs, count = module('aligned_complete_counts', base/'frontier/j_face_complete_survival_linear_heads.py').prepare_jobs(source, engine)
    ...
    for tag, name in (('heavy553', 'cost0'), ('h4', 'hinge4')):
        ...
        require(doc['name'] == prefix['name'] == name
                ...
                and doc['prefix_sha256'] == pins['certificates/source_norms/j_aligned_retained375_'+tag+'_prefix.json']
                and doc['provider_sha256'] == pins['frontier/j_aligned_retained375_source.py']
