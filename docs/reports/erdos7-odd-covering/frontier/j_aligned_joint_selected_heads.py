...
def build_source(base,namespace='aligned_source_provider'):
 ...
 j=module(namespace+'_bridge',base/'frontier/j_face_coupled_seven_heads.py')
 ...
 return {'io':io,'j':j,'head':head,'actual_source_mass':F(1,4),'actual_survivor_mass':F(413,2700),
  ...
  'source_sha256':{p:sha256(io.read_artifact_bytes(base/p)).hexdigest()for p in('frontier/j_face_coupled_seven_heads.py','frontier/k_face_common_seven_hinges.py')},
...
def build_joint(base,data):
 ...
 rawmod=module('aligned_joint_raw',base/'frontier/joint_selected_source_comparison.py')
 ret=module('aligned_joint_retained',base/'frontier/retained_deletion_heavy_comparison.py')
 joint=module('aligned_joint_scan',base/'frontier/j_face_joint_selected_heads.py')
...
SOURCES=('certificate_io.py','frontier/j_face_coupled_seven_heads.py','frontier/k_face_common_seven_hinges.py',
 'frontier/joint_selected_source_comparison.py','frontier/retained_deletion_heavy_comparison.py','frontier/j_face_joint_selected_heads.py',
 ORIGINAL,'profile-notes/312-an-aligned-j-source-has-a-complete-own-test-and-residual-deletion-interface.md',
 'certificates/source_norms/j_aligned_own_test_interface.json',
...
def main():
 ...
 parser.add_argument('--base',type=Path,default=Path(__file__).resolve().parents[1]);parser.add_argument('--certificate',type=Path)
