...
def scan_job(base_string, job):
    ...
    provider = module('j_additional_scanner',base/'frontier/j_face_coupled_seven_heads.py')
...
def calculate(base, workers=2):
    ...
    predecessor = json.loads(io.read_artifact_bytes(base/'certificates/source_norms/j_face_coupled_seven_heads.json'))
    pins = dict(PINS)
    for path,pin in predecessor['source_sha256'].items():
        ...
        pins[path] = pin
    engine = module('j_additional_inventory',base/'frontier/source_barrier_saturation.py').Experiment(base)
