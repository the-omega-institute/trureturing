...
def calculate(base):
    ...
    prior = read('retained-transport/retained135_heavy_comparison')
    ...
    retained = module('e5_row_retained', base/'frontier/transport/retained135_heavy_comparison.py')
    ...
    study = module('e5_row_study', base/'frontier/k_neighborhood_radius_study.py').Study(base)
    ...
    output = []
    for name, doc, provider in zip(domain_names, domains, (study.get('wide_fresh_full_slot_source_comparison'), study.get('transport/extended_source_bridge_comparison')), strict=True):
