...
PINS = {'certificate_io.py': '287582353eeb0674f4e80530ebf268228b023f6088d14c819488a56111d0b232', 'frontier/transport/retained_pair_row_transport.py': '97efea8c615889c73964130b597f6246620a0133c4e2144a18e6cfb4f24528be', 'certificates/source_norms/retained-transport/retained_pair_row_transport.json': '80afd2bee701381bc29f91fec7c98155415f0f53c814c1b7935a2a67160abaaa', 'frontier/transport/retained135125_survival_comparison.py': '600cc560ea2d8c7d5698aca79539e0c1b1de8fc4440bda7f323fef605ab7cdff', 'certificates/source_norms/retained-transport/retained135125_survival_comparison.json': '67d7003462bde8fa391245e76f226694cea286ab005d0404a7cfe4046453b797'}
...
def calculate(base):
    ...
    parent, heavy, survival = [read('certificates/source_norms/retained-transport/'+name+'.json') for name in (
    ...
    load = lambda name: module('complete_retained_'+name, base/'frontier'/(name+'.py'))
    partial, exact, pair = (load('transport/retained_pair_row_transport'), load('transport/retained135_heavy_comparison'), load('transport/retained135125_heavy_comparison'))
    bridge, old, e5 = (load('k_face_common_seven_hinges'), load('transport/joint_deletion_row_transport'), load('transport/selected_deletion_mask_row_transport'))
    ...
    for domain, provider in zip(parent['domains'], (study.get('wide_fresh_full_slot_source_comparison'), study.get('transport/extended_source_bridge_comparison')), strict=True):
        ...
        source = read('certificates/source_norms/'+name+'.json')
