...
def calculate(base):
    ...
    prior = json.loads(io.read_artifact_bytes(base/'certificates/source_norms/second_depth_seven_comparison.json'))
    pins = dict(PINS)
    ...
    load = lambda name: module('linear_seven_'+name, base/'frontier'/(name+'.py'))
