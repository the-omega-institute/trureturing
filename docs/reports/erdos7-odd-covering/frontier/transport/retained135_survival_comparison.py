...
def calculate(base, bank=None, proposer=None):
    ...
    prior, laws, identities_cert = (read('retained-transport/retained135_heavy_comparison'), read('retained-transport/retained_deletion_survival_comparison'), read('load_two_cost_remainders'))
    pins = dict(PINS)
    ...
    load = lambda name: module('retained135_survival_'+name, base/'frontier'/(name+'.py'))
    ...
    retained = load('transport/retained135_heavy_comparison')
...
def main():
    ...
    retained = module('retained135_survival_codec', args.base/'frontier/transport/retained135_heavy_comparison.py')
