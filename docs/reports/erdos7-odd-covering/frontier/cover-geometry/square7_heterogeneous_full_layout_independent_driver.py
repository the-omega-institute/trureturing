...
p.add_argument('--witness',type=Path,default=Path(__file__).with_name('row0_column2_full_layout_certificate.json'))
...
roles=w['square7_roles'];ck('field_witness_roles',len(roles)==3 and roles[0] in (0,1) and roles[1] in range(4) and roles[2]==5)
...
# Full S3 acts on the layout AND on the selected field with its whole source.
...
for u,v in ((0,1),(1,2)):
 ...
 ck('joint_regular_transport_local_profile',all((l//3==roleRow,l==5)==(sw(l)//3==roleRow,sw(l)==5) for l in I))
