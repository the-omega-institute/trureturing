...
struct Engine {
 ...
 void include(){
  ...
  for(int p=0;p<3;++p){gate[p][0]=profiles[p].gain*(sum0+2*sum1);gate[p][1]=profiles[p].gain*(2*sum0+sum1);}
  for(int mask=0;mask<32;++mask){
   ...
   for(int p=0;p<3;++p)for(int mode=0;mode<6;++mode){I c=profiles[p].coefficient[mode*32+mask];gate[p][0]-=c*s1[mode];gate[p][1]-=c*s2[mode];}
  }
  cases+=2;for(int p=0;p<3;++p)for(int t=0;t<2;++t)if(gate[p][t]<profiles[p].minimum){profiles[p].minimum=gate[p][t];profiles[p].best=path;profiles[p].best_t=t+1;}
 }
...
int main(int argc,char**argv){try{
 ...
 if(read(in)!=3)throw std::runtime_error("expected three profiles");
 for(int pi=0;pi<3;++pi){auto&p=e.profiles[pi];p.scale=read(in);p.gain=read(in);I expected_scale=pi==2?100000000:1000000000;I expected_gain=pi==0?11551020408LL:(pi==1?1000000000LL:17900000000LL);if(p.scale!=expected_scale||p.gain!=expected_gain)throw std::runtime_error("wrong scale/gain");__int128 bound=p.gain;for(auto&c:p.coefficient){c=read(in);bound+=c;}if(bound*SD>=std::numeric_limits<I>::max())throw std::runtime_error("unsafe int64 bound");}
 for(int q=0;q<5;++q)for(int code=0;code<64;++code)for(int k=0;k<8;++k){I l=read(in),h=read(in);if(l>h||h>D)throw std::runtime_error("factor interval");e.factor_lo[q][code][k]=l;e.factor_hi[q][code][k]=h;}
 std::string extra;if(in>>extra)throw std::runtime_error("extra input");
 for(int k=0;k<8;++k)e.lo[0][0][k]=e.hi[0][0][k]=k?D:0;
 e.visit(0,0);
 for(int pi=0;pi<3;++pi){auto&p=e.profiles[pi];std::cout<<pi<<' '<<e.cases<<' '<<p.minimum<<' '<<p.scale*SD<<' '<<p.best_t;for(auto v:p.best)std::cout<<' '<<v;std::cout<<'\n';}
