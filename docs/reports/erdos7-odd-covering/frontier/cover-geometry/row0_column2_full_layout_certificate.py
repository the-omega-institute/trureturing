#!/usr/bin/env python3
"""Certify all row0 column2 leaf5 layouts with ten heterogeneous exact fields.
...
def summarize(directory, manifest, result, producer, checker):
    ...
    ck('family_count', family_count==10)
    ...
    return dict(
        schema='row0-column2-full-five-live-leaf-heterogeneous-field-cover-v1',
        status='PASS', new_lean_verification=False,
        scope='All ten 9qs central roles independently in live leaves 0,1,2,4,5; '
              'fixed low pure phases/nulls/central15/square7 roles (0,2,5), all 50 retained '
        ...
        square7_roles=[0,2,5],families=manifest['families'],computed_corner_count=20, represented_corner_count=95,
...
def main():
    ...
    preparer = source_dir / 'row0_column2_full_layout_prepare.py'
    checker = source_dir / 'row0_column2_full_layout_checker.cpp'
