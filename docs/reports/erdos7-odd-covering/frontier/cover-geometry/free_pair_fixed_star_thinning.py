#!/usr/bin/env python3
"""A fixed common thinning certifies every central source corner for one star layout.
...
def main():
 ...
 co=[g*(l+p)+c*w for l,p,w in zip(loss,pair,query)]
 ...
 for i,j in product(range(len(orb3)),range(len(orb5))):
  ...
  ck(f'positive_complete_corner_{i}_{j}',gate>F(1,400))
 ...
 ck('exact_minimum',F(worst['gate'])==F(904989942890553166134679994443,331080094584857932554240000000000))
 ...
 head=alpha*gate
 ck('strict_head_bound',head>F(1,17000))
 result={'schema':'free-pair-fixed-star-common-thinning-v1','status':'PASS',
  'scope':{
   ...
   'other_originals':'All other Report626 mixed inventory and unrestricted23/29/31 continuation, literal reference head primes and distinct numerical labels.',
  ...
  'pair_labels':sorted(labels,key=lambda row:row['modulus']),'pair_loss_coefficients':list(map(str,pair)),
  'source_corners':rows,'minimum':worst,'gate_strict_lower':'1/400',
  'head_haar_lower':str(head),'head_strict_lower':'1/17000',
