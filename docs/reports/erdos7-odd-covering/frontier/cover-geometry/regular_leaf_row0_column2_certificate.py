#!/usr/bin/env python3
"""Certify all fixed regular-leaf0 row0 column2 layouts with five whole fields.
...
def summarize(directory, manifest, result, producer, checker):
    ...
    ck('family_count', family_count==5)
    ...
    return dict(
        schema='regular-leaf-row0-column2-product-group-five-field-cover-v1',
        status='PASS', new_lean_verification=False,
        scope='All ten 9qs central roles independently in live leaves 0,1,2,4,5; '
              'fixed low pure phases/nulls/central15/square7 roles (0,2,0), all 50 retained '
        ...
        square7_roles=[0,2,0],families=manifest['families'],computed_corner_count=20, represented_corner_count=95,
...
def main():
    ...
    preparer = source_dir / 'regular_leaf_row0_column2_prepare.py'
    checker = source_dir / 'regular_leaf_row0_column2_checker.cpp'
