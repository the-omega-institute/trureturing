#!/usr/bin/env python3
"""Independent exact audit of supplied coupled fields for theleaf4 pair layout.
Reads pinned640/658 and candidate field data; does not read or import the
producer. Independently rebuilds matching responses, all95 complete gates,
priority inequalities and the complete network reserves.
"""
...
witness_path=args.witness or ROOT/'leaf_pair_adapted_priority_fields_certificate.json'
witness_raw=witness_path.read_bytes(); witness=json.loads(witness_raw)
checks={}
def ck(k,c):
...
ck('pinned_witness_data',sha256(witness_raw).hexdigest()=='5dd59a96df8e8cd3eed09a6df5594c7a529a450ec61b1c824486ad6e8a5d90c8')
...
THETA_DEN=witness['field_denominator']
ck('dyadic_field_denominator',THETA_DEN==2**20)
fieldmap={}
for row in witness['fields_by_corner']:
 key=(row['weak3'],row['weak5'])
 ck('unique_admitted_corner_field',key not in fieldmap and key[0] in I and key[1] in J)
 fieldmap[key]=row['numerators']
 ck('full_field_dimensions',len(fieldmap[key])==120)
 for l in range(6):
  for m in range(20):
   t=fieldmap[key][l*20+m]
   ck('admissible_integer_field',isinstance(t,int) and 0<=t<=THETA_DEN)
   if l==3 or m==5 or (l<3 and m<5):ck('common_zero_mask',t==0)
ck('complete_ninety_five_field_family',len(fieldmap)==95)
for l,m in product(I,J):
 if l<3 and m<5:continue
 for i,j in product(I,J):
  t=fieldmap[(i,j)][l*20+m]
  ck('ternary_weak_marker_priority',fieldmap[(l,j)][l*20+m]>=t)
  ck('quinary_weak_marker_priority',fieldmap[(i,m)][l*20+m]>=t)
...
for i,j in product(I,J):
 ...
 for support in range(32):
  mat=[[H[support][l][m]*fieldmap[(i,j)][l*20+m] for m in range(20)] for l in range(6)]
 ...
 gates.append(dict(weak3=i,weak5=j,source_mass=str(F(mass,9*75*THETA_DEN*hden)),gate=str(value),decimal=float(value)))
...
for p in net['policies']:
 ...
 ck('common_full_density_above_one_over_one_hundred_seventy_thousand',alpha*margin>F(1,170000))
...
# Compare claimed results only after independent reconstruction is complete.
expected={(row['weak3'],row['weak5']):row for row in gates}
ck('candidate_ninety_five_results',len(witness['corner_gates'])==95)
for row in witness['corner_gates']:
 key=(row['weak3'],row['weak5']); ref=expected.pop(key)
 ck('candidate_corner_gate_matches',F(row['gate'])==F(ref['gate']))
 ck('candidate_corner_source_mass_matches',F(row['source_mass'])==F(ref['source_mass']))
ck('no_missing_candidate_corner',not expected)
ck('candidate_minimum_matches',F(witness['head_gate'])==minimum)
ck('candidate_minimizers_match',[(row['weak3'],row['weak5']) for row in witness['minimizers']]==mins)
ck('two_complete_candidate_policies',len(witness['policies'])==len(policies)==2)
for row,ref in zip(witness['policies'],policies):
 ck('candidate_policy_identity',row['kind']==ref['kind'] and row['K']==ref['K'])
 ck('candidate_raw_reserve',F(row['raw_margin'])==F(ref['raw_margin']))
 ck('candidate_projected_reserve',F(row['projected_margin'])==F(ref['projected_margin']))
 ck('candidate_common_density',row['density_denominator']==170000)
result=dict(schema='leaf4-adapted-priority-independent-v1',status='PASS',scope='Fixed null3/5, central9qs leaf4 and7squarestar roles(1,2,5),50retained incidences; arbitrary finite higher central pure phases via624actualsource and645priority.',new_lean_verification=False,read_same_round_producer=False,verifier_sha256=sha256(Path(__file__).read_bytes()).hexdigest(),witness_sha256=sha256(witness_raw).hexdigest(),source640_sha256=sha256(raw).hexdigest(),source658_sha256=sha256(netraw).hexdigest(),profiles=len(profiles),strict_induced_minima=strict,response_common_denominator=hden,coefficient_common_denominator=cden,minimum_gate=str(minimum),minimum_gate_decimal=float(minimum),minimizing_weak_pairs=mins,gates=gates,policies=policies,check_count=sum(checks.values()),checks=checks)
