...
int main(int argc,char**argv){try{
 ...
 if(read_u64(in)!=0x3155304647455255ULL||read_u64(in)!=1)throw std::runtime_error("input schema");
 const U scale=read_u64(in),gL=read_u64(in),target=read_u64(in),expected=read_u64(in);
 if(scale!=(U(1)<<32)||gL>scale||expected!=4912337||target!=2122057442ULL)throw std::runtime_error("input constants");
 ...
 std::function<void(int,bool)> gen=[&](int at,bool seen){if(at==10){layouts.push_back(current);return;}for(int c:{0,4,5}){current[at]=c;gen(at+1,seen);}current[at]=1;gen(at+1,true);if(seen){current[at]=2;gen(at+1,true);}};gen(0,false);
 U fixed_layouts=0,represented=0;for(const auto&layout:layouts){bool seen=false;for(auto c:layout){if(c==1||c==2){if(!seen&&c!=1)throw std::runtime_error("noncanonical first label");seen=true;}}fixed_layouts+=!seen;represented+=seen?2:1;}
 if(fixed_layouts!=59049||represented!=9765625)throw std::runtime_error("S2 orbit census");
 ...
 auto worker=[&](){try{while(true){auto start=next.fetch_add(32);if(start>=limit)break;for(auto a=start;a<std::min(start+32,limit);a++){
   ...
   std::array<std::array<U,10>,32>L,H;for(unsigned u=0;u<5;u++){unsigned l=u<3?u:u+1;unsigned n=unsigned(l/3==unsigned(row_role))+unsigned(l==0);for(unsigned col=0;col<2;col++)for(unsigned T=0;T<32;T++){auto idx=((n+col)*1024+masks[u])*32+T;L[T][2*u+col]=lo[idx];H[T][2*u+col]=hi[idx];}}
   Result r;for(int fi:{0}){const auto&f=families[fi];S minq=std::numeric_limits<S>::max();bool passed=true;
 ...
 U minq=std::numeric_limits<U>::max(),minidx=0,total_cases=0,total_screens=0,failed=0;std::array<U,1>counts{};
 ...
 std::ofstream out(argv[2],std::ios::binary);if(!out)throw std::runtime_error("result output");write_u64(out,0x3155314647455255ULL);write_u64(out,U(row_role));write_u64(out,0);write_u64(out,limit);write_u64(out,failed);write_u64(out,minq);write_u64(out,minidx);write_u64(out,total_cases);write_u64(out,total_screens);for(auto c:counts)write_u64(out,c);for(auto c:layouts[minidx])write_u64(out,c);for(const auto&r:results)out.put(char(r.family<0?255:r.family));out.close();if(!out)throw std::runtime_error("result write failure");
