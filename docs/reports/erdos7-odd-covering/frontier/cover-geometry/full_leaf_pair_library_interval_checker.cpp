...
int main(int argc,char**argv){try{
 ...
 std::vector<int>case_order={13,17,1,5,9};for(int k=0;k<20;k++)if(std::find(case_order.begin(),case_order.end(),k)==case_order.end())case_order.push_back(k);
 auto worker=[&](){try{while(true){auto start=next.fetch_add(32);if(start>=limit)break;for(auto a=start;a<std::min(start+32,limit);a++){
   ...
   std::array<std::array<U,10>,32>L,H;for(unsigned u=0;u<5;u++){unsigned n=u<3?0:u-2;for(unsigned col=0;col<2;col++)for(unsigned T=0;T<32;T++){auto idx=((n+col)*1024+masks[u])*32+T;L[T][2*u+col]=lo[idx];H[T][2*u+col]=hi[idx];}}
   Result r;for(int fi:{3,0,1,2}){const auto&f=families[fi];S minq=std::numeric_limits<S>::max();bool passed=true;
 ...
 U minq=std::numeric_limits<U>::max(),minidx=0,total_cases=0,total_screens=0,failed=0;std::array<U,4>counts{};
