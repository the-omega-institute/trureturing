...
Requires only Python's standard library, a C++17 compiler with int128, three
...
p.add_argument('--cpp',type=Path,default=Path(__file__).with_name('full_layout_independent.cpp'))
...
base,net,lib=map(json.loads,raw);cppraw=a.cpp.read_bytes()
...
words=[0x314c4146454c5546,1,A,gL,target,1657470]+CU
...
keys=[tuple(k) for k in lib['orbit_keys']];ki={k:i for i,k in enumerate(keys)}
def orbit(i,j,l,m):
...
ck('four_families',len(lib['families'])==4)
for fi,fam in enumerate(lib['families']):
 den=fam['denominator'];nums=fam['numerators'];ck('field_denominator',den==(180 if fi==0 else 1<<20));ck('field_shape',len(nums)==180)
 words.append(den)
 ...
 for i,j in corners:
  ...
  for xmode,ymode in product(range(4),repeat=2):
   ...
   for x,y in product(xm[xmode],ym[ymode]):
    ...
    coeff=[0]*10
    for l,m in product(I,J):coeff[profileidx[l,int(m//5==2)]]+=x[l]*y[m]*theta[l,m]
...
ck('complete_literal_and_retained_counts',rawcount==44720 and retcount==7485)
ck('binary_schema_word_count',len(words)==338796)
...
with TemporaryDirectory(prefix='independent-five-leaf-',dir='/tmp') as tmp:
 ...
 run=subprocess.run([str(exePath),str(inputPath),str(selectionPath),str(a.threads)],stdout=subprocess.PIPE,text=True,check=True)
 enumeration=json.loads(run.stdout);selection=selectionPath.read_bytes()
ck('cpp_source_unchanged',a.cpp.read_bytes()==cppraw)
ck('complete_enumeration_status',enumeration['status']=='PASS' and enumeration['canonical_layout_count']==sum(counts) and enumeration['actual_layout_count']==5**10)
...
ck('complete_whole_family_selection',len(selection)==sum(counts) and all(x<4 for x in selection))
ck('family_selection_counts',list(map(selection.count,range(4)))==enumeration['family_selection_counts'])
