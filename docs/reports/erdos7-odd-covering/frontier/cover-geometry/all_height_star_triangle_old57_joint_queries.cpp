// Report550 signed boundary with the complete old57 union charged once.
// Exact finite optimizer; numerical expected values are checked by the driver.
...
int leaf(int a,int b,int c){
 ...
 return ((3-n[0])*(5-n[1])-n[6]-1)*(9-n[2])*(11-n[3])*(15-n[4])*(17-n[5]);
}
...
int main(){
 ...
 for(int j=0;j<6;j++){
  ...
  if(best>bestfull)throw std::runtime_error("off minimum exceeds saturated minimum");
  if(j)std::cout<<",";
 ...
 std::cout<<"],\"node_candidates\":"<<nodework<<",\"root_convolution_candidates\":"<<convwork<<",\"checks_passed\":8}\n";
}
