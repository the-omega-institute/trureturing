// Exact finite engine for the accompanying Python profile constructor.
// Enumerates the 16^5 two-center root/equality layouts and both ternary
// endpoint masses once. The input contains the H=3 coefficient row.
...
int main(int argc, char **argv) {
    try {
        ...
        const Int scaled_G = read_nonnegative(input);
        if (denominator == 0 || denominator > LIMIT / SCREEN_DENOMINATOR)
            throw std::runtime_error("invalid common denominator");
        if (scaled_G > LIMIT / 49 || 49 * scaled_G != 566 * denominator)
            throw std::runtime_error("invalid continuation coefficient");
        std::array<Profile, 1> profiles;
        for (int index = 0; index < 1; ++index) {
            if (read_nonnegative(input) != 3)
                throw std::runtime_error("expected cutoff 3");
            profiles[index].cutoff = 3;
            Int bound = scaled_G;
