#!/usr/bin/env python3
"""Certify all column1 five-live-leaf layouts using two exact interval fields.
...
def summarize(directory, manifest, result, producer, checker):
    ...
    return dict(
        schema='row0-column1-full-five-live-leaf-two-field-cover-v1',
        status='PASS', new_lean_verification=False,
        scope='All ten 9qs central roles independently in live leaves 0,1,2,4,5; '
              'fixed low pure phases/nulls/central15/square7 roles (0,1,5), all 50 retained '
        ...
        family_selection_rule='First passing whole family in order optimized_common_field,constant_one; '
                              'one family for all 95 corners and every query.',
        square7_roles=[0,1,5],orbit_keys=manifest['orbit_keys'],families=manifest['families'],computed_corner_count=20, represented_corner_count=95,
...
def main():
    ...
    preparer = source_dir / 'row0_column1_full_leaf_pair_library_interval_prepare.py'
    checker = source_dir / 'row0_column1_full_leaf_pair_library_interval_checker.cpp'
