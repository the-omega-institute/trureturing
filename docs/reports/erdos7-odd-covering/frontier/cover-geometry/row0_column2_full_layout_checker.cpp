...
int main(int argc,char**argv){try{
 ...
 if(argc>=6){
 std::ifstream indices_in(argv[5]);if(!indices_in)throw std::runtime_error("indices input");
 std::vector<std::array<std::uint8_t,10>> selected_layouts;std::size_t idx;
 while(indices_in>>idx){if(idx>=layouts.size())throw std::runtime_error("index outside canonical domain");selected_layouts.push_back(layouts[idx]);}
 if(!indices_in.eof()||selected_layouts.empty())throw std::runtime_error("malformed or empty residual");
 layouts.swap(selected_layouts);
 }
 ...
 std::vector<Result>results(limit);std::atomic<std::size_t>next{0};std::atomic<bool>bad{false};std::atomic<std::size_t>done{0};std::mutex errors;
 ...
 auto worker=[&](){try{while(true){auto start=next.fetch_add(32);if(start>=limit)break;for(auto a=start;a<std::min(start+32,limit);a++){
   ...
   std::array<std::array<U,10>,32>L,H;for(unsigned u=0;u<5;u++){unsigned l=u<3?u:u+1;unsigned n=unsigned(l/3==0)+unsigned(l==5);for(unsigned col=0;col<2;col++)for(unsigned T=0;T<32;T++){auto idx=((n+col)*1024+masks[u])*32+T;L[T][2*u+col]=lo[idx];H[T][2*u+col]=hi[idx];}}
   ...
   results[a]=r;if(r.family<0){bad.store(true);std::lock_guard<std::mutex>lock(errors);std::cerr<<"first_failed_layout_index "<<a<<" layout";for(auto c:layout)std::cerr<<" "<<unsigned(c);std::cerr<<"\n";}
