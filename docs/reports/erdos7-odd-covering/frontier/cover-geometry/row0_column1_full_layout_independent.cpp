...
static void process(const Input& in,const Job& state,u64 rank,Stats& stats){
 ...
 for(unsigned leaf=0;leaf<5;++leaf){
  unsigned base=leaf<3?1:leaf==3?0:1; // [actualLeaf/3=0]+[actualLeaf=5]
...
int main(int argc,char** argv){
 try{
  ...
  if(argc==5){
   ...
   std::cout<<"{\"schema\":\"row0-column1-independent-smoke-v1\",\"status\":\"PASS\",\"checked_layouts\":"<<ns<<",\"family_count\":"<<in.families.size()<<",\"accepted_complete_corner_gates\":"<<20*ns<<",\"accepted_complete_screen_maxima\":"<<10240*ns<<",\"minimum_selected_lower_q40\":"<<stats.minimum<<"}\n";return 0;
  }
