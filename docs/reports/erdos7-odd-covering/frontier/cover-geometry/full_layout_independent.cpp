...
struct Input{
 u64 scale=0,g=0,target=0,expected=0;std::array<u64,512> costs{};
 std::vector<u64> lower,upper;std::array<Family,4> families;
 explicit Input(const char* file){
  Reader r(file);require(r.word()==0x314c4146454c5546ULL,"magic");require(r.word()==1,"version");
  scale=r.word();g=r.word();target=r.word();expected=r.word();
  ...
  for(unsigned fi=0;fi<4;++fi){
   auto& f=families[fi];f.den=r.word();require(f.den==(fi?1048576ULL:180ULL),"family denominator");
  ...
  require(retained==7485,"retained complete menus");require(r.f.peek()==std::char_traits<char>::eof(),"trailing input");
 }
};
...
struct Stats{
 ...
 std::array<u64,4> regularClasses{},familyCounts{};
...
int main(int argc,char** argv){
 try{
  require(argc==4,"usage: checker INPUT_BIN SELECTION_OUT THREADS");
  ...
  std::vector<Stats> stats(n);std::vector<std::thread> pool;for(unsigned k=0;k<n;++k)pool.emplace_back(worker,std::cref(in),std::ref(stats[k]));for(auto& t:pool)t.join();
  ...
  for(const auto& s:stats){
   ...
   for(unsigned k=0;k<4;++k){total.regularClasses[k]+=s.regularClasses[k];total.familyCounts[k]+=s.familyCounts[k];}
  ...
  require(std::all_of(selection.begin(),selection.end(),[](unsigned char x){return x<4;}),"complete selection witness");
