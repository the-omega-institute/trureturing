...
p.add_argument('--cpp',type=Path,default=Path(__file__).with_name('square7_role_full_layout_independent.cpp'))
p.add_argument('--witness',type=Path,default=Path(__file__).with_name('column0_leaf5_full_layout_certificate.json'))
...
roleList=w['square7_roles'];ck('both_column0_roles',roleList==[[0,0,5],[1,0,5]])
roleRow,roleColumn,_=roleList[0]
for name,h in zip(names,pins):ck('witness_source_pin',w['source_sha256'][name]==h)
ck('two_whole_field_library',len(w['families'])==2)
families=[]
for f in w['families']:
 den=f['denominator'];nums=f['numerators'];ck('native_field_denominator',isinstance(den,int) and 0<den<=1<<20)
 families.append(dict(denominator=den,numerators=nums))
...
words=[0x314c4146454c5546,3,A,gL,target,1657470,len(families),0,roleRow,roleColumn]+CU
...
for u,v in ((0,1),(1,2)):
 ...
 ck('regular_transport_local_profile',all((l//3==roleRow,l==5)==(sw(l)//3==roleRow,sw(l)==5) for l in I))
...
with TemporaryDirectory(prefix='independent-column0-roles-',dir='/tmp') as tmp:
 ...
 for roles in roleList:
  ...
  ck('explicit_role_header',int.from_bytes(roleBinary[64:72],'little')==roles[0] and int.from_bytes(roleBinary[72:80],'little')==roles[1])
