...
struct Input{
 ...
 explicit Input(const char* file){
  Reader r(file);require(r.word()==0x314c4146454c5546ULL,"magic");require(r.word()==3,"version");
...
static void process(const Input& in,const Job& state,u64 rank,Stats& stats){
 ...
 for(unsigned leaf=0;leaf<5;++leaf){
  ...
  unsigned base=unsigned(actualLeaf/3==in.row)+unsigned(actualLeaf==5);
...
int main(int argc,char** argv){
 try{
  ...
  if(argc==5){
   ...
   for(unsigned rank=0;rank<ns;++rank){Job state;unsigned label=rank==0?4:rank==1?3:0;state.labels.fill(label);if(rank==3){state.labels.fill(4);state.labels[4]=state.labels[5]=3;state.labels[9]=0;}for(unsigned e=0;e<10;++e)state.masks[state.labels[e]]|=1U<<e;state.used=(rank==2||rank==3)?1:0;process(in,state,rank,stats);require(!failed,"smoke fixed layout failed");}
   ...
   std::cout<<"{\"schema\":\"square7-role-independent-smoke-v1\",\"status\":\"PASS\",\"checked_layouts\":"<<ns<<",\"family_count\":"<<in.families.size()<<",\"accepted_complete_corner_gates\":"<<20*ns<<",\"accepted_complete_screen_maxima\":"<<10240*ns<<",\"minimum_selected_lower_q40\":"<<stats.minimum<<"}\n";return 0;
  }
