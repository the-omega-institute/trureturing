#!/usr/bin/env python3
"""Independent exact audit of the binary leaf-pair layout library.
...
paths=[args.directory/'remaining33_global_root_exclusion_certificate.json',args.directory/'joint_square_pair_225_star_certificate.json',args.witness or args.directory/'binary_leaf_pair_library_certificate.json']
raw=[p.read_bytes() for p in paths]; base,net,witness=map(json.loads,raw)
...
reps=list(product((0,4,5),(0,6,10,15)))
def corner_rep(i,j): return (0 if i<3 else i,(0,6,10,15)[j//5])
mult=Counter(corner_rep(i,j) for i,j in corners)
ck('corner_coverage',len(corners)==95 and len(reps)==12 and sum(mult.values())==95 and set(mult)==set(reps))
...
# Actual 17 permutation generators certify the orbit quotient. They preserve
...
for axis,a,b in generators:
 ...
 ck('generator_preserves_all_binary_local_profiles',all((int(l//3==1)+int(m//5==2)+int(l==5),l==4,l==5)==(int(mv((0,0,l,m))[2]//3==1)+int(mv((0,0,l,m))[3]//5==2)+int(mv((0,0,l,m))[2]==5),mv((0,0,l,m))[2]==4,mv((0,0,l,m))[2]==5) for l,m in cells))
...
ck('exact_actual_group_orbits',len(components)==180 and all(len(v)==1 for v in components.values()))
...
selection=witness['selection']; ck('one_family_per_layout',len(selection)==1024 and all(isinstance(x,int) and 0<=x<4 for x in selection))
ck('all_candidate_layout_records',len(witness['layout_records'])==1024)
records={x['layout_mask']:x for x in witness['layout_records']}
ck('unique_candidate_layouts',set(records)==set(range(1024)))
...
for layout in range(1024):
 family=selection[layout]; den=families[family]['denominator']; field=fieldtables[family]
 tables={(0,0):regular[0],(1,0):regular[1],(1,1):responses(1,layout),(2,1):responses(2,layout),(2,2):responses(2,1023^layout),(3,2):responses(3,1023^layout)}
 ...
 cellprof=[[(int(l//3==1)+int(m//5==2)+int(l==5),1 if l==4 else 2 if l==5 else 0) if (l,m) in cells else None for m in J] for l in I]
 H=[[[0 if key is None else tables[key][T] for key in row] for row in cellprof] for T in range(32)]
 candidate=records[layout]; ck('candidate_whole_layout_family',candidate['family']==family)
 cg={(x['weak3'],x['weak5']):x for x in candidate['corner_gates']}
 ck('candidate_exact_corner_set',len(candidate['corner_gates'])==12 and set(cg)==set(reps))
 layout_gates=[]
 for corner in reps:
  ...
  for T in range(32):
   ...
   if layout in (0,1,341,682,1023) and T in (0,3,17,31):
  ...
  key=dict(layout_mask=layout,family=family,weak3=corner[0],weak5=corner[1])
  if minimum is None or gate<minimum: minimum=gate; minrows=[key]
  elif gate==minimum: minrows.append(key)
 outrows.append(dict(layout_mask=layout,family=family,corner_gates=layout_gates))
 if layout%128==127: print(json.dumps(dict(progress_layouts=layout+1,elapsed_seconds=round(monotonic()-started,3))),flush=True)
ck('all_complete_screens',screen_count==1024*12*512==6291456)
...
ck('previous_661_minimum_preserved',minimum==F(1864487415907319442048626989,931162766019912935308800000000))
...
for policy,claim in zip(net['policies'],witness['policies']):
 ...
 ck('uniform_complete_density',projected>F(1,420000))
 ...
 ck('candidate_density_denominator',claim['density_denominator']==420000)
 policies.append(dict(kind=policy['kind'],K=policy['K'],complete_fee=str(fee),raw_margin=str(margin),projected_margin=str(projected),projected_margin_decimal=float(projected),density_denominator=420000))
result=dict(schema='binary-leaf-pair-library-independent-v1',status='PASS',new_lean_verification=False,read_same_round_producer=False,scope='Fixed null3/5, fixed7squarestar roles(1,2,5), every one of1024 globally fixed binary9qs leaf4/5 layouts; fixed library family chosen once per entire layout, all95priority corners and full512query fees, both complete658policies.',input_files=[p.name for p in paths],input_sha256=[sha256(b).hexdigest() for b in raw],verifier_sha256=sha256(Path(__file__).read_bytes()).hexdigest(),group_generators=generators,field_orbit_count=len(components),corner_representatives=[dict(weak3=i,weak5=j,multiplicity=mult[i,j]) for i,j in reps],field_family_denominators=[f['denominator'] for f in families],family_selection_counts=dict(Counter(selection)),uniform_all_induced_strictness_margin=str(strictgap),complete_screen_count=screen_count,minimum_gate=str(minimum),minimum_gate_decimal=float(minimum),minimizers=minrows,policies=policies,layout_records=outrows,checks=checks,check_count=sum(checks.values()))
