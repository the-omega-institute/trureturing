...
| `projection` | `x = f(I_prod(x))` | 四项合取,见下 |
...
**公理 4(按域的引用完整性:全称,不是存在)**:`π, I_run, I_prod, J, C` 中出现的每一个名字,
必须解析到其**声明的对象域**;逐名检查,不得对条目做存在量化。工件引用须解析到 `A(R)`;
消费端若使用仓外 actor,则须属于仓库刻意定义的闭字母表 `ProgramActorWords`,不得冒充工件引用。
