...
**仓库政策(投影不设门)**:四项合取判为 `projection` 后,不对投影本身设置准入门。依据来自
`CLAUDE.md` 第〇节记录的成本,不是本文自证:守投影会长出冲突分类器、自动重算链、FIFO 租约;
实测 `pr-shepherd` 一族 shell **2,817 行** + 专职测试 **3,913 行**,其 85 个测试是 CI test 阶段最慢的
**≈396s**;另有 `conservative` **529s/次**、`c0-renew` **24 min/次**。**这是成本论证,不是信息论定理。**
...
| 权威 | 可否手改 | 准入义务 | 单次改动代价 |
|---|---|---|---|
| `projection` | 手改不承权威;应重新发射 | 四项合取分类 + CLAUDE.md 成本政策:无门;重算只验第三项 | 0,重新发射 |
...
2. **分轴**:每条目定 (权威, 居所)。无法判定者在**唯一登记真源**内 fail-closed:
   判为非投影、拒绝准入并记具名 `open`;四项合取成立者按成本政策不设投影门;不得另立平行隔离清单。
...
- **(h) 「不设投影门」的依据形状**`open` — **第 3 轮三席一致的未决 blocking,本版未修**。
  ...
  故本文把症状改称依据,**与唯一真源的论证形状不一致**。
  **正确形状(未采纳)**:§3 改动规则表的 `projection → 无门` 一行,只能由
  `CLAUDE.md` 的**规范性裁决**直接授权,并把成本读数如实标为**该裁决的理由**而非本文的推导;
  §7 / §8 / §9 / 附录中一切「成本政策」标签同此。
  **本版未改的原因**:bounded pass 预算(5 次)已尽,且该改动触及断言的真值条件,
  不属预先写死的「纯机械错误」例外。另案立项处置。

- **(i) `π` 分型只传播了一半**`open` — **第 3 轮 architecture 席的未决 blocking,本版未修**。
  §1 已把 `π` 定型为 `A(R) → ProducerActor` 并引入部分解析 `resolve_R : ProducerActor ⇀ A(R)`,
  但 §2 仍把 `π(x)` 直接放在同域工件边上(`Π_D = {(π(x),x) : x, π(x) ∈ D}`),
  §3 亦仍以「某节点因是某个 `π(y)` 而成为 producer」表述。这些在新定义下**不良类型**,
  其后果是**无环性契约实际上没有覆盖实现该 producer 的可执行工件**。
  **正确形状(未采纳)**:凡需要工件节点处一律写 `resolve_R(π(x))`,未解析处保持未定义且 fail-closed。
