...
The edge counts below are historical D5-filtered value/type constants with auxiliary expansion. External means D5 declarations outside the row’s module. Mathlib constants are omitted. `missing` means no row was captured, not zero dependencies. These counts do not certify reduced proof liveness or admission. Exact semantic owners, source clauses and directed uses are recorded separately in the audit.
