...
The three unchanged `proof-edges-*.json` exports have distinct observed scopes: **internal arrays expand local auxiliaries; external arrays record direct outside-module D5 constants from value and type**. The raw `external_deps` arrays, recorded as `external_dependencies` in the audit, omit prerequisites reached only through local helpers. Both arrays are filtered to D5-owned constants, omit Mathlib and Lean-core constants, and do not certify a reduced live-proof closure or proof shape. `Rat.div_int_inj`, `Rat.num_div_den` and `inv_div` are absent despite actual source use. The exports' historical `Expr.getUsedConstants` source label is preserved; the exact historical producer implementation is `ASSUMED-UNVERIFIED` in the permitted evidence. This describes observed array scope, not verified producer provenance.

The six affected rows are `RichRational.integerEmbedding`, `RichRational.add`, `RichRational.div`, `RichRational.mul`, `RationalQuotient.rationalSection` and `IntegerExactDivision.half`. For example, `add` captures the internal `Fraction.denominator_ne_zero` reference through `add._proof_1`, but its external array omits `GeneratedProduct.q_product`; `div` also omits that guard owner. Its existing declaration ID is `sha256:0317eecfaee3d90e91417539d454d8b3952e72ce7609a0490a3fc76bd1605f29`. The guard helpers of `integerEmbedding`, `rationalSection` and `half` use `ComplementFibers.balancedSection_rightInverse`, absent from their external arrays, with existing declaration ID `sha256:446bbc5f35cca36fae4196336e30efc02b896ef609fdab79e5fa1376cb619229`. The internal `mul` array reaches `add` through `mul._proof_1`; the authored owner/use chain `mul → add → q_product` supplies its guard dependency separately. Existing semantic-owner citations and directed source-use accounting remain distinct from historical capture. No omitted edge is added to the raw exports or their audit arrays.
...
Historical capture covers **87/90** included declarations. The three missing rows have explicit `missing-capture` status and `null` dependency arrays in the audit; the shapes table displays `missing`. Missing capture is not an empty dependency set. All captured arrays retain their historical contents. The inherited `direct_frozen_dependencies` field is repeated module prerequisite context; declaration-specific semantic citations carry exact existing D5 owner identities separately.
...
The authorized prior tests conclusion reports canonical focused Lean exit 0 and a meaningful boundary probe exit 0 on the unchanged mathematics. Its probe covered zero and negative inputs, exact source arithmetic examples, canonical quotient/section behavior, domain guards, history loss and the typed negation. The caller-supplied packet reports all three existing CI checks green on the starting HEAD. These are attributed verification evidence, not semantic approval. This bounded capture-scope correction uses the immutable source/report/export evidence, all 90 audit rows, document/JSON checks and byte-preservation comparisons. Mathematical sources, the canonical report and Scribe/Markdown mirrors are unchanged, so full Lean/report/emit and engineering builds are not rerun and no implementation-mirroring tests are introduced.
