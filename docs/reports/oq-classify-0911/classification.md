...
| 7 / `0aeb6b17305b0ea10af6e81726ca256b1a737c1a68f3bd78436f0a9d30d17a06` | `proposition`：量子 Fenchel–Young 恒等式有明确数学内容；本段同时夹有不能原样照收的「不裂」等价链。 | 对所有 d≥1、正定复 Hermitian 矩阵 ρ∈M_d(ℂ)（Trρ=1）及 Hermitian H∈M_d(ℂ)，令 Z=Tr(exp H)、G=exp H/Z、S(ρ)=−Re Tr(ρ logρ)、D(ρ∥G)=Re Tr(ρ(logρ−logG))（log 用正定矩阵谱演算），则 log Z=Re Tr(Hρ)+S(ρ)+D(ρ∥G)，特别 H=0 时 S(ρ)+D(ρ∥I/d)=log d。 | — | 可：G 算法以非对易的二阶矩阵直接从 G 的谱计算 D，a=0,1/2,1 时 D=0.082282878505,0.341738727333,0.794746392727，等式残差绝对值≤4.45×10⁻¹⁶；附带 R 探针给退化配对根基 4 元且交换子非平凡，阻止无条件照收「不裂⇔完美」。 | 若要覆盖本段其余辛化、张量分解或最小化子句，必须先另定其辖域；尤其把「中心扩张不裂」等同于「配对非退化」时停手。 |
...
| 10 / `13f6e0af21c9969253940a6564dea347af2a340de7a7c5f7c2c21988fb79fe8d` | `proposition`：模流与外自同构类是已定义的数学对象；「兑物理时间」不随数学自动成立。 | 对任意作用在复 Hilbert 空间上的 von Neumann 代数 M 及忠实正规态 φ、ψ，设 σᵠ、σᵠ以外的第二群记为 σᵠ′=σ^ψ，第一群 σᵠ=σ^φ，均为标准模自同构群，则存在强连续 u:ℝ→U(M)，使所有 s,t∈ℝ 有 u(s+t)=u(s)σ_s^φ(u(t)) 且 σ_t^ψ=Ad(u(t))∘σ_t^φ，因而二者在 Out(M)=Aut(M)/Inn(M) 中逐时相同；当 M=M_d(ℂ)、d≥1、φ(A)=Tr(ρA)、ρ 正定且 Trρ=1 时，σ_t^φ(A)=ρ^(it)Aρ^(−it)，其外类恒等。 | — | 无穷维唯一性不能由数值验证；有限维边界可运行 T：ρ=diag(1/3,2/3)，t=0,1,2 时 σ_t(E₀₁) 的系数为 1、0.769238901364−0.638961276314i、0.183456974743−0.983027740411i，演示「外类平凡」不等于流本身平凡。 | 若第三方只将结论放进假设、或把有限维外类恒等写成所有 σ_t 恒等、或据模参数推物理秒，停手。 |
