...
def execute(store,phase,max_boxes,limits,producer):
    ...
    if candidates: return dict(reason='candidate-needs-independent-certification',candidates=candidates,**counts)
...
def main(argv=None):
    ...
    with Store(args.output/'results.sqlite3',cpu.EXPERIMENT) as store:
        ...
        report=dict(schema='actual-prime-cpu-run-result-v1',producer=producer,
        ...
        store.set_meta('last_result',report)
