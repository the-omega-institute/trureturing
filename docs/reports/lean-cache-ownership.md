...
| Dependency retrieval | Dependency layer; mathlib's `lake exe cache get` and Lake | Cache-get failure is a warning for the wrapped `lake build`, which continues under the same target writer guard. Symlink refusal, a busy guard, and damaged target state remain blocking correctness boundaries. |
