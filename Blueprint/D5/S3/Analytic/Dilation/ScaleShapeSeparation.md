...
A positive spectrum is represented by a positive real sequence lambda. Its overall scale a produces the spectrum n maps to a lambda(n), and its zero set consists exactly of the complex s for which the existing spectralZeta(lambda,s) vanishes.
...
The factorization Z_(a lambda)(s)=a^(-s) Z_lambda(s) follows termwise from complex powers. Positivity makes the scale factor nonzero, so multiplication by it cannot create or remove a zero.
