...
internal sealed class GoldenGermWindowLineDichotomyDocument
    : IScribeDocumentDefinition
{
    private const string Module =
        "D5/S3/Analytic/GermWindow/GoldenGermWindowLineDichotomy.";

    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        "Under RH, the continued third-order golden germ has a sharp on-line and "
            + "off-line zero dichotomy inside the open golden window.",
        H("Golden Germ Window Line Dichotomy"),
        Blocks(
            Describe.Lean(
                DescribeId.Create("golden-continued-germ-line-zero-iff-rh"),
                DeclarationHandle.Create(
                    Module + "golden_continued_germ_line_zero_iff_of_rh"),
                H("On-line germ zeros come from the zeta pullback or p = 2, 3"),
                StatementSource.FromAuthor(LineZeroFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(
                    Paragraph(Text(
                        "On the pulled-back critical line, the frozen open-window "
                            + "classification and the critical-line nonvanishing theorem "
                            + "exclude every local factor at primes at least five. The "
                            + "remaining alternatives are the phi-squared zeta pullback "
                            + "and the local factors at p = 2 or p = 3.")),
                    Paragraph(Text(
                        "Whether either small-prime local factor actually vanishes is a "
                            + "numerical question: this theorem neither asserts nor excludes "
                            + "such zeros. The result assumes RH and is not an RH proof path."))),
                DescribeRole.Theorem),
            Describe.Lean(
                DescribeId.Create("golden-continued-germ-off-line-zero-iff-rh"),
                DeclarationHandle.Create(
                    Module + "golden_continued_germ_off_line_zero_iff_of_rh"),
                H("Off-line window zeros are exactly local-factor zeros"),
                StatementSource.FromAuthor(OffLineZeroFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(
                    Paragraph(Text(
                        "Away from the pulled-back critical line, the zeta-zero branch of "
                            + "the frozen window criterion is impossible. Thus a continued "
                            + "germ zero in the open window is exactly a zero of some "
                            + "canonical golden local factor.")),
                    Paragraph(Text(
                        "This is a conditional consequence of RH and the frozen "
                            + "third-order factorization. It supplies no converse and is "
                            + "not a route to proving RH."))),
                DescribeRole.Theorem)),
        [
    ...
    private static Formula OffLineZeroFormula()
    {
        Formula continuedGerm = F.Id("continuedGerm");
        Formula s = F.Id("s");
        Formula p = F.Id("p");
        ...
        Formula classification = Iff(
            Equal(Apply(continuedGerm, s), F.D(0)), localZero);
        Formula conclusion = new Formula.BindMany(
            FormulaQuantifier.ForAll,
            [Bound("s", domain)],
            Implies(
                ...
                Implies(
                    UpperWindow(s),
                    Implies(
                        NotEqual(RealPart(s), CriticalLine()),
                        classification))));
        Formula factorization = new Formula.BindMany(
            FormulaQuantifier.ForAll,
            [Bound("s", domain)],
