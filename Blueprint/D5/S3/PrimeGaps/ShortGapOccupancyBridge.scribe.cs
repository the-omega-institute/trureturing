    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(Describe.Lean(
            ...
            StatementSource.FromAuthor(F.Disp(
                F.Seq(F.Forall, F.Sp, F.Id("H"), F.InMacro, Call("Finset", F.Id("Nat")), F.Comma, F.Sp, F.Id("B"), F.Comma, F.Id("n"), F.InMacro, F.Id("Nat"), F.Comma, F.Sp, F.Grp(F.Forall, F.Sp, F.Id("h"), F.InMacro, F.Id("H"), F.Comma, F.Id("h"), F.Le, F.Id("B")), F.Land, F.D(2), F.Le, Call("primeTranslateOccupancy", F.Id("H"), F.Id("n")), F.Rightarrow, Call("BoundedConsecutivePrimeGapAt", F.Id("B"), F.Id("n"))))),
