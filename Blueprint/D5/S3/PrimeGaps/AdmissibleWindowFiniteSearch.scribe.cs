    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(Describe.Lean(
            ...
            StatementSource.FromAuthor(F.Disp(
                F.Seq(F.Forall, F.Sp, F.Id("k"), F.Comma, F.Id("B"), F.InMacro, F.Sp, F.Id("Nat"), F.Comma, F.D(0), F.Lt, F.Id("k"), F.Rightarrow, F.Grp(Call("admissibleWindowCheck", F.Id("k"), F.Id("B")), F.Eq, F.Id("true"), F.Iff, Call("AdmissibleWindowWitness", F.Id("k"), F.Id("B")))))),
