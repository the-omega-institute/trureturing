...
$$\operatorname{Tendsto}(\operatorname{budget}\left(certificate\right), windows, 0) \Rightarrow (\exists window, finite \subseteq \operatorname{contents}\left(family, window\right)) \land \operatorname{Tendsto}(\operatorname{reading}\left(certificate\right), windows, value)$$
