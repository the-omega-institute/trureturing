    ...
    : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Describe.Lean(
                ...
                StatementSource.FromAuthor(EpsOneFormula()),
                AssessedProvenance.FromLiterature(Source),
                Blocks(Paragraph(Text(
                    "Erdos's density object is the natural density of integers having exactly "
                        + "one divisor in the open interval from n to m. The formal function is "
                        + "totalized by returning zero only when no density exists. In the other "
                        + "branch, chosenDensity denotes the unique density."))),
                DescribeRole.Definition),
            Describe.Lean(
                ...
                DescribeRole.Definition),
            Describe.Lean(
                DescribeId.Create("erdos-one-divisor-valley-obstruction"),
                DeclarationHandle.Create(Prefix + "not_unimodal_of_valley"),
                H("A strict valley obstructs weak unimodality"),
                StatementSource.FromAuthor(ValleyFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "If a proposed mode is at or before the valley bottom, the decreasing tail "
                        + "contradicts the later rise. If it is after the bottom, the increasing "
                        + "head contradicts the preceding fall."))),
    ...
    private static Formula EpsOneFormula()
    {
        var n = F.Id("n");
        var m = F.Id("m");
        var number = F.Id("N");
        var delta = DeltaLower;
        var eventSet = SetWhere(number, Call("exactlyOneDivisorIn", n, m, number));
        var existsDensity = Seq(Exists, Sp, delta, Colon, Sp, Reals(), Comma, Sp,
            Parenthesized(Call("hasDensity", eventSet, delta)));
        var value = IfThenElse(existsDensity, Call("chosenDensity", n, m), D(0));
        return Disp(ForAllMany([Bound("n", Naturals()), Bound("m", Naturals())],
            Equal(Call("epsOne", n, m), value)));
    }
    ...
    private static Formula ValleyFormula()
    {
        var function = F.Id("f");
        var lower = F.Id("lo");
        var a = F.Id("a");
        ...
        var hypotheses = And(
            ...
            Less(a, b),
            Less(b, c),
            Less(Apply(function, b), Apply(function, a)),
            Less(Apply(function, b), Apply(function, c)));
        var conclusion = new Formula.Not(Call("Unimodal", function, lower));
        return Disp(ForAll("f", FunctionsNaturalsToReals(),
            ForAllMany([Bound("lo", Naturals()), Bound("a", Naturals()),
                Bound("b", Naturals()), Bound("c", Naturals())],
    ...
    private static Formula IfThenElse(Formula predicate, Formula whenTrue, Formula whenFalse) =>
        Parenthesized(Seq(Named("if"), Sp, Parenthesized(predicate), Sp, Named("then"), Sp,
            whenTrue, Sp, Named("else"), Sp, whenFalse));
    private static Formula SetWhere(Formula variable, Formula predicate) =>
        Seq(OpenBrace, variable, Sp, Mid, Sp, Parenthesized(predicate), CloseBrace);
