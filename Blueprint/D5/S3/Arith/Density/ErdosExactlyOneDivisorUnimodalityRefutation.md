...
**Definition 1.2 (Exactly one divisor in an interval).**
...
**Definition 1.3 (The one-divisor interval density).**

$$\forall n \in \mathbb{N}, m \in \mathbb{N},\; epsOne\left(n, m\right) = (\operatorname{if} (\exists \delta: \mathbb{R}, (hasDensity\left(\{N \mid (exactlyOneDivisorIn\left(n, m, N\right))\}, \delta\right))) \operatorname{then} chosenDensity\left(n, m\right) \operatorname{else} 0)$$
...
Erdos's density object is the natural density of integers having exactly one divisor in the open interval from n to m. The formal function is totalized by returning zero only when no density exists. In the other branch, chosenDensity denotes the unique density.

**Definition 1.4 (Weak unimodality on a tail).**
...
**Theorem 1.5 (A strict valley obstructs weak unimodality).**

$$\forall f \in \mathbb{N} \to \mathbb{R},\; \forall lo \in \mathbb{N}, a \in \mathbb{N}, b \in \mathbb{N}, c \in \mathbb{N},\; ((lo \le a) \land \left((a < b) \land \left((b < c) \land \left((f\left(b\right) < f\left(a\right)) \land (f\left(b\right) < f\left(c\right))\right)\right)\right)) \Rightarrow (\neg Unimodal\left(f, lo\right))$$

*Proof.* Machine-checked in Lean as `D5/S3/Arith/Density/ErdosExactlyOneDivisorUnimodalityRefutation.not_unimodal_of_valley` (`✓ std3`). ∎

*Source.* Repository-derived.

*Commentary.*

If a proposed mode is at or before the valley bottom, the decreasing tail contradicts the later rise. If it is after the bottom, the increasing head contradicts the preceding fall.
...
- Truth anchor: `D5/S3/Arith/Density/ErdosExactlyOneDivisorUnimodalityRefutation.not_unimodal_of_valley`
- Truth anchor: `D5/S3/Arith/Density/ErdosExactlyOneDivisorUnimodalityRefutation.result`
- Dependency: [D5/S3/ObserverMemory/Prediction/EventualCycleAverage](../../ObserverMemory/Prediction/EventualCycleAverage.md)
