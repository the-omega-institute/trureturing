...
**Lemma 1.3 (The paired totative-sum identity).**

$$\forall n \in \mathbb{N},\; (1 < n) \Rightarrow (2 \cdot \operatorname{totativeSum}\left(n\right) = n \cdot \operatorname{totient}\left(n\right))$$

*Proof.* Machine-checked in Lean as `D5/S3/Arith/KrizekTotativeSumDivisorProductParity.two_mul_totativeSum` (`✓ std3`). ∎

*Source.* Repository-derived.

*Commentary.*

When n is greater than one, subtraction from n pairs every reduced residue k with n-k. Each pair sums to n, and the number of residues is the Euler totient of n.

**Theorem 1.4 (The odd totative-sum classification).**
...
The boundary values one and two are odd. Above them, oddness forces a single odd prime factor, and the paired identity excludes primes congruent to one modulo four. Conversely, every positive power of a prime congruent to three modulo four has an odd totative sum.

**Theorem 1.5 (The divisor-product parity equivalence).**
...
- Truth anchor: `D5/S3/Arith/KrizekTotativeSumDivisorProductParity.two_mul_totativeSum`
