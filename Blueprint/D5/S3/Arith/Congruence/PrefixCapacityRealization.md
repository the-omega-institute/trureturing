...
The existing comb comparison then bounds the actual root mass below by the positive comb flow. Dividing the constructed measure by its actual root mass gives the stated probability law. No monotonicity of the depth capacities is assumed. The formal result supplies real finite leaf weights and their marginal caps; a rational FiniteLaw representation and its subsequent conditional-kernel application are separate proof obligations.
