...
$$\forall K, V, Y, T, C,\\{}N_{\infty} := \operatorname{iInf}\left(k, \operatorname{ker}\left(C \circ T^{k}\right)\right);\\{}N_{\infty} \subseteq \operatorname{ker}\left(C\right) \land \operatorname{MapsTo}\left(T, N_{\infty}, N_{\infty}\right) \land\\{}\forall M, (M \subseteq \operatorname{ker}\left(C\right) \land \operatorname{MapsTo}\left(T, M, M\right)) \Rightarrow M \subseteq N_{\infty}.$$
