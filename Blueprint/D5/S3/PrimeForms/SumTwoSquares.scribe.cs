...
{
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create("A prime congruent to one modulo four is a sum of two natural squares.",
        H("Prime Representation as a Sum of Two Squares"),
        Blocks(
            Describe.Lean(
                DescribeId.Create("prime-congruent-to-one-is-a-sum-of-two-squares"),
                DeclarationHandle.Create("D5/S3/PrimeForms/SumTwoSquares.prime_eq_sq_add_sq_of_mod_four_eq_one"),
                H("A prime congruent to one modulo four is a sum of two squares"),
                StatementSource.FromAuthor(Disp(Seq(F.Id("p"), Esc, F.Text, Grp(F.Id("prime")), Esc, Land, Esc, F.Id("p"), Equiv, Sp, D(1), Esc, Open, Operatorname, Grp(F.Id("mod")), Esc, D(4), Close, Quad, Rightarrow, Quad, Sp, Exists, Sp, F.Id("a"), Comma, F.Id("b"), InMacro, Mathbb, Grp(F.Id("N")), Comma, Esc, F.Id("p"), Eq, F.Id("a"), Caret, D(2), Plus, F.Id("b"), Caret, D(2)))),
