...
{
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create("Squares occupy only residues zero and one modulo four, obstructing residue three.",
        H("Quadratic Residues Modulo Four"),
        Blocks(
            Describe.Lean(
                DescribeId.Create("square-residues-and-sum-obstruction"),
                DeclarationHandle.Create("D5/S3/PrimeForms/QuadraticResidues.square_residues_and_sum_obstruction"),
                H("Square residues and the two-square obstruction"),
                StatementSource.FromAuthor(Disp(Seq(Left, Open, Forall, Sp, F.Id("n"), InMacro, Mathbb, Grp(F.Id("N")), Comma, Esc, F.Id("n"), Caret, D(2), Operatorname, Grp(F.Id("mod")), D(4), InMacro, OpenBrace, D(0), Comma, D(1), CloseBrace, Right, Close, Esc, Land, Esc, Left, Open, Forall, Sp, F.Id("a"), Comma, F.Id("b"), InMacro, Mathbb, Grp(F.Id("N")), Comma, Esc, Open, F.Id("a"), Caret, D(2), Plus, F.Id("b"), Caret, D(2), Close, Operatorname, Grp(F.Id("mod")), D(4), Neq, D(3), Right, Close, Dot))),
