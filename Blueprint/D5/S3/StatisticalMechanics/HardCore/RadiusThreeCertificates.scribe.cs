    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Describe.Lean(
                ...
                DescribeRole.Theorem),
            Describe.Lean(
                DescribeId.Create("hard-core-radius-three-certificates-sub-five-thousand-forty-one-controller"),
                DeclarationHandle.Create("D5/S3/StatisticalMechanics/HardCore/RadiusThreeCertificates.SubFiveThousandFortyOneController"),
                H("A controller below the certified floor"),
                StatementSource.WithoutFormula(), AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text("The claim asserts that some history-dependent policy and some natural depth have a scaled descendant count strictly below the radius-three floor: 2000 to that depth times the count is less than 5041 to that depth, starting from the empty history and initial parent-blocked mask."))),
                DescribeRole.Definition),
            Describe.Lean(
                DescribeId.Create("hard-core-radius-three-certificates-no-sub-five-thousand-forty-one-controller"),
                DeclarationHandle.Create("D5/S3/StatisticalMechanics/HardCore/RadiusThreeCertificates.no_sub_five_thousand_forty_one_controller"),
                H("No controller falls below the certified floor"),
                StatementSource.WithoutFormula(), AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text("The universal lower bound refutes the existential claim for every policy, including arbitrary dependence on direction history, and every natural depth. The exclusion concerns the radius-three memory model."))),
                DescribeRole.Theorem),
            Describe.Lean(
