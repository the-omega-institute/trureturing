...
**Definition 1.11 (A controller below the certified floor).**

Lean statement: `D5/S3/StatisticalMechanics/HardCore/RadiusThreeCertificates.SubFiveThousandFortyOneController`

*Formalization.* `D5/S3/StatisticalMechanics/HardCore/RadiusThreeCertificates.SubFiveThousandFortyOneController` (`✓ std3`).

*Source.* Repository-derived.

*Commentary.*

The claim asserts that some history-dependent policy and some natural depth have a scaled descendant count strictly below the radius-three floor: 2000 to that depth times the count is less than 5041 to that depth, starting from the empty history and initial parent-blocked mask.

**Theorem 1.12 (No controller falls below the certified floor).**

Lean statement: `D5/S3/StatisticalMechanics/HardCore/RadiusThreeCertificates.no_sub_five_thousand_forty_one_controller`

*Proof.* Machine-checked in Lean as `D5/S3/StatisticalMechanics/HardCore/RadiusThreeCertificates.no_sub_five_thousand_forty_one_controller` (`✓ std3`). ∎

*Source.* Repository-derived.

*Commentary.*

The universal lower bound refutes the existential claim for every policy, including arbitrary dependence on direction history, and every natural depth. The exclusion concerns the radius-three memory model.

**Theorem 1.13 (A larger fixed-order lower bound).**
...
**Theorem 1.14 (An actual finite-grid-domain consumer).**
...
- Truth anchor: `D5/S3/StatisticalMechanics/HardCore/RadiusThreeCertificates.SubFiveThousandFortyOneController`
- Truth anchor: `D5/S3/StatisticalMechanics/HardCore/RadiusThreeCertificates.no_sub_five_thousand_forty_one_controller`
