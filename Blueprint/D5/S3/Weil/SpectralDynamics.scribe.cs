...
internal sealed class SpectralDynamicsDocument : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeDocument.Create(
        ...
        Blocks(
            ...
            new DocumentBlock.Describe(
                DescribeId.Create("inequalities-carry-the-narrative-arrow-of-time"),
                DescribeKind.Remark,
                H("Inequalities carry the narrative arrow of time"),
                DescribeStatement.FromFormula(Equal(Id("arrowOfTime"), Id("inequalityLayer"))),
                DescribeProvenance.RepoDerived(),
                Blocks(Paragraph(Text(
                    "The source separates reversible equality laws from directional inequality laws. Newton, Schrodinger, and Maxwell equations are placed in the time-reversal-symmetric layer, while dS >= 0 supplies the physical arrow. The arithmetic analogy places antiunitary identities in the reversible ledger and Weil, Li, and finite Q_n positivity in a zeta second-law layer; monotone bookkeeping cost is its internal dS >= 0. In this reading the Riemann hypothesis is a second-law statement that no account goes negative. A four-frame toy de Bruijn-Newman heat flow is recorded as moving a complex pair onto the real axis, and Lambda = 0 is read as crystallization completed exactly at t = 0. The accompanying scattering dictionary calls an on-line zero a stable real-energy eigenstate and an off-line zero a finite-lifetime complex resonance, matching the standard statement that stable S-matrix poles are real and unstable ones are off-axis. The source explicitly limits the metaphor: if RH is true, ordinary matter may be a resonance of another Hamiltonian, or the internal reader may live in the observation window rather than in the spectrum. Both alternatives preserve only the interpretive conclusion that finite-lived readers belong to the inequality layer, while equality laws themselves neither arise nor decay.")))),
            new DocumentBlock.Describe(
                ...
                DescribeStatement.FromFormula(Equal(
                    new Formula.Subscript(Id("H"), Num(0)),
                    Id("Xi"))),
                DescribeProvenance.RepoDerived(),
                Blocks(Paragraph(Text(
            ...
            new DocumentBlock.Describe(
                DescribeId.Create("riemann-hypothesis-shaped-real-zero-properties-form-a-family"),
                DescribeKind.Remark,
                H("Riemann-hypothesis-shaped real-zero properties form a family"),
                DescribeStatement.FromFormula(Equal(Id("RH"), Id("LeeYangPhi"))),
                DescribeProvenance.RepoDerived(),
                Blocks(Paragraph(Text(
                    "The source uses the de Bruijn-Newman constant as a functional of a measure: for each measure rho there is a threshold Lambda_DN(rho) such that the Fourier transform of exp(lambda*t^2) d rho has only real zeros exactly above that threshold. The Riemann hypothesis is one member of this family, expressed there as Lambda_DN(Phi) <= 0. The family contains both outcomes. Ising-type models satisfy the Lee-Yang property, including the cited general single-spin and Lieb-Sokal results, while the cited Villain model and complex Gaussian multiplicative chaos furnish failures; real-axis spectrum is therefore reported to fail in natural systems. Newman-Wu tail criteria are used to say that these questions can be governed by the measure's tail behavior. The proposed statistical-physics name for O-6 is consequently to prove that Phi belongs to a Lee-Yang class. Phi's superexponential tail is recorded as favorable structure, not a proof. The balance of examples is deliberately two-sided: Ising models aid the true side, Villain-type failures aid the false side, and none of the known failures is the zeta member.")))),
            new DocumentBlock.Describe(
                ...
                DescribeStatement.FromFormula(Equal(
                    Id("causality"),
                    Multiply(Id("logic"), Id("irreversibility")))),
                DescribeProvenance.RepoDerived(),
                Blocks(Paragraph(Text(
