...
internal sealed class PaperFTWindowLimitDocument : IScribeDocumentDefinition
{
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        "From genuine varying operator-domain Rayleigh certificates and an explicit scaled rate, transfers a candidate Fourier limit to projectively normalized actual eigenvectors on any target subset of the strip, including compact sets or contours. Candidate convergence is required only on that target. No Xi convergence is postulated or proved here.",
        ...
        Blocks(
            ...
            Describe.Lean(DescribeId.Create("item-2"),
                DeclarationHandle.Create("D5/S3/Weil/FourierReadout/PaperFTWindowLimit.paperFT_fixed_window_uniform"), H("Fixed-window L2 convergence suffices"),
                StatementSource.FromLean(), AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text("Ordinary L2 convergence on a fixed finite interval gives uniform convergence of the existing paperFT on every closed horizontal strip. No separate uniform Fourier convergence hypothesis is required."))), DescribeRole.Theorem),
