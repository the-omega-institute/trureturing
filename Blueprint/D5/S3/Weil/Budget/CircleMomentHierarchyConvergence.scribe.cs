...
internal sealed class CircleMomentHierarchyConvergenceDocument : IScribeDocumentDefinition
{
    private const string Prefix =
        "D5/S3/Weil/Budget/CircleMomentHierarchyConvergence.";

    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        "Nested finite circle-moment completion frontiers converge to the frontier determined "
            + "by the full moment family.",
        H("Circle Moment Hierarchy Convergence"),
        Blocks(Describe.Lean(
            DescribeId.Create("circle-moment-hierarchy-converges"),
            DeclarationHandle.Create(Prefix + "circle_moment_hierarchy_converges"),
            H("Finite circle-moment frontiers decrease to the full frontier"),
            StatementSource.FromAuthor(HierarchyFormula()),
            AssessedProvenance.FromRepo(),
            Blocks(Paragraph(Text(
                "Nested circle-moment constraints make the finite frontiers antitone. A strict "
    ...
    private static Formula HierarchyFormula()
    {
        ...
        Formula circle = Call("Circle"), measure = Call("Measure", circle);
