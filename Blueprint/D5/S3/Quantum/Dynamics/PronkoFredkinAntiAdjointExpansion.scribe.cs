    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            TheoremNode("result", "Conjecture 2 holds", ClaimFormula(),
                "For spin words y and x let b count the sites with y up and x down, and c the sites with y down and x up. Let Z_r be the matrix with entry one exactly when b = r + 1 and c = r, and E_s the matrix with entry one exactly when b = c = s. Summing single-site flips site by site gives the anticommutator identities {S^-, Z_r} = 2(r + 1) E_(r+1) + (N - 2r) E_r and {S^+, E_s} = 2(s + 1) Z_s + (N - 2s + 1) Z_(s-1), where the counts of sites with equal letters combine to N - b - c. Composing them, the map X to {S^+, {S^-, X}} sends Z_r to 4(r + 1)(r + 2) Z_(r+1) + 2(r + 1)(2N - 4r - 1) Z_r + (N - 2r)(N - 2r + 1) Z_(r-1). The case s = 0 gives S^+ = Z_0 because E_0 is the identity, and the leading coefficient 4(r + 1)(r + 2) is nonzero, so by induction on r each Z_r lies in the span of the first r + 1 iterates of that map applied to S^+. Reading the Kronecker sum (3.1) entrywise, every factor is 0 or 1 and at each site pair exactly one exponent gives 1, so Sigma^+(y, x) = 1 exactly when b - c = 1, and b + c is at most N, so Sigma^+ is the sum of Z_r over r below the ceiling of N/2 and lies in the span of the first ceiling of N/2 iterates, which yields the coefficients. Transposition exchanges sigmaPlus with sigmaMinus, maps Sigma^+ to Sigma^- and each raising iterate to the corresponding lowering iterate, so the same coefficients give the lowering identity.",
