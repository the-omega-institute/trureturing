...
$$\forall \rho, \sigma \in \operatorname{DensityState}(a), \phi \in \operatorname{QuantumChannel}(a, b), \psi \in \operatorname{QuantumChannel}(b, c), \delta_{\psi\circ\phi}(\rho, \sigma) = \delta_{\phi}(\rho, \sigma) + \delta_{\psi}(\phi\rho, \phi\sigma).$$
