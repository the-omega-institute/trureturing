...
$$\begin{gathered}\forall index: \operatorname{Type}, tau: \operatorname{Perm}\left(index\right), f: index \to \mathbb{C},\\L_{tau}(f) = 0 \Leftrightarrow f \circ tau = f; \\\ker L_{tau} = \operatorname {Inv}_{tau}; \\\forall M \in \mathbb{N}_{>0},\ \forall g: \operatorname {ZMod}(M)\to \mathbb{C},\ g \in \ker L_{+1} \iff \exists c\in \mathbb{C},\ g = (i\mapsto c).\end{gathered}$$
