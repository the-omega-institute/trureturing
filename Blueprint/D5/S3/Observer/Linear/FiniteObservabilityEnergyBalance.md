...
$$\begin{aligned}\forall K, V, Y: \operatorname{Type},\\{}[\operatorname{RCLike}(K)] \land [\operatorname{NormedAddCommGroup}(V)] \land [\operatorname{InnerProductSpace}(K, V)] \land [\operatorname{CompleteSpace}(V)] \land\\{}[\operatorname{NormedAddCommGroup}(Y)] \land [\operatorname{InnerProductSpace}(K, Y)] \land [\operatorname{CompleteSpace}(Y)] \land\\\forall A: \operatorname{ContinuousLinearMap}(K, V, V), C: \operatorname{ContinuousLinearMap}(K, V, Y), N: \mathbb{N}, (\operatorname{adjoint}(A) \circ A) + (\operatorname{adjoint}(C) \circ C) = \operatorname{id}() \Rightarrow\\{}\\{}\operatorname{id}() - (\operatorname{adjoint}(A^{N}) \circ A^{N}) = \sum_{k\in\operatorname{range}(N)} (\operatorname{adjoint}(A^{k}) \circ (\operatorname{adjoint}(C) \circ C) \circ A^{k}) \land \operatorname{IsPositive}(\sum_{k\in\operatorname{range}(N)} (\operatorname{adjoint}(A^{k}) \circ (\operatorname{adjoint}(C) \circ C) \circ A^{k})) \land \forall x: V, \left\lVert x \right\rVert^{2} - \left\lVert (A^{N} x) \right\rVert^{2} = \sum_{k\in\operatorname{range}(N)} \left\lVert (C A^{k} x) \right\rVert^{2}.\end{aligned}$$
