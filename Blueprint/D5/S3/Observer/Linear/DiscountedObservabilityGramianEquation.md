...
$$\begin{gathered}\forall K, V, Y, T, C, \beta,\\{}\operatorname{RCLike}(K) \land \operatorname{FiniteDimensional}(K, V) \land \operatorname{FiniteDimensional}(K, Y) \land\\{}0 < \beta < 1 \land \sqrt{\beta} \left\lVert T \right\rVert < 1 \Rightarrow\\{}\operatorname{discountedObservabilityGramian}(T, C, \beta) = C^{*} C + \beta T^{*} \operatorname{discountedObservabilityGramian}(T, C, \beta) T.\end{gathered}$$
