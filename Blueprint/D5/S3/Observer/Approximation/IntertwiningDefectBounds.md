...
$$\forall K, X, Y, A, C, T, L, n,\\{}\operatorname{NontriviallyNormedField}(K) \land \operatorname{SeminormedAddCommGroup}(X) \land \operatorname{NormedSpace}(K, X) \land\\{}\operatorname{SeminormedAddCommGroup}(Y) \land \operatorname{NormedSpace}(K, Y) \land A \in \operatorname{ContinuousLinearMap}(K, Y, Y) \land\\{}C \in \operatorname{ContinuousLinearMap}(K, X, Y) \land T \in \operatorname{ContinuousLinearMap}(K, X, X) \land L \in R \land n \in N \Rightarrow\\{}\Vert C \cdot T^{n} - A^{n} \cdot C \Vert \leq \sum_{j=0}^{n-1} \Vert A \Vert^{n-1-j} \cdot \Vert C \cdot T - A \cdot C \Vert \cdot \Vert T \Vert^{j} \land\\{}{\Vert A \Vert \leq L \land \Vert T \Vert \leq L \Rightarrow \Vert C \cdot T^{n} - A^{n} \cdot C \Vert \leq n \cdot L^{n-1} \cdot \Vert C \cdot T - A \cdot C \Vert}$$
