    ...
    : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            Node("point", "Hidden critical points",
                PointFormula(),
            ...
            Node("query", "Discrete query locations",
                QueryFormula(),
                "Query(N) is the dependent product of Fin(N(i)+1), representing physical "
                    + "query coordinates 0 through N(i). For integer hidden coordinates, "
                    + "a real drop position has the same outcomes as its coordinatewise "
                    + "floor, so real-valued positions reduce to this discrete carrier.",
                DescribeRole.Definition, AssessedProvenance.FromLiterature(Source)),
            Node("drop-outcome", "Broken-or-intact query outcome",
                DropOutcomeFormula(),
                "A hidden value h(i) encodes the physical coordinate x(i)=h(i)+1, while "
                    + "q(i) is the physical query coordinate. Hence q(i)<x(i) is equivalent "
                    + "to val(q(i))<=val(h(i)); the value is one exactly when this holds in "
                    + "every coordinate, and is zero otherwise.",
                DescribeRole.Definition, AssessedProvenance.FromLiterature(Source)),
            Node("egg-strategy", "Adaptive strategies with egg and drop budgets",
                ...
                "A stop node returns a point at any remaining budget. A drop node asks one "
                    + "query from Query(N), sends outcome zero to a child with one fewer egg, "
                    + "sends outcome one to a child with the same egg count, and consumes one "
                    + "drop.",
                DescribeRole.Definition, AssessedProvenance.FromLiterature(Source)),
            ...
            Node("result", "The universal assertion is false",
                ...
                new OpenProblemResolutionClaim(
                    ProblemSlugRef.Create("cao-chen-miller-egg-drop-conjecture-one"),
    ...
    private static Formula QueryFormula()
    {
        Formula d = F.Id("d"), n = F.Id("N"), i = F.Id("i");
        Formula queryBody = ForAll("i", Fin(d), Fin(Add(Apply(n, i), D(1))));
        return Disp(ModelBinders(Equal(Call("Query", n), queryBody)));
    }
    ...
    private static Formula EggStrategyFormula()
    {
        ...
        Formula dropType = ForAll("e", Naturals(), ForAll("h", Naturals(),
            Arrow(query, Arrow(strategy, Arrow(intactStrategy, nextStrategy)))));
    ...
    private static Formula PredictionFormula()
    {
        ...
        Formula stopNode = Coerce(Call("stop", guess), Call("EggStrategy", n, eggs, drops));
        Formula stopEquation = Equal(
            Call("prediction", stopNode, hidden), guess);
    ...
    private static Formula PaddedTranscriptFormula()
    {
        ...
        Formula stopNode = Coerce(Call("stop", guess), Call("EggStrategy", n, eggs, drops));
        Formula stopEquation = Equal(
            Call("paddedTranscript", stopNode, hidden, coordinate), D(0));
