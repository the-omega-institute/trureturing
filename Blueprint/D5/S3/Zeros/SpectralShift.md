# Multiplicative Address Shifts and Labeled Zeta
...
*Formalization.* `D5/S3/Zeros/SpectralShift.backwardShift` (`✓ std3`).
...
$$\forall s\in\mathbb{C},\ \forall u,a\in\operatorname{PrimeAxisTable},\ \operatorname{backwardShift}(u,\operatorname{labeledZetaCoefficient}(s),a)=\operatorname{labeledZetaCoefficient}(s,u)\operatorname{labeledZetaCoefficient}(s,a)$$

*Proof.* Machine-checked in Lean as `D5/S3/Zeros/SpectralShift.labeled_zeta_backward_shift_eigen` (`✓ std3`). ∎
...
*Proof.* Machine-checked in Lean as `D5/S3/Zeros/SpectralShift.labeled_zeta_vector_backward_shift_eigen` (`✓ std3`). ∎
