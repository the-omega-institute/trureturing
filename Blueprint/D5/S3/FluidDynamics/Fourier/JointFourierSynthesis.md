# Joint Fourier Synthesis

## Abstract

Weighted Fourier synthesis of grade n+2 is jointly real C^n in coefficients and position.

**Theorem 1.1 (Joint differentiability of weighted Fourier synthesis).**

Lean statement: `D5/S3/FluidDynamics/Fourier/JointFourierSynthesis.joint_contdiff_fourier_synthesis`

*Proof.* Machine-checked in Lean as `D5/S3/FluidDynamics/Fourier/JointFourierSynthesis.joint_contdiff_fourier_synthesis` (`✓ std3`). ∎

*Source.* Repository-derived.

*Commentary.*

Let K be the full lattice Z x Z and V be EuclideanSpace C (Fin 2), the complex two-dimensional vector space with its Euclidean norm. The coefficient space H is lp (fun _ : K => V) 2, with arbitrary square-summable vector coefficients. The spatial space X is Fin 2 -> R with its usual supremum norm. Differentiation is over R on the product H x X, so it includes every real coefficient direction as well as every spatial direction.
