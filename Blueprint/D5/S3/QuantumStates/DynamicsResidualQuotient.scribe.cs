...
internal sealed class DynamicsResidualQuotientDocument : IScribeDocumentDefinition
{
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        "The orthogonal residual of the adjoint observable closure is invariant and has a quotient dynamics.",
        H("Dynamics on the Final Orthogonal Residual"),
        Blocks(
            Describe.Lean(
                DescribeId.Create("adjoint-observable-residual-is-dynamically-invariant"),
                DeclarationHandle.Create(
                    "D5/S3/QuantumStates/DynamicsResidualQuotient."
                        + "dynamics_residual_invariant_and_quotient"),
                H("The final orthogonal residual is invariant and quotients the dynamics"),
                ...
                Blocks(
                    Paragraph(Text(
                        "The visible space is constructed from every forward iterate of the "
                            ...
                            + "The final residual is its orthogonal complement.")),
                    Paragraph(Text(
                        "The displayed adjoint pairing transfers orthogonality from the residual "
                            + "through Phi, yielding Phi(R) contained in R. The canonical quotient "
                            + "lift then constructs the induced linear map and its projection law.")),
                    Paragraph(Text(
                        "Repository search found no packaged theorem containing all clauses. "
                            + "Pinned Mathlib supplies and is applied through "
                            + "Submodule.mem_orthogonal', Submodule.liftQ, "
                            + "Submodule.liftQ_apply, and Submodule.Quotient.mk_eq_zero."))),
    ...
    private static Formula ResidualFormula()
    {
        ...
        Formula visible = Call("observableClosure", map, visibleSeed);
        ...
        return Disp(Seq(
            ...
            Call("mkQ", residual, Apply(evolution, x)), Dot));
    }
}
