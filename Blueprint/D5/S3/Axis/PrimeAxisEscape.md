...
$$\forall S\subset_{\operatorname{fin}}\mathbb{N},\ (\forall p\in S,\ p\ \text{prime}) \Rightarrow ((\forall p\in S,\ \prod_{r\in S}r+1\equiv 1\ (\operatorname{mod}\ p)) \land \exists q\in\mathbb{N},\ q\ \text{prime}\land q\mid \prod_{r\in S}r+1\land \neg(q\in S))$$
