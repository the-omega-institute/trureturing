...
internal sealed class PositiveExactRealizationFiberDocument : IScribeDocumentDefinition
{
    ...
    private static Formula CallFormula(string name, params Formula[] arguments) =>
    ...
    private static Formula TheoremFormula()
    {
        ...
        Formula zeroAtN = CallFormula("zero", z, n);
        ...
        Formula exhaustive = Seq(
            ...
            ImpliesFormula(
                And(
                    EqualTo(CallFormula("riemannZeta", rho), D(0)),
                    nontrivial,
                    Seq(rho, Sp, Neq, Sp, D(1))),
                Seq(Exists, Sp, n, Sp, InMacro, Sp, natural, Comma, Sp,
                    EqualTo(zeroAtN, rho))));
        Formula fiber = CallFormula("PositiveExactRealization", z);
        Formula conclusion = ImpliesFormula(
            CallFormula("Nonempty", fiber),
