...
internal sealed class ReferenceFrameTaxExactDocument : IScribeDocumentDefinition
{
    ...
    private static Formula ExactTaxFormula() => F.Disp(F.Seq(
        ...
        F.Mathbb, F.Grp(F.Id("R")), F.Caret, F.Grp(F.Id("N")),
        F.Comma, F.Quad, F.Sp,
        ...
        F.Comma, F.Sp, F.RowBreak, F.Sp,
        F.Operatorname, F.Grp(F.Id("squaredTopEigenspace")), F.Underscore,
        F.Grp(F.Mathbb, F.Grp(F.Id("R"))),
        F.Open, F.Id("N"), F.Close, F.Sp, F.Eq, F.Sp,
        F.Operatorname, F.Grp(F.Id("topModeSpace")), F.Underscore,
        F.Grp(F.Mathbb, F.Grp(F.Id("R"))),
