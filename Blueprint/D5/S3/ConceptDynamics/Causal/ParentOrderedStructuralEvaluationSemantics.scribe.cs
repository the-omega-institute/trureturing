...
internal sealed class ParentOrderedStructuralEvaluationSemanticsDocument : IScribeDocumentDefinition
{
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        "A finite parent-ordered structural model has a unique post-intervention evaluation trace.",
        H("Parent-Ordered Structural Evaluation Semantics"),
        Blocks(
            Describe.Lean(
                DescribeId.Create("parent-ordered-structural-evaluation-is-unique"),
                DeclarationHandle.Create(
                    "D5/S3/ConceptDynamics/Causal/ParentOrderedStructuralEvaluationSemantics."
                        + "parent_ordered_structure_evaluation_semantics"),
                H("Parent-ordered structural evaluation is unique"),
                StatementSource.FromAuthor(TheoremFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(
                    Paragraph(Text(
                        "The model carries finite nodes, parent sets, structural equations "
                            + "whose inputs are parent coordinates, and an external-state "
                            + "initialization. A supplied topological-order certificate places "
