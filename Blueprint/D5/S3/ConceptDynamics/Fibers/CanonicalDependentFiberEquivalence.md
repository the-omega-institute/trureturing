...
$$\forall X, B: \operatorname{Type}, q: X \to B, \operatorname{let} R(b) := \operatorname{ConceptFiber}\left(q, b\right), e_{q} := \operatorname{canonical}(q): X \equiv \sum _{b: B} R(b), (\forall x: X, e_{q}(x) = \langle q(x), \langle x, refl\rangle\rangle) \land (\forall b: B, x: X, p: q(x) = b, e_{q}^{-1}(\langle b, \langle x, p\rangle\rangle) = x).$$
