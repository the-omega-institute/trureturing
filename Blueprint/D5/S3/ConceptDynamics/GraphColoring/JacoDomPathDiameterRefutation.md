...
At n = 33, kernel-decided positive entries in powers of the adjacency matrix give a walk of length at most seven between every two vertices through Mathlib's walk-counting identity. Four disjoint closed neighborhoods force every dominating set to contain at least four vertices, while a path graph on at most nine vertices has domination number at most three. A ten-vertex dom-path exists, so the contradiction is not caused by an empty notion.
