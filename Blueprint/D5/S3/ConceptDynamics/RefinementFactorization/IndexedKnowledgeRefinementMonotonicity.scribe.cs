...
internal sealed class IndexedKnowledgeRefinementMonotonicityDocument
    : IScribeDocumentDefinition
{
    private const string Declaration =
        "D5/S3/ConceptDynamics/RefinementFactorization/IndexedKnowledgeRefinementMonotonicity."
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        H("Indexed Knowledge Refinement Monotonicity"),
        Blocks(Describe.Lean(
            DescribeId.Create(
                "knowledge-monotone-under-indexed-refinement-with-converse-countermodel"),
            ...
            Blocks(
                Paragraph(Text(
                    "Knowledge is target factorization through the canonical joint readout. "
                        + "Restriction from the fine joint output to the coarse output supplies "
                        + "the readout refinement for nested finite budgets.")),
                Paragraph(Text(
                    "Composing the coarse target factor with that restriction proves the first "
                        + "public clause for arbitrary state, output, and target carriers.")),
                Paragraph(Text(
                    "The second public clause uses one Boolean target, one indexed Boolean "
                        + "readout, and nested budgets as a shared countermodel. The fine budget "
                        + "knows the target while the coarse budget does not."))),
