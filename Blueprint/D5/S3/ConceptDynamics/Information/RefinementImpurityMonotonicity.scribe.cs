...
internal sealed class RefinementImpurityMonotonicityDocument : IScribeDocumentDefinition
{
    private const string Declaration =
        "D5/S3/ConceptDynamics/Information/RefinementImpurityMonotonicity."
            + "refinement_impurity_monotone";

    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        "Factor-map refinement cannot increase conditional logical impurity.",
        H("Refinement Lowers Conditional Logical Impurity"),
        Blocks(Describe.Lean(
            DescribeId.Create("refinement-impurity-monotone"),
            DeclarationHandle.Create(Declaration),
            H("Refinement cannot increase impurity"),
            StatementSource.FromAuthor(TheoremFormula()),
            AssessedProvenance.FromRepo(),
            Blocks(
                Paragraph(Text(
                    "A coarse concept fiber is the union of the refined fibers selected by "
                        + "the refinement factor map.")),
                Paragraph(Text(
                    "The countable Cauchy inequality bounds each coarse target-collision "
                        + "term by the corresponding refined terms. The complementary "
                        + "pair-disagreement normalization then reverses this comparison "
                        + "for conditional logical impurity."))),
