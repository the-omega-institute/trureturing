                ...
                Blocks(
                    ...
                    Paragraph(Text(
                        "The first conjunct applies residual_join_law. The second uses the same "
                            ...
                            + "premise. The third assumes exactly that the added definition is "
                            + "constant on every fiber of the baseline readout. It does not "
                            + "assert a total codomain map outside the baseline readout's image. "
                            + "An Empty-state example satisfies this fiber condition while "
                            + "Refines is false, pinning the weaker premise used by the conjunct. "
                    ...
                    Paragraph(Text(
                        "CAS section 4.4 defines M(S) as remaining mass, defines F(S) as "
                            ...
                            + "uses the canonical defectRelation on (Nat times Bool) pairs and "
                            + "the complement of the Prod.snd kernel. It proves that the residual, "
                            + "remaining relation, and captured cut have the source-prescribed "
                            + "relation shape. M(empty) and M(singleton) are both infinity, so "
                            + "ENNReal subtraction makes F(singleton) zero while captured mass is "
                            + "infinity. Clause six remains open under TASK D5-T0049 and is not "
                            + "represented by a constant data declaration. The sixth displayed "
