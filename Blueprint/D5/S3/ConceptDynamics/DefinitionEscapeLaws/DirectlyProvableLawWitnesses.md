...
Each of those eight source-law positions consumes the matching projection of directly_provable_laws. The final present label is an adjacent strict captured-mass submodularity example. It is consumed by the package but is not source clause six and does not close TASK D5-T0049.
...
There are ten names because clause three has two independent checks and the adjacent capture boundary is retained separately. The package references every name, so deleting any witness makes the package fail to elaborate instead of silently removing an anonymous example.
