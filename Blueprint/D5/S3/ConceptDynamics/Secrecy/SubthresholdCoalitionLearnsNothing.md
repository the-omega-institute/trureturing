...
$$\left(\neg \operatorname{Refines}\left(\operatorname{canonicalTargetReadout}\left(id\right), fst\right)\right) \land \exists target \in Bool \times Bool \to Bool, secretFunction \in Bool \times Bool \to Bool,\; target = secretFunction \circ id \land \left(\operatorname{Refines}\left(\operatorname{canonicalTargetReadout}\left(target\right), fst\right) \land \exists x \in Bool \times Bool, y \in Bool \times Bool,\; target\left(x\right) \ne target\left(y\right)\right)$$
