...
$$\forall X \in Type, M \in Type, Tval \in Type,\; \operatorname{Nonempty}\left(X\right) \Rightarrow \forall message \in X \to M, target \in X \to Tval,\; \forall a \in X, b \in X,\; message\left(a\right) = message\left(b\right) \Rightarrow target\left(a\right) = target\left(b\right) \Rightarrow \left(\exists delta \in M \to Tval,\; \left(\forall actual \in X,\; delta\left(message\left(actual\right)\right) = target\left(actual\right)\right) \land \left(\forall actual \in X,\; \neg \operatorname{PartitionManipulation}\left(message, target, delta, actual\right)\right)\right)$$
