...
internal sealed class GlobalProfileCounterfactualTargetMinimalityDocument : IScribeDocumentDefinition
{
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        "Fiber-constant target families factor uniquely through the canonical profile image.",
        H("Global Profile Counterfactual Target Minimality"),
        Blocks(
            Describe.Lean(
                DescribeId.Create("target-family-factors-through-global-profile-image"),
                DeclarationHandle.Create(
                    "D5/S3/ConceptDynamics/CanonicalImage/GlobalProfileCounterfactualTargetMinimality."
                        + "global_profile_target_family_factors"),
                H("Target families factor through the canonical global-profile image"),
                StatementSource.FromAuthor(TheoremFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(
                    Paragraph(Text(
                        "The query family sends each model to the set of possible values of each "
                            + "query, and the canonical globalProfile collects those answers into "
                            + "one dependent profile. CounterfactualImage is the realized image "
                            + "of this profile, with counterfactualProjection as its canonical map.")),
                    Paragraph(Text(
                        "For every target index, constancy on global-profile fibers gives a "
                            + "target-valued factor on the image. Surjectivity of the canonical "
                            + "image map makes that factor unique, so all targets in the family "
                            + "descend through the same named image object."))),
    ...
    private static Formula TheoremFormula()
    {
        ...
        Formula fiberConstancy = Seq(
            ...
            Apply("globalProfile", queries, left), Sp, Eq, Sp,
            Apply("globalProfile", queries, right), Sp, Rightarrow, Sp,
