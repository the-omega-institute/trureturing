...
The proof removes isolates only for the incidence reduction, transfers the mixed populations to the finite-set API, applies the substantive `small_population_bound` certificate, and restores the isolated vertices with the reciprocal monotonicity estimate. The public theorem has only graph, coloring, degree, and mixed-count hypotheses.
