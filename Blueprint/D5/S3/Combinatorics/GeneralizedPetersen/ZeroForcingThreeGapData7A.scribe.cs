...
internal sealed class ZeroForcingThreeGapData7ADocument : IScribeDocumentDefinition
{
    private const string Prefix = "D5/S3/Combinatorics/GeneralizedPetersen/ZeroForcingThreeGapData7A.";
    ...
    public DocumentDefinition Create()
    {
        ...
        return DocumentDefinition.Create(ScribeNode.Create(
            ...
            H("Bounded Gap Roots"),
            Blocks(
            Node("roots-7-7-7", "7 gaps with root 7", "roots7_top",
                Disp(Q(Imp(And(Le(Num(7), q), Le(q, Num(7))), Eq(Call("maskCheck", Num(7), Num(6), q, ListOf(q)), True())), ("q", Nat()))),
                "For 7 positive gaps, maskCheck accepts the root prefix [q] with 6 remaining entries and maximum gap q, for q equal to 7.",
                DescribeRole.Theorem),
            Node("roots-7-1-4", "7 gaps with root 1 to 4", "roots7",
                Disp(Q(Imp(And(Le(Num(1), q), Le(q, Num(4))), Eq(Call("maskCheck", Num(7), Num(6), q, ListOf(q)), True())), ("q", Nat()))),
