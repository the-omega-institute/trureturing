...
internal sealed class ZeroForcingThreeGapData8CDocument : IScribeDocumentDefinition
{
    private const string Prefix = "D5/S3/Combinatorics/GeneralizedPetersen/ZeroForcingThreeGapData8C.";
    ...
    public DocumentDefinition Create()
    {
        ...
        return DocumentDefinition.Create(ScribeNode.Create(
            ...
            H("Bounded Gap Roots"),
            Blocks(
            Node("roots-8-7-7", "8 gaps with root 7", "roots8_top",
                Disp(Q(Imp(And(Le(Num(7), q), Le(q, Num(7))), Eq(Call("maskCheck", Num(8), Num(7), q, ListOf(q)), True())), ("q", Nat()))),
                "For 8 positive gaps, maskCheck accepts the root prefix [q] with 7 remaining entries and maximum gap q, for q equal to 7.",
                DescribeRole.Theorem),
            Node("roots-8-6-6", "8 gaps with root 6", "roots8",
                Disp(Q(Imp(And(Le(Num(6), q), Le(q, Num(6))), Eq(Call("maskCheck", Num(8), Num(7), q, ListOf(q)), True())), ("q", Nat()))),
