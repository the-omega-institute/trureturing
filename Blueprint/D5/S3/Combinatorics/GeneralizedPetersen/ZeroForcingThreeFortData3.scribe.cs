...
internal sealed class ZeroForcingThreeFortData3Document : IScribeDocumentDefinition
{
    private const string Prefix = "D5/S3/Combinatorics/GeneralizedPetersen/ZeroForcingThreeFortData3.";
    ...
    public DocumentDefinition Create()
    {
        ...
        return DocumentDefinition.Create(ScribeNode.Create(
            ...
            H("Forts for the outerPrev Anchor"),
            Blocks(
            Node("certified-family", "A complete ordered fort family", "certifiedFamily",
                Disp(Ex("rows", List(Seq(Nat(), Times, Sp, Nat())), And(Eq(Call("familyOrderOK", rows), True()), Q(Imp(Mem(row, rows), And(Eq(Call("candidateShapeOK", Name("outerPrev"), First(row)), True()), Eq(Call("rowOK", row), True()))), ("row", Seq(Nat(), Times, Sp, Nat())))))),
