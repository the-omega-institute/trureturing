...
internal sealed class ZeroForcingThreeFortData5Document : IScribeDocumentDefinition
{
    private const string Prefix = "D5/S3/Combinatorics/GeneralizedPetersen/ZeroForcingThreeFortData5.";
    ...
    public DocumentDefinition Create()
    {
        ...
        return DocumentDefinition.Create(ScribeNode.Create(
            ...
            H("Forts for the innerNext Anchor"),
            Blocks(
            Node("certified-family", "A complete ordered fort family", "certifiedFamily",
                Disp(Ex("rows", List(Seq(Nat(), Times, Sp, Nat())), And(Eq(Call("familyOrderOK", rows), True()), Q(Imp(Mem(row, rows), And(Eq(Call("candidateShapeOK", Name("innerNext"), First(row)), True()), Eq(Call("rowOK", row), True()))), ("row", Seq(Nat(), Times, Sp, Nat())))))),
