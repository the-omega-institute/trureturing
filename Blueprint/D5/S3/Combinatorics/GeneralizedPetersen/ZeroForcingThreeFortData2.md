# Forts for the outerNext Anchor
...
$$\exists rows \in \operatorname{List}\left(\mathrm{Nat}\times \mathrm{Nat}\right),\; \operatorname{familyOrderOK}\left(rows\right) = \mathrm{true} \land \left(\forall row \in \mathrm{Nat}\times \mathrm{Nat},\; row \in rows \Rightarrow \left(\operatorname{candidateShapeOK}\left(\mathrm{outerNext}, \operatorname{fst}\left(row\right)\right) = \mathrm{true} \land \operatorname{rowOK}\left(row\right) = \mathrm{true}\right)\right)$$

*Proof.* Machine-checked in Lean as `D5/S3/Combinatorics/GeneralizedPetersen/ZeroForcingThreeFortData2.certifiedFamily` (`✓ std3`). ∎
...
- Truth anchor: `D5/S3/Combinatorics/GeneralizedPetersen/ZeroForcingThreeFortData2.certifiedFamily`
