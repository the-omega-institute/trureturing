...
internal sealed class LehmerTotientDichotomyDocument : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Describe.Lean(
                ...
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "The frozen SieveCoefficients theorem proves the displayed product identity "
                        + "from squarefreeness alone, so the nonzero assumption is retained only "
                        + "to mirror the source obligation."))),
                DescribeRole.Theorem),
            Describe.Lean(
    ...
    private static Formula Product(Formula value) => Seq(
        new Formula.Subscript(Prod, Seq(P, Sp, InMacro, Sp, PrimeFactors(value))), Sp,
        Parenthesized(Pred(P)));
    ...
    private static Formula DichotomyFormula() => Disp(Seq(
        ...
        Call("Prime", N), Sp, Lor, Sp, Parenthesized(CompositePackage(N)), Dot,
