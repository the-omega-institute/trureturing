...
**Theorem 1.11 (The singleton ring has no nonzero recovery base).**

$$\neg \left(\exists x0 \in \mathrm{ZMod}\left(1\right),\; \mathit{x0} \ne 0\right)$$

*Proof.* Machine-checked in Lean as `D5/S3/Factorization/IdealClassGroups/IdealIdentityPrincipalityGeneratorLayers.singleton_ring_has_no_nonzero_generator` (`✓ std3`). ∎

*Source.* Repository-derived.

*Commentary.*

ZMod one audits the one-element carrier: every possible base is zero, so the positive unique-recovery theorem has no admissible base.

**Theorem 1.12 (A zero element excludes the empty carrier).**
...
- Truth anchor: `D5/S3/Factorization/IdealClassGroups/IdealIdentityPrincipalityGeneratorLayers.singleton_ring_has_no_nonzero_generator`
