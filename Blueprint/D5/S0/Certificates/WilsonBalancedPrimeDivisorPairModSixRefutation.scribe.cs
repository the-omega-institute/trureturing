    ...
    : IScribeDocumentDefinition
{
    private const string Prefix =
        "D5/S0/Certificates/WilsonBalancedPrimeDivisorPairModSixRefutation.";
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Describe.Lean(DescribeId.Create("a260310-inverse-prime-divisor-sum"),
                DeclarationHandle.Create(Prefix + "R"),
                H("The inverse prime-divisor sum"),
                StatementSource.FromAuthor(RFormula()),
                AssessedProvenance.FromLiterature(Source),
                Blocks(Paragraph(Text(
                    "For every natural n, R(n) sums n divided by each distinct prime "
                        + "divisor. The division is natural-number division; it is exact for "
                        + "every member of Nat.primeFactors(n). This is OEIS A069359."))),
                DescribeRole.Definition),
            Describe.Lean(DescribeId.Create("a260310-balanced-pair"),
                ...
                AssessedProvenance.FromLiterature(Source),
                Blocks(Paragraph(Text(
                    "A pair has its smaller member first, has equal combined S and R values, "
                        + "has unequal S values, and excludes the equality S(y) = R(y). "
                        + "These are the textual and program filters used by the entry."))),
                DescribeRole.Definition),
            ...
            Describe.Lean(DescribeId.Create("a260310-mod-six-conjecture-refuted"),
                ...
                Blocks(Paragraph(Text(
                    "The pair is (140140, 141601). Here 140140 = 2^2 * 5 * 7^2 * 11 * 13 "
                        ...
                        + "S(140140) = 38, R(140140) = 141638, S(141601) = 141601, and "
                        + "R(141601) = 1 establish the balance and filters. Minimality and "
                        + "the prime/composite alternation sentence are not claimed."))),
    ...
    private static Formula RFormula()
    {
        ...
        return Disp(new Formula.Bind(
            ...
            Naturals(),
            Equal(Call("R", n),
                SumOverPrimeFactors(p, n, new Formula.Fraction(n, p)))));
    }
        ...
        var body = Iff(
            ...
            And(
                ...
                Equal(Add(Call("S", x), Call("S", y)),
                    Add(Call("R", x), Call("R", y))),
                Relation(Call("S", x), FormulaRelationOperator.NotEqual, Call("S", y)),
                Relation(Call("S", y), FormulaRelationOperator.NotEqual, Call("R", y))));
