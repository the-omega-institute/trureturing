...
internal sealed class KnasterTarskiDocument : IScribeDocumentDefinition
{
    public DocumentDefinition Create() =>
        DocumentDefinition.Create(ScribeNode.Create("Monotone endomorphisms have extremal fixed points, separated by a three-state cycle.",
            H("Knaster-Tarski Extremal Fixed Points"),
            Blocks(
                Describe.Lean(DescribeId.Create("least-and-greatest-fixed-points"),
                    DeclarationHandle.Create("D5/S1/Dynamics/KnasterTarski.knaster_tarski_extremal_fixed_points"),
                    H("Least and greatest fixed points"),
                    StatementSource.FromAuthor(Disp(Seq(F.Id("f"), Colon, F.Id("L"), To, Sp, F.Id("L"), Esc, F.Text, Grp(F.Id("monotone")), Rightarrow, Sp, Mu, Eq, Operatorname, Grp(F.Id("lfp")), Open, F.Id("f"), Close, Eq, Min, Operatorname, Grp(F.Id("Fix")), Open, F.Id("f"), Close, Comma, Esc, Nu, Eq, Operatorname, Grp(F.Id("gfp")), Open, F.Id("f"), Close, Eq, Max, Operatorname, Grp(F.Id("Fix")), Open, F.Id("f"), Close, Dot))),
                ...
                Describe.Lean(DescribeId.Create("three-state-successor-cycle"),
                    DeclarationHandle.Create("D5/S1/Dynamics/KnasterTarski.three_cycle_extremal_fixed_points"),
                    H("Three-state successor cycle"),
                    StatementSource.FromAuthor(Disp(Seq(F.Id("F"), Open, F.Id("X"), Close, Eq, OpenBrace, F.Id("s"), Mid, Operatorname, Grp(F.Id("succ")), Open, F.Id("s"), Close, InMacro, Sp, F.Id("X"), CloseBrace, Rightarrow, Sp, Operatorname, Grp(F.Id("lfp")), Open, F.Id("F"), Close, Eq, Varnothing, Comma, Esc, Operatorname, Grp(F.Id("gfp")), Open, F.Id("F"), Close, Eq, F.Id("S"), Dot))),
                    AssessedProvenance.FromRepo(),
                    Blocks(
                                            Paragraph(Text(
