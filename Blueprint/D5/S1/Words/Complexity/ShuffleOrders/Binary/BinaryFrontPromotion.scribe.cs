    ...
    private static DocumentBlock.Describe D(string id, string declaration, string title,
        ...
        Describe.Lean(DescribeId.Create(id), DeclarationHandle.Create(Module + "." + declaration),
            H(title), StatementSource.FromAuthor(Disp(Seq(Forall, Sp,
                F.Id("first"), Comma, F.Id("second"), Comma,
                F.Id("schedule"), Comma, F.Id("word"), Comma, Sp,
                F.Id("second"), Leq, Sp, F.Id("first"), Land,
                Call("ValidTwoSchedule", F.Id("first"), F.Id("second"), F.Id("schedule")),
                Land, Equal(Call("headOptional", F.Id("schedule")),
                    Call("some", F.Id("secondSide"))),
                Land, Equal(Call("evaluateTwo", F.Id("first"), F.Id("second"),
                    F.Id("schedule")), Call("some", F.Id("word"))), Rightarrow,
                Exists, Sp, F.Id("promotedSchedule"), Comma, F.Id("promotedWord"),
                Comma, Sp, Call("ValidTwoSchedule", F.Id("first"), F.Id("second"),
                    F.Id("promotedSchedule")), Land,
                Equal(Call("headOptional", F.Id("promotedSchedule")),
                    Call("some", F.Id("firstSide"))),
                Land, Equal(Call("evaluateTwo", F.Id("first"), F.Id("second"),
                    F.Id("promotedSchedule")), Call("some", F.Id("promotedWord"))),
                Land, Sp, F.Id("word"), Leq, Sp, F.Id("promotedWord")))),
