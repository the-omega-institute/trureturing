...
$$\begin{aligned}\operatorname{runCompress}(\operatorname{nil}()) = \operatorname{nil}(),\\\forall a\in\operatorname{Bool}, \operatorname{runCompress}(\operatorname{cons}(a, \operatorname{nil}())) = \operatorname{cons}(a, \operatorname{nil}()),\\\forall a, b\in\operatorname{Bool}, \forall word\in List(\operatorname{Bool}), \operatorname{runCompress}(\operatorname{cons}(a, \operatorname{cons}(b, word))) = \operatorname{if}(a = b, \operatorname{runCompress}(\operatorname{cons}(b, word)), \operatorname{cons}(a, \operatorname{runCompress}(\operatorname{cons}(b, word)))).\end{aligned}$$
...
Here nil and cons denote the empty list and the list constructor. Equal adjacent letters are collapsed recursively; when they differ the first letter is retained. The singleton and empty cases are explicit.
...
**Theorem 1.13 (Power of two plus one has complexity three).**
