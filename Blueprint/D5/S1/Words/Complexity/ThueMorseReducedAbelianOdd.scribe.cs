...
internal sealed class ThueMorseReducedAbelianOddDocument : IScribeDocumentDefinition
{
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Describe.Lean(
                ...
                DeclarationHandle.Create(
                    "D5/S1/Words/Complexity/ThueMorseReducedAbelianOdd.factor"),
                H("The literal factor at a natural start"),
                StatementSource.FromAuthor(FactorFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "List.range lists the indices from zero through length minus one in order. "
                        + "Mapping each index to thueMorse(start+i) gives the actual factor, "
                        + "including the empty list when length is zero."))),
                DescribeRole.Definition),
            Describe.Lean(
                DescribeId.Create("literal-run-compression"),
                DeclarationHandle.Create(
                    "D5/S1/Words/Complexity/ThueMorseReducedAbelianOdd.runCompress"),
                H("Collapse every maximal constant run"),
                StatementSource.FromAuthor(RunCompressFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "Here nil and cons denote the empty list and the list constructor. Equal "
                        + "adjacent letters are collapsed recursively; when they differ the "
                        + "first letter is retained. The singleton and empty cases are explicit."))),
                DescribeRole.Definition),
            Describe.Lean(
                DescribeId.Create("literal-parikh-vector"),
                DeclarationHandle.Create(
                    "D5/S1/Words/Complexity/ThueMorseReducedAbelianOdd.parikh"),
                H("Count false first and true second"),
                StatementSource.FromAuthor(ParikhFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "Both coordinates are natural numbers. List.count counts occurrences of "
                        + "the specified Boolean letter, with false in the first coordinate "
                        + "and true in the second."))),
                DescribeRole.Definition),
            Describe.Lean(
                ...
                DescribeRole.Definition),
            Describe.Lean(
                DescribeId.Create("arithmetic-vector-equals-literal-run-compression"),
                DeclarationHandle.Create(
                    "D5/S1/Words/Complexity/ThueMorseReducedAbelianOdd."
                        + "reducedParikh_eq_parikh_runCompress"),
                H("The arithmetic and literal reduced Parikh vectors agree"),
                StatementSource.FromAuthor(LiteralBridgeFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "List induction proves the alternating counts after literal run compression. "
                        + "A second induction identifies the list run count with the transition "
                        + "sum used by the arithmetic definition. The identity holds at every "
                        + "natural length and start, including length zero."))),
    ...
    private static Formula FactorFormula()
    {
        ...
        Formula i = F.Id("i");
        Formula map = Seq(i, Sp, Mapsto, Sp,
            Call("thueMorse", Seq(start, Sp, Plus, Sp, i)));
        return Disp(Seq(
            Forall, Sp, length, Comma, Sp, start, InMacro, Naturals(), Comma, Sp,
            Call("factor", length, start), Sp, Eq, Sp,
            Apply(Seq(F.Id("List"), Dot, F.Id("map")), map,
                Apply(Seq(F.Id("List"), Dot, F.Id("range")), length)), Dot));
    }

    private static Formula RunCompressFormula()
    {
        Formula a = F.Id("a");
        Formula b = F.Id("b");
        Formula word = F.Id("word");
        Formula nil = Call("nil");
        Formula tail = Call("cons", b, word);
        Formula compressedTail = Call("runCompress", tail);
        return Disp(new Formula.Aligned([
            Seq(Call("runCompress", nil), Sp, Eq, Sp, nil, Comma),
            Seq(Forall, Sp, a, InMacro, BoolType(), Comma, Sp,
                Call("runCompress", Call("cons", a, nil)), Sp, Eq, Sp,
                Call("cons", a, nil), Comma),
            Seq(Forall, Sp, a, Comma, Sp, b, InMacro, BoolType(), Comma, Sp,
                Forall, Sp, word, InMacro, Sp, ListBoolType(), Comma, Sp,
                Call("runCompress", Call("cons", a, tail)), Sp, Eq, Sp,
                Call("if", Seq(a, Sp, Eq, Sp, b), compressedTail,
                    Call("cons", a, compressedTail)), Dot)]));
    }

    private static Formula ParikhFormula()
    {
        Formula word = F.Id("word");
        Formula count = Seq(F.Id("List"), Dot, F.Id("count"));
        return Disp(Seq(
            Forall, Sp, word, InMacro, Sp, ListBoolType(), Comma, Sp,
            Call("parikh", word), Sp, Eq, Sp,
            Pair(Apply(count, F.Id("false"), word), Apply(count, F.Id("true"), word)),
            Dot));
    }

    private static Formula LiteralBridgeFormula()
    {
        Formula length = F.Id("length");
        Formula start = F.Id("start");
        return Disp(Seq(
            ...
            Call("parikh", Call("runCompress", Call("factor", length, start))), Dot));
    }
    ...
    private static Formula ListBoolType() => Apply(F.Id("List"), BoolType());
}
