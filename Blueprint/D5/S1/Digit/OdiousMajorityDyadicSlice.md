...
$$\forall m \in \mathbb{N}, \operatorname{val}(\operatorname{residue}(m)) = m \bmod 21$$
...
**Definition 1.5 (The binary population count).**

$$\forall m \in \mathbb{N}, \operatorname{popcount}(m) = \operatorname{count}(\operatorname{bits}(m), \mathrm{true})$$

*Formalization.* `D5/S1/Digit/OdiousMajorityDyadicSlice.popcount` (`✓ std3`).

*Source.* Repository-derived.

*Commentary.*

popcount(m) counts the true entries of Nat.bits(m).

**Definition 1.6 (The signed residue state).**
...
**Definition 1.7 (The eligible selector row).**
...
**Definition 1.8 (The six-bit block matrix).**
...
**Definition 1.9 (The signed eligible difference).**
...
**Definition 1.10 (The eligible odious count).**
...
**Definition 1.11 (The eligible evil count).**
...
**Theorem 1.12 (Signed residue counts obey the 21-state transfer).**
...
**Theorem 1.13 (Eligibility and the six-bit block coefficient agree exactly).**
...
**Theorem 1.14 (The exact annihilator yields a third-order recurrence).**
...
**Theorem 1.15 (Odious integers dominate for every k >= 1).**
...
The majority conjecture is Shevelev's (OEIS A229826; Int. J. Math. Math. Sci. 2008). The dyadic-slice theorem and its transfer, certificate, and sign-induction proofs here are repository-derived.
...
- Truth anchor: `D5/S1/Digit/OdiousMajorityDyadicSlice.popcount`
