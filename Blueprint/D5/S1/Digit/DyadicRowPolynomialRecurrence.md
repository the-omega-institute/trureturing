...
$$\forall n: \mathbb{N}, \operatorname{R}(2 \cdot n) = X \cdot (\operatorname{comp}(\operatorname{R}(n), X + 1) - (\operatorname{R}(n)))$$
...
For positive n this recurrence is transcribed from OEIS A373183 (Kurkov, 2024) as scope-matched context. The Lean statement is a repository strengthening to all n; the n=0 clause follows from R_zero and the definition of D. Conjectures 1 and 9 are stated there as conjectures; the proofs are this repository's.
...
$$\forall r, k, q: \mathbb{N}, (2 \leq r \land (0 < q \land q \leq r)) \implies \operatorname{e}(r, k, q) = \operatorname{e}(r - (1), k, q) + \operatorname{e}(r - (1), k, q - (1)) + \operatorname{ite}(r \leq \operatorname{wt}(k) + q, \operatorname{T}(2 \cdot k, \operatorname{wt}(k) + q + 1 - (r)), 0)$$
...
**Theorem 1.20 (Accumulator after the degree bound).**
...
**Theorem 1.21 (The Pascal accumulator identity).**
