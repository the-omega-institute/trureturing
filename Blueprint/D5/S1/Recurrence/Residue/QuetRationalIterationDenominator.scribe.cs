    ...
    : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Describe.Lean(
                ...
                Blocks(Paragraph(Text(
                    "For every m at least two, the square of den(m-1) divides the "
                        + "cube of den(m), and den(m+1) satisfies Quet's formula. "
                        + "The divisibility clause makes the natural-number quotient "
    ...
    {
        Formula m = F.Id("m");
        Formula previous = Den(Subtract(m, D(1)));
        Formula current = Den(m);
