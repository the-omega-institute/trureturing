...
internal sealed class BilateralQuarterShiftThetaModFourDocument : IScribeDocumentDefinition
{
    private const string Prefix = "D5/S1/Recurrence/Bilateral/BilateralQuarterShiftThetaModFour.";
    private const string ThetaModule = "D5/S1/Recurrence/Parity/ThetaSelfCompositionModFour";
    private static readonly LibraryNoteRef Source =
        LibraryNoteRef.Create("D5/L/Recurrence/hanna2023a363104");

    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        "The integer solution of the A363104 bilateral equation is theta_3 modulo four.",
        H("Bilateral Quarter-Shift Theta Congruence"),
        Blocks(
            Paragraph(Text("Paul D. Hanna's entry hanna2023a363104, dated May 21, 2023, "
                + "defines A by 4 = sum over integer n of (-x)^n (4*A(x)+x^(n-1))^(n+1). "
                + "It conjectures the theta_3 congruence and the square classification "
            ...
            Paragraph(Text("The imported thetaSeries and its coefficient formula are from "),
                ...
                Text(". Its constant coefficient is one, and each positive square "
                    + "coefficient is two; all other coefficients are zero.")),
            ...
            Node("cancellation_identity", "Cancellation over the integers", CancellationFormula(),
                ...
                + "A times the positive-square support series. Multiplication by eight "
                + "removes the alternating signs modulo sixteen. Hence the difference "
                + "between the remainder and 4*A*(thetaSeries-1) is 16*Q with integral "
                + "Q. The generating equation gives 4=4*(A*thetaSeries+4*Q); cancelling "
            ...
            Node("mod_four_identity", "The series congruence", ModFourFormula(),
                "The reduction of thetaSeries is 1+2*S, where S indicates the positive "
                + "squares. Its square is one over ZMod(4). Reducing the integer "
                + "cancellation identity and multiplying by this self-inverse series "
                + "identifies the reduction of generatingSeries."),
            Node("hanna_conjecture", "Hanna's A363104 conjecture", ConjectureFormula(),
                "At a positive index, the imported theta coefficient is two exactly "
                + "at squares and zero otherwise. Coefficient comparison and the "
                + "integer-cast remainder equivalence prove both biconditionals in "
                + "hanna2023a363104.", DescribeRole.Theorem,
                AssessedProvenance.FromLiterature(Source),
                new OpenProblemResolutionClaim(
                    ProblemSlugRef.Create("oeis-a363104-bilateral-quarter-shift-theta-mod-four"),
    ...
    private static DocumentBlock Node(string name, string title, Formula formula, string prose,
        ...
        Describe.Lean(DescribeId.Create("a363104-" + name.Replace('_', '-').ToLowerInvariant()),
    ...
    private static Formula TermFormula()
    {
        ...
        Formula positive = Mul(Mul(Sign(Add(m, D(1))), Power(X(), Add(m, D(1)))),
            Power(Add(Mul(D(4), b), Power(X(), m)), Add(m, D(2))));
        Formula inverse = Call("invOfUnit",
            Add(D(1), Mul(Mul(D(4), b), Power(X(), Add(m, D(3))))), D(1));
        Formula negative = Mul(Mul(Sign(Add(m, D(2))),
            Power(X(), Add(Add(Power(m, D(2)), Mul(D(3), m)), D(1)))),
