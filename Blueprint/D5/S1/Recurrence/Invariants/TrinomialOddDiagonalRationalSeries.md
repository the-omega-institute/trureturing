...
**Definition 1.2 (The odd diagonal sum).**

$$\forall n \in \mathbb{N}, \operatorname{diagonal}\left(n\right) = \sum_{j \in \operatorname{range}\left(\operatorname{div}\left(n, 2\right) + 1\right)} (\operatorname{trinomial}\left(n + 1 - j, 2 \cdot j + 1\right))$$

*Formalization.* `D5/S1/Recurrence/Invariants/TrinomialOddDiagonalRationalSeries.diagonal` (`✓ std3`).

*Source.* Repository-derived.

*Commentary.*

For each natural n, sum the odd-indexed entries along the reflected diagonal through row n+1. The bound n/2 is natural integer division.

**Definition 1.3 (The rational generating series).**

$$\operatorname{generatingSeries} = \operatorname{inv}\left(((1 - X) \cdot (1 - X - 2 \cdot (X)^{2} - (X)^{3}))\right)$$
...
**Theorem 1.5 (The odd coefficient identity).**
...
**Theorem 1.6 (Schulte's A077864 conjecture).**
...
- Truth anchor: `D5/S1/Recurrence/Invariants/TrinomialOddDiagonalRationalSeries.diagonal`
