...
internal sealed class RestrictedGrowthLabelOccurrencesDocument : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Node("words", "Restricted-growth words", WordsFormula(),
                "This reverse-chronological generator starts from the empty word. Each step "
                + "prepends every label in the inclusive interval from 1 to one above the "
                + "previous maximum. Its membership is identified with the source predicate "
                + "by mem_words_iff.",
                DescribeRole.Definition, AssessedProvenance.FromLiterature(Source)),
            Node("IsRestrictedGrowth", "Chronological restricted-growth predicate",
                IsRestrictedGrowthFormula(),
                "The empty list is admitted. A nonempty chronological list begins with 1, "
                + "and every indexed label is positive and at most one plus the maximum of "
                + "the preceding prefix.", DescribeRole.Definition,
                AssessedProvenance.FromLiterature(Source)),
            Node("mem_words_iff", "Generator membership equals the source predicate",
                MemWordsIffFormula(),
                "A word belongs to the reverse-chronological generator at length n exactly "
                + "when it has length n and its chronological reversal satisfies the "
                + "restricted-growth predicate.", DescribeRole.Theorem,
    ...
    private static Formula IsRestrictedGrowthFormula()
    {
        var f = F.Id("f");
        var j = F.Id("j");
        var fj = new Formula.Subscript(f, j);
        var length = new Formula.Absolute(f);
        var bounds = And(
            LessOrEqual(D(1), fj),
            LessOrEqual(fj, Add(D(1),
                Call("foldr", F.Id("max"), D(0), Call("take", j, f)))));
        var indexedBounds = Universal(j, Naturals(),
            ImpliesTo(Less(j, length), Parenthesized(bounds)));
        var nonempty = And(Equal(Call("head", f), D(1)), indexedBounds);
        var predicate = IffTo(Call("IsRestrictedGrowth", f), Parenthesized(
            Or(Equal(f, EmptyList()), Parenthesized(nonempty))));
        return Disp(new Formula.Aligned([
            Seq(F.Id("IsRestrictedGrowth"), Colon, Sp, Lists(), Sp, To, Sp, F.Id("Prop")),
            Universal(f, Lists(), predicate)
        ]));
    }

    private static Formula MemWordsIffFormula()
    {
        var n = F.Id("n");
        var w = F.Id("w");
        var characterization = And(
            Equal(new Formula.Absolute(w), n),
            Call("IsRestrictedGrowth", Call("reverse", w)));
        return Disp(Universal(n, Naturals(), Universal(w, Lists(),
            IffTo(Member(w, Words(n)), Parenthesized(characterization)))));
    }
