...
internal sealed class TrinomialOddDiagonalRationalSeriesDocument : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Node("diagonal", "The odd diagonal sum", DiagonalFormula(),
                "For each natural n, sum the odd-indexed entries along the reflected "
                    + "diagonal through row n+1. The bound n/2 is natural integer division.",
    ...
    private static Formula DiagonalFormula() => Disp(Seq(
        Bound("n", Naturals()), Equal(Call("diagonal", N()), Sum(N()))));

    private static Formula GeneratingSeriesFormula() => Disp(Equal(
        Named("generatingSeries"),
        Call("inv", Parenthesized(Multiply(
            Parenthesized(Subtract(D(1), X())),
            Parenthesized(Subtract(
