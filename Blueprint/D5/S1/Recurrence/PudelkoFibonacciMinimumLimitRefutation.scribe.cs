    ...
    : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            Node(
                "bilateral-fibonacci-predicate",
                "The bilateral Fibonacci recurrence",
                "IsBilateralFibonacci",
                IsBilateralFibonacciFormula(),
                "A sequence belongs to the bilateral Fibonacci recurrence when its values at "
                    + "indices zero and one are x and y and the Fibonacci recurrence holds at "
                    + "every integer index. Thus both negative and positive indices are part of "
                    + "the same recurrence.",
            ...
            Node(
                ...
                "The predicate allows ties: position zero need only be one global minimizer "
                    + "of the absolute values for every sequence satisfying the bilateral "
                    + "recurrence. The index quantifier ranges over all integers rather than a "
    ...
    private static Formula IsBilateralFibonacciFormula()
    {
        var x = F.Id("x");
        var y = F.Id("y");
        var a = F.Id("a");
        var k = F.Id("k");
        var recurrence = ForAll(
            [Bound("k", Integers())],
            Equal(
                Apply(a, Add(k, D(2))),
                Add(Apply(a, Add(k, D(1))), Apply(a, k))));
        var condition = And(
            Equal(Apply(a, D(0)), x),
            Equal(Apply(a, D(1)), y),
            recurrence);
        return Disp(ForAll(
            [Bound("x", Integers()), Bound("y", Integers()),
                Bound("a", Seq(Integers(), Sp, To, Sp, Integers()))],
            Iff(Call("IsBilateralFibonacci", x, y, a), Parenthesized(condition))));
    }
    ...
    private static Formula MinimumAtZeroFormula()
    {
        ...
        var minimum = ForAll(
            [Bound("k", Integers())],
            AtMost(
                new Formula.Absolute(Apply(a, D(0))),
                new Formula.Absolute(Apply(a, k))));
        var condition = ForAll(
            [Bound("a", Seq(Integers(), Sp, To, Sp, Integers()))],
            Implies(
                Call("IsBilateralFibonacci", x, y, a),
                Parenthesized(minimum)));
        return Disp(ForAll(
            [Bound("x", Integers()), Bound("y", Integers())],
