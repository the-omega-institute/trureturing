...
triage: theorem
motivation_gids:
  - D5/S3/Combinatorics/Parking/CircularTwoChoiceParkingBijection.result
---
...
- Formal endpoint:
  `D5/S3/Combinatorics/Parking/CircularTwoChoiceParkingBijection.result`, whose statement
  certifies the bijectivity of the explicit `fixedFiberEquiv` construction.
- Operational support: `D5/S3/Combinatorics/Parking/OperationalDynamics` owns
  literal `actualStep`, prefix freshness, unique vacancy, and rotation laws.
- Fiber transport: `D5/S3/Combinatorics/Parking/FixedIncrementFiberTransport`
  owns increment encode/decode and `fixedFiberOneChoiceEquiv`.
- Cut-run support: `D5/S3/Combinatorics/Parking/CutRunCorrespondence` owns
  `firstFree_cut`, `firstFree_ne_vacancy`, and `cut_run`.
- Classical bridge: `oneChoiceClassicalEquiv` remains in the endpoint module and
  is built against the frozen `UnitIntervalParkingFoata` supplier without
  changing that supplier.
