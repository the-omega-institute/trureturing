...
have the smallest super condensed neighborhoods among words of the same
length over a fixed alphabet. The quantified super condensed assertion says
that for every finite alphabet, lengths `n,d`, letter `a`, and word `w` of
length `n`, the cardinality of `SCN(a^n,d)` is at most that of `SCN(w,d)`.
