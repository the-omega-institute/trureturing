---
slug: oeis-a300734-diagonal-index-divisibility
...
# A300734: square-index divisibility

## Problem

Quoted directly from the seat-local file `oeis-A300734.src`
...
The formal object has `a(0)=a(1)=1` and exponent `e(n)=2*n^2`.
...
For this entry specialize `e(n)=2*n^2` and `k=2`.
...
The formal coefficient is `a (fun m => 2 * m ^ 2) n`.
...
- Entry-specific theorem and Scribe claim: `hanna_conjecture_a300734`.
- Exact conclusion for `n : ℕ`, `hn : 1 ≤ n`: `(n : ℤ) ^ 2 ∣ a (fun m => 2 * m ^ 2) n`.
...
`theorem`. The claim for this slug is attached only to `hanna_conjecture_a300734`
