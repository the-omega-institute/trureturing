...
### GDD. Divided-difference integral elements and the exact denominator threshold

#### GDD.1 A quadratic certificate without local-field hypotheses

**Definition.** In a commutative ring, for n>=0 define

$$
S_n(X,a)=\sum_{i=0}^{n-1}X^i a^{n-1-i},\qquad
T_n(X,a)=\sum_{i=0}^{n-1}a^{n-1-i}S_i(X,a).
$$

Both are polynomials; an empty sum is zero. Their definitions do not divide by X-a.

**Theorem GDD1.** Suppose R is a characteristic-zero domain embedded in a field L, n>=1, a,b in R and x in L satisfy x^n=a^n+n^2b. Then y=S_n(x,a)/n satisfies

$$
\boxed{(x-a)y=nb,\qquad
 y^2=a^{n-1}y+bT_n(x,a).}\tag{GDD1}
$$

Moreover y is integral over R. These statements include b=0 and x=a.

**Proof.** Difference of powers and summation give

$$
(x-a)S_n(x,a)=x^n-a^n,\qquad
(x-a)T_n(x,a)=S_n(x,a)-na^{n-1}.
$$

For the second identity, expand each summand as
 a^(n-1-i)(x^i-a^i), and use S_n(a,a)=na^(n-1).
The first identity and the defining equation give (x-a)y=nb, cancelling only the nonzero integer n. Multiply the second identity by y, substitute S_n=ny and (x-a)y=nb, and cancel n again. This proves the quadratic equation. The element x is integral over R because x^n is in R. Hence T_n(x,a) is integral over R. The displayed quadratic is monic over R[x], so integrality transitivity proves y integral over R. No division by x-a or b occurs. The quadratic coefficients contain x in general; no bound of two on the degree of y over the fraction field of R is asserted.

**Corollary GDD2.** In the original golden ring O=Z[phi], if n>=1, theta^n=phi and phi=a^n+n^2b with a,b in O, then

$$\gamma_n(a)=\frac1n\sum_{i=0}^{n-1}\theta^i a^{n-1-i}$$

is an algebraic integer and satisfies GDD1. This remains valid under any ring embedding of O into a characteristic-zero field.

**Proof.** Every element A+B*phi of O is integral over Z, since phi satisfies the monic equation X^2-X-1. Apply GDD1 and integrality transitivity to Z subset O subset L.

#### GDD.2 An exact integrality criterion over an unramified base

**Definition.** For n>=1, rad(n) is the product of its distinct rational prime divisors.

**Theorem GDD3.** Let K be a number field that is unramified at every rational prime dividing n>=2. Suppose a,u belong to O_K and X^n-u is irreducible over K. Put theta^n=u, D=u-a^n, and gamma=S_n(theta,a)/n. Then

$$
\boxed{\gamma\in O_{K(\theta)}
\quad\Longleftrightarrow\quad
n\operatorname{rad}(n)\mid D\text{ in }O_K.}\tag{GDD3}
$$

**Proof of necessity.** Irreducibility and n>=2 imply D!=0. Since (theta-a)S_n(theta,a)=D,

$$
N_{K(\theta)/K}(\gamma)
=(-1)^{n-1}\frac{D^{n-1}}{n^n}.\tag{GDD4}
$$

The sign follows from N(theta-a)=(-1)^(n-1)D and must be kept for even n. If gamma is integral, its relative norm is integral. Fix a prime P of K over p|n, and let e=v_p(n). Normalize v_P to have value group Z. Unramifiedness gives v_P(n)=e. Thus

$$
(n-1)v_P(D)\ge ne.
$$

Since 1<=e<=n-1, the least integral value allowed is e+1. This holds at every prime over every p|n, exactly giving divisibility by n*rad(n).

**Proof of sufficiency.** Consider the monic polynomial over K

$$
P(Y)=Y^n-\sum_{k=1}^n
 \frac{\binom nk a^{n-k}D^{k-1}}{n^k}Y^{n-k}.\tag{GDD5}
$$

The equality theta*gamma=a*gamma+D/n, raised to its nth power, gives P(gamma)=0 after cancelling nonzero D. Every coefficient is integral at primes not dividing n. At a prime P over p|n, write e=v_p(n). The identity k*binom(n,k)=n*binom(n-1,k-1) gives

$$v_p\binom nk\ge e-v_p(k).$$

Using v_P(D)>=e+1 and v_P(a)>=0, the kth coefficient of the displayed sum has valuation at least

$$
 e-v_p(k)+(k-1)(e+1)-ke=k-1-v_p(k)\ge0.
$$

Thus all coefficients are integral at every finite prime and belong to O_K. The monic equation proves gamma integral. This argument also covers p=2 when the base is unramified there. It does not assert necessity over ramified bases.

The precision scale n*rad(n) already occurs in integral-basis literature. K.-H. Nguyen-Dang, *The minimal periodicity for integral bases of pure number fields*, arXiv:2509.09457v2, January 8, 2026, Section 1 and Theorem 2.1, discusses this scale through the earlier bases of Jakhar-Khanduja-Sangwan. GDD3 concerns the single specified element over the displayed relative base, and is proved by GDD4-GDD5 without assuming an integral-basis periodicity theorem. Source: https://arxiv.org/html/2509.09457v2 .

**Proposition.** The n^2 divisibility assumption in GDD1 is sufficient but is not necessary in GDD3. It agrees with the sharp modulus exactly when n is squarefree.

**Proof.** The equality n*rad(n)=n^2 is equivalent to squarefreeness. For a concrete strict example, X^9-28 is Eisenstein at seven. With a=1, D=27 is divisible by 9*rad(9)=27 and is not divisible by 81. GDD3 proves that (1+theta+...+theta^8)/9 is integral. Its monic equation is

$$
Y^9-Y^8-12Y^7-84Y^6-378Y^5-1134Y^4
-2268Y^3-2916Y^2-2187Y-729=0.
$$

In contrast, X^9-10 is Eisenstein at two, and the corresponding element with a=1 has norm 1/9 by GDD4, so it is not integral. These rational-binomial examples assert nothing about an actual golden WSS depth.

#### GDD.3 Explicit higher-depth elements in the original golden tower

**Definition.** Fix p>5. Let sigma_p be the identity on K=Q(sqrt5) if (5/p)=1, and golden conjugation otherwise. For e>=1 put

$$
n=p^e,\qquad a_{p,e}=\sigma_p^e(\phi),\qquad
\theta_{p,e}^{p^e}=\phi,\qquad
\Gamma_{p,e}=\frac1{p^e}S_{p^e}(\theta_{p,e},a_{p,e}).
$$

Choose the positive real root theta_(p,e). Retain h_p=v_p(F_(p-(5/p))).

**Theorem GDD6.** At every prime P of K above p,

$$v_P\bigl(\phi-a_{p,e}^{p^e}\bigr)=h_p,$$

and

$$\boxed{\Gamma_{p,e}\text{ is integral}\quad\Longleftrightarrow\quad h_p\ge e+1.}\tag{GDD6}$$

**Proof.** In the split case a_(p,e)=phi. The difference, up to a unit, is phi^(p^e-1)-1. Factor p^e-1 through p-1; the quotient 1+p+...+p^(e-1) is a p-adic unit, so the valuation is the depth in RNI2. In the inert case with e even, use a_(p,e)=phi and factor p^e-1 through p^2-1, again with quotient prime to p. For inert p and odd e, a_(p,e)=psi, and the difference is a unit times 1+phi^(p^e+1). The ratio (p^e+1)/(p+1) is odd and prime to p. Factoring the odd power plus one preserves the valuation of 1+phi^(p+1), which is h_p by RNI2-RNI3. These facts hold at every completion. Now apply GDD3: the base is unramified at p, X^(p^e)-phi is irreducible by RNI1, and the sharp modulus is p^(e+1).

**Proposition.** For e=1, Gamma_(p,1) differs from eta_p in RNI8 by an element of O[theta_p]. Consequently it supplies the same exceptional integral basis and the same O/pO missing lattice when q_p=0.

**Proof.** In any characteristic-p polynomial ring,

$$S_p(X,a)=(X-a)^{p-1}.$$

Indeed multiplication by the monic polynomial X-a gives X^p-a^p=(X-a)^p, and monic multiplication is injective. Hence the integer polynomial S_p(X,a)-(X-a)^(p-1) has coefficients divisible by p. Evaluate at theta_p and divide by p. The resulting difference belongs to O[theta_p]. The replacement of one candidate by the other therefore preserves the generated O-lattice; apply RNI8-RNI9.

**Proposition.** GDD6 constructs a nested family of exact depth tests but does not prove any new prime belongs to the initial-quotient zero set. In particular Gamma_(p,1) is integral exactly on the original WSS set; the proof retains that arithmetic condition.

**Proof.** The e=1 equivalence is h_p>=2, which is precisely q_p=0. All higher equivalences retain the same actual h_p and only change the threshold. No step establishes integrality of one of these golden elements independently of its depth condition. The tests also do not identify the class-group exponent c_p of RCR7 with h_p-1.
