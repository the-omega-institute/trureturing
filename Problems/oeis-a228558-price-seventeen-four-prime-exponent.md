---
slug: oeis-a228558-price-seventeen-four-prime-exponent
...
motivation_gids:
  ...
  - D5/S3/Arith/Congruence/PricePrimeQuotientExponent.result_a228558
---

# Price's A228558 prime-exponent conjecture

## Problem

OEIS A228558, NAME (`%N`, verbatim):
...
`forall k : Nat, 21 | 17^k + 4^k -> Prime((17^k + 4^k)/21) -> Prime(k)`.
...
hypotheses, through a general result that also applies to A231329.
...
A natural `k` for which 21 divides `17^k+4^k`, the exact quotient is prime,
and `k` is not prime would contradict `result_a228558`.
...
- For `k<400`, the exponents producing prime exact quotients are
  `13, 61, 67, 107, 383`, all prime.
- The bounded scan carries no proof of the universal statement.

## Triage

`theorem`. Price's A228558 comment is proved for every natural exponent under
