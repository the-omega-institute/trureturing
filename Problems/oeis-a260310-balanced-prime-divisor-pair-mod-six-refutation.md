...
`S(n) = ∑ p ∈ n.primeFactors, p` and
`R(n) = ∑ p ∈ n.primeFactors, n / p`, where division in `R` is natural-number
division and is exact because each indexed prime divides `n`. The predicate
`IsPair(x, y)` means
...
`140140 = 2^2 * 5 * 7^2 * 11 * 13` and derives
...
- Lean module:
  `D5/S0/Certificates/WilsonBalancedPrimeDivisorPairModSixRefutation.lean`.
- Main theorem: `result : ¬ claim`, with exactly the std3 axioms
  `propext`, `Classical.choice`, and `Quot.sound`.
- The profiled Lean process completed in 8.74 seconds, with cumulative kernel
  type-checking time 7.85 milliseconds and maximum resident set size
  1,510,227,968 bytes.
- The orchestrator independently obtained `S(140140) = 38`,
  `R(140140) = 141638`, `S(141601) = 141601`, and `R(141601) = 1`, so both
  sides equal 141639, while `140140 ≡ 4 (mod 6)`.
- The orchestrator scan for composite `c ≤ 300000`, with prime partner
  `q = R(c) - S(c) + 1`, found 26,235 pairs and exactly three
