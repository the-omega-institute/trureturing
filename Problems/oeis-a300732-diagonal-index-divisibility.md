---
slug: oeis-a300732-diagonal-index-divisibility
...
# A300732: index divisibility

## Problem

Quoted directly from the seat-local file `oeis-A300732.src`
...
The formal object has `a(0)=a(1)=1` and exponent `e(n)=2*n`.
...
For this entry specialize `e(n)=2*n` and `k=1`.
...
The formal coefficient is `a (fun m => 2 * m) n`.
...
- Entry-specific theorem and Scribe claim: `hanna_conjecture_a300732`.
- Exact conclusion for `n : ℕ`, `hn : 1 ≤ n`: `(n : ℤ) ∣ a (fun m => 2 * m) n`.
...
that this entry's published DATA is reproduced exactly (13 terms), with
...
`theorem`. The claim for this slug is attached only to `hanna_conjecture_a300732`
