---
slug: florez-junes-ramirez-cubic-lattice-xz-plane-path-count-refutation
bibkey: florez2018cubiclattice
doi: null
url: https://cs.uwaterloo.ca/journals/JIS/VOL21/Florez/florez4.pdf
triage: theorem
motivation_gids:
  - D5/S0/Certificates/FlorezCubicLatticePlanePathCountRefutation.result1
---

# Flórez-Junes-Ramírez xz-plane path-count refutation
...
The formal `claim1` retains the
universal quantifier, lower bound, printed path predicate, plane-containment
condition, and binomial sum. The theorem `result1` proves its negation.
...
containment in the xz-plane requires the second coordinate of every vertex to
be zero; the initial vertex already is the origin.

Under that printed reading, the xz-plane counts for `k = 1, 2, 3` are
...
and assigns it to the xz-plane count. Direct enumeration from the printed
...
`C_3^+(3)` and xz-plane predicates gives `xzCount 3 = 14`; evaluating the
printed binomial sum gives `formula 3 = 36`. Instantiating `claim1` at `k = 3`
therefore requires `14 = 36`, a contradiction.

## Falsifier

The refutation would fail if the formal path space omitted a signed coordinate
step, if the prefix, final-zero, or xz-plane condition differed from the
...
- Result declaration identity:
  `sha256:7deaf40d91dd570275e135d6849fa5064679e1b9848b0f6c2e6e592baea4dfbe`.
...
`certified-instance` with a typed `refutes` edge from `result1` to `claim1`.
