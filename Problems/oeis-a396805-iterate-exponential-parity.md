---
slug: oeis-a396805-iterate-exponential-parity
...
# Parity of the A396805 iterated-exponential coefficients
...
```text
%N A396805 E.g.f. satisfies A(x) = x*exp( A^5(x) ).
%C A396805 Conjecture: a(n) is odd iff n is odd for n >= 1.
%C A396805 Conjecture: a(n) == [0,1,0] repeating (mod 3) for n >= 3.
%C A396805 Conjecture: a(n) == [1,2,3,4,0] repeating (mod 5) for n >= 1.
%A A396805 Paul D. Hanna, Jun 09 2026
```

In this entry `A^5(x)` denotes the 5-fold COMPOSITIONAL ITERATE of `A`, not
...
together with its siblings A396803, A396806.
...
5. Induction over the approximation stages carries the congruence to the fixed
   point, giving `parity_mod_two` and then `odd_iff_odd`; `parity_iterate_five` is its
   specialization at k = 5.

## Falsifier

An index `n ≥ 1` with `aK 5 n` odd while `n` is even, or even while `n` is odd,
...
- Resolution theorem: `parity_iterate_five`, a specialization of `odd_iff_odd`.
