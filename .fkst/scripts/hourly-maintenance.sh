...
# shellcheck source=.fkst/scripts/maintenance-restart.sh
...
write_platform_pin_revision() {
  local target_revision="$1" current_revision stamp temporary_workspace
  ...
  temporary_workspace="$FKST_HOST_ROOT/fkst.workspace.toml.next-$stamp-$$-$RANDOM"
  ...
  if ! sed "s/$current_revision/$target_revision/g" \
    ...
    if ! rm -f -- "$temporary_workspace"; then
      say "PLATFORM-PIN-TEMP-CLEANUP-FAIL: $temporary_workspace"
    fi
    return 1
  fi
}
...
sync_checkout() {
  ...
  # real host: `.metadata_never_index`, an intentional Spotlight-exclusion marker that must stay,
  # blocked it on its own. The checkout sat 24 commits behind with no path forward.
  if ! checkout_status="$(
