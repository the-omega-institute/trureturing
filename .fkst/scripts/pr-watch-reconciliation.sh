...
reconcile_pr_watch() {
  ...
  if [[ ! -e "$state" ]]; then
    if [[ -e "$state.lock" || -L "$state.lock" ]]; then
      say "PR-WATCH IDENTITY UNKNOWN: state missing with ownership lease path=$state.lock"
      return 1
    fi
  ...
  if ! kill -0 "$PR_WATCH_STATE_PID" 2>/dev/null; then
    say "PR-WATCH INACTIVE: stale state retained for owner recovery pid=$PR_WATCH_STATE_PID"
    return 0
  fi
