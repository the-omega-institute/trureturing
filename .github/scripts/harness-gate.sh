...
if [[ -z "$JUDGE_DLL" ]]; then
  # Building the candidate judge also prepares the compile assets that its Roslyn
  # analysis consumes. External-judge callers own that precondition and must not make
  # this gate restore or build the candidate tree as a side effect.
