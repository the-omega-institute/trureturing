...
assert text.count(old_holonomy) == 1
text = text.replace(old_holonomy, new_holonomy, 1)
...
\operatorname{Hol}_q(w,x).
$$'''
assert text.count(old_detection) == 1
text = text.replace(old_detection, new_detection, 1)

old_joint_kernel_tail = r'''加入新坐标只会缩小联合 kernel；若新坐标分离了旧联合 kernel 中的一对状态，则该精化严格。'''
...
assert text.count(old_joint_kernel_tail) == 1
text = text.replace(old_joint_kernel_tail, new_joint_kernel_tail, 1)
...
assert text.count(old_advance) == 1
