...
# 已从 required 路径退役(用户 2026-08-08 裁决)。确定性是「生成程序」的性质,
# 生成程序本身受 harness;在未触碰 emitter 的 PR 上重跑真重建,是重新验证一个
# 没变的东西——第 5 条(门只设在会说谎处)与第 20 条(预防贵、检测廉)。
# 本目标保留为即时报告,仅因 base 侧 dev workflow 仍按旧名调用它;dev 更新后
# 由紧接的后续 PR 删除(禁兼容垫层,不留 grandfather)。
# 重活改由显式的 refactor-pr-a-audit 承担,按需/canary 调用。
refactor-pr-a-verify:
	@printf '{"schema":"refactor-pr-a-verify-v2","lane":"retired-from-required","reason":"determinism is a property of the generator program, which is itself harnessed; per-PR re-proof was over-gating","on_demand_target":"refactor-pr-a-audit","pass":true}\n' > "$(OUT)" 2>/dev/null || true
