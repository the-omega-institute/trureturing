...
worktree:
	@$(WORKTREE_DEST_CHECK) /bin/bash tools/scripts/worktree-init.sh "$(NAME)" "$(WORKTREE_DEST)" "$(BASE)"

worktree-hold:
	@$(WORKTREE_DEST_CHECK) dotnet run --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- worktree hold --path "$(WORKTREE_DEST)" $(if $(strip $(REASON)),--reason "$(REASON)",)

worktree-release:
	@$(WORKTREE_DEST_CHECK) dotnet run --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- worktree release --path "$(WORKTREE_DEST)"
