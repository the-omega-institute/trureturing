...
pr-open:
	@/bin/bash tools/scripts/pr.sh open --head "$(HEAD)" --title "$(TITLE)" $(if $(BODY),--body-file "$(BODY)",) $(if $(WATCH_TIMEOUT_SECONDS),--timeout-seconds "$(WATCH_TIMEOUT_SECONDS)",) $(if $(WATCH_INTERVAL_SECONDS),--interval-seconds "$(WATCH_INTERVAL_SECONDS)",)
