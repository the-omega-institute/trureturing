...
.PHONY: help dotnet test lua-test lean lean-report build c0-renew clean-lanes emit emit-check ingest echo-residual-summary echo-verify record-golden selftest scratch-sweep gate perf-report worktree

help:
	@printf '%s\n' 'make help                         Show this target list' 'make dotnet                       Restore and build .NET with warnings as errors' 'make test                         Run the .NET test suite' 'make lua-test                     Run the pinned fkst Lua package gate' 'make lean                         Build the pinned Lean project' 'make lean-report                  Produce the canonical raw Lean report' 'make build                        Run make dotnet and make lean' 'make c0-renew [BASE=origin/dev]   Renew C0 ceremony projections; admission remains separate' 'make clean-lanes [BASE=origin/dev] [FORCE=1]  List reclaimable lanes (dry-run); FORCE=1 removes them' 'make emit                         Emit canonical Scribe documents, catalog, and values' 'make emit-check                   Check canonical Scribe documents, catalog, and values' 'make ingest [BASE=origin/dev]     Consume the raw Lean report and align theory receipts' 'make echo-residual-summary [BASE=origin/dev]  Emit the base-bound residual projection bytes' 'make echo-verify [FILE=f] [BASE=origin/dev]  Byte-verify the committed residual projection when affected' 'make record-golden                Record Engine diagnostics into golden TOML' 'make selftest                     Run deterministic StrataLint selftest' 'make scratch-sweep                Reclaim leaked ceremony scratch checkouts older than 24h' 'make gate [BASE=origin/dev]       Run the local CI-equivalent admission flow' 'make preflight [BASE=origin/dev]  Pre-verify BOTH required CI checks locally before pushing' 'make devloop-verify               Run the local gate wrapped in the fkst typed verdict marker (local_iteration_result v1)' 'make perf-report [RECENT=10]      Summarize the external performance ledger' 'make worktree NAME=x [BASE=origin/dev] [PATH=DIR]  Initialize an isolated worktree; .lake is copied, never symlinked' 'make pr-open HEAD=branch TITLE=t [BODY=file]  Open a PR to dev and arm auto-merge (canonical PR path)' 'make pr-watch [INTERVAL=60] [CYCLES=360]  Poll armed PRs; stale derivations -> local merge+regen, otherwise update-branch'
...
devloop-verify:
	@/bin/bash Meta/StrataLint/scripts/devloop-verify.sh
