SHELL := /bin/bash
.DEFAULT_GOAL := help

BASE ?= origin/dev
WORKTREE_DEST = $(if $(DEST),$(abspath $(DEST)),$(abspath ../trureturing-$(NAME)))
LEAN_REPORT ?= .lake/build/stratalint/raw-lean-report.json
.PHONY: help test lean-cache-ensure lean-cache-to-github-without-mathlib lean-cache-from-github-without-mathlib warm-donor lean lean-report build emit ingest align-digestion-status mathlib-reanchor echo-residual-summary digestion-readiness show-atom atom-context truth-export deliver-check deposit cover cover-batch decompose scribe-seed scribe-refresh quarantine quarantine-clear settle settle-clear worktree worktree-clean pr-open pr-watch preflight gate

help:
	@printf '%s\n' 'make help                         Show this target list' 'make test                         Run the mathematical gate (Lean, admission, Scribe, values)' 'make lean-cache-ensure            Ensure a private Lean build cache is available' 'make lean-cache-to-github-without-mathlib  Publish this tree'\''s own Lean build output to a GitHub release; dependency packages are excluded' 'make lean-cache-from-github-without-mathlib  Fetch this tree'\''s own Lean build output from a GitHub release, or fail closed; mathlib still comes from lake exe cache get' 'make warm-donor                  Update and build a clean dev checkout for optional machine-local scheduling' 'make lean                         Build the pinned Lean project' 'make lean-report                  Produce the canonical raw Lean report' 'make build                        Build the pinned Lean content' 'make emit                         Emit canonical Scribe documents, catalog, and values' 'make ingest [BASE=origin/dev]     Digest uncovered-only theory changes without a Lean report' 'make align-digestion-status [BASE=origin/dev]  Consume the Lean report and align coverage/truth status' 'make mathlib-reanchor [BASE=origin/dev]  Reanchor frozen statement identities after a pin change' 'make echo-residual-summary [BASE=origin/dev]  Emit the content-addressed residual projection bytes' 'make digestion-readiness          Print exhaustive residual-open readiness as JSON' 'make show-atom ATOM_ID=x          Print and verify one digestion atom without writing' 'make atom-context ATOM_ID=x       Print an atom and its source neighbors without writing' 'make truth-export OUT=dir [LEAN_REPORT=path]  Export immutable STRICT active frozen truth to dir/truth-export.v1.json' 'make deliver-check [BASE=origin/dev]  Run theorem delivery in canonical freeze-last order' 'make deposit ATOM_ID=x GID=g [BASE=origin/dev]  Build, emit, and freeze a theorem, then cover the anchor atom in the working tree' 'make cover ATOM_ID=x GID=g [BASE=origin/dev]  Write an atom coverage edge in the working tree' 'make cover-batch ATOMS=file [BASE=origin/dev]  Cover strict TSV atom/GID pairs with one Lean report' 'make scribe-seed PAIRS=file [BASE=origin/dev]  Register missing Scribe receipts atomically from strict TSV pairs' 'make scribe-refresh DOCUMENTS=file [BASE=origin/dev] [DRY_RUN=1]  Re-attest all current Scribe receipts for selected documents' 'make quarantine REQUEST=file [BASE=origin/dev]  Write one atom'\''s receipts.quarantine from a strict request file' 'make quarantine-clear ATOM_ID=x [BASE=origin/dev]  Clear one atom'\''s receipts.quarantine' 'make settle REQUEST=file [BASE=origin/dev]  Settle one atom as nonpropositional from a strict request' 'make settle-clear ATOM_ID=x [BASE=origin/dev]  Clear one nonpropositional receipt' 'make worktree KIND=x NAME=y [BASE=origin/dev] [DEST=DIR]  Initialize an isolated worktree; Lean cache is lazy and never symlinked' 'make worktree-clean [BASE=origin/dev]  Reclaim registered lanes that are merged into BASE and have no uncommitted work' 'make pr-open HEAD=branch MESSAGE=file [AUTO_MERGE=1]  Create from a message file, optionally arm auto-merge, and wait for required-CI verdict' 'make pr-watch PR=n                Wait for required-CI verdict on an existing PR' 'make preflight BASE=<40-hex-sha>  Pre-verify all three required CI checks (engineering / lean-inspect / admission) locally before pushing' 'make gate [BASE=origin/dev]       Run the local CI-equivalent admission flow' 'make decompose ATOM_ID=x [BASE=origin/dev] [DRY_RUN=1]  Decompose one CAS atom into a lossless clause chain'

test:
	@/bin/bash tools/scripts/workflow/math-gate.sh

lean-cache-ensure:
	@/bin/bash tools/scripts/worktree/lean-cache-ensure.sh

lean-cache-to-github-without-mathlib:
	@/bin/bash tools/scripts/worktree/lean-cache-publish.sh publish

lean-cache-from-github-without-mathlib:
	@/bin/bash tools/scripts/worktree/lean-cache-publish.sh fetch

warm-donor:
	@/bin/bash tools/scripts/worktree/warm-donor.sh

lean:
	@/bin/bash tools/scripts/worktree/lean-cache-run.sh lake build

lean-report:
	@/bin/bash tools/scripts/report/lean-report.sh

build: lean

emit:
	@/bin/bash tools/scripts/scribe.sh emit

ingest:
	@/bin/bash tools/scripts/ingest.sh ingest "$(BASE)"

align-digestion-status:
	@/bin/bash tools/scripts/ingest.sh align-digestion-status "$(BASE)"

mathlib-reanchor:
	@/bin/bash tools/scripts/ingest.sh mathlib-reanchor "$(BASE)"

echo-residual-summary:
	@/bin/bash tools/scripts/report/echo-residual-summary.sh "$(BASE)"

digestion-readiness:
	@dotnet run --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- digest-status --readiness

show-atom:
	@dotnet run --no-build --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- show-atom --atom-id "$(ATOM_ID)"

atom-context:
	@dotnet run --no-build --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- atom-context --atom-id "$(ATOM_ID)"

truth-export:
	@dotnet run --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- truth-export --out "$(OUT)" --candidate-lean-report "$(LEAN_REPORT)"

deliver-check:
	@/bin/bash tools/scripts/workflow/playbook-workflows.sh deliver-check "$(BASE)"

deposit:
	@/bin/bash tools/scripts/workflow/playbook-workflows.sh deposit "$(BASE)" "$(ATOM_ID)" "$(GID)"

cover:
	@/bin/bash tools/scripts/workflow/playbook-workflows.sh cover "$(BASE)" "$(ATOM_ID)" "$(GID)"

cover-batch:
	@/bin/bash tools/scripts/workflow/playbook-workflows.sh cover-batch "$(BASE)" "$(ATOMS)"

scribe-seed:
	@dotnet run --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- align-scribe-receipt --seed-missing --pairs "$(PAIRS)" --base "$(BASE)"

scribe-refresh:
	@dotnet run --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- align-scribe-receipt --refresh --documents "$(DOCUMENTS)" --base "$(BASE)" $(if $(filter 1,$(DRY_RUN)),--dry-run,)
decompose:
	@dotnet run --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- decompose-atom --atom "$(ATOM_ID)" --base "$(BASE)" $(if $(filter 1,$(DRY_RUN)),--dry-run,)

quarantine:
	@/bin/bash tools/scripts/ingest.sh quarantine "$(BASE)" "$(REQUEST)"

quarantine-clear:
	@/bin/bash tools/scripts/ingest.sh quarantine-clear "$(BASE)" "$(ATOM_ID)"

settle:
	@dotnet run --no-build --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- settle-atom --request "$(REQUEST)" --base "$(BASE)"

settle-clear:
	@dotnet run --no-build --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- settle-atom --clear "$(ATOM_ID)" --base "$(BASE)"

worktree:
	@/bin/bash tools/scripts/worktree-init.sh "$(KIND)" "$(NAME)" "$(WORKTREE_DEST)" "$(BASE)"

worktree-clean:
	@/bin/bash tools/scripts/clean-lanes.sh --base "$(BASE)" --lanes-only --force

pr-open:
	@/bin/bash tools/scripts/pr.sh open --head "$(HEAD)" --message-file "$(MESSAGE)" $(if $(filter 1,$(AUTO_MERGE)),--auto-merge,) $(if $(WATCH_TIMEOUT_SECONDS),--timeout-seconds "$(WATCH_TIMEOUT_SECONDS)",) $(if $(WATCH_INTERVAL_SECONDS),--interval-seconds "$(WATCH_INTERVAL_SECONDS)",)

pr-watch:
	@/bin/bash tools/scripts/pr.sh watch --pr "$(PR)" $(if $(WATCH_TIMEOUT_SECONDS),--timeout-seconds "$(WATCH_TIMEOUT_SECONDS)",) $(if $(WATCH_INTERVAL_SECONDS),--interval-seconds "$(WATCH_INTERVAL_SECONDS)",)

preflight:
	@BASE="$(BASE)" /bin/bash tools/scripts/preflight.sh
ifeq ($(origin BASE),file)
preflight: BASE :=
endif

gate:
	@/bin/bash tools/scripts/local-harness-gate.sh --base "$(BASE)" $(GATE_ARGS)
