...
CENSUS_PREFIX ?= D5
.PHONY: help test lean-cache-ensure lean-cache-to-github-without-mathlib lean-cache-from-github-without-mathlib warm-donor lean lean-report build emit ingest align-digestion-status refresh-source-registry mathlib-reanchor echo-residual-summary digestion-readiness show-atom atom-context truth-export deliver-check deposit deposit-uncovered cover cover-batch decompose quarantine quarantine-clear settle settle-clear settle-upstream settle-upstream-clear worktree worktree-clean worktree-remove pr-open pr-watch preflight gate census census-derivational

help:
	@printf '%s\n' 'make help                         Show this target list' 'make test                         Run the mathematical gate (Lean, admission, Scribe, values)' 'make lean-cache-ensure            Ensure a private Lean build cache is available' 'make lean-cache-to-github-without-mathlib  Build and validate this tree'\''s current Inspector report, then publish the complete Lean buildDir; dependency packages are excluded' 'make lean-cache-from-github-without-mathlib  Fetch this tree'\''s own Lean build output from a GitHub release, or fail closed; mathlib still comes from lake exe cache get' 'make warm-donor                  Update and build a clean dev checkout for optional machine-local scheduling' 'make lean [LEAN_TARGETS="..."]    Build the pinned Lean project or selected modules' 'make lean-report                  Produce the canonical raw Lean report' 'make build                        Build the pinned Lean content' 'make emit                         Emit canonical Scribe documents, catalog, and values' 'make ingest [BASE=origin/dev] [SOURCE="id path ..."]  Atomize theory sources; add only atom ids absent from the on-disk ledger' 'make align-digestion-status [BASE=origin/dev] [PLAN=1]  Consume the Lean report; preview or align coverage/truth status' 'make refresh-source-registry SOURCE=id [BASE=origin/dev] [PLAN_HASH=sha256]  Validate and publish one existing source registry plan; apply its exact hash' 'make mathlib-reanchor [BASE=origin/dev]  Reanchor frozen statement identities after a pin change' 'make echo-residual-summary [BASE=origin/dev]  Emit the content-addressed residual projection bytes' 'make digestion-readiness          Print exhaustive residual-open readiness as JSON' 'make show-atom ATOM_ID=x          Print and verify one digestion atom without writing' 'make atom-context ATOM_ID=x       Print an atom and its source neighbors without writing' 'make truth-export OUT=dir [LEAN_REPORT=path]  Export current Closed truth with freeze status (V2 dialect) to dir/truth-export.v1.json' 'make deliver-check [BASE=origin/dev]  Run theorem delivery in canonical freeze-last order' 'make deposit ATOM_ID=x GID=g [BASE=origin/dev]  Build, emit, and freeze a theorem, then cover the anchor atom in the working tree' 'make deposit-uncovered GID=g [BASE=origin/dev]  Build, emit, and freeze a theorem with no atom; leave it uncovered' 'make cover ATOM_ID=x GID=g [BASE=origin/dev]  Write an atom coverage edge in the working tree' 'make cover-batch ATOMS=file [BASE=origin/dev]  Cover strict TSV atom/GID pairs with one Lean report' 'make quarantine REQUEST=file [BASE=origin/dev]  Write one atom'\''s receipts.quarantine from a strict request file' 'make quarantine-clear ATOM_ID=x [BASE=origin/dev]  Clear one atom'\''s receipts.quarantine' 'make settle REQUEST=file [BASE=origin/dev]  Settle one atom as nonpropositional from a strict request' 'make settle-clear ATOM_ID=x [BASE=origin/dev]  Clear one nonpropositional receipt' 'make settle-upstream REQUEST=file [BASE=origin/dev]  Settle one atom with a verified upstream probe; declarations are sorted and duplicates rejected' 'make settle-upstream-clear ATOM_ID=x [BASE=origin/dev]  Clear one upstream receipt and its probe' 'make worktree KIND=x NAME=y [BASE=origin/dev] [DEST=DIR]  Initialize an isolated worktree; Lean cache is lazy and never symlinked' 'make worktree-clean [BASE=origin/dev]  Reclaim registered lanes that are merged into BASE and have no uncommitted work' 'make worktree-remove NAMES="full-directory-name ..."  Force-delete registered worktrees by complete directory name (whitespace-separated; whitespace in directory names unsupported); bypass unmerged/dirty/age/open PR/process criteria; locked trees require git worktree unlock <path> first; GNU make returns only 0/2, read WORKTREE_REMOVE_RESULT for the CLI exit code' 'make pr-open HEAD=branch MESSAGE=file [AUTO_MERGE=1]  Create from a message file, optionally arm auto-merge, and wait for required-CI verdict' 'make pr-watch PR=n                Wait for required-CI verdict on an existing PR' 'make preflight BASE=<40-hex-sha>  Pre-verify all three required CI checks (engineering / lean-inspect / admission) locally before pushing' 'make gate [BASE=origin/dev]       Run the local CI-equivalent admission flow' 'make decompose ATOM_ID=x [BASE=origin/dev] [SPLIT_AT="bytes ..."] [DRY_RUN=1]  Decompose one CAS atom into a lossless clause chain' 'make census [CENSUS_OUT=dir] [CENSUS_PREFIX=D5]  Run the bounded census using the current Lean report' 'make census-derivational CENSUS_OUT=dir  Publish support-positive candidates from an existing structural sidecar (report-only)' 'Census is report-only, never a gate; status=complete means accounting completion only.' 'Observed rows are unclassified: never certified, never a closed reason, never AC-023.' 'Explicit CLI input: --fixture-truth-export is synthetic-fixture-only; production paths and revisions are rejected.'
...
settle-upstream:
	@test -x tools/StrataLint.Cli/bin/Release/net10.0/StrataLint || dotnet build tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release >/dev/null; dotnet run --no-build --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- settle-upstream --request "$(REQUEST)" --base "$(BASE)"

settle-upstream-clear:
	@test -x tools/StrataLint.Cli/bin/Release/net10.0/StrataLint || dotnet build tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release >/dev/null; dotnet run --no-build --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- settle-upstream --clear "$(ATOM_ID)" --base "$(BASE)"
