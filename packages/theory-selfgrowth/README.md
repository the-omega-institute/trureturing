...
- Emits generation-scoped requests: same `detected_at` replays with the same `dedup_key`; the next idle generation receives a new `dedup_key` so a closed prior issue cannot suppress it. The stable open-request marker stays separate in the body for open-only exclusion.
