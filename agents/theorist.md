...
Method requirement: before publishing a theory batch, run `make echo-residual-summary BASE=origin/dev`, paste its complete output into `.echo-review.md` verbatim, and run `make echo-review-verify BASE=origin/dev` before using the file as evidence. Never hand-count or hand-edit residual totals, mother residual `atom_id` lists, or `unresolved-subitem` lists; rerun and reverify the machine projection after every Lean, Scribe, or digestion-ledger change.
