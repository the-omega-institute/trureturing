...
| SL-024 Scribe prose assertion single source | Regexes scan `RepositoryFile.Text` under `StrataLint.Scribe.Tests`; literal `Assert.Contains`/`Assert.DoesNotContain` calls are rejected when their second argument is a simple local whose `var`/`string` initializer has a recognized joined/direct `Inline.Text` extraction, or a canonical-markdown render in `Describe/**/*DocumentTests.cs` (`ScribeProseAssertionRule.cs`). | Yes: joined prose, direct text, and rendered-markdown red fixtures plus alias-propagation and structural LaTeX/declaration green counterfixtures; the repository-tail test failed on 220 existing assertions before cleanup and then passed. | Literal copies of Blueprint prose in the recognized local-declaration/assertion text shapes. It deliberately does not follow aliases or helper-return/field data flow, parse arbitrary declaration/assertion syntax, accept named or reordered arguments or nonliteral expected values, or recognize aggregate markdown outside the document-test path shape. |
