...
| Inline JSON/YAML literals | `GoldenCorpusMaterializer.cs:222`, `RuleFixture.cs:52,97-99`, and golden cases such as `GoldenCorpus.Cases03.cs:65-95`. | Structured canonical writers validate admitted repository artifacts, not C# fixture literals. | `HC-OPEN-003`: malformed-input tests must preserve noncanonical bytes; replacing them with serializers would erase the behavior under test. A global ban would mostly reject valid negative fixtures. |
