...
{
    private const string ScribeTestsPrefix = "Meta/StrataLint/StrataLint.Scribe.Tests/";
    ...
    private static IEnumerable<RepositoryReadSource> ProductionSources(string repositoryRoot) =>
        GitIndexRepositoryFiles.Enumerate(repositoryRoot)
            .Where(static file => file.RelativePath.StartsWith("Meta/StrataLint/StrataLint.", StringComparison.Ordinal)
                && file.RelativePath.EndsWith(".cs", StringComparison.Ordinal)
    ...
    {
        var slash = path.IndexOf('/', "Meta/StrataLint/".Length);
        return path["Meta/StrataLint/".Length..slash];
    }
