...
internal static class ObserverAtomizer
{
    ...
    private static string? Identify(string paragraph)
    {
        var locator = paragraph switch
        {
            ...
            _ when Starts(paragraph, "**§37.1 率的世界**") => "metric-rates/rate-world",
            _ when Starts(paragraph, "**§37.2 尺子的性格表**") => "metric-rates/metric-character",
            _ when Starts(paragraph, "**§38.1 塔的全貌**") => "research-boundary/tower-overview",
            _ when Starts(paragraph, "**§38.2 周期之句点**") => "research-boundary/cycle-closure",
            _ when Starts(paragraph, "**§39.1 最后的地图**") => "nameability/final-map",
            _ when Starts(paragraph, "**§39.2 匿名的教益**") => "nameability/anonymous-lesson",
            _ when Starts(paragraph, "**§40.1 算术的住址**") => "stationary-points/arithmetic-address",
            _ when Starts(paragraph, "**§40.2 水位线与门**") => "stationary-points/waterline-gates",
            _ when Starts(paragraph, "**§41.1 词典与墙**") => "mountainside/dictionary-wall",
            _ when Starts(paragraph, "**§41.2 山腰的账**") => "mountainside/ledger",
            _ when Starts(paragraph, "**§42.1 四律**") => "diagonal-ledger/four-laws",
            _ when Starts(paragraph, "**§42.2 刀锋之外**") => "diagonal-ledger/beyond-blade",
            _ when Starts(paragraph, "**§43.1 问与答的形状**") => "six-questions/shape",
            _ when Starts(paragraph, "**§43.2 语法与门后**") => "six-questions/grammar",
            _ when Starts(paragraph, "**§44.1 价目与王冠**") => "tower-top/prices-crowns",
            _ when Starts(paragraph, "**§44.2 三层与塔顶**") => "tower-top/three-layers",
            _ when Starts(paragraph, "**§45.1 蒙难记与星座**") => "final-volume/trials-constellation",
            _ when Starts(paragraph, "**§45.2 合卷**") => "final-volume/closure",
            _ when Starts(paragraph, "**§46.1 界面**") => "interface/interface",
            _ when Starts(paragraph, "**§46.2 合卷**") => "interface/closure",
            _ => null,
        };
