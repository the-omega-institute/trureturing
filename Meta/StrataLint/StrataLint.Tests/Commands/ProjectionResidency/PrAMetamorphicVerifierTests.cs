...
public sealed class PrAMetamorphicVerifierTests
{
    ...
    [Fact]
    public void RequiredLaneRunsExactlyTwoCanonicalRebuildsAndAccountsForCanary()
    {
        ...
        var result = PrAMetamorphicVerifier.VerifyRequired(testCase =>
        {
            ...
            return new PrARealRebuildOutcome(
                ...
                generated.Add(testCase.Checkout));
        });

        Assert.True(result.Pass, string.Join("\n", result.Diagnostics));
        Assert.Equal("required", result.Lane);
        Assert.Equal(2, result.RealRebuildsRun);
        Assert.Equal(94, result.CanaryDeferredCount);
        Assert.Equal(94, result.DeferredEnvTuples.Length);
        Assert.Equal(192, rebuilds.Count);
        Assert.All(rebuilds, static item =>
        {
            Assert.Equal("C", item.Locale);
            Assert.Equal("UTC", item.Timezone);
            Assert.Equal("canonical", item.Order);
            Assert.Equal(1, item.Parallelism);
            Assert.Equal(0, item.SourceDateEpoch);
        });
    }

    [Fact]
    public void CanaryLaneContainsExactlyTheDeferredEnvironmentTuples()
    {
        var required = PrAMetamorphicVerifier.RequiredScope();
        var canary = PrAMetamorphicVerifier.CanaryScope();

        Assert.Equal(2, required.Length);
        Assert.Equal(94, canary.Length);
        Assert.Empty(required.Intersect(canary));
        Assert.Equal(96, required.Concat(canary).Distinct().Count());
    }
    ...
    public void RealCanonicalGeneratorNondeterminismMakesRequiredLaneRed()
    {
        ...
        using var runner = new PrARealRebuildRunner(root, inventory, (checkoutName, checkout) =>
        {
            if (checkoutName != "checkout-b") return;
            var generator = Path.Combine(checkout, "Meta", "StrataLint", "scripts", "scribe.sh");
            File.AppendAllText(generator, "\nprintf 'checkout-b\\n' >> Generated/DAG.md\n", Encoding.UTF8);
        });
        var manifest = new string('a', 64);
        var result = PrAMetamorphicVerifier.VerifyRequired(testCase => runner.Rebuild(
            testCase, manifest, manifest[..32], RunHandleDigests.Inventory(inventory), new string('b', 64)));
    ...
    [Fact]
    [Trait("Category", "PrAEffectiveness")]
    public void PerturbedCleanCheckoutMakesRequiredLaneRed()
    {
        var root = FindRepositoryRoot();
        var inventory = RunHandleCommand.Inventory(root);
        using var runner = new PrARealRebuildRunner(root, inventory, (checkoutName, checkout) =>
        {
            if (checkoutName == "checkout-b")
                File.AppendAllText(Path.Combine(checkout, "CLAUDE.md"), "\nperturbed\n", Encoding.UTF8);
        }, validateAfterMutation: true);
        var manifest = new string('c', 64);
        var result = PrAMetamorphicVerifier.VerifyRequired(testCase => runner.Rebuild(
            testCase, manifest, manifest[..32], RunHandleDigests.Inventory(inventory), new string('d', 64)));

        Assert.Equal(1, result.ExitCode);
        Assert.Contains(result.Diagnostics, static item => item.Contains("PR_A_CHECKOUT_NOT_CLEAN", StringComparison.Ordinal));
    }
    ...
    private static string FindRepositoryRoot()
    {
        ...
        while (current is not null && !File.Exists(Path.Combine(current.FullName, "CLAUDE.md"))) current = current.Parent;
