...
public sealed class ValuesProjectionLoaderTests
{
    ...
    private static readonly ImmutableArray<string> PreBindingInputPaths = InputPaths
        .Where(static path => path is not ValuesProjectionLoader.LeanModulePath
            and not ValuesProjectionLoader.KernelDataPath)
        .ToImmutableArray();

    [Fact]
    public void PreBindingProjectionWithoutLeanSourcesRemainsAdmitted()
    {
        var inputs = Inputs(StrictUtf8.GetBytes("formal values producer input\n"), PreBindingInputPaths);
        var snapshot = Snapshot(inputs, PreBindingProjection(inputs));
    ...
    [Fact]
    public void PreBindingProjectionPreservesTheLegacyNonEmptyProvenanceContract()
    {
        var inputs = Inputs(StrictUtf8.GetBytes("formal values producer input\n"), PreBindingInputPaths);
        var snapshot = Snapshot(inputs, PreBindingProjection(inputs, provenance: "legacy-reference"));
    ...
    [Fact]
    public void PreBindingProjectionIsRejectedOnceTheLeanSourceExists()
    {
        var inputs = Inputs(StrictUtf8.GetBytes("formal values producer input\n"), PreBindingInputPaths)
            .Add((
                ValuesProjectionLoader.LeanModulePath,
                StrictUtf8.GetBytes("pre-binding downgrade fixture\n")));
        var attested = inputs.RemoveAt(inputs.Length - 1);
        var snapshot = Snapshot(inputs, PreBindingProjection(attested));
    ...
    private static ImmutableArray<byte> PreBindingProjection(
        ImmutableArray<(string Path, byte[] Bytes)> inputs,
        string provenance = "Lean")
    {
        ...
        var root = JsonSerializer.SerializeToElement(new
        {
            attestation = new
            {
                ...
                emitter_version = 1,
                input_sha256 = CombinedInputSha(inputReceipts, version: 1),
            ...
            constants = ids.Select(id => PreBindingConstant(id, emitted.Contains(id), provenance)).ToArray(),
            schema_version = 1,
        });
        ...
        ImmutableArray<(string Path, byte[] Bytes)> inputs)
    {
        var inputReceipts = inputs.Select(static input =>
            (input.Path, Sha256: Convert.ToHexStringLower(SHA256.HashData(input.Bytes)))).ToArray();
        var combinedSha = CombinedInputSha(inputReceipts, version: 2);
        var emitted = new HashSet<string>(StringComparer.Ordinal)
        {
            "D5/Ah", "D5/C0", "D5/Cphi", "D5/E", "D5/cstar", "D5/hbar", "D5/kappa", "D5/s1",
        };
    ...
    private static object PreBindingConstant(string id, bool emitted, string provenance)
    {
        var kernels = emitted
            ? id == "D5/Cphi"
                ? new[]
