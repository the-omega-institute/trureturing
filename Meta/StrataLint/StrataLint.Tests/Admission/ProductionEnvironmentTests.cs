...
public sealed partial class ProductionEnvironmentTests
{
    ...
    public void EchoVerifyMachineRejectsHandEditedStaleAndMissingBlocks()
    {
        ...
        File.WriteAllText(candidatePath, "prose\n" + emitted.Output + "verdict\n", new UTF8Encoding(false));
        ...
        File.WriteAllText(
            candidatePath,
            emitted.Output.Replace("git-sha1:aaaaaaaa", "git-sha1:cccccccc", StringComparison.Ordinal),
            new UTF8Encoding(false));
        var stale = environment.EchoVerify(["--file", candidatePath, "--base", "baseline"]);
        File.WriteAllText(candidatePath, "prose only\n", new UTF8Encoding(false));
        ...
        Assert.Contains("no echo residual", missing.Error, StringComparison.Ordinal);
    }
