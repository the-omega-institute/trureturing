...
public sealed partial class FormulaCorpusInventoryTests
{
    ...
    public void InventoryAllLegacyLatexStatementsAndSyntaxFamilies()
    {
        ...
        Assert.Equal(171, entries.Length);
        Assert.Equal(79, entries.Select(static entry => entry.SourcePath).Distinct().Count());
    ...
    public void EveryMigratedFormulaHasAStableCorpusAddress()
    {
        ...
        Assert.Equal(171, actual.Length);
        Assert.Equal(171, actual.Select(static entry =>
