...
=======
        Assert.Equal(183, entries.Length);
>>>>>>> origin/dev
        Assert.Equal(83, entries.Select(static entry => entry.SourcePath).Distinct().Count());
...
=======
        Assert.Equal(183, actual.Length);
        Assert.Equal(183, actual.Select(static entry =>
>>>>>>> origin/dev
