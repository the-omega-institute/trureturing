...
public sealed partial class FormulaCorpusInventoryTests
{
    ...
    public void InventoryAllLegacyLatexStatementsAndSyntaxFamilies()
    {
        ...
        Assert.Equal(188, entries.Length);
        Assert.Equal(88, entries.Select(static entry => entry.SourcePath).Distinct().Count());
    ...
    public void EveryMigratedFormulaHasAStableCorpusAddress()
    {
        ...
        Assert.Equal(188, actual.Length);
        Assert.Equal(188, actual.Select(static entry =>
