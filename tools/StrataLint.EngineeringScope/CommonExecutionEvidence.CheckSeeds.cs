...
internal static partial class CommonExecutionEvidence
{
        ...
        CommonCheckRecord? accepted, string testNotification)
    {
        ...
        if (selected.Count < registered.Length)
        {
            try
            {
                var previous = ValidateCheckSeedBundle(root, stage);
                ...
                sources.AddRange(retained.Select(unit => (unit with { Status = "reused" }, Path.Combine(root, CheckSeedPath(stage)))));
            }
    ...
    internal static CommonCheckRecord ValidateCheckSeedBundle(string root, string stage)
    {
        ...
        if (materials is null || materials.Any(material => material is null || material.Path is null)
            || !record.Units.SelectMany(unit => unit.Materials).Distinct().OrderBy(material => material.Path, StringComparer.Ordinal)
