...
abbrev arena : StructuralArena := ⟨Option Int⟩
...
structural_theorem independentSource in law
  readout via (template arena signature (fun _ => InformationSourceFixture.compositeReadout))
  realization (template arena signature (fun _ => InformationSourceFixture.compositeReadout)) := by
    intro x
    rfl
...
run_meta do
  ...
  logInfo "[PASS] independent_composite_source_binding_validated"
  let some source := certificate.argumentInputs.find?
      (·.name == ``InformationSourceFixture.compositeReadout)
    | throwError "[FAIL] independent_source_binding_retains_native_identities"
  unless source.owner == `InformationSourceFixture && source.typeIdentity.length == 64 &&
