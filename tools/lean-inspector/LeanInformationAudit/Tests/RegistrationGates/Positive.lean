...
theorem variation : arena.Law good ∧ ¬arena.Law bad := ⟨rfl, Bool.noConfusion⟩
theorem sensitivity : FiniteSlotSensitivity arena := by
  constructor
  · intro i
    refine ⟨good, bad, ?_, ?_, ?_⟩
    · intro j ne; exact (ne (Subsingleton.elim _ _)).elim
    · intro j; exact Fin.elim0 j
    · exact ⟨fun _ => variation.2, fun _ => variation.1⟩
...
register_information_theorem source in arena primitives good.toPrimitiveBundle
  realization bridge variation variation sensitivity sensitivity
...
#guard_msgs in
run_cmd Elab.Command.liftTermElabM do
  let entry := (InformationRegistry.find? (← getEnv) ``source).get!
