...
run_cmd liftTermElabM do
  ...
  let some registration := included.finite.find? (fun entry =>
      entry.theoremName == key.theoremName &&
      entry.registrationModuleName == `LeanInformationAudit.Tests.Census.Query.Registration)
  ...
  | .observed value =>
    unless value.queryCompleted && value.importScope.completed &&
        value.candidates.contains registration.unitName &&
