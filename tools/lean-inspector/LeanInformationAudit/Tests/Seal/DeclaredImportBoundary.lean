...
run_cmd do
  ...
  -- The split closure has 99 D5/Impl modules and five Interface source owners.
  let interface := modules.filter ((`LeanInformationAuditInterface).isPrefixOf ·)
  let unchanged := modules.size == 104 && interface.size == 5
  (if unchanged then logInfo else logError) m!"[{if unchanged then "PASS" else "FAIL"}] finite_seal_family_closure_unchanged"
  logInfo m!"DTR_FINITE_IMPORTS modules={modules.size} expected=104 interface={interface.size}"
