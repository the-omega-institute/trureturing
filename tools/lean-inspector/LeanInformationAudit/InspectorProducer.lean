...
unsafe def main (args : List String) : IO Unit := do
  ...
  let producerName := `LeanInformationAudit.informationTemplateReportDriver
  ...
  unless env.header.moduleNames[owner.toNat]! == `LeanInformationAudit.DispositionEvidence do
    throw <| IO.userError "IE-C050 reason=incomplete_closure rule=dtr.report_producer_owner"
  let driver ← IO.ofExcept <| env.evalConstCheck InformationTemplateReportDriver {}
    ``InformationTemplateReportDriver producerName
  let (bindings, _) ← (driver.generate (inputs.map (·.moduleName.toName))).run' |>.toIO
