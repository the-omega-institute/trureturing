...
def valid_baseline(
    ...
    if not (report.is_file() and sidecar_path(report).is_file()
            ...
            and pathlib.Path(str(report) + ".input.attestation").is_file()
            and pathlib.Path(str(report) + ".provenance.json").is_file()):
        return None
    try:
        value = baseline_identity(report)
        if (value["input_address"] != "sha256:" + entry.name
                or value["producer_sha256"] != producer_sha
                or value["repository_inspector_sha256"] != resident_sha
                or value["lean_config_sha256"] != config_sha):
            return None
...
def plan(args: argparse.Namespace) -> int:
    ...
    if best is None:
        ...
        # source-identical surviving importer in a correctly produced baseline,
        # the old import list is identical to the current one. Together with
        # declared refutation inputs, these edges close changed/added roots and
        # surviving dependents of removed modules without inspecting first.
