...
class NativeRegTests:
    ...
    def prepare_sidecar_enrollment(self, output):
        ...
        for name in filter(None, paths):
            self.copy(name)
        # Stage the pinned upstream checkouts, without their expanded Lake
        # outputs. Canonical ensure/cache-get supplies the complete upstream
        # build closure; the selected authored project builds remain private.
        from test_native_support import ROOT
        ...
        self.assertFalse((self.root / '.lake/.stratalint-lean-cache-stamp.json').exists())
        ...
        self.assertEqual(prepared['method'], 'cache-get')
