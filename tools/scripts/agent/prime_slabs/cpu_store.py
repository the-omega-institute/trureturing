...
class Store:
    ...
    def add_producer(self,name,document):
        if name!=cpu.digest(document): raise InputGap('producer content digest mismatch: '+name)
    ...
    def audit_costs(self):
        """A missing cost row must not silently reset an existing pilot allowance."""
        totals={'pilot':0,'window':0}
        for field in ('tiers','interruptions'):
            for phase,spent in self.connection.execute(
                "SELECT json_extract(e.value,'$.phase'),sum(json_extract(e.value,'$.evaluation_ns')) "
                "FROM nodes n,json_each(n.document,?) e GROUP BY 1",('$.'+field,)):
                if phase not in totals or type(spent) is not int or spent<0:
                    raise InputGap('retained tier/phase expenditure correction required')
                totals[phase]+=spent
        for phase,total in totals.items():
            if self.spent_ns(phase)!=total:
                raise InputGap(phase+': retained cumulative expenditure conflicts with tier events')
    ...
    def validate_record(self,node,record):
        ...
        for index,t in enumerate(tiers):
            ...
            producer=self.connection.execute('SELECT document FROM producers WHERE id=?',
                                             (t.get('producer'),)).fetchone()
            need(producer is not None,'producer binding')
            need(cpu.digest(json.loads(producer[0]))==t['producer'],'producer content digest')
            bounds=t.get('bounds',{}).get('G')
            if t['outcome'] in COMPLETED:
                need(t.get('domain_enclosed') is True,'positive domain enclosure')
