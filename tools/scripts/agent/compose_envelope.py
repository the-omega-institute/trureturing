...
Fail-fast: 64 usage / bad --pr --pass, 65 not a worktree, 66 no origin/dev, 69 verb failed (its REVIEW_ENVELOPE_INVALID
line is relayed). Sentinel: COMPOSE_OK deposited=<n> ejected=<m> head=<sha9>. Idempotent.
Override the verb launcher with STRATALINT_CLI="dotnet /path/to/StrataLint.dll" (used before the verb lands in the target tree).
"""
...
def main(argv):
    ...
    launcher = shlex.split(os.environ.get("STRATALINT_CLI", "")) or [
        "dotnet", "run", "--project", "tools/StrataLint.Cli/StrataLint.Cli.csproj", "--configuration", "Release", "--"]
    verb = subprocess.run([*launcher, "review-envelope", "--base", base, "--head", head],
