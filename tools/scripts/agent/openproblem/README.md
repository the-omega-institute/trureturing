...
2. **靶清查**(`TARGET-GATES.md`):派席前四关,按成本升序——① curl OEIS 原文查是否已被证(语料的 `research open` 标注只说明没人改过那个文件,实测滞后文献三个月);② 对象级去重,**先查钉版 mathlib** 再查本仓(上游一般定理会静默实例化、不留标识符痕迹);③ 档位(悬赏题、名题的精确等价物、只剩有限残余的已发表结果,全部出局);④ 从**定义**而非猜想模式算数值。四关各自独立,过三关不算过。
