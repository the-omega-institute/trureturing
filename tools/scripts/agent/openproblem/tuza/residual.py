...
def reducible_two(n,adj):
    """Two-edge version: delete two edges, kill triangle t, t meets every survivor in <=1 vertex."""
    T=tris(n,adj)
    E=[(a,b) for a in range(n) for b in range(a+1,n) if adj[a]>>b&1]
    for t in T:
...
degs=Counter(); ntri=Counter(); nedge=Counter(); two=0; resid=0
for adj in level_at(N).values():
    ...
    resid+=1
    d=tuple(sorted((bin(adj[v]).count("1") for v in range(N)),reverse=True))
    degs[d]+=1
    ntri[len(tris(N,adj))]+=1
    nedge[sum(bin(x).count("1") for x in adj)//2]+=1
    if reducible_two(N,adj): two+=1
print("residual =",resid)
print("also reducible by TWO edges =",two, f"({two/resid:.4f})")
print("triangle counts:", sorted(ntri.items()))
print("edge counts:", sorted(nedge.items()))
print("top degree sequences:")
for d,c in degs.most_common(10): print("  ",d,c)
