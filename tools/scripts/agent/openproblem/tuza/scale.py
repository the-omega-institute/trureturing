...
for N in (5,6,7,8):
    def lvl(N):
        ...
        return level
    tot=out=r1=r2=0
    for adj in lvl(N).values():
        if not conn(N,adj): continue
        tot+=1
        if maxmatch(N,adj)==N-maxind(N,adj): continue
        out+=1
        if reducible(N,adj): r1+=1
        elif red2(N,adj): r2+=1
    cov=(r1+r2)/out if out else 1.0
    print(f"n={N}: connected={tot} outside_KE={out} one_edge={r1} two_edge={r2} covered={r1+r2} ({cov:.4f}) hard={out-r1-r2}")
