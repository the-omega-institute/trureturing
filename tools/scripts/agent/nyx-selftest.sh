...
__selftest() {  # 分类器的阳性/阴性对照。**立条依据(2026-09-06)**:`__classify` 的 OK 分支要求
  ...
  check_run() {  # rc | final line | submissions | recorded IDs | polled IDs | next pools | last verdict
    ...
    verdict=$(awk '/^NYX_(OK|EXTRACTION|INFRA|QUOTA|NOFILE|UNKNOWN|DELIVERY|UNCERTAIN|NOPOOL|BUSY|EXPIRED|LOCKBUSY|TIMEOUT|IO|ERR|CANCELLED)( |$)/ {v=$1} END {print v}' "$run_dir/stdout")
