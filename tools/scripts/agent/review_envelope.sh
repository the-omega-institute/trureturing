...
# The one place the repository invokes `review-envelope` (CliVerbLinkageTests scans *.sh callers): one
# invocation form only, so the linkage test sees exactly what runs.
set -euo pipefail
