...
def transport(args):
    ...
    if args.command == "pack":
        ...
        checks_archive = args.checks_seed_archive
        if (seed_archive is None and args.stage == "current"
                and os.environ.get("STRATALINT_CACHE_WRITES") == "true"):
            layers = ["current"]
            if os.environ.get("CI_PLAN_PATH"):
                import ci_plan
                plan = ci_plan.validate_plan(args.repository, oid(args.commit),
                    args.repository / os.environ["CI_PLAN_PATH"], args.repository / os.environ["CI_CHANGES_PATH"])
                layers = ci_plan.stage_requirements(args.repository, plan, "current")["cache_layers"]
            if "current" in layers:
                seed_archive = args.archive.with_name("ci-current-seed.tar.gz")
            if "checks" in layers and checks_archive is None:
                checks_archive = args.archive.with_name("ci-checks-seed.tar.gz")
        if seed_archive is not None:
            command.extend(["--seed-archive", str(seed_archive)])
        if checks_archive is not None:
            command.extend(["--checks-seed-archive", str(checks_archive)])
...
def main():
    ...
    parser.add_argument("--checks-seed-archive", type=pathlib.Path)
