...
def resolve(root, head):
    # The event fixes M before checkout; a regenerated merge ref can retain the
    # same PR head while changing M and B. Never accept that later candidate.
    event_candidate = os.environ.get("GITHUB_SHA", "")
    if not re.fullmatch(r"[0-9a-f]{40}", event_candidate) or event_candidate == "0" * 40:
        raise ValueError("pull request event GITHUB_SHA must be a nonzero immutable 40-hex commit")
    ...
    if candidate != event_candidate:
        raise ValueError("checkout does not match pull request event GITHUB_SHA")
    ...
    with tarfile.open(archive) as source:
        ...
        for member in members:
            ...
            if (path.is_absolute() or any(part in ("", ".", "..") for part in member.name.split("/"))
                    or not (member.isfile() or member.islnk())):
        ...
        if not entries[manifest].isfile():
            raise ValueError("invalid stage archive manifest")
        with source.extractfile(manifest) as stream:
            materials = json.load(stream)["materials"]
        declared = [item["path"] for item in materials]
        ...
        records = {item["path"]: item for item in materials}
        aliases = [member for member in members if member.islnk()]
        for member in aliases:
            target = entries.get(member.linkname)
            if (target is None or not target.isfile() or member.linkname not in records
                    or any(records[member.name][key] != records[member.linkname][key] for key in ("sha256", "mode"))
                    or member.mode != records[member.name]["mode"]
                    or target.mode != records[member.linkname]["mode"]):
                raise ValueError("invalid stage archive alias")
        source.extractall(root, members=[member for member in members if member.isfile()])
        # Tar aliases save transfer bytes only. Restore independent files with
        # their own timestamps, without links or repeated gzip seeks.
        for member in aliases:
            destination = root / member.name
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(root / member.linkname, destination)
...
def prepare_current(args):
    if args.stage != "current":
        raise ValueError("prepare-current requires the current stage")
    requirements = stage_input(args)
    if not requirements["required"]:
        return {"needs_lean_build": False}
    if not requirements["report_required"]:
        import ci_plan
        plan = ci_plan.validate_plan(args.repository, ci_plan.checked_head(args.repository, args.commit),
            args.repository / (args.plan or os.environ["CI_PLAN_PATH"]),
            args.repository / (args.changes or os.environ["CI_CHANGES_PATH"]))
        return {"needs_lean_build": "lean" in plan["execution"]["steps"]}
    directory = os.environ.get("STRATALINT_LEAN_REPORT_PREPARATION")
    if not directory:
        raise ValueError("prepare-current requires an explicit report preparation directory")
    completed = subprocess.run(["make", "--no-print-directory", "lean-report-prepare"],
        cwd=args.repository, capture_output=True, text=True)
    if completed.stderr:
        print(completed.stderr, end="", file=sys.stderr)
    if completed.returncode:
        raise ValueError("report preparation failed: " + completed.stdout.strip())
    import ci_plan
    result = ci_plan.strict_json_bytes(completed.stdout.encode())
    ci_plan.exact(result, {"needs_lean_build"}, "report preparation")
    if type(result["needs_lean_build"]) is not bool:
        raise ValueError("report preparation needs_lean_build must be boolean")
    return result
...
def main():
    ...
    parser.add_argument("command", choices=("resolve", "checkout", "pack", "restore", "verify", "advisory", "summary",
                                           "plan", "pr-paths", "pr-plan", "push-plan", "validate-plan", "no-work", "validate-no-work", "stage-input", "prepare-current"))
    ...
    try:
        ...
        if args.command in ("stage-input", "prepare-current") and args.stage in ("build", "engineering", "current", "delta"):
            import ci_plan
