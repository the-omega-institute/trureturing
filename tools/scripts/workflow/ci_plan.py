...
def filemap_policy_path(value):
    return value == FILEMAP or bool(FILEMAP_FRAGMENT.fullmatch(value))
...
def symlink_target(entry):
    declaration = entry["symlink"]
    exact(declaration, {"target", "kind"}, entry["pattern"] + ":symlink")
    def literal(value):
        path(value)
        if any(c.isspace() or ord(c) < 32 or 127 <= ord(c) < 160 or c in "*?[]:" for c in value):
            raise ValueError("symlink requires canonical literal paths")
        return value
    alias = literal(entry["pattern"])
    if entry["runtime_disposition"] != "committed-source" or declaration["kind"] not in {"file", "directory"}:
        raise ValueError(f"{alias}: symlink requires committed-source and file/directory kind")
    parts, descending = alias.split("/")[:-1], False
    for part in string(declaration["target"], alias + ":symlink target").split("/"):
        if part == ".." and not descending and parts:
            parts.pop()
        else:
            literal(part)
            parts.append(part)
            descending = True
    target = "/".join(parts)
    if (not target or any(p.lower() in {".git", ".lake"} for p in (alias + "/" + target).split("/"))
            or filemap_policy_path(alias) or filemap_policy_path(target) or FILEMAP.startswith(alias + "/")
            or target == alias or target.startswith(alias + "/")
            or declaration["kind"] == "directory" and alias.startswith(target + "/")):
        raise ValueError(f"{alias}: symlink cannot alias reserved state, FILEMAP, itself or an ancestor")
    return target
...
def load_filemap(raw, read_include=None, document_bytes=None):
    ...
    if "include" in data:
        ...
        if (not isinstance(includes, list) or not includes
                or any(not isinstance(item, str)
                       or not re.fullmatch(r"FILEMAP(?:\.[a-z][a-z0-9]*)+\.toml", item) for item in includes)
                or includes != sorted(set(includes))):
            raise ValueError("FILEMAP include names must be nonempty, unique, ordinally sorted sibling fragments")
        if read_include is None:
            raise ValueError("FILEMAP include requires a reader for the same candidate snapshot")
        for include in includes:
            include_path = "Meta/" + include
            included = read_include(include_path)
            if (not included or included.startswith(b"\xef\xbb\xbf") or b"\r" in included
                    or not included.endswith(b"\n")):
    ...
    if document_bytes is not None:
        document_bytes.extend((path, contents) for path, contents, _ in documents)
    ...
    entries = []
    for document_path, _, local_entries in documents:
        if local_entries is None:
            continue
        if not isinstance(local_entries, list) or not local_entries:
            raise ValueError(document_path + ": files must be a nonempty array")
        local_patterns = [entry.get("pattern") if isinstance(entry, dict) else None for entry in local_entries]
        if any(not isinstance(pattern, str) for pattern in local_patterns):
            raise ValueError(document_path + ": every file row must have a string pattern")
        if local_patterns != sorted(set(local_patterns)):
            raise ValueError(document_path + ": file patterns must be unique and ordinally sorted")
        entries.extend(local_entries)
    data["files"] = sorted(entries, key=lambda entry: entry["pattern"])
    ...
    if len(patterns) != len(set(patterns)):
        raise ValueError("file patterns must be unique across FILEMAP documents")
    for entry in entries:
        if "symlink" in entry and sum(bool(glob(p).fullmatch(entry["pattern"])) for p in patterns) != 1:
            raise ValueError(entry["pattern"] + ": symlink must match exactly one FILEMAP entry")
    return data


def filemap_digest(documents):
    digest = hashlib.sha256()
    for document_path, raw in documents:
        encoded = document_path.encode("utf-8")
...
def diff_paths(root, base, commit):
    ...
    while i < len(fields):
        ...
        if status.startswith("R"):
            ...
            status = "M"  # A file/link type change keeps both mode/blob endpoints.
...
def worktree_entries(root):
    ...
    for p in sorted(paths):
        for parent in reversed(pathlib.PurePosixPath(p).parents):
            if parent not in ancestors:
                ancestors.add(parent)
                if (root / parent).is_symlink():
                    raise ValueError(f"{p}: symlink ancestor in working-tree input")
        file = root / p
        try:
...
def make_plan(root, commit, changes_file):
    ...
    def read(p):
        ...
        raw = ((os.readlink(root / p).encode("utf-8") if tree[p]["mode"] == "120000" else (root / p).read_bytes())
               if local else committed_file(root, commit, p))
    ...
    def match(p):
        ...
        return matches[0]
    links = {p for p, value in tree.items() if value["mode"] == "120000"}
    for p in links:
        entry = match(p)
        if "symlink" not in entry or read(p) != entry["symlink"]["target"].encode("utf-8"):
            raise ValueError(f"{p}: symlink target must exactly match its FILEMAP declaration")
        target = symlink_target(entry)
        if any(target == link or target.startswith(link + "/") for link in links):
            raise ValueError(f"{p}: symlink target cannot have symlink ancestors or chains")
        if entry["symlink"]["kind"] == "file":
            valid = target in tree and tree[target]["mode"] in {"100644", "100755"}
        else:
            members = [v for name, v in tree.items() if name.startswith(target + "/")]
            valid = target not in tree and bool(members) and all(v["mode"] in {"100644", "100755"} for v in members)
        if not valid:
            raise ValueError(f"{p}: symlink target must contain plain files in the same snapshot")
...
def validate_plan(root, commit, plan, changes):
    ...
    current_documents = []
    load_filemap((root / FILEMAP).read_bytes(), lambda path: (root / path).read_bytes(), current_documents)
    for material in sorted({*(path for path, _ in current_documents), *value["materials"]}):
...
def stage_requirements(root, plan, stage):
    ...
    manifest = load_filemap((root / FILEMAP).read_bytes(), lambda path: (root / path).read_bytes())
    ...
    return {"required": bool(rows), "tools": union("tools"), "cache_layers": union("cache_layers"),
            "cache_activation": {layer: phase for row in rows for layer, phase in row["cache_activation"].items()},
