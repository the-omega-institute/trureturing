...
CACHE_PHASES = {"stage-start", "lean-production"}
...
def symlink_target(entry):
    ...
    if (not target or any(p.lower() in {".git", ".lake"} for p in (alias + "/" + target).split("/"))
            or alias == FILEMAP or FILEMAP.startswith(alias + "/")
...
def load_filemap(raw):
    ...
    for resource in data["resources"]:
        ...
        for layer, phase in activation.items():
            ...
            if phase == "lean-production" and resource["stage"] != "current":
                raise ValueError(f"{rid}: lean-production cache activation requires current stage")
