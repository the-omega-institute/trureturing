...
internal static partial class CommonExecutionEvidence
{
        ...
        string[]? selectedProjects = null)
    {
        ...
        object ExecutionMaterial(string path, HashSet<string> fileMapRelevant, bool queryFileMap)
        {
            ...
            // has a separate explicit query-address contract for its policy readers.
            if (path == EngineeringProjectRegistry.ManifestPath)
                return new { path, sha256 = Convert.ToHexStringLower(SHA256.HashData(
                    snapshot.Files[RepoPath.CreateKnown(path)].RawBytes.AsSpan())) };
            if (path == "Meta/FILEMAP.toml")
            {
                var documents = FileMapDocuments.Resolve(
                    snapshot.Files[RepoPath.CreateKnown(path)].RawBytes.AsSpan(), path,
                    included => snapshot.Files.TryGetValue(RepoPath.CreateKnown(included), out var file)
                        ? file.RawBytes.ToArray()
                        : throw new InvalidDataException("registered FILEMAP include is unavailable: " + included));
                var rows = documents.SelectMany(document => document.Table.TryGetValue("files", out var value)
    ...
    {
        var assemblies = CommonExecutionEvidence.TestAssemblies(root, build);
