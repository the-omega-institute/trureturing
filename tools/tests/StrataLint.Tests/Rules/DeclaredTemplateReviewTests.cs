...
public sealed class DeclaredTemplateReviewTests
{
    ...
    [InlineData(13, 8)]
    [InlineData(13, 12)]
    public void bumped_manifest_rejects_prior_semantics(int current, int prior)
    {
        var manifest = System.Text.Json.Nodes.JsonNode.Parse(PolicyFiles()["lean-report-inputs.json"])!;
        manifest["report_semantic_version"] = current;
        var error = ReadChangedManifest(manifest.ToJsonString(), prior);
        Assert.True(error is FormatException && error.Message.Contains("DTR-EvidenceVersion", StringComparison.Ordinal),
            "[FAIL] bumped_manifest_rejects_prior_semantics: " + error?.Message);
    }
