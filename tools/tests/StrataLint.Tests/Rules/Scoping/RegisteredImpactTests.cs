    ...
    public void InvalidScribeInputRegistrationFailsExplicitly(string defect)
    {
        ...
        else
        {
            ...
            if (defect == "duplicate-input")
                manifest["projects"]!.AsArray().Add("tools/StrataLint.Scribe/StrataLint.Scribe.csproj");
            else
            {
                fixture.Files["absent/Unregistered.csproj"] = "<Project />";
                manifest["projects"]!.AsArray().Add("absent/Unregistered.csproj");
            }
        ...
        if (defect == "unregistered-project")
            Assert.Equal("unregistered engineering project: absent/Unregistered.csproj", error.Message);
        else Assert.Contains("registration", error.Message);
    }
