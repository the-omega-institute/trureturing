...
{
    private const string Blob7967fb65Part2 = """
  change LucasPair certificate.phaseCertificate.a certificate.phaseCertificate.b ∧
    3 ≤ certificate.phaseCertificate.b
  exact lucas_of_phase_machine hcertificate.phase_machine
...
semantic providers remain open at their declared types:
`frontier_step_semantics`, `source_index_successor_delta`, and
`six_phase_gap_stream`. The current S1 interfaces prove the two-valued
return-gap spectrum but do not identify the complete F/G/H gap itinerary of a
negative-tail prefix cylinder; no missing theorem is replaced by a weaker
