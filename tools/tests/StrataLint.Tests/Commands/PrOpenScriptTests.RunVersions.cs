...
public sealed partial class PrOpenScriptTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void PrWatchAcceptsSupersedingRunFromObservedPr9253(bool reverse)
    {
        ...
        Assert.Contains("35562164528", Text(result.StandardError), StringComparison.Ordinal);
        Assert.Contains("35562213366", Text(result.StandardError), StringComparison.Ordinal);
        ...
        Assert.Equal(3, evidence.RootElement.EnumerateArray().Count(item =>
            item.GetProperty("run_id").GetInt64() == 35562164528 && item.GetProperty("superseded").GetBoolean()));
        Assert.Equal(3, evidence.RootElement.EnumerateArray().Count(item =>
            item.GetProperty("run_id").GetInt64() == 35562213366 && !item.GetProperty("superseded").GetBoolean()));
    }
    ...
    [InlineData("CANCELLED", "IN_PROGRESS", null, 124)]
    public void PrWatchUsesNewRunStateRegardlessOfOldSuccess(string oldConclusion, string status, string? conclusion, int exit)
    {
        using var fixture = new PrScriptFixture();
        fixture.SnapshotResponses(Ok(Snapshot("OPEN",
            Check("engineering", status, conclusion, checkId: 102, runId: 202, runNumber: 2),
    ...
    [Fact]
    public void PrWatchDoesNotBorrowMissingJobsFromAnOlderRun()
    {
        using var fixture = new PrScriptFixture();
        fixture.SnapshotResponses(Ok(Snapshot("OPEN",
            Check("engineering", "COMPLETED", "SUCCESS"),
            Check("resolve", "IN_PROGRESS", null, checkId: 102, runId: 202, runNumber: 2))));
    ...
    [Fact]
    public void PrWatchCannotFillAnOriginsMissingCurrentJobFromAnotherOrigin()
    {
        using var fixture = new PrScriptFixture();
        fixture.SnapshotResponses(Ok(Snapshot("OPEN",
            Check("engineering", "COMPLETED", "SUCCESS"), Context("engineering", "SUCCESS"),
            Check("resolve", "IN_PROGRESS", null, checkId: 102, runId: 202, runNumber: 2))));
    ...
    public void PrWatchOrdersExecutionsByRunNumberRatherThanJobOrRunId()
    {
        using var fixture = new PrScriptFixture();
        fixture.SnapshotResponses(Ok(Snapshot("OPEN",
            Check("engineering", "COMPLETED", "FAILURE", checkId: 900, runId: 900, runNumber: 1),
            Check("engineering", "COMPLETED", "SUCCESS", checkId: 100, runId: 100, runNumber: 2))));
    ...
    public void PrWatchRejectsAmbiguousRunVersions(string defect)
    {
        ...
        var older = JsonSerializer.SerializeToNode(Check("engineering", "COMPLETED", "CANCELLED"))!;
        ...
        switch (defect)
        {
            ...
            case "inconsistent-origin": run["databaseId"] = 201; run["runNumber"] = 1; run["event"] = "push"; break;
