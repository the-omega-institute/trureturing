        ...
        string temporary, string mode, string failedPhase = "", bool alwaysFail = false, bool prebuilt = false)
    {
        ...
        const string producer = """
            ...
            if phase == 'runtime-prefix':
                ...
                pathlib.Path(args[8]).write_text(json.dumps({'status': 'delta' if mode.startswith('delta') else mode,
                    'baseline': os.environ['STUB_BASELINE'], 'recheck': selected}))
            elif phase == 'inspect':
        ...
        var result = Run("/bin/bash", ["--noprofile", "--norc", "-c",
            ...
            $"LAKE_BIN={lake}", $"STRATALINT_REPORT_CACHE_ROOT={cache}",
