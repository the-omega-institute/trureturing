...
{
    [Fact]
    public void MissingBothPackageFilesBlocksCurrentSl015()
    {
        var files = Files();
        files.Remove(RegManifestAgreement.LakefilePath);
        files.Remove(RegManifestAgreement.ManifestPath);
        Assert.Contains(Current(files), finding =>
            finding.Message.Contains("REG-MANIFEST-MISSING", StringComparison.Ordinal));
    }

    [Fact]
    public void MissingBothPackageFilesBlocksWorktreePins()
    {
        ...
        Assert.Contains("REG-MANIFEST-MISSING", reason, StringComparison.Ordinal);
    }
