...
public sealed class LeanCacheStampPersistenceTests
{
    ...
    public void CorruptStampPersistenceRunsProducerAndUsesItsVerdict(int producerExit)
    {
        ...
        var result = LeanCacheEnsureCommand.Run(root,
            ["--", "/bin/sh", "-c", "printf 'producer output\\n'; printf 'producer diagnostic\\n' >&2; exit \"$1\"",
                "producer", producerExit.ToString(CultureInfo.InvariantCulture)],
            new ProductionWorktreeProcessRunner(), runCommand: true);
        ...
        using var released = LeanCacheWriterGuard.TryAcquire(lake, fixture.LockDirectory);
