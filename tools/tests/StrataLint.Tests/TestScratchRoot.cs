...
/// <summary>
/// 脚本测试在 scratch 目录里搭建「假可执行环境」的具名 helper。
///
/// 这些操作不是仓库 I/O:它们只写 <see cref="TestScratchRoot"/> 下的临时树,
/// 而 RepositoryIoAccessPolicy 是**语法**判定(其注释自陈 "does not judge indirect
/// reads through production loaders"),对 Directory/File 一视同仁。把它们收进本项目
/// (已在 DeferredProjectExemptions 内)的具名 helper,使脚本测试程序集不必带着
/// 直接 I/O 出生 —— 那条豁免名单的注释明写 "migration may only remove entries"。
/// </summary>
internal static class ScriptHarnessScratch
{
    /// <summary>在 scratch 树里建目录(可含多级)。</summary>
    internal static void EnsureDirectory(string path) => Directory.CreateDirectory(path);

    /// <summary>把仓库里的脚本拷进 scratch 树,并建好其父目录。</summary>
    ...
    /// <summary>写一个可执行的假命令(bash 前导 + 正文 + 可执行位)。</summary>
