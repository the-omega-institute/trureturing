...
public sealed class TrustTopologyTests
{
    ...
    public void RuleCatalogIsExplicitCompleteAndCannotDowngradeEffectsFromConfiguration()
    {
        ...
        Assert.Equal(26, descriptors.Length);
        Assert.Equal(26, descriptors.Select(item => item.Id).Distinct().Count());
        Assert.Equal(
            Enumerable.Range(1, 23).Select(RuleId.CreateKnown)
                .Append(RuleId.CreateKnown(25)).Append(RuleId.CreateKnown(26)).Append(RuleId.CreateKnown(27)),
