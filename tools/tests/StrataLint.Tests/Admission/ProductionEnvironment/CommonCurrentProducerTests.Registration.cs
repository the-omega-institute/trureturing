...
public sealed class ScribeInvocationRegistrationTests(ITestOutputHelper output)
{
    ...
    private sealed class InvocationFixture : IDisposable
    {
        ...
        private readonly IReadOnlyDictionary<string, string> dotnetEnvironment;
        ...
        internal InvocationFixture(ITestOutputHelper output)
        {
            ...
            dotnetEnvironment = DotnetFixtureProfile.Create(program);
            ...
            Write(manifest["registration"]!.GetValue<string>(), new JsonObject
            {
                ...
                ["producer_scopes"] = new JsonObject
                {
                    ...
                    {
                        ["include"] = new JsonArray(consumer["materials"]!.AsArray().Select(item => (JsonNode)new JsonObject
                            { ["pattern"] = item!.GetValue<string>(), ["optional"] = false }).ToArray()),
                        ["exclude"] = new JsonArray(),
                    },
                    ["scribe-content"] = JsonNode.Parse("{\"include\":[{\"pattern\":\"Blueprint/**/*.scribe.cs\",\"optional\":false}],\"exclude\":[]}"),
                },
            }.ToJsonString());
        ...
        {
            var result = TestProcessRunner.Run("env", dotnetEnvironment.Select(pair => pair.Key + "=" + pair.Value)
                .Concat(["dotnet", "build", "Fixture.csproj", "--configuration", "Release", "--nologo"]),
