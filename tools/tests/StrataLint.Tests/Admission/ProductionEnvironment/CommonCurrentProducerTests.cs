...
public sealed class CommonCurrentProducerTests
{
    ...
    public void ActualScribeCapabilityFeedsCurrentOwnerAndWarmPredicatesDoNotExecute(bool selected)
    {
        ...
        fixture.Files["Meta/ReportProducers/lean-report.json"] = "{\"schema\":\"report-producer-scope-v2\",\"registration\":\"lean-report-inputs.json\",\"scope\":\"lean-report\",\"projects\":[]}";
        fixture.Files["Meta/ReportProducers/scribe-content.json"] = "{\"schema\":\"report-producer-scope-v2\",\"registration\":\"lean-report-inputs.json\",\"scope\":\"scribe-content\",\"projects\":[\"tools/StrataLint.Scribe/StrataLint.Scribe.csproj\"]}";
        ...
        File.WriteAllText(rawPath + ".sha256", CommonExecutionEvidence.Hash(rawPath) + "  " + Path.GetFileName(rawPath) + "\n");
        foreach (var suffix in new[] { ".input.attestation", ".provenance.json" }) File.WriteAllText(rawPath + suffix, "fixture companion\n");
