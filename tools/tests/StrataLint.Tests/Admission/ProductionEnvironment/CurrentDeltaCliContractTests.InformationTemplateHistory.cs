...
public sealed partial class CurrentDeltaCliContractTests
{
    ...
    private static InformationTemplateDebtRow TemplateDebt(RepositorySnapshot snapshot, string seed) => new(
        TemplateOccurrence, seed,
        InformationTemplateJson.Sha256(System.Text.Encoding.UTF8.GetBytes(TemplateOccurrence.Theorem)),
        InformationTemplateJson.Sha256(snapshot.Files[RepoPath.CreateKnown(RuleFixture.RingPath)].RawBytes.AsSpan()),
        [TemplateInput(snapshot, RuleFixture.RingPath)]);
    ...
    private static LeanAxiomReport TemplateReport(RepositorySnapshot snapshot,
        IReadOnlyDictionary<string, LeanFileReport> reports, bool debt)
    {
        ...
        return LeanAxiomReport.Create(reports.ToDictionary(pair => pair.Key, pair =>
        {
            ...
            var own = debt && pair.Key == RuleFixture.RingPath ? new[] { TemplateOccurrence } : [];
            var wire = JsonSerializer.SerializeToElement(new
            {
                ...
                {
                    key = InformationTemplateDebtStore.KeyJson(key), registration_source_path = RuleFixture.RingPath,
            ...
            {
                InformationTemplates = InformationTemplateEvidence.Read(wire, pair.Key, snapshot),
