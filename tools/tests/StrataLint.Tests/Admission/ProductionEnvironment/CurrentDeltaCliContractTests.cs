...
public sealed partial class CurrentDeltaCliContractTests(Xunit.Abstractions.ITestOutputHelper log)
{
    ...
    [InlineData("valid", 0, "")]
    [InlineData("reused", 0, "")]
    [InlineData("template-activation", 0, "DTR-Inactive seed relation checked")]
    [InlineData("template-debt", 0, "DTR-Domain completed declared-template consumer")]
    [InlineData("template-missing-history", 1, "information-template-history")]
    ...
    public void DeltaConsumesValidatedCommonResultsAndEnforcesOnlyCrossTreePredicates(string scenario, int expectedExit, string diagnostic)
    {
        ...
        var templateHistory = scenario.StartsWith("template-", StringComparison.Ordinal);
        var templateDebt = templateHistory && scenario != "template-activation";
        using var temporary = new TemporaryDirectory();
        var root = temporary.Path;
        var fixture = new RuleFixture();
        fixture.AddBackfillTargets();
        ...
        var templateSeedRevision = basis;
        var templateSeed = templateHistory ? CommonExecutionEvidence.Snapshot(root) : null;
        if (templateHistory)
        {
            Write(InformationTemplateDebtStore.ActivationPath, DeclaredTemplateReviewTests.Text(
                InformationTemplateDebtStore.WriteActivation(new(basis, templateDebt))));
            if (templateDebt)
                Write(InformationTemplateDebtStore.PathFor(TemplateOccurrence), DeclaredTemplateReviewTests.Text(
                    InformationTemplateDebtStore.WriteRow(TemplateDebt(templateSeed!, basis))));
            Git(root, "add", ".");
            Git(root, "-c", "user.name=Fixture", "-c", "user.email=fixture@example.invalid", "commit", "-qm", "protected template authority");
            basis = Git(root, "rev-parse", "HEAD");
        }
        var templateProtected = templateHistory ? CommonExecutionEvidence.Snapshot(root) : null;
        Write(EngineeringRegistrationFixture.Path, registration.ToJsonString());
        switch (scenario)
        {
            case "template-activation":
                Write(InformationTemplateDebtStore.ActivationPath, DeclaredTemplateReviewTests.Text(
                    InformationTemplateDebtStore.WriteActivation(new(templateSeedRevision, true))));
                break;
            case "template-debt":
            case "template-missing-history":
                Write(AdmissionPlanePolicy.FileMapPath, File.ReadAllText(Path.Combine(root, AdmissionPlanePolicy.FileMapPath))
                    + "\n# Recheck the protected template debt without changing content.\n");
                break;
        ...
        RawLeanReportArtifact.WriteFile(report, candidateSnapshot, templateHistory
            ? TemplateReport(candidateSnapshot, fixture.Reports, templateDebt) : LeanAxiomReport.Create(fixture.Reports));
        foreach (var suffix in new[] { ".sha256", ".input.attestation", ".provenance.json" })
        ...
        if (templateHistory && scenario != "template-missing-history")
            foreach (var (revision, snapshot) in new[] { (templateSeedRevision, templateSeed!), (basis, templateProtected!) })
            {
                var historical = InformationTemplateEvidence.HistoricalInputs(snapshot, candidateSnapshot);
                RawLeanReportArtifact.WriteFile(Path.Combine(Path.GetDirectoryName(report)!, "information-template-history", revision,
                    "raw-lean-report.json"), historical, TemplateReport(historical, fixture.Reports, templateDebt));
            }
        ...
        if (templateHistory)
            Assert.DoesNotContain("historical evidence reader unavailable", console.Output + console.Error, StringComparison.Ordinal);
