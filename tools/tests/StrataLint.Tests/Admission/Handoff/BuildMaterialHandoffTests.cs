...
public sealed class BuildMaterialHandoffTests
{
    ...
    public void BuildTransportsSealedBinariesWithoutAnAnalysisHandoff(string scenario)
    {
        ...
        try
        {
            ...
            Directory.CreateDirectory(Path.Combine(producer, "Meta"));
            File.WriteAllText(Path.Combine(producer, EngineeringRegistrationFixture.Path), EngineeringRegistrationFixture.Manifest());
            Git(producer, "init", "-q");
            Git(producer, "add", ".");
