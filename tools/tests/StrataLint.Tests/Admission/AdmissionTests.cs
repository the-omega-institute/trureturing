...
public sealed class AdmissionTests
{
    ...
    [Fact]
    public void ProtectedSurfaceChangeCarriesAndRendersSl028ObservationWithoutChangingExitCode()
    {
        ...
        var protectedChange = Assert.IsType<AdmissionOutcome.ProtectedSurfaceChange>(
            AdmissionEngine.Decide(
                context.Policy,
                canonical.Capability,
                context.Lean,
                completed,
                context.MetaEvaluation));
        var observationsProperty = typeof(AdmissionOutcome.ProtectedSurfaceChange)
            .GetProperty("Observations");
        Assert.NotNull(observationsProperty);
        var observations = Assert.IsType<ImmutableArray<Diagnostic>>(
            observationsProperty!.GetValue(protectedChange));
        ...
        Assert.Equal(duplicatePath, observation.Path);

        var protectedConsole = new BufferedConsole();
        var protectedExit = CliApplication.Run(
            ["check"],
            new StubCliEnvironment(protectedChange),
            protectedConsole);
        var admittedConsole = new BufferedConsole();
        var admittedExit = CliApplication.Run(
            ["check"],
            new StubCliEnvironment(new AdmissionOutcome.Admitted(
                protectedChange.ContentCertificate,
                observations)),
            admittedConsole);

        Assert.Equal(3, protectedExit);
        Assert.Equal(0, admittedExit);
        Assert.Contains("SL-022", protectedConsole.Output, StringComparison.Ordinal);
        Assert.Contains("OBSERVED SL-028", protectedConsole.Output, StringComparison.Ordinal);
        var protectedDispositionLines = protectedConsole.Output
        ...
        var admittedDispositionLines = admittedConsole.Output
            .Split('\n', StringSplitOptions.RemoveEmptyEntries)
            .Where(static line => line.StartsWith("DEFERRED ", StringComparison.Ordinal)
                || line.StartsWith("OBSERVED ", StringComparison.Ordinal));
        Assert.Equal(admittedDispositionLines, protectedDispositionLines);
        Assert.True(
            protectedConsole.Output.IndexOf("SL-022", StringComparison.Ordinal)
            < protectedConsole.Output.IndexOf("OBSERVED SL-028", StringComparison.Ordinal));
    }
