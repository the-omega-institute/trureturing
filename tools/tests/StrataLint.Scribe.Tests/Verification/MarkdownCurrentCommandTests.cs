...
public sealed class MarkdownCurrentCommandTests
{
    ...
    public void CurrentChecksEveryDefinitionWithoutGitOrPublishedMarkdown()
    {
        ...
        var exit = Run(temporary, output, error, "--paths-from", PathsFile(temporary,
    ...
    public void CurrentRejectsInvalidPublishedMarkdownAndOrphans(string path, string diagnostic)
    {
        ...
        Assert.Equal(1, Run(temporary, TextWriter.Null, error, "--paths-from", PathsFile(temporary, path)));
    ...
    private static string PathsFile(TemporaryRoot temporary, string path)
    {
        var file = temporary.Resolve("selected-paths.txt");
        TemporaryFileSystem.File.WriteAllText(file, path + "\n");
        return file;
    }
