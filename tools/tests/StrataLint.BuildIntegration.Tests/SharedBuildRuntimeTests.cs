...
public sealed class SharedBuildRuntimeTests(Xunit.Abstractions.ITestOutputHelper diagnostics)
{
    ...
    public void CompilerOwnedRuntimeMovesAndExecutesWithoutProducerOrPackagePaths()
    {
        ...
        foreach (var pair in dotnetEnvironment) environment[pair.Key] = pair.Value;
        ...
        var runtime = Assert.Single(CommonExecutionEvidence.TestAssemblies(root, build));
        ...
        try
        {
            ...
            var assemblies = CommonExecutionEvidence.TestAssemblies(destination, received);
