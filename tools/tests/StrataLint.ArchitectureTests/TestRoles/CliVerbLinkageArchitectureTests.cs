...
public sealed class CliVerbLinkageTests
{
    ...
    private readonly record struct CallerSource(string RelativePath, string Contents);
    ...
    private static ImmutableArray<VerbInvocation> CollectInvocations(string root)
    {
        ...
        foreach (var source in CallerSources(root))
        {
            var relative = source.RelativePath;
            var physicalLines = source.Contents.Replace("\r\n", "\n", StringComparison.Ordinal).Split('\n');
    ...
    private static IEnumerable<CallerSource> CallerSources(string root)
    {
        yield return new CallerSource(
            "Makefile",
            TestRepositoryLayout.ReadAllText(RepositoryRelativePath.Create("Makefile")));
        yield return new CallerSource(
            "tools/Makefile",
            TestRepositoryLayout.ReadAllText(RepositoryRelativePath.Create("tools/Makefile")));

        var scripts = GitIndexRepositoryFiles
            .EnumerateDeclared(root, "tools/scripts")
            .Select(static file => new CallerSource(
                file.RelativePath,
                File.ReadAllText(file.FullPath)))
            .Concat(GitIndexRepositoryFiles
                .EnumerateDeclared(root, ".github/scripts")
                .Select(static file => new CallerSource(
                    file.RelativePath,
                    File.ReadAllText(file.FullPath))));
