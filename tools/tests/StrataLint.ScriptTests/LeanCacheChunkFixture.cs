...
internal sealed class LeanCacheChunkFixture : IDisposable
{
    ...
    internal LeanCacheChunkFixture(string archive = Archive)
    {
        ...
        foreach (var name in new[] { "lean-cache-publish.sh", "lean-cache-input.sh", "lean_cache_release.py", "lean_cache.py", "cache_material.py" })
        ...
        Write(Path.Combine(repository, "Trureturing.lean"), "import Mathlib\n");
        Write(Path.Combine(repository, "lean-toolchain"), "leanprover/lean4:v4.19.0\n");
        Write(Path.Combine(repository, "lakefile.toml"), "name = \"fixture\"\n");
        ScriptHarnessScratch.EnsureDirectory(Path.Combine(repository, "D5"));
        WriteStub("make",
