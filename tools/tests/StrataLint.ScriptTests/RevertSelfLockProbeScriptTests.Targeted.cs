...
public sealed partial class RevertSelfLockProbeScriptTests
{
    ...
    public void RealControllerExtractsOneCanonicalBlockerRecord()
    {
        ...
        var output = JsonNode.Parse(fixture.BlockersText())!.AsObject();
    ...
    public void RealTargetedRunnerWritesSemanticMissingEvidenceFromFilteredTrx()
    {
        ...
        Assert.True(result.ExitCode == 0, Diagnostics(result));
        Assert.Empty(result.StandardError);
        var supervisor = JsonNode.Parse(fixture.SupervisorText())!.AsObject();
        ...
        Assert.Equal("ExampleTests.Missing", blocker["test_id"]!.GetValue<string>());
        Assert.Contains("ExampleTests.Present", fixture.NormalizedTrxText(), StringComparison.Ordinal);
        Assert.DoesNotContain("ExampleTests.Missing", fixture.NormalizedTrxText(), StringComparison.Ordinal);
    }
}
