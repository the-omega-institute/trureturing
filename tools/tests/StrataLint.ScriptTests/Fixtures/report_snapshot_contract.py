...
class SnapshotContracts(CacheFixture, unittest.TestCase):
    ...
    def prepare_report(self):
        ...
        fixture = InspectorTests("test_inspector_reuses_validated_seed_without_build_or_reinspection")
        fixture.setUp()
        self.addCleanup(fixture.doCleanups)
...
''')
    ...
    def test_report_snapshot_keeps_only_current_complete_seed(self):
        ...
        self.assertEqual(["build"] * 3, [command for command in commands if command.startswith("build")])
        ...
        for case in cases:
            with self.subTest(case=case):
                if case == "missing-summary": summary.unlink()
                    ...
                    self.assertIn("mode=delta changed=1 added=0 removed=0 recheck=2", failed.stdout)
                    self.assertIn("LEAN_INSPECTOR_FAILED phase=build exit=19", failed.stderr)
                    self.assertEqual(bundle, fixture.bundle_bytes(fixture.output))
