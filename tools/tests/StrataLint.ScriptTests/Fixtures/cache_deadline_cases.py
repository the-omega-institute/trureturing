...
class CacheDeadlineCases:
    ...
    def test_cache_deadline_allows_pull_request_merge_ref_writes(self):
        for job, stage in (("build", "build"), ("engineering", "engineering"),
            ...
            with self.subTest(job=job):
                owner, env, epoch, clock, jobs, calls, fetch = self.deadline_fixture()
                env["GITHUB_JOB"] = jobs["jobs"][0]["name"] = job
                first = owner.begin(self.root, stage, 60, env=env, fetch_jobs=fetch,
                                    now=lambda: epoch, monotonic=lambda: clock[0])
                ...
                jobs["jobs"][0].update(head_sha="b" * 40)
                calls.clear()
    ...
    def test_cache_deadline_integration_push_retains_its_write_window(self):
        owner, env, epoch, clock, jobs, calls, fetch = self.deadline_fixture()
        env["GITHUB_REF"] = "refs/heads/integration-ci-tests"
        first = owner.begin(self.root, "current", 60, env=env, fetch_jobs=fetch,
                            now=lambda: epoch, monotonic=lambda: clock[0])
        self.assertEqual("available", first.reason)
        self.assertEqual(1, len(calls))
        loaded = owner.load_deadline(self.root, "current", env=env, monotonic=lambda: clock[0])
        self.assertEqual({"save_allowed": True, "save_timeout_minutes": 12}, owner.save_outputs(loaded))
