...
class PairFixture(PartitionFixture):
    def setUp(self):
        ...
        for name in ("Init.olean", "Lean.olean", "libleanshared.so"):
            write(self.root / "runtime/lib/lean" / name, "declared runtime fixture")
    ...
    def pair(self, **extra):
        return subprocess.run([str(self.root / "tools/scripts/lean-report-pair.sh"),
            ...
            text=True, capture_output=True, env={**os.environ,
                "STRATALINT_SUPERVISOR_ROOT": self.supervisor.name, "LEAN_BIN": str(self.lake),
...
''')
    ...
    def test_missing_empty_or_file_logs_fail_before_publication(self):
        write(self.producer, PAIR_PRODUCER + '''
owners = list(pathlib.Path(os.environ["STRATALINT_SUPERVISOR_ROOT"]).glob("slots/slot-*.lock/owner"))
with (root / "observed-slot-owners").open("a") as observation:
    observation.write(str(len(owners)) + "\\n")
shutil.rmtree(logs)
if os.environ["LOG_SHAPE"] == "file": logs.write_text("not a log directory")
if os.environ["LOG_SHAPE"] == "empty": logs.mkdir()
''')
        for index, shape in enumerate(("missing", "file", "empty"), 1):
            with self.subTest(log_shape=shape):
                self.output = self.root / shape / "raw-lean-report.json"
                result = self.pair(LOG_SHAPE=shape)
                diagnostics = result.stdout + result.stderr
                self.assertEqual(2, result.returncode, diagnostics)
                self.assertIn("lean-report-cache: producer left no log sidecar", result.stderr)
                self.assertEqual(index, self.runs(), diagnostics)
                self.assertEqual(["1"] * index, (self.root / "observed-slot-owners").read_text().splitlines())
                self.assertFalse(self.output.exists(), diagnostics)
                self.assertFalse(pathlib.Path(str(self.output) + ".provenance.json").exists(), diagnostics)
                self.assertEqual([], list(self.root.glob(".lean-report-bundle.*")), diagnostics)
                self.assertEqual([], self.seeds(), diagnostics)
...
class InspectorTests(PairFixture, unittest.TestCase):
    ...
    def current_report(self):
        ...
        environment = {**os.environ, "LAKE_BIN": str(self.lake), "LEAN_BIN": str(self.lake),
    ...
    def test_inspector_reuses_validated_seed_without_build_or_reinspection(self):
        self.output = self.root / "out/candidate-lean-report.json"
        first = self.pair()
        self.assertEqual(0, first.returncode, first.stdout + first.stderr)
        ...
        self.assertEqual(["build", "build +D5.A"],
                         [command for command in commands if command.startswith("build")])
...
''')
        ...
        build_log = pathlib.Path(str(self.output) + ".logs/build.stdout.log")
        receipts = [json.loads(line.removeprefix("LEAN_CACHE "))
                    for line in build_log.read_text().splitlines() if line.startswith("LEAN_CACHE ")]
