...
if [[ "$#" -ge 4 && "$1" == "$STRATALINT_LEAN_CLI_DLL" &&
      "$2" == worktree && "$3" == with-cache-writer && "$4" == -- ]]; then
  exec "$ANALYSIS_REAL_DOTNET" "$@"
fi
if [[ "$#" -ge 3 && "$1" == "$STRATALINT_LEAN_CLI_DLL" &&
      "$2" == worktree && "$3" == ensure-cache ]]; then
  exec "$ANALYSIS_REAL_DOTNET" "$@"
fi
if [[ "$1" == run ]]; then
  # The production wrapper uses `dotnet run` when no prebuilt CLI is
  # configured.  Fixture roots intentionally contain no CLI project, so
  # translate that launcher form to the compiled CLI under test while
  # preserving the command and its arguments after the final `--`.
  args=("$@")
  command_index=-1
  for ((i=${#args[@]}-1; i>=0; i--)); do
    if [[ "${args[$i]}" == -- ]]; then command_index=$i; break; fi
  done
  [[ "$command_index" -ge 0 ]] || exit 86
  exec "$ANALYSIS_REAL_DOTNET" "$STRATALINT_LEAN_CLI_DLL" "${args[@]:command_index+1}"
fi
exit 86
''')
    ...
    if not os.environ.get("LAKE_BUILD_FAIL"):
        ...
        target.write_text("current build output")''')
                ...
                env = dict(os.environ, PATH=str(runtime) + os.pathsep + os.environ["PATH"],
                    ...
                    STRATALINT_LEAN_CLI_DLL=str(cli),
...
class AnalysisCacheTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("lean-cache-ensure.sh", "lean-cache-input.sh", "lean-cache-publish.sh",
                     "lean_cache_release.py", "lean_cache.py", "cache_material.py"):
    ...
    def prepare(self, seed="absent"):
        # Install the fake Lake before publishing a seed.  Otherwise the
        # publish helper resolves the host elan Lake and may attempt a real
        # network download, making the fixture hang instead of exercising the
        # transport contract.
...
mkdir -p "$1/output"
bash "$1/tools/scripts/worktree/lean-cache-ensure.sh"
...
''', "analysis-caller", str(self.root), str(self.runner)], cwd=self.root,
    ...
    def assert_produced(self, result, archive_status):
        ...
        self.assertEqual(["exe cache get", "build LeanInformationAuditAnalysis"],
                         [line for line in runs if line in ("exe cache get", "build LeanInformationAuditAnalysis")])
    ...
    def test_absent_seed_reaches_analysis(self):
        self.prepare()
        # A missing Release snapshot is a structured cache miss; analysis
        # still runs from source and must produce the complete artifact set.
    ...
    def test_corrupt_seed_reaches_analysis(self):
        ...
        self.assertIn('"archive_status":"miss"', result.stdout)
    ...
    def test_preparation_failure_stops_before_analysis(self):
        ...
        self.assertIn(".lake exists but is not a directory", result.stdout + result.stderr)
...
if args[:1] == ["pack"]:
    ...
    with tarfile.open(archive, "w:gz") as bundle:
        bundle.add(root / ".lake/build", arcname=".lake/build")
    sys.exit(0)
if args[:1] == ["unpack"]:
