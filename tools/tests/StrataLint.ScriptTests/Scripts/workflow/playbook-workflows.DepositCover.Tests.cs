...
public sealed partial class PlaybookWorkflowsTests
{
    ...
    public void DepositFreezesASecondModuleUnderTheExistingAtomReceiptHost()
    {
        ...
        var primary = fixture.Run("deposit");
        Assert.True(primary.ExitCode == 0, Diagnostics(primary));
        var receipt = ReadFixtureBytes(fixture.ReceiptPath);
        fixture.AddSecondaryFormalization();
        fixture.ClearCalls();
        ...
        Assert.NotEqual(receipt, ReadFixtureBytes(fixture.ReceiptPath));
        using var document = JsonDocument.Parse(ReadFixtureBytes(fixture.ReceiptPath));
        ...
        Assert.Single(new DirectoryInfo(
            Path.GetDirectoryName(fixture.ReceiptPath)!).GetFiles("*.v1.json"));
        Assert.Empty(fixture.Status());
    }
    ...
    public void DepositRejectsAHostReceiptChangedAfterExtensionValidation()
    {
        ...
        Assert.Contains(
            ...
            Encoding.UTF8.GetString(secondary.StandardError),
            StringComparison.Ordinal);
        Assert.Equal(concurrentReceipt, ReadFixtureText(fixture.ReceiptPath));
    }
    ...
    private static byte[] ReadFixtureBytes(string path)
    {
        using var stream = File.OpenRead(path);
        using var reader = new BinaryReader(stream);
        return reader.ReadBytes(checked((int)stream.Length));
    }
