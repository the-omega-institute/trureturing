...
[ScriptSubject("tools/scripts/workflow/playbook-workflows.sh")]
public sealed partial class PlaybookWorkflowsTests
{
    ...
    public void CanonicalPlaybookVerbNeverExecutesBranchMerge(string command)
    {
        ...
        if (command is "deposit" or "cover")
        {
            ...
            if (command == "cover")
            {
                ...
                Assert.True(deposit.ExitCode == 0, PlaybookDiagnostics(deposit));
                fixture.ClearCalls();
            }

            var result = fixture.Run(command);

            Assert.True(result.ExitCode == 0, PlaybookDiagnostics(result));
    ...
    private static string PlaybookDiagnostics(ProcessOutput result) =>
