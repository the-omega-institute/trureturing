...
[ScriptSubject("tools/scripts/worktree/warm-donor.sh")]
public sealed class WarmDonorScriptTests
{
        ...
        string reason)
    {
        ...
        var calls = TemporaryFileSystem.File.ReadAllText(run.Calls)
    ...
    {
        var fixture = TemporaryFileSystem.Directory.CreateTempSubdirectory("warm-donor-tests-");
        try
        {
            ...
            var bin = Path.Combine(fixture.FullName, "bin");
            var calls = Path.Combine(fixture.FullName, "calls");
            TemporaryFileSystem.Directory.CreateDirectory(Path.GetDirectoryName(script)!);
            TemporaryFileSystem.Directory.CreateDirectory(bin);
            RepositoryAccessor.Discover(RepositoryRootCriterion.ClaudeDirectoryNotFound).CopyTo(
            ...
            var process = TestProcessRunner.Run(
                ...
                64 * 1024);
            return new ScriptRun(fixture.FullName, calls, process);
        }
        catch
        {
            TemporaryFileSystem.Directory.Delete(fixture.FullName, recursive: true);
            throw;
        }
    }
    ...
    {
        TemporaryFileSystem.File.WriteAllText(path, "#!/usr/bin/env bash\n" + content + "\n");
    ...
    {
        internal string[] CallLines => TemporaryFileSystem.File.ReadAllText(Calls)
            .Split('\n', StringSplitOptions.RemoveEmptyEntries);

        internal string CallsText => TemporaryFileSystem.File.ReadAllText(Calls);

        public void Dispose() => TemporaryFileSystem.Directory.Delete(Fixture, recursive: true);
    }
}
