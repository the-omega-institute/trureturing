...
public sealed class StandaloneLeanBootstrapScriptTests
{
    ...
    private sealed class BootstrapFixture : IDisposable
    {
        ...
        {
            const string projectName = "StrataLint.Lean";
            const string assemblyName = projectName;
            projectDirectory = "tools/" + projectName;
            producerDll = projectDirectory + "/bin/Release/net10.0/" + assemblyName + ".dll";
            producerVariable = "STRATALINT_LEAN_PRODUCER_DLL";
        ...
        internal void ChangeSource(string marker) => Write(projectDirectory + "/Program.cs", $$"""
            ...
            if (args is ["ensure-cache"]) return int.Parse(Environment.GetEnvironmentVariable("LEAN_BOOTSTRAP_PRODUCER_EXIT"));
            if (args.Length < 3 || args[0] != "with-cache-reader" || args[1] != "--") return 92;
