...
public sealed class StandaloneLeanBootstrapScriptTests
{
    ...
    private sealed class BootstrapFixture : IDisposable
    {
        ...
        {
            // Both wrappers launch the CLI; the target only changes the command
            // delegated by that CLI (lake build versus cache ensure).
            var projectName = "StrataLint.Cli";
            var assemblyName = "StrataLint";
            projectDirectory = "tools/" + projectName;
            producerDll = projectDirectory + "/bin/Release/net10.0/" + assemblyName + ".dll";
            producerVariable = "STRATALINT_LEAN_CLI_DLL";
