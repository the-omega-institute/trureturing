...
public sealed class FourierBridgeSourceEvidenceTests
{
        ...
        string sourcePath, string theoremName, int telescope, int state, int scope, int[] coordinates)
    {
        ...
        var cases = new[] { (Source: sourcePath, Name: theoremName, Telescope: telescope, State: state) };
        ...
        foreach (var item in cases)
        {
            ...
            foreach (var mutation in new[] { "state-out-of-scope", "wrong-source", "wrong-owner", "no-slot", "bad-path" })
            {
                ...
                switch (mutation)
                {
                    case "state-out-of-scope": changedBinding["readouts"]![0]!["state_binder"] = scope; break;
            ...
            var unimported = new Dictionary<string, LeanFileReport>(files) {
                [path] = files[path] with { Imports = [] }
            };
            Assert.Throws<FormatException>(() => InformationTemplateEvidence.Collect(joined,
                LeanAxiomReport.Create(unimported), selected));
