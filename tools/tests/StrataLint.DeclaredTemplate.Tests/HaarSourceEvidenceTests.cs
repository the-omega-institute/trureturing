...
public sealed class HaarSourceEvidenceTests
{
    [Fact]
    public void original_source_native_artifacts_pass_production_consumer_and_frozen_identity()
    {
        var directory = Environment.GetEnvironmentVariable("HAAR_SOURCE_ARTIFACTS");
        Skip.If(string.IsNullOrEmpty(directory), "Focused original/Reg native artifacts were not supplied.");
        const string owner = "D5.S3.Fourier.Asymptotics.TorusSubgroupHaarContinuity";
        ...
        Assert.Equal(5, binding["telescope_size"]!.GetValue<int>());
        Assert.Equal(1, binding["level_count"]!.GetValue<int>());
        Assert.Equal(new[] { 0 }, binding["coordinates"]!.AsArray().Select(n => n!.GetValue<int>()));
