...
[Collection("StrataLint.ResourcePlanning.Tests process boundary")]
    ...
    : IClassFixture<RegisteredAdmissionResourcesTests.RegisteredBase>
{
    ...
    public void CurrentAndPolicyPlansBuildTransportWithOnlyExplicitTests(string input, string mode)
    {
        ...
        Assert.Equal(input == "CLAUDE.md"
            ? new[] { "tools/tests/StrataLint.InstructionContract.Tests/StrataLint.InstructionContract.Tests.csproj", "tools/tests/StrataLint.PrScript.Tests/StrataLint.PrScript.Tests.csproj", RepositoryContractProject } : [],
    ...
    public void CommonBuildAndWorkflowProgramsRetainTheirRegisteredChecks(string input, string mode)
    {
        ...
        Assert.Equal(WithRepositoryContract(new[] { "StrataLint.ArchitectureTests", "StrataLint.Tests" }
    ...
    public void CachePathChangesRunOnlyRegisteredCacheConsumers(string mode)
    {
        ...
        Assert.Equal(WithRepositoryContract(new[] { "StrataLint.BuildIntegration.Tests", "StrataLint.Cache.Release.Tests", "StrataLint.Cache.Tests", "StrataLint.PlanningIntegration.Tests", "StrataLint.TransportIntegration.Tests" }.Select(name => $"tools/tests/{name}/{name}.csproj")), Strings(plan["execution"]!["tests"]!));
        Assert.Empty(plan["execution"]!["lean_targets"]!.AsArray());
        Assert.Equal(new[] { "filemap" }, Strings(plan["execution"]!["checks"]!));
        Assert.Equal(mode == "push" ? new[] { "filemap" } : ["lean-report", "filemap"],
            Strings(plan["execution"]!["steps"]!));
    ...
    public void RegisteredJudgeChangesKeepDeltaReachableWithOrWithoutNoResourceContent(string judge, bool mixed)
    {
        ...
        Assert.Equal(judge == "tools/scripts/preflight.sh"
            ? new[] { "architecture", "build", "current", "delta", "engineering-guards", "filemap", "lean", "lean-report", "scribe", "test-cli", "test-planning-integration", "test-repository-contract", "test-stage-integration", "test-workflow-script" }
            : ["build", "current", "delta", "engineering", "filemap", "lean", "lean-report", "scribe", "test-repository-contract"],
    ...
    public void PolicyDocumentsRetainAdmissionAndTheirExplicitConsumers(string document, string mode) =>
    ...
    {
        var plan = Plan("tools/StrataLint.Engine/Rules/CurrentCapacity.cs", document);
        var tests = Strings(plan["execution"]!["tests"]!);
        Assert.Equal(Strings(Plan("tools/StrataLint.Engine/Rules/CurrentCapacity.cs", "")["execution"]!["tests"]!), tests);
    ...
    private void AssertJudgeDocumentResources(string document, string mode, bool mixed)
    {
        ...
        Assert.Empty(plan["execution"]!["lean_targets"]!.AsArray());
        Assert.Equal(new[] { "filemap" }, Strings(plan["execution"]!["checks"]!));
        Assert.Equal(readsPrPolicy ? "required" : "not-required",
            plan["stages"]!["engineering"]!["status"]!.GetValue<string>());
        Assert.Equal(readsPrPolicy, Strings(plan["cache_layers"]!).Contains("engineering"));
        Assert.Equal(mixed ? 2 : 1, plan["paths"]!.AsArray().Count);
        if (mode == "push")
        {
        ...
        {
            Assert.Equal(new[] { "build", "delta-judge", "filemap", "lean", "lean-report" }.Concat(testResources),
                Strings(plan["resources"]!));
            Assert.Equal(new[] { "lean-report", "filemap" }, Strings(plan["execution"]!["steps"]!));
            Assert.Equal("required", plan["stages"]!["delta"]!["status"]!.GetValue<string>());
        }
    ...
    public void TheoryDocumentsDoNotRemoveOtherInputsRequirements(string input)
    {
        ...
        Assert.Equal(CommonCheckRegistrationFixture.Ids.Where(id => input is "Meta/ci-checks.json"
                || id is not ("banned-api-proof" or "capability-proof" or "selftest-pair")).Order(StringComparer.Ordinal),
            Strings(plan["execution"]!["checks"]!));
    }
    ...
    public void AdditionalSemanticOrJudgeInputRetainsItsFullRegisteredRequirements(string input)
    {
        ...
        Assert.Contains("scribe", Strings(plan["resources"]!));
        Assert.Equal(CommonCheckRegistrationFixture.Ids.Where(id => input is "Meta/ci-checks.json"
            || id is not ("banned-api-proof" or "capability-proof" or "selftest-pair")).Order(StringComparer.Ordinal), Strings(plan["execution"]!["checks"]!));
        Assert.Equal(new[] { "lean-report", "scribe", "filemap", "check-current" }, Strings(plan["execution"]!["steps"]!));
    }
    ...
    [InlineData("StrataLint.CiArtifacts.Tests", "TransportIdentityTests.cs", "test-ci-artifacts")]
    [InlineData("StrataLint.CliIntegration.Tests", "CurrentDeltaCliContractTests.cs", "test-cli-integration")]
    public void TestChangesSelectTheirRegisteredProjectWithoutUnrelatedTestExecution(string project, string file, string resource)
    {
        foreach (var mode in new[] { "push", "pr" })
        {
            ...
            Assert.Equal(WithRepositoryContract(new[] { $"tools/tests/{project}/{project}.csproj" }
    ...
    [Theory]
    [InlineData("StrataLint.Cli", "StrataLint.ArchitectureTests,StrataLint.CliIntegration.Tests,StrataLint.RepositoryTopology.Tests,StrataLint.Tests")]
    ...
    public void ProductionProjectsSelectTheirExplicitTestConsumers(string project, string assemblies)
    {
        foreach (var mode in new[] { "push", "pr" })
        {
            ...
            Assert.Equal(WithRepositoryContract(assemblies.Split(',').Select(name => $"tools/tests/{name}/{name}.csproj")),
                Strings(plan["execution"]!["tests"]!));
            Assert.DoesNotContain("engineering", Strings(plan["resources"]!));
            Assert.Contains("engineering-guards", Strings(plan["resources"]!));
    ...
    public void ProductionProjectConfigurationAlsoSelectsBuildConsumers(string project)
    {
        foreach (var file in new[] { project + ".csproj", "packages.lock.json" })
        {
            var plan = Plan($"tools/{project}/{file}", "");
            Assert.Contains("tools/tests/StrataLint.BuildIntegration.Tests/StrataLint.BuildIntegration.Tests.csproj",
                Strings(plan["execution"]!["tests"]!));
            Assert.Contains("current", Strings(plan["resources"]!));
            Assert.Equal(CommonCheckRegistrationFixture.Ids.Where(id => id.StartsWith("SL-", StringComparison.Ordinal)),
                Strings(plan["execution"]!["checks"]!).Where(id => id.StartsWith("SL-", StringComparison.Ordinal)));
    ...
    public void ProductionAndTestChangesKeepBothRegisteredConsumerSets()
    {
        ...
        Assert.Equal(WithRepositoryContract(new[] { "StrataLint.ArchitectureTests", "StrataLint.CliIntegration.Tests", "StrataLint.Engine.Tests", "StrataLint.RepositoryTopology.Tests", "StrataLint.Tests" }
            .Select(name => $"tools/tests/{name}/{name}.csproj")), Strings(plan["execution"]!["tests"]!));
        Assert.DoesNotContain("engineering", Strings(plan["resources"]!));
    }

    [Theory]
    [InlineData("tools/StrataLint.ReleaseSelection/TruthReleaseSelection.cs", "StrataLint.ArchitectureTests,StrataLint.ReleaseIntegration.Tests,StrataLint.ReleaseSelection.Tests,StrataLint.RepositoryTopology.Tests")]
    [InlineData("tools/tests/StrataLint.ReleaseSelection.Tests/TruthReleaseSelectionTests.cs", "StrataLint.ArchitectureTests,StrataLint.ReleaseSelection.Tests,StrataLint.RepositoryTopology.Tests")]
    [InlineData("tools/tests/StrataLint.ReleaseIntegration.Tests/ReleaseCommandTests.cs", "StrataLint.ArchitectureTests,StrataLint.ReleaseIntegration.Tests,StrataLint.RepositoryTopology.Tests")]
    ...
    public void SharedTestSupportSelectsItsDeclaredConsumerProjects()
    {
        ...
        Assert.Equal(WithRepositoryContract(new[] { "JudgeSeedTask.Tests", "StrataLint.ArchitectureTests", "StrataLint.BuildIntegration.Tests", "StrataLint.BuildRuntime.Tests", "StrataLint.Cache.Native.Tests", "StrataLint.Cache.Release.Tests", "StrataLint.Cache.Tests", "StrataLint.CheckIntegration.Tests", "StrataLint.CiArtifacts.Tests", "StrataLint.CliIntegration.Tests", "StrataLint.Configuration.Tests", "StrataLint.DeclaredTemplate.Tests", "StrataLint.Digestion.Tests", "StrataLint.Engine.Tests", "StrataLint.ExecutionEvidence.Tests", "StrataLint.FileMap.Tests", "StrataLint.HeaderScript.Tests", "StrataLint.InspectionIntegration.Tests", "StrataLint.InspectionScope.Tests", "StrataLint.InstructionContract.Tests", "StrataLint.Lean.Tests", "StrataLint.LeanCacheScript.Tests", "StrataLint.LeanReportScript.Tests", "StrataLint.NativeTransportIntegration.Tests", "StrataLint.PlanningIntegration.Tests", "StrataLint.PlaybookScript.Tests", "StrataLint.PrScript.Tests", "StrataLint.ReleaseIntegration.Tests", "StrataLint.ReleaseSelection.Tests", "StrataLint.ReportSupervisor.Tests", "StrataLint.RepositoryConfiguration.Tests", "StrataLint.RepositoryTopology.Tests", "StrataLint.ResourceObservation.Tests", "StrataLint.ResourcePlanning.Tests", "StrataLint.Rules.Tests", "StrataLint.Scribe.Tests", "StrataLint.StageIntegration.Tests", "StrataLint.Tests", "StrataLint.TransportIntegration.Tests", "StrataLint.WorkflowScript.Tests" }.Select(name => $"tools/tests/{name}/{name}.csproj")), Strings(plan["execution"]!["tests"]!));
        Assert.DoesNotContain("engineering", Strings(plan["resources"]!));
    }

    [Theory]
    [InlineData("StrataLint.LeanTestSupport/LeanReportRegistrationFixture.cs", "StrataLint.ArchitectureTests,StrataLint.LeanReportScript.Tests,StrataLint.RepositoryTopology.Tests,StrataLint.Tests")]
    [InlineData("StrataLint.ConfigurationTestSupport/TestRegistry.cs", "StrataLint.ArchitectureTests,StrataLint.CliIntegration.Tests,StrataLint.Configuration.Tests,StrataLint.DeclaredTemplate.Tests,StrataLint.Digestion.Tests,StrataLint.FileMap.Tests,StrataLint.RepositoryConfiguration.Tests,StrataLint.RepositoryTopology.Tests,StrataLint.Rules.Tests,StrataLint.Tests")]
    [InlineData("StrataLint.AdmissionTestSupport/ProducerInputFixture.cs", "StrataLint.ArchitectureTests,StrataLint.CliIntegration.Tests,StrataLint.PlaybookScript.Tests,StrataLint.RepositoryTopology.Tests,StrataLint.Tests")]
    [InlineData("StrataLint.ProcessTestSupport/TestProcessRunner.cs", "StrataLint.ArchitectureTests,StrataLint.CliIntegration.Tests,StrataLint.Engine.Tests,StrataLint.Lean.Tests,StrataLint.LeanCacheScript.Tests,StrataLint.LeanReportScript.Tests,StrataLint.PlaybookScript.Tests,StrataLint.PrScript.Tests,StrataLint.ReportSupervisor.Tests,StrataLint.RepositoryTopology.Tests,StrataLint.ResourceObservation.Tests,StrataLint.Tests,StrataLint.WorkflowScript.Tests")]
    [InlineData("StrataLint.RegistrationTestSupport/EngineeringRegistrationFixture.cs", "StrataLint.ArchitectureTests,StrataLint.BuildIntegration.Tests,StrataLint.CheckIntegration.Tests,StrataLint.CliIntegration.Tests,StrataLint.DeclaredTemplate.Tests,StrataLint.Digestion.Tests,StrataLint.Engine.Tests,StrataLint.NativeTransportIntegration.Tests,StrataLint.PlanningIntegration.Tests,StrataLint.ReleaseIntegration.Tests,StrataLint.RepositoryTopology.Tests,StrataLint.ResourcePlanning.Tests,StrataLint.Rules.Tests,StrataLint.Scribe.Tests,StrataLint.StageIntegration.Tests,StrataLint.Tests,StrataLint.TransportIntegration.Tests")]
    ...
    [InlineData("StrataLint.NativeReportTestSupport/NativeReportFixture.cs", "StrataLint.ArchitectureTests,StrataLint.NativeTransportIntegration.Tests,StrataLint.RepositoryTopology.Tests")]
    [InlineData("StrataLint.RuleTestSupport/RuleFixture.cs", "StrataLint.ArchitectureTests,StrataLint.CliIntegration.Tests,StrataLint.DeclaredTemplate.Tests,StrataLint.Digestion.Tests,StrataLint.RepositoryTopology.Tests,StrataLint.Rules.Tests,StrataLint.Tests")]
    ...
    [Theory]
    [InlineData("source_reference.py")]
    [InlineData("truth_release.py")]
    public void ReleaseWorkflowChangesSelectOnlyTheirReleaseConsumers(string script)
    {
        foreach (var mode in new[] { "push", "pr" })
        {
    ...
    {
        var plan = Plan(path, "");
        Assert.Equal(WithRepositoryContract(new[] { "tools/tests/StrataLint.ArchitectureTests/StrataLint.ArchitectureTests.csproj", RepositoryTopologyProject }), Strings(plan["execution"]!["tests"]!));
    ...
    [Theory]
    [InlineData("StrataLint.ResourcePlanning", "StrataLint.ResourcePlanning.Tests", "StrataLint.BuildRuntime.Tests")]
    [InlineData("StrataLint.InspectionScope", "StrataLint.InspectionScope.Tests", "StrataLint.BuildRuntime.Tests")]
    [InlineData("StrataLint.ExecutionEvidence", "StrataLint.ExecutionEvidence.Tests", "StrataLint.ReleaseSelection.Tests")]
    [InlineData("StrataLint.BuildRuntime", "StrataLint.BuildRuntime.Tests", "StrataLint.ResourcePlanning.Tests")]
    [InlineData("StrataLint.CiArtifacts", "StrataLint.CiArtifacts.Tests", "StrataLint.BuildRuntime.Tests")]
    public void SplitProductionChangesExecuteTheirOwnedTestsWithoutSelectingCompileReferences(
        string project, string owner, string unrelated)
    {
        foreach (var mode in new[] { "push", "pr" })
        {
            var plan = Plan($"tools/{project}/Changed.cs", "", mode);
            var tests = Strings(plan["execution"]!["tests"]!);
            Assert.Contains($"tools/tests/{owner}/{owner}.csproj", tests);
            Assert.DoesNotContain($"tools/tests/{unrelated}/{unrelated}.csproj", tests);
            Assert.DoesNotContain("tools/tests/StrataLint.Engine.Tests/StrataLint.Engine.Tests.csproj", tests);
            Assert.DoesNotContain("engineering", Strings(plan["resources"]!));
        }
    }
    ...
    public void SharedConfigurationFixtureSelectsItsConsumers()
    {
        ...
        Assert.Equal(WithRepositoryContract(new[] { "StrataLint.CliIntegration.Tests", "StrataLint.Configuration.Tests", "StrataLint.DeclaredTemplate.Tests", "StrataLint.Digestion.Tests", "StrataLint.Rules.Tests", "StrataLint.Tests" }
            .Select(name => $"tools/tests/{name}/{name}.csproj")), Strings(plan["execution"]!["tests"]!));
    }
        ...
        public RegisteredBase()
        {
            var result = Python(Path, """
                ...
                for path in sorted(materials):
                    target = root / path
                    target.parent.mkdir(parents=True, exist_ok=True)
                    documents_read = {name for name, _ in documents} | {
                        'Meta/ci-checks.json', 'Meta/ci-resources.json', 'Meta/engineering-projects.json'}
                    target.write_bytes((source / path).read_bytes() if path in documents_read
                        else b'registered fixture material\n')
