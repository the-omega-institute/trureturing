    ...
    : IClassFixture<RegisteredAdmissionResourcesTests.RegisteredBase>
{
    ...
    public void CurrentAndPolicyPlansBuildTransportWithOnlyExplicitTests(string input, string mode)
    {
        ...
        Assert.Equal(input == "CLAUDE.md"
            ? new[] { "tools/tests/StrataLint.PrScript.Tests/StrataLint.PrScript.Tests.csproj", RepositoryContractProject } : [],
    ...
    private void AssertJudgeDocumentResources(string document, string mode, bool mixed)
    {
        ...
        var testResources = readsPrPolicy ? new[] { "test-pr-script", "test-repository-contract" } : [];
    ...
    {
        var plan = Plan(path, "");
        Assert.Equal(WithRepositoryContract(new[] { "tools/tests/StrataLint.ArchitectureTests/StrataLint.ArchitectureTests.csproj" }), Strings(plan["execution"]!["tests"]!));
