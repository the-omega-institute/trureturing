...
namespace StrataLint.CheckIntegration.Tests;

[Collection("StrataLint.CheckIntegration.Tests process boundary")]
public sealed partial class CommonCheckExecutionTests
{
    ...
    {
        using var fixture = new Fixture();
        fixture.Tree.Write(ReportInputsFixture.NativeRegistration, "{\"producer_scopes\":{\"lean-report\":{\"include\":[],\"exclude\":[]}}}");
        fixture.Tree.Write("Meta/ReportProducers/check.json", ReportInputsFixture.Producer("lean-report"));
        ...
        var current = Run(calls.Add);
        Assert.Equal(expected, calls.Count);
        Assert.Equal(damage is "candidate" or "round" ? original.Units.Select(row => row.Id) : new[] { "SL-001" }, calls);
        Assert.All(current.Units, row => Assert.Equal(calls.Contains(row.Id) ? "executed" : "reused", row.Status));
        CommonCheckRecord Run(Action<string>? executed = null)
        {
    ...
    [Fact]
    public void CurrentColdThenWarmRunsZeroUnitsAndOneChangedPredicateRunsAlone()
    {
        using var fixture = new Fixture();
