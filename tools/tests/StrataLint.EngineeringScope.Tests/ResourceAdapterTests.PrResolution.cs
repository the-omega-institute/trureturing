...
public sealed partial class ResourceAdapterTests
{
    ...
    public void PlannerRejectsStageOnlyScopeBeforeIssuingNoWork(string eventName)
    {
        ...
        {
            ["schema_version"] = 1, ["mode"] = "current",
            ["candidate"] = new JsonObject { ["commit"] = fixture.Commit,
                ["tree"] = SharedBuildContractTests.Git(fixture.Root, "rev-parse", "HEAD^{tree}") },
            ["base"] = null, ["head"] = null, ["complete"] = true,
            ["change_count"] = 0, ["changes"] = new JsonArray(),
        };
        fixture.Write("build/rejected-changes.json", changes.ToJsonString());
        var result = SharedBuildContractTests.Process(fixture.Root, "python3",
            ["-B", "tools/scripts/workflow/ci.py", "plan", "--repository", fixture.Root,
                "--commit", fixture.Commit, "--changes", "build/rejected-changes.json", "--output", "build/rejected-plan.json"],
            environment, TestBudgets.WorkflowProcessHangGuard);
        Assert.True(result.Exit == 2, result.Text);
        Assert.Contains("invalid changed scope version/mode", result.Text, StringComparison.Ordinal);
        Assert.False(File.Exists(Path.Combine(fixture.Root, "build/rejected-plan.json")));
    }

    [Fact]
    public void PullRequestPlannerCannotSubstituteAnEmptyPushRange()
    {
        ...
        var changes = new JsonObject
        {
            ["schema_version"] = 1, ["mode"] = "push",
            ["candidate"] = new JsonObject { ["commit"] = fixture.Commit,
                ["tree"] = SharedBuildContractTests.Git(fixture.Root, "rev-parse", "HEAD^{tree}") },
            ["base"] = null, ["head"] = null, ["complete"] = true,
            ["change_count"] = 0, ["changes"] = new JsonArray(),
            ["origin"] = new JsonObject { ["kind"] = "event-range", ["before"] = fixture.Commit, ["after"] = fixture.Commit },
        };
        fixture.Write("build/rejected-changes.json", changes.ToJsonString());
        var result = SharedBuildContractTests.Process(fixture.Root, "python3",
            ["-B", "tools/scripts/workflow/ci.py", "plan", "--repository", fixture.Root,
            ...
            environment, TestBudgets.WorkflowProcessHangGuard);
        Assert.True(result.Exit == 2, result.Text);
        Assert.Contains("pull request requires its complete PR scope", result.Text, StringComparison.Ordinal);
        Assert.False(File.Exists(Path.Combine(fixture.Root, "build/rejected-plan.json")));
    }
