...
public sealed partial class ResourceAdapterTests
{
    ...
    [Theory]
    [InlineData("current", "stage-start", 0)]
    [InlineData("current", "lean-production", 2)]
    [InlineData("lean", "lean-production", 0)]
    public void CacheActivationDefersHeavyLayersUntilPreparedOrStandaloneLean(string resource, string phase, int expected)
    {
        ...
        map = string.Join('\n', map.Split('\n').Select(line => line.Contains("id = \"lean\"", StringComparison.Ordinal)
            ? line.Replace("cache_layers = [], cache_activation = {}", "cache_layers = [\"dependency\", \"elan\", \"project\"], cache_activation = {dependency = \"lean-production\", elan = \"stage-start\", project = \"lean-production\"}", StringComparison.Ordinal)
            : line.Contains("id = \"lean-report\"", StringComparison.Ordinal)
                ? line.Replace("cache_layers = [], cache_activation = {}", "cache_layers = [\"report\"], cache_activation = {report = \"stage-start\"}", StringComparison.Ordinal)
                : line));
        ...
        environment["STRATALINT_LEAN_REPORT_PREPARATION"] = "";
        environment["STRATALINT_REPORT_CACHE_ROOT"] = Path.Combine(fixture.Root, "build/report-cache");
        var result = SharedBuildContractTests.Process(fixture.Root, Path.Combine(fixture.Root, "build/adapter-bin/python3"),
            ["-B", Path.Combine(TestRepositoryLayout.FindRoot(), "tools/scripts/worktree/lean_actions.py"), "keys",
                "--repository", fixture.Root, "--stage", "current", "--phase", phase], environment, TestBudgets.WorkflowProcessHangGuard);
        Assert.True(result.Exit == expected, result.Text);
        if (expected == 2) Assert.Contains("requires explicit report preparation", result.Text, StringComparison.Ordinal);
        else if (phase == "stage-start")
        {
            Assert.Contains("report_key=", result.Text, StringComparison.Ordinal);
            Assert.Contains("elan_key=", result.Text, StringComparison.Ordinal);
            foreach (var layer in new[] { "dependency", "project" }) Assert.DoesNotContain(layer + "_key=", result.Text, StringComparison.Ordinal);
        }
        else
        {
            Assert.Contains("dependency_key=", result.Text, StringComparison.Ordinal);
            Assert.Contains("project_key=", result.Text, StringComparison.Ordinal);
    ...
    public void PrepareCurrentConsumesTheExplicitPlanWithoutRequiringReportWork(string resource, bool build)
    {
        using var fixture = new ResourceRouteTests.ResourceFixture(resource == "none" ? [] : [resource]);
        var environment = EnvironmentFor(fixture);
        ...
        var result = SharedBuildContractTests.Process(fixture.Root, Path.Combine(fixture.Root, "build/adapter-bin/python3"),
            ["-B", "tools/scripts/workflow/ci.py", "prepare-current", "--repository", fixture.Root,
                "--commit", fixture.Commit, "--stage", "current", "--plan", fixture.Plan, "--changes", fixture.Changes], environment);
        Assert.True(result.Exit == 0, result.Text);
        Assert.Equal(build, JsonNode.Parse(result.Text)!["needs_lean_build"]!.GetValue<bool>());
        Assert.False(Directory.Exists(Path.Combine(fixture.Root, ".lake")));
    }
