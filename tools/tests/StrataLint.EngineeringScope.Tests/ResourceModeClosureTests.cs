...
public sealed class ResourceModeClosureTests(Xunit.Abstractions.ITestOutputHelper output)
{
    ...
    [InlineData("pr", "delta", "build,current,delta,lean,lean-report", "build,current,delta", "dotnet,git,lake", "current,judge,project", "lean-report,check-current")]
    [InlineData("push", "current,delta", "build,current,lean,lean-report", "build,current", "dotnet,lake", "current,judge,project", "lean-report,check-current")]
    [InlineData("pr", "current,delta", "build,current,delta,lean,lean-report", "build,current,delta", "dotnet,git,lake", "current,judge,project", "lean-report,check-current")]
    [InlineData("push", "delta,engineering", "build,engineering", "build,engineering", "dotnet", "engineering,judge", "")]
    [InlineData("pr", "delta,engineering", "build,current,delta,engineering,lean,lean-report", "build,engineering,current,delta", "dotnet,git,lake", "current,engineering,judge,project", "lean-report,check-current")]
    [InlineData("push", "current", "build,current,lean,lean-report", "build,current", "dotnet,lake", "current,judge,project", "lean-report,check-current")]
    [InlineData("pr", "current", "build,current,lean,lean-report", "build,current", "dotnet,lake", "current,judge,project", "lean-report,check-current")]
        ...
        string? invalidDependency = null)
    {
        ...
        string Row(string id, string stage, string dependency, string tool, string cache) =>
            ...
            + "\"], tools = [\"" + tool + "\"], cache_layers = " + (cache.Length == 0 ? "[]" : "[\"" + cache + "\"]")
            + ", cache_activation = " + (cache.Length == 0 ? "{}" : "{" + cache + " = \"stage-start\"}") + ", " + materials + " },\n";
        ...
        foreach (var (id, tool, cache) in new[] { ("build", "dotnet", "judge"), ("current", "dotnet", "current"), ("lean", "lake", "project"), ("lean-report", "lake", "project") })
            for (var i = 0; i < rows.Length; i++)
                if (rows[i].Contains("id = \"" + id + "\"", StringComparison.Ordinal))
                {
                    rows[i] = rows[i].Replace("tools = []", "tools = [\"" + tool + "\"]", StringComparison.Ordinal)
                        .Replace("cache_layers = [], cache_activation = {}", "cache_layers = [\"" + cache + "\"], cache_activation = {" + cache + " = \"stage-start\"}", StringComparison.Ordinal);
