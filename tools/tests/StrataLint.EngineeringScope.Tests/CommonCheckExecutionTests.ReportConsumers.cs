...
public sealed partial class CommonCheckExecutionTests
{
    ...
    public void TransportInputChangeReusesChecksAgainstTheValidatedReport(bool changeDeclaration)
    {
        ...
        foreach (var path in new[] { ReportInputsFixture.LeanProducer, ReportInputsFixture.ScribeProducer })
        {
            var producer = JsonNode.Parse(File.ReadAllText(Path.Combine(fixture.Root, path)))!;
            producer["scripts"] = new JsonArray("fixtures/transport.py");
            fixture.Tree.Write(path, producer.ToJsonString());
        }
        ...
        if (changeDeclaration)
        {
            ...
            {
                var producer = JsonNode.Parse(File.ReadAllText(Path.Combine(fixture.Root, path)))!;
                producer["scripts"]!.AsArray().Add("fixtures/extra-transport.py");
                fixture.Tree.Write(path, producer.ToJsonString());
            }
        }
        fixture.Tree.Track();
