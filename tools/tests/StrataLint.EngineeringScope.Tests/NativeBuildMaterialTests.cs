...
public sealed class NativeBuildMaterialTests(ITestOutputHelper output)
{
    ...
    public void NativeSelectionRelocatesTransitiveRuntimeAndRejectsMissingOrCorruptAssets()
    {
        ...
        foreach (var assembly in new[] { CommonExecutionEvidence.CliPath, CommonExecutionEvidence.RunnerPath,
            ...
            foreach (var runtime in new[] { assembly, Path.ChangeExtension(assembly, ".deps.json"), Path.ChangeExtension(assembly, ".runtimeconfig.json") })
            {
                Assert.Contains(runtime, paths);
                output.WriteLine("REQUIRED_RUNTIME " + runtime);
            }
        ...
        try
        {
            ...
            var lean = SharedBuildContractTests.Process(consumer.Root, "dotnet", [CommonExecutionEvidence.LeanProducerPath], environment);
            Assert.True(lean.Exit == 0, lean.Text);
            Assert.Contains("bonjour", lean.Text, StringComparison.Ordinal);
            output.WriteLine("RELOCATED_LEAN_RUNTIME exit=" + lean.Exit + " producer_available=" + Directory.Exists(root) + "\n" + lean.Text);
...
internal sealed class NativeMaterialFixture : IDisposable
{
    ...
    internal NativeMaterialFixture(ITestOutputHelper output)
    {
        ...
        Run("dotnet", "sln", "tools/StrataLint.sln", "add", TestProject, excluded,
            ...
            "tools/StrataLint.Lean/StrataLint.Lean.csproj",
