...
internal static partial class CleanLanesCommand
{
    ...
    private static DateTimeOffset? TryReadBirthtime(string gitDirectory)
    {
        try
        {
            var value = Directory.GetCreationTimeUtc(gitDirectory);
            if (value <= DateTime.UnixEpoch) return null;
