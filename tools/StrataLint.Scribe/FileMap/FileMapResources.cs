...
internal static partial class FileMapLoader
{
    ...
    private static readonly string[] ResourceCaches = ["dependency", "project", "report", "judge", "elan", "engineering", "current"];
    ...
    private static ImmutableArray<FileMapResource> ParseResources(TomlTable root, string location)
    {
        ...
        var resources = tables.Select(table =>
        {
            ...
            var phases = activation.ToImmutableDictionary(pair => pair.Key, pair => pair.Value is string phase
                && phase is "stage-start" or "lean-production" ? phase : throw Invalid(id, "unknown cache activation"), StringComparer.Ordinal);
            if (stage != "current" && phases.Values.Contains("lean-production", StringComparer.Ordinal))
                throw Invalid(id, "lean-production cache activation requires current stage");
