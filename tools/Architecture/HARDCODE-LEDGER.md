...
| csproj/props repetition | The same test SDK/xUnit bundle appears in `StrataLint.Tests.csproj:10-16`, `StrataLint.Scribe.Tests.csproj:11-16`, `StrataLint.Scribe.Documents.Tests.csproj:15-20`, and `StrataLint.ArchitectureTests.csproj:20-28`. | Central package management prevents per-project version copies, but item-group duplication is allowed. | `HC-OPEN-007`: the exact four-project bundle can be centralized in evaluated MSBuild targets, but a generic duplicate-item predicate cannot know project-specific asset metadata. Treat as a scoped build refactor with lock-file verification. |
...
| `Blueprint/D5/**/*.scribe.cs` and `StrataLint.Scribe.Documents.Tests/**/*.cs` | These are typed D5 document data and tests of those documents, not a generic theory registry. |
