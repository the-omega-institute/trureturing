...
|---|---|---|---|
| FILEMAP custody and producer policy | `Meta/FILEMAP.toml` is parsed fail-closed; every tracked/unignored path matches exactly one pattern, FILEMAP entries own path membership and Evidence policy, generated files match the producer inventory and name that producer, data names an existing program verifier, and class directories remain pure. Residence policy states the no-data-under-`tools/` ideal and now freezes the closed zero-violation state. | Yes: unclassified, ambiguous, missing registered references, missing loader, missing/mismatched producer, missing generated `produced_by`, directory mixing, unmarked protected-surface data, exact residence inventory drift, data-to-generated reference, and Lean-to-generated import red fixtures. | Hand-maintained or implicit file ownership, generated outputs without an executable producer, and unmarked or newly added protected-surface data. The bounded dependency predicate remains recorded under `HC-OPEN-012`; `RESIDENCE-EPOCH` is closed. |
...
| SL-018 Machine-produced values | Values use `Evidence/D5/values/<encoded-id>.value.json`; `ValuesProjectionAddress` round-trips strict UTF-8 keys as lowercase hex prefixed with `k`, bounded to 120 input bytes. The producer checks its computed key/path/payload/GID binding and includes the complete normalized row input. SL-018 validates authoritative Lean GID, kind, standard three axioms and statement binding; it does not police tracked projection bytes. | Yes: synthetic key, route, payload-substitution, complete-byte locality and inventory fixtures, plus stale tracked bytes remain admitted. | Noncanonical values addresses and invalid authoritative Lean bindings. `emit-values --check` checks producer determinism only. Numeric results explicitly remain `not-kernel-evaluated:noncomputable-real`; regeneration uses the catalog, and membership is derived without numerical evaluation. |
...
|---|---|
| `StrataLint.Scribe/Values/ValuesKernelDataLoader.cs`, `StrataLint.Scribe/Writers/CanonicalValuesWriter.cs`, and `StrataLint.Engine/Coordinates/ValuesProjectionAddress.cs` | These constants bind the D5 values pipeline specifically: D5 Lean truth, D5 kernel data, and the D5 evidence projection. The constant ID set lives only in catalog data; shard addresses are reversible encodings of those IDs, not a member list. A future theory needs its own admitted source before sharing this path. |
| `tools/TestSupport/StrataLint.CliTestSupport/RuleFixture.cs` and `tools/tests/StrataLint.Tests/Rules/TestFileMap.cs` | The conservative corpus is an explicit synthetic fixture for the sole instantiated M0 theory. Those D5 examples remain valid fixtures after another theory is admitted. |
...
| `Meta/FILEMAP.toml` `evidence.artifact_kinds` | `csv` has no object but is a reserved member of the Evidence GID `--tag` alphabet | 0 |
...
| `agents/` charters | 11 files, each externally referenced and registered in `Meta/FILEMAP.toml` | 0 |
