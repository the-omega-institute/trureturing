...
internal sealed class ResourceExecutionPlan
{
    ...
    internal string[] ChangedPaths => Document.GetProperty("paths").EnumerateArray()
        .Select(row => row.GetProperty("path").GetString()!).ToArray();
    ...
    internal ResourcePlanBinding Retain(string root, string destinationRoot)
    {
        var plan = destinationRoot + "/resource-plan.json";
        var changes = destinationRoot + "/resource-scope.json";
