...
public static class RepositoryCanonicalizer
{
        ...
        RawChangeSet? changes)
    {
        ...
        try
        {
            ...
            if (changes is null || changes.Paths.Any(path => FileMapDocuments.IsPolicyPath(path.Value)))
            {
                var snapshotFileMap = FileMapCanonicalWriter.Write(FileMapLoader.LoadSnapshot(snapshot));
                if (!snapshotFileMap.AsSpan().SequenceEqual(policy.CanonicalFileMapBytes.AsSpan()))
                    throw new FormatException("Repository FILEMAP policy differs from the validated canonical policy.");
