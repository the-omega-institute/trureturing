...
{
    private static readonly string RetiredRelationshipKey = "coverage_" + "gids";
    ...
    {
        if (entry.ContainsKey("coverage"))
        {
            RequireExactMigrationKeys(
                entry,
                ["fingerprints", "cas_ref", "coverage", "receipts"],
                path);
            foreach (var rawEdge in MigrationList(entry, "coverage", path))
            {
        ...
        RequireExactMigrationKeys(
            entry,
            ["fingerprints", "cas_ref", RetiredRelationshipKey, "receipts"],
            path);
        var orderedGids = new List<string>();
        var seenGids = new HashSet<string>(StringComparer.Ordinal);
        foreach (var rawGid in MigrationList(entry, RetiredRelationshipKey, path))
        {
        ...
        entry.Remove(RetiredRelationshipKey);
        entry.Add(
            "coverage",
