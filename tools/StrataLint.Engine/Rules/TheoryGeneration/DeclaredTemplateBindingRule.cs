...
internal static class DeclaredTemplateBindingRule
{
    ...
    internal static ImmutableArray<RuleFinding> Evaluate(DeltaRuleContext context)
    {
        ...
        foreach (var path in InformationTemplateSelection.ChangedProducers(context))
        {
            ...
            Assess(path, () => InformationTemplateSelection.IsRegSource(path)
                ? InformationTemplateSelection.CollectProducer(context.Current, context.Lean.Report, path)
                : InformationTemplateEvidence.Collect(context.Current, context.Lean.Report, [path]));
