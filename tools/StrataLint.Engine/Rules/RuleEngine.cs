...
public sealed class CurrentRuleContext
{
    ...
    internal AcceptedLeanClosure Lean => lean ?? throw new InvalidOperationException("selected current predicate requires Lean evidence");
    ...
    internal static CurrentRuleContext CreateWithoutLean(RepositorySnapshot current, ValidatedPolicy policy,
        VerifiedScribeEmissions? emissions, CurrentRuleSelection selection) =>
        new(current, policy, null, emissions, selection);
}
