...
internal static class FileMapSymlinkPolicy
{
    ...
    internal static ImmutableArray<FileMapSymlink> Parse(ReadOnlySpan<byte> bytes, string location)
    {
        ...
        if (!root.TryGetValue("schema_version", out var version) || version is not (2L or 3L))
            throw Invalid(location, "symlink declarations require schema_version 2 or 3");
        ...
        var files = rawFiles switch
        {
            ...
            TomlArray array when version is 3L && array.All(static value => value is TomlTable)
                => array.Cast<TomlTable>().ToArray(),
            _ => throw Invalid(location, "files must be tables; inline arrays require schema_version 3"),
        };
