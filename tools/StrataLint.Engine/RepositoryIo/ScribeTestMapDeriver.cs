...
internal static class ScribeTestMapDeriver
{
        ...
        HashSet<TestMapUnknownReason> reasons)
    {
        ...
        ExpressionSyntax chain = invocation;
        while (chain.Parent is ExpressionSyntax parent)
        {
            chain = parent;
        }

        var boundedParameter = chain.DescendantNodesAndSelf()
            .OfType<InvocationExpressionSyntax>()
            .Select(static candidate => candidate.Expression is MemberAccessExpressionSyntax
                {
                    Expression: MemberAccessExpressionSyntax
                    {
                        Expression: IdentifierNameSyntax file,
                        Name.Identifier.ValueText: "RelativePath",
                    },
                    Name.Identifier.ValueText: "StartsWith",
                }
                && candidate.ArgumentList.Arguments.FirstOrDefault()?.Expression
                    is LiteralExpressionSyntax
                    {
                        RawKind: (int)SyntaxKind.StringLiteralExpression,
                    } prefix
                && prefix.Token.ValueText == "tools/tests/"
                    ? file.Identifier.ValueText
                    : null)
            .FirstOrDefault(static parameter => parameter is not null);
        if (boundedParameter is null)
        {
            return false;
        }

        var hasModeledContentRead = chain.DescendantNodesAndSelf()
            .OfType<InvocationExpressionSyntax>()
            .Any(candidate => IsBoundedTrackedContentRead(candidate, boundedParameter));
        if (!hasModeledContentRead)
        {
            reasons.Add(TestMapUnknownReason.Other);
            return true;
        }
        ...
        string boundedParameter)
    {
        if (invocation.Expression is not MemberAccessExpressionSyntax
            ...
            || (owner.Identifier.ValueText, method.Identifier.ValueText)
                is not (("File", "ReadLines") or ("XDocument", "Load")))
        {
            return false;
        }
        ...
        return invocation.ArgumentList.Arguments.FirstOrDefault()?.Expression
            ...
            && file.Identifier.ValueText == boundedParameter;
    }
